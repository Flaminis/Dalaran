...
/// Describes a single store found in an DLR file.
#[pyclass(
    frozen,
    from_py_object,
...
/// Internal DLR reader binding.
///
/// Opens an DLR file. Store discovery is lazy: `store_entries()` scans the file (footer or
...
impl PyRrdReaderInternal {
    ...
    fn new(py: Python<'_>, path: &str) -> PyResult<Self> {
        ...
        // legacy DLR that has no manifest. Without one, store enumeration falls
        ...
        if let Ok(file) = std::fs::File::open(&path)
            && matches!(
                wait_for_future(py, dl_log_encoding::read_dlr_footer(&file)),
                Ok(None)
            )
        {
            crate::utils::py_dalaran_warn(&format!(
                "DLR file has no footer/manifest: {}. \
    ...
    /// List all store entries in this DLR file.
    ///
    ...
    ///
    /// If `store` is `None`, streams the first recording store found in this DLR. Errors
    ...
    fn store(
        ...
        wait_for_future(py, async move {
            ...
            match dl_log_encoding::read_dlr_footer(&reader).await {
                Ok(Some(dlr_footer)) => {
                    let raw = pick_manifest(&dlr_footer, &path, &target_store_id)?;
                    let provider = Arc::new(
                        ...
                        .map_err(|err| ChunkPipelineError::RrdRead {
                            path: path_buf.clone(),
                            reason: format!("Invalid DLR manifest: {err}"),
                        })?,
                    );
...
impl PyRrdReaderInternal {
    ...
    fn ensure_cached_stores(&self, py: Python<'_>) -> PyResult<Vec<StoreId>> {
        ...
        if cache.is_none() {
            *cache = Some(enumerate_dlr_stores(py, &self.path).map_err(PyErr::from)?);
        }
    ...
    ///
    /// If `store` is `Some`, returns its id after validating it belongs to this DLR.
    ...
    fn resolve_target(
        ...
        let first = recordings
            .first()
            .cloned()
            .ok_or_else(|| PyValueError::new_err("No recording store found in DLR file"))?;
        if recordings.len() > 1 {
            crate::utils::py_dalaran_warn(&format!(
                "DLR contains {} recording stores; implicitly using {:?}. \
...
/// Factory for creating DLR chunk streams targeting a specific store.
...
/// Chunk stream that lazily decodes an DLR file, yielding chunks from a single target store.
///
...
fn enumerate_dlr_stores(py: Python<'_>, path: &Path) -> Result<Vec<StoreId>, ChunkPipelineError> {
    let reader = std::fs::File::open(path).map_err(|err| ChunkPipelineError::RrdRead {
        path: path.to_path_buf(),
        reason: err.to_string(),
    })?;
    wait_for_future(py, dl_log_encoding::enumerate_dlr_stores(&reader)).map_err(|err| {
...
/// Look up `target`'s manifest in an DLR footer.
fn pick_manifest(
    dlr_footer: &dl_log_encoding::RrdFooter,
    path: &Path,
    target: &StoreId,
    ...
    let raw_manifest =
        dlr_footer
            ...
            .ok_or_else(|| ChunkPipelineError::RrdRead {
                path: path.to_path_buf(),
                reason: format!("Store {target:?} not found in DLR footer"),
            })?;
