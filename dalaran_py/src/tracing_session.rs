...
//!
//! See `dalaran_py/dalaran_sdk/dalaran/_tracing_session.py` for the user-facing API.
//!
//! The bridge has two pyo3 entry points:
//!
//! - [`get_tracing_session_var`] — exposes the Python `ContextVar` whose current value is
//!   read by the Rust-side [`dl_perf_telemetry::current_dalaran_session_id`] on every
//!   outbound gRPC request. The `TraceStateEnricher` uses it to merge
//!   `dalaran_session_id=<id>` into the `tracestate` header.
//!
...
//!
//! [`dl_perf_telemetry::current_dalaran_session_id`]: https://docs.rs/dl_perf_telemetry
...
pub fn is_telemetry_active() -> bool {
    cfg_select! {
        feature = "perf_telemetry" => {
            dl_perf_telemetry::is_telemetry_active()
        }
        _ => {
            false
        }
...
pub fn get_tracing_session_var(py: Python<'_>) -> PyResult<Py<PyAny>> {
    cfg_select! {
        feature = "perf_telemetry" => {
            let context_var = get_dalaran_session_var(py)?;
...
// `dl_perf_telemetry` invokes that closure to get an
// `Option<DalaranTracingSessionId>` without knowing how it was sourced.

/// Name of the Python `ContextVar` carrying the active `tracing_session()` id.
#[cfg(feature = "perf_telemetry")]
const DALARAN_SESSION_VAR_NAME: &str = "DALARAN_SESSION_ID";

/// Get the dalaran session id `ContextVar` object.
///
/// Set by the Python `tracing_session()` context manager. The Rust-side
/// [`dl_perf_telemetry::current_dalaran_session_id`] helper reads it on every
/// outbound RPC to enrich the W3C `tracestate` with `dalaran_session_id=<id>`.
#[cfg(feature = "perf_telemetry")]
fn get_dalaran_session_var(py: Python<'_>) -> PyResult<pyo3::Bound<'_, PyAny>> {
    use pyo3::prelude::*;
    ...
    let var = contextvar_class.call((DALARAN_SESSION_VAR_NAME,), Some(&kwargs))?;
    *guard = Some(var.clone().unbind());

    Ok(var)
}

/// Read the current dalaran session id from the Python `ContextVar`.
///
/// Returns `None` when no `tracing_session()` is active, the value is unset, or
/// the value fails [`dl_perf_telemetry::DalaranTracingSessionId::parse`].
#[cfg(feature = "perf_telemetry")]
pub(crate) fn current_dalaran_session_id_from_contextvar(
    py: Python<'_>,
) -> Option<dl_perf_telemetry::DalaranTracingSessionId> {
    use pyo3::prelude::*;

    let var = get_dalaran_session_var(py).ok()?;
    let value = var.call_method0("get").ok()?;
    if value.is_none() {
        return None;
    }
    let raw = value.extract::<String>().ok()?;
    dl_perf_telemetry::DalaranTracingSessionId::parse(&raw)
}
...
pub fn inc_active_tracing_sessions() {
    #[cfg(feature = "perf_telemetry")]
    {
        dl_perf_telemetry::inc_active_tracing_session_count();
    }
}
...
pub fn dec_active_tracing_sessions() {
    #[cfg(feature = "perf_telemetry")]
    {
        dl_perf_telemetry::dec_active_tracing_session_count();
    }
}
...
pub fn log_tracing_session_started(dalaran_session_id: &str) {
    tracing::info!("dalaran tracing session started: {dalaran_session_id}");
}
...
#[pyo3(signature = (dalaran_session_id, elapsed_s, cpu_user_s, cpu_system_s, cpu_iowait_s, net_rx_mb))]
