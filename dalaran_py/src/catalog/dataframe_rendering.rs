...
#[pyclass(
    eq,
    from_py_object,
    name = "DalaranHtmlTable",
    module = "dalaran_bindings.dalaran_bindings"
)]
#[derive(Clone, PartialEq, Eq)]
pub struct PyDalaranHtmlTable {
...
impl PyDalaranHtmlTable {
    fn build_table_container_start(&self) -> String {
        ...
        format!(
            ...
            <style>
            .dalaran-table.dalaran-table table {{
                border-collapse: collapse;
                min-width: 100%;
                text-align: left;
            }}

            .dalaran-table.dalaran-table th {{
                font-weight: normal;
                text-align: left;
            }}

            .dalaran-table.dalaran-table td {{
                text-align: left;
            }}
            </style>
            <table class="dalaran-table">
            "#
        )
    }
        ...
        let cells = schema
            ...
            .map(|field| {
                format!(
                    ...
                    field.name(),
                    dl_arrow_util::format_field_datatype(field)
                )
            })
            .collect::<Vec<String>>();
