...
pub fn py_dalaran_warn_cstr(msg: &std::ffi::CStr) -> PyResult<()> {
    Python::attach(|py| {
        let warning_type = PyModule::import(py, "dalaran")?
            .getattr("error_utils")?
            .getattr("DalaranWarning")?;
        PyErr::warn(py, &warning_type, msg, 0)?;
        Ok(())
    })
}

/// Logs a warning using dalaran logging system and issues the warning to python runtime.
pub fn py_dalaran_warn(msg: &str) -> PyResult<()> {
    let cmsg = CString::new(msg)?;
    py_dalaran_warn_cstr(&cmsg)
}
