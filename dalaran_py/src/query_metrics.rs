//! PyO3 bridge for `dalaran.experimental.query_metrics()`.
//!
//! See [`dalaran_py/dalaran_sdk/dalaran/experimental/_query_metrics.py`] for the
...
/// `dalaran.experimental._query_metrics` and return the underlying Rust
/// collectors.
///
...
pub fn active_metrics_collectors(py: Python<'_>) -> Vec<MetricsCollector> {
    let read = || -> PyResult<Vec<MetricsCollector>> {
        let module = py.import("dalaran.experimental._query_metrics")?;
