...
A high-level overview of writing and previewing the Dalaran Python documentation.

## How docs coverage works

The set of symbols documented at `ref.dalaran.dev/docs/python/` is derived
...
- **Define in-file in a single-file module:** include `"Foo"` in the module's
  `__all__` (e.g., see `dalaran_sdk/dalaran/urdf.py`, `dalaran_sdk/dalaran/server.py`).
- **Stand up a Track A page for a brand-new subpackage:** add a row to
  ...
  to its nav title (e.g., `"dalaran.foo": ("Foo",)` for a top-level entry, or
  `"dalaran.bar.baz": ("Bar", "Baz")` to nest it under "Bar"). The first build
...
- A new top-level subpackage or module under `dalaran_sdk/dalaran/` — or a new
...
This will watch the contents of the `dalaran_py` folder and refresh documentation live as files are changed.
...
The documentation exists as bucket on GCS which is hosted on the <https://ref.dalaran.dev> domain.
...
[`docs/python/main`](https://ref.dalaran.dev/docs/python/main).

Releases will push to a version instead: [`docs/python/0.23.3`](https://ref.dalaran.dev/docs/python/0.23.3)
