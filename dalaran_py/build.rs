fn main() {
    ...
    dl_build_tools::export_build_info_vars_for_crate("dalaran_py");
    ...
    if dl_build_tools::is_tracked_env_var_set("DALARAN_BUILDING_WHEEL")
    ...
    // Fail if bin/dalaran is missing and we haven't specified it's ok.
    if dl_build_tools::is_tracked_env_var_set("DALARAN_BUILDING_WHEEL")
        && !dl_build_tools::is_tracked_env_var_set("DALARAN_ALLOW_MISSING_BIN")
    {
        #[cfg(target_os = "windows")]
        #[expect(clippy::unwrap_used)]
        let dalaran_bin = std::env::current_dir()
            .unwrap()
            .join("dalaran_sdk/dalaran_cli/dalaran.exe");

        #[cfg(target_os = "macos")]
        let dalaran_bin = {
            let base = std::env::current_dir().expect("std::env::current_dir() failed");
            let bundled = base.join("dalaran_sdk/dalaran_cli/Dalaran.app/Contents/MacOS/Dalaran");
            if bundled.exists() {
                bundled
            } else {
                base.join("dalaran_sdk/dalaran_cli/dalaran")
            }
        };
        ...
        let dalaran_bin = std::env::current_dir()
            ...
            .join("dalaran_sdk/dalaran_cli/dalaran");

        if !dalaran_bin.exists() {
            eprintln!("ERROR: Expected to find `dalaran` at `{dalaran_bin:?}`.");
            std::process::exit(1);
        }
    }
}
