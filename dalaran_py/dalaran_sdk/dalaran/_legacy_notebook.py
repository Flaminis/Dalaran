...
def as_html(
    *,
    ...
    app_url:
        Alternative HTTP url to find the Dalaran web viewer. This will default to using `https://app.dalaran.dev`
        or localhost if [dalaran.start_web_viewer_server][] has been called.
    timeout_ms:
        The number of milliseconds to wait for the Dalaran web viewer to load.
    blueprint:
    ...
    if application_id is None:
        raise ValueError(
            "No application id found. You must call dalaran.init before using the notebook APIs, or provide a recording.",
        )
