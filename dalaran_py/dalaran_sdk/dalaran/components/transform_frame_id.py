...
class TransformFrameId(datatypes.Utf8, ComponentMixin):
    ...
    **Component**: A string identifier for a transform frame.
    ...
    entity path driven hierarchy which is defined via [`archetypes.Transform3D`][dalaran.archetypes.Transform3D], [`archetypes.Pinhole`][dalaran.archetypes.Pinhole] etc..
    These implicit transform frames look like `tf#path/to/entity`.

    Note that any [`archetypes.Transform3D`][dalaran.archetypes.Transform3D]s logged with both `parent_frame` and `child_frame` set
    describes a relationship between these parent and child transform frames, **not** the transform frame
    that the entity path may be using (defined by an [`archetypes.CoordinateFrame`][dalaran.archetypes.CoordinateFrame]).
    """
...
class TransformFrameIdBatch(datatypes.Utf8Batch, ComponentBatchMixin):
    _COMPONENT_TYPE: str = "dalaran.components.TransformFrameId"
