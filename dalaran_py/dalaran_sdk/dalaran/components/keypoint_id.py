...
class KeypointId(datatypes.KeypointId, ComponentMixin):
    ...
    **Component**: A 16-bit ID representing a type of semantic keypoint within a class.
    ...
    `KeypointId`s are only meaningful within the context of a [`dalaran.datatypes.ClassDescription`].

    Used to look up an [`dalaran.datatypes.AnnotationInfo`] for a Keypoint within the
    [`dalaran.components.AnnotationContext`].
    """
...
class KeypointIdBatch(datatypes.KeypointIdBatch, ComponentBatchMixin):
    _COMPONENT_TYPE: str = "dalaran.components.KeypointId"
