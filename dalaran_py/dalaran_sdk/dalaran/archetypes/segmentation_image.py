...
class SegmentationImage(SegmentationImageExt, Archetype, VisualizableArchetype):
    """
    **Archetype**: An image made up of integer [`components.ClassId`][dalaran.components.ClassId]s.

    Each pixel corresponds to a [`components.ClassId`][dalaran.components.ClassId] that will be mapped to a color based on [`archetypes.AnnotationContext`][dalaran.archetypes.AnnotationContext].

    In the case of floating point images, the label will be looked up based on rounding to the nearest
    integer value.

    Use [`archetypes.AnnotationContext`][dalaran.archetypes.AnnotationContext] to associate each class with a color and a label.
    ...
    NAME: ClassVar[str] = "dalaran.archetypes.SegmentationImage"
