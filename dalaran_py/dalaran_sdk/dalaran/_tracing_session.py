...
Use [`tracing_session`][dalaran._tracing_session.tracing_session] when you want to
attach a single, copy-pasteable identifier to a block of catalog calls so support
can look them up in distributed tracing.

```python
from dalaran import tracing_session

with tracing_session():
    dataset.scan(...).read_all()
# → INFO message printed at scope entry: "dalaran tracing session started: rs_8f3a91e2"
```
...
`dalaran_session_id=<id>` into the W3C `tracestate` header. The catalog server
records it as a span attribute, queryable in Tempo as
`{ .dalaran_session_id = "…" }`.

This module is private — public re-export lives in `dalaran.__init__`.
"""
...
logger = logging.getLogger("dalaran")
...
def _is_valid_session_id(sid: str) -> bool:
    """Mirror of `dl_perf_telemetry::is_valid_dalaran_session_id`."""
...
def tracing_session() -> Iterator[str]:
    ...
    The id is logged to the `dalaran` logger at INFO level the moment the scope is
    ...
    `{ .dalaran_session_id = "<id>" }` in Tempo to surface every related request.
    ...
    When the dalaran telemetry stack is not active (typically because
    `TELEMETRY_ENABLED=true` was not set before importing dalaran), the context
    ...
    client = dl.catalog.CatalogClient("dalaran://…")
    with tracing_session():
        ds = client.get_dataset(name="…")
        _ = ds.scan().read_all()
    # The session id appears in the logs at scope entry:
    #   INFO dalaran: dalaran tracing session started: rs_8f3a91e2
    ```
    ...
    if var is None:
        logger.warning(
            "tracing_session() is a no-op: the dalaran telemetry stack is not active. "
            "Set the environment variable TELEMETRY_ENABLED=true before importing "
            "dalaran to enable session correlation.",
        )
    ...
    try:
        ...
        try:
            ...
            # `RUST_LOG` and fmt-layer pipeline as every other dalaran log,
            # rather than the Python `logging` pipeline (which has no default
            # handler attached to the `dalaran` logger and would silently drop
