...
def default_strict_mode() -> bool:
    if "DALARAN_STRICT" in os.environ:
        var = os.environ["DALARAN_STRICT"].lower()
        if var in ("0", "false", "off", "no"):
            return False
        elif var in ("1", "true", "on", "yes"):
            return True
        else:
            print(f"Expected DALARAN_STRICT to be one of 0/1 false/true off/on no/yes, found {var}")
            return _strict_mode
    else:
        return False
...
_dalaran_exception_ctx = threading.local()
...
def strict_mode() -> bool:
    ...
    In strict mode, incorrect use of the Dalaran API (wrong parameter types etc.)
    will result in exception being raised.
    When strict mode is on, such problems are instead logged as warnings.

    The default is controlled with the `DALARAN_STRICT` environment variable,
    or `False` if it is not set.
    """
    # If strict was set explicitly, we are in struct mode
    if getattr(_dalaran_exception_ctx, "strict_mode", None) is not None:
        return _dalaran_exception_ctx.strict_mode  # type: ignore[no-any-return]
    else:
        return _strict_mode
...
class DalaranWarning(Warning):
    """A custom warning class that we use to identify warnings that are emitted by the Dalaran SDK itself."""
...
def _send_warning_or_raise(
    ...
    *,
    ...
    warning_type: type[Warning] = DalaranWarning,
) -> None:
    """
    Sends a warning about the usage of the Dalaran SDK.

    Note: in strict mode this will instead raise the specified exception type
    (defaults to ValueError).

    This will both send a message to the Dalaran viewer and log a warning using
    `warning.warn` with a custom `DalaranWarning` class.
    ...
    """
    from dalaran._log import log
    from dalaran.archetypes import TextLog
    ...
    # Logging the warning to Dalaran is a complex operation could produce another warning. Avoid recursion.
    if not getattr(_dalaran_exception_ctx, "sending_warning", False):
        _dalaran_exception_ctx.sending_warning = True
        ...
        _dalaran_exception_ctx.sending_warning = False
    else:
        warnings.warn(
...
class catch_and_log_exceptions:
    ...
    points to the user call-site rather than somewhere buried in Dalaran code.
    ...
    def __enter__(self) -> Self:
        ...
        self.original_strict = getattr(_dalaran_exception_ctx, "strict_mode", None)
        if self.strict is not None:
            _dalaran_exception_ctx.strict_mode = self.strict
        if getattr(_dalaran_exception_ctx, "depth", None) is None:
            _dalaran_exception_ctx.depth = 1
        else:
            _dalaran_exception_ctx.depth += 1
    ...
    def __call__(self, func: _TFunc) -> _TFunc:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            with self:
                if "strict" in kwargs:
                    _dalaran_exception_ctx.strict_mode = kwargs["strict"]
    ...
    def __exit__(
        ...
        try:
            ...
            if exc_type is not None and issubclass(exc_type, Exception) and not strict_mode():
                if getattr(_dalaran_exception_ctx, "pending_warnings", None) is None:
                    _dalaran_exception_ctx.pending_warnings = []

                context = f"{self.context}: " if self.context is not None else ""

                warning_message = f"{context}{exc_type.__name__}({exc_val})"

                _dalaran_exception_ctx.pending_warnings.append(warning_message)
                return True
            else:
                return False
        finally:
            if getattr(_dalaran_exception_ctx, "depth", None) is not None:
                _dalaran_exception_ctx.depth -= 1
                if _dalaran_exception_ctx.depth == 0:
                    pending_warnings = getattr(_dalaran_exception_ctx, "pending_warnings", [])
                    _dalaran_exception_ctx.pending_warnings = []
                    _dalaran_exception_ctx.depth = None
            ...
            _dalaran_exception_ctx.strict_mode = self.original_strict
...
class DalaranMissingDependencyError(ImportError):
    """Raised when an optional dependency is not installed."""

    def __init__(self, package: str, optional_dep: str) -> None:
        super().__init__(
            f"'{package}' could not be imported. "
            f"Please install it, or install dalaran as dalaran-sdk[{optional_dep}]/dalaran-sdk[all] "
            "to use this functionality."
        )


class DalaranIncompatibleDependencyVersionError(ImportError):
    ...
    def __init__(self, package: str, actual_version: str, compatible_versions: list[int]) -> None:
        super().__init__(
            f"'{package}' version {actual_version} is incompatible with dalaran. "
            f"Please install dalaran as dalaran-sdk[{package}]/dalaran-sdk[all] "
