...
def segment_url(
    ...
    *,
    ...
    This is a Dalaran focused DataFusion function that will create a DataFusion
    expression for the segment URL.

    Parameters
    ----------
    dataset:
        The input Dalaran Dataset.
    segment_id:
        Expression or column name for the segment ID. If not provided, the column named `dalaran_segment_id` will be used.
    timestamp:
    ...
    if segment_id is None:
        segment_id = col("dalaran_segment_id")
...
def _make_rust_udf() -> ScalarUDF:
    ...
    from dalaran_bindings import SegmentUrlUdfInternal  # type: ignore[attr-defined]
