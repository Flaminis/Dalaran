...
`query_metrics()` is part of `dalaran.experimental` — once the upstream
...
# `dalaran_py/src/catalog/dataset_view.rs::reader()` to bind metrics capture to
...
class MetricsCollector:
    """
    Accumulator yielded by [`query_metrics`][dalaran.experimental.query_metrics] on `__enter__`.
...
def query_metrics() -> Iterator[MetricsCollector]:
    ...
    Yields a [`MetricsCollector`][dalaran.experimental.MetricsCollector]; read `.last_query()` or
    ...
    """
    try:
        from dalaran_bindings import _new_metrics_collector
    except ImportError:
        ...
        # runs.
        logger.warning(
            "dalaran.experimental.query_metrics() is a no-op: the catalog "
            "bindings are not available in this build of dalaran.",
        )
