...
from dalaran_bindings import Mp4ReaderInternal, Mp4TranscodeOptionsInternal
...
class Mp4TranscodeOptions:
    ...
    """
    Re-encode to this [`VideoCodec`][dalaran.components.VideoCodec] instead of keeping
    ...
    def __post_init__(self) -> None:
        if self.output_codec is not None and not isinstance(self.output_codec, VideoCodec):
            raise TypeError(
                f"output_codec must be a dalaran.components.VideoCodec, got {type(self.output_codec).__name__}"
            )
        ...
        *,
        ...
        mode:
            ...
            - `"stream"` (default): emit a static `VideoStream(codec=…)` chunk
              ...
              [`VideoCodec`][dalaran.components.VideoCodec].
        ...
        chunk_by_gop:
            ...
            emitted Dalaran chunk contains a keyframe plus all dependent samples
            up to (but not including) the next keyframe. When `False`, each
            sample becomes its own one-row Dalaran chunk.
        ...
        transcode:
            ...
            [`Mp4TranscodeOptions`][dalaran.experimental.Mp4TranscodeOptions]
