...
if TYPE_CHECKING:
    ...
    from dalaran_bindings import ViewerClientInternal


_DEFAULT_URL = "dalaran+http://127.0.0.1:9876/proxy"


class ViewerClient:
    """
    A connection to an instance of a Dalaran viewer.

    Use the [`connect`][dalaran.experimental.ViewerClient.connect] classmethod
    to attach to an already-running viewer, or
    [`spawn`][dalaran.experimental.ViewerClient.spawn] to start a fresh one
    (e.g. in headless mode for CI screenshots).

    Spawned-viewer teardown:

    - Explicit [`close`][dalaran.experimental.ViewerClient.close] always
    ...
    def __init__(
        ...
        *,
        ...
        Prefer
        [`ViewerClient.connect`][dalaran.experimental.ViewerClient.connect] or
        [`ViewerClient.spawn`][dalaran.experimental.ViewerClient.spawn].

        Parameters
        ----------
        url:
            The URL to connect to. The scheme must be one of `dalaran://`,
            `dalaran+http://`, or `dalaran+https://`, and the pathname must be
            `/proxy` — the same form accepted by [`dalaran.connect_grpc`][].
            Defaults to `dalaran+http://127.0.0.1:9876/proxy`.
        _pid:
    ...
    def spawn(
        ...
        *,
        ...
        headless:
            ...
            [`save_screenshot`][dalaran.experimental.ViewerClient.save_screenshot].
        ...
        detach_process:
            ...
            [`close`][dalaran.experimental.ViewerClient.close] explicitly.
        ...
        """
        from dalaran._spawn import _spawn_viewer
    ...
    @property
    def url(self) -> str:
        """The `dalaran+http://…/proxy` URL of the viewer this client is connected to."""
        return self._url
    ...
    def close(self) -> None:
        ...
        try:
            # The python `dalaran` command is a shim (see `dalaran_cli/__main__.py`) that spawns the
