...
if TYPE_CHECKING:
    ...
    from dalaran import ComponentColumn
    from dalaran._baseclasses import AsComponents, ComponentDescriptor, DescribedComponentBatch
    from dalaran._send_columns import TimeColumnLike
    from dalaran_bindings import ChunkInternal
...
class Chunk:
    ...
    def from_record_batch(
        ...
        *,
        ...
        """
        Interpret an Arrow [`RecordBatch`][pyarrow.RecordBatch] as Dalaran chunk data.
        ...
        The `dalaran:*` arrow metadata, if it exists, drives the kind of each input column,
        ...
        a fully identified chunk, e.g. as produced by [`Chunk.to_record_batch`][dalaran.experimental.Chunk.to_record_batch].
        ...
        index:
            ...
            - Omitted (the default): derive the index columns from the batch's Dalaran metadata.
        ...
        entity_path:
            ...
            Resolution order per component column is: its `dalaran:entity_path` metadata, then the
            batch-level `dalaran:entity_path` metadata, then the column-name convention (see
        ...
        Notes
        -----
        **Column-name convention.** When a component column carries no `dalaran:entity_path` /
        `dalaran:component` metadata, its entity path is read from the column name: if the name starts
    ...
    def from_dataframe(
        ...
        *,
        ...
        [`Chunk.from_record_batch`][dalaran.experimental.Chunk.from_record_batch] applied to every
        ...
        ValueError
            See [`Chunk.from_record_batch`][dalaran.experimental.Chunk.from_record_batch].
    ...
    def from_columns(
        ...
        """
        Create a Chunk from columns, mirroring the [`dalaran.send_columns`][] API.
        ...
        chunk = Chunk.from_columns(
            ...
            columns=dl.Points3D.columns(positions=[[1, 2, 3], [4, 5, 6], [7, 8, 9]]),
        )
        ```

        """
        from dalaran._send_columns import build_column_args
        from dalaran_bindings import ChunkInternal
    ...
    def from_property(
        ...
        """
        Create a Chunk from a property, mirroring the [`dalaran.send_property`][] API.
        ...
        chunk = Chunk.from_property(
            "my_property",
            values=dl.Points3D.positions([[1, 2, 3], [4, 5, 6], [7, 8, 9]]),
        )
        ```

        """
        from dalaran._properties import build_property_args
        from dalaran_bindings import ChunkInternal
    ...
    def apply_selector(
        ...
        For better performance, prefer [`MutateLens`][dalaran.experimental.MutateLens]
        with [`apply_lenses`][dalaran.experimental.Chunk.apply_lenses]
        ...
        selector:
            A [`Selector`][dalaran.experimental.Selector] or selector query string to apply to the component.

        Returns
        -------
        A new [`Chunk`][dalaran.experimental.Chunk] with the component transformed.
        ...
        """
        from dalaran._baseclasses import ComponentDescriptor as CD
    ...
    def apply_lenses(
        ...
        ----------
        lenses:
            One or more [`Lens`][dalaran.experimental.Lens] objects.
    ...
    def format(self, *, width: int = 240, redact: bool = False, trim_metadata_keys: bool = True) -> str:
        ...
        trim_metadata_keys:
            If True, trim the `dalaran:` / `sorbet:` prefix from metadata keys.
            Default: True.
