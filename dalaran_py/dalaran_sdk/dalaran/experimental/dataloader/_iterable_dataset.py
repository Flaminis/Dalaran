...
from dalaran._tracing import set_current_span_attributes, tracing_scope
...
def _count_yields(
    ...
    try:
        for sample in samples:
            set_current_span_attributes({"dalaran.dataloader.iter.num_samples_yielded": count})
            yield sample
            count += 1
    finally:
        samples.close()
...
class DalaranIterableDataset(torch.utils.data.IterableDataset[dict[str, torch.Tensor | None]]):
    """
    ...
    timeline_sampling
        ...
        integer indices. Pass [`FixedRateSampling`][dalaran.experimental.dataloader.FixedRateSampling]
    ...
    shuffle_strategy
        The [`ShuffleStrategy`][dalaran.experimental.dataloader.ShuffleStrategy]
        that determines the order samples are fetched in, and — for
        [`BlockShuffle`][dalaran.experimental.dataloader.BlockShuffle] — the
        optional post-decode buffer samples are emitted through. Defaults to
        [`SampleShuffle`][dalaran.experimental.dataloader.SampleShuffle]; pass
        [`NoShuffle`][dalaran.experimental.dataloader.NoShuffle] for natural order.
    decode_threads
    ...
    def from_manifest(
        ...
        *,
        ...
        """
        Build a dataset that replays a frozen [`Manifest`][dalaran.experimental.dataloader.Manifest]'s sampling order.
    ...
    def sample_index(self) -> SampleIndex:
        """The underlying [`SampleIndex`][dalaran.experimental.dataloader.SampleIndex]."""
    ...
    def _iter_catalog(self) -> Iterator[dict[str, torch.Tensor | None]]:
        ...
        """
        with (
            tracing_scope("DalaranIterableDataset._iter_catalog"),
            ...
            set_current_span_attributes({
                "dalaran.dataloader.iter.num_chunks": len(chunks),
                "dalaran.dataloader.shuffle_strategy": self._shuffle_strategy.RECIPE_TAG,
            })
    ...
    def _iter_manifest(self) -> Iterator[dict[str, torch.Tensor | None]]:
        ...
        with (
            tracing_scope("DalaranIterableDataset._iter_manifest"),
            _decode_pool(self._decode_threads, len(self._fields)) as executor,
        ):
            view, decoders = self._connection.ensure()
            ...
            if not chunks:
                set_current_span_attributes({
                    "dalaran.dataloader.iter.num_samples_yielded": 0,
                    "dalaran.dataloader.iter.num_chunks": len(chunks),
                })
                return
            ...
            set_current_span_attributes({
                "dalaran.dataloader.iter.num_chunks": len(chunks),
                "dalaran.dataloader.shuffle_strategy": self._manifest.metadata.shuffle_strategy,
            })
