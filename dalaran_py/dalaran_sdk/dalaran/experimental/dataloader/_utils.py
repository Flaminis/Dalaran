"""Shared helpers used by both the iterable and map-style Dalaran datasets."""
...
if TYPE_CHECKING:
    ...
    from dalaran.catalog._entry import DatasetEntry
    from dalaran.experimental._selector import Selector
...
def _warn_if_fork_unsafe(stacklevel: int) -> None:
    ...
    Dalaran's `dalaran_bindings` extension uses a process-global tokio runtime.
    ...
    if not will_be_fork:
        return
    warnings.warn(
        "The default multiprocessing start method is 'fork'. The Dalaran "
...
class _WorkerConnection:
    ...
    def from_source(cls, source: DataSource, fields: dict[str, Field]) -> _WorkerConnection:
        """Build a connection for a [`DataSource`][dalaran.experimental.dataloader.DataSource]'s catalog."""
        return cls(catalog_url=source.dataset.catalog.url, dataset_name=source.dataset.name, fields=fields)

    @with_tracing("DalaranDataset._ensure_initialized")
...
@with_tracing("DalaranDataset._fetch_arrow")
def _fetch_arrow(
    ...
    *,
    ...
    set_current_span_attributes({
        "dalaran.dataloader.fetch.num_requested_indices": len(indices),
        "dalaran.dataloader.fetch.num_located_targets": len(located),
        "dalaran.dataloader.fetch.num_fields": len(fields),
        "dalaran.dataloader.fetch.num_segments": len({seg.segment_id for seg, _ in located}),
        "dalaran.dataloader.fetch.index_values_bytes_estimate": len(indices) * 8,
    })
...
def _interleave_fetch_and_decode(
    ...
    *,
    ...
    executor = ThreadPoolExecutor(max_workers=1, thread_name_prefix="dalaran-fetch")
...
def _run_parallel(tasks: list[Callable[[], Any]], *, max_workers: int | None = None) -> list[Any]:
    ...
    with ThreadPoolExecutor(max_workers=workers, thread_name_prefix="dalaran-parallel") as executor:
...
def _fetch_group(
    *,
    ...
    with tracing_scope(f"DalaranDataset._fetch_group[{group}]"):
        ...
        set_current_span_attributes({
            "dalaran.dataloader.group.num_fields": len(fields),
            "dalaran.dataloader.group.num_segments": len(query_indices),
            "dalaran.dataloader.group.num_query_indices": num_query_indices,
            "dalaran.dataloader.group.fill_latest_at": fill_latest_at,
            "dalaran.dataloader.group.anchored": anchored,
        })
        ...
        # `index` and `dalaran_segment_id` are preserved because `_decode_iter` and `_split_by_segment` read them.
        select_exprs = [col(index), col("dalaran_segment_id")]
        select_exprs.extend(col(field.path).alias(key) for key, field in fields.items())

        with tracing_scope(f"DalaranDataset._fetch_group.to_arrow_table[{group}]"):
            arrow_table = df.select(*select_exprs).to_arrow_table()
            set_current_span_attributes({
                "dalaran.dataloader.group.arrow_rows": arrow_table.num_rows,
                "dalaran.dataloader.group.arrow_columns": arrow_table.num_columns,
                "dalaran.dataloader.group.arrow_nbytes": arrow_table.nbytes,
            })
...
def _decode_pool(decode_threads: int, num_fields: int) -> Generator[ThreadPoolExecutor | None, None, None]:
    ...
    if workers <= 1:
        yield None
        return
    executor = ThreadPoolExecutor(max_workers=workers, thread_name_prefix="dalaran-decode")
    try:
        yield executor
    finally:
        executor.shutdown(wait=False)
...
@with_tracing("DalaranDataset._decode_field")
def _decode_field(
    *,
    target: Target,
    key: str,
...
def _decode_iter(
    *,
    ...
    """
    with tracing_scope("DalaranDataset._decode_chunk"):
        set_current_span_attributes({"dalaran.dataloader.iter.chunk.num_targets": len(targets)})
        for target in targets:
            with tracing_scope("DalaranDataset._decode_sample"):
...
@with_tracing("DalaranDataset._fetch_prior_keyframes")
def _fetch_prior_keyframes(
    *,
    ...
    [`_prior_keyframe`][dalaran.experimental.dataloader._utils._prior_keyframe].
    """
    ...
    with tracing_scope("DalaranDataset._fetch_prior_keyframes.to_arrow_table"):
        table = (
            view
            ...
            .select(col(index), col("dalaran_segment_id"), *[col(p) for p in unique_paths])
            .to_arrow_table()
        )
    ...
    for path in unique_paths:
        ...
        sub_segs = sub.column("dalaran_segment_id").to_pylist()
...
def _split_by_segment(table: pa.Table) -> dict[str, pa.Table]:
    ...
    seg_col = table.column("dalaran_segment_id")
