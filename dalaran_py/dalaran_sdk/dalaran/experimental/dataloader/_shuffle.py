...
class ShuffleStrategy(ABC):
    ...
    See [the training guide](https://dalaran.dev/docs/howto/train/dataloader) for the trade-offs.
    """
    ...
    def emission_buffer(self) -> ShuffleBuffer | None:
        ...
        Only [`BlockShuffle`][dalaran.experimental.dataloader.BlockShuffle] defines
        ...
        [`SampleShuffle`][dalaran.experimental.dataloader.SampleShuffle] already
        fetches a uniform permutation, so a buffer would add nothing, and
        [`NoShuffle`][dalaran.experimental.dataloader.NoShuffle] is a deterministic
...
class SampleShuffle(ShuffleStrategy):
    ...
    [`BlockShuffle`][dalaran.experimental.dataloader.BlockShuffle] when fetch
...
class BlockShuffle(ShuffleStrategy):
    ...
    buffer_size
        ...
        [`emission_buffer`][dalaran.experimental.dataloader.ShuffleStrategy.emission_buffer].
...
class ShuffleBuffer:
    ...
    def emit_order(self, n: int, *, rng: np.random.Generator) -> np.ndarray:
        ...
        Rolls the very same buffer [`shuffle`][dalaran.experimental.dataloader.ShuffleBuffer.shuffle]
