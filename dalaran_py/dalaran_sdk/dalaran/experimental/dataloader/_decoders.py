...
from dalaran._tracing import set_current_span_attributes, with_tracing
...
def _to_video_codec(codec: str) -> VideoCodec | None:
    """
    Map a codec string to [`VideoCodec`][dalaran.components.VideoCodec].

    Returns `None` for codecs Dalaran doesn't know; every known codec has a
    keyframe detector in `dalaran.experimental.video.detect_gop_start`.
    """
...
class ColumnDecoder(ABC):
    ...
    (images, scalars) only need to implement [`decode`][dalaran.experimental.dataloader.ColumnDecoder.decode].
    Context-aware decoders (compressed video) should also override
    [`context_range`][dalaran.experimental.dataloader.ColumnDecoder.context_range] so the prefetcher fetches surrounding data.
    """
...
class VideoFrameDecoder(ColumnDecoder):
    ...
    def _decode_to_target(
        ...
        set_current_span_attributes({
            "dalaran.dataloader.video.session_cache_hits": self._cache_hits,
            "dalaran.dataloader.video.session_cache_misses": self._cache_misses,
            "dalaran.dataloader.video.session_cache_hit_rate": self._cache_hits / total,
            "dalaran.dataloader.video.session_cache_miss_rate": self._cache_misses / total,
            "dalaran.dataloader.video.window_samples": len(samples),
            "dalaran.dataloader.video.packets_fed": len(samples) - len(session.fed_samples),
        })
        ...
        if session.frames_emitted == len(samples) and session.last_frame is not None:
            ...
            while len(self._sessions) > self._max_decoder_sessions:
                ...
                evicted += 1
            set_current_span_attributes({
                "dalaran.dataloader.video.session_kept": True,
                "dalaran.dataloader.video.sessions_evicted": evicted,
                "dalaran.dataloader.video.live_sessions": len(self._sessions),
            })
        ...
        set_current_span_attributes({
            "dalaran.dataloader.video.session_kept": False,
            "dalaran.dataloader.video.live_sessions": len(self._sessions),
        })
