...
def build_manifest_table(
    ...
    *,
    ...
    [`NoShuffle`][dalaran.experimental.dataloader.NoShuffle]) into per-`(rank, worker)`
    `fetch_group` and `emit_rank` columns. Re-order it later with
    [`schedule_samples`][]. Returns the Arrow table (header in the schema
    metadata); [`Manifest.generate`][dalaran.experimental.dataloader.Manifest.generate] wraps it into a
    manifest, which is persisted with
    [`Manifest.write_parquet`][dalaran.experimental.dataloader.Manifest.write_parquet].

    Parameters
    ----------
    source, index, fields, timeline_sampling
        Describe the sample space, exactly as for
        [`DalaranIterableDataset`][dalaran.experimental.dataloader.DalaranIterableDataset].
    fetch_size
...
def _emit_rank(n: int, buffer: ShuffleBuffer | None, *, seed: int, rank: int, worker: int) -> np.ndarray:
    ...
    `buffer` is the strategy's own [`emission_buffer`][dalaran.experimental.dataloader.ShuffleStrategy.emission_buffer],
