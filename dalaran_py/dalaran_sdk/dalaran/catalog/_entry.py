...
from dalaran._tracing import with_tracing
from dalaran_bindings import (
...
if TYPE_CHECKING:
    ...
    from dalaran.experimental import LazyStore
...
class DatasetEntry(Entry[DatasetEntryInternal]):
    ...
    def unregister_asset(self, segment_id: str) -> None:
        ...
        [`register_asset`][dalaran.catalog.DatasetEntry.register_asset].
        ...
        ----------
        segment_id:
            The segment id of the asset to unregister, as returned by [`register_asset`][dalaran.catalog.DatasetEntry.register_asset].
    ...
    def segment_table(
        ...
        join_key: str = "dalaran_segment_id",
        ...
        join_meta
            ...
            using `reader()`.
        join_key
            The column name to use for joining, defaults to "dalaran_segment_id".
    ...
    def _manifest(self, include_diagnostic_data: bool = False) -> datafusion.DataFrame:
        ...
        if not include_diagnostic_data:
            df = df.filter(col("dalaran_registration_status") == "done").drop("dalaran_registration_status")
    ...
    def register(
        ...
        *,
        ...
        """
        from dalaran.error_utils import _send_warning_or_raise
    ...
    def segment_store(self, segment_id: str) -> LazyStore:
        """
        Open a remote segment as a [`LazyStore`][dalaran.experimental.LazyStore].

        The manifest is fetched immediately; chunk data is loaded on demand
        via [`LazyStore.stream`][dalaran.experimental.LazyStore.stream]. To fully
        materialize into a [`ChunkStore`][dalaran.experimental.ChunkStore], call
        `lazy.stream().collect()`.
        """
        from dalaran.experimental import LazyStore
    ...
    def filter_segments(self, segment_ids: str | Sequence[str] | datafusion.DataFrame) -> DatasetView:
        ...
        segment_ids
            ...
            with a column named 'dalaran_segment_id'. When passing a DataFrame,
        ...
        if isinstance(segment_ids, str):
            ...
            segment_ids = segment_ids.select("dalaran_segment_id").to_pydict()["dalaran_segment_id"]
    ...
    def reader(
        ...
        *,
        ...
        The returned DataFrame supports server side filtering for both `dalaran_segment_id`
        and the index (timeline) column, which can greatly improve performance. For
        example, the following filters will effectively be handled by the Dalaran server.

        ```python
        dataset.reader(index="real_time").filter(col("dalaran_segment_id") == "aabbccddee")
        dataset.reader(index="real_time").filter(col("real_time") == "1234567890")
        dataset.reader(index="real_time").filter(
            (col("dalaran_segment_id") == "aabbccddee") & (col("real_time") == "1234567890")
        )
        ```
        ...
        using_index_values
            ...
            - **dict**: keys are segment IDs, values are per-segment index values
              to sample at.
            - **DataFrame**: must have `dalaran_segment_id` and index columns;
...
class DatasetView:
    ...
    def segment_table(
        ...
        if filtered_segment_ids is not None:
            ...
            segment_table_df = segment_table_df.filter(
                F.in_list(col("dalaran_segment_id"), [literal(seg) for seg in filtered_segment_ids])
            )
    ...
    def _dataframe_to_index_values_dict(
        ...
        if "dalaran_segment_id" not in df.schema().names:
            raise ValueError("using_index_values DataFrame must have a 'dalaran_segment_id' column")
        ...
        table = pa.table(
            df.aggregate(
                ["dalaran_segment_id"], [dfn.functions.array_agg(dfn.col(index), order_by=dfn.col(index)).alias(index)]
            )
        )

        # Group by segment_id
        segment_id_col = table.column("dalaran_segment_id")
