...
from ..error_utils import DalaranIncompatibleDependencyVersionError, DalaranMissingDependencyError
from . import EntryId
...
def _are_datafusion_versions_compatible(v1: int, v2: int) -> bool:
    ...
    In some rare cases, we may need to have a mismatch, e.g. in some deployed Dalaran Hub docker images. So we have a
...
class VersionInfo:
    """Version and deployment information from a Dalaran server."""
...
class CatalogClient:
    """
    Client for a remote Dalaran catalog server.
    ...
    def __init__(self, url: str, *, token: str | None = None, addr: str | None = None) -> None:
        """
        Connect to a remote Dalaran catalog server.
        ...
        if addr is not None:
            ...
            _send_warning_or_raise(
                "The `addr` parameter is deprecated in Dalaran 0.29, and has been renamed to `url`.",
        ...
        if find_spec("datafusion") is None:
            raise DalaranMissingDependencyError("datafusion", "datafusion")
        ...
        if not _are_datafusion_versions_compatible(datafusion_major_version, expected_df_version):
            raise DalaranIncompatibleDependencyVersionError(
    ...
    def create_dataset(self, name: str, *, exist_ok: bool = False) -> DatasetEntry:
        ...
        from dalaran_bindings import AlreadyExistsError
    ...
    def create_table(self, name: str, schema: pa.Schema, url: str | None = None) -> TableEntry:
        ...
        url
            ...
            its configured writable storage. On Dalaran Hub, custom table URLs are currently not supported: the request
