...
class TimeColumnLike(Protocol):
    """Describes interface for objects that can be converted to a column of dalaran time values."""
...
class TimeColumn(TimeColumnLike):
    ...
    Columnar equivalent to [`dalaran.set_time`][].
    """
...
def send_columns(
    ...
    *,
    ...
    Note that this API ignores any stateful time set on the log stream via [`dalaran.set_time`][].
    ...
    indexes:
        ...
        of timestamps. You usually want to use [`dalaran.TimeColumn`][] for this.
    columns:
