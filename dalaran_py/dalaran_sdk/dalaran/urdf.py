...
from dalaran_bindings import _UrdfJointInternal, _UrdfLinkInternal, _UrdfMimicInternal, _UrdfTreeInternal
...
class UrdfJoint:
    ...
    def compute_transform_columns(self, values: Sequence[float], *, clamp: bool = True) -> ComponentColumnList:
        ...
        -------
        ComponentColumnList
            Columnar transform data ready for use with [`dalaran.send_columns`][].
...
class UrdfTree:
    ...
    Use [`log_urdf_to_recording`][dalaran.urdf.UrdfTree.log_urdf_to_recording] to log the full model (geometry + static transforms), then animate
    individual joints by logging [`archetypes.Transform3D`][dalaran.archetypes.Transform3D] computed via
    [`UrdfJoint.compute_transform`][dalaran.urdf.UrdfJoint.compute_transform].
    """
    ...
    def log_urdf_to_recording(self, recording: RecordingStream | None = None) -> None:
        ...
        This can be used as alternative to [`dalaran.log_file_from_path`][] for URDF files,
    ...
    def stream(self, *, include_joint_transforms: bool = True) -> LazyChunkStream:
        ...
        !!! warning
            ...
            `dalaran.experimental.LazyChunkStream` API.
