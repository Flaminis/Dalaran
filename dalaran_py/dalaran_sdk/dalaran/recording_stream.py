...
from dalaran_bindings import ChunkBatcherConfig as ChunkBatcherConfig  # noqa: TC001
...
if TYPE_CHECKING:
    ...
    from dalaran import AsComponents, BlueprintLike, ComponentColumn, DescribedComponentBatch as DescribedComponentBatch
    from dalaran._memory import MemoryRecording
    from dalaran.experimental import Chunk, ChunkStore, LazyChunkStream, LazyStore
    from dalaran.experimental._chunk import DataframeLike
    from dalaran.sinks import LogSinkLike
...
def is_enabled(
    ...
    """
    Is this Dalaran recording enabled.

    If false, all calls to the recording are ignored.

    The default can be set in [`dalaran.init`][], but is otherwise `True`.

    This can be controlled with the environment variable `DALARAN` (e.g. `DALARAN=on` or `DALARAN=off`).
...
class RecordingStream:
    """
    A RecordingStream is used to send data to Dalaran.

    You can instantiate a RecordingStream by calling either [`dalaran.init`][] (to create a global
    recording) or [`dalaran.RecordingStream`][] (for more advanced use cases).

    Multithreading
    --------------

    A RecordingStream can safely be copied and sent to other threads.
    You can also set a recording as the global active one for all threads ([`dalaran.set_global_data_recording`][])
    or just for the current thread ([`dalaran.set_thread_local_data_recording`][]).
    ...
    by using the [`dalaran.recording_stream_generator_ctx`][] decorator.
    ...
    See also: [`dalaran.get_data_recording`][], [`dalaran.get_global_data_recording`][],
    [`dalaran.get_thread_local_data_recording`][].

    Available methods
    -----------------

    Every function in the Dalaran SDK that takes an optional RecordingStream as a parameter can also
    be called as a method on RecordingStream itself.

    This includes, but isn't limited to:

    - Metadata-related functions:
        [`dalaran.is_enabled`][], [`dalaran.get_recording_id`][], …
    - Sink-related functions:
        [`dalaran.connect_grpc`][], [`dalaran.spawn`][], …
    - Time-related functions:
        [`dalaran.set_time`][], [`dalaran.disable_timeline`][], [`dalaran.reset_time`][], …
    - Log-related functions:
        [`dalaran.log`][], …

    For an exhaustive list, see `help(dalaran.RecordingStream)`.
    ...
    def __init__(
        ...
        *,
        ...
        If you only need a single global recording, [`dalaran.init`][] might be simpler.

        Note that new recording streams always begin connected to a buffered sink.
        To send the data to a viewer or file you will likely want to call [`dalaran.connect_grpc`][] or [`dalaran.save`][]
        explicitly.
        ...
        !!! Warning
            ...
            rec = dl.RecordingStream(application_id="test", recording_id=uuid4())
            rec = dl.RecordingStream(application_id="test", recording_id=uuid4())
            ```
        ...
        -------
        RecordingStream
            A handle to the [`dalaran.RecordingStream`][]. Use it to log data to Dalaran.
        ...
        ```python
        from uuid import uuid4
        stream = dl.RecordingStream("my_app", recording_id=uuid4())
        stream.connect_grpc()
        stream.log("hello", dl.TextLog("Hello world"))
        ```
    ...
    def __del__(self) -> None:  # type: ignore[no-untyped-def]
        ...
        # It's definitely a problem if we are in a forked child process. The dalaran SDK will still
    ...
    def set_sinks(
        ...
        *sinks: LogSinkLike,
        ...
        Duplicate sinks are not allowed. For example, two [`dalaran.GrpcSink`][]s that
        use the same `url` will cause this function to throw a `ValueError`.

        This _replaces_ existing sinks. Calling `dl.init(spawn=True)`, `dl.spawn()`,
        `dl.connect_grpc()` or similar followed by `set_sinks` will result in only
        ...
        sinks:
            ...
            See [`dalaran.GrpcSink`][], [`dalaran.GrpcServerSink`][], [`dalaran.FileSink`][].
        default_blueprint:
    ...
    def stdout(
        ...
        *,
        ...
        Pipe it into a Dalaran Viewer to visualize it.
    ...
    def disconnect(self) -> None:
        ...
        [`dalaran.RecordingStream.connect_grpc`][], [`dalaran.RecordingStream.serve_grpc`][],
        [`dalaran.RecordingStream.save`][] or [`dalaran.RecordingStream.spawn`][].
        """
    ...
    def serve_grpc(
        ...
        *,
        ...
        You can to this server with the native viewer using `dalaran dalaran+http://localhost:{grpc_port}/proxy`.
        ...
        default_blueprint:
            ...
            already has an active blueprint, the new blueprint won't become active until the user
            clicks the "reset blueprint" button. If you want to activate the new blueprint
            immediately, instead use the [`dalaran.RecordingStream.send_blueprint`][] API.
        server_memory_limit:
        ...
        cors_allow_origin:
            ...
            By default, only localhost and dalaran.dev are allowed.
    ...
    def spawn(
        ...
        *,
        ...
        """
        Spawn a Dalaran Viewer, listening on the given port.

        You can also call [dalaran.init][] with a `spawn=True` argument.
    ...
    def notebook_show(
        ...
        *,
        ...
        blueprint:
            ...
            Setting this is equivalent to calling [`dalaran.RecordingStream.send_blueprint`][] before initializing the viewer.
    ...
    def send_property(
        ...
        values:
            Anything that implements the [`dalaran.AsComponents`][] interface, usually an archetype,
    ...
    def send_recording_name(self, name: str) -> None:
        ...
        This name is shown in the Dalaran Viewer.
    ...
    def send_recording_start_time_nanos(self, nanos: int) -> None:
        ...
        This timestamp is shown in the Dalaran Viewer.
    ...
    def set_time(
        ...
        *,
        ...
        [`dalaran.RecordingStream.set_time`][], [`dalaran.RecordingStream.reset_time`][] or
        [`dalaran.RecordingStream.disable_timeline`][].
        ...
        The columnar equivalent to this function is [`dalaran.TimeColumn`][].
    ...
    def reset_time(self) -> None:
        ...
        until the next call to [`dalaran.RecordingStream.set_time`][].
        """
    ...
    def log(
        ...
        *extra: AsComponents | Iterable[DescribedComponentBatch],
        ...
        that implements the [`dalaran.AsComponents`][] interface, or a collection of [`dalaran.ComponentBatchLike`][] objects.

        When logging data, you must always provide an [entity_path](https://www.dalaran.dev/docs/concepts/logging-and-ingestion/entity-path)
        for identifying the data. Note that paths prefixed with "__" are considered reserved for use by the Dalaran SDK
        itself and should not be used for logging user data. This is where Dalaran will log additional information
        such as properties and warnings.

        The most common way to log is with one of the dalaran archetypes, all of which implement
        the [`dalaran.AsComponents`][] interface.
        ...
        dl.log(
            "my/points",
            dl.Points3D([[0.2, 0.5, 0.3], [0.9, 1.2, 0.1], [1.0, 4.2, 0.3]], radii=[0.1, 0.2, 0.3]),
            dl.Arrows3D(vectors=[[0.3, 2.1, 0.2], [0.9, -1.1, 2.3], [-0.4, 0.5, 2.9]]),
            dl.AnyValues(confidence=[0.3, 0.4, 0.9]),
        )
        ```
        ...
        *extra:
            An arbitrary number of additional component bundles implementing the [`dalaran.AsComponents`][]
        ...
        static:
            ...
            Additional timelines set by [`dalaran.RecordingStream.set_time`][] will also be included.

        strict:
            If True, raise exceptions on non-loggable data.
            If False, warn on non-loggable data.
            if None, use the global default from [`dalaran.strict_mode`][]
    ...
    def log_file_from_contents(
        ...
        *,
        ...
        See <https://www.dalaran.dev/docs/getting-started/data-in/open-any-file> for more information.
    ...
    def send_columns(
        ...
        *,
        ...
        r"""
        Send columnar data to Dalaran.

        Unlike the regular `log` API, which is row-oriented, this API lets you submit the data
        in a columnar form. Each `TimeColumnLike` and `ComponentColumn` object represents a column
        of data that will be sent to Dalaran. The lengths of all these columns must match, and all
        data that shares the same index across the different columns will act as a single logical row,
        equivalent to a single call to `dl.log()`.

        Note that this API ignores any stateful time set on the log stream via [`dalaran.RecordingStream.set_time`][].
        ...
        indexes:
            ...
            of timestamps. Generally, you should use one of the provided class [`TimeColumn`][dalaran.TimeColumn].
        columns:
            The columns of components to log. Each object represents a single column of data.

            In order to send multiple components per time value, explicitly create a [`ComponentColumn`][dalaran.ComponentColumn]
    ...
    def send_chunks(
        ...
        See [`dalaran.experimental.send_chunks`][].
        ...
        chunks:
            ...
            - A single [`Chunk`][dalaran.experimental.Chunk].
            - A [`LazyChunkStream`][dalaran.experimental.LazyChunkStream] — consume
              the stream and forward all chunks to this recording stream.
            - A [`LazyStore`][dalaran.experimental.LazyStore] — send all chunks to this
              recording stream. This triggers loading all chunks from the source.
            - A [`ChunkStore`][dalaran.experimental.ChunkStore] — send all chunks to
    ...
    def send_record_batch(
        ...
        *,
        ...
        """
        Coerce a single pyarrow `RecordBatch` to Dalaran structure and log it.

        A thin wrapper over [`Chunk.from_record_batch`][dalaran.experimental.Chunk.from_record_batch]
        followed by [`send_chunks`][dalaran.experimental.send_chunks]. See `Chunk.from_record_batch` for
        ...
        index:
            ...
            [`Chunk.from_record_batch`][dalaran.experimental.Chunk.from_record_batch] for the full
            semantics. Defaults to deriving the index from the batch's Dalaran metadata.
        entity_path:
    ...
    def send_dataframe(
        ...
        *,
        ...
        """
        Coerce a pyarrow `Table` / `RecordBatch` / `RecordBatchReader`, or a datafusion `DataFrame`, to Dalaran structure and log it.

        A thin wrapper over [`Chunk.from_dataframe`][dalaran.experimental.Chunk.from_dataframe] followed by
        [`send_chunks`][dalaran.experimental.send_chunks]. See `Chunk.from_dataframe` for the accepted input
        ...
        index:
            Determines which columns are index (timeline) columns. See
            [`Chunk.from_record_batch`][dalaran.experimental.Chunk.from_record_batch] for the full
            semantics. Defaults to deriving the index from the dataframe's Dalaran metadata.
        entity_path:
            Default entity path for component columns that do not otherwise specify one.
