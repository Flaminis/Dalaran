...
class DynamicArchetype(AsComponents):
    ...
    Component values can also be Dalaran batch types, which is useful when you need
    proper type handling (e.g. RGBA color packing):

    ```python
    dl.log(
        "my_entity", dl.DynamicArchetype(
            archetype="MyColors",
            components={
                "colors": dl.components.ColorBatch([[255, 0, 0], [0, 255, 0]]),
            },
        ),
    )
    ```
    ...
    def __init__(
        ...
        Each of the provided components will be logged as a separate component batch with the same archetype using the provided data.
         ...
         - The value can be any Dalaran component batch type (e.g. `dl.components.ColorBatch(...)`,
           `dl.components.ScalarBatch(...)`) or any data convertible to a pyarrow array.
           Using Dalaran batch types is recommended when the component needs specific type
