"""
OpenTelemetry tracing helpers shared across Dalaran-based Python code.

Provides [`with_tracing`][dalaran._tracing.with_tracing] (decorator) and
[`tracing_scope`][dalaran._tracing.tracing_scope] (context manager), which both
create a Python OpenTelemetry span and bridge the trace context into Dalaran's
...
the consumer package rather than importing `dalaran._tracing` directly.
"""
...
def _init_once() -> None:
    ...
    try:
        ...
        logger.warning("`with_tracing` is a no-op: install OpenTelemetry via `pip install dalaran-sdk[tracing]`")
        return
    ...
    service_name = os.environ.get("OTEL_SERVICE_NAME") or "dalaran-py"
...
def current_trace_carrier() -> dict[str, str] | None:
    ...
    [`attach_parent_carrier`][dalaran._tracing.attach_parent_carrier] in the child
...
def attach_parent_carrier(carrier: dict[str, str] | None) -> None:
    ...
    All subsequent spans created on this thread (via [`with_tracing`][dalaran._tracing.with_tracing]
...
def _push_carrier_to_rust() -> tuple[Any, Any] | None:
    """
    Write the current OTel context into Dalaran's `ContextVar` so Rust can read it.
    ...
    """
    from dalaran_bindings import _get_trace_context_var
...
def tracing_scope(name: str) -> Iterator[None]:
    """
    Open an OpenTelemetry span for the duration of a `with` block and propagate trace context into Dalaran's Rust SDK.

    Context-manager counterpart to [`with_tracing`][dalaran._tracing.with_tracing] —
    ...
    with trace.get_tracer("dalaran").start_as_current_span(name):
...
def with_tracing(name: str) -> Callable[[F], F]:
    """
    Wrap a function in an OpenTelemetry span and propagate trace context into Dalaran's Rust SDK.

    When enabled, creates a span named `name`, injects the W3C `traceparent` header into
    Dalaran's shared `ContextVar`, and runs the wrapped function. Any Rust-side
    ...
    [`tracing_scope`][dalaran._tracing.tracing_scope] instead.
