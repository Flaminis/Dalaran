...
class PixelFormat(Enum):
    """
    **Datatype**: Specifieds a particular format of an [`archetypes.Image`][dalaran.archetypes.Image].

    Most images can be described by a [`datatypes.ColorModel`][dalaran.datatypes.ColorModel] and a [`datatypes.ChannelDatatype`][dalaran.datatypes.ChannelDatatype],
    e.g. `RGB` and `U8` respectively.

    However, some image formats has chroma downsampling and/or
    use differing number of bits per channel, and that is what this [`datatypes.PixelFormat`][dalaran.datatypes.PixelFormat] is for.

    All these formats support random access.

    For more compressed image formats, see [`archetypes.EncodedImage`][dalaran.archetypes.EncodedImage].
    """
    ...
    Also known as just "gray". This is virtually identical to a 8bit luminance/grayscale (see [`datatypes.ColorModel`][dalaran.datatypes.ColorModel]).
    ...
    If not for this range limitation/remapping, this is almost identical to 8bit luminace/grayscale (see [`datatypes.ColorModel`][dalaran.datatypes.ColorModel]).
    """
