...
# Based on "crates/store/dl_sdk_types/definitions/dalaran/datatypes/class_description.fbs".
...
class ClassDescription(ClassDescriptionExt):
    ...
    **Datatype**: The description of a semantic Class.
    ...
    If an entity is annotated with a corresponding [`components.ClassId`][dalaran.components.ClassId], Dalaran will use
    the attached [`datatypes.AnnotationInfo`][dalaran.datatypes.AnnotationInfo] to derive labels and colors.

    Keypoints within an annotation class can similarly be annotated with a
    [`components.KeypointId`][dalaran.components.KeypointId] in which case we should defer to the label and color for the
    [`datatypes.AnnotationInfo`][dalaran.datatypes.AnnotationInfo] specifically associated with the Keypoint.

    Keypoints within the class can also be decorated with skeletal edges.
    Keypoint-connections are pairs of [`components.KeypointId`][dalaran.components.KeypointId]s. If an edge is
    defined, and both keypoints exist within the instance of the class, then the
    keypoints should be connected with an edge. The edge should be labeled and
    colored as described by the class's [`datatypes.AnnotationInfo`][dalaran.datatypes.AnnotationInfo].

    Note that a `ClassDescription` can be directly logged using `dalaran.log`.
    This is equivalent to logging a `dalaran.AnnotationContext` containing
    ...
    # The [`datatypes.AnnotationInfo`][dalaran.datatypes.AnnotationInfo] for the class.
    #
    ...
    # The [`datatypes.AnnotationInfo`][dalaran.datatypes.AnnotationInfo] for all of the keypoints.
    #
    # (Docstring intentionally commented out to hide this field from the docs)
