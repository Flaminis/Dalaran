...
"""
Type alias for values accepted by [`AnyBatchValue`][dalaran.any_batch_value.AnyBatchValue] and [`DynamicArchetype`][dalaran.dynamic_archetype.DynamicArchetype].

This includes:
- Dalaran component batch types implementing [`ComponentBatchLike`][dalaran.ComponentBatchLike] (e.g. `dl.components.ColorBatch(...)`)
...
class TypeRegistry:
    ...
    Dalaran requires that a given component only takes on a single type. This registry
    ...
    def register(self, descriptor: ComponentDescriptor, new: TypeRegistryValue, *, expect_column: bool = False) -> None:
        ...
        if existing is not None and existing != new:
            _send_warning_or_raise(
                ...
                "Dalaran requires that a given component only takes on a single type.",
...
class AnyBatchValue(ComponentBatchLike):
    ...
    def __init__(
        ...
        *,
        ...
        the `as_arrow_array()` method if it's defined. All Dalaran Batch datatypes implement
        ...
        Note: dalaran requires that a given component only take on a single type.
