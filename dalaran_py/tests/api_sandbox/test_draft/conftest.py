...
import dalaran_draft as dl  # noqa: E402


@pytest.fixture(scope="function")
def populated_client(simple_dataset_prefix: Path) -> Iterator[dl.catalog.CatalogClient]:
    ...
    with dl.server.Server() as server:
        ...
        meta.append(
            dalaran_segment_id=["simple_recording_0", "simple_recording_2"],
            success=[True, False],
        )
...
@pytest.fixture(scope="function")
def populated_client_complex(complex_dataset_prefix: Path) -> Iterator[dl.catalog.CatalogClient]:
    """Create a temporary dataset prefix with a few simple recordings."""
    ...
    with dl.server.Server() as server:
        ...
        meta.append(
            dalaran_segment_id=["complex_recording_1", "complex_recording_2", "complex_recording_3"],
...
@pytest.fixture(scope="function")
def basic_dataset(populated_client: dl.catalog.CatalogClient) -> Iterator[dl.catalog.DatasetEntry]:
    yield populated_client.get_dataset(name="basic_dataset")


@pytest.fixture(scope="function")
def basic_metadata(populated_client: dl.catalog.CatalogClient) -> Iterator[dl.catalog.TableEntry]:
    yield populated_client.get_table(name="basic_dataset_metadata")


@pytest.fixture(scope="function")
def complex_dataset(populated_client_complex: dl.catalog.CatalogClient) -> Iterator[dl.catalog.DatasetEntry]:
    yield populated_client_complex.get_dataset(name="complex_dataset")


@pytest.fixture(scope="function")
def complex_metadata(populated_client_complex: dl.catalog.CatalogClient) -> Iterator[dl.catalog.TableEntry]:
