...
def segment_stable_snapshot(df: datafusion.DataFrame) -> str:
    ...
    # `dalaran_size_bytes` is the on-disk IPC byte length (matching the commercial
    # redap impl), which varies by a few bytes between runs due to timestamp/metadata
    # differences in the written RRDs.
    return str(
        df.drop("dalaran_storage_urls", "dalaran_last_updated_at", "dalaran_size_bytes").sort("dalaran_segment_id"),
    )
