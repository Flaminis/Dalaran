...
def test_dataframe_api_filter_segment_id(simple_dataset_prefix: Path) -> None:
    with dl.server.Server(datasets={"ds": simple_dataset_prefix}) as server:
        ...
        df = view.reader(index="timeline").sort("dalaran_segment_id")
...
# `filter_segments`, `using_index_values`, and DataFusion `WHERE dalaran_segment_id`
...
def test_dataframe_api_filter_unknown_segment_id_pushdown(simple_dataset_prefix: Path) -> None:
    with dl.server.Server(datasets={"ds": simple_dataset_prefix}) as server:
        ...
        table = ds.reader(index="timeline").filter(col("dalaran_segment_id") == "does_not_exist").to_arrow_table()
...
def test_dataframe_api_filter_segments_mixed_known_unknown(simple_dataset_prefix: Path) -> None:
    with dl.server.Server(datasets={"ds": simple_dataset_prefix}) as server:
        ...
        assert table.column("dalaran_segment_id").to_pylist() == ["simple_recording_0"]
...
def test_dataframe_api_filter_mixed_segment_id_pushdown(simple_dataset_prefix: Path) -> None:
    with dl.server.Server(datasets={"ds": simple_dataset_prefix}) as server:
        ...
        table = (
            ...
            .filter(in_list(col("dalaran_segment_id"), [lit("simple_recording_0"), lit("does_not_exist")]))
            .to_arrow_table()
        )
