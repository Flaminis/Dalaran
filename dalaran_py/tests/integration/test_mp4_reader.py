"""Integration tests for `dalaran.experimental.Mp4Reader`."""
...
import pytest
from dalaran.components import VideoCodec
from dalaran.experimental import Chunk, Mp4Reader, Mp4TranscodeOptions, StreamingReader
...
def _cols(chunk: Chunk) -> list[str]:
    ...
    return sorted(f.name for f in rb.schema if f.name not in timelines and not f.name.startswith("dalaran.controls"))
...
# ---------------------------------------------------------------------------
# Asset mode — matches the existing `dalaran video.mp4` behavior
# ---------------------------------------------------------------------------
