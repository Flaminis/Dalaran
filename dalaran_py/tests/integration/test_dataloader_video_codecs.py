...
# `DalaranMapDataset.__init__` warns when the default start method is `fork`,
...
CODEC_CONFIGS = {
    "h264": CodecConfig("libx264", "h264_mp4toannexb", dl.VideoCodec.H264, "h264", force_no_b_frames=True),
    "h265": CodecConfig("libx265", "hevc_mp4toannexb", dl.VideoCodec.H265, "h265", force_no_b_frames=True),
    "av1": CodecConfig("libaom-av1", None, dl.VideoCodec.AV1, "av1"),
}
    ...
    with dl.RecordingStream(
        ...
        if keyframe_logging == "sparse":
            ...
            rec.send_columns(
                "/video",
                indexes=[dl.TimeColumn("frame", sequence=list(range(len(samples))))],
                columns=dl.VideoStream.columns(is_keyframe=flags),
            )
...
def _build_temporal_video_dlr(
    ...
    *,
    ...
    def _time_column(values: np.ndarray) -> dl.TimeColumn:
        if kind == "timestamp":
            return dl.TimeColumn(timeline, timestamp=values)
