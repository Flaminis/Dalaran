...
# `.github/workflows/dalaran_reusable_test_wheels.yml`). On linux-arm64 the
...
def test_save_screenshot(tmp_path: Path) -> None:
    ...
    with ViewerClient.spawn(headless=True, port=port, hide_welcome_screen=True) as viewer:
        rec = dl.RecordingStream("dalaran_example_headless_test")
        rec.connect_grpc(url=viewer.url)
        rec.log("points", dl.Points3D([[0, 0, 0], [1, 1, 1], [2, 0, 1]], colors=[255, 0, 0]))
        rec.flush()
