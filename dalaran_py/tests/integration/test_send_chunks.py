"""Tests for dalaran.experimental.send_chunks."""
...
if TYPE_CHECKING:
    ...
    from dalaran.experimental import ChunkStore, LazyChunkStream, LazyStore
...
def _make_chunk(entity_path: str, value: int) -> Chunk:
    return Chunk.from_columns(
        entity_path,
        indexes=[dl.TimeColumn("frame", sequence=[value])],
        columns=dl.Scalars.columns(scalars=[float(value)]),
    )
