...
if TYPE_CHECKING:
    ...
    from dalaran.catalog import ContentFilter, DatasetEntry, IndexValuesLike
...
def _build_fixture_store() -> ChunkStore:
    ...
    `/a` also gets a `dl.Clear` to produce a tombstone column, and `/q` logs
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        with dl.RecordingStream("dalaran_example_fixture", recording_id="fix") as rec:
            ...
            for i in FIXTURE_INDEX_RANGE:
                ...
                rec.log("/a", dl.Scalars(scalars=[float(i)]))
                if i % 2 == 0:
                    rec.log("/b", dl.Scalars(scalars=[float(-i)]))
            # Tombstone column: `Clear:is_recursive` on /a.
            rec.set_time("t", sequence=FIXTURE_NUM_ROWS // 2)
            rec.log("/a", dl.Clear(recursive=False))
            ...
            rec.log("/q", dl.Points3D(positions=[[0.0, 0.0, 0.0]], colors=[]))
            rec.disconnect()
...
def _drop_segment_id(df: datafusion.DataFrame) -> datafusion.DataFrame:
    return df.drop("dalaran_segment_id")
