"""Tests for dalaran.experimental.LazyChunkStream and RrdReader."""
...
def test_dlr_reader_properties(test_dlr_path: Path) -> None:
...
def test_dlr_reader_file_not_found(tmp_path: Path) -> None:
    with pytest.raises(ValueError, match="not found"):
        RrdReader(tmp_path / "nonexistent.dlr")
...
def test_to_chunks(test_dlr_path: Path) -> None:
...
def test_iter(test_dlr_path: Path) -> None:
    stream = RrdReader(test_dlr_path).stream()
    collected = stream.to_chunks()

    stream2 = RrdReader(test_dlr_path).stream()
...
def test_collect_returns_chunk_store(test_dlr_path: Path) -> None:
...
def test_identity_roundtrip(test_dlr_path: Path, tmp_path: Path) -> None:
    reader = RrdReader(test_dlr_path)
    out = tmp_path / "roundtrip.dlr"

    reader.stream().write_dlr(out, application_id=APP_ID, recording_id=RECORDING_ID)

    original = reader.store()
    roundtripped = RrdReader(out).store()
    assert original.schema() == roundtripped.schema()

    # Strong check: `dalaran dlr compare` verifies semantic equality of the data.
    process = subprocess.run(
        ["dalaran", "dlr", "compare", "--unordered", str(test_dlr_path), str(out)],
        check=False,
        capture_output=True,
    )
    ...
    assert process.returncode == 0, f"DLR compare failed: {process.stderr.decode('utf-8')}"
...
def test_filter_content(test_dlr_path: Path) -> None:
    # Single string
    store = RrdReader(test_dlr_path).stream().filter(content="/robots/**").collect()
    assert store.schema().entity_paths() == ["/robots/arm"]

    # List of strings
    store2 = RrdReader(test_dlr_path).stream().filter(content=["/robots/**", "/cameras/**"]).collect()
    assert store2.schema().entity_paths() == ["/cameras/front", "/robots/arm"]


def test_filter_is_static(test_dlr_path: Path) -> None:
    static_store = RrdReader(test_dlr_path).stream().filter(is_static=True).collect()
    assert static_store.schema().entity_paths() == ["/config"]

    temporal_store = RrdReader(test_dlr_path).stream().filter(is_static=False).collect()
    assert "/config" not in temporal_store.schema().entity_paths()


def test_filter_has_timeline(test_dlr_path: Path) -> None:
    store = RrdReader(test_dlr_path).stream().filter(has_timeline="other_timeline").collect()
    # Only /robots/arm has the other_timeline
    assert store.schema().entity_paths() == ["/robots/arm"]


def test_filter_component(test_dlr_path: Path) -> None:
    store = RrdReader(test_dlr_path).stream().filter(components="Points3D:positions").collect()
    assert store.schema().entity_paths() == ["/robots/arm"]
...
def test_component_slice_gets_new_chunk_id(test_dlr_path: Path) -> None:
    """Slicing by component must produce chunks with fresh IDs, not reuse the original."""
    original_ids = {c.id for c in RrdReader(test_dlr_path).stream().to_chunks()}

    # filter keeps only the matching column -> sliced chunk
    filtered = RrdReader(test_dlr_path).stream().filter(components="Points3D:positions").to_chunks()
    ...
    dropped = RrdReader(test_dlr_path).stream().drop(components="Points3D:positions").to_chunks()
...
def test_filter_multiple_components(test_dlr_path: Path) -> None:
    """filter(components=[A, B]) keeps both columns when present (OR semantics)."""
    store = RrdReader(test_dlr_path).stream().filter(components=["Points3D:positions", "Points3D:colors"]).collect()
    assert store.schema().entity_paths() == ["/robots/arm"]
    arm_cols = store.schema().columns_for(entity_path="/robots/arm")
    assert len(arm_cols) == 2


def test_filter_multiple_components_partial(test_dlr_path: Path) -> None:
    """filter(components=[A, Z]) where Z doesn't exist: keep A only."""
    store = RrdReader(test_dlr_path).stream().filter(components=["Points3D:positions", "Nonexistent:foo"]).collect()
    assert store.schema().entity_paths() == ["/robots/arm"]


def test_filter_multiple_components_none_present(test_dlr_path: Path) -> None:
    """filter(components=[Z1, Z2]) where neither exist: empty store."""
    store = RrdReader(test_dlr_path).stream().filter(components=["Nonexistent:a", "Nonexistent:b"]).collect()
    assert store.schema().entity_paths() == []


def test_drop_multiple_components(test_dlr_path: Path) -> None:
    """drop(components=[A, B]) removes both columns."""
    store = RrdReader(test_dlr_path).stream().drop(components=["Points3D:positions", "Points3D:colors"]).collect()
    # /robots/arm had only those two components, so it should be gone
    assert "/robots/arm" not in store.schema().entity_paths()


def test_split_multiple_components(test_dlr_path: Path) -> None:
...
def test_drop(test_dlr_path: Path) -> None:
    store = RrdReader(test_dlr_path).stream().drop(content="/robots/**").collect()
    paths = store.schema().entity_paths()
...
def test_split_merge_roundtrip(test_dlr_path: Path) -> None:
    original = RrdReader(test_dlr_path).stream().collect()
...
def test_split_drop_one_branch(test_dlr_path: Path) -> None:
...
def test_from_iter(test_dlr_path: Path) -> None:
    original = RrdReader(test_dlr_path).stream().to_chunks()

    roundtripped = LazyChunkStream.from_iter(original).collect()
    assert roundtripped.schema() == RrdReader(test_dlr_path).store().schema()
...
def test_chained_filters(test_dlr_path: Path) -> None:
    store = RrdReader(test_dlr_path).stream().filter(is_static=False).filter(content="/robots/**").collect()
    assert store.schema().entity_paths() == ["/robots/arm"]
...
def test_dangling_split_matched_only(test_dlr_path: Path, capfd: pytest.CaptureFixture[str]) -> None:
...
def test_dangling_split_unmatched_only(test_dlr_path: Path, capfd: pytest.CaptureFixture[str]) -> None:
...
def test_stream_consumed_after_filter(test_dlr_path: Path) -> None:
...
def test_stream_consumed_after_split(test_dlr_path: Path) -> None:
...
def test_merge_same_stream_twice(test_dlr_path: Path) -> None:
...
def test_merge_indirect_reuse(test_dlr_path: Path) -> None:
...
def test_terminal_does_not_consume(test_dlr_path: Path) -> None:
    """Terminals (collect, write_dlr, iter) borrow without consuming."""
    stream = RrdReader(test_dlr_path).stream()
...
def test_lenses_identity(test_dlr_path: Path) -> None:
    """A lens with Selector('.') passes through the struct component data unchanged."""

    lens = MutateLens("Imu:accel", Selector("."))

    store = RrdReader(test_dlr_path).stream().filter(content="/sensors/**").lenses(lens).collect()
    assert store.summary() == inline_snapshot(
        "/sensors/imu rows=2 static=False timelines=['my_index'] cols=['Imu:accel', 'my_index']"
    )


def test_lenses_field_selector(test_dlr_path: Path) -> None:
    """A lens with Selector('.x') extracts a struct field and reinterprets it as a Dalaran Scalar."""

    lens = DeriveLens("Imu:accel").to_component(dl.Scalars.descriptor_scalars(), Selector(".x"))
    ...
    chunks = RrdReader(test_dlr_path).stream().filter(content="/sensors/**").lenses(lens).to_chunks()
    rb = chunks[0].to_record_batch()
    scalars = rb.column("Scalars:scalars")
    assert scalars.to_pylist() == [[0.1], [0.4]]


def test_lenses_multiple_outputs(test_dlr_path: Path) -> None:
    """A single lens can produce multiple output groups at different entity paths."""

    lenses = [
        DeriveLens("Imu:accel", output_entity="/out/x").to_component(dl.Scalars.descriptor_scalars(), Selector(".x")),
        DeriveLens("Imu:accel", output_entity="/out/z").to_component(dl.Scalars.descriptor_scalars(), Selector(".z")),
    ]

    store = RrdReader(test_dlr_path).stream().filter(content="/sensors/**").lenses(lenses).collect()
    assert store.summary() == inline_snapshot("""\
...
def test_lenses_drop_unmatched(test_dlr_path: Path) -> None:
    ...
    store = RrdReader(test_dlr_path).stream().lenses(lens, output_mode="drop_unmatched").collect()
    assert store.summary() == inline_snapshot("")


def test_lenses_forward_unmatched(test_dlr_path: Path) -> None:
    ...
    lens = DeriveLens("Imu:accel", output_entity="/transformed").to_component(
        dl.Scalars.descriptor_scalars(), Selector(".x")
    )

    store = (
        RrdReader(test_dlr_path)
        .stream()
...
def test_lenses_forward_all(test_dlr_path: Path) -> None:
...
def test_lenses_consumes_stream(test_dlr_path: Path) -> None:
...
def test_lenses_chained_with_filter(test_dlr_path: Path) -> None:
    """Lenses can be composed with filter in a pipeline."""

    lens = DeriveLens("Imu:accel").to_component(dl.Scalars.descriptor_scalars(), Selector(".z"))
...
def test_lenses_content_filter_match(test_dlr_path: Path) -> None:
...
def test_lenses_content_filter_excludes_lens_target(test_dlr_path: Path) -> None:
...
def test_lenses_invalid_output_mode(test_dlr_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="Unknown output_mode"):
        RrdReader(test_dlr_path).stream().lenses(lens, output_mode="invalid")  # type: ignore[arg-type]


def test_lenses_time_extraction(test_dlr_path: Path) -> None:
    """A lens can extract a timestamp field from a struct component as a new timeline."""

    lens = (
        DeriveLens("Imu:accel")
        .to_component(dl.Scalars.descriptor_scalars(), Selector(".x"))
...
def test_lenses_dynamic_selector(test_dlr_path: Path) -> None:
    ...
    lens = DeriveLens("Imu:accel").to_component(dl.Scalars.descriptor_scalars(), selector)
...
def test_map_identity(test_dlr_path: Path) -> None:
    """map(identity) preserves all chunks."""
    original = RrdReader(test_dlr_path).stream().to_chunks()
    mapped = RrdReader(test_dlr_path).stream().map(lambda c: c).to_chunks()
    assert len(mapped) == len(original)


def test_flat_map_identity(test_dlr_path: Path) -> None:
    """flat_map(lambda c: [c]) preserves all chunks."""
    original = RrdReader(test_dlr_path).stream().to_chunks()
    mapped = RrdReader(test_dlr_path).stream().flat_map(lambda c: [c]).to_chunks()
    assert len(mapped) == len(original)


def test_flat_map_drop_all(test_dlr_path: Path) -> None:
    """flat_map(lambda c: []) produces an empty stream."""
    chunks = RrdReader(test_dlr_path).stream().flat_map(lambda _c: []).to_chunks()
    assert len(chunks) == 0


def test_map_after_filter(test_dlr_path: Path) -> None:
    """Map composes after filter."""
    store = RrdReader(test_dlr_path).stream().filter(content="/robots/**").map(lambda c: c).collect()
    assert store.schema().entity_paths() == ["/robots/arm"]


def test_map_in_split_branch(test_dlr_path: Path) -> None:
...
def test_map_error_propagation(test_dlr_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="test error from map"):
        RrdReader(test_dlr_path).stream().map(_raise).to_chunks()


def test_flat_map_error_propagation(test_dlr_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="test error from flat_map"):
        RrdReader(test_dlr_path).stream().flat_map(_raise).to_chunks()


def test_map_consumed_stream(test_dlr_path: Path) -> None:
...
def test_flat_map_consumed_stream(test_dlr_path: Path) -> None:
...
def test_map_multiple_executions(test_dlr_path: Path) -> None:
    """Calling to_chunks() twice on a mapped stream produces the same results."""
    stream = RrdReader(test_dlr_path).stream().map(lambda c: c)
