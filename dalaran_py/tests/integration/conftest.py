...
def test_rrd_path(tmp_path_factory: pytest.TempPathFactory) -> Path:
    ...
    with dl.RecordingStream(TEST_APP_ID, recording_id=TEST_RECORDING_ID) as rec:
        ...
        rec.send_columns(
            "/robots/arm",
            indexes=[
                dl.TimeColumn("my_index", sequence=[1, 2]),
                dl.TimeColumn("other_timeline", sequence=[10, 20]),
            ],
        ...
        rec.send_columns(
            "/cameras/front",
            indexes=[dl.TimeColumn("my_index", sequence=[1])],
            columns=dl.TextLog.columns(text=["frame_001"]),
        )
        ...
        rec.send_columns(
            "/config",
            indexes=[],
            columns=dl.TextLog.columns(text=["v1"]),
        )
        ...
        rec.send_columns(
            "/sensors/imu",
            indexes=[dl.TimeColumn("my_index", sequence=[1, 2])],
