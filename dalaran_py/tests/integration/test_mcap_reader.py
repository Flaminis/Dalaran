"""Tests for dalaran.experimental.McapReader and StreamingReader protocol."""
...
import pytest
from dalaran.experimental import Chunk, McapInfo, McapReader, StreamingReader
...
def chunk_summary(chunks: list[Chunk]) -> str:
    ...
    for c in sorted(chunks, key=lambda c: (c.entity_path, not c.is_static)):
        ...
        cols = sorted(f.name for f in rb.schema if not f.name.startswith("dalaran.controls"))
