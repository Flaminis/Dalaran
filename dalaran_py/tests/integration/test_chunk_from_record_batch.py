...
import pytest
from dalaran import AUTO_INDEX
...
def test_metadata_driven_temporal() -> None:
    ...
    schema = pa.schema([
        pa.field("frame", index.type, metadata={DALARAN_KIND: b"index"}),
        pa.field("/e:c", _VALUES.type, metadata={SORBET_ENTITY_PATH: b"/e"}),
    ])
...
def test_index_name_only_temporal() -> None:
    """An `index_name`-only column (no `dalaran:kind`) is still promoted under AUTO."""
    index = pa.array([0, 1], type=pa.int64())
    schema = pa.schema([
...
def test_plain_array_is_list_wrapped() -> None:
    ...
    [component_field] = [f for f in batch.schema if f.metadata and f.metadata.get(DALARAN_KIND) == b"data"]
...
def test_rejects_plain_batch() -> None:
    """A batch with no Dalaran metadata at all is ambiguous under AUTO → ValueError."""
...
def test_no_component_columns_raises() -> None:
    index = pa.array([0, 1], type=pa.int64())
    schema = pa.schema([pa.field("frame", index.type, metadata={DALARAN_KIND: b"index"})])
    with pytest.raises(ValueError):
