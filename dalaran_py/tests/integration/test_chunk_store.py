...
# (filename, dalaran codec) pairs exercised by the VideoStream compaction test.
VIDEO_CASES = [
    ("Big_Buck_Bunny_1080_10s_av1.mp4", dl.VideoCodec.AV1),
    ("Big_Buck_Bunny_1080_1s_h264_nobframes.mp4", dl.VideoCodec.H264),
    ("Sintel_1080_10s_av1.mp4", dl.VideoCodec.AV1),
]
...
def _build_video_stream_rrd(tmp_dir: Path, filename: str, codec: dl.VideoCodec) -> tuple[Path, int]:
    ...
    try:
        ...
        # Dalaran's VideoStream expects AnnexB for H.264/H.265, but mp4 demuxing yields
        ...
        filter_name = {
            dl.VideoCodec.H264: "h264_mp4toannexb",
            dl.VideoCodec.H265: "hevc_mp4toannexb",
        }.get(codec)
        ...
        with dl.RecordingStream("dalaran_example_video_compact", recording_id=f"video-compact-{video_path.stem}") as rec:
            ...
            def log_packet(packet: av.Packet) -> None:
                ...
                rec.send_columns(
                    "/video",
                    indexes=[dl.TimeColumn("video_time", duration=[pts_seconds])],
                    columns=dl.VideoStream.columns(sample=[bytes(packet)]),
                )
...
def test_write_rrd_metadata(test_rrd_path: Path, tmp_path: Path) -> None:
    ...
    assert len(recs) == 1
    assert recs[0].application_id == "dalaran_example_my_app"
