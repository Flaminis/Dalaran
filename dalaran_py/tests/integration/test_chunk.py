...
def test_chunk_from_columns_temporal() -> None:
    ...
    chunk = Chunk.from_columns(
        "/test/entity",
        indexes=[dl.TimeColumn("frame", sequence=[0, 1])],
        columns=dl.Points3D.columns(positions=[[1, 2, 3], [10, 20, 30], [4, 5, 6]]).partition(lengths=[2, 1]),
    )
...
def test_chunk_format_keeps_dalaran_metadata_prefixes() -> None:
    """`trim_metadata_keys=False` preserves the `dalaran:` / `sorbet:` prefixes on metadata keys."""
    chunk = Chunk.from_columns(
        "/test/static",
        indexes=[],
        columns=dl.Points3D.columns(positions=[[1, 2, 3], [4, 5, 6]]),
...
┌─────────────────────────────────────────────────────────────────────────────────────────────────────┐
│ METADATA:                                                                                           │
│ * dalaran:entity_path: /test/static                                                                   │
│ * dalaran:id: [**REDACTED**]                                                                          │
...
│ │ ---                                           ┆ ---                                             │ │
│ │ type: non-null FixedSizeBinary(16)            ┆ type: List(FixedSizeList(3 x non-null Float32)) │ │
│ │ ARROW:extension:metadata: {"namespace":"row"} ┆ dalaran:archetype: Points3D                       │ │
│ │ ARROW:extension:name: TUID                    ┆ dalaran:component: Points3D:positions             │ │
│ │ dalaran:is_sorted: true                         ┆ dalaran:component_type: Position3D                │ │
...
def test_chunk_from_columns_into_store() -> None:
    ...
    from dalaran.experimental import ChunkStore

    chunk = Chunk.from_columns(
        "/test",
        indexes=[dl.TimeColumn("frame", sequence=[0])],
        columns=dl.Points3D.columns(positions=[[1, 2, 3]]),
    )
...
def test_chunk_from_columns_multiple_timelines() -> None:
    ...
    chunk = Chunk.from_columns(
        "/test/multi",
        indexes=[
            dl.TimeColumn("frame", sequence=[0, 1]),
            dl.TimeColumn("timestamp", timestamp=[1000.0, 2000.0]),
        ],
        columns=dl.Points3D.columns(positions=[[1, 2, 3], [4, 5, 6]]),
    )
    assert chunk.format(redact=True) == inline_snapshot("""\
...
def test_chunk_from_columns_length_mismatch() -> None:
    ...
    with pytest.raises(ValueError, match="same length"):
        Chunk.from_columns(
            "/test/bad",
            indexes=[dl.TimeColumn("frame", sequence=[0, 1, 2])],  # 3 rows
            columns=dl.Points3D.columns(positions=[[1, 2, 3], [4, 5, 6]]),  # 2 rows
        )
...
def test_apply_lenses_field_extraction() -> None:
    ...
    chunk = Chunk.from_columns(
        "/sensor",
        indexes=[dl.TimeColumn("frame", sequence=[0, 1])],
        columns=dl.DynamicArchetype.columns(archetype="Imu", components={"accel": imu_data}),
    )
...
""")
    ...
    lens = DeriveLens("Imu:accel").to_component(dl.Scalars.descriptor_scalars(), ".x")
...
def test_apply_lenses_string_prefix_builtin() -> None:
    ...
    chunk = Chunk.from_columns(
        "/camera",
        indexes=[dl.TimeColumn("frame", sequence=[0, 1])],
        columns=dl.DynamicArchetype.columns(archetype="Image", components={"format": image_data}),
    )
...
def test_apply_lenses_multiple_outputs() -> None:
    ...
    chunk = Chunk.from_columns(
        "/sensor",
        indexes=[dl.TimeColumn("frame", sequence=[0])],
        columns=dl.DynamicArchetype.columns(archetype="Imu", components={"accel": data}),
    )
...
""")
    ...
    lenses = [
        DeriveLens("Imu:accel", output_entity="/out/x").to_component(dl.Scalars.descriptor_scalars(), ".x"),
        DeriveLens("Imu:accel", output_entity="/out/y").to_component(dl.Scalars.descriptor_scalars(), ".y"),
    ]
...
def test_apply_lenses_combined_mutate_derive_and_derive_to_entity() -> None:
    ...
    lenses: list[Lens] = [
        ...
        DeriveLens("Imu:accel").to_component(dl.Scalars.descriptor_scalars(), ".y"),
        # Derive .x as a Scalar at a different entity
        DeriveLens("Imu:accel", output_entity="/derived").to_component(dl.Scalars.descriptor_scalars(), ".x"),
    ]
    results = chunk.apply_lenses(lenses)
...
def _int64_accel_chunk() -> Chunk:
    ...
    return Chunk.from_columns(
        "/sensor",
        indexes=[dl.TimeColumn("frame", sequence=[0, 1])],
        columns=dl.DynamicArchetype.columns(archetype="Imu", components={"accel": pa.array([1, 2], type=pa.int64())}),
    )
...
def test_apply_lenses_cast_to_auto() -> None:
    ...
    lens = DeriveLens("Imu:accel", output_entity="/derived").to_component(
        dl.Scalars.descriptor_scalars(), ".", cast_to="auto"
    )
    results = chunk.apply_lenses([lens])
...
def test_apply_lenses_cast_to_explicit_type() -> None:
    ...
    chunk = _int64_accel_chunk()
    lens = DeriveLens("Imu:accel", output_entity="/derived").to_component(
        dl.Scalars.descriptor_scalars(), ".", cast_to=pa.float32()
    )
    results = chunk.apply_lenses([lens])
...
def test_apply_lenses_no_cast_preserves_type() -> None:
    ...
    chunk = _int64_accel_chunk()
    lens = DeriveLens("Imu:accel", output_entity="/derived").to_component(dl.Scalars.descriptor_scalars(), ".")
    results = chunk.apply_lenses([lens])
...
def test_apply_lenses_time_extraction() -> None:
    ...
    chunk = Chunk.from_columns(
        "/sensor",
        indexes=[dl.TimeColumn("frame", sequence=[0, 1])],
        columns=dl.DynamicArchetype.columns(archetype="Sensor", components={"data": data}),
    )
...
""")
    ...
    lens = (
        ...
        .to_component(dl.Scalars.descriptor_scalars(), ".value")
...
def test_apply_lenses_with_pipe() -> None:
    ...
    chunk = Chunk.from_columns(
        "/sensor",
        indexes=[dl.TimeColumn("frame", sequence=[0, 1])],
        columns=dl.DynamicArchetype.columns(archetype="S", components={"d": data}),
    )
...
""")
    ...
    lens = DeriveLens("S:d").to_component(dl.Scalars.descriptor_scalars(), selector)
    results = chunk.apply_lenses(lens)
...
def test_apply_selector_doubles_values() -> None:
    ...
    chunk = Chunk.from_columns(
        "/sensor",
        indexes=[dl.TimeColumn("tick", sequence=[0, 1])],
        columns=dl.DynamicArchetype.columns(
...
def test_apply_selector_component_not_found() -> None:
    ...
    chunk = Chunk.from_columns(
        "/test",
        indexes=[dl.TimeColumn("tick", sequence=[0])],
        columns=dl.DynamicArchetype.columns(
...
def _write_simple_dlr(path: Path, app_id: str, recording_id: str, *, send_properties: bool) -> None:
    ...
    with dl.RecordingStream(app_id, recording_id=recording_id, send_properties=send_properties) as rec:
        ...
        rec.send_columns(
            "/log",
            indexes=[dl.TimeColumn("frame", sequence=[0])],
            columns=dl.TextLog.columns(text=["hello"]),
        )
...
def test_merge_two_rrds_with_distinct_entity_path_prefixes(tmp_path: Path, send_properties: bool) -> None:
    ...
    def prefixed(reader: RrdReader, prefix: str) -> LazyChunkStream:
        ...
        if send_properties:
            ...
            stream = stream.drop(content=f"{dl.RECORDING_PROPERTIES_PATH}/**")
