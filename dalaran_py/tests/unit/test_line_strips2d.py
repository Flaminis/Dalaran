...
from dalaran.datatypes import ClassIdArrayLike, Float32ArrayLike, Float32Like, Rgba32ArrayLike, Utf8ArrayLike, Vec2D
...
def test_single_line_strip2d() -> None:
    ...
    reference = dl.LineStrips2D([dl.components.LineStrip2D([[0, 0], [1, 1]])])
    assert reference.strips is not None and len(reference.strips) == 1
    assert reference == dl.LineStrips2D(dl.components.LineStrip2D([[0, 0], [1, 1]]))
    assert reference == dl.LineStrips2D([[[0, 0], [1, 1]]])
    assert reference == dl.LineStrips2D([[0, 0], [1, 1]])
    assert reference == dl.LineStrips2D(np.array([[0, 0], [1, 1]]))
    assert reference == dl.LineStrips2D([np.array([0, 0]), np.array([1, 1])])
