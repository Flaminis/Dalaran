...
import torch
from dalaran.archetypes.image import Image
from dalaran.datatypes.tensor_data import TensorData
...
IMAGE_INPUTS: list[Any] = [
    ...
    {"image": RANDOM_IMAGE_SOURCE, "color_model": dl.datatypes.ColorModel.RGB, "width": 20, "height": 10},
    {
    ...
    },
    {
        "bytes": RANDOM_IMAGE_SOURCE.tobytes(),
        "color_model": "RGB",
        "datatype": dl.datatypes.ChannelDatatype.F64,
        "width": 20,
        "height": 10,
    },
    {
...
def test_image_compress() -> None:
    ...
    compressed = dl.Image(image_data).compress(jpeg_quality=80)
    assert type(compressed) is dl.EncodedImage
    ...
    with pytest.warns(DalaranWarning) as warnings:
        ...
        compressed = dl.Image(image_data, "RGBA").compress(jpeg_quality=80)

        assert len(warnings) == 1
        assert "Cannot JPEG compress an image of type" in str(warnings[0])

        assert type(compressed) is dl.Image
