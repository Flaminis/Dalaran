...
def test_init_twice() -> None:
    ...
    dl.init("dalaran_example_test_app_id")
    recording_id = dl.get_recording_id()

    dl.init("dalaran_example_test_app_id")
    assert recording_id == dl.get_recording_id()

    # Using a custom recording id
    recording_id = "test_recording_id"
    dl.init("dalaran_example_test_app_id", recording_id=recording_id)
    dl.init("dalaran_example_test_app_id", recording_id=recording_id)
    assert recording_id == dl.get_recording_id()
...
def test_recording_stream_twice() -> None:
    ...
    with dl.RecordingStream("dalaran_example_bug") as rec:
...
def test_isolated_streams(tmp_path: Path) -> None:
    ...
    rec2 = dl.RecordingStream("dalaran_example_multi_stream", recording_id="rec2")
    rec2.log("/data2", dl.TextLog("Data2"))
    rec2.save(rec2_path)

    server = dl.server.Server(datasets={"test_dataset": tmp_path})
...
def test_cleanup_reinit() -> None:
    ...
    if system == "Linux":
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert not is_file_open(rec_path), "Recording file should be closed after calling dl.init"
