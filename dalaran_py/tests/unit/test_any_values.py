...
def test_any_value() -> None:
    values = dl.AnyValues(foo=[1.0, 2.0, 3.0], bar="hello")
    ...
    assert foo_batch.component_descriptor() == dl.ComponentDescriptor("foo")
    assert bar_batch.component_descriptor() == dl.ComponentDescriptor("bar")
...
def test_any_value_datatypes() -> None:
    values = dl.AnyValues(my_points=dl.datatypes.Vec2DBatch([(0, 1), (2, 3), (4, 5)]))

    batches = list(values.as_component_batches())

    foo_batch = batches[0]

    assert foo_batch.component_descriptor() == dl.ComponentDescriptor("my_points")
    assert len(foo_batch.as_arrow_array()) == 3
...
def test_bad_any_value() -> None:
    ...
    dl.set_strict_mode(False)
    with pytest.warns(DalaranWarning) as warnings:
        values = dl.AnyValues(bad_data=[Foo()])
    ...
    with pytest.warns(DalaranWarning) as warnings:
        values = dl.AnyValues(good_data=1)
        ...
        values = dl.AnyValues(good_data="foo")
    ...
    with pytest.warns(DalaranWarning, match="using the components argument"):
        non_keyword_arg = 1
        dl.AnyValues(non_keyword_arg)  # type: ignore[arg-type]

    with pytest.warns(DalaranWarning, match="Inconsistent with previous type provided."):
        dl.AnyValues(value=1)
        dl.AnyValues(value="1")
...
def test_none_any_value() -> None:
    ...
    with pytest.warns(DalaranWarning) as warnings:
        ...
        values = dl.AnyValues(none_data=None)
        assert len(warnings) == 0
        ...
        values = dl.AnyValues(none_data=None, drop_untyped_nones=False)
        running_warning_count += 1
        batches = list(values.as_component_batches())
        running_warning_count += 1
        ...
        # Log as not None
        values = dl.AnyValues(none_data=7, drop_untyped_nones=False)
        batches = list(values.as_component_batches())
...
def test_iterable_any_value() -> None:
    ...
    values = dl.AnyValues(str_values=SHORT_TEXT, bytes_values=SHORT_BYTES)
    batches = list(values.as_component_batches())
    ...
    values = dl.AnyValues(str_values=LONG_TEXT, bytes_values=LONG_BYTES)
    batches = list(values.as_component_batches())
    ...
    values = dl.AnyValues(str_values=[SHORT_TEXT, LONG_TEXT], bytes_values=[SHORT_BYTES, LONG_BYTES])
    batches = list(values.as_component_batches())
...
def test_any_values_with_field() -> None:
    dl.set_strict_mode(False)
    values = dl.AnyValues().with_component_from_data(descriptor="value", value=np.array([5], dtype=np.int64))
    assert values.as_component_batches()[0].component_descriptor() == dl.ComponentDescriptor("value")
...
def test_any_values_columns_scalar() -> None:
    cols = dl.AnyValues.columns(any_val_scalars=[1.0, 2.0, 3.0])
...
def test_any_values_columns_list_of_lists() -> None:
    cols = dl.AnyValues.columns(any_val_arrays=[[1, 2, 3], [4, 5], [6]])
    column_list = list(cols)
    assert len(column_list) == 1
...
def test_any_batch_value_type_registry_string(first_mode: str, second_mode: str) -> None:
    ...
    for i, mode in enumerate([first_mode, second_mode]):
        if mode == "row":
            batch = dl.AnyBatchValue(descriptor, row_values[i])
            assert batch.is_valid()
            assert batch.as_arrow_array().to_pylist() == [row_values[i]]  # type: ignore[union-attr]
        else:
            col = dl.AnyBatchValue.column(descriptor, col_values[i])
...
def test_any_batch_value_type_registry_column_flat_then_flat() -> None:
    ...
    col1 = dl.AnyBatchValue.column(descriptor, [1.0, 2.0, 3.0])
    assert col1 is not None
    assert col1.as_arrow_array().to_pylist() == [[1.0], [2.0], [3.0]]  # type: ignore[union-attr]

    col2 = dl.AnyBatchValue.column(descriptor, [4.0, 5.0])
...
def test_any_batch_value_type_registry_column_flat_then_jagged() -> None:
    ...
    col2 = dl.AnyBatchValue.column(descriptor, [[10, 20], [30]])
    assert col2 is not None
