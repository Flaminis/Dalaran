"""
Tests that a `DalaranIterableDataset` built from a `Manifest` replays a live (manifest-less) run exactly.
...
from dalaran.experimental.dataloader._sample_index import SampleIndex, SegmentMetadata
from dalaran.experimental.dataloader._utils import Target
from dalaran.experimental.dataloader.manifest._manifest import (
    MANIFEST_FORMAT_VERSION,
    ManifestMeta,
    _metadata_to_arrow,
)
from dalaran.experimental.dataloader.manifest._manifest_build import (
    _compact_index,
    _resolve_rows,
    _ResolvedRows,
    _sample_table,
...
if TYPE_CHECKING:
    ...
    from dalaran.experimental.dataloader._config import DataSource
    from dalaran.experimental.dataloader._shuffle import ShuffleStrategy
...
_SOURCE = cast(
    "DataSource",
    SimpleNamespace(
        dataset=SimpleNamespace(catalog=SimpleNamespace(url="dalaran+http://fake"), name="ds", id="id"),
    ),
)
...
def _build_live(strategy: ShuffleStrategy) -> DalaranIterableDataset:
    """A live (manifest-less) dataset over the same fake dataset, strategy, seed, and topology."""
    live = DalaranIterableDataset(
        _SOURCE,
        "t",
        _FIELDS,
        fetch_size=FETCH_SIZE,
...
def test_manifest_replays_live_order_exactly(monkeypatch: pytest.MonkeyPatch, strategy: ShuffleStrategy) -> None:
    ...
    manifest_order = [
        cast("int", s["anchor"])
        for s in DalaranIterableDataset.from_manifest(_build_manifest(strategy), _SOURCE, _FIELDS)
    ]
...
def test_manifest_replays_live_order_across_ranks(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    for rank in range(world_size):
        with monkeypatch.context() as m:
            ...
            manifest_order = [
                cast("int", s["anchor"]) for s in DalaranIterableDataset.from_manifest(manifest, _SOURCE, _FIELDS)
            ]
            live_order = [cast("int", s["anchor"]) for s in _build_live(strategy)]
...
def test_manifest_and_live_decode_identical_content(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    for sid, anchor in zip(SEGMENT_IDS, ANCHORS, strict=True):
        cols = by_segment.setdefault(sid, {"t": [], "dalaran_segment_id": [], "x": []})
        cols["t"].append(anchor)
        cols["dalaran_segment_id"].append(sid)
        cols["x"].append(anchor * 1.5)
    seg_tables = {
        "x": {
            sid: pa.table({
                "t": pa.array(cols["t"], pa.int64()),
                "dalaran_segment_id": pa.array(cols["dalaran_segment_id"], pa.string()),
    ...
    strategy = NoShuffle()
    manifest_samples = list(DalaranIterableDataset.from_manifest(_build_manifest(strategy), _SOURCE, _FIELDS))
