...
import pytest
from dalaran.experimental import MetricsCollector, QueryMetrics, query_metrics
from dalaran.experimental._query_metrics import _active_collectors
...
# lazily from `dalaran_bindings` on every call. Monkeypatching that symbol on
# `dalaran_bindings` is what gets picked up at scope entry — same pattern as
...
def install_fake_handles(monkeypatch: pytest.MonkeyPatch) -> Iterator[list[_FakeHandle]]:
    ...
    monkeypatch.setattr(dalaran_bindings, "_new_metrics_collector", factory)
    yield handles
...
def test_inert_fallback_on_import_error(
    ...
    If `dalaran_bindings` is missing the symbol, the context manager logs a
    ...
    monkeypatch.delattr(dalaran_bindings, "_new_metrics_collector", raising=False)
...
def test_allocation_failure_yields_inert_collector(
    ...
    monkeypatch.setattr(dalaran_bindings, "_new_metrics_collector", boom)

    with caplog.at_level("ERROR", logger="dalaran"):
        with query_metrics() as m:
            assert m.queries == []
            assert m.last_query() is None
