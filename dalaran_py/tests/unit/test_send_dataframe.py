"""Tests for dl.send_dataframe and dl.send_record_batch."""
...
APP_ID = "dalaran_example_test_send_dataframe"


def _filter_dalaran_columns(table: pa.Table) -> pa.Table:
    """Filter to only include columns with proper dalaran metadata (skip dalaran_segment_id)."""
    ...
    for field in table.schema:
        ...
        if field.metadata is None or b"dalaran:kind" not in field.metadata:
            continue
...
def test_send_dataframe_roundtrip(tmp_path: Path, snapshot: SnapshotAssertion) -> None:
    ...
    with dl.RecordingStream(APP_ID, recording_id=uuid.uuid4()) as rec:
        ...
        rec.log("points", dl.Points3D([[10, 11, 12]], colors=[[255, 0, 0]]))

    # Load via Server + Catalog API and read as Arrow table
    with dl.server.Server(datasets={"test_dataset": original_dir}) as server:
        ds = server.client().get_dataset("test_dataset")
        original_table = _filter_dalaran_columns(ds.reader(index="my_index").to_arrow_table())
    ...
    with dl.server.Server(datasets={"roundtrip_dataset": roundtrip_dir}) as server:
        ds = server.client().get_dataset("roundtrip_dataset")
        roundtrip_table = _filter_dalaran_columns(ds.reader(index="my_index").to_arrow_table())
    ...
    schema = pa.schema([
        ...
        pa.field(
            ...
            metadata={
                ...
                SORBET_ARCHETYPE_NAME: b"dalaran.archetypes.Points3D",
                SORBET_COMPONENT: b"Points3D:positions",
                SORBET_COMPONENT_TYPE: b"dalaran.components.Position3D",
                DALARAN_KIND: b"data",
            },
        ),
    ])
...
def test_control_kind_is_treated_as_row_id() -> None:
    ...
    assert "dalaran:component: C:c" in formatted
...
def test_no_component_type_is_left_unset(
    ...
    schema = pa.schema([
        ...
        pa.field(
            "/e:thing",
            _VALUES.type,
            metadata={SORBET_ENTITY_PATH: b"/e", SORBET_COMPONENT: b"thing", DALARAN_KIND: b"data"},
        ),
    ])
    [chunk] = send_dataframe_and_get_chunks(pa.Table.from_arrays([index, _VALUES], schema=schema))
    formatted = chunk.format(redact=True, trim_metadata_keys=False)
    assert "dalaran:component: thing" in formatted
    assert "dalaran:component_type" not in formatted
