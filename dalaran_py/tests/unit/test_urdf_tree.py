...
def test_urdf_tree_transform() -> None:
    ...
    assert transform.parent_frame == dl.components.TransformFrameIdBatch(
        dl.components.TransformFrameId(joint.parent_link)
    )
    assert transform.child_frame == dl.components.TransformFrameIdBatch(
        dl.components.TransformFrameId(joint.child_link)
    )
...
def test_urdf_tree_frame_prefix() -> None:
    ...
    assert transform.parent_frame == dl.components.TransformFrameIdBatch(
        dl.components.TransformFrameId(f"{prefix}{joint.parent_link}")
    )
    assert transform.child_frame == dl.components.TransformFrameIdBatch(
        dl.components.TransformFrameId(f"{prefix}{joint.child_link}")
    )
    ...
    from dalaran._baseclasses import ComponentColumnList
...
def test_urdf_tree_log() -> None:
    rec = dl.RecordingStream("dalaran_example_test_urdf_tree_log", make_default=False, make_thread_default=False)
...
def test_urdf_tree_custom_static_transform_entity_path(tmp_path: Path) -> None:
    dlr_path = tmp_path / "urdf_static_transforms.dlr"
    ...
    paths = RrdReader(dlr_path).stream().collect().schema().entity_paths()
...
def test_urdf_transform_batches_in_lens_pipeline() -> None:
    ...
    output_transforms = (
        ...
        .to_component(dl.Transform3D.descriptor_translation(), Selector(".[].translation"))
        .to_component(dl.Transform3D.descriptor_quaternion(), Selector(".[].quaternion"))
        .to_component(dl.Transform3D.descriptor_parent_frame(), Selector(".[].parent_frame"))
        .to_component(dl.Transform3D.descriptor_child_frame(), Selector(".[].child_frame"))
    )
...
def assert_quat_equivalent(actual: dl.components.RotationQuatBatch, expected: list[float]) -> None:
...
def assert_translation_expected(actual: dl.components.Translation3DBatch, expected: tuple[float, float, float]) -> None:
    actual_values = actual.pa_array.to_pylist()[0]
    assert actual_values == pytest.approx(expected)
