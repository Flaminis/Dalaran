...
def test_archetype_builder() -> None:
    ...
    values = dl.DynamicArchetype(archetype=archetype, components={"foo": [1.0, 2.0, 3.0], "bar": "hello"})
    values.with_component_override("baz", dl.components.ScalarBatch._COMPONENT_TYPE, [1.2, 3.4, 5.6])
    ...
    assert foo_batch.component_descriptor() == dl.ComponentDescriptor("foo").with_builtin_archetype(archetype)
    assert bar_batch.component_descriptor() == dl.ComponentDescriptor("bar").with_builtin_archetype(archetype)
    assert baz_batch.component_descriptor() == dl.ComponentDescriptor(
        "baz", component_type=dl.components.ScalarBatch._COMPONENT_TYPE
...
def test_dynamic_archetype_columns_scalar() -> None:
    cols = dl.DynamicArchetype.columns(
