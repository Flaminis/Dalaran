...
def test_component_column_selector_explicit() -> None:
    ...
    assert selector.entity_path == dl.datatypes.EntityPath("entity/path")
    assert selector.component == dl.datatypes.Utf8("Component")
...
def test_dataframe_query_property() -> None:
    ...
    assert query.filter_by_range == blueprint_components.FilterByRangeBatch(
        blueprint_components.FilterByRange(dl.datatypes.TimeInt(seq=1), dl.datatypes.TimeInt(seq=10)),
    )
