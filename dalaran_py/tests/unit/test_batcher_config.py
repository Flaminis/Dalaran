...
def log_and_get_size(rec: dl.RecordingStream, path: str) -> int:
    """Helper function to log a message and sleep for a short duration."""
    rec.log("/data1", dl.Scalars(1))
    time.sleep(0.1)
...
def test_partial_overrides() -> None:
    ...
    with patch.dict(os.environ, {"DALARAN_FLUSH_TICK_SECS": "42", "DALARAN_FLUSH_NUM_ROWS": "666"}):
        assert "DALARAN_FLUSH_TICK_SECS" in os.environ
        assert "DALARAN_FLUSH_NUM_ROWS" in os.environ
    ...
    assert "DALARAN_FLUSH_TICK_SECS" not in os.environ
    assert "DALARAN_FLUSH_NUM_ROWS" not in os.environ
