...
from dalaran.datatypes import DVec2D, Float32ArrayLike, Rgba32ArrayLike
...
def test_geo_line_strings() -> None:
    ...
    for strips, radii, colors in all_arrays:
        ...
        print(f"dl.GeoLineStrings(\n    lat_lon={strips}\n    radii={radii!r}\n    colors={colors!r}\n)")
        arch = dl.GeoLineStrings(
            lat_lon=strips,
            radii=radii,
            colors=colors,
        )
...
def test_geo_line_strings_segment(data: GeoLineStringArrayLike) -> None:
    arch = dl.GeoLineStrings(lat_lon=data)
...
def test_geo_line_strings_single_line() -> None:
    ...
    reference = dl.GeoLineStrings(lat_lon=[dl.components.GeoLineString(lat_lon=[[0, 0], [1, 1]])])
    assert reference.line_strings is not None and len(reference.line_strings) == 1
    assert reference == dl.GeoLineStrings(lat_lon=dl.components.GeoLineString(lat_lon=[[0, 0], [1, 1]]))
    assert reference == dl.GeoLineStrings(lat_lon=[[[0, 0], [1, 1]]])
    assert reference == dl.GeoLineStrings(lat_lon=[[0, 0], [1, 1]])
    assert reference == dl.GeoLineStrings(lat_lon=np.array([[0, 0], [1, 1]]))
    assert reference == dl.GeoLineStrings(lat_lon=[np.array([0, 0]), np.array([1, 1])])
