...
if TYPE_CHECKING:
    from dalaran.datatypes import ClassIdArrayLike, Float32ArrayLike, Rgba32ArrayLike, Utf8ArrayLike, Vec2DArrayLike
...
def test_with_sizes() -> None:
    assert dl.Boxes2D(sizes=[1, 2]) == dl.Boxes2D(half_sizes=[0.5, 1])


def test_with_centers_and_sizes() -> None:
    assert dl.Boxes2D(centers=[1, 2], sizes=[4, 6]) == dl.Boxes2D(centers=[1, 2], half_sizes=[2, 3])


def test_with_mins_and_sizes() -> None:
    assert dl.Boxes2D(mins=[-1, -1], sizes=[2, 4]) == dl.Boxes2D(centers=[0, 1], half_sizes=[1, 2])


def test_with_array_xywh() -> None:
    assert dl.Boxes2D(mins=[1, 2], sizes=[3, 4]) == dl.Boxes2D(array=[1, 2, 3, 4], array_format=dl.Box2DFormat.XYWH)


def test_with_array_yxhw() -> None:
    assert dl.Boxes2D(mins=[1, 2], sizes=[3, 4]) == dl.Boxes2D(array=[2, 1, 4, 3], array_format=dl.Box2DFormat.YXHW)


def test_with_array_xyxy() -> None:
    assert dl.Boxes2D(mins=[1, 2], sizes=[2, 2]) == dl.Boxes2D(array=[1, 2, 3, 4], array_format=dl.Box2DFormat.XYXY)


def test_with_array_yxyx() -> None:
    assert dl.Boxes2D(mins=[1, 2], sizes=[2, 2]) == dl.Boxes2D(array=[2, 1, 4, 3], array_format=dl.Box2DFormat.YXYX)


def test_with_array_xcycwh() -> None:
    assert dl.Boxes2D(mins=[1, 1], sizes=[2, 4]) == dl.Boxes2D(array=[2, 3, 2, 4], array_format=dl.Box2DFormat.XCYCWH)


def test_with_array_xcycw2h2() -> None:
    assert dl.Boxes2D(mins=[1, 1], sizes=[2, 4]) == dl.Boxes2D(array=[2, 3, 1, 2], array_format=dl.Box2DFormat.XCYCW2H2)
...
def test_with_array_types(array: npt.ArrayLike) -> None:
    assert dl.Boxes2D(mins=[1, 2], sizes=[3, 4]) == dl.Boxes2D(array=array, array_format=dl.Box2DFormat.XYWH)
...
def test_invalid_parameter_combinations() -> None:
    ...
    with pytest.raises(ValueError):
        dl.Boxes2D(half_sizes=[1, 2], sizes=[3, 4])
    with pytest.raises(ValueError):
        dl.Boxes2D(centers=[1, 2], mins=[3, 4])
    with pytest.raises(ValueError):
        dl.Boxes2D(mins=[3, 4])

    # with array
    with pytest.raises(ValueError):
        dl.Boxes2D(array=[3, 4, 5, 6])
    with pytest.raises(ValueError):
        dl.Boxes2D(array=[3, 4, 5, 6], array_format=dl.Box2DFormat.XYWH, half_sizes=[1, 2])
    with pytest.raises(ValueError):
        dl.Boxes2D(array=[3, 4, 5, 6], array_format=dl.Box2DFormat.XYWH, sizes=[1, 2])
    with pytest.raises(ValueError):
        dl.Boxes2D(array=[3, 4, 5, 6], array_format=dl.Box2DFormat.XYWH, mins=[1, 2])
    with pytest.raises(ValueError):
        dl.Boxes2D(array=[3, 4, 5, 6], array_format=dl.Box2DFormat.XYWH, centers=[1, 2])
    with pytest.raises(ValueError):
        dl.Boxes2D(array=[3, 4, 5, 6], array_format="bonkers")  # type: ignore[arg-type]
