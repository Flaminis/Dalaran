...
def test_uuid() -> None:
    ...
    expected = dl.datatypes.UuidBatch([uuid_bytes0]).as_arrow_array()
    for uuid in uuids:
        assert dl.datatypes.UuidBatch([uuid]).as_arrow_array() == expected  # type: ignore[arg-type]

    # Several uuids.
    for uuids in uuids_arrays:  # type: ignore[assignment]
        expected = uuids_expected(uuids)
        assert dl.datatypes.UuidBatch(uuids).as_arrow_array() == expected.as_arrow_array()  # type: ignore[arg-type]
