...
import pytest
from dalaran._tracing_session import (
...
def test_logs_session_id_at_scope_entry(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # The context manager imports its bindings lazily from `dalaran_bindings`, so
    ...
    monkeypatch.setattr(dalaran_bindings, "_is_telemetry_active", lambda: True)
    monkeypatch.setattr(dalaran_bindings, "_log_tracing_session_started", fake_log)
    monkeypatch.setattr(dalaran_bindings, "_log_tracing_session_finished", lambda *_args: None)
...
def test_logs_metrics_at_scope_exit(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(dalaran_bindings, "_is_telemetry_active", lambda: True)
    monkeypatch.setattr(dalaran_bindings, "_log_tracing_session_started", lambda _sid: None)
    monkeypatch.setattr(
        dalaran_bindings,
...
def test_psutil_failure_does_not_propagate(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    import dalaran._tracing_session as ts
...
def test_finished_log_failure_does_not_propagate(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(dalaran_bindings, "_is_telemetry_active", lambda: True)
    monkeypatch.setattr(dalaran_bindings, "_log_tracing_session_started", lambda _sid: None)
    monkeypatch.setattr(dalaran_bindings, "_log_tracing_session_finished", boom)
...
def test_warns_and_no_ops_when_telemetry_inactive(
    ...
    monkeypatch.setattr(dalaran_bindings, "_is_telemetry_active", lambda: False)

    with caplog.at_level("WARNING", logger="dalaran"):
        with tracing_session() as sid:
            assert sid == ""
...
def test_nested_sessions_shadow_and_restore(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    var = dalaran_bindings._get_tracing_session_var()
