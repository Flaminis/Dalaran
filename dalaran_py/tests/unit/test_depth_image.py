...
def depth_image_expected() -> Any:
    return dl.DepthImage(RANDOM_IMAGE_SOURCE, meter=1000)
...
def test_depth_image() -> None:
    ...
    for img, meter, depth_range in itertools.zip_longest(IMAGE_INPUTS, METER_INPUTS, ranges):
        ...
        arch = dl.DepthImage(img, meter=meter, depth_range=depth_range)

        assert arch.buffer == dl.components.ImageBufferBatch._converter(img.tobytes())
        assert arch.format == dl.components.ImageFormatBatch._converter(
            ImageFormat(
                width=img.shape[1],
                height=img.shape[0],
        ...
        assert arch.meter == dl.components.DepthMeterBatch._converter(meter)
        assert arch.depth_range == dl.components.ValueRangeBatch._converter(depth_range)
...
def test_depth_image_compress() -> None:
    ...
    compressed = dl.DepthImage(depth_data, meter=1000).compress()
    assert type(compressed) is dl.EncodedDepthImage

    # U8 supported
    depth_data = np.asarray(rng.uniform(0, 255, (10, 20)), dtype=np.uint8)
    compressed = dl.DepthImage(depth_data).compress()
    assert type(compressed) is dl.EncodedDepthImage
    ...
    with pytest.warns(DalaranWarning) as warnings:
        ...
        assert type(compressed) is dl.DepthImage
...
def test_depth_image_compress_reduces_size() -> None:
    ...
    compressed = dl.DepthImage(gradient_u16, meter=1000).compress()
    assert type(compressed) is dl.EncodedDepthImage
    ...
    compressed = dl.DepthImage(constant_u16).compress()
    compressed_size = _compressed_blob_size(compressed)
    ...
    compressed = dl.DepthImage(gradient_u8).compress()
    compressed_size = _compressed_blob_size(compressed)
    assert compressed_size < raw_size, (
    ...
    compressed = dl.DepthImage(stepped_u16).compress()
    compressed_size = _compressed_blob_size(compressed)
    assert compressed_size < raw_size, (
...
def test_depth_image_compress_level() -> None:
    ...
    size_level_0 = _compressed_blob_size(dl.DepthImage(gradient_u16).compress(compress_level=0))
    size_level_9 = _compressed_blob_size(dl.DepthImage(gradient_u16).compress(compress_level=9))
