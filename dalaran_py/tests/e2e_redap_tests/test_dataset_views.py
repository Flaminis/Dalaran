...
if TYPE_CHECKING:
    ...
    import datafusion
    from dalaran.catalog import DatasetEntry, IndexValuesLike
...
def test_dataset_view_filter_segments_with_dataframe(
    ...
    segments.append(
        dalaran_segment_id=test_segments,
    )
...
def test_dataframe_api_using_index_values_partial_overlap(
    ...
    table = pa.table(df)
    seg_col = table.column("dalaran_segment_id").to_pylist()
...
def test_dataframe_api_using_index_values_dataframe(
    ...
    df = (
        ...
        .select("dalaran_segment_id", "time_1", "/obj2:Points3D:positions")
        .sort("dalaran_segment_id", "time_1")
    )
