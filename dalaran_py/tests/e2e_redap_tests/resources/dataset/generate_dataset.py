...
def generate_data(filename: str, n_rows: int) -> None:
    ...
    for idx in range(0, n_rows):
        ...
        if timestamp is not None:
            dl.set_time("time_1", timestamp=timestamp)

        if duration is not None:
            dl.set_time("time_2", duration=duration)

        if sequence is not None:
            dl.set_time("time_3", sequence=sequence)

        if obj1_pos is not None:
            dl.log("/obj1", dl.Points3D([[obj1_pos, 0.0, 0.0]]))

        if obj2_pos is not None:
            dl.log("/obj2", dl.Points3D([[obj2_pos, 1.0, 0.0]]))

        if obj3_pos is not None:
            dl.log("/obj3", dl.Points3D([[obj3_pos, 2.0, 0.0]]))

    dl.log("/text2", dl.TextDocument("After text"), static=True)
