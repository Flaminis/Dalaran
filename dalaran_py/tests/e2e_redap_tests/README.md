...
End-to-end test suite for redap (Dalaran Data Protocol).

## Overview

This test suite exercises the full redap stack by using the Python SDK (primarily `CatalogClient`) against a live Dalaran server.
...
```bash
pixi run uvpy -m pytest -c dalaran_py/pyproject.toml dalaran_py/tests/e2e_redap_tests
```

Run against an external redap server:
```bash
pytest -c dalaran_py/pyproject.toml dalaran_py/tests/e2e_redap_tests --redap-url=dalaran+http://localhost:51234
```

With authentication:
```bash
pytest -c dalaran_py/pyproject.toml dalaran_py/tests/e2e_redap_tests --redap-url=dalaran+https://example.com --redap-token=your_token
```

Skip local-only tests (useful for Docker/containerized environments):
```bash
pytest -c dalaran_py/pyproject.toml dalaran_py/tests/e2e_redap_tests -m "not local_only"
```
...
In Dalaran's internal CI, this suite runs in several OSS↔Hub compatibility configurations:
...
- `@pytest.mark.min_hub_version("0.16.0")` — for behavior changes without a feature flag.
  Only enforced when the target is a hub (any profile except `local`), because the OSS server is versioned on the dalaran scheme, not the hub scheme.
