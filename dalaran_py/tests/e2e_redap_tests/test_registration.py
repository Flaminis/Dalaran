...
def test_register_unregister_batch(
    ...
    segment_ids = table.column("dalaran_segment_id").to_pylist()
    assert sorted(segment_ids) == sorted(recording_ids)

    df = ds.filter_contents("/points").reader(index="log_time").sort("dalaran_segment_id").drop("log_time")
...
├╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌┤
│ ┌──────────────────────────────────────┬──────────────────────┬─────────────────────────────────────────────────┐ │
│ │ dalaran_segment_id                     ┆ log_tick             ┆ /points:Points2D:positions                      │ │
│ │ ---                                  ┆ ---                  ┆ ---                                             │ │
...
def test_register_conflicting_schema(entry_factory: EntryFactory, tmp_path: Path) -> None:
    ...
    with dl.RecordingStream("dalaran_example_conflicting_schema", recording_id="segment1") as rec:
        rec.save(seg_1_path)
        rec.log("/data", dl.AnyValues(test=pa.array([1.0, 2.0, 3.0], type=pa.float64())))

    with dl.RecordingStream("dalaran_example_conflicting_schema", recording_id="segment2") as rec:
        rec.save(seg_2_path)
        rec.log("/data", dl.AnyValues(test=pa.array([1.0, 2.0, 3.0], type=pa.float32())))
...
def test_register_conflicting_property_schema(entry_factory: EntryFactory, tmp_path: Path) -> None:
    ...
    with dl.RecordingStream("dalaran_example_conflicting_schema", recording_id="segment1") as rec:
        rec.save(seg_1_path)
        rec.send_property("prop", dl.AnyValues(test=pa.array([1.0, 2.0, 3.0], type=pa.float64())))

    with dl.RecordingStream("dalaran_example_conflicting_schema", recording_id="segment2") as rec:
        rec.save(seg_2_path)
        rec.send_property("prop", dl.AnyValues(test=pa.array([1.0, 2.0, 3.0], type=pa.float32())))
...
def test_failed_layer_registration_not_in_segment_table(entry_factory: EntryFactory, tmp_path: Path) -> None:
    ...
    table = df.to_arrow_table()
    segment_ids = table.column("dalaran_segment_id").to_pylist()
    layer_names = table.column("dalaran_layer_names").to_pylist()
...
def test_registration_crossregion(catalog_client: CatalogClient) -> None:
    ...
    usw2 = "s3://dalaran-redap-datasets-pdx/test-resources/dataset/"
    use1 = "s3://dalaran-redap-datasets/test-resources/dataset/"
...
def _write_dlr(path: Path, recording_id: str, column_type: object) -> None:
    ...
    with dl.RecordingStream("dalaran_example_schema_contract", recording_id=recording_id) as rec:
        rec.save(path)
        rec.log("/data", dl.AnyValues(value=data))
...
def _schema_contract_cases() -> list[tuple[str, list[object], str]]:
    ...
    # sequence-to-array path for unions, and Dalaran's SDK doesn't emit them.
