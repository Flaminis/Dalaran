...
def test_df_filters(catalog_client: CatalogClient, readonly_test_dataset: DatasetEntry) -> None:
    ...
    all_segments = (
        readonly_test_dataset.reader(index=None).select("dalaran_segment_id").sort(col("dalaran_segment_id")).collect()
    )
    ...
    def find_time_boundaries(time_index: str, segment: pa.Scalar) -> list[pa.Scalar]:
        ...
        rbs = (
            ...
            .filter(col("dalaran_segment_id") == segment)
            .select(time_index)
            .sort(col(time_index))
            .collect()
        )
    ...
    def generate_tests(time_index: str, segments: list[pa.Scalar]) -> list[tuple[Expr, str]]:
        ...
        return [
            ...
            (col("dalaran_segment_id") == segments[0], _PUSHABLE),
            (col("dalaran_segment_id") == segments[1], _PUSHABLE),
            # Negated in_list: not a simple set membership, may not push.
            (f.in_list(col("dalaran_segment_id"), [lit(segments[0]), lit(segments[1])], negated=True), _UNCERTAIN),
            (f.in_list(col("dalaran_segment_id"), [lit(segments[0]), lit(segments[1])], negated=False), _PUSHABLE),
            # Segment + time AND combinations — each conjunct is pushable.
            ((col("dalaran_segment_id") == segments[0]) & (col(time_index) == s1_lower), _PUSHABLE),
            ((col("dalaran_segment_id") == segments[0]) & (col(time_index) > s1_lower), _PUSHABLE),
            ((col("dalaran_segment_id") == segments[0]) & (col(time_index) >= s1_lower), _PUSHABLE),
            ((col("dalaran_segment_id") == segments[0]) & (col(time_index) < s1_lower), _PUSHABLE),
            ((col("dalaran_segment_id") == segments[0]) & (col(time_index) <= s1_lower), _PUSHABLE),
            ...
            ((col("dalaran_segment_id") == segments[0]) & (col(time_index) < s1_min), _PUSHABLE),
            ((col("dalaran_segment_id") == segments[0]) & (col(time_index) > s1_max), _PUSHABLE),
            # OR combinations — pushdown behavior depends on the optimizer's
            # disjunction handling. Don't pin a side.
            ((col("dalaran_segment_id") == segments[0]) | (col("dalaran_segment_id") == segments[1]), _UNCERTAIN),
            (
                ((col("dalaran_segment_id") == segments[0]) & (col(time_index) > s1_lower))
                | ((col("dalaran_segment_id") == segments[1]) & (col(time_index) < s2_lower)),
                _UNCERTAIN,
            ),
            # Edge cases
            ...
            (f.substring(col("dalaran_segment_id"), lit(2), lit(3)) == "some_value", _UNCERTAIN),
        ]
    ...
    for time_idx in ["time_2", "time_3"]:
        ...
        for test_filter, pushdown_class in all_tests:
            ...
            # Dalaran-side read in `query_metrics()` so `m.last_query()`
