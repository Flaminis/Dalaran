...
if TYPE_CHECKING:
    ...
    from dalaran.catalog import DatasetEntry, VersionInfo
...
def pytest_configure(config: pytest.Config) -> None:
    ...
    config.addinivalue_line(
        "markers",
        "min_hub_version(version): skip when running against a Dalaran Hub older than `version`. "
...
def _server_compat_gate(request: pytest.FixtureRequest) -> None:
    ...
    # The OSS server reports dalaran versions (e.g. 0.36.x) while the hub has its own versioning
