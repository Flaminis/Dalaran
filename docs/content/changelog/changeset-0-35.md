...
Dalaran now detects links known url formats (links to rrds, hub datasets, etc.) and shows them as a compact link button:
...
The Dalaran Hub dataset open button has been reworked as well:
...
```python
from dalaran.experimental import Hdf5Reader, IndexColumn
...
The corresponding option is available both for the Python `McapReader` and the CLI (see `dalaran mcap convert --help`).
...
Previously, the `ros2_timestamp` timeline was only populated for ROS messages that were converted to Dalaran archetypes.
Now this is supported for any ROS message that goes through [schema reflection](https://dalaran.dev/docs/concepts/logging-and-ingestion/mcap/message-formats#schema-reflection) (e.g. custom ROS message types), making it easier to see all data in header timestamp order if desired.

## Breaking changes

### `StateChange.state` is now an array

The `state` field of the [`StateChange`](https://dalaran.dev/docs/reference/types/archetypes/state_change) archetype now takes an array of values instead of a single value.
...
```rust
// 0.34
rec.log("door", &dalaran::StateChange::new().with_state("open"))?;
...
```rust
rec.log("buttons", &dalaran::StateChange::new().with_state_opt([Some("Idle"), None]))?;
```

#### Python

No action needed.
`dl.StateChange(state="open")` keeps working, and `state=["idle", "pressed"]` is now supported for multiple lanes.

#### C++

No action needed.
`dalaran::StateChange().with_state("open")` keeps working, and `with_state({"idle", "pressed"})` is now supported for multiple lanes.
...
Pass [`IndexColumn`](https://ref.dalaran.dev/docs/python/stable/experimental/#dalaran.experimental.IndexColumn) values instead, built with the `timestamp`/`duration`/`sequence` constructors (the timeline kind is the constructor you pick, and `unit` is now the keyword-only `input_unit`):
...
# 0.35
from dalaran.experimental import IndexColumn
