...
Internally, the [`Importer`](https://docs.rs/dl_importer/latest/dl_importer/trait.Importer.html) trait takes care of loading files into the Viewer and/or SDK.
...
Once notified, an `Importer` can return an [`ImporterError::Incompatible`](https://docs.rs/dl_importer/latest/dl_importer/enum.ImporterError.html#variant.Incompatible) error to indicate that it doesn't support a given file type.
