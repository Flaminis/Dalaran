...
title: DLR format
order: 725
description: How chunks are framed and serialized in .dlr files
---

An DLR is the file format Dalaran uses to persist recordings and blueprints. At the lowest level it is a linear sequence of framed messages — store announcements and chunks of data — optionally followed by a footer index that makes random access cheap. This page covers the envelope around chunks and how they are serialized; the chunk data model itself is described in [Chunks](chunks.md).
...
A single DLR can hold any number of stores.
The file extension is either `.dlr` or `.dbl`.
Both refer to the exact same on-disk format and are used conventionally:
- `.dlr` files hold any combination of recording and blueprint stores;
- `.dbl` files hold a single blueprint store.


## Message kinds (`LogMsg`)

The body of an DLR is a sequence of `LogMsg`s. There are three variants:
...
- **`ArrowMsg`** carries the actual data: an [Apache Arrow IPC](https://arrow.apache.org/docs/format/Columnar.html#ipc-streaming-format) payload encoding a single chunk, tagged with the `StoreId` it belongs to.
  This is what makes up the bulk of every DLR.
- **`BlueprintActivationCommand`** is the only non-data control message.
  It is emitted after a blueprint's chunks have been sent, and lets the producer atomically activate the blueprint via the [`make_active` / `make_default`](https://ref.dalaran.dev/docs/python/stable/blueprint/) flags.
...
Every `ArrowMsg` carries a single **chunk** — an Apache Arrow `RecordBatch` with Dalaran-specific schema metadata. A chunk belongs to one entity path and holds a contiguous run of rows for that entity, with one column per timeline and one column per component. See [Chunks](chunks.md) for the conceptual deep-dive (how chunks are built, batched, sorted, compacted); this section just shows what a chunk looks like when you crack one open.

The schema is laid out per **Sorbet**, Dalaran's object-model spec — it defines how chunks, archetypes, components, and timelines map onto Arrow column names, types, and metadata. The easiest way to see it concretely is to save a recording and reopen it with [`RrdReader`](https://ref.dalaran.dev/docs/python/stable/experimental/#dalaran.experimental.RrdReader).

First, let's create an DLR file with some content:

snippet: concepts/dlr_format[write]

Then we can inspect the first chunk it contains:

snippet: concepts/dlr_format[inspect]

> [!NOTE]
> By default, `chunk.format()` trims metadata keys to keep the representation concise.
> Using `trim_metadata_keys=False` disables this behavior, so the typical `dalaran:` / `sorbet:` prefixes are visible here.
...
│ * dalaran:id: chunk_18B0AA9FA7B7B1A61d23c55ca87b18b4                                                                                                                                                   │
...
│ │ ARROW:extension:metadata:           ┆ dalaran:index_name: frame ┆ dalaran:index_name: log_tick ┆ dalaran:index_name: log_time ┆ dalaran:archetype: Points3D        ┆ non-null Float32))                  │ │
│ │ {"namespace":"row"}                 ┆ dalaran:is_sorted: true   ┆ dalaran:is_sorted: true      ┆ dalaran:is_sorted: true      ┆ dalaran:component: Points3D:colors ┆ dalaran:archetype: Points3D           │ │
│ │ ARROW:extension:name: TUID          ┆ dalaran:kind: index       ┆ dalaran:kind: index          ┆ dalaran:kind: index          ┆ dalaran:component_type: Color      ┆ dalaran:component: Points3D:positions │ │
│ │ dalaran:is_sorted: true               ┆                         ┆                            ┆                            ┆ dalaran:kind: data                 ┆ dalaran:component_type: Position3D    │ │
│ │ dalaran:kind: control                 ┆                         ┆                            ┆                            ┆                                  ┆ dalaran:kind: data                    │ │
│ ╞═════════════════════════════════════╪═════════════════════════╪════════════════════════════╪════════════════════════════╪══════════════════════════════════╪═════════════════════════════════════╡ │
...
- All Dalaran-specific metadata keys are prefixed with `dalaran:` (`dalaran:entity_path`, `dalaran:id`, `dalaran:kind`, `dalaran:index_name`, …). Sorbet's own metadata uses the `sorbet:` prefix (`sorbet:version`).
- The chunk-level **metadata** identifies the entity path the chunk belongs to and the chunk's id.
- The **`RowId`** column is the row identity column (`dalaran:kind: control`).
...
Every store in an DLR is identified by a `StoreId` and described by a `StoreInfo`:

- **`StoreId`** combines:
    - **`kind`** — `Recording` or `Blueprint`. The on-disk format treats both identically; the kind is just a flag. What differs is the *expected content*: recordings hold user-logged data on user-defined entity paths, blueprints hold `dl.blueprint.*` objects on Viewer-reserved paths. The Viewer dispatches on the kind — recordings populate the data store, blueprints populate the Viewer's UI/layout state.
...
- **`StoreInfo`** wraps the `StoreId` and adds:
    ...
    - **`store_version`** — the Dalaran version that produced the data.

Matching `application_id` and `recording_id` is how the Viewer merges multiple `.dlr` files (or multiple stores within one file) into a single logical recording.

`.dbl` is just an DLR whose store happens to have `kind = Blueprint` — nothing in the bytes makes it special.
The convention of using `.dbl` for blueprints instead of `.dlr` is purely a filename hint to the Viewer and users.

When an DLR holds multiple [stores](#stores) each store begins with its own `SetStoreInfo`, and every subsequent `ArrowMsg` is tagged with its store's `StoreId`.
...
The footer is an optional manifest appended at the end of an DLR that enables random access into the file.
For each chunk in the DLR, the manifest carries chunk-level metadata (id, byte offset in the file, byte size — compressed and uncompressed) along with per-component and per-timeline statistics and the chunk's schema hash.
Like all data in Dalaran, the manifest is internally stored as an Arrow `RecordBatch`, with one row per chunk.
...
All tooling included in recent versions of the Dalaran SDK emit footers by default.
An DLR may still miss a footer for a variety of reasons — for example, when a stream is not shut down cleanly, or legacy RRDs written before footers existed.
In those cases, readers fall back to a linear scan, which is semantically equivalent — just slower for partial reads.

For illustration, let's see what a footer looks like in an DLR.
This can be done with the following command:

```sh
dalaran dlr print --footers --footers-lod 2 my.dlr
```
...
```text
Showing data after migration to latest Dalaran version
StoreInfo {
    store_id: StoreId(
        Recording,
        "dalaran_example_dlr_format",
        "example",
    ),
    cloned_from: None,
...
The high-level shape of any DLR is:
...
Every DLR opens with the same fixed 12 bytes:
...
| `[4..8)`   | Version     | 4-byte encoded Dalaran crate version                          |
| `[8]`      | compression | `0` = Off, `1` = LZ4                                        |
| `[9]`      | serializer  | `2` = Protobuf (`1` once meant MsgPack and is now rejected) |
| `[10..12)` | reserved    | `0x00 0x00`                                                 |

- Older `"RRF0"` / `"RRF1"` FourCCs are recognized but rejected with `OldRrdVersion` — there is no in-place reader for them, you have to migrate through an older Dalaran release.
...
> [!NOTE]
> The header format exposes legacy details that are no longer supported and may require an older Dalaran SDK version to migrate.
> However, RRDs created by Dalaran SDK 0.23 and later are guaranteed to be migrated, and this guarantee holds for future releases — see the next section.
...
| `[16..20)` | `crc32` | u32 LE — `xxh32(payload)` with the fixed seed `7850921` (`"DALARAN"` in base-26) |
...
A file can legally have more than one trailer — that happens when streams are simply concatenated (`cat a.dlr b.dlr > both.dlr`), but tools like `dalaran dlr merge` collapse them back into a single trailer with a single entry.
...
This concerns the binary structure of the DLR file.
...
That's the cut-off below which we do not attempt to migrate at load time; a manual hop through an older Dalaran SDK release is required.
...
The `dalaran dlr migrate` CLI will also be available for offline batch conversion.

### Sorbet

We refer to the high-level data model specification as Sorbet.
Its reference implementation lives in the Rust `dl_sorbet` crate.
...
Sorbet is versioned and **subject to change**, but `dl_sorbet` performs in-memory migration to the current Sorbet version as chunks (and the footer manifest) are loaded.
Any CLI tool that rewrites an DLR (`dalaran dlr merge`, `dalaran dlr optimize`, `dalaran dlr migrate`, …) emits chunks in the current Sorbet version, so a round-trip through any of these is also a migration.
