...
An RRD is the file format Dalaran uses to persist recordings and blueprints. At the lowest level it is a linear sequence of framed messages — store announcements and chunks of data — optionally followed by a footer index that makes random access cheap. This page covers the envelope around chunks and how they are serialized; the chunk data model itself is described in [Chunks](chunks.md).
...
```sh
dalaran rrd print --footers --footers-lod 2 my.rrd
```
...
A file can legally have more than one trailer — that happens when streams are simply concatenated (`cat a.rrd b.rrd > both.rrd`), but tools like `dalaran rrd merge` collapse them back into a single trailer with a single entry.
...
The `dalaran rrd migrate` CLI will also be available for offline batch conversion.
...
Any CLI tool that rewrites an RRD (`dalaran rrd merge`, `dalaran rrd optimize`, `dalaran rrd migrate`, …) emits chunks in the current Sorbet version, so a round-trip through any of these is also a migration.
