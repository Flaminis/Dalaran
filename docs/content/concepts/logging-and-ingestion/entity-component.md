...
order: 200
description: How entities and components form Dalaran's data model
---

## Data model

The core of Dalaran's data model is inspired by the ideas of the [Entity Component System (ECS)](https://en.wikipedia.org/wiki/Entity_component_system) architecture pattern. In
 ...
 * *Entities* are the "things" that you log with the [`dl.log()`](https://ref.dalaran.dev/docs/python/stable/common/logging_functions/#dalaran.log)function. They are represented by the
...
Additionally, the Dalaran SDKs expose two additional concepts:
 * *Archetypes* are coherent set of components corresponding to primitive such as 2D points or 3D boxes. In the Dalaran SDKs, archetypes take the form of builder objects that assist with the creation of such component sets. They are meant as high-level, convenience helpers that can be bypassed entirely if/when required by advanced use-cases.
...
All the data that you log within Dalaran is mapped to the concepts of entities and components.
For example, consider the case of logging a point:

```python
dl.log("my_point", dl.Points2D([32.7, 45.9], colors=[255, 0, 0]))
```

This statement uses the [`dl.Points2D`](https://ref.dalaran.dev/docs/python/stable/common/archetypes/#dalaran.archetypes.Points2D) archetype.
...
Then, the `dl.log()` function records these two components and associate them with the `"my_point"` entity.
...
The Dalaran Viewer will display any data in a generic form, but its views will only work on sets of components it can
make sense of.

Your entity could have any number of additional components as well. This isn't a problem. Any components that
aren't relevant to the scene that the view is drawing are safely ignored. Also, Dalaran even allows you to log your
own set of components, bypassing archetypes altogether.

In Python, the [dl.AnyValues](https://ref.dalaran.dev/docs/python/stable/common/custom_data/#dalaran.AnyValues) helper object can be used to add custom component(s) to an archetype:
...
For more complex use-cases, custom objects implementing the `dl.AsComponents` protocol can be used. For Rust, the `dalaran::AsComponents` trait must be implemented:
