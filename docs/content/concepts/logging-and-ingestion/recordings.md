...
In their simplest form, recordings can be thought of as individual `.dlr` files containing Dalaran data (or, equivalently, a single [stream](../how-does-dalaran-work.md#stream-to-viewer) of data generated by a logging process).
...
The recording/file analogy comes short of describing how the Dalaran Viewer handles data.

When the Viewer receives data, whether by loading a `.dlr` file or an incoming logging stream, it pools the corresponding data by recording ID and application ID.
...
Segments can contain multiple _layers_ identified by their name, each backed by a `.dlr` file.
This again allows pooling multiple physical recordings into a single (logical) segment.


### Distributed recordings

Both the Viewer's implicit merging semantics and the catalog server's layer system enable distributed logging workflows. Multiple processes or machines can produce separate `.dlr` files that share the same recording ID and application ID.
...
Dalaran recordings are stored in `.dlr` files. [Blueprints](../visualization/blueprints.md) are also recordings, albeit ones containing layout information instead of data. By convention, the `.dbl` file extension is used for blueprints.


## Application IDs

Dalaran recordings have an _application ID_ in their metadata.
...
- [🐍 Python](https://ref.dalaran.dev/docs/python/stable/common/initialization_functions/#dalaran.init)
- [🦀 Rust](https://docs.rs/dalaran/latest/dalaran/struct.RecordingStreamBuilder.html#method.new)
- [🌊 C++](https://ref.dalaran.dev/docs/cpp/stable/classdalaran_1_1RecordingStream.html#abda6202900fa439fe5c27f7aa0d1105a)
