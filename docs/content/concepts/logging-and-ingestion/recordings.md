...
In their simplest form, recordings can be thought of as individual `.rrd` files containing Dalaran data (or, equivalently, a single [stream](../how-does-dalaran-work.md#stream-to-viewer) of data generated by a logging process).
...
Dalaran recordings are stored in `.rrd` files. [Blueprints](../visualization/blueprints.md) are also recordings, albeit ones containing layout information instead of data. By convention, the `.rbl` file extension is used for blueprints.
