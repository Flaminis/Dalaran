...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let rec = dalaran::RecordingStreamBuilder::new(
        "dalaran_example_any_batch_value_column_updates",
    )
    .spawn()?;
    ...
    let one_per_timestamp = dalaran::SerializedComponentBatch::new(
        Arc::new(arrow::array::Float64Array::from_iter(
            (0..STEPS).map(|v| ((v as f64) / 10.0).sin()),
        )),
        dalaran::ComponentDescriptor::partial("custom_component_single"),
    );

    let ten_per_timestamp = dalaran::SerializedComponentBatch::new(
        Arc::new(arrow::array::Float64Array::from_iter((0..STEPS).flat_map(
            |_| (0..STEPS * 10).map(|v| ((v as f64) / 100.0).cos()),
        ))),
        dalaran::ComponentDescriptor::partial("custom_component_multi"),
    );
