...
//!
//! You need to enable the `web_viewer` feature on the `dalaran` crate to use this.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let rec =
        dalaran::RecordingStreamBuilder::new("dalaran_example_serve_web_viewer")
            .serve_grpc()?;

    // Connect the web viewer to the gRPC server and open it in the browser.
    // Feature check here is for the in-repository build.
    // For your own code remove it and enable the `web_viewer` feature on the `dalaran` crate.
    #[cfg(feature = "web_viewer")]
    let _server_guard =
        dalaran::serve_web_viewer(dalaran::web_viewer::WebViewerConfig {
            connect_to: vec!["dalaran+http://localhost/proxy".to_owned()],
            ..Default::default()
        })?;

    // Log some data to the gRPC server.
    rec.log("data", &dalaran::Boxes3D::from_half_sizes([(2.0, 2.0, 1.0)]))?;
