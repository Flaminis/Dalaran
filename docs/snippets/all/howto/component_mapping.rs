...
use dalaran::external::arrow::datatypes::{DataType, Field};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let rec =
        dalaran::RecordingStreamBuilder::new("dalaran_example_component_mapping")
            .spawn()?;

    // Generate columns for regular scalars (sin)
    let sin = (0..64).map(|step| (step as f64 / 10.0).sin());
    let sin_columns = dalaran::Scalars::new(sin).columns_of_unit_batches()?;
    ...
    let custom_columns: Vec<_> = dalaran::DynamicArchetype::new("custom")
    ...
    let nested_columns: Vec<_> = dalaran::DynamicArchetype::new("custom")
    ...
    rec.send_columns(
        "plot",
        [dalaran::TimeColumn::new_sequence("step", 0..64)],
    ...
    rec.log_static(
        "plot",
        &dalaran::SeriesLines::new().with_colors([[255, 0, 0]]),
    )?;

    // Create a blueprint with explicit component mappings
    let blueprint = dalaran::blueprint::Blueprint::new(
        dalaran::blueprint::TimeSeriesView::new("Component Mapping Demo")
            .with_origin("/")
            // Set default color for series to green
            .with_defaults(&dalaran::SeriesLines::new().with_colors([[0, 255, 0]]))
            .with_overrides(
                "plot",
                [
                    // region: custom_value
                    ...
                    dalaran::SeriesLines::new()
                        ...
                        .visualizer()
                        .with_mappings(vec![
                            dalaran::blueprint::VisualizerComponentMapping::new_default(
                                dalaran::SeriesLines::descriptor_colors().component,
                            )
                            .into(),
                        ]),
                    ...
                    dalaran::SeriesLines::new()
                        ...
                        .with_mappings(vec![
                            dalaran::blueprint::VisualizerComponentMapping::new_source_component(
                                dalaran::Scalars::descriptor_scalars().component,
                    ...
                    dalaran::SeriesLines::new()
                        ...
                        .visualizer()
                        .with_mappings(vec![
                            dalaran::blueprint::VisualizerComponentMapping::new_source_component_with_selector(
                                "Scalars:scalars",
