"""Update specific properties of a point cloud over time."""

import dalaran as dl

dl.init("dalaran_example_points3d_partial_updates", spawn=True)

positions = [[i, 0, 0] for i in range(10)]

dl.set_time("frame", sequence=0)
dl.log("points", dl.Points3D(positions))

for i in range(10):
    colors = [[20, 200, 20] if n < i else [200, 20, 20] for n in range(10)]
    radii = [0.6 if n < i else 0.2 for n in range(10)]

    # Update only the colors and radii, leaving everything else as-is.
    dl.set_time("frame", sequence=i)
    dl.log("points", dl.Points3D.from_fields(radii=radii, colors=colors))

# Update the positions and radii, and clear everything else in the process.
dl.set_time("frame", sequence=20)
dl.log(
    "points",
    dl.Points3D.from_fields(clear_unset=True, positions=positions, radii=0.3),
)
