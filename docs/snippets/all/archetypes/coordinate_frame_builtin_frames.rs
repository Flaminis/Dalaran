...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    rec.log(
        "red_box",
        &[
            &dalaran::Boxes3D::from_half_sizes([(0.5, 0.5, 0.5)])
                .with_colors([(255, 0, 0)])
            ...
            &dalaran::Transform3D::from_translation([2.0, 0.0, 0.0]),
        ],
    )?;
    rec.log(
        "blue_box",
    ...
    rec.log(
        "point",
        &dalaran::Points3D::new([(0.0, 0.0, 0.0)]).with_radii([0.5]),
    )?;
    ...
    for (t, frame_id) in ["tf#/red_box", "tf#/blue_box"].into_iter().enumerate()
    {
        ...
        rec.log("point", &dalaran::CoordinateFrame::new(frame_id))?;
    }
