...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let rec = dalaran::RecordingStreamBuilder::new(
        "dalaran_example_image_column_updates",
    )
    .spawn()?;
    ...
    let format =
        dalaran::components::ImageFormat::rgb8([width as _, height as _]);
    rec.log_static(
        "images",
        &dalaran::Image::update_fields().with_format(format),
    )?;

    // Split up the image data into several components referencing the underlying data.
    let image_size_in_bytes = width * height * 3;
    let timeline_values =
        dalaran::TimeColumn::new_sequence("step", times.clone());
    let buffer = images.into_raw_vec_and_offset().0;
    rec.send_columns(
        "images",
        [timeline_values],
        dalaran::Image::update_fields()
