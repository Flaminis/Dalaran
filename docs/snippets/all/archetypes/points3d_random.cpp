...
int main(int argc, char* argv[]) {
    const auto rec = dalaran::RecordingStream("dalaran_example_points3d_random");
    ...
    std::vector<dalaran::Position3D> points3d(10);
    ...
    rec.log(
        "random",
        dalaran::Points3D(points3d).with_colors(colors).with_radii(radii)
    );
}
