...
int main(int argc, char* argv[]) {
    const auto rec =
        dalaran::RecordingStream("dalaran_example_scalar_multiple_plots");
    ...
    rec.log_static(
        "scatter/lcg",
        dalaran::SeriesPoints().with_markers(dalaran::MarkerShape::Circle)
    );
    ...
    for (int t = 0; t < static_cast<int>(TAU * 2.0 * 100.0); ++t) {
        ...
        rec.log(
            "trig/sin",
            dalaran::Scalars(sin(static_cast<double>(t) / 100.0))
        );
        rec.log(
            "trig/cos",
            dalaran::Scalars(cos(static_cast<double>(t) / 100.0))
        );

        lcg_state = (1140671485 * lcg_state + 128201163) %
                    16777216; // simple linear congruency generator
        rec.log("scatter/lcg", dalaran::Scalars(static_cast<double>(lcg_state)));
    }
}
