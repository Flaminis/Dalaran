...
int main(int argc, char* argv[]) {
    ...
    rec.log(
        "red_box",
        dalaran::Boxes3D::from_half_sizes({{0.5f, 0.5f, 0.5f}})
        ...
        dalaran::Transform3D::from_translation({2.0f, 0.0f, 0.0f})
    );
    rec.log(
        "blue_box",
        dalaran::Boxes3D::from_half_sizes({{0.5f, 0.5f, 0.5f}})
    ...
    rec.log("point", dalaran::Points3D({{0.0f, 0.0f, 0.0f}}).with_radii({0.5f}));
    ...
    for (int t = 0; t < 2; t++) {
        ...
        rec.log("point", dalaran::CoordinateFrame(frame_ids[t]));
    }
}
