...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let rec = dalaran::RecordingStreamBuilder::new(
        "dalaran_example_points3d_column_updates",
    )
    .spawn()?;

    let times = dalaran::TimeColumn::new_duration_secs("time", 10..15);
    ...
    let position = dalaran::Points3D::update_fields()
        .with_positions(positions)
        .columns([2, 4, 4, 3, 4])?;
    let color_and_radius = dalaran::Points3D::update_fields()
        .with_colors(colors)
        .with_radii(radii)
