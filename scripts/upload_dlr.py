...
Use the script:
    ...
    python3 scripts/upload_dlr.py --help

or the pixi command:

    pixi run upload-dlr --help
"""
...
class Uploader:
    def __init__(self) -> None:
        ...
        self.bucket = gcs.bucket("dalaran-dlr")
...
DESCRIPTION = """Upload an .dlr to static.dalaran.dev.

    pixi run upload-dlr --version 0.15.0 path/to/recording.dlr
...
def main() -> None:
    ...
    parser.add_argument("path", type=str, help="Recording .dlr to upload")
    parser.add_argument(
        "--name",
        type=str,
        required=False,
    ...
    try:
        if not args.path.endswith(".dlr"):
            raise RuntimeError("File path expected to have .dlr extension")
        ...
        gcp_path = f"{version}/{name}_{digest}.dlr"
