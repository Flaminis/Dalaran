...
Usage:
- Start a Dalaran Viewer in release mode with 200MiB of memory limit:
  `cargo r -p dalaran-cli --release --no-default-features --features native_viewer -- --memory-limit 200MiB`
...
for i in range(20000000):
    dl.init("dalaran_example_recording_gc", recording_id=f"recording-gc-rec-{i}", spawn=True)
