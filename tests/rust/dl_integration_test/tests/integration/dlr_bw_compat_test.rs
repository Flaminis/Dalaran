//! Integration test: load example .dlr files from the previous release into the current viewer.
//!
...
const DOWNLOAD_TEMP_PREFIX: &str = ".dlr-download-";
...
async fn resolve_latest_patch(client: &reqwest::Client, major: u32, minor: u32) -> String {
    ...
    loop {
        ...
        let url = format!("https://app.dalaran.dev/version/{version}/examples/plots.dlr");
...
async fn ensure_dlr_cached(
    ...
    eprintln!("Downloading {}.dlr ({version})…", entry.name);
    download(client, &entry.dlr_url, path).await;
}

/// Load example .dlr files from the previous release into the current viewer.
///
/// Asserts:
/// - No panics during load + render
/// - At least one .dlr was downloaded and loaded
...
async fn test_old_rrds_in_current_viewer() {
    ...
    let mut downloads = futures::stream::iter(manifest.into_iter().map(|entry| {
        let path = cache_dir.join(format!("{}.dlr", entry.name));
        let version = version.clone();
        let client = client.clone();
        async move {
            ensure_dlr_cached(&client, &entry, &path, &version).await;
            path
        }
    }))
    ...
    while let Some(dlr_path) = downloads.next().await {
        let example_name = dlr_path.file_stem().unwrap().to_str().unwrap().to_owned();
        eprintln!("Loading {example_name}.dlr…");

        // Open the .dlr via the viewer's normal file-open path (same as Cmd+O).
        let file_path = dlr_path.canonicalize().unwrap().display().to_string();
        ...
        assert!(
            ...
            "{example_name}.dlr did not produce a recording."
        );
        ...
        if !NONDETERMINISTIC_EXAMPLES.contains(&example_name.as_str()) {
            ...
            let snapshot_options = dl_ui::testing::default_snapshot_options_for_3d(WINDOW_SIZE)
                .threshold(dl_ui::testing::strict_on_ci(2.0, lenient_threshold))
                .max_failed_pixels(dl_ui::testing::strict_on_ci(10, 50));
            harness.snapshot_options(format!("dlr_bw_compat_{example_name}"), &snapshot_options);
        }
        ...
        assert!(
            ...
            "{example_name}.dlr produced unexpected notifications:\n{}",
