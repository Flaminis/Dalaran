...
//!
//! See also: `dl_dataframe_ui::tests::grid_view::test_grid_view_flagging` for
...
pub async fn grid_view_flagging() {
    ...
    let schema = Arc::new(Schema::new_with_metadata(
        vec![
            ...
            Field::new("flagged", DataType::Boolean, true).with_metadata(
                [(
                    dl_dataframe_ui::experimental_field_metadata::IS_FLAG_COLUMN.to_owned(),
                    "true".to_owned(),
                )]
                .into(),
            ),
    ...
    let table = client
        .create_table_entry(
            dl_log_types::EntryName::new("flag_test").unwrap(),
            None,
            schema,
        )
        .await
