...
use dl_sdk::{Component as _, Timeline};
use dl_view_time_series::TimeSeriesView;
use dl_viewer::external::dl_sdk_types::{self, components, datatypes};
    ...
    for frame in 0..10 {
        ...
        for i in 0..6 {
            harness.log_entity(format!("native_{i}"), |builder| {
                builder.with_archetype_auto_row(
                    [(timeline, frame)],
                    &dl_sdk_types::archetypes::Scalars::single(frame as f64 * i as f64),
                )
            });
        }
        ...
        for i in 6..12 {
            harness.log_entity(format!("custom_{i}"), |builder| {
                builder.with_archetype_auto_row(
                    [(timeline, frame)],
                    &dl_sdk_types::DynamicArchetype::new("custom").with_component_from_data(
                        "custom_scalar",
...
pub async fn test_time_series_does_not_spawn_for_non_scalar_physical_type() {
    ...
    for frame in 0..10 {
        harness.log_entity("string_scalar", |builder| {
            builder.with_archetype_auto_row(
                [(timeline, frame)],
                &dl_sdk_types::DynamicArchetype::new("custom")
