...
use dl_sdk::external::dl_log_types::EntryId;
use dl_viewer::App;
...
pub async fn dataset_folders() {
    ...
    let summary_route = Route::from(dl_uri::EntryUri::new(
        origin.clone(),
        EntryId::from_str(summary_id_str).expect("valid entry id"),
    ));
    let metrics_route = Route::from(dl_uri::EntryUri::new(
        origin.clone(),
...
async fn create_table(
    ...
    let table = client
        .create_table_entry(
            dl_sdk::external::dl_log_types::EntryName::new(name)
