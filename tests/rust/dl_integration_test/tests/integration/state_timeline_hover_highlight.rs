...
use dl_viewer::external::dl_viewer_context::{RecommendedView, TimeControlCommand, ViewClass as _};
    ...
    for t in 0..120i64 {
        harness.log_entity("scalars/value", |builder| {
            builder.with_archetype(
                RowId::new(),
                [(timeline, t)],
                &dl_sdk_types::archetypes::Scalars::single((t as f64 / 20.0).sin()),
            )
        });
    }
        ...
        app_context
            ...
            .filter(|h| {
                h.timeline == timeline_name
                    && h.kind
                        == dl_viewer::external::dl_viewer_context::TimeRangeHighlightKind::StateTimeline
            })
            .cloned()
    });
