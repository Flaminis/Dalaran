...
use dl_sdk::external::arrow::array::Float64Array;
use dl_sdk::log::RowId;
use dl_test_context::VisualizerBlueprintContext as _;
use dl_viewer::external::dl_log_types::EntityPath;
use dl_viewer::external::dl_sdk_types::{self, VisualizableArchetype as _, archetypes};
use dl_viewer::external::dl_viewer_context::{RecommendedView, ViewClass as _};
...
pub async fn test_view_visualizers_section() {
    ...
    let timeline = dl_sdk::Timeline::new_sequence("frame");
    ...
    for i in 0..50 {
        harness.log_entity("plots/sin_line", |builder| {
            builder.with_archetype(
                RowId::new(),
                [(timeline, i)],
                &dl_sdk_types::archetypes::Scalars::single((i as f64 / 50.0 * TAU).sin()),
            )
        });
    }
    ...
    for i in 0..50 {
        harness.log_entity("plots/cos_points", |builder| {
            builder.with_archetype(
                RowId::new(),
                [(timeline, i)],
                &dl_sdk_types::archetypes::Scalars::single((i as f64 / 50.0 * TAU).cos()),
            )
        });
    }
    ...
    for i in 0..50 {
        harness.log_entity("other/linear", |builder| {
            builder.with_archetype(
                RowId::new(),
                [(timeline, i)],
                &dl_sdk_types::archetypes::Scalars::single(i as f64 / 50.0),
            )
        });
    }
    ...
    let mut view_1 = ViewBlueprint::new(
        dl_view_time_series::TimeSeriesView::identifier(),
    ...
    for i in 0..50 {
        ...
        harness.log_entity("multi/double", |builder| {
            builder.with_archetype(
                RowId::new(),
                [(timeline, i)],
                &dl_sdk_types::archetypes::Scalars::new([(t * TAU).sin(), (t * TAU).cos()]),
            )
        });
    }
    ...
    for i in 0..50 {
        ...
        harness.log_entity("multi/single", |builder| {
            builder.with_archetype(
                RowId::new(),
                [(timeline, i)],
                &dl_sdk_types::archetypes::Scalars::single(t),
            )
        });
    }
...
/// interaction tests.
fn setup_trig_view() -> egui_kittest::Harness<'static, dl_viewer::App> {
    let mut harness = viewer_test_utils::viewer_harness(&HarnessOptions {
        window_size: Some(egui::Vec2::new(1200.0, 1000.0)),
        max_steps: Some(100),
        ..Default::default()
    ...
    for i in 0..100 {
        harness.log_entity("trig/sin", |builder| {
            builder.with_archetype(
                RowId::new(),
                [(timeline, i)],
                &dl_sdk_types::archetypes::Scalars::single((i as f64 / 100.0 * TAU).sin()),
            )
        });
    }
    ...
    for i in 0..100 {
        harness.log_entity("trig/cos", |builder| {
            builder.with_archetype(
                RowId::new(),
                [(timeline, i)],
                &dl_sdk_types::archetypes::Scalars::single((i as f64 / 100.0 * TAU).cos()),
            )
        });
    }
    ...
    for i in 0..20 {
        ...
        harness.log_entity("waves/sin", |builder| {
            builder.with_archetype_auto_row(
                [(timeline, i)],
                &dl_sdk_types::DynamicArchetype::new("wave_data")
