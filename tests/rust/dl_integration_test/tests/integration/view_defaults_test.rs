...
use dl_viewer::external::dl_chunk::LatestAtQuery;
use dl_viewer::external::dl_sdk_types;
use dl_viewer::external::dl_viewer_context::{
...
pub async fn test_view_defaults_stroke_width() {
    ...
    for i in 0..50 {
        harness.log_entity("plot", |builder| {
            builder.with_archetype_auto_row(
                [(timeline, i)],
                &dl_sdk_types::archetypes::Scalars::single((i as f64 / 10.0).sin()),
            )
        });
    }
    ...
    let (defaults_path, component_id) =
        harness.setup_viewport_blueprint(|viewer_context, blueprint| {
            ...
            let descriptor = dl_sdk_types::archetypes::SeriesLines::descriptor_widths();
            let component_id = descriptor.component;
            let stroke_width = dl_sdk_types::components::StrokeWidth::from(5.0f32);
    ...
    let stroke_width = harness.run_with_app_context(move |app_context| {
        ...
        results.component_mono::<dl_sdk_types::components::StrokeWidth>(component_id)
    });
