...
pub async fn watch_events_auto_refresh_test() {
    ...
    let persistent = client
        .create_dataset_entry(
            dl_log_types::EntryName::new(persistent_dataset).expect("Failed to create entry name"),
            None,
        )
        .await
    ...
    let dataset = client
        .create_dataset_entry(
            dl_log_types::EntryName::new(transient_dataset).expect("Failed to create entry name"),
            None,
        )
        .await
...
async fn create_table(
    ...
    let table = client
        .create_table_entry(
            dl_log_types::EntryName::new(name).expect("Failed to create entry name"),
            None,
            schema.clone(),
        )
        .await
