...
use dl_sdk::external::{dl_log_types, dl_tuid};
...
pub async fn preview_table() {
    ...
    let dataset_id = dl_tuid::Tuid::from_str(DATASET_ID).expect("Failed to parse TUID");
    let segment_uris: Vec<dl_uri::DatasetSegmentUri> = segment_ids
        .iter()
    ...
    let table = client
        .create_table_entry(
            dl_log_types::EntryName::new("preview_table").expect("valid entry name"),
            None,
            schema.clone(),
        )
        .await
    ...
    client
        .write_table(
            ...
            table.details.id,
            dl_protos::cloud::v1alpha1::ext::TableInsertMode::Append,
        )
        .await
    ...
    let preview_uris: Vec<dl_uri::DatasetSegmentUri> = segment_uris
        .iter()
        .map(|uri| uri.clone().without_fragment())
        .collect();
    let preview_entity = dl_log_types::EntityPath::from("test_entity");
