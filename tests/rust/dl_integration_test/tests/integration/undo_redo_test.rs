...
fn make_undo_test_harness<'a>() -> egui_kittest::Harness<'a, dl_viewer::App> {
...
fn send_undo(harness: &mut egui_kittest::Harness<'_, dl_viewer::App>) {
    send_recording_command(harness, RecordingCommandKind::Undo);
}

/// Send redo command (Cmd/Ctrl+Shift+Z).
fn send_redo(harness: &mut egui_kittest::Harness<'_, dl_viewer::App>) {
