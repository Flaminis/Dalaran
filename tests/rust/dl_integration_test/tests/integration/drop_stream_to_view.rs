...
fn make_harness<'a>() -> egui_kittest::Harness<'a, dl_viewer::App> {
    ...
    for i in 0..100 {
        ...
        harness.log_entity("line_curve", |builder| {
            builder.with_archetype(
                RowId::new(),
                [(timeline, i)],
                &dl_sdk_types::archetypes::Scalars::single(i as f64 / 100.0),
            )
        });
    }
    ...
    let mut view_1 = ViewBlueprint::new(
        ...
        RecommendedView {
            origin: "/".into(),
            query_filter: dl_sdk::external::dl_log_types::EntityPathFilter::default(),
        },
    );
...
fn assert_view_entity_inclusion(
    ...
    let expectations = expectations
        .iter()
        .map(|(entity_path, expected)| (dl_sdk::EntityPath::from(*entity_path), *expected))
        .collect::<Vec<_>>();
...
fn view_id(harness: &mut egui_kittest::Harness<'_, dl_viewer::App>, view_name: &str) -> ViewId {
    let view_name = view_name.to_owned();
...
fn assert_selected_item(harness: &mut egui_kittest::Harness<'_, dl_viewer::App>, expected: Item) {
...
pub async fn test_drop_stream_to_view() {
    ...
    let sin_curve = Item::from(dl_sdk::EntityPath::from("sin_curve"));
