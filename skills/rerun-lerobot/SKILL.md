...
Dalaran has a **built-in LeRobot importer**: point `log_file_from_path` (or the viewer, or `dalaran <dir>` on the CLI) at the dataset _directory_ and it ingests episodes, camera videos, and state/action tables with no conversion code.
...
3. The combined DLR contains a metadata-only root recording; skip stores with no entity paths or you register an empty segment.
