...
For mechanism details see `dalaran-chunk-processing` (pipeline mechanics) and the importer skills it routes to: `dalaran-mcap`,`dalaran-urdf`,`dalaran-parquet`, `dalaran-lerobot`.
For exact signatures, the docs at `dalaran.dev/docs/concepts/logging-and-ingestion`.
...
- **Does a reader exist for this source?** MCAP→`McapReader`, URDF→`UrdfTree`, parquet→`ParquetReader`, DLR→`RrdReader`, LeRobot dir→`log_file_from_path`. **Yes →** `reader.stream()` + lenses; the reader produces the chunks, you do not.
...
- **Segment** = one episode; on registration its `recording_id` becomes the `segment_id`. **Layer** = an extra `.dlr` on top of a segment. It attaches by matching the segment's `recording_id` and nothing else: that shared id is the whole layering mechanism.
...
- **Layer**: a whole derived `.dlr` over a base segment (FK transforms, point clouds, gripper state, labels, quality scores). Queryable as if part of base.
...
- **Columnar ingest**: for an existing _file_, use the matching reader (`dalaran-mcap`/`-parquet`/`-lerobot`), which produces chunks directly — do not hand-assemble `send_columns` from a custom parser. When you do log columns directly (live logging), `send_columns` adds **no** automatic timelines, so pass every timeline you want.

## Gotchas that cause real failures

1. Component columns come back as `ListArray` in queries: index `[0]`/`[0][0]` (0-based DataFrame, 1-based SQL). See `dalaran-catalog-queries`.
...
4. Static shadows all temporal data of that component for all time; static is overwritten in the viewer but every write stays on disk until `dalaran dlr optimize`.
