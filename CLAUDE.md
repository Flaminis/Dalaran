...
**Running:**
- `pixi run dalaran` - Run the viewer
...
- Type definitions live in `crates/store/dl_sdk_types/definitions/dalaran/`
...
- validate various custom conventions via `pixi run lint-dalaran <file>` (not passing any file will check everything)
...
The docs span multiple sites: the main docs at `dalaran.dev/docs` (built from `docs/content/`), plus API reference sites for Python (MkDocs), C++ (Doxygen), and JS (TypeDoc) at `ref.dalaran.dev/docs/{python,cpp,js}/`.
...
- [`dalaran_py/README.md`](dalaran_py/README.md) - Python SDK specific instructions
