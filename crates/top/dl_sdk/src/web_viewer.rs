...
use dl_web_viewer_server::{WebViewerServer, WebViewerServerError, WebViewerServerPort};
...
struct WebViewerSink {
    ...
    sender: dl_log_channel::LogSender,
...
impl WebViewerSink {
    ...
    pub fn new(
        ...
        let (server_shutdown_signal, shutdown) = dl_grpc_server::shutdown::shutdown();
        ...
        let server_handle = std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                rt.block_on(dl_grpc_server::serve_from_channel(
                    grpc_server_addr,
                    server_options,
                    shutdown,
                    channel_rx,
...
impl crate::sink::LogSink for WebViewerSink {
    fn send(&self, msg: LogMsg) {
        if let Err(err) = self.sender.send(msg.into()) {
            dl_log::error_once!("Failed to send log message to web server: {err}");
        }
    }
    ...
    fn flush_blocking(&self, timeout: std::time::Duration) -> Result<(), SinkFlushError> {
        self.sender
            ...
            .map_err(|err| match err {
                dl_log_channel::FlushError::Closed => {
                    SinkFlushError::failed("The viewer is no longer subscribed")
                }
                dl_log_channel::FlushError::Timeout => SinkFlushError::Timeout,
            })
    }
...
impl Drop for WebViewerSink {
    fn drop(&mut self) {
        if self.open_browser {
            ...
            dl_log::info!("Sleeping a short while to give the browser time to connect…");
