...
use dl_sdk_types::blueprint::archetypes::ContainerBlueprint;
use dl_sdk_types::blueprint::components::{ColumnShare, ContainerKind, IncludedContent, RowShare};
use dl_sdk_types::components::{Name, Visible};
use dl_sdk_types::datatypes::{Bool, Float32};
...
fn log_container(
    ...
    if let Some(grid_columns) = container.grid_columns {
        arch = arch.with_grid_columns(dl_sdk_types::blueprint::components::GridColumns(
            dl_sdk_types::datatypes::UInt32(grid_columns),
        ));
    }

    if let Some(ref active_tab) = container.active_tab {
        arch = arch.with_active_tab(dl_sdk_types::blueprint::components::ActiveTab(
            dl_sdk_types::datatypes::EntityPath::from(active_tab.as_str()),
        ));
    }
