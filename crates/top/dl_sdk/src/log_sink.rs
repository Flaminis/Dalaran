    ...
    fn drop(&mut self) {
        for msg in self.0.lock().iter() {
            ...
            if !matches!(msg, LogMsg::SetStoreInfo(_)) {
                dl_log::warn!("Dropping data in BufferedSink");
                return;
            }
        }
    }
    ...
    fn drop(&mut self) {
        ...
        if !inner.has_been_used {
            for msg in &inner.msgs {
                ...
                // warning.
                if !matches!(msg, LogMsg::SetStoreInfo(_)) {
                    dl_log::warn!("Dropping data in MemorySink");
                    return;
                }
            }
        }
...
pub type GrpcSinkConnectionState = dl_grpc_client::write::ClientConnectionState;

/// The reason why a [`GrpcSink`] was disconnected.
pub type GrpcSinkConnectionFailure = dl_grpc_client::write::ClientConnectionFailure;
...
impl Default for GrpcSink {
    fn default() -> Self {
        ...
        Self::new(
            dl_uri::ProxyUri::from_str(crate::DEFAULT_CONNECT_URL).expect("failed to parse uri"),
        )
    }
}
...
impl LogSink for GrpcSink {
    fn send(&self, mut log_msg: LogMsg) {
        if let Some(metadata_key) = dl_sorbet::TimestampLocation::GrpcSink.metadata_key() {
