...
pub enum RecordingStreamError {
    ...
    FileSink(#[from] dl_log_encoding::FileSinkError),
    ...
    /// An error occurred while attempting to use a [`dl_importer::Importer`].
    #[cfg(feature = "importers")]
    #[error(transparent)]
    ImporterError(#[from] dl_importer::ImporterError),
...
impl RecordingStreamBuilder {
    ...
    pub fn new(application_id: impl Into<ApplicationId>) -> Self {
        ...
        Self {
            ...
            recording_info: RecordingInfo::new()
                .with_start_time(dl_sdk_types::components::Timestamp::now()),
    ...
    pub fn connect_grpc_opts(
        ...
        self.create_recording_stream("connect_grpc", || {
            ...
            let dl_uri::RedapUri::Proxy(uri) = url.as_str().parse()? else {
    ...
    pub fn serve_grpc(self) -> RecordingStreamResult<RecordingStream> {
        use dl_grpc_server::ServerOptions;

        self.serve_grpc_opts(
            "0.0.0.0",
            crate::DEFAULT_SERVER_PORT,
            ServerOptions {
                memory_limit: dl_memory::MemoryLimit::from_fraction_of_total(0.25),
                ..Default::default()
            },
        )
    }
    ...
    pub fn serve_grpc_opts(
        ...
        port: u16,
        server_options: dl_grpc_server::ServerOptions,
    ) -> RecordingStreamResult<RecordingStream> {
...
struct RecordingStreamInner {
    ...
    /// running.
    cmds_tx: dl_quota_channel::Sender<Command>,
...
impl Drop for RecordingStreamInner {
    fn drop(&mut self) {
        ...
        if let Err(err) = self.batcher.flush_blocking(timeout) {
            dl_log::error!("Failed to flush batcher: {err}");
        }
...
fn resolve_batcher_config(
    ...
    if let Some(explicit_batcher_config) = batcher_config {
        ...
        default_config.apply_env().unwrap_or_else(|err| {
            dl_log::error!("Failed to parse ChunkBatcherConfig from env: {err}");
            default_config
        })
    }
}
...
impl RecordingStreamInner {
    fn new(
        ...
        let (cmds_tx, cmds_rx) = dl_quota_channel::channel(
        ...
        if let Some(recording_info) = recording_info.as_ref() {
            ...
            dl_log::trace!(recording_info = ?recording_info, "Adding RecordingInfo to batcher");
...
impl RecordingStream {
    ...
    pub fn new(
        ...
        let sink = store_info
            ...
            .flatten()
            .map_or(sink, |path| {
                dl_log::info!("Forcing FileSink because of env-var {ENV_FORCE_SAVE}={path:?}");
                Box::new(
...
impl RecordingStream {
    ...
    pub fn log_serialized_batches(
        ...
        ent_path: impl Into<EntityPath>,
        static_: bool,
        comp_batches: impl IntoIterator<Item = dl_sdk_types::SerializedComponentBatch>,
    ) -> RecordingStreamResult<()> {
        let row_id = RowId::new(); // Create row-id as early as possible. It has a timestamp and is used to estimate e2e latency.
    ...
    /// Logs the file at the given `path` using all [`dl_importer::Importer`]s available.
    ///
    ...
    /// Logs the given `contents` using all [`dl_importer::Importer`]s available.
    ///
    /// A single `path` might be handled by more than one importer.
    ///
    /// This method blocks until either at least one [`dl_importer::Importer`] starts
    ...
    fn log_file(
        ...
        let mut settings = crate::ImporterSettings {
            ...
            timeline_type: dl_log_types::TimeType::TimestampNs,
        };
        ...
        if let Some(contents) = contents {
            dl_importer::import_from_file_contents(
                &settings,
                dl_log_types::FileSource::Sdk,
                filepath,
                contents,
                &tx,
            )?;
        } else {
            dl_importer::import_from_path(&settings, dl_log_types::FileSource::Sdk, filepath, &tx)?;
        }
        drop(tx);
            ...
            .spawn({
                ...
                move || {
                    while let Some(msg) = rx.recv().ok().and_then(|msg| msg.into_data()) {
                        match msg {
                            dl_log_channel::DataSourceMessage::LogMsg(log_msg) => {
...
fn forwarding_thread(
    ...
    cmds_rx: dl_quota_channel::Receiver<Command>,
    chunks: dl_quota_channel::Receiver<Chunk>,
        ...
        match cmd {
            ...
            Command::SwapSink { new_sink, timeout } => {
                ...
                let backlog = {
                    ...
                    if let Err(err) = sink.flush_blocking(timeout) {
                        dl_log::error!("Failed to flush previous sink: {err}");
                    }
            ...
            Command::Flush { on_done, timeout } => {
                    ...
                    && let Err(err) = result
                {
                    ...
                    dl_log::error!("Failed to flush sink: {err}");
                }
            }
            Command::FinalizeDeferredSinks { on_done, timeout } => {
                if sink.defers_finalization_to_shutdown() && !sink.finalize_deferred_in_place() {
                    ...
                    let backlog = sink.drain_backlog();
                    if let Err(err) = sink.flush_blocking(timeout) {
                        dl_log::error!("Failed to flush previous sink during finalize: {err}");
                    }
        ...
        dl_quota_channel::select! {
            ...
            recv(cmds_rx) -> res => {
                let Ok(cmd) = res else {
                    ...
                    dl_log::trace!("Shutting down forwarding_thread: all command senders are gone");
                    break;
                };
...
impl RecordingStream {
    ...
    pub fn record_msg(&self, msg: LogMsg) {
        ...
        if self.with(f).is_none() {
            dl_log::warn_once!("Recording disabled - call to record_msg() ignored");
        }
    }
    ...
    pub fn record_row(&self, entity_path: EntityPath, mut row: PendingRow, inject_time: bool) {
        ...
        if self.with(f).is_none() {
            dl_log::warn_once!("Recording disabled - call to record_row() ignored");
        }
    }
    ...
    pub fn log_chunk(&self, mut chunk: Chunk) {
        let f = move |inner: &RecordingStreamInner| {
            ...
            if inner.default_timelines.log_time() {
                ...
                let time =
                    TimeInt::new_temporal(dl_log_types::Timestamp::now().nanos_since_epoch());
        ...
        if self.with(f).is_none() {
            dl_log::warn_once!("Recording disabled - call to log_chunk() ignored");
        }
    }
    ...
    pub fn send_chunk(&self, chunk: Chunk) {
        ...
        if self.with(f).is_none() {
            dl_log::warn_once!("Recording disabled - call to send_chunk() ignored");
        }
    }
    ...
    pub fn set_sink(&self, new_sink: Box<dyn LogSink>) {
        ...
        let f = move |inner: &RecordingStreamInner| {
            ...
            if let Err(err) = inner.batcher.flush_blocking(timeout) {
                dl_log::warn!("Failed to flush batcher in `set_sink`: {err}");
            }
            ...
            dl_log::trace!("Waiting for sink swap to complete…");
            let (cmd, oneshot) = Command::flush(timeout);
            inner.cmds_tx.send(cmd).ok();
            oneshot.recv().ok();
            dl_log::trace!("Sink swap completed.");
        };

        if self.with(f).is_none() {
            dl_log::warn_once!("Recording disabled - call to set_sink() ignored");
        }
    }
    ...
    fn flush(&self, timeout: Option<Duration>) -> Result<(), SinkFlushError> {
        ...
        match self.with(f) {
            ...
            None => {
                dl_log::warn_once!("Recording disabled - call to flush ignored");
                Ok(())
            }
        }
    }
...
impl RecordingStream {
    ...
    pub fn set_sinks(&self, sinks: impl crate::log_sink::IntoMultiSink) {
        if forced_sink_path().is_some() {
            dl_log::debug!("Ignored setting new MultiSink since {ENV_FORCE_SAVE} is set");
            return;
        }
    ...
    pub fn connect_grpc_opts(&self, url: impl Into<String>) -> RecordingStreamResult<()> {
        if forced_sink_path().is_some() {
            dl_log::debug!("Ignored setting new GrpcSink since {ENV_FORCE_SAVE} is set");
            return Ok(());
        }
    ...
    pub fn serve_grpc_opts(
        ...
        if forced_sink_path().is_some() {
            dl_log::debug!("Ignored setting GrpcServerSink since {ENV_FORCE_SAVE} is set");
            return Ok(());
        }
    ...
    pub fn save_opts(
        ...
        if forced_sink_path().is_some() {
            dl_log::debug!("Ignored setting new file since {ENV_FORCE_SAVE} is set");
            return Ok(());
        }
    ...
    pub fn disconnect(&self) {
        ...
        if self.with(f).is_none() {
            dl_log::warn_once!("Recording disabled - call to disconnect() ignored");
        }
    }
    ...
    pub fn finalize_deferred_sinks(&self) {
        ...
        let f = move |inner: &RecordingStreamInner| {
            ...
            if let Err(err) = inner.batcher.flush_blocking(timeout) {
                dl_log::warn!("Failed to flush batcher in `finalize_deferred_sinks`: {err}");
            }
            inner.cmds_tx.send(Command::PopPendingChunks).ok();
        ...
        if self.with(f).is_none() {
            dl_log::warn_once!("Recording disabled - call to finalize_deferred_sinks() ignored");
        }
    }
...
impl RecordingStream {
    ...
    pub fn now(&self) -> TimePoint {
        ...
        if let Some(res) = self.with(f) {
            ...
            dl_log::warn_once!("Recording disabled - call to now() ignored");
            TimePoint::default()
        }
    }
    ...
    pub fn set_timepoint(&self, timepoint: impl Into<TimePoint>) {
        ...
        if self.with(f).is_none() {
            dl_log::warn_once!("Recording disabled - call to set_timepoint() ignored");
        }
    }
    ...
    pub fn set_time(&self, timeline: impl Into<TimelineName>, value: impl TryInto<TimeCell>) {
        ...
        if self.with(f).is_none() {
            dl_log::warn_once!("Recording disabled - call to set_time() ignored");
        }
    }
    ...
    pub fn set_duration_secs(&self, timeline: impl Into<TimelineName>, secs: impl Into<f64>) {
        ...
        if let Ok(duration) = std::time::Duration::try_from_secs_f64(secs) {
            ...
            dl_log::error_once!("set_duration_secs: can't set time to {secs}");
        }
    }
    ...
    pub fn disable_timeline(&self, timeline: impl Into<TimelineName>) {
        ...
        if self.with(f).is_none() {
            dl_log::warn_once!("Recording disabled - call to disable_timeline() ignored");
        }
    }
    ...
    pub fn reset_time(&self) {
        ...
        if self.with(f).is_none() {
            dl_log::warn_once!("Recording disabled - call to reset_time() ignored");
        }
    }
    ...
    pub fn set_log_tick_enabled(&self, enabled: bool) {
        ...
        if self.with(f).is_none() {
            dl_log::warn_once!("Recording disabled - call to set_log_tick_enabled() ignored");
        }
    }
    ...
    pub fn set_log_time_enabled(&self, enabled: bool) {
        ...
        if self.with(f).is_none() {
            dl_log::warn_once!("Recording disabled - call to set_log_time_enabled() ignored");
        }
    }
}
...
mod tests {
    ...
    fn example_rows(static_: bool) -> Vec<PendingRow> {
        use dl_log_types::example_components::{MyColor, MyLabel, MyPoint};
        use dl_sdk_types::Loggable;
        ...
        let mut timepoint = |frame_nr: i64| {
            ...
            if !static_ {
                tp.insert(Timeline::log_time(), dl_log_types::Timestamp::now());
