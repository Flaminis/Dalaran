...
impl<S: LogSink> LensesSink<S> {
    ...
    fn send_or_log_error(&self, store_id: StoreId, chunk: &Chunk) {
        match chunk.to_arrow_msg() {
            ...
            Err(err) => {
                dl_log::error_once!("Failed to create log message from chunk: {err}");
            }
        }
    }
}
    ...
    fn send(&self, msg: dl_log_types::LogMsg) {
        match &msg {
            ...
            LogMsg::ArrowMsg(store_id, arrow_msg) => match Chunk::from_arrow_msg(arrow_msg) {
                Ok(original_chunk) => {
                    let runtime = dl_lenses::default_runtime();
                        ...
                        match maybe_chunk {
                            ...
                            Err(partial_chunk) => {
                                for error in partial_chunk.errors() {
                                    ...
                                    dl_log::error_once!("Error encountered for lens: {error}");
                                }
                ...
                Err(err) => {
                    dl_log::error_once!("Failed to convert arrow message to chunk: {err}");
                    self.sink.send(msg);
                }
            },
        }
