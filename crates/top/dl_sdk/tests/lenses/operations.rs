...
fn test_destructure_cast() {
    ...
    let lenses = Lenses::new(OutputMode::DropUnmatched).add_lens_with_filter(
        dl_log_types::EntityPathFilter::parse_forgiving("nullability"),
        destructure,
    );

    let res: Vec<dl_chunk::Chunk> = lenses
        .apply(&original_chunk, &dl_lenses::default_runtime())
        .try_collect()
        .unwrap();
...
fn test_time_column_extraction() {
    ...
    let lenses = Lenses::new(OutputMode::DropUnmatched).add_lens_with_filter(
        dl_log_types::EntityPathFilter::parse_forgiving("timestamped"),
        time_lens,
    );
