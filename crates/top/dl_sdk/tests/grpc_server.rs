...
fn grpc_server_sink_streams_recording_data_and_shuts_down() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    rec.log(
        "test/points",
        &dl_sdk_types::archetypes::Points2D::new([[1.0, 2.0]]),
    )?;
    rec.flush_blocking()?;
    ...
    let runtime = tokio::runtime::Builder::new_multi_thread() // NOLINT: needed for testing.
        ...
        .enable_all()
        .build()?;
    let async_runtime = dl_async::AsyncRuntimeHandle::new_native(runtime.handle().clone());
    let receiver = dl_grpc_client::read::stream(&async_runtime, uri);
