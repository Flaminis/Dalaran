//! The Dalaran C SDK.
//!
//! The functions here must match `dalaran_cpp/src/dalaran/c/dalaran.h`.
...
pub struct CGrpcSink {
    /// A Dalaran gRPC URL
    ///
    /// Default is `dalaran+http://127.0.0.1:9876/proxy`.
    pub url: CStringView,
}
...
) -> Result<CRecordingStream, CError> {
    {
        ...
        INIT.call_once(|| {
            ...
            if cfg!(debug_assertions) {
                ...
                // This should however _never_ cause a panic if DALARAN_PANIC_ON_WARN is set, e.g. in test environments.
                const DEBUG_BUILD_WARNING: &str =
                    "Using a DEBUG BUILD of the Dalaran SDK with Dalaran crash handlers!";
                let can_log_warning = std::env::var("DALARAN_PANIC_ON_WARN")
                    .map(|value| value == "0")
                    .unwrap_or(true);
                if can_log_warning {
