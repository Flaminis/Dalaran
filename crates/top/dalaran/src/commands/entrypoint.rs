...
Environment variables:
    DALARAN_CHUNK_MAX_BYTES     Maximum chunk size threshold for the compactor.
    DALARAN_CHUNK_MAX_ROWS      Maximum chunk row count threshold for the compactor (sorted chunks).
    DALARAN_CHUNK_MAX_ROWS_IF_UNSORTED
                              Maximum chunk row count threshold for the compactor (unsorted chunks).
    DALARAN_SHADER_PATH         The search path for shader/shader-imports. Only available in developer builds.
    DALARAN_TRACK_ALLOCATIONS   Track memory allocations to diagnose memory leaks in the viewer.
                              WARNING: slows down the viewer by a lot!
    DALARAN_MAPBOX_ACCESS_TOKEN The Mapbox access token to use the Mapbox-provided backgrounds in the map view.
...
Examples:
    Open a Dalaran Viewer that listens for incoming SDK connections:
        dalaran
...
struct Args {
    ...
    #[clap(
        long,
        long_help = r"An upper limit on how much memory the Dalaran Viewer should use.
...
Example: `16GB` or `50%` (of system total)."
    ...
    #[clap(
        ...
        long_help = r"Whether the Dalaran Viewer should persist the state of the viewer to disk.
...
- Windows: `C:\Users\UserName\AppData\Roaming\dalaran`"
    ...
    /// logging SDKs, and forwarding it to Dalaran viewers.
    //
    // TODO(andreas): The Rust/Python APIs deprecated `serve_web` and instead encourage separate usage of `rec.serve_grpc()` + `dalaran::serve_web_viewer()` instead.
...
If no arguments are given, a server will be hosted which a Dalaran SDK can connect to.")]
    ...
    /// Marks the relaunched child of a detached Dalaran Viewer.
    #[clap(long, hide = true)]
...
impl Args {
    fn generate_markdown_manual() -> String {
        ...
        fn generate_markdown_manual(
            ...
            // E.g. "## dalaran analytics"
            let header = format!("{} {}", "##", full_name.join(" "))
                .trim()
                .to_owned();

            // E.g. "**Usage**: `dalaran [OPTIONS] [URL_OR_PATHS]… [COMMAND]`"
            let usage = {
...
enum Command {
    ...
    /// Generates the Dalaran CLI manual (markdown).
    ///
    /// Example: `dalaran man > docs/content/reference/cli.md`
    #[command(name = "man")]
    Manual,
    ...
    /// Run an MCP server that controls a running Dalaran Viewer.
    ///
    /// See the [mcp docs](https://dalaran.dev/docs/reference/viewer/mcp) for more info about using
    /// `dalaran viewer-mcp`.
    ///
    ...
    /// Reset the memory of the Dalaran Viewer.
    ///
    /// Only run this if you're having trouble with the Viewer,
    /// e.g. if it is crashing on startup.
    ///
    /// Dalaran will forget all blueprints, as well as the native window's size, position and scale factor.
    #[cfg(feature = "native_viewer")]
    Reset,
...
/// Run the Dalaran application and return an exit code.
///
/// This is used by the `dalaran` binary and the Dalaran Python SDK via `python -m dalaran [args…]`.
///
    ...
    T: Into<std::ffi::OsString> + Clone,
{
    ...
    dl_memory::accounting_allocator::turn_on_tracking_if_env_var(
        dl_viewer::env_vars::DALARAN_TRACK_ALLOCATIONS,
    );
    ...
    #[expect(unsafe_code)]
    unsafe {
        std::env::set_var("OTEL_SERVICE_NAME", "dalaran");
    }
...
fn relaunch_detached(raw_args: &[std::ffi::OsString]) -> anyhow::Result<()> {
    let executable = std::env::current_exe()
        .map_err(|err| anyhow::anyhow!("failed to locate the Dalaran executable: {err}"))?;
    ...
    command
        .spawn()
        .map_err(|err| anyhow::anyhow!("failed to launch the detached Dalaran Viewer: {err}"))?;
    Ok(())
}
...
fn run_impl(
    ...
    if let Some(url) = args.connect.clone() {
        let url = url.unwrap_or_else(|| format!("dalaran+http://{server_addr}/proxy"));
    ...
    if args.test_receive || args.save.is_some() {
        ...
        cfg_select! {
            ...
            _ => Err(anyhow::anyhow!(
                "dalaran-cli must be compiled with the 'server' feature enabled"
            )),
        }
    } else if args.serve_web {
        cfg_select! {
            not(feature = "server") => Err(anyhow::anyhow!(
                "Can't host server - dalaran was not compiled with the 'server' feature"
            )),
            not(feature = "web_viewer") => Err(anyhow::anyhow!(
                "Can't host web-viewer - dalaran was not compiled with the 'web_viewer' feature"
            )),
            _ => {
    ...
    } else if !wants_new && args.connect.is_none() && is_another_server_already_running(server_addr)
    {
        ...
        cfg_select! {
            ...
            _ => Err(anyhow::anyhow!(
                "Can't start viewer - dalaran was compiled without the 'native_viewer' feature"
            )),
        }
    }
}
    ...
    let create_app = move |cc: &eframe::CreationContext<'_>| -> dl_viewer::App {
        {
            ...
            async_runtime.spawn_future(async move {
                ...
                match tokio::signal::ctrl_c().await {
                    Ok(()) => {
                        dl_log::info!("Caught Ctrl-C, quitting Dalaran Viewer…");
...
fn connect_to_existing_server(
    ...
    let uri: dl_uri::ProxyUri = format!("dalaran+http://{server_addr}/proxy").parse()?;
...
fn serve_web(
    ...
    if spawn_server {
        ...
        let proxy_url = if server_addr.ip().is_unspecified() || server_addr.ip().is_loopback() {
            format!("dalaran+http://localhost:{}/proxy", server_addr.port())
        } else {
            format!("dalaran+http://{server_addr}/proxy")
        };
...
fn serve_grpc(
    ...
    if !cfg!(feature = "server") {
        anyhow::bail!("Can't host server - dalaran was not compiled with the 'server' feature");
    }
    ...
    fn detach_relaunches_a_native_viewer_exactly_once() {
        for cli_args in [
            ...
            let child = Args::try_parse_from(std::iter::chain(
                std::iter::once(std::ffi::OsStr::new("dalaran")),
                child_args,
            ))
            .unwrap();
