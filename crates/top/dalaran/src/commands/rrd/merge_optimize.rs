...
pub struct OptimizeCommand {
    ...
    /// `dalaran rrd optimize` is always headless.
...
/// Stub for the old `dalaran rrd compact` name. Accepts any arguments and errors out with a
...
impl CompactCommand {
    ...
    pub fn run(&self) -> anyhow::Result<()> {
        anyhow::bail!(
            "`dalaran rrd compact` has been renamed to `dalaran rrd optimize`. \
             Please run `dalaran rrd optimize --help` for usage."
        )
    }
}
