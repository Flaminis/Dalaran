...
pub enum RrdCommands {
    ...
    ///
    /// Example: `dalaran rrd migrate foo.rrd`
    ...
    ///   `dalaran rrd optimize my.rrd -o my-compacted.rrd`
    ///
    /// * Merge many recordings into one optimized file:
    ///   `dalaran rrd optimize --max-size 2MiB /my/recordings/*.rrd -o output.rrd`
    ///
    /// * Pipe through stdin/stdout, overriding both row and size thresholds:
    ///   `cat my.rrd | dalaran rrd optimize --max-rows 4096 --max-size 2MiB > output.rrd`
    ///
    /// * Directory mirror mode — optimize every `.rrd`/`.rbl` under a tree, preserving structure:
    ///   `dalaran rrd optimize --max-size 2MiB /my/recordings -o /my/recordings-compacted`
    ...
    /// Example: `dalaran rrd route --recording-id my_recording /my/recordings/*.rrd > output.rrd`
    ///
