//! `dalaran mcap check` — check an MCAP file for structural and timeline issues.
//!
//! Each MCAP chunk holds messages from many topics interleaved together.
//! When loaded into Dalaran, an MCAP chunk is split into one Dalaran chunk per topic, and each Dalaran
...
/// Full mode: run the decoder pipeline and inspect every dalaran chunk it emits.
