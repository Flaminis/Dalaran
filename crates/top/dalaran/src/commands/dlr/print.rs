...
impl PrintCommand {
    pub fn run(self) -> anyhow::Result<()> {
        ...
        if migrate {
            println!("Showing data after migration to latest Dalaran version");
        } else {
        ...
        if footers {
            for (_, dlr_manifests) in rx_done {
                for (source, dlr_manifest) in dlr_manifests {
                    ...
                    let filter_lod_1 = |f: &arrow::datatypes::Field| {
                        footers_lod >= 1
                            && ((f.name().ends_with(":start") || f.name().ends_with(":end"))
                                && !f.metadata().contains_key("dalaran:component"))
                    };
                    ...
                    let filtered = dlr_manifest.data.filter_columns_by(|f| {
            ...
            if options.migrate {
                ...
                if let Some(only_this_entity) = &options.entity
                    ...
                    && let Some(chunk_entity_path) = metadata
                        ...
                        .or_else(|| metadata.get("dalaran.entity_path"))
