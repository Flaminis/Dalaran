...
impl StatsCommand {
    pub fn run(&self) -> anyhow::Result<()> {
        ...
        let (rx_raw, rx_footers) = read_raw_dlr_streams_from_file_or_stdin(path_to_input_rrds);
...
/// Prints statistics about the DLR footer(s), i.e. the `RrdManifest`s carried by the trailing
...
fn print_footer_stats(
    ...
    if footers.is_empty() {
        println!("(none — no DLR footer was found)");
        return Ok(());
    }
        ...
        let manifest = match res {
            ...
            Err(err) => {
                ...
                if !continue_on_error {
                    anyhow::bail!(
                        "one or more corrupt DLR footers in the input stream (check logs)"
                    )
                }
                continue;
            }
    ...
    if let Msg::ArrowMsg(arrow_msg) = msg {
        ...
        let app = if app {
            ...
            let entity_path = {
                ...
                let entity_path =
                    entity_path.or_else(|| schema.metadata().get("dalaran.entity_path"));
            ...
            let indexes = schema
                ...
                .filter(|&field| {
                    field
                        .metadata()
                        .get(dl_sorbet::metadata::DALARAN_KIND)
                        .map(|s| s.as_str())
                        == Some("index")
                        || field.metadata().get("dalaran.kind").map(|s| s.as_str()) == Some("index")
                })
            ...
            let components = schema
                ...
                .filter(|&field| {
                    field
                        ...
                        .map(|s| s.as_str())
                        == Some("data")
                        || field.metadata().get("dalaran.kind").map(|s| s.as_str()) == Some("data")
                })
                .map(|field| field.name().to_owned())
                .collect_vec();
