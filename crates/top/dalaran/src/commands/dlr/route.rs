...
impl RouteCommand {
    pub fn run(&self) -> anyhow::Result<()> {
        ...
        let (rx, rx_footers) = read_raw_dlr_streams_from_file_or_stdin(path_to_input_rrds);
...
fn process_messages<W: std::io::Write>(
    ...
    // TODO(grtlr): encoding should match the original (just like in `dlr stats`).
        ...
        match res {
            Ok(mut msg) => {
                ...
                let (byte_span_excluding_header, byte_size_uncompressed) = unsafe {
                    // Reminder: this will implicitly discard DLR footers.
    ...
    let mut dlr_footer = dl_log_encoding::RrdFooter::default();
    let mut i = 0;
    for (_, manifests) in rx_footers {
        for (_, res) in manifests {
            ...
            // NOTE: We currently enforce a single DLR footer per recording to keep things
            // manageable end-to-end, therefore we must merge the manifests if multiple recordings
            // get routed to the same ID.
            match dlr_footer.manifests.entry(patched_store_id.clone()) {
    ...
    for manifest in dlr_footer.manifests.values_mut() {
    ...
    #[expect(unsafe_code)]
    unsafe {
        encoder.finish_with_custom_footer(&dlr_footer)?;
    }
