...
pub struct MergeCommand {
    ...
    #[arg(short = 'o', long = "output", value_name = "dst.(dlr|dbl)")]
...
pub struct OptimizeCommand {
    ...
    /// underneath it, with each `.dlr`/`.dbl` file optimized independently.
    #[arg(short = 'o', long = "output", value_name = "dst.(dlr|dbl)")]
    path_to_output_dlr: Option<String>,

    /// Optimization profile to start from.
    ///
    /// Per-knob flags and `DALARAN_CHUNK_MAX_*` env vars override the profile's
    /// values. `DALARAN_STORE_ENABLE_CHANGELOG` is ignored by this command —
    /// `dalaran dlr optimize` is always headless.
    ...
    ///
    /// Compaction in Dalaran is an iterative, convergent process: every single pass will improve the
    ...
    ///
    /// By default, `dlr optimize` validates user-supplied keyframe labels against
...
impl OptimizeCommand {
    pub fn run(&self) -> anyhow::Result<()> {
        ...
        // to its `*.dlr`/`*.dbl` files and optimize each one independently, mirroring
        ...
        if any_input_is_dir {
            let output_root = path_to_output_dlr.as_ref().ok_or_else(|| {
                anyhow::anyhow!(
...
/// Walk every input (file or directory), pair each `*.dlr`/`*.dbl` with an output
...
/// Stub for the old `dalaran dlr compact` name. Accepts any arguments and errors out with a
...
impl CompactCommand {
    ...
    pub fn run(&self) -> anyhow::Result<()> {
        anyhow::bail!(
            "`dalaran dlr compact` has been renamed to `dalaran dlr optimize`. \
             Please run `dalaran dlr optimize --help` for usage."
        )
    }
}
...
fn merge_and_compact(
    ...
    let (rx, rx_size_bytes) = read_dlr_streams_from_file_or_stdin(path_to_input_rrds);
    ...
    let mut dlr_out = if let Some(path) = path_to_output_dlr {
    ...
    let dlr_out_size = dl_log_encoding::Encoder::encode_into(
        version,
        encoding_options,
    ...
    dlr_out.flush().context("couldn't flush output")?;
    ...
    let size_reduction = if let (Some(rrds_in_size), dlr_out_size) = (rrds_in_size, dlr_out_size) {
        format!(
            "-{:3.3}%",
            100.0 - dlr_out_size as f64 / (rrds_in_size as f64 + f64::EPSILON) * 100.0
        )
    } else {
        "N/A".to_owned()
    };
    ...
    dl_log::info!(
        ...
        file_size_to_string(Some(dlr_out_size)),
    );
