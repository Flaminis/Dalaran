...
pub struct CompareCommand {
    ...
    ///
    /// The Dalaran data model is itself unordered, and because many of the internal pipelines are
...
impl CompareCommand {
    /// Checks whether two .dlr files are _similar_, i.e. not equal on a byte-level but
...
/// Given a path to an dlr file, builds up a `ChunkStore` and returns its contents a stream of
/// `Chunk`s.
///
/// Fails if there are more than one data recordings present in the dlr file.
fn load_chunks(
    path_to_dlr: &Path,
    ignore_chunks_without_components: bool,
    ...
    let dlr_file = std::fs::File::open(path_to_dlr).context("couldn't open dlr file contents")?;
    ...
    // the ability of `dlr compare` pre-0.25 rrds. If we want to keep the ability to migrate here,
    ...
    let decoder = dl_log_encoding::DecoderApp::decode_lazy(dlr_file);
    for msg in decoder {
        let msg = msg.context("decode dlr message")?;
        stores
            ...
            .add_log_msg(&msg)
            .context("decode dlr file contents")?;
    }
    ...
    anyhow::ensure!(!stores.is_empty(), "no data recording found in dlr file");
    anyhow::ensure!(
        stores.len() == 1,
        "more than one data recording found in dlr file"
    );
