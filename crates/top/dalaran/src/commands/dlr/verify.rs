...
use crate::commands::{read_dlr_streams_from_file_or_stdin, stdio::InputSource};
...
impl VerifyCommand {
    pub fn run(&self) -> anyhow::Result<()> {
        ...
        let (rx, rx_done) = read_dlr_streams_from_file_or_stdin(path_to_input_rrds);
        ...
        if *check_footers {
            for (_, dlr_manifests) in rx_done {
                for (source, dlr_manifest) in dlr_manifests {
                    let dlr_manifest = match dlr_manifest {
                        Ok(m) => m,

                        Err(err) => {
                            verifier
                                .errors
                                .insert(format!("{source}: Corrupt DLR manifest: {err:#}"));
                            continue;
                        }
                    };

                    let dlr_footer = seen_files.entry(source.clone()).or_default();
                    if dlr_footer
                        .manifests
                        .insert(dlr_manifest.store_id.clone(), dlr_manifest.clone())
                        .is_some()
                    {
                        verifier.errors.insert(format!(
                            "{source}/{}/{}: More than one DLR manifest",
                            dlr_manifest.store_id.application_id(),
                            dlr_manifest.store_id.recording_id(),
                        ));
                    }

                    if let InputSource::File(path) = &source
                        && let Err(err) =
                            load_from_dlr_filepath_with_dlr_manifest(path, &dlr_manifest)
                    {
                        verifier.errors.insert(format!(
                            "{source}: Corrupt DLR manifest, cannot deserialize chunk data: {err:#}"
                        ));
                    }
                }
            }

            for (source, dlr_footer) in &seen_files {
                if dlr_footer.manifests.is_empty() {
                    verifier
                        .errors
                        .insert(format!("{source}: Missing DLR footer / no DLR manifests"));
                }
            }
        }
...
impl Verifier {
    ...
    fn verify_component_column(
        ...
        if !component_type.full_name().starts_with("dalaran.") {
            dl_log::debug_once!("Ignoring non-Dalaran component {component_type:?}");
            return Ok(());
        }

        if component.starts_with("dalaran.components.") && component.ends_with("Indicator") {
        ...
        if let Some(archetype_name) = archetype_name {
            if archetype_name.full_name().starts_with("dalaran.") {
                // Verify archetype.
                ...
                if &component_type != expected_component_type {
                    return Err(anyhow::anyhow!(
                        "Component {component:?} of {archetype_name:?} has type {expected_component_type:?} in this version of Dalaran, but the data column has type {component_type:?}"
                    ));
                }
            } else {
                dl_log::debug_once!("Ignoring non-Dalaran archetype {archetype_name:?}");
            }
        }
...
/// Attempts to deserialize chunks according to the given DLR manifest.
///
/// Useful to check for corrupt manifests.
fn load_from_dlr_filepath_with_dlr_manifest(
    path_to_dlr: impl AsRef<std::path::Path>,
    dlr_manifest: &RawRrdManifest,
) -> anyhow::Result<()> {
    ...
    let mut file = std::fs::File::open(path_to_dlr)?;

    let chunk_ids = dlr_manifest.col_chunk_id()?;
    let byte_offsets = dlr_manifest.col_chunk_byte_offset()?;
    let byte_sizes = dlr_manifest.col_chunk_byte_size()?;
    ...
    for (chunk_id, offset, size) in itertools::izip!(chunk_ids, byte_offsets, byte_sizes) {
        ...
        let chunk = dl_protos::log_msg::v1alpha1::ArrowMsg::from_dlr_bytes(&buf)?;
        anyhow::ensure!(
            chunk_id
            ...
            "corrupt DLR manifest, chunk IDs don't align"
        );
    }
