...
impl FilterCommand {
    pub fn run(&self) -> anyhow::Result<()> {
        ...
        let path_to_output_dlr = path_to_output_dlr.clone();
        ...
        let (rx_decoder, rx_size_bytes) = read_dlr_streams_from_file_or_stdin(path_to_input_rrds);

        // TODO(cmc): might want to make this configurable at some point.
        let (tx_encoder, rx_encoder) = crossbeam::channel::bounded(100);

        let encoding_handle = std::thread::Builder::new()
            .name("dalaran-dlr-filter-out".to_owned())
            .spawn(move || -> anyhow::Result<u64> {
                use std::io::Write as _;

                let mut dlr_out = if let Some(path) = path_to_output_dlr.as_ref() {
        ...
        let dlr_out_size = encoding_handle
        ...
        let size_reduction =
            if let (Some(rrds_in_size), dlr_out_size) = (rrds_in_size, dlr_out_size) {
                format!(
                    "-{:3.3}%",
        ...
        dl_log::info!(
            dst_size_bytes = %file_size_to_string(Some(dlr_out_size)),
            time = ?now.elapsed(),
            size_reduction,
