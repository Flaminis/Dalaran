...
impl SplitCommand {
    pub fn run(&self) -> anyhow::Result<()> {
        ...
        let input_dlr_stem = PathBuf::from(&path_to_input_dlr)
            .file_stem()
            .with_context(|| {
                format!("couldn't grab file stem from input DLR file {path_to_input_dlr:?}")
            })?
            .to_string_lossy()
            .to_string();
        ...
        dl_log::info!(srcs = ?path_to_input_dlr, "split started");

        // TODO(RR-941): multi-recording DLR files need to go away.
        ...
        let (rx_decoder, rx_size_bytes) =
            read_dlr_streams_from_file_or_stdin(std::slice::from_ref(path_to_input_dlr));

        // We need to keep track of the non-data messages (stateful store switches, blueprint
        // activations), so that we can properly rebuild the final DLR files.
        //
                ...
                .spawn({
                    ...
                    move || -> anyhow::Result<(String, u64)> {
                        ...
                        let mut encoder = {
                            ...
                            dl_log_encoding::Encoder::new_eager(version, options, &mut dlr_out)
        ...
        let dlr_in_size = rx_size_bytes.recv().ok().map(|(size, _footers)| size);
        ...
        let mut dlr_out_paths = Vec::new();
        let mut dlr_out_sizes = Vec::new();
        for handle in encoding_handles {
            let (dlr_out_path, dlr_out_size) = handle
                .context("couldn't spawn IO thread")?
                .join()
                .map_err(|err| anyhow::anyhow!("Unknown error: {err:?}"))??; // NOLINT: there is no `Display` for this `err`
            dlr_out_paths.push(dlr_out_path);
            dlr_out_sizes.push(file_size_to_string(Some(dlr_out_size)));
        }

        dl_log::info!(
            src = path_to_input_dlr,
            src_size_bytes = %file_size_to_string(dlr_in_size),
            dsts = ?dlr_out_paths,
            dsts_size_bytes = ?dlr_out_sizes,
            time = ?now.elapsed(),
            "split finished"
        );
