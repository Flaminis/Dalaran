...
impl MigrateCommand {
    pub fn run(&self) -> anyhow::Result<()> {
        ...
        path_to_input_rrds.retain(|f| !f.to_string().ends_with(".backup.dlr"));

        let num_files = path_to_input_rrds.len();

        if num_files < num_files_before {
            eprintln!(
                "Ignored {} file(s) that are called .backup.dlr, and are therefore assumed to already have been migrated",
        ...
        eprintln!("Migrating {num_files} .dlr file(s)…");

        let progress =
            ProgressBar::new(path_to_input_rrds.len() as u64).with_message("Migrating dlr:s");
...
fn migrate_file_at(original_path: &Utf8PathBuf) -> anyhow::Result<()> {
    // Rename `old_name.dlr` to `old_name.backup.dlr`:
    let backup_path = original_path.with_extension("backup.dlr");
...
fn migrate_from_to(from_path: &Utf8PathBuf, to_path: &Utf8PathBuf) -> anyhow::Result<()> {
    ...
    // the ability of `dlr migrate` as well. If we want to keep the ability to migrate here, then
    ...
    .with_context(|| format!("Failed to write new .dlr file to {to_path:?}"))?;
