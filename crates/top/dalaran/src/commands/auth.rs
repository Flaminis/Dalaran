...
pub enum AuthCommands {
    ...
    ///
    /// The access token is part of the credentials produced by `dalaran auth login`,
    /// and is used to authorize requests to Dalaran Hub.
    Token(TokenCommand),
    ...
    pub fn run(self, runtime: &tokio::runtime::Handle) -> Result<(), dl_auth::cli::Error> {
        match self {
            ...
            Self::GenerateToken(args) => {
                let server = parse_http_or_dalaran_uri(&args.server)
...
impl std::fmt::Display for InvalidUri {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "invalid uri, expected one of: `dalaran://`, `dalaran+http://`, `dalaran+https://`, `http://`, `https://`"
        )
    }
}
...
fn parse_http_or_dalaran_uri(s: &str) -> Result<url::Origin, InvalidUri> {
