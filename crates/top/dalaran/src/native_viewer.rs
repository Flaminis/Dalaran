...
use dl_viewer::external::eframe;

/// Starts a Dalaran viewer to visualize the contents of a given array of messages.
...
pub fn show(main_thread_token: crate::MainThreadToken, msgs: Vec<LogMsg>) -> eframe::Result {
    let log_source = dl_log_channel::LogSource::Sdk; // Whatever. Nobody is really using this functions anyhow.

    if msgs.is_empty() {
        dl_log::debug!("Empty array of msgs - call to show() ignored");
        return Ok(());
    }

    let store_source = dl_log_types::StoreSource::RustSdk {
    ...
    let (tx, rx) = dl_log_channel::log_channel(log_source);
    for log_msg in msgs {
    ...
    dl_viewer::run_native_app(
        ...
        Box::new(move |cc| {
            let mut app = dl_viewer::App::new(
                ...
                dl_viewer::AppEnvironment::from_store_source(&store_source),
                startup_options,
                cc,
                None,
                dl_viewer::AsyncRuntimeHandle::new_native(runtime.handle().clone()),
            );
