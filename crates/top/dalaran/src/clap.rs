...
/// This struct implements a `clap::Parser` that defines all the arguments that a typical Dalaran
...
#[clap(author, version, about)]
pub struct DalaranArgs {
    /// Start a new Dalaran Viewer process and feed it data in real-time.
    ...
    /// Log data to standard output, to be piped into a Dalaran Viewer.
    #[clap(long, short = 'o')]
    stdout: bool,

    /// Connects and sends the logged data to a remote Dalaran viewer.
    ///
    ...
    /// Connects and sends the logged data to a web-based Dalaran viewer.
    #[cfg(feature = "web_viewer")]
    #[clap(long)]
    serve: bool,
...
/// [`DalaranArgs::init`] might have to spawn a bunch of background tasks depending on what arguments
/// were passed in.
...
impl DalaranArgs {
    ...
    fn init_with_blueprint_opts(
        ...
        match self.to_behavior()? {
            DalaranBehavior::Stdout => Ok((builder.stdout()?, Default::default())),

            DalaranBehavior::Connect(url) => {
                Ok((builder.connect_grpc_opts(url)?, Default::default()))
            }

            DalaranBehavior::Save(path) => Ok((builder.save(path)?, Default::default())),

            DalaranBehavior::Spawn => Ok((builder.spawn()?, Default::default())),

            #[cfg(feature = "web_viewer")]
            DalaranBehavior::Serve => {
                ...
                crate::serve_web_viewer(crate::web_viewer::WebViewerConfig {
                    open_browser: true,
                    connect_to: vec!["dalaran+http://localhost:9876/proxy".to_owned()],
                    ..Default::default()
                })?
                .detach();
    ...
    fn to_behavior(&self) -> anyhow::Result<DalaranBehavior> {
        if self.stdout {
            return Ok(DalaranBehavior::Stdout);
        }

        if let Some(path) = self.save.as_ref() {
            return Ok(DalaranBehavior::Save(path.clone()));
        }

        #[cfg(feature = "web_viewer")]
        if self.serve {
            return Ok(DalaranBehavior::Serve);
        }

        match &self.connect {
            Some(Some(url)) => return Ok(DalaranBehavior::Connect(url.clone())),
            Some(None) => {
                return Ok(DalaranBehavior::Connect(
