...
/// Implements a [`log::Log`] that forwards all events to the Dalaran SDK.
///
/// ```
/// let rec = dalaran::RecordingStreamBuilder::new("dalaran_example_app").buffered()?;
///
...
fn build_filter(filter: &str) -> env_filter::Filter {
    ...
    if let Err(err) = builder.try_parse(filter) {
        dl_log::warn!("Ignoring invalid log filter: {err}\nFilter: {filter}");
    }
    builder.build()
}
...
impl Logger {
    ...
    pub fn init(mut self) -> Result<(), log::SetLoggerError> {
        if self.filter.is_none() {
            self.filter = Some(build_filter(&dl_log::default_log_filter()));
        }
...
impl log::Log for Logger {
    ...
    fn log(&self, record: &log::Record<'_>) {
        ...
        let level = log_level_to_dalaran_level(record.metadata().level());
...
fn log_level_to_dalaran_level(lvl: log::Level) -> TextLogLevel {
    match lvl {
