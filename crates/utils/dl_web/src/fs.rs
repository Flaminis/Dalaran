...
pub async fn metadata(path: &Path) -> io::Result<Metadata> {
    ...
    dl_async::spawn_local_with_result(async move {
...
pub async fn read_file(file: web_sys::File) -> io::Result<Vec<u8>> {
    dl_async::spawn_local_with_result(async move { read_blob(file.as_ref()).await })
        .await
        .map_err(io::Error::other)?
}
...
impl File {
    ...
    pub async fn open(path: impl AsRef<Path>) -> io::Result<Self> {
        ...
        let file = dl_async::spawn_local_with_result(async move {
            let file_handle = open_file(&path).await?;
...
impl dl_async::AsyncReadAt for File {
...
mod test {
    ...
    use dl_async::AsyncReadAt as _;
    use wasm_bindgen_test::wasm_bindgen_test;

    wasm_bindgen_test::wasm_bindgen_test_configure!(run_in_browser);

    fn unique_opfs_test_dir() -> String {
        format!("opfs-test-{}", dl_tuid::Tuid::new())
    }
