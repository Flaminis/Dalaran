...
pub fn arrays_to_list_array(
    ...
    let data = if arrays_dense.is_empty() {
        ...
        dl_tracing::profile_scope!("concatenate", arrays_dense.len().to_string());
        concat_arrays(&arrays_dense)
            .map_err(|err| {
                dl_log::warn_once!("failed to concatenate arrays: {err}");
                err
            })
            .ok()?
    };
...
pub fn wrap_in_list_array(field: &Field, array: ArrayRef) -> (Field, ListArray) {
    ...
    dl_log::debug_assert_eq!(field.data_type(), array.data_type());
