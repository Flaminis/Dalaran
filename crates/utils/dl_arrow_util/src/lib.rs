...
/// aligner ([`align_record_batch_to_schema`](../../dl_dataframe/utils/fn.align_record_batch_to_schema.html))
...
mod reject_unsupported_widenings_tests {
    ...
    /// Documents a known over-rejection that surfaces in `dl_server`'s `add_layer` flow
    /// (`crates/store/dl_server/src/store/dataset.rs`): when a new field differs from the
