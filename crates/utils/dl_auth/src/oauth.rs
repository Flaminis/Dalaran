...
pub fn load_credentials() -> Result<Option<Credentials>, CredentialsLoadError> {
    if let Some(credentials) = storage::load()? {
        dl_log::debug_once!("Found credentials for {}", credentials.user.email);
        Ok(Some(credentials))
    } else {
        dl_log::debug_once!("No credentials stored locally");
        Ok(None)
    }
}
...
pub fn clear_credentials(
        ...
        cfg_select! {
            ...
            _ => {
                ...
                match crate::callback_server::start_logout_server(&creds.claims.sid) {
                    ...
                    Err(err) => {
                        dl_log::warn!("Failed to start logout callback server: {err}");
                        LogoutOutcome {
    ...
    storage::clear()?;
    dl_analytics::set_logged_in(false);
...
pub async fn refresh_credentials_with_org(
    ...
    dl_log::debug!("credentials refreshed successfully");
    Ok(credentials)
}
...
impl Credentials {
    ...
    pub fn link_analytics_id_to_user(&self) {
        dl_log::debug!("Linking analytics ID to user: '{}'", self.user.email);
        dl_analytics::set_logged_in(true);
        dl_analytics::record(|| dl_analytics::event::SetPersonProperty {
