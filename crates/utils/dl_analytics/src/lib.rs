...
impl Drop for Analytics {
    fn drop(&mut self) {
        ...
        {
            dl_log::debug!("Failed to flush analytics events during shutdown: {err}");
        }
    }
}
...
fn load_config(_user_logged_in: bool) -> Result<Config, ConfigError> {
    let config = match Config::load() {
        ...
        Err(err) => {
            ...
            if !cfg!(target_arch = "wasm32") {
                dl_log::warn!("failed to load analytics config file: {err}");
            }
            None
        }
    };
    ...
    if let Some(config) = config {
        dl_log::trace!(?config, "loaded analytics config");

        Ok(config)
    } else {
        dl_log::trace!(?config, "initialized analytics config");
        ...
        cfg_select! {
            target_arch = "wasm32" => {
                ...
                config.save()?;
                dl_log::trace!(?config, "saved analytics config");
            }
            _ => {
...
impl Analytics {
    ...
    pub fn global_or_init_with_login_state(user_logged_in: bool) -> Option<&'static Self> {
        GLOBAL_ANALYTICS
            .get_or_init(|| match Self::new(Duration::from_secs(2), user_logged_in) {
                ...
                Err(err) => {
                    dl_log::error!("Failed to initialize analytics: {err}");
                    None
                }
            })
            .as_ref()
    ...
    fn new(tick: Duration, user_logged_in: bool) -> Result<Self, AnalyticsError> {
        ...
        dl_log::trace!("initialized analytics pipeline");
...
///
/// For example, [`dl_build_info::BuildInfo`] has an implementation of this trait,
...
impl Properties for dl_build_info::BuildInfo {
    fn serialize(self, event: &mut AnalyticsEvent) {
