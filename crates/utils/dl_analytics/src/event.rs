...
use dl_build_info::{BuildInfo, CrateVersion};
use url::Url;
...
/// Used in `dl_viewer`.
pub struct Identify {
    /// Info on how the `dl_viewer` crate was built.
    pub build_info: dl_build_info::BuildInfo,
...
pub struct ViewerRuntimeInformation {
    ...
    /// The device tier `dl_renderer` identified for the graphics adapter.
    ///
    /// For possible values see [`dl_renderer::config::DeviceTier`](https://docs.rs/dl_renderer/latest/dl_renderer/config/enum.DeviceTier.html).
