...
impl Pipeline {
    pub(crate) fn new(config: &Config, tick: Duration) -> Result<Option<Self>, PipelineError> {
        if dl_log::env_var_is_truthy(ENV_FORCE_ANALYTICS) {
            dl_log::debug_once!("Analytics enabled by environment variable");
        } else {
            if !config.analytics_enabled {
                dl_log::debug_once!("Analytics disabled by configuration");
                return Ok(None);
            }
            if std::env::var("CI").is_ok() {
                dl_log::debug_once!("Analytics disabled on CI");
                return Ok(None);
            }
            if cfg!(feature = "testing") {
                dl_log::debug_once!("Analytics disabled in tests");
                return Ok(None);
            }
            if cfg!(debug_assertions) {
                dl_log::debug_once!("Analytics disabled in debug builds");
                return Ok(None);
            }
        }
        ...
        if let Err(err) = std::thread::Builder::new()
            ...
            .spawn({
                ...
                move || {
                    ...
                    dl_log::trace!(%analytics_id, %session_id, "pipeline catchup thread started");
                    let res = flush_pending_events(&config, &sink, &abort_signal);
                    dl_log::trace!(%analytics_id, %session_id, ?res, "pipeline catchup thread shut down");
                }
            })
        {
            dl_log::debug!("Failed to spawn analytics thread: {err}");
        }
        ...
        if let Err(err) = std::thread::Builder::new().name("pipeline".into()).spawn({
            ...
            move || {
                ...
                dl_log::trace!(%analytics_id, %session_id, "pipeline thread started");
                realtime_pipeline(
                    &config,
                    &sink,
                    session_file,
                ...
                dl_log::trace!(%analytics_id, %session_id, "pipeline thread shut down");
            }
        }) {
            dl_log::debug!("Failed to spawn analytics thread: {err}");
        }
    ...
    pub fn flush_blocking(&self, timeout: Duration) -> Result<(), FlushError> {
        ...
        dl_log::trace!("Flushing analytics events…");
...
fn try_send_event(event_tx: &channel::Sender<PipelineEvent>, event: PipelineEvent) {
    match event_tx.try_send(event) {
        ...
        Err(channel::TrySendError::Full(_)) => {
            dl_log::trace!("dropped event, analytics channel is full");
        }
        Err(channel::TrySendError::Disconnected(_)) => {
            ...
            // error instead.
            dl_log::debug_once!("dropped event, analytics channel is disconnected");
        }
    }
}
    ...
    for entry in read_dir {
        ...
        if metadata.is_file() {
            ...
            match flush_events(
                ...
                Ok(()) => {
                    dl_log::trace!(%analytics_id, %session_id, ?path, "flushed pending events");
                    match std::fs::remove_file(&path) {
                        Ok(()) => {
                            dl_log::trace!(%analytics_id, %session_id, ?path, "removed session file");
                        }
                        Err(err) => {
                            // NOTE: this will eventually lead to duplicated data, though we'll be
                            // able to deduplicate it at query time.
                            dl_log::trace!(%analytics_id, %session_id, ?path, %err,
                                "failed to remove session file");
                        }
                    }
                }
                Err(err) => dl_log::trace!(%analytics_id, %session_id, ?path, %err,
...
fn realtime_pipeline(
    ...
    let on_flush = |session_file: &mut _| {
        ...
        {
            dl_log::debug_once!("couldn't flush analytics data file: {err}");
            // We couldn't flush the session file: keep it intact so that we can retry later.
            return abort_signal.abort();
        }

        if let Err(err) = session_file.set_len(0) {
            dl_log::debug_once!("couldn't truncate analytics data file: {err}");
        ...
        if let Err(err) = session_file.rewind() {
            ...
            // never happen.
            dl_log::debug_once!("couldn't seek into analytics data file: {err}");
            return abort_signal.abort();
        }
    };
            ...
            recv(event_rx) -> event => {
                ...
                match event {
                    ...
                    PipelineEvent::Flush => {
                        ...
                        dl_quota_channel::send_crossbeam(flush_done_tx, ()).ok();
                    },
                }
...
fn append_event(
    ...
    let mut event_str = match serde_json::to_string(&event) {
        ...
        Err(err) => {
            dl_log::debug!(%err, %analytics_id, %session_id, "corrupt analytics event: discarding");
            return Ok(());
        }
    };
    event_str.push('\n');
    ...
    if let Err(err) = session_file.write_all(event_str.as_bytes()) {
        ...
        dl_log::debug!(%err, %analytics_id, %session_id, "couldn't write to analytics data file");
        return Err(event);
    }
...
fn flush_events(
    ...
    if let Err(err) = session_file.rewind() {
        dl_log::debug!(%err, %analytics_id, %session_id, "couldn't seek into analytics data file");
        return Err(err);
    }
            ...
            Ok(event_str) => {
                match serde_json::from_str::<AnalyticsEvent>(&event_str) {
                    ...
                    Err(err) => {
                        ...
                        dl_log::debug!(%err, %analytics_id, %session_id,
