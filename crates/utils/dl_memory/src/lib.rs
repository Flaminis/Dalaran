...
//! ## First steps
//!
//! Add `dl_memory` to your `Cargo.toml`:
//!
//! ```toml
//! cargo add dl_memory
//! ```
...
//!
//! ```rs
//! dl_memory::accounting_allocator::set_tracking_callstacks(true);
//! ```
//!
...
impl std::fmt::Debug for CountAndSize {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        f.debug_struct("CountAndSize")
            .field("count", &dl_format::format_uint(*count))
            .field("size", &dl_format::format_bytes(*size as _))
            .finish()
    }
}
