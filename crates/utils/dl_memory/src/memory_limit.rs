...
///
/// It is recommended that they log using [`dl_log::info_once`] when they
...
impl std::fmt::Display for MemoryLimit {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self.max_bytes {
            Some(max_bytes) => write!(f, "{}", dl_format::format_bytes(max_bytes as _)),
...
impl MemoryLimit {
    ...
    pub fn from_fraction_of_total(fraction: f32) -> Self {
        ...
        if let Some(total_memory) = total_memory {
            ...
            dl_log::debug_once!(
                ...
                dl_format::format_bytes(max_bytes),
                100.0 * fraction,
                dl_format::format_bytes(total_memory as _),
            );
    ...
    pub fn parse(limit: &str) -> Result<Self, String> {
        if limit == "0" {
            ...
            let num_bytes = dl_format::parse_bytes(limit)
