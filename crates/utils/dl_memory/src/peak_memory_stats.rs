...
impl PeakMemoryStats {
    ...
    pub fn finish(self) -> Option<TrackingStatistics> {
        ...
        dl_quota_channel::send_crossbeam(&shutdown_tx, Shutdown).ok();

        match handle.join() {
            Err(err) => {
                dl_log::warn_once!("Failed to collect PeakMemoryStates: {err:?}"); // NOLINT: err does not implement Display
                None
            }
            Ok(result) => result,
        }
...
fn current_memory_use() -> usize {
    if let Some(stats) = crate::accounting_allocator::global_allocs() {
        ...
        dl_log::error_once!("accounting_allocator is OFF. Can't collect peak memory use");
        0
    }
}
