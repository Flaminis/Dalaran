...
pub fn new_client_telemetry_layer() -> ClientTelemetryLayer {
    let trace_layer = tower_http::trace::TraceLayer::new_for_grpc()
        // Note: we're actually disabling all DEBUG level logs for `tower` in dl_log, so if you want to enable it
        // you'll need to adjust that as well. See crates/utils/dl_log/src/lib.rs
