...
/// don't spam the log.
#[derive(Debug)]
struct AuthRefreshingSpanExporter<P: dl_auth::credentials::CredentialsProvider> {
...
where
    P: dl_auth::credentials::CredentialsProvider + Send + Sync + std::fmt::Debug + 'static,
{
    async fn export(
        &self,
    ...
    P: dl_auth::credentials::CredentialsProvider + Send + Sync + std::fmt::Debug + 'static,
{
    ...
    // and so the TLS config matches `dl_redap_client` (rustls + system roots
...
impl Telemetry {
    ...
    /// `dl_perf_telemetry` never see the extra public API.
    ...
    ///
    /// ```ignore
    /// use dl_perf_telemetry::{Telemetry, TelemetryArgs, TelemetryDropBehavior};
    ///
...
mod tests {
    ...
    /// then `{}` then a stub signature. `dl_auth::Jwt::try_from` only checks
