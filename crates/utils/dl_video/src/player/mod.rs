...
impl<T: Default> dl_byte_size::SizeBytes for VideoPlayer<T> {
...
impl<T: Default> Drop for VideoPlayer<T> {
    fn drop(&mut self) {
        dl_log::trace!("Dropping VideoPlayer {:?}", self.debug_name());
    }
}
...
impl<T: Default> VideoPlayer<T> {
    ...
    pub fn new(
        ...
        {
            if bit_depth < 8 {
                dl_log::warn_once!("{debug_name} has unusual bit_depth of {bit_depth}");
            } else if 8 < bit_depth {
                dl_log::warn_once!(
    ...
    fn enqueue_samples(
        ...
        {
            dl_log::trace!("Signaling end of video");
