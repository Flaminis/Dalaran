...
impl dl_byte_size::SizeBytes for VideoDataDescription {
...
impl dl_byte_size::SizeBytes for SamplesStatistics {
    fn heap_size_bytes(&self) -> u64 {
        let Self {
...
///
/// The interpretation is up to the caller of [`SampleMetadata::get`]: `dl_video`
...
// Close enough.
#[derive(thiserror::Error, Debug, dl_byte_size::SizeBytes)]
pub enum VideoLoadError {
    ...
    /// Not used in `dl_video` itself, but useful for media type detection ahead of calling [`VideoDataDescription::load_from_bytes`].
