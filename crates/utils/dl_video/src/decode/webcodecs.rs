...
impl Drop for WebVideoDecoder {
    fn drop(&mut self) {
        dl_log::debug!("Dropping WebVideoDecoder");
...
impl AsyncDecoder for WebVideoDecoder {
    ...
    fn submit_chunk(&mut self, video_chunk: Chunk) -> Result<()> {
        ...
        match self.decoder.state() {
            ...
            other => {
                dl_log::warn_once!("Unexpected video decoder state: {other:?}");
            }
        }
    ...
    fn reset(&mut self, video_descr: &VideoDataDescription) -> Result<()> {
        dl_log::trace!("Resetting video decoder.");
        ...
        if let Err(_err) = self.decoder.reset() {
            ...
            dl_log::debug!("Video decoder reset failed, recreating decoder.");
            let (decoder, output_callback_tx) = init_video_decoder(self.output_sender.clone())?;
            self.decoder = decoder;
            self.output_callback_tx = output_callback_tx;
        }
    ...
    fn end_of_video(&mut self) -> Result<()> {
        ...
        dl_async::spawn_local(async move {
            ...
            if let Err(err) = flush_result {
                ...
                dl_log::debug!("Failed to flush video: {}", string_from_js_value(&err));
            }
        });
    ...
    fn min_num_samples_to_enqueue_ahead(&self) -> usize {
        ...
        // `s3://dalaran-redap-datasets-pdx/larger-than-ram-demo/very-large.dlr`. (requires dalaran login)
        //
...
fn init_video_decoder(
    ...
    let (output_callback_tx, output_callback_rx) =
        dl_quota_channel::channel("web_callbacks", 1024 * 1024);
...
pub fn string_from_js_value(value: &wasm_bindgen::JsValue) -> String {
    let error = dl_web::Error::from(value).to_string();
