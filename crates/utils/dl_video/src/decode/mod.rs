...
#[derive(thiserror::Error, Debug, Clone, dl_byte_size::SizeBytes)]
pub enum DecodeError {
    ...
    #[error(transparent)]
    RvlDecoder(#[size_bytes(ignore)] dl_rvl::RvlDecodeError),
...
///
/// For details see `dl_renderer`'s `YuvPixelLayout` type.
...
///
/// For details see `dl_renderer`'s `YuvRange` type.
...
///
/// For details see `dl_renderer`'s `YuvMatrixCoefficients` type.
#[derive(Debug, Clone, Copy)]
