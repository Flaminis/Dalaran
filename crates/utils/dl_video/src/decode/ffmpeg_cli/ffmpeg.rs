...
impl Drop for FFmpegProcessAndListener {
    fn drop(&mut self) {
        ...
        if false {
            {
                dl_tracing::profile_scope!("shutdown write thread");
                if let Some(write_thread) = self.write_thread.take()
                    && write_thread.join().is_err()
                {
                    dl_log::error!("Failed to join ffmpeg listener thread.");
                }
            }
            {
                dl_tracing::profile_scope!("shutdown listen thread");
...
fn write_ffmpeg_input(
    ...
    while let Ok(data) = frame_data_rx.recv() {
        ...
        if let Err(err) = write_result {
            ...
            ffmpeg_stdin.flush().ok();
            dl_log::trace!("Wrote chunk {} to ffmpeg", chunk.sample_idx);
        }
    }
}
    ...
    fn on_frame(
        ...
        let frame_info = loop {
            ...
            let Ok(frame_info) = frame_info_rx.recv() else {
                dl_log::trace!("frame-tx channel closed, stopping ffmpeg decoder");
                return None;
            };
        ...
        match event {
            FfmpegEvent::Log(level, msg) => {
                if msg.contains("system signals, hard exiting") {
                    ...
                    dl_log::debug!("FFmpeg process for {debug_name} was killed");
                    return None;
                }
            ...
            FfmpegEvent::ParsedInput(input) => {
                dl_log::trace!("{debug_name} decoder: {input:?}");
            }

            FfmpegEvent::ParsedOutput(output) => {
                dl_log::trace!("{debug_name} decoder: {output:?}");
            }
            ...
            FfmpegEvent::Done => {
                // This happens on `pkill ffmpeg`, for instance.
                dl_log::trace!("{debug_name}'s ffmpeg is Done");
                return None;
            }
            ...
            FfmpegEvent::ParsedVersion(ffmpeg_version) => {
                ...
                if let Some(ffmpeg_version) = FFmpegVersion::parse(&ffmpeg_version.version) {
                    dl_log::debug_once!("Parsed FFmpeg version: {ffmpeg_version}");
            ...
            FfmpegEvent::OutputChunk(_) => {
                ...
                dl_log::error!("Unexpected ffmpeg output chunk for {debug_name}");
                send_output(
                    output_sender,
...
pub(super) fn check_ffmpeg_version(
    ...
    match ffmpeg_version_result {
        ...
        Err(FFmpegVersionParseError::ParseVersion { raw_version }) => {
            ...
            dl_log::warn_once!("Failed to parse FFmpeg version: {raw_version}");
            Ok(())
        }
...
impl AsyncDecoder for FFmpegCliDecoder {
    ...
    fn end_of_video(&mut self) -> crate::decode::Result<()> {
        dl_log::trace!("End of video - flushing ffmpeg decoder {}", self.debug_name);
    ...
    fn reset(&mut self, video_descr: &VideoDataDescription) -> crate::decode::Result<()> {
        dl_tracing::profile_function!();
        dl_log::trace!("Resetting ffmpeg decoder {}", self.debug_name);
