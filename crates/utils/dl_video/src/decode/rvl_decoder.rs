...
impl SyncDecoder for RvlDecoder {
    fn submit_chunk(
        ...
        chunk: super::Chunk,
        output_sender: &dl_quota_channel::Sender<super::FrameResult>,
    ) {
        if should_stop.load(std::sync::atomic::Ordering::Relaxed) {
            return;
        }

        let meta = match dl_rvl::RosRvlMetadata::parse(&chunk.data) {
            Ok(meta) => meta,
            Err(err) => {
        ...
        let data = match dl_rvl::decode_rvl_with_quantization(&chunk.data, &meta) {
            Ok(data) => data,
            Err(err) => {
                let _send_error = output_sender.send(Err(super::DecodeError::RvlDecoder(err)));
                return;
