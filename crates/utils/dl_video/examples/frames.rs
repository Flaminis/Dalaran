...
fn main() {
    ...
    let video = dl_video::VideoDataDescription::load_mp4(&video_blob, video_path)
    ...
    let (output_sender, output_receiver) = dl_video::channel("video_output");
    ...
    let mut decoder = dl_video::new_decoder(
        video_path,
        &video,
        &dl_video::DecodeSettings::default(),
        output_sender,
    )
        ...
        if let Ok(frame) = frame {
            ...
            match frame.format {
                ...
                dl_video::PixelFormat::Yuv { .. } => {
                    dl_log::error_once!("YUV frame writing is not supported");
                }
                dl_video::PixelFormat::L8
                | dl_video::PixelFormat::L16
                | dl_video::PixelFormat::R32Float => {
                    dl_log::error_once!("L8 & L16 & R32Float frame writing is not supported");
                }
            }
        }
    }
