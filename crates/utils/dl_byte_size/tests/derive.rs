...
// `dl_byte_size` re-exports our derive macro behind its (default) `derive` feature, so a single
...
// `crate_root` names the `dl_byte_size` crate directly, sidestepping the `Cargo.toml` lookup.
#[derive(SizeBytes)]
#[size_bytes(crate_root = dl_byte_size)]
