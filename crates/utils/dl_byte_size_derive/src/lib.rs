//! Derive macro for the `SizeBytes` trait from `dl_byte_size`.
//!
//! ```ignore
//! use ::dl_byte_size::SizeBytes;
//! use dl_byte_size_derive::SizeBytes;
//!
//! #[derive(SizeBytes)]
//! struct Foo {
//!     name: String,
...
/// computation.
///
/// Annotate the type with `#[size_bytes(profile)]` to insert a `dl_tracing::profile_function!()`
/// at the top of the generated `heap_size_bytes`. The consuming crate must depend on `dl_tracing`.
///
/// By default the macro finds the `dl_byte_size` crate by reading the consuming crate's `Cargo.toml`.
/// Annotate the type with `#[size_bytes(crate_root = some::path::to::dl_byte_size)]` to name it
...
fn expand(input: &DeriveInput) -> syn::Result<TokenStream2> {
    ...
    let profile_stmt = if options.profile {
        quote! { ::dl_tracing::profile_function!(); }
    } else {
        quote!()
    };
...
#[derive(Default)]
struct TypeOptions {
    /// Insert a `dl_tracing::profile_function!()` at the top of `heap_size_bytes`.
    profile: bool,

    /// Path to the `dl_byte_size` crate, overriding the automatic lookup.
