...
//! Microbench for the per-row hot loop in `dl_dataframe::QueryHandle::next_row`,
//! `next_row_batch`, and `next_n_rows`.
//!
//! Background: the redap query path
//! (`dl_datafusion::dataframe_query_provider::flush`) calls `next_row` once
...
//!
//! ```text
//! cargo bench -p dl_dataframe --bench next_row
//! ```
...
use dl_chunk::{Chunk, ChunkId, EntityPath, RowId, TimePoint, Timeline};
use dl_chunk_store::{ChunkStore, ChunkStoreConfig, QueryExpression};
use dl_dataframe::QueryEngine;
use dl_log_types::{StoreId, StoreKind, TimeInt, TimeType, TimelineName};
...
) -> QueryEngine<dl_query::StorageEngine> {
...
fn run_per_row_plus_concat(
    engine: &QueryEngine<dl_query::StorageEngine>,
    batch_size: usize,
) -> usize {
    ...
    'outer: loop {
        ...
        for col_idx in 0..num_fields {
            ...
            let _combined = dl_arrow_util::concat_arrays(&parts).unwrap();
        }
        total_rows += acc_rows;
    }
...
fn build_chunked_engine(
    ...
    n_chunks: usize,
    build_outer: fn(usize) -> ArrayRef,
) -> (QueryEngine<dl_query::StorageEngine>, usize) {
    assert!(n_chunks >= 1);
