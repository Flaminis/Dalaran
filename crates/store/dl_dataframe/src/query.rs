...
use dl_chunk::external::arrow::array::ArrayRef;
use dl_chunk::{
...
use dl_log::{ResultExt as _, debug_assert, debug_assert_eq, debug_panic};
use dl_log_types::AbsoluteTimeRange;
use dl_query::{QueryCache, StorageEngineLike};
use dl_sorbet::{
...
use dl_types_core::{Loggable as _, SerializedComponentColumn, archetypes};
...
impl<E: StorageEngineLike> QueryHandle<E> {
    ...
    fn init_(query: &QueryExpression, store: &ChunkStore, cache: &QueryCache) -> QueryHandleState {
        dl_tracing::profile_scope!("QueryHandle::init");
        ...
        let arrow_schema = ArrowSchemaRef::from(ArrowSchema::new_with_metadata(
            selected_contents
                .iter()
                .map(|(_, descr)| descr.to_arrow_field(dl_sorbet::BatchType::Dataframe))
                .collect::<ArrowFields>(),
            Default::default(),
        ));
        ...
        {
            dl_tracing::profile_scope!("clear_chunks");
                    ...
                    chunks.extend(clear_chunks.iter().map(|chunk| {
                        ...
                        chunk
                            .add_component(SerializedComponentColumn::new(
                                dl_arrow_util::new_list_array_of_empties(
                                    &child_datatype,
                                    chunk.num_rows(),
                                ),
                                dl_types_core::ComponentDescriptor {
        ...
        let unique_index_values = if let Some(filtered_index) = filtered_index.as_ref() {
            if let Some(using_index_values) = query.using_index_values.as_ref() {
                ...
                dl_tracing::profile_scope!("index_values");
        ...
        let selected_static_values = {
            dl_tracing::profile_scope!("static_values");
            ...
            selected_contents
                ...
                .map(|(_view_idx, descr)| match descr {
                    ...
                    ColumnDescriptor::Component(descr) => {
                        ...
                        let query = dl_chunk::LatestAtQuery::new_static();
    ...
    fn fetch_chunks(
        ...
        let Some(range_query) = range_query else {
            ...
            let results = cache.latest_at(
                ChunkTrackingMode::Report,
                &dl_chunk::LatestAtQuery::new_static(),
                entity_path,
                components,
            );
    ...
    fn _next_row<'state>(
        ...
        // dl_tracing::profile_function!(); // too many and short-lived
            ...
            SparseFillStrategy::LatestAtGlobal => {
                ...
                for (view_idx, streaming_state) in null_streaming_states {
                    ...
                    let query = match state.filtered_index {
                        Some(filtered_index) => {
                            dl_chunk::LatestAtQuery::new(filtered_index, *cur_index_value)
                        }
                        None => dl_chunk::LatestAtQuery::new_static(),
                    };
    ...
    fn _next_n_rows(
        ...
        dl_tracing::profile_scope!("finalize");
            ...
            match emitter {
                SelectedEmitter::Source {
                    ...
                    if let [single] = extends.as_slice() {
                        ...
                        dl_tracing::profile_scope!("SelectedEmitter::Source fast-path");
                        match *single {
                        ...
                        dl_tracing::profile_scope!("SelectedEmitter::Source slow-path");
                ...
                SelectedEmitter::Time { values, valid } => {
                    dl_tracing::profile_scope!("SelectedEmitter::Time");
    ...
    fn try_bulk_emit_run(
        ...
        // dl_tracing::profile_function!(); // even here we hit this too many times, with too much overhead
        ...
        dl_tracing::profile_scope!("bulk_emit", format!("len={min_len} slices={slice_count}"));
        ...
        match ArrowRecordBatch::try_new_with_options(
            ...
            Err(err) => {
                if cfg!(debug_assertions) {
                    ...
                    dl_log::error_once!("Failed to create record batch: {err}");
                    None
                }
            }
        }
...
mod tests {
    ...
    use dl_chunk::{Chunk, ChunkId, ComponentIdentifier, RowId, TimePoint};
    use dl_chunk_store::{
    ...
    use dl_log_types::{EntityPath, Timeline, build_frame_nr, build_log_time};
    use dl_query::StorageEngine;
    use dl_sdk_types::{AnyValues, AsComponents as _, ComponentDescriptor};
    use dl_sorbet::ComponentColumnSelector;
    use dl_types_core::components;
    ...
    impl std::fmt::Display for DisplayRB {
        ...
        fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
            let width = 200;
            dl_arrow_util::format_record_batch_with_width(&self.0, Some(width), f.sign_minus())
                .fmt(f)
        }
    }

    // NOTE: The best way to understand what these tests are doing is to run them in verbose mode,
    // e.g. `cargo t -p dl_dataframe -- --show-output barebones`.
    ...
    fn query_static_any_values() -> anyhow::Result<()> {
        ...
        let query_expr = QueryExpression {
            ...
            include_static_columns: dl_chunk_store::StaticColumnSelection::Both,
            filtered_index: None,
            filtered_index_range: None,
            filtered_index_values: None,
            using_index_values: None,
            filtered_is_not_null: None,
            sparse_fill_strategy: dl_chunk_store::SparseFillStrategy::None,
            selection: None,
        };
    ...
    fn next_n_rows_matches_next_row() -> anyhow::Result<()> {
        ...
        for query in queries {
            ...
            for (col_idx, candidate_parts) in candidate_columns.iter().enumerate() {
                ...
                let reference =
                    dl_arrow_util::concat_arrays(&ref_parts).expect("ref concat failed");
                let candidate =
                    dl_arrow_util::concat_arrays(&cand_parts).expect("cand concat failed");
                assert_eq!(
                    reference.to_data(),
                    candidate.to_data(),
    ...
    fn next_n_rows_bulk_disjoint_single_column() -> anyhow::Result<()> {
        ...
        for (col_idx, parts) in candidate_columns.iter().enumerate() {
            ...
            let r = dl_arrow_util::concat_arrays(&ref_parts).expect("ref concat");
            let c = dl_arrow_util::concat_arrays(&cand_parts).expect("cand concat");
    ...
    fn next_n_rows_bulk_emits_explicit_row_id_selection() -> anyhow::Result<()> {
        ...
        let expected_row_id_parts: Vec<&dyn arrow::array::Array> = chunks
            ...
            .collect();
        let expected_row_ids = dl_arrow_util::concat_arrays(&expected_row_id_parts)?;

        let got_row_id_parts: Vec<&dyn arrow::array::Array> =
            columns[0].iter().map(|a| a.as_ref()).collect();
        let got_row_ids = dl_arrow_util::concat_arrays(&got_row_id_parts)?;
