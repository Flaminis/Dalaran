...
use dl_chunk_store::ComponentColumnDescriptor;
use dl_chunk_store::{ChunkStoreHandle, QueryExpression};
use dl_log_types::EntityPathFilter;
use dl_query::{QueryCache, QueryCacheHandle, StorageEngine, StorageEngineLike};
use dl_sorbet::{ChunkColumnDescriptors, ColumnDescriptor};
...
mod tests {
    ...
    use dl_chunk::{Chunk, RowId, TimeInt};
    use dl_chunk_store::{ChunkStore, ChunkStoreConfig, ChunkStoreHandle, QueryExpression};
    use dl_log_types::example_components::{MyColor, MyPoint, MyPoints};
    use dl_log_types::{EntityPath, EntityPathFilter, TimePoint, build_frame_nr};
    use dl_sorbet::ColumnDescriptor;
    ...
    fn selected_schema_for_query_synthesizes_placeholders_for_misses() -> anyhow::Result<()> {
        use dl_sorbet::{ColumnSelector, ComponentColumnSelector, TimeColumnSelector};
        ...
        let query = QueryExpression {
            selection: Some(vec![
                ...
                ColumnSelector::Time(TimeColumnSelector::from(dl_log_types::TimelineName::from(
                    "does_not_exist",
                ))),
    ...
    fn selected_schema_for_query_matches_query_handle_selected_contents() -> anyhow::Result<()> {
        use dl_sorbet::{ColumnSelector, ComponentColumnSelector};
