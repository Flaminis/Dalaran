...
use dl_log_channel::{LogReceiver, LogSource, RecordingOpenBehavior};
...
pub enum LogDataSource {
    ...
    FilePath {
        ...
        file_source: dl_log_types::FileSource,
...
impl LogDataSource {
    ...
    pub fn from_uri(
        _file_source: dl_log_types::FileSource,
        url: &str,
        ...
        #[cfg(not(target_arch = "wasm32"))]
        {
            ...
            fn looks_like_a_file_path(uri: &str) -> bool {
                ...
                if !dl_importer::is_supported_file_extension(file_extension) {
                    return false;
                }
        ...
        if let Ok(uri) = url.parse::<dl_uri::DatasetSegmentUri>() {
            Some(Self::RedapDatasetSegment {
                uri,
                open_behavior: RecordingOpenBehavior::OpenAndSelect,
            })
        } else if let Ok(uri) = url.parse::<dl_uri::ProxyUri>() {
            ...
            if dl_importer::is_supported_file_extension(extension) {
                ...
                && !contains_viewer_query_url_param
            {
                ...
                if let Ok(uri) = value.parse::<dl_uri::DatasetSegmentUri>() {
                    Some(Self::RedapDatasetSegment {
                        uri,
                        open_behavior: RecordingOpenBehavior::OpenAndSelect,
                    })
                } else if let Ok(uri) = value.parse::<dl_uri::ProxyUri>() {
                    Some(Self::RedapProxy(uri))
                } else {
                    None
                }
    ...
    pub fn stream(
        ...
        self.stream_with_options(
            ...
            on_auth_err,
            connection_registry,
            dl_redap_client::StreamingOptions::default(),
        )
    }
    ...
    pub fn stream_with_options(
        ...
        on_auth_err: AuthErrorHandler,
        connection_registry: &ConnectionRegistryHandle,
        streaming_options: dl_redap_client::StreamingOptions,
    ) -> anyhow::Result<LogReceiver> {
        ...
        match self {
            ...
            Self::FilePath { file_source, path } => {
                let (tx, rx) = dl_log_channel::log_channel(LogSource::File { path: path.clone() });
                ...
                let settings = dl_importer::ImporterSettings {
                    ...
                    ..dl_importer::ImporterSettings::recommended(shared_recording_id)
                };
                dl_importer::import_from_path(&settings, file_source, &path, &tx)
            ...
            Self::FileHandle {
                ...
                async_runtime.spawn_future(async move {
                    dl_log::debug!("Reading {}…", path.display());
                    let bytes = match dl_web::fs::read_file(file).await {
                        Ok(bytes) => bytes,
                        Err(err) => {
                    ...
                    dl_log::debug!(
                        "{} was {}",
                        path.display(),
                        dl_format::format_bytes(bytes.len() as _)
                    );
                    ...
                    if let Err(err) = dl_importer::import_from_file_contents(
                        &settings,
                        file_source,
                        &path,
            ...
            #[cfg(not(target_arch = "wasm32"))]
            Self::Stdin => {
                let (tx, rx) = dl_log_channel::log_channel(LogSource::Stdin);
            ...
            Self::RedapDatasetSegment { uri, open_behavior } => {
                ...
                let stream_segment = async move {
                    ...
                    dl_redap_client::stream_blueprint_and_segment_from_server(
                        client,
                        tx,
                        uri_clone,
                        streaming_options,
            ...
            Self::RedapProxy(uri) => Ok(dl_grpc_client::stream(async_runtime, uri)),
        }
    }
    ...
    fn file_source_to_analytics_str(file_source: &dl_log_types::FileSource) -> &'static str {
