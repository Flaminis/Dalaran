...
pub fn load_stdin(tx: dl_log_channel::LogSender) -> anyhow::Result<()> {
    let stdin = std::io::BufReader::new(std::io::stdin());

    let decoder = dl_log_encoding::DecoderApp::decode_eager(stdin)?;

    rayon::spawn(move || {
        dl_tracing::profile_scope!("stdin");

        for msg in decoder {
            let msg = match msg {
                Ok(msg) => msg,
                Err(err) => {
                    dl_log::warn_once!("Failed to decode message in stdin: {err}");
                    continue;
                }
            };
