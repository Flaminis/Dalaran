use std::sync::Arc;

use dl_log::ResultExt as _;
use dl_log_channel::{LogReceiver, LogSource, url_display_name};
use dl_log_types::{FileSource, RecordingId};

/// Fetch a file from an HTTP URL and load it using all available importers.
///
/// Unlike RRD streaming which decodes incrementally, this downloads the entire file
/// first, then passes the bytes through [`dl_importer::import_from_file_contents`].
///
/// This works for all file types supported by the importers (MCAP, images, 3D models, etc.).
pub fn fetch_and_load(url: &url::Url) -> LogReceiver {
    let url_string = url.to_string();

    let filename = url
        .path_segments()
        .and_then(|mut segments| segments.next_back())
        .unwrap_or("downloaded_file")
        .to_owned();

    let (tx, rx) = dl_log_channel::log_channel(LogSource::HttpStream {
        url: url_string.clone(),
    });

    dl_log::debug!("Fetching file from {:?}…", url_display_name(&url_string));

    ehttp::fetch(
        ehttp::Request::get(&url_string).with_timeout(None),
        move |result| {
            match result {
                Ok(response) => {
                    if !response.ok {
                        dl_log::error!(
                            url = url_display_name(&url_string),
                            "Failed to fetch: {} {}",
                            response.status,
                            response.status_text
                        );
                        tx.quit(Some(Box::new(std::io::Error::other(format!(
                            "Failed to fetch file: HTTP {} {}",
                            response.status, response.status_text
                        )))))
                        .warn_on_err_once("Failed to send quit marker");
                        return;
                    }

                    dl_log::debug!(
                        "Fetched {} ({}), loading…",
                        url_display_name(&url_string),
                        dl_format::format_bytes(response.bytes.len() as f64)
                    );

                    // If the URL-derived filename has no recognized extension,
                    // try to detect the format from Content-Type header or magic bytes.
                    let filename = detect_filename(&filename, &response, &response.bytes);

                    let bytes: Arc<[u8]> = response.bytes.into();

                    let shared_recording_id = RecordingId::random();
                    let settings = dl_importer::ImporterSettings {
                        force_store_info: true,
                        ..dl_importer::ImporterSettings::recommended(shared_recording_id)
                    };

                    if let Err(err) = dl_importer::import_from_file_contents(
                        &settings,
                        FileSource::Uri,
                        &std::path::PathBuf::from(&filename),
                        std::borrow::Cow::Borrowed(&bytes),
                        &tx,
                    ) {
                        dl_log::error!(path = filename, "Failed to load: {err}");
                        tx.quit(Some(Box::new(err)))
                            .warn_on_err_once("Failed to send quit marker");
                    }

                    // `import_from_file_contents` internally calls `send()` which calls `tx.quit(None)`
                    // when all data has been forwarded, so we don't need to call it here on success.
                }
                Err(err) => {
                    dl_log::error!(
                        url = url_display_name(&url_string),
                        "Failed to fetch: {err}"
                    );
                    tx.quit(Some(Box::new(std::io::Error::other(format!(
                        "Failed to fetch file: {err}",
                    )))))
                    .warn_on_err_once("Failed to send quit marker");
                }
            }
        },
    );

    rx
}

/// If `url_filename` has no recognized extension, try to detect the format
/// from the Content-Type header, then from magic bytes.
fn detect_filename(url_filename: &str, response: &ehttp::Response, bytes: &[u8]) -> String {
    let has_known_extension = std::path::Path::new(url_filename)
        .extension()
        .and_then(|e| e.to_str())
        .is_some_and(dl_importer::is_supported_file_extension);

    if has_known_extension {
        return url_filename.to_owned();
    }

    // Try Content-Type header
    if let Some(content_type) = response.content_type()
        && let Some(ext) = dl_importer::content_type_to_extension(content_type)
    {
        return format!("{url_filename}.{ext}");
    }

    // Try magic bytes
    if let Some(ext) = dl_importer::detect_format_from_bytes(bytes) {
        return format!("{url_filename}.{ext}");
    }

    url_filename.to_owned()
}
