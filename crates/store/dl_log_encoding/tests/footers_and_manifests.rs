...
fn simple_manifest() {
    let dlr_manifest = {
        ...
        for msg in generate_recording_chunks(1) {
            let chunk_batch = dl_sorbet::ChunkBatch::try_from(&msg.batch).unwrap();

            let transport_uncompressed = msg
                .to_transport((
                    generate_recording_store_id(),
                    dl_log_encoding::Compression::Off,
                ))
                .unwrap();
            let transport_compressed = msg
                .to_transport((
                    generate_recording_store_id(),
                    dl_log_encoding::Compression::LZ4,
                ))
                .unwrap();
            ...
            let chunk_byte_span_excluding_header =
                dl_span::Span::from_start_len(byte_offset_excluding_header, chunk_byte_size);
            builder
                .append(
                    &chunk_batch,
...
fn footer_roundtrip() {
    ...
    fn decode_messages(msgs_encoded: &[u8], dlr_manifest: &RrdManifest) -> Vec<ArrowMsg> {
        ...
        .map(|(&offset, &size)| {
            ...
            let arrow_msg = dl_protos::log_msg::v1alpha1::ArrowMsg::decode(buf).unwrap();
...
fn footer_interleaved_stores_without_set_store_info() {
    ...
    let msgs = chain!(
        std::iter::once(LogMsg::SetStoreInfo(dl_log_types::SetStoreInfo {
            row_id: *RowId::ZERO,
            info: dl_log_types::StoreInfo {
                store_id: store_id_recording.clone(),
                cloned_from: None,
                store_source: dl_log_types::StoreSource::Unknown,
                store_version: Some(dl_build_info::CrateVersion::new(1, 2, 3)),
            },
        })),
        recording_chunks
            .into_iter()
...
fn generate_recording(
    chunks: impl Iterator<Item = dl_log_types::ArrowMsg>,
...
fn generate_recording_chunks(tuid_prefix: u64) -> impl Iterator<Item = dl_log_types::ArrowMsg> {
    use dl_log_types::example_components::{MyColor, MyLabel, MyPoint, MyPoints};
    use dl_log_types::{TimeInt, TimeType, Timeline, build_frame_nr};
    ...
    std::iter::chain(
        std::iter::once(LogMsg::SetStoreInfo(dl_log_types::SetStoreInfo {
            ...
            info: dl_log_types::StoreInfo {
                ...
                cloned_from: None,
                store_source: dl_log_types::StoreSource::Unknown,
                store_version: Some(dl_build_info::CrateVersion::new(4, 5, 6)),
            },
        })),
        chunks.map(move |chunk| LogMsg::ArrowMsg(store_id.clone(), chunk)),
    )
...
fn generate_blueprint_chunks(tuid_prefix: u64) -> impl Iterator<Item = dl_log_types::ArrowMsg> {
    use dl_log_types::{EntityPath, TimeInt, build_frame_nr};
    use dl_sdk_types::blueprint::archetypes::TimePanelBlueprint;
...
fn concat_with_mixed_chunk_keys() {
    ...
    use dl_log_types::{TimeInt, build_frame_nr};
...
fn size_bytes_accounts_for_extracted_arrays() {
    use dl_chunk::external::dl_byte_size::SizeBytes as _;
    use dl_log_types::example_components::{MyPoint, MyPoints};
    use dl_log_types::{TimeInt, build_frame_nr};
    ...
    let total_size =
        dl_chunk::external::dl_byte_size::SizeBytes::heap_size_bytes(manifest.as_ref());
