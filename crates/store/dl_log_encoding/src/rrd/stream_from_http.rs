...
pub fn stream_from_http(url: String, on_msg: Arc<HttpMessageCallback>) {
    dl_log::debug!("Downloading .rrd file from {url:?}…");
        ...
        move |part| match part {
            Ok(part) => match part {
                ehttp::streaming::Part::Response(ehttp::PartialResponse {
                    ...
                    if ok {
                        dl_log::debug!("Decoding .rrd file from {url:?}…");
                ...
                ehttp::streaming::Part::Chunk(chunk) => {
                    if chunk.is_empty() {
                        dl_log::debug!("Finished decoding .rrd file from {url:?}…");
                        return on_msg(HttpMessage::Success);
                    }

                    dl_tracing::profile_scope!("decoding_rrd_stream");
...
pub mod web_decode {
    ...
    pub fn decode_rrd(rrd_bytes: Vec<u8>, on_msg: Arc<HttpMessageCallback>) {
        dl_async::spawn_local(decode_rrd_async(rrd_bytes, on_msg));
    }
