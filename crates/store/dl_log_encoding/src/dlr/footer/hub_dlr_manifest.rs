...
impl HubRrdManifest {
    ...
    pub fn try_from_raw(
        raw: &RawRrdManifest,
        segment_id: &dl_types_core::SegmentId,
        layer: &dl_types_core::LayerName,
        storage_url: &url::Url,
        etag: Option<&dl_protos::cloud::v1alpha1::ext::ETag>,
        ...
        for name in [
            ...
            if raw.data.schema_ref().column_with_name(name).is_some() {
                return Err(CodecError::ArrowDeserialization(ArrowError::SchemaError(
                    format!("raw DLR manifest already has a '{name}' column, cannot hub-extend it"),
                )));
            }
        }
    ...
    /// Byte ranges extended to cover the 16-byte DLR message header preceding each chunk payload.
    ///
...
/// Extends `data`'s payload-only offset/size columns to cover the DLR message header.
//
...
fn header_inclusive_offsets_and_sizes(
    ...
    let offsets: UInt64Array = offsets.try_unary(|offset| {
        offset.checked_sub(header_size).ok_or_else(|| {
            CodecError::FrameDecoding(format!(
                "chunk byte offset {offset} is smaller than the DLR message header ({header_size} bytes)"
            ))
        })
    })?;
    ...
    let sizes: UInt64Array = sizes.try_unary(|size| {
        size.checked_add(header_size).ok_or_else(|| {
            CodecError::FrameDecoding(format!(
                "chunk byte size {size} overflows when extended by the DLR message header ({header_size} bytes)"
            ))
        })
    })?;
...
fn build_chunk_key_column(
    ...
    use dl_protos::cloud::v1alpha1::ext::{ChunkKey, DataSourceKind, RrdChunkLocation};
    ...
    .map(|(chunk_id, offset, length)| {
        ChunkKey {
            chunk_id,
            data_source_kind: DataSourceKind::Dlr,
...
#[cfg(test)]
mod tests {
    use dl_protos::cloud::v1alpha1::ext::{ChunkKey, ETag, RrdChunkLocation};
    use dl_types_core::{LayerName, SegmentId};

    use super::HubRrdManifest;
    use crate::dlr::footer::RawRrdManifest;
    use crate::dlr::test_util::{encode_test_dlr, make_test_chunks};
    ...
    fn build_manifest() -> RawRrdManifest {
        ...
        let mut footer = futures::executor::block_on(crate::dlr::read_dlr_footer(&file))
            .expect("reading the footer of a freshly encoded DLR file cannot fail")
            .expect("a freshly encoded DLR file always has a footer");
    ...
    fn hub_extends_the_raw_manifest_with_the_three_columns() {
        ...
        let storage_url = url::Url::parse("s3://bucket/recording.dlr").expect("valid url");
    ...
    fn try_from_raw_errors_on_zero_offsets() {
        ...
        let store_id = dl_log_types::StoreId::random(dl_log_types::StoreKind::Recording, "test");
        let raw =
