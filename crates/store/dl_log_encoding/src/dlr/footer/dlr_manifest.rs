...
use dl_chunk::{ChunkId, EntityPath};
use dl_log_types::StoreId;
use dl_sorbet::SorbetSchema;
...
impl dl_byte_size::SizeBytes for RrdManifest {
    fn heap_size_bytes(&self) -> u64 {
        ...
        self.chunk_fetcher_rb.heap_size_bytes()
            + dl_byte_size::SizeBytes::heap_size_bytes(
...
impl RrdManifest {
    ...
    pub fn concat(manifests: &[&Self]) -> CodecResult<Self> {
        ...
        let combined_batches = arrow::compute::concat_batches(&combined_schema, batches_to_concat)
            .map_err(|err| {
                CodecError::FrameDecoding(format!(
                    "Failed to concatenate DLR manifest parts: {err}"
                ))
            })?;
        ...
        let chunk_ids = {
            ...
            dl_arrow_util::concat_arrays(&arrays)
    ...
    /// Returns the map-based representation of the static data in this DLR manifest.
    #[inline]
    pub fn static_map(&self) -> &RrdManifestStaticMap {
        &self.static_data_map
    }

    /// Returns the map-based representation of the temporal data in this DLR manifest.
    #[inline]
