...
/// over at [`dl_protos::log_msg::v1alpha1::RrdManifest`].
///
/// ## What's in the box?
///
...
///
/// You can think of an DLR Manifest as a dataframe of the time panel, effectively.
///
/// The best way to understand what an DLR manifest does or doesn't do is to look at snapshots for
/// a simple recording:
/// ```text,ignore
/// ┌───────────────────────────────────────────────┬──────────────────────────────────┬──────────────────────────────────┐
...
///   storage size. For a backend that makes use of compression, such as an DLR file with
...
/// ## A note on filtering
///
/// Always be on your toes when filtering rows out of an DLR manifest. Due to how the Dalaran data
...
///
/// Filtering DLR manifests is very non trivial and should only be performed with great care.
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, dl_byte_size::SizeBytes)]
...
/// The sha256 hash of an [DLR manifest's payload].
///
...
///
/// [DLR manifest's payload]: `RawRrdManifest::data`
...
impl RawRrdManifest {
    ...
    pub fn merge_polymorphic_parts(
        ...
        use dl_arrow_util::{RecordBatchExt as _, concat_polymorphic_batches};
    ...
    /// High-level helper to parse [`RawRrdManifest`]s from raw DLR bytes.
    ///
    /// This does not decode all the data, but rather goes straight to the DLR footer (if any).
    ///
    ...
    /// Usage:
    /// ```text,ignore
    /// let dlr_bytes = std::fs::read("/path/to/my/recording.dlr");
    /// let dlr_manifests = RrdManifest::from_dlr_bytes(&dlr_bytes)?;
    /// let dlr_manifest = dlr_manifests
    ///     .into_iter()
    ///     .find(|m| m.store_id.kind() == StoreKind::Recording)?;
    /// ```
    pub fn from_dlr_bytes(dlr_bytes: &[u8]) -> CodecResult<Vec<Self>> {
        let stream_footer = match crate::StreamFooter::from_dlr_bytes(dlr_bytes) {
            Ok(footer) => footer,
        ...
        for entry in stream_footer.entries {
            ...
            let dlr_footer_byte_span = dlr_footer_byte_span_from_start_excluding_header;

            let dlr_footer_byte_span = dlr_footer_byte_span
                .try_cast::<usize>()
                .ok_or_else(|| {
                    CodecError::FrameDecoding(
                        "DLR footer too large for native bit width".to_owned(),
                    )
                })?
                .range();

            let dlr_footer_bytes = &dlr_bytes[dlr_footer_byte_span];

            let crc = crate::StreamFooter::compute_crc(dlr_footer_bytes);
            ...
            let dlr_footer =
                dl_protos::log_msg::v1alpha1::RrdFooter::from_dlr_bytes(dlr_footer_bytes)?;
            let new_manifests: Vec<_> = dlr_footer
                .manifests
                .iter()
    ...
    pub fn build_in_memory_from_chunks<'a>(
        store_id: StoreId,
        chunks: impl Iterator<Item = &'a dl_chunk::Chunk>,
    ) -> CodecResult<Self> {
        let mut dlr_manifest_builder = crate::RrdManifestBuilder::default();
        ...
        for chunk in chunks {
            ...
            let uncompressed_byte_span =
                dl_span::Span::from_start_len(offset, byte_size_uncompressed);

            offset += byte_size_uncompressed;

            dlr_manifest_builder.append(
                &chunk_batch,
        ...
        let dlr_manifest = dlr_manifest_builder.build(store_id)?;
    ...
    /// Computes a map-based representation of the static data in this DLR manifest.
    ...
    /// Computes a map-based representation of the temporal data in this DLR manifest.
...
impl RawRrdManifest {
    ...
    pub fn compute_column_name(
        ...
        use dl_types_core::reflection::ComponentDescriptorExt as _;
        let full_name = [
...
impl RawRrdManifest {
                ...
                if let Some((_, suffix)) = field.name().rsplit_once(':') {
                    ...
                    match field.name().as_str() {
                        ...
                        name => {
                            return Err(CodecError::from(ChunkError::Malformed {
                                reason: format!(
                                    "unexpected field '{name}' should not be present in an DLR manifest",
                                ),
                            }));
                        }
                    }
    ...
    fn check_manifest_schema_matches_sorbet_schema(&self) -> CodecResult<()> {
        ...
        let mut dlr_manifest_fields: HashMap<_, _> = self
            .data
            .schema_ref()
            .fields()
            .iter()
        ...
        for sorbet_index in &sorbet_indexes {
            ...
            for suffix in ["start", "end"] {
                let field = dlr_manifest_fields.remove(&format!("{sorbet_index_name_normalized}:{suffix}"))
                    .ok_or_else(|| {
                        CodecError::from(ChunkError::Malformed {
                            reason: format!(
            ...
            for sorbet_column in &sorbet_columns {
                ...
                for suffix in ["start", "end"] {
                    ...
                    if md.get("dalaran:is_static").map(|s| s.as_str()) == Some("true") {
                        ...
                        _ = dlr_manifest_fields.remove(&column_name);
                        continue;
                    }

                    let Some(field) = dlr_manifest_fields.remove(&column_name) else {
...
impl RawRrdManifest {
    ...
    pub fn field_chunk_id() -> Field {
        use dl_log_types::external::dl_types_core::Loggable as _;
