...
/// Read the full DLR footer using positional I/O.
///
/// Returns `Ok(None)` if the data is a valid DLR but has no footer (legacy DLR).
/// Returns `Err` if the data is not a valid DLR or is corrupted.
///
...
pub async fn read_dlr_footer<R: AsyncReadAt>(reader: &R) -> Result<Option<RrdFooter>, CodecError> {
    let Some(payload) = read_dlr_footer_payload(reader).await? else {
        return Ok(None);
    };

    let transport_footer = dl_protos::log_msg::v1alpha1::RrdFooter::from_dlr_bytes(&payload)?;
    Ok(Some(transport_footer.to_application(())?))
}

pub(super) async fn read_dlr_footer_payload<R: AsyncReadAt>(
    reader: &R,
) -> Result<Option<bytes::Bytes>, CodecError> {
    let file_len = reader.size().await?;

    // 1. Validate the StreamHeader to confirm this is actually an DLR file.
    if file_len < StreamHeader::ENCODED_SIZE_BYTES as u64 {
        return Err(CodecError::FrameDecoding(
            "file too small to be an DLR".to_owned(),
        ));
    }
    let header_buf = reader
        .read_exact_at(0, StreamHeader::ENCODED_SIZE_BYTES)
        .await?;
    StreamHeader::from_dlr_bytes(&header_buf)?; // validates FourCC + version
    ...
    let Ok(stream_footer) = StreamFooter::from_dlr_bytes(&footer_buf) else {
        return Ok(None); // Valid DLR, but no footer (legacy).
    };
    ...
    let span = &entry.dlr_footer_byte_span_from_start_excluding_header;
...
/// Enumerate all [`StoreId`]s present in an DLR file, without reading chunk data.
///
...
pub async fn enumerate_dlr_stores<R: AsyncReadAt>(reader: &R) -> Result<Vec<StoreId>, CodecError> {
    // Try footer first (cheap: a few small reads, no chunk data read).
    if let Some(footer) = read_dlr_footer(reader).await? {
...
/// Metadata collected from DLR message frames without decoding chunk data.
...
/// Enumerate the metadata available through legacy DLR message frames.
///
...
pub async fn enumerate_legacy_metadata<R: AsyncReadAt>(
    ...
    let stream_header_buf = reader
        .read_exact_at(0, StreamHeader::ENCODED_SIZE_BYTES)
        .await?;
    let stream_header = StreamHeader::from_dlr_bytes(&stream_header_buf)?;
    ...
    loop {
        ...
        let header = MessageHeader::from_dlr_bytes(&msg_header_buf)?;
        ...
        match header.kind {
            ...
            MessageKind::SetStoreInfo | MessageKind::BlueprintActivationCommand => {
                ...
                offset += header.len;
                let byte_span = dl_chunk::Span::from_start_len(0, header.len);
                match LogMsg::decode(
                    payload,
                    byte_span,
                    header.kind,
...
mod tests {
    ...
    fn set_store_info(store_id: StoreId) -> LogMsg {
        LogMsg::SetStoreInfo(dl_log_types::SetStoreInfo {
            row_id: *dl_chunk::RowId::ZERO,
            info: dl_log_types::StoreInfo::new(store_id, dl_log_types::StoreSource::Unknown),
        })
    }
    ...
    fn test_read_footer_roundtrip() {
        ...
        let footer = futures::executor::block_on(read_dlr_footer(&file)).unwrap();
    ...
    fn test_read_footer_no_footer() {
        ...
        let file = File::open(file.path()).unwrap();
        let footer = futures::executor::block_on(read_dlr_footer(&file)).unwrap();
        assert!(footer.is_none(), "Legacy DLR should have no footer");
    }

    #[test]
    fn test_enumerate_dlr_stores_footer_path() {
        let chunks = make_test_chunks(5);
        let (file, store_id) = encode_test_dlr(&chunks);

        let file = File::open(file.path()).unwrap();
        let ids = futures::executor::block_on(enumerate_dlr_stores(&file)).unwrap();
        assert_eq!(ids, vec![store_id]);
    }

    #[test]
    fn test_enumerate_dlr_stores_legacy_path() {
        let file = tempfile::NamedTempFile::new().unwrap();
        let chunks = make_test_chunks(3);
        encode_test_dlr_to_file(file.path(), &chunks, false);
        ...
        assert_eq!(
            ids.len(),
            1,
            "Legacy DLR should have its single store discovered"
        );
    }

    /// Cross-check: the legacy (frame-scan) and modern (footer) paths must return identical
    /// results on the same logical DLR content.
    #[test]
    fn test_enumerate_dlr_stores_footer_vs_legacy_agree() {
        let chunks = make_test_chunks(5);
        let store_id =
            dl_log_types::StoreId::random(dl_log_types::StoreKind::Recording, "cross_check");

        let with_footer = tempfile::NamedTempFile::new().unwrap();
        let without_footer = tempfile::NamedTempFile::new().unwrap();
        crate::dlr::test_util::encode_test_dlr_to_file_with_options(
            with_footer.path(),
            &chunks,
            &store_id,
            true,
        ...
        let ids_footer =
            futures::executor::block_on(enumerate_dlr_stores(&with_footer_file)).unwrap();
        let without_footer_file = File::open(without_footer.path()).unwrap();
        let ids_legacy =
            futures::executor::block_on(enumerate_dlr_stores(&without_footer_file)).unwrap();
    ...
    /// Legacy DLR where the *same* `StoreId` is announced by several `SetStoreInfo`
    /// messages (can happen e.g. after a flush/reconnect). Enumeration must dedup.
    #[test]
    fn test_enumerate_dlr_stores_legacy_duplicate_set_store_info() {
    ...
    /// Two-store DLR with `SetStoreInfo` messages interleaved with `ArrowMsg`s. Both the
    /// legacy (no-footer) path and the modern (footer) path must discover both stores and
    /// return the same result.
    #[test]
    fn test_enumerate_dlr_stores_interleaved_footer_vs_legacy() {
        use dl_log_types::{LogMsg, SetStoreInfo, StoreId, StoreInfo, StoreKind, StoreSource};
    ...
    fn test_enumerate_legacy_metadata_default_blueprint_last_wins() {
        use dl_log_types::{BlueprintActivationCommand, LogMsg, StoreId, StoreKind};

        let app_id = dl_log_types::ApplicationId::from("metadata_app");
    ...
    #[test]
    fn test_read_footer_not_an_dlr() {
        let file = tempfile::NamedTempFile::new().unwrap();
        std::fs::write(file.path(), b"this is not an dlr file at all").unwrap();

        let file = File::open(file.path()).unwrap();
        let result = futures::executor::block_on(read_dlr_footer(&file));
        assert!(result.is_err(), "Non-DLR file should return an error");
    }
    ...
    fn test_read_footer_corrupted_crc() {
        ...
        let stream_footer = StreamFooter::from_dlr_bytes(footer_bytes).unwrap();
        let entry = &stream_footer.entries[0];
        let payload_start = entry.dlr_footer_byte_span_from_start_excluding_header.start as usize;
