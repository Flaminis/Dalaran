...
/// Read chunks from an DLR reader by their IDs, using byte offsets from the manifest.
///
...
fn decode_chunk_from_bytes(buf: &[u8]) -> Result<Chunk, CodecError> {
    ...
    let transport_arrow_msg = dl_protos::log_msg::v1alpha1::ArrowMsg::from_dlr_bytes(buf)?;
...
mod tests {
    ...
    use super::*;
    use crate::dlr::test_util::{
        encode_test_dlr, encode_test_dlr_to_file_with_options, make_test_chunks,
    };

    #[test]
    fn test_read_chunks_roundtrip() {
        let chunks = make_test_chunks(5);
        let (dlr, store_id) = encode_test_dlr(&chunks);
        let footer_file = File::open(dlr.path()).unwrap();
        let footer = futures::executor::block_on(crate::read_dlr_footer(&footer_file))
            .unwrap()
            .unwrap();
        let file = File::open(dlr.path()).unwrap();
