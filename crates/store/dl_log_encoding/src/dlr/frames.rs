// --- FileHeader ---
pub use dl_build_info::CrateVersion; // convenience
pub use dl_protos::common::v1alpha1::ext::Compression;

use crate::dlr::{Decodable, Encodable, OptionsError}; // convenience
...
impl Encodable for EncodingOptions {
    fn to_dlr_bytes(&self, out: &mut Vec<u8>) -> Result<u64, crate::dlr::CodecError> {
        let Self {
            compression,
            serializer,
        } = *self;
...
impl Decodable for EncodingOptions {
    fn from_dlr_bytes(data: &[u8]) -> Result<Self, crate::dlr::CodecError> {
        match data {
...
/// The opening frame in an DLR stream.
///
/// During normal operations, there can only be a single [`StreamHeader`] per DLR stream.
...
impl StreamHeader {
    pub fn to_version_and_options(
        self,
    ) -> Result<(CrateVersion, EncodingOptions), crate::dlr::CodecError> {
        {
            // We used 0000 for all .dlr files up until 2023-02-27, post 0.2.0 release:
...
impl Encodable for StreamHeader {
    fn to_dlr_bytes(&self, out: &mut Vec<u8>) -> Result<u64, crate::dlr::CodecError> {
        ...
        out.extend_from_slice(version);
        {
            ...
            options.to_dlr_bytes(&mut options_out)?;
...
impl Decodable for StreamHeader {
    fn from_dlr_bytes(data: &[u8]) -> Result<Self, crate::dlr::CodecError> {
        if data.len() != Self::ENCODED_SIZE_BYTES {
            return Err(crate::dlr::CodecError::FrameDecoding(format!(
        ...
        if crate::dlr::OLD_DLR_FOURCC.contains(&fourcc) {
            return Err(crate::dlr::CodecError::OldRrdVersion);
        } else if fourcc != crate::dlr::DLR_FOURCC {
            return Err(crate::dlr::CodecError::NotAnRrd(
                crate::dlr::NotAnRrdError {
                    expected_fourcc: crate::dlr::DLR_FOURCC,
        ...
        let options = EncodingOptions::from_dlr_bytes(&data[8..])?;
        Ok(Self {
            fourcc,
            version,
            options,
...
/// The closing frame in an DLR stream. Keeps track of where the [`RrdFooter`]s can be found.
///
/// During normal operations, there can only be a single [`StreamFooter`] per DLR stream.
/// It is possible to break that invariant by concatenating streams using external tools,
/// e.g. by doing something like `cat *.dlr > all_my_recordings.dlr`.
/// Passing that stream back through Dalaran tools, e.g. `cat *.dlr | dalaran dlr merge > all_my_recordings.dlr`,
...
pub struct StreamFooter {
    /// Same as the one in the [`StreamHeader`], i.e. [`crate::dlr::DLR_FOURCC`].
    ///
    /// Used to go straight to the footer of an DLR stream ("backwards").
    pub fourcc: [u8; 4], // RRF2

    /// A unique identifier to disambiguate the footer from other frames.
    ///
    /// Always set to [`Self::DLR_IDENTIFIER`].
    pub identifier: [u8; 4], // FOOT

    /// One entry per DLR footer that is pointed at from this stream footer.
    ///
    /// The stream footer supports pointing to an arbitrary number of DLR footers.
    /// This variable length allows for incremental DLR manifests, where the manifest for a single
...
/// One specific entry in the [`StreamFooter`]. Each entry corresponds to an DLR footer.
///
...
pub struct StreamFooterEntry {
    ...
    /// ```text
    /// let start = dlr_footer_byte_span_from_start_excluding_header.start;
    /// let end = dlr_footer_byte_span_from_start_excluding_header.end();
    /// let bytes = &file[start..end];
    /// let dlr_footer = dl_protos::RrdFooter::decode(bytes)?;
    /// let dlr_footer = dlr_footer.to_application()?;
    /// ```
    ///
    /// [`RrdFooter`]: [crate::RrdFooter]
    pub dlr_footer_byte_span_from_start_excluding_header: dl_span::Span<u64>,
    ...
    /// DLR footers, which can potentially get very, very large.
    ///
    /// [`RrdFooter`]: [crate::RrdFooter]
    //
...
impl StreamFooter {
    ...
    pub const DLR_IDENTIFIER: [u8; 4] = *b"FOOT";

    pub fn new(
        dlr_footer_byte_span_from_start_excluding_header: dl_span::Span<u64>,
        crc_excluding_header: u32,
    ) -> Self {
        Self {
            fourcc: crate::DLR_FOURCC,
            identifier: Self::DLR_IDENTIFIER,
    ...
    pub fn from_dlr_footer_bytes(
        dlr_footer_byte_offset_from_start_excluding_header: u64,
        dlr_footer_bytes: &[u8],
    ) -> Self {
        let crc_excluding_header = Self::compute_crc(dlr_footer_bytes);
        let dlr_footer_byte_span_from_start_excluding_header = dl_span::Span {
            start: dlr_footer_byte_offset_from_start_excluding_header,
            len: dlr_footer_bytes.len() as u64,
        };
        Self {
            fourcc: crate::DLR_FOURCC,
            identifier: Self::DLR_IDENTIFIER,
    ...
    pub fn compute_crc(dlr_footer_bytes_excluding_header: &[u8]) -> u32 {
        xxhash_rust::xxh32::xxh32(dlr_footer_bytes_excluding_header, Self::CRC_SEED)
    }
}
...
impl Encodable for StreamFooter {
    fn to_dlr_bytes(&self, out: &mut Vec<u8>) -> Result<u64, crate::dlr::CodecError> {
        ...
        let num_dlr_footers = entries.len() as u32;
        ...
        out.extend_from_slice(&num_dlr_footers.to_le_bytes());

        let n = out.len() as u64 - before;
        assert_eq!(
            Self::ENCODED_SIZE_BYTES as u64,
            n,
            "Stream footers always point to a single DLR footer at the moment"
        );
...
impl Decodable for StreamFooter {
    fn from_dlr_bytes(data: &[u8]) -> Result<Self, crate::dlr::CodecError> {
        ...
        if identifier != Self::DLR_IDENTIFIER {
            return Err(crate::dlr::CodecError::FrameDecoding(format!(
        ...
        let num_dlr_footers = u32::from_le_bytes(
            fixed_data[8..12]
                .try_into()
