...
pub type DecoderTransport = Decoder<dl_protos::log_msg::v1alpha1::log_msg::Msg>;
...
///
/// See also [`DecoderApp`].
pub type DecoderApp = Decoder<dl_log_types::LogMsg>;
...
pub struct Decoder<T> {
    /// The Dalaran version used to encode the DLR data.
    ///
    ...
    /// All the DLR manifests accumulated so far by parsing incoming footers in the DLR stream.
    ///
    /// Transport-level types to keep decoding cheap.
    dlr_manifests: Vec<dl_protos::log_msg::v1alpha1::RrdManifest>,
...
pub enum DecoderState {
    ...
    /// encoding options.
    ///
    /// We can come back to this state at any point because multiple DLR streams might have been
    /// concatenated together (e.g. `cat *.dlr | dalaran`).
    ...
    /// footer or another concatenated DLR stream shows up.
    WaitingForMessageHeader,
    ...
    ///
    /// The [`StreamFooter`] contains information about where the DLR manifests can be found, but we
...
impl<T: DecoderEntrypoint> Decoder<T> {
    ...
    /// Returns all the DLR manifests accumulated _so far_.
    ///
    /// DLR manifests are parsed from footers, of which there might be more than one e.g. in the
    /// case of concatenated streams.
    ///
    /// This is not cheap: it automatically performs the transport to app level conversion.
    pub fn dlr_manifests(&self) -> Result<Vec<RawRrdManifest>, DecodeError> {
        ...
        match self.state {
            DecoderState::WaitingForStreamHeader => {
                    ...
                    self.byte_chunks.try_read(StreamHeader::ENCODED_SIZE_BYTES)
                {
                    ...
                    let version_and_options = StreamHeader::from_dlr_bytes(&header_data)
                        ...
                        Err(err) => {
                            if is_first_header {
                                ...
                                dl_log::error!(
                                    is_first_header,
                                    position,
                                    "trailing bytes in dlr stream: {header_data:?} ({err})"
                                );
            ...
            DecoderState::WaitingForMessageHeader => {
                ...
                if self.byte_chunks.try_peek(&mut peeked) == peeked.len() {
                    let header = match MessageHeader::from_dlr_bytes(&peeked) {
                        Ok(header) => header,

                        Err(crate::dlr::CodecError::FrameDecoding(_)) => {
                    ...
                    dl_log::trace!(?header, "found MessageHeader");
            ...
            DecoderState::WaitingForMessagePayload(header) => {
                ...
                if let Some(bytes) = self.byte_chunks.try_read(header.len as usize) {
                    ...
                    let byte_span = dl_chunk::Span::from_start_len(start_offset, bytes_len);
                    ...
                    if let Some(message) = message {
                        ...
                        if !bytes.is_empty() {
                            let dlr_footer =
                                dl_protos::log_msg::v1alpha1::RrdFooter::from_dlr_bytes(&bytes)?;
                            self.dlr_manifests.extend(dlr_footer.manifests);
            ...
            DecoderState::WaitingForStreamFooter => {
                ...
                if let Some(bytes) = self
                    .byte_chunks
                    .try_read(crate::dlr::StreamFooter::ENCODED_SIZE_BYTES)
                {
                    match crate::dlr::StreamFooter::from_dlr_bytes(&bytes) {
                        Ok(footer) => {
                            dl_log::trace!(?footer, "found StreamFooter");
                            ...
                            for entry in entries {
                                ...
                                let dlr_footer_end =
                                    dlr_footer_byte_span_from_start_excluding_header.end();

                                if dlr_footer_end > position as u64 {
                                    // The DLR footer cannot possibly end after the stream footer starts, since it must
...
mod tests {
    use dl_chunk::RowId;
    use dl_log_types::{LogMsg, SetStoreInfo, StoreInfo};

    use super::*;
    use crate::Encoder;
    use crate::dlr::EncodingOptions;
