...
use crate::dlr::{DecodeError, Decoder, DecoderEntrypoint};
...
impl<T: DecoderEntrypoint> Decoder<T> {
    ...
    /// fail if the reader doesn't even contain valid DLR data.
    ///
    ...
    /// synchronously if the underlying reader doesn't even contain valid DLR data at all (e.g. magic
...
mod tests {
    ...
    use dl_protos::log_msg::v1alpha1 as proto;
    use dl_protos::log_msg::v1alpha1::log_msg::Msg as LogMsgProto;

    use crate::dlr::{Compression, DecoderApp, EncodingOptions, Serializer};
    ...
    fn fake_log_messages() -> Vec<LogMsg> {
        ...
        let arrow_msg = dl_chunk::Chunk::builder("test_entity")
            .with_archetype(
                dl_chunk::RowId::new(),
    ...
    impl<W: std::io::Write> Encoder<W> {
        ...
        fn encode_into_without_eos(
            ...
            messages: impl IntoIterator<Item = dl_chunk::ChunkResult<impl std::borrow::Borrow<LogMsg>>>,
            write: &mut W,
    ...
    fn test_encode_decode() {
        let dlr_version = CrateVersion::LOCAL;
        ...
        for options in options {
            let mut file = vec![];
            crate::Encoder::encode_into(dlr_version, options, messages.iter().map(Ok), &mut file)
                .unwrap();
    ...
    fn test_decode_legacy() {
        ...
        let mut encoder = Encoder::new_eager(dlr_version, options, &mut file).unwrap();
