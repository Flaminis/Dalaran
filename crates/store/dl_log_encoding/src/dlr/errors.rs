...
use dl_chunk::ChunkError;
use dl_protos::common::v1alpha1::ext;

pub type CodecResult<T> = Result<T, CodecError>;

/// Possible errors when encoding and decoding DLR data.
///
...
pub enum CodecError {
    ...
    /// Something went wrong when attempting to decode any kind of DLR frame.
    ///
    /// There are 3 kinds of DLR frames:
    /// * [`crate::StreamHeader`]
    /// * [`crate::MessageHeader`]
    /// * [`crate::StreamFooter`]
    ...
    ProtobufEncode(#[from] dl_protos::external::prost::EncodeError),

    #[error("Protobuf error: {0}")]
    ProtobufDecode(#[from] dl_protos::external::prost::DecodeError),

    #[error("Could not convert type from protobuf: {0}")]
    TypeConversion(Box<dl_protos::TypeConversionError>),
    ...
    StoreIdMissingApplicationId {
        store_kind: dl_log_types::StoreKind,
        recording_id: dl_log_types::RecordingId,
    },
    ...
    Sorbet(#[from] dl_sorbet::SorbetError),
    ...
    ChunkNotInManifest { chunk_id: dl_chunk::ChunkId },

    #[error("Invalid timeline name: {0}")]
    InvalidTimelineName(#[from] dl_log_types::InvalidTimelineNameError),
}
...
impl From<dl_protos::TypeConversionError> for CodecError {
    fn from(value: dl_protos::TypeConversionError) -> Self {
    ...
    fn from(value: ext::StoreIdFromProtoError) -> Self {
        match value {
            ...
            ext::StoreIdFromProtoError::InvalidApplicationId(err) => {
                dl_protos::TypeConversionError::from(err).into()
            }
        }
    }
}

/// When the file does not have the expected .dlr [FourCC](https://en.wikipedia.org/wiki/FourCC) header.
#[derive(Debug)]
...
impl std::fmt::Display for NotAnRrdError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        write!(
            f,
            "Not an DLR file: expected FourCC header {:?}, got {:?}",
...
/// On failure to decode [`crate::dlr::EncodingOptions`]
...
pub enum OptionsError {
    ...
    #[error(
        "You are trying to load an old .dlr file that's not supported by this version of Dalaran."
    )]
