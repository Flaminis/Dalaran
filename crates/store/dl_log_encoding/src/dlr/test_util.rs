...
/// Encode test chunks into a temporary DLR file (with footer).
///
/// Returns the temp file (keeps it alive) and the `StoreId`.
/// Use `.path()` on the returned file to get the path.
pub fn encode_test_dlr(chunks: &[Arc<Chunk>]) -> (NamedTempFile, StoreId) {
    let file = NamedTempFile::new().unwrap();
    let store_id = encode_test_dlr_to_file(file.path(), chunks, true);
    (file, store_id)
}

/// Encode chunks into an DLR file at the given path. Returns the `StoreId` used.
pub fn encode_test_dlr_to_file(path: &Path, chunks: &[Arc<Chunk>], with_footer: bool) -> StoreId {
...
pub fn encode_test_dlr_to_file_with_options(
    path: &Path,
    chunks: &[Arc<Chunk>],
    store_id: &StoreId,
    with_footer: bool,
    ...
    let mut encoder =
        crate::Encoder::new_eager(dl_build_info::CrateVersion::LOCAL, options, &mut file).unwrap();
    if !with_footer {
