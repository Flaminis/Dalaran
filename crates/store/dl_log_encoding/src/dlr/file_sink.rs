...
pub enum FileSinkError {
    ...
    LogMsgEncode(#[from] crate::dlr::EncodeError),
}
...
/// Stream log messages to an `.dlr` file.
pub struct FileSink {
    // None = quit
...
pub struct FileSinkOptions {
    /// Whether to emit a complete DLR footer (including a manifest of every chunk) at the end
    ...
    /// grows unboundedly. Set to `false` to opt out: the resulting file is still valid DLR,
    /// but without the manifest which may hurt random-access performance.
    ///
    /// A footer can be added after the fact via `dalaran dlr optimize`.
...
impl FileSink {
    ...
    pub fn with_options(
        ...
        let encoding_options = crate::dlr::EncodingOptions::PROTOBUF_COMPRESSED;

        let (tx, rx) = crossbeam::channel::bounded(1024);

        let path = path.into();

        dl_log::debug!("Saving file to {path:?}…");
        ...
        let mut encoder =
            crate::Encoder::new_eager(dl_build_info::CrateVersion::LOCAL, encoding_options, file)?;
        if !options.write_footer {
            // The SDK's `FileSink` may stream for the entire lifetime of the host process.
            // The footer's DLR manifest accumulates per-chunk metadata in memory and is only
            // serialized when the encoder is dropped, so leaving it enabled here grows the heap
            // unboundedly (see #12623).
            dl_log::warn!(
                "FileSink at {path:?}: `write_footer=false` — the resulting .dlr will not \
    ...
    pub fn stdout_with_options(options: FileSinkOptions) -> Result<Self, FileSinkError> {
        ...
        dl_log::debug!("Writing to stdout…");
        ...
        .spawn({
            move || {
                    ...
                    match cmd {
                        Command::Send(log_msg) => {
                            if let Err(err) = encoder.append(&log_msg) {
                                dl_log::error!("Failed to write log stream to {target}: {err}");
                                return;
                            }
                        }
                ...
                if let Err(err) = encoder.finish() {
                    dl_log::error!("Failed to end log stream for {target}: {err}");
                    return;
                }
                dl_log::debug!("Log stream written to {target}");
            }
        })
