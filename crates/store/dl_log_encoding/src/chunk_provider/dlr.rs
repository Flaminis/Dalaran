...
mod tests {
    ...
    use dl_chunk::{RowId, TimePoint, Timeline};
    use dl_log_types::{
    ...
    /// Build a small DLR file containing `num_chunks` chunks at `path`. Returns `(store_id, chunks)`.
    fn write_test_dlr(path: &Path, num_chunks: usize) -> (StoreId, Vec<Arc<Chunk>>) {
    ...
    #[test]
    fn test_dlr_chunk_provider_round_trip() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("test.dlr");
        let (store_id, chunks) = write_test_dlr(&path, 3);
