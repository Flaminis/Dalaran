...
//
// pub type LogMsgTransport = dl_protos::log_msg::v1alpha1::log_msg::Msg;
// pub type LogMsgApp = dl_log_types::LogMsg;
...
impl ToTransport for crate::RrdFooter {
    type Output = dl_protos::log_msg::v1alpha1::RrdFooter;
    type Context<'a> = ();
...
impl ToTransport for crate::RawRrdManifest {
    type Output = dl_protos::log_msg::v1alpha1::RrdManifest;
    type Context<'a> = ();
    ...
    fn to_transport(&self, (): Self::Context<'_>) -> Result<Self::Output, CodecError> {
        ...
        let sorbet_schema = dl_protos::common::v1alpha1::Schema::try_from(&self.sorbet_schema)
...
impl ToApplication for dl_protos::log_msg::v1alpha1::log_msg::Msg {
    type Output = dl_log_types::LogMsg;
...
impl ToApplication for dl_protos::log_msg::v1alpha1::LogMsg {
    type Output = dl_log_types::LogMsg;
    type Context<'a> = (&'a mut dyn ApplicationIdInjector, Option<CrateVersion>);
    ...
    fn to_application(
        ...
        let Some(msg) = self.msg.as_ref() else {
            return Err(dl_protos::missing_field!(Self, "msg").into());
        };
...
impl ToApplication for dl_protos::log_msg::v1alpha1::ArrowMsg {
    type Output = dl_log_types::ArrowMsg;
    type Context<'a> = ();
...
impl ToApplication for dl_protos::log_msg::v1alpha1::RrdFooter {
...
impl ToApplication for dl_protos::log_msg::v1alpha1::RrdManifest {
    ...
    fn to_application(&self, _context: Self::Context<'_>) -> Result<Self::Output, CodecError> {
        let store_id = self
            .store_id
            .as_ref()
            .ok_or_else(|| dl_protos::missing_field!(Self, "store_id"))?;

        let sorbet_schema = self
            .sorbet_schema
            .as_ref()
            .ok_or_else(|| dl_protos::missing_field!(Self, "sorbet_schema"))?;

        let sorbet_schema_sha256 = self
            .sorbet_schema_sha256
            .as_ref()
            .ok_or_else(|| dl_protos::missing_field!(Self, "sorbet_schema_sha256"))?;
        let sorbet_schema_sha256: [u8; 32] = (**sorbet_schema_sha256)
            .try_into()
            .map_err(|err| dl_protos::invalid_field!(Self, "sorbet_schema_sha256", err))?;
...
fn log_msg_transport_to_app<I: ApplicationIdInjector + ?Sized>(
    ...
    message: &dl_protos::log_msg::v1alpha1::log_msg::Msg,
) -> Result<dl_log_types::LogMsg, CodecError> {
    dl_tracing::profile_function!();

    use dl_protos::common::v1alpha1::ext::StoreIdFromProtoError;
    ...
    match message {
        Msg::SetStoreInfo(set_store_info) => {
            ...
            Ok(dl_log_types::LogMsg::SetStoreInfo(set_store_info))
        }
        ...
        Msg::ArrowMsg(arrow_msg) => {
            ...
            let store_id: dl_log_types::StoreId = match arrow_msg
                .store_id
                .as_ref()
                .ok_or_else(|| missing_field!(dl_protos::log_msg::v1alpha1::ArrowMsg, "store_id"))?
                .clone()
                .try_into()
            {
            ...
            Ok(dl_log_types::LogMsg::ArrowMsg(store_id, encoded))
        }

        Msg::BlueprintActivationCommand(blueprint_activation_command) => {
            //TODO(#10730): clean that up when removing 0.24 back compat
            let blueprint_id: dl_log_types::StoreId = match blueprint_activation_command
                .blueprint_id
                .as_ref()
                .ok_or_else(|| {
                    missing_field!(
                        dl_protos::log_msg::v1alpha1::BlueprintActivationCommand,
                        "blueprint_id"
                    )
                })?
                .clone()
            ...
            Ok(dl_log_types::LogMsg::BlueprintActivationCommand(
...
fn arrow_msg_transport_to_app(
    arrow_msg: &dl_protos::log_msg::v1alpha1::ArrowMsg,
) -> Result<dl_log_types::ArrowMsg, CodecError> {
    dl_tracing::profile_function!();

    use dl_protos::log_msg::v1alpha1::Encoding;
    ...
    let chunk_id = dl_sorbet::chunk_id_of_schema(batch.schema_ref())?.as_tuid();
    ...
    //     .chunk_id
    //     .ok_or_else(|| missing_field!(dl_protos::log_msg::v1alpha1::ArrowMsg, "chunk_id"))?
    //     .try_from()?;

    // This also ensures that we perform all required migrations from `dl_sorbet`.
    // TODO(#10343): Would it make sense to change `dl_types_core::ArrowMsg` to contain the
    // `ChunkBatch` directly?
...
fn log_msg_app_to_transport(
    ...
    let proto_msg = match message {
        dl_log_types::LogMsg::SetStoreInfo(set_store_info) => {
            dl_protos::log_msg::v1alpha1::log_msg::Msg::SetStoreInfo(set_store_info.clone().into())
        }
        ...
        dl_log_types::LogMsg::ArrowMsg(store_id, arrow_msg) => {
            ...
            dl_protos::log_msg::v1alpha1::log_msg::Msg::ArrowMsg(arrow_msg)
        }

        dl_log_types::LogMsg::BlueprintActivationCommand(blueprint_activation_command) => {
            dl_protos::log_msg::v1alpha1::log_msg::Msg::BlueprintActivationCommand(
...
fn arrow_msg_app_to_transport(
    ...
    Ok(dl_protos::log_msg::v1alpha1::ArrowMsg {
        store_id: Some(store_id.into()),
        chunk_id: Some((*chunk_id).into()),
        compression: dl_protos::common::v1alpha1::Compression::from(compression) as i32,
        uncompressed_size: payload.uncompressed_size,
        encoding: dl_protos::log_msg::v1alpha1::Encoding::ArrowIpc as i32,
        payload: payload.data.into(),
        is_static: dl_sorbet::is_static_chunk(batch),
    })
}
