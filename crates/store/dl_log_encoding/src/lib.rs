...
//!
//! To put it in simple terms: transport-level types are the types that you find in `dl_protos`, while
...
//!
//! There are 3 different `LogMsg`-related types that you will very often encounter: `dl_log_types::LogMsg`,
//! `dl_protos::log_msg::v1alpha1::LogMsg` and `dl_protos::log_msg::v1alpha1::log_msg::Msg`.
//!
//! Mixing them up is a common source of pain and confusion, so let's go over what each does:
//! * `dl_log_types::LogMsg` is the application-level type that we use all across the viewer
...
//!   `dl_log_types::LogMsg` isn't used in Redap, where everything is done at the transport-level, always.
//! * `dl_protos::log_msg::v1alpha1::LogMsg` is the transport-level definition of `LogMsg`. It is an
