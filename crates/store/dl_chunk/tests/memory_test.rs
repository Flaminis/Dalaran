...
use dl_byte_size::testing::TrackingAllocator;
...
fn concat_does_allocate() {
    ...
    let (
        ...
        let concatenated =
            memory_use(|| dl_arrow_util::concat_arrays(&unconcatenated_refs).unwrap());
    ...
    eprintln!(
        ...
        dl_format::format_bytes(unconcatenated_size_bytes as _),
        dl_format::format_bytes(concatenated_size_bytes as _),
        dl_format::format_bytes(total_size_bytes as _),
    );
...
fn concat_single_is_noop() {
    ...
    let (
        ...
        let concatenated =
            memory_use(|| dl_arrow_util::concat_arrays(&[&*unconcatenated.0]).unwrap());
...
fn filter_does_allocate() {
    ...
    let (((unfiltered, unfiltered_size_bytes), (filtered, filtered_size_bytes)), total_size_bytes) =
        memory_use(|| {
            ...
            let filtered = memory_use(|| dl_arrow_util::filter_array(&unfiltered.0, &filter));
    ...
    eprintln!(
        ...
        dl_format::format_bytes(unfiltered_size_bytes as _),
        dl_format::format_bytes(filtered_size_bytes as _),
        dl_format::format_bytes(total_size_bytes as _),
    );
...
fn take_does_not_allocate() {
    ...
    let (((untaken, untaken_size_bytes), (taken, taken_size_bytes)), total_size_bytes) =
        memory_use(|| {
            ...
            let taken = memory_use(|| dl_arrow_util::take_array(&untaken.0, &indices));
    ...
    eprintln!(
        "untaken={} taken={} total={}",
        dl_format::format_bytes(untaken_size_bytes as _),
        dl_format::format_bytes(taken_size_bytes as _),
        dl_format::format_bytes(total_size_bytes as _),
    );
