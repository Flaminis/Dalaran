//! Tests the assumptions of [`dl_sorbet`].
//!
...
use dl_types_core::{ChunkId, ComponentDescriptor, RowId};
...
fn sorbet_version_presence() {
    ...
    let chunk = dl_chunk::ChunkBuilder::new(ChunkId::new(), EntityPath::root())
        .with_row(
            RowId::new(),
            TimePoint::STATIC,
    ...
    assert!(
        ...
        "migration code in `dl_sorbet` relies on `sorbet:version` to be present"
    );
}
