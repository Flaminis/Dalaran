...
impl Chunk {
    ...
    /// It is probably a good idea to sort the chunk first.
    // TODO(#8744): this is infallible, so we should not return a `Result` here.
    pub fn to_chunk_batch(&self) -> ChunkResult<dl_sorbet::ChunkBatch> {
        dl_tracing::profile_function!();
        self.sanity_check()?;

        dl_tracing::profile_function!(format!(
        ...
        let row_id_schema = dl_sorbet::RowIdColumnDescriptor {
            is_sorted: *is_sorted,
        };

        let (index_schemas, index_arrays): (Vec<_>, Vec<_>) = {
            dl_tracing::profile_scope!("timelines");
            ...
            let mut timelines = timelines
                ...
                .map(|info| {
                    ...
                    dl_log::debug_assert_eq!(&timeline.datatype(), array.data_type());

                    let schema =
                        dl_sorbet::IndexColumnDescriptor::from_timeline(*timeline, *is_sorted);
        ...
        let (data_schemas, data_arrays): (Vec<_>, Vec<_>) = {
            dl_tracing::profile_scope!("components");
            ...
            let mut components = components
                ...
                .map(|column| {
                    ...
                    let schema = dl_sorbet::ComponentColumnDescriptor {
        ...
        let schema = dl_sorbet::ChunkSchema::new(
            *id,
            entity_path.clone(),
            row_id_schema,
            index_schemas,
        ...
        Ok(dl_sorbet::ChunkBatch::try_new(
            schema,
    ...
    pub fn from_chunk_record_batch(batch: &ArrowRecordBatch) -> ChunkResult<Self> {
        ...
        Self::from_chunk_batch(&dl_sorbet::ChunkBatch::try_from(batch)?)
    }

    /// Convert an arbitrary record batch to one or more [`Chunk`]s.
    ///
    /// See [`dl_sorbet::chunk_batches_from_dataframe_record_batch`] for details.
    pub fn from_record_batch(
        batch: &ArrowRecordBatch,
        index: &dl_sorbet::DataframeIndex,
        entity_path: Option<&dl_log_types::EntityPath>,
        ...
        dl_sorbet::chunk_batches_from_dataframe_record_batch(batch, index, entity_path)
            .map_err(Box::new)?
            .iter()
    ...
    pub fn from_chunk_batch(batch: &dl_sorbet::ChunkBatch) -> ChunkResult<Self> {
        dl_tracing::profile_function!(format!(
            "num_columns={} num_rows={}",
            batch.num_columns(),
            batch.num_rows()
...
impl Chunk {
    #[inline]
    pub fn from_arrow_msg(msg: &dl_log_types::ArrowMsg) -> ChunkResult<Self> {
        dl_tracing::profile_function!();
    ...
    #[inline]
    pub fn to_arrow_msg(&self) -> ChunkResult<dl_log_types::ArrowMsg> {
        dl_tracing::profile_function!();
        self.sanity_check()?;
...
mod tests {
    ...
    use dl_types_core::{ChunkId, Loggable as _, RowId, TimelineName};
    ...
    fn roundtrip() -> anyhow::Result<()> {
        ...
        let components = [
            (MyPoints::descriptor_points(), {
                let list_array = dl_arrow_util::arrays_to_list_array_opt(&[
                    Some(&*points1),
                    points2,
                    Some(&*points3),
                    Some(&*points4),
        ...
        for timelines in [timelines1, timelines2] {
            ...
            let chunk_batch_after = dl_sorbet::ChunkBatch::try_from(&arrow_record_batch).unwrap();
    ...
    fn dataframe_batch(index: arrow::array::ArrayRef) -> ArrowRecordBatch {
        ...
        let values = ArrowField::new("/e:c", arrow::datatypes::DataType::Float32, true)
            .with_metadata(
                [(
                    dl_sorbet::metadata::SORBET_ENTITY_PATH.to_owned(),
                    "/e".to_owned(),
                )]
                .into(),
            );
    ...
    fn from_dataframe_record_batch_temporal() {
        ...
        let chunks =
            Chunk::from_record_batch(&batch, &dl_sorbet::DataframeIndex::Auto, None).unwrap();
    ...
    fn from_dataframe_record_batch_bad_index_dtype() {
        ...
        let err = Chunk::from_record_batch(
            &batch,
            &dl_sorbet::DataframeIndex::Columns(vec![TimelineName::from("frame")]),
            None,
        )
        .unwrap_err();
        assert!(matches!(
            ...
            ChunkError::DataframeToChunks(ref e)
                if matches!(**e, dl_sorbet::DataframeToChunksError::Sorbet(
                    dl_sorbet::SorbetError::IndexColumn(_)
                ))
        ));
    }
    ...
    fn from_dataframe_record_batch_null_index() {
        // A null in a promoted index column is rejected eagerly by the dl_sorbet conversion.
        let index = Arc::new(Int64Array::from(vec![Some(0_i64), None]));
        let batch = dataframe_batch(index);
        let err =
            Chunk::from_record_batch(&batch, &dl_sorbet::DataframeIndex::Auto, None).unwrap_err();
        assert!(
            matches!(
                err,
                ChunkError::DataframeToChunks(ref e)
                    if matches!(**e, dl_sorbet::DataframeToChunksError::NullIndexColumn(_))
            ),
            "got {err}"
        );
    }
