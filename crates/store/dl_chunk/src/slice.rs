...
impl Chunk {
    /// Returns the cell corresponding to the specified [`RowId`] for a given [`dl_types_core::ComponentIdentifier`].
    ///
    ...
    fn row_sliced_impl(&self, index: usize, len: usize, deep: bool) -> Self {
        dl_tracing::profile_function!(if deep { "deep" } else { "shallow" });
        ...
        let mut chunk = Self {
            ...
            is_sorted,
            row_ids: if deep {
                dl_arrow_util::deep_slice_array(row_ids, index, len)
            } else {
            ...
            components: components
                ...
                .map(|column| {
                    SerializedComponentColumn::new(
                        if deep {
                            dl_arrow_util::deep_slice_array(&column.list_array, index, len)
                        } else {
    ...
    pub fn densified(&self, component_pov: ComponentIdentifier) -> (Self, bool) {
        ...
        let mut chunk = Self {
            ...
            heap_size_bytes: Default::default(),
            is_sorted,
            row_ids: dl_arrow_util::filter_array(row_ids, &validity_filter),
            timelines: timelines
                .iter()
            ...
            components: components
                ...
                .map(|column| {
                    let filtered =
                        dl_arrow_util::filter_array(&column.list_array, &validity_filter);
    ...
    pub fn deduped_latest_on_index(&self, index: &TimelineName) -> Self {
        ...
        let chunk = Self {
            ...
            row_ids: dl_arrow_util::take_array(
                &self.row_ids,
            ...
            components: self
                ...
                .values()
                .map(|column| {
                    let filtered = dl_arrow_util::take_array(&column.list_array, &indices);
                    SerializedComponentColumn::new(filtered, column.descriptor.clone())
                })
                .collect(),
        };
    ...
    pub fn filtered(&self, filter: &ArrowBooleanArray) -> Option<Self> {
        ...
        let mut chunk = Self {
            ...
            heap_size_bytes: Default::default(),
            is_sorted,
            row_ids: dl_arrow_util::filter_array(row_ids, filter),
            timelines: timelines
                .iter()
            ...
            components: components
                .values()
                .map(|column| {
                    let filtered = dl_arrow_util::filter_array(&column.list_array, filter);
                    SerializedComponentColumn::new(filtered, column.descriptor.clone())
                })
                .collect(),
        };
    ...
    pub fn taken(&self, indices: &arrow::array::Int32Array) -> Self {
        ...
        let mut chunk = Self {
            ...
            components: components
                .values()
                .map(|column| {
                    let taken = dl_arrow_util::take_array(&column.list_array, indices);
...
impl TimeColumn {
    ...
    pub(crate) fn taken(&self, indices: &arrow::array::Int32Array) -> Self {
        ...
        let new_times = dl_arrow_util::take_array(
            &arrow::array::Int64Array::new(times.clone(), None),
            &arrow::array::Int32Array::from(indices.clone()),
        )
        .into_parts()
...
mod tests {
    ...
    fn cell() -> anyhow::Result<()> {
        ...
        let expectations: &[(_, _, Option<&dyn dl_types_core::ComponentBatch>)] = &[
        ...
        for (row_id, component, expected) in expectations {
            let expected = expected
                .and_then(|expected| dl_types_core::ComponentBatch::to_arrow(expected).ok());
    ...
    fn slice_memory_size_conservation() -> anyhow::Result<()> {
        ...
        let chunk = Chunk::new(
            crate::ChunkId::new(),
            dl_log_types::EntityPath::from(entity_path),
            Some(true), // is_sorted
