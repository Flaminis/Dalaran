...
impl Chunk {
    ...
    fn concatenated_unsorted(&self, rhs: &Self) -> ChunkResult<Self> {
        dl_tracing::profile_function!(format!(
            "lhs={} rhs={}",
            dl_format::format_uint(self.num_rows()),
            dl_format::format_uint(rhs.num_rows())
        ));
        ...
        let row_ids = {
            dl_tracing::profile_scope!("row_ids");

            let row_ids = dl_arrow_util::concat_arrays(&[&cl.row_ids, &cr.row_ids])?;
        ...
        let mut components: ChunkComponents = {
            dl_tracing::profile_scope!("components (r2l)");
            lhs_per_component
                .values()
                .filter_map(|lhs_column| {
                    dl_tracing::profile_scope!(lhs_column.descriptor.to_string());
                        ...
                        rhs_per_component.get(&lhs_column.descriptor.component)
                    {
                        if lhs_column.descriptor != rhs_column.descriptor {
                            dl_log::warn_once!("lhs and rhs have different component descriptors for the same component: {} != {}", lhs_column.descriptor, rhs_column.descriptor);
                        }

                        dl_tracing::profile_scope!(format!(
                            "concat (lhs={} rhs={})",
                            dl_format::format_uint(lhs_column.list_array.values().len()),
                            dl_format::format_uint(rhs_column.list_array.values().len()),
                        ));

                        let list_array =
                            dl_arrow_util::concat_arrays(&[&lhs_column.list_array, &rhs_column.list_array]).ok()?;
                        let list_array = list_array.downcast_array_ref::<ArrowListArray>()?.clone();

                        Some((lhs_column.descriptor.clone(), list_array))
                    } else {
                        dl_tracing::profile_scope!("pad");
                        Some((
                            lhs_column.descriptor.clone(),
                            dl_arrow_util::pad_list_array_back(
        ...
        {
            dl_tracing::profile_scope!("components (l2r)");
            let rhs = rhs_per_component
                ...
                .filter_map(|rhs_column| {
                    ...
                    dl_tracing::profile_scope!(rhs_column.descriptor.component.to_string());
                        ...
                        lhs_per_component.get(&rhs_column.descriptor.component)
                    {
                        ...
                        Some((
                            ...
                            SerializedComponentColumn::new(
                                dl_arrow_util::pad_list_array_front(
...
#[cfg(test)]
mod tests {
    use dl_log_types::example_components::{MyColor, MyLabel, MyPoint, MyPoint64, MyPoints};
    ...
    fn concat_and_sort_sorts_and_doesnt_warn_about_intermediate_steps() -> anyhow::Result<()> {
        ...
        let log_rx = dl_log::add_log_msg_receiver(dl_log::LevelFilter::WARN);
    ...
    fn malformed() -> anyhow::Result<()> {
        ...
        // Different datatypes
        {
            ...
            let points32bit =
                <MyPoint as dl_types_core::ComponentBatch>::to_arrow(&MyPoint::new(1.0, 1.0))?;
            let points64bit =
                <MyPoint64 as dl_types_core::ComponentBatch>::to_arrow(&MyPoint64::new(1.0, 1.0))?;
    ...
    /// iterate differently.
    fn with_reinserted_timelines(chunk: &Chunk, order: &[dl_log_types::TimelineName]) -> Chunk {
    ...
    fn concatenation_is_insensitive_to_timeline_map_iteration_order() -> anyhow::Result<()> {
        ...
        let points1 = &[MyPoint::new(1.0, 1.0)];
        let points2 = &[MyPoint::new(2.0, 2.0)];
        let build_chunk = |chunk_index: i64, points: &dyn dl_types_core::ComponentBatch| {
