...
pub struct BatcherHooks {
    ...
    ///
    /// See [`dl_log_types::ArrowRecordBatchReleaseCallback`] for more information.
    //
    // TODO(#6412): probably don't need this anymore.
    pub on_release: Option<dl_log_types::ArrowRecordBatchReleaseCallback>,
}
...
#[derive(Clone, Copy, Debug, PartialEq, Eq, dl_byte_size::SizeBytes)]
...
impl ChunkBatcherConfig {
    ...
    pub fn apply_env(&self) -> ChunkBatcherResult<Self> {
        ...
        if let Ok(s) = std::env::var(Self::ENV_FLUSH_NUM_BYTES) {
            if let Some(num_bytes) = dl_format::parse_bytes(&s) {
                // e.g. "10MB"
...
impl ChunkBatcher {
    ...
    pub fn new(config: ChunkBatcherConfig, hooks: BatcherHooks) -> ChunkBatcherResult<Self> {
        let (tx_cmds, rx_cmd) =
            dl_quota_channel::channel("batcher_input", config.max_bytes_in_flight / 2);
        let (tx_chunk, rx_chunks) =
            dl_quota_channel::channel("batcher_output", config.max_bytes_in_flight / 2);
        ...
        dl_log::debug!(?config, "creating new chunk batcher");
...
fn batching_thread(
    ...
    fn do_flush_all(
        ...
        dl_log::trace!(
            ...
            rows.len(),
            dl_format::format_bytes(acc.pending_num_bytes as _)
        );
        ...
        for chunk in chunks {
            let chunk = match chunk {
                Ok(chunk) => chunk,
                Err(err) => {
                    dl_log::error!(%err, "corrupt chunk detected, dropping");
                    continue;
                }
            };
    ...
    dl_log::trace!(
        ...
        dl_format::format_bytes(config.flush_num_bytes as _),
    );
    ...
    loop {
        crossbeam::select! {
            recv(rx_cmd.inner()) -> cmd => {
                ...
                let dl_quota_channel::SizedMessage { msg, size_bytes } = cmd;
                ...
                match msg {
                    ...
                    Command::Flush{ on_done } => {
                        ...
                        dl_quota_channel::send_crossbeam(&on_done, ()).ok();
                    },
                    ...
                    Command::UpdateConfig(new_config) => {
                        ...
                        if config.max_bytes_in_flight != new_config.max_bytes_in_flight {
                            dl_log::warn!("Cannot change max_bytes_in_flight after batcher has been created. \
                                Previous max_bytes_in_flight: {:?}, new max_bytes_in_flight: {:?}",
                                config.max_bytes_in_flight, new_config.max_bytes_in_flight);
                        }

                        dl_log::trace!("Updated batcher config: {:?}", new_config);
                        if let Some(on_config_change) = hooks.on_config_change.as_ref() {
...
impl PendingRow {
    ...
    pub fn many_into_chunks(
        ...
        // true no matter what.
        {
            dl_tracing::profile_scope!("sort rows");
        ...
        {
            dl_tracing::profile_scope!("compute timeline sets");
        ...
        per_timeline_set.into_values().flat_map(move |rows| {
            dl_tracing::profile_scope!("iterate per timeline set");
            ...
            let mut per_datatype_set: IntMap<u64 /* ArrowDatatype set */, Vec<Self>> =
                Default::default();
            {
                dl_tracing::profile_scope!("compute datatype sets");
            ...
            per_datatype_set.into_values().flat_map(move |rows| {
                dl_tracing::profile_scope!("iterate per datatype set");
...
mod tests {
    ...
    use dl_log_types::example_components::{MyIndex, MyLabel, MyPoint, MyPoint64, MyPoints};
