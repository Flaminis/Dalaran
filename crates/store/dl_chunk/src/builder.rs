...
impl ChunkBuilder {
    ...
    pub fn with_component<Component: dl_types_core::Component>(
        self,
        row_id: RowId,
        timepoint: impl Into<TimePoint>,
        component_descr: dl_types_core::ComponentDescriptor,
        value: &Component,
    ) -> dl_types_core::SerializationResult<Self> {
        dl_log::debug_assert_eq!(component_descr.component_type, Some(Component::name()));
        Ok(self.with_serialized_batches(
            row_id,
            timepoint,
            vec![dl_types_core::SerializedComponentBatch {
        ...
        let components = {
            ...
            components
                ...
                .filter_map(|(component_desc, arrays)| {
                    ...
                    dl_arrow_util::arrays_to_list_array_opt(&arrays)
                ...
                components
                    ...
                    .filter_map(|(component_desc, arrays)| {
                        ...
                        if let Some(datatype) = datatypes.get(&component_desc) {
                            dl_arrow_util::arrays_to_list_array(datatype.clone(), &arrays)
                                .map(|list_array| (component_desc, list_array))
                        } else {
                            dl_arrow_util::arrays_to_list_array_opt(&arrays)
                                .map(|list_array| (component_desc, list_array))
