...
pub enum ChunkError {
    ...
    #[error(transparent)]
    IndexColumn(#[from] dl_sorbet::IndexColumnError),

    #[error(transparent)]
    WrongDatatypeError(#[from] dl_arrow_util::WrongDatatypeError),

    #[error(transparent)]
    MismatchedChunkSchemaError(#[from] dl_sorbet::MismatchedChunkSchemaError),

    #[error(transparent)]
    InvalidSorbetSchema(#[from] dl_sorbet::SorbetError),

    // Boxed: `DataframeToChunksError` is large (it wraps a `SorbetError`), and this variant is rare.
    #[error(transparent)]
    DataframeToChunks(#[from] Box<dl_sorbet::DataframeToChunksError>),
}
...
impl ChunkComponents {
    ...
    pub fn ensure_similar(left: &Self, right: &Self) -> anyhow::Result<()> {
        ...
        for (component, left_column) in left.iter() {
            ...
            dl_arrow_util::ensure_similar(&left_array.to_data(), &right_array.to_data())
...
/// data within. For transport, see [`dl_sorbet::ChunkBatch`] instead.
///
...
#[derive(Debug, Clone, PartialEq, Eq, dl_byte_size::SizeBytes)]
...
impl TimeColumn {
    ...
    pub fn new(is_sorted: Option<bool>, timeline: Timeline, times: ArrowScalarBuffer<i64>) -> Self {
        dl_tracing::profile_function_if!(1000 < times.len(), format!("{} times", times.len()));
    ...
    pub fn new_sequence(
        name: impl Into<dl_log_types::TimelineName>,
...
impl std::fmt::Display for Chunk {
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let batch = self.to_record_batch().map_err(|err| {
            dl_log::error_once!("couldn't display Chunk: {err}");
            std::fmt::Error
        })?;
        dl_arrow_util::format_record_batch_with_width(&batch, f.width(), f.sign_minus()).fmt(f)
    }
}
...
impl dl_byte_size::SizeBytes for Chunk {
    #[inline]
            ...
            if advertised != 0 && advertised != measured {
                return Err(ChunkError::Malformed {
                    reason: format!(
                        ...
                        dl_format::format_bytes(advertised as _),
                        dl_format::format_bytes(measured as _),
                    ),
                });
            }
        }
