...
impl Chunk {
    ...
    pub fn sort_by_row_ids_if_needed(&mut self) {
        ...
        let swaps = {
            dl_tracing::profile_scope!("swaps");
    ...
    pub(crate) fn shuffle_with(&mut self, swaps: &[usize]) {
        ...
        // Row IDs
        {
            dl_tracing::profile_scope!("row ids");
            ...
            self.row_ids = dl_types_core::RowId::arrow_from_slice(&sorted_row_ids);
        }
        ...
        dl_tracing::profile_scope!("components (offsets & data)");
        {
            for original in components.list_arrays_mut() {
                ...
                let values = dl_arrow_util::concat_arrays(&sorted_arrays).unwrap();
