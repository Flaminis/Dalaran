    ...
    let data_sources_def = DataSourcesDefinition::new_with_tuid_prefix(
        ...
        [
            ...
            LayerDefinition::properties(
                ...
                [prop(
                    "text_log",
                    dl_sdk_types::archetypes::TextLog::new("i'm segment 1"),
                )],
            )
            .layer_name("props"),
            LayerDefinition::simple_with_time(
...
async fn per_segment_chunk_id_set<T: DalaranCloudService>(
    ...
    let mut ids: BTreeSet<dl_chunk::ChunkId> = BTreeSet::new();
...
fn per_segment_segment_ids() -> Vec<dl_types_core::SegmentId> {
...
async fn query_dataset_per_segment_values_wire_level_by_time_type<T: DalaranCloudService>(
    ...
    let index_name = dl_log_types::build_index_value(0_i64, time_type)
        .0
        .name()
        .as_str();
    ...
    let range_baseline_request = QueryDatasetRequest {
        ...
        query: Some(ext::Query {
            range: Some(ext::QueryRange {
                ...
                index_range: dl_log_types::AbsoluteTimeRange::EVERYTHING,
            }),
            ..Default::default()
        }),
        ..Default::default()
...
pub async fn query_dataset_per_segment_values_validation_rejected(service: impl DalaranCloudService) {
    ...
    let request = WireQueryDatasetRequest {
        segment_ids: vec![
            dl_protos::common::v1alpha1::SegmentId {
        ...
        select_all_entity_paths: true,
        query: Some(dl_protos::cloud::v1alpha1::Query {
            latest_at: Some(dl_protos::cloud::v1alpha1::QueryLatestAt {
                index: Some(dl_protos::common::v1alpha1::IndexColumnSelector {
                    timeline: Some(dl_protos::common::v1alpha1::Timeline {
                        name: "frame_nr".into(),
                    }),
                }),
                ...
                at: Some(1010),
                per_segment_values: vec![
                    dl_protos::cloud::v1alpha1::IndexValueList { values: vec![1010] },
                    dl_protos::cloud::v1alpha1::IndexValueList { values: vec![2010] },
                    dl_protos::cloud::v1alpha1::IndexValueList { values: vec![3010] },
                ],
            }),
            ..Default::default()
        }),
...
>(
    ...
    let request_wire: dl_protos::cloud::v1alpha1::QueryDatasetRequest = intersected_request.into();
    ...
    let mut intersected_ids: BTreeSet<dl_chunk::ChunkId> = BTreeSet::new();
    while let Some(resp) = stream.next().await {
        let resp: QueryDatasetResponse = resp.unwrap();
        if let Some(part) = resp.data {
            let batch: arrow::array::RecordBatch = part.try_into().unwrap();
