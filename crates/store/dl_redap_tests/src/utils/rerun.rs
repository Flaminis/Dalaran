...
pub fn next_chunk_id_generator(prefix: u64) -> impl FnMut() -> dl_chunk::ChunkId {
    let mut tuid = Tuid::from_nanos_and_inc(prefix, 0);
    move || {
        tuid = tuid.next();
        dl_chunk::ChunkId::from_tuid(tuid)
    }
}

pub fn next_row_id_generator(prefix: u64) -> impl FnMut() -> dl_chunk::RowId {
    let mut tuid = Tuid::from_nanos_and_inc(prefix, 0);
    move || {
        tuid = tuid.next();
        dl_chunk::RowId::from_tuid(tuid)
    }
}
...
pub fn create_nasty_recording(
    ...
    use dl_chunk::{Chunk, TimePoint};
    use dl_log_types::example_components::{MyColor, MyLabel, MyPoint, MyPoints};
    use dl_log_types::{EntityPath, TimeInt, build_frame_nr, build_log_time};
...
pub fn create_recording_with_embeddings(
    ...
    use dl_chunk::Chunk;
    use dl_log_types::{TimeInt, build_log_time};
    use dl_sdk::{ComponentDescriptor, SerializedComponentBatch};
    ...
    for (i, sentence) in sentences.iter().enumerate() {
        ...
        let chunk = Chunk::builder_with_id(next_chunk_id(), "/my_text")
            .with_archetype(
                next_row_id(),
                [build_log_time(frame.into())],
                &dl_sdk_types::archetypes::TextLog::new(sentence.to_owned()),
            )
            .build()?;
...
pub fn create_recording_with_properties(
    ...
    for (prop_name, properties) in user_defined_properties {
        let property_path =
            dl_log_types::EntityPath::properties().join(&dl_log_types::EntityPath::from(prop_name));
