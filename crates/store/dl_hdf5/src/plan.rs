...
fn warn_unsupported(walked: &Walk) {
    ...
    if !datasets.is_empty() {
        dl_log::warn!("Ignoring HDF5 datasets with unsupported element types: {datasets}");
    }
    ...
    if !attrs.is_empty() {
        dl_log::warn!("Ignoring HDF5 attributes of unsupported type: {attrs}");
    }
}
...
fn build_timeline(
    ...
    if let (Some(index), Some(path)) = (&config.index_column, &resolution.index_path) {
        ...
        let timeline_name = dl_chunk::TimelineName::try_new(leaf)
