...
#[derive(
    Clone, Debug, Hash, PartialEq, Eq, dl_byte_size::SizeBytes, serde::Deserialize, serde::Serialize,
)]
pub struct EntityPathPart(
    /// We use an interned string for fast copies, fast hashing, and to save memory.
    /// Note that `dl_string_interner` never frees memory, but even if a user
