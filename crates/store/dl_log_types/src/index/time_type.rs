...
impl TimeType {
    ...
    pub fn parse_sequence(s: &str) -> Option<TimeInt> {
        match s {
            ...
            _ => {
                ...
                dl_format::parse_i64(s).map(TimeInt::new_temporal)
            }
        }
    }
                ...
                match self {
                    Self::Sequence => {
                        if let Some(s) = s.strip_prefix('#') {
                            TimeInt::try_from(dl_format::parse_i64(s)?).ok()
                        } else {
                            TimeInt::try_from(dl_format::parse_i64(s)?).ok()
                        }
                    }
                    Self::DurationNs => {
                        if let Some(nanos) = dl_format::parse_i64(s) {
    ...
    pub fn format_opt(
        ...
        match time_int {
            ...
            _ => match self {
                Self::Sequence => format!("#{}", dl_format::format_int(time_int.as_i64())),
