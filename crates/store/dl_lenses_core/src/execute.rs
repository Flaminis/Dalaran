...
use dl_chunk::{ArrowArray as _, Chunk, ChunkComponents, ChunkId};
use dl_log_types::TimeType;
use dl_sdk_types::{ComponentDescriptor, SerializedComponentColumn};
...
fn output_components_iter<'a>(
    ...
    target_entity: &'a dl_chunk::EntityPath,
    runtime: &'a Runtime,
...
fn output_timelines_iter<'a>(
    ...
    target_entity: &'a dl_chunk::EntityPath,
    runtime: &'a Runtime,
) -> impl Iterator<Item = Result<(dl_chunk::TimelineName, TimeType, ListArray), LensRuntimeError>> + 'a
{
...
fn try_convert_time_column(
    timeline_name: dl_chunk::TimelineName,
    timeline_type: TimeType,
    list_array: &ListArray,
) -> Result<(dl_chunk::TimelineName, dl_chunk::TimeColumn), LensRuntimeError> {
    if let Some(time_vals) = list_array.values().as_any().downcast_ref::<Int64Array>() {
        let time_column = dl_chunk::TimeColumn::new(
            None,
            dl_chunk::Timeline::new(timeline_name, timeline_type),
...
fn finalize_chunk(
    entity_path: dl_chunk::EntityPath,
...
fn apply_one_to_one(work: &DeriveWork<'_>, runtime: &Runtime) -> Result<Chunk, LensError> {
    ...
    let mut component_results = dl_chunk::ChunkComponents::default();
    ...
    for (timeline_name, time_column) in original_timelines {
        ...
        match take(&time_values_array, scatter_indices, None) {
            Ok(scattered) => {
                ...
                let new_time_column = dl_chunk::TimeColumn::new(
                    None,
...
fn apply_one_to_many(work: &DeriveWork<'_>, runtime: &Runtime) -> Result<Chunk, LensError> {
    ...
    let mut chunk_components = dl_chunk::ChunkComponents::default();
