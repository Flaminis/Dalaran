...
use dl_log_types::{ResolvedEntityPathFilter, TimeType};
...
impl Lenses {
    ...
    pub fn add_lens(self, lens: Lens) -> Self {
        self.add_lens_with_filter(dl_log_types::EntityPathFilter::all(), lens)
    }

    /// Adds a lens with an entity path filter.
    pub fn add_lens_with_filter(
        mut self,
        filter: dl_log_types::EntityPathFilter,
        lens: Lens,
    ) -> Self {
