...
use dl_chunk::{ComponentIdentifier, EntityPath, TimelineName};
...
///
/// May carry a partial [`dl_chunk::Chunk`] containing the columns that
...
struct LensErrorInner {
    chunk: Option<dl_chunk::Chunk>,
    errors: Vec<LensRuntimeError>,
}

impl LensError {
    pub(crate) fn new(chunk: Option<dl_chunk::Chunk>, errors: Vec<LensRuntimeError>) -> Self {
        Self {
    ...
    pub fn with_partial_chunk(chunk: dl_chunk::Chunk, errors: Vec<LensRuntimeError>) -> Self {
        Self::new(Some(chunk), errors)
    }

    /// Returns the partial chunk, if any, and consumes `self`.
    pub fn partial_chunk(self) -> Option<dl_chunk::Chunk> {
        self.inner.chunk
    }
...
pub enum LensRuntimeError {
    ...
    ChunkValidationFailed(#[from] dl_chunk::ChunkError),
