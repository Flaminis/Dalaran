...
use dl_log_types::{EntityPath, TimeInt, TimeType, Timeline, entity_path};
use dl_query::{LatestAtResults, QueryCache, clamped_zip_1x1};
...
/// Build a ([`Timeline`], [`TimeInt`]) tuple from `frame_nr` suitable for inserting in a [`dl_log_types::TimePoint`].
