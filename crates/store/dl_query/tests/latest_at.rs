...
use dl_chunk_store::{ChunkStore, ChunkStoreSubscriber as _, ChunkTrackingMode, LatestAtQuery};
use dl_log_types::example_components::{MyColor, MyPoint, MyPoints};
use dl_log_types::{EntityPath, TimeInt, TimePoint, build_frame_nr};
...
fn simple_query() {
    ...
    let query = dl_chunk_store::LatestAtQuery::new(*timepoint[0].0.name(), timepoint[0].1);
...
fn simple_query_with_differently_tagged_components() {
    ...
    let points2_serialized = points2
        .serialized(dl_sdk_types::ComponentDescriptor {
            archetype: Some("MyPoints2".into()),
            component: "points2".into(),
            component_type: Some(<MyPoint as dl_types_core::Component>::name()),
        })
        .unwrap();
...
fn invalidation_of_future_optionals() {
    ...
    let query = dl_chunk_store::LatestAtQuery::new(*query_time[0].0.name(), query_time[0].1);
