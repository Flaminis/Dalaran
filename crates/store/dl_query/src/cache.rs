...
#[derive(Clone, PartialEq, Eq, PartialOrd, Ord, Hash, dl_byte_size::SizeBytes)]
...
impl ChunkStoreSubscriber for QueryCache {
    ...
    fn on_events(&mut self, events: &[ChunkStoreEvent]) {
        dl_tracing::profile_function!(format!("num_events={}", events.len()));
        ...
        for event in events {
            ...
            match diff {
                ...
                ChunkStoreDiff::Addition(add) => {
                    dl_tracing::profile_scope!("compact event (physical addition)");
                ...
                ChunkStoreDiff::Deletion(del) => {
                    dl_tracing::profile_scope!("compact event (physical deletion)");
