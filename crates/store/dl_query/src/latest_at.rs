...
impl QueryCache {
    ...
    pub fn gc(&self, target: dl_chunk_store::GarbageCollectionTarget) {
...
impl LatestAtResults {
    ...
    pub fn add(
        ...
        dl_log::debug_assert_eq!(chunk.num_rows(), 1);
...
impl LatestAtResults {
    ...
    pub fn component_batch<C: Component>(&self, component: ComponentIdentifier) -> Option<Vec<C>> {
        self.component_batch_with_log_level(dl_log::Level::ERROR, component)
    }
    ...
    pub fn component_instance_raw(
        ...
        self.component_instance_raw_with_log_level(dl_log::Level::ERROR, component, instance_index)
    }
    ...
    pub fn component_instance<C: Component>(
        ...
        self.component_instance_with_log_level(dl_log::Level::ERROR, instance_index, component)
    }
    ...
    pub fn component_mono_raw(&self, component: ComponentIdentifier) -> Option<ArrowArrayRef> {
        self.component_mono_raw_with_log_level(dl_log::Level::ERROR, component)
    }
    ...
    pub fn component_mono<C: Component>(&self, component: ComponentIdentifier) -> Option<C> {
        self.component_mono_with_log_level(component, dl_log::Level::ERROR)
    }
    ...
    fn ok_or_log_err<T>(
        ...
        match res {
            ...
            Err(dl_chunk::ChunkError::IndexOutOfBounds { len: 0, .. }) => None,

            Err(err) => {
                let entity_path = &self.entity_path;
                let index = self.max_index;
                let err = dl_error::format_ref(&err);
                dl_log::log_once!(
                    log_level,
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        for (query_time, unit) in per_query_time {
            strings.push(format!(
                ...
                dl_format::format_bytes(unit.total_size_bytes() as _)
            ));
        }
...
impl LatestAtCache {
    ...
    fn latest_at(
        ...
        //dl_tracing::profile_scope!("latest_at", format!("{component_type} @ {query:?}"));

        dl_log::debug_assert_eq!(query.timeline(), self.cache_key.timeline_name);
...
mod tests {
    ...
    fn next_chunk_id_generator(prefix: u64) -> impl FnMut() -> dl_chunk::ChunkId {
        let mut chunk_id = dl_chunk::ChunkId::from_tuid(Tuid::from_nanos_and_inc(prefix, 0));
        move || {
