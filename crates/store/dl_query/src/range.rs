...
impl QueryCache {
    ...
    /// even though [`dl_chunk::Chunk::range`] internally creates new IDs when slicing.
...
impl std::fmt::Debug for RangeCache {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        strings.push(format!(
            "{:?} ({})",
            self.time_range(),
            dl_format::format_bytes(chunks.total_size_bytes() as _),
        ));
...
impl RangeCache {
    ...
    fn range(
        ...
        dl_tracing::profile_scope!("range", format!("{query:?}"));
...
mod tests {
    ...
    use dl_chunk_store::{ChunkDeletionReason, ChunkStore, ChunkStoreConfig, ChunkStoreHandle};
