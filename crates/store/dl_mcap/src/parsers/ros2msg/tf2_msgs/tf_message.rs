...
use dl_sdk_types::components::{RotationQuat, Translation3D};
...
impl MessageParser for TfMessageParser {
    ...
    ) -> anyhow::Result<Vec<dl_chunk::TimePoint>> {
    ...
    fn finalize(self: Box<Self>, ctx: ParserContext) -> anyhow::Result<Vec<dl_chunk::Chunk>> {
        dl_tracing::profile_function!();
        let Self {
            translations,
            quaternions,
...
mod tests {
    ...
    use dl_log_types::{TimeCell, TimeType, TimelineName};
