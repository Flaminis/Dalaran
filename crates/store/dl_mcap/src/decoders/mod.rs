        ...
        let decode_chunk = |chunk: &::mcap::records::ChunkIndex| -> Result<Vec<Chunk>, Error> {
            ...
            for msg in summary.stream_chunk(mcap_bytes, chunk)? {
                match msg {
                    ...
                    Err(err) => {
                        dl_log::error!("Failed to read message from MCAP file: {err}");
                    }
                }
            }
            ...
            for mut chunk in decoder.finish() {
                ...
                match chunk {
                    Ok(c) => batch.push(c),
                    Err(err) => dl_log::error!("Failed to decode chunk: {err}"),
                }
            }
        ...
        #[cfg(not(target_arch = "wasm32"))]
        {
            ...
            let (producer_result, ()) = rayon::join(
                            ...
                            .map(|_| {
                                ...
                                scope.spawn(move || -> Result<(), Error> {
                                    loop {
                                        ...
                                        dl_quota_channel::send_crossbeam(&batch_tx, (idx, batch))
                                            .map_err(|err| {
                ...
                || {
                    ...
                    dl_log::debug_assert!(buffer.is_empty(), "All batches should've been consumed");
                },
            );
...
impl DecoderRegistry {
    ...
    pub fn register_file_decoder<L: Decoder + Default + 'static>(mut self) -> Self {
        ...
        if self
            ...
            .is_some()
        {
            dl_log::warn_once!("Inserted file decoder {} twice.", id);
        }
        self
    }
    ...
    pub fn register_message_decoder<M: MessageDecoder + Default + 'static>(mut self) -> Self {
        ...
        if self
            ...
            .is_some()
        {
            dl_log::warn_once!("Inserted message decoder {} twice.", id);
        }
...
mod tests {
    ...
    use dl_log_types::TimeType;
    use dl_sdk_types::archetypes::McapMessage;
