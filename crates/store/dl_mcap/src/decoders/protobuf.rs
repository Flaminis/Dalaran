...
fn append_value(
    ...
    match val {
        ...
        Value::List(vec) => {
            dl_log::trace!("Append called on a list with {} elements: {val}", vec.len(),);
            ...
            dl_log::trace!("Finished append on list with elements {val}");
        }
        Value::Map(hash_map) => {
...
fn struct_builder_from_message(message_descriptor: &MessageDescriptor) -> StructBuilder {
    ...
    dl_log::debug_assert_eq!(fields.len(), field_builders.len());
...
impl MessageDecoder for McapProtobufDecoder {
    ...
    fn init(&mut self, summary: &mcap::Summary) -> Result<(), Error> {
        for channel in summary.channels.values() {
            ...
            dl_log::debug_assert!(found.is_none());
        }
