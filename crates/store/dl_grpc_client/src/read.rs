...
use dl_protos::sdk_comms::v1alpha1::{ReadMessagesRequest, ReadMessagesResponse};
...
pub fn stream(
    ...
    uri: dl_uri::ProxyUri,
) -> dl_log_channel::LogReceiver {
    dl_log::debug!(?uri, "Loading via gRPC…");

    let (tx, rx) =
        dl_log_channel::log_channel(dl_log_channel::LogSource::MessageProxy(uri.clone()));
...
async fn stream_async(
    ...
    dl_log::debug!(?uri, "Streaming messages from gRPC endpoint");
    ...
    loop {
        match stream.try_next().await {
            Ok(Some(ReadMessagesResponse {
                ...
                if let Some(metadata_key) = dl_sorbet::TimestampLocation::IPCDecode.metadata_key() {
                    ...
                    target_arch = "wasm32" => {
                        ...
                        loop {
                            match tx.try_send(msg) {
                                Ok(()) => break,
                                Err(dl_log_channel::TrySendError::Full(unsent_msg)) => {
                                    msg = *unsent_msg;
                                    dl_async::yield_now().await;
                                    last_yield = web_time::Instant::now();
                                }
                                Err(dl_log_channel::TrySendError::Disconnected(_)) => {
                                    dl_log::debug!("gRPC stream smart channel closed");
                                    return Ok(());
                                }
                            }
                        }
            ...
            Ok(Some(ReadMessagesResponse { log_msg: None })) => {
                dl_log::debug!("empty ReadMessagesResponse");
            }

            // Stream closed
            Ok(None) => {
                dl_log::debug!("gRPC stream disconnected");
                break;
            }
