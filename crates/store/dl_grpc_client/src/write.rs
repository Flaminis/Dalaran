...
impl Drop for Client {
    fn drop(&mut self) {
        ...
        dl_log::debug!("Message proxy client has shut down");
    }
}
...
async fn message_proxy_client(
    ...
    let endpoint = match Endpoint::from_shared(uri.origin.as_url()) {
        ...
        Err(err) => {
            ...
            dl_log::error!("Invalid message proxy server endpoint: {err}");
            return;
        }
    };
    ...
    let channel = loop {
        match endpoint.connect().await {
            ...
            Err(err) => {
                ...
                {
                    dl_log::debug!(?uri, "Failed to connect: {err}, retrying…");
                ...
                tokio::select! {
                    _ = shutdown_rx.recv() => {
                        ...
                        dl_log::debug!("Shutting down client without flush");
                        return;
                    }
                ...
                cmd = cmd_rx.recv() => {
                    match cmd {
                        Some(Cmd::LogMsg(mut log_msg)) => {
                            if let Some(metadata_key) = dl_sorbet::TimestampLocation::IPCEncode.metadata_key() {
                        ...
                        Some(Cmd::Flush { on_done }) => {
                            ...
                            dl_log::trace!("Flush requested");
                            if dl_quota_channel::send_crossbeam(&on_done, ()).is_err() {
                                // Flush channel may already be closed for non-blocking flush, so this isn't an error.
                                dl_log::debug!("Failed to respond to flush: flush report channel was closed");
                                break;
                            }
                        }
                        ...
                        None => {
                            ...
                            dl_log::debug!("Shutdown channel closed");
                            break;
                        }
                    }
                }
