...
/// It always has a [`dl_types_core::RowId`] column.
#[derive(Debug, Clone)]
...
impl ChunkBatch {
    ...
    ///
    /// See `dl_chunk_store::ChunkStore::schema` or [`crate::SchemaBuilder`] to compute
...
impl std::fmt::Display for ChunkBatch {
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        dl_arrow_util::format_record_batch_with_width(self, f.width(), f.sign_minus()).fmt(f)
    }
}
