...
fn migrate_tuid_column(
    ...
    if let Some(struct_array) = array.as_struct_opt() {
        ...
        let new_field = ArrowField::new(
            field.name(),
            <Tuid as dl_types_core::Loggable>::arrow_datatype(),
            false,
        )
        .with_metadata(field.metadata().clone());
        let new_array = dl_types_core::tuids_to_arrow(&tuids);

        dl_log::debug_once!("Migrated legacy TUID encoding of column {}", field.name());
