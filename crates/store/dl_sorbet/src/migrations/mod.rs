    ...
    batch = match get_or_guess_version(&batch) {
        Ok(batch_version) => match batch_version.cmp(&SorbetSchema::METADATA_VERSION) {
            Ordering::Equal => {
                ...
                dl_log::trace!("Batch version matches Sorbet version ({batch_version})");
                batch
            }
            Ordering::Less => {
                ...
                if batch_version < first_supported {
                    ...
                    dl_log::debug_once!("Performing migrations from {batch_version}…");
        ...
        Err(err) => {
            dl_log::error_once!("Skipping migrations due to error: {err}");
            batch
        }
    };
