...
use dl_byte_size::{MemUsageNode, MemUsageTree, SizeBytes};
use dl_log_channel::{DataSourceMessage, DataSourceUiCommand};
use dl_log_encoding::{ToApplication as _, ToTransport as _};
use dl_log_types::TableMsg;
use dl_protos::common::v1alpha1::{
    DataframePart as DataframePartProto, StoreKind as StoreKindProto, TableId as TableIdProto,
};
use dl_protos::log_msg::v1alpha1::LogMsg as LogMsgProto;
...
use dl_quota_channel::{async_broadcast_channel, async_mpsc_channel};
...
impl std::fmt::Display for TonicStatusError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        // NOTE: duplicated in `dl_grpc_client` and `dl_redap_client`
...
async fn serve_impl(
    ...
    dl_log::debug!("Server memory limit set at {}", options.memory_limit);
    ...
    let routes = extra_services.add_service(
        dl_protos::sdk_comms::v1alpha1::message_proxy_service_server::MessageProxyServiceServer::new(
            message_proxy,
        )
...
pub async fn serve_from_channel(
    ...
    options: ServerOptions,
    shutdown: shutdown::Shutdown,
    channel_rx: dl_log_channel::LogReceiver,
) {
    let message_proxy = MessageProxy::new(options.clone());
    let event_tx = message_proxy.event_tx.clone();

    tokio::task::spawn_blocking(move || {
        use dl_log_channel::SmartMessagePayload;
            ...
            let msg = if let Ok(msg) = channel_rx.recv() {
                match msg.payload {
                    ...
                    SmartMessagePayload::Quit(err) => {
                        if let Some(err) = err {
                            dl_log::debug!("smart channel sender quit: {err}");
                        } else {
                            dl_log::debug!("smart channel sender quit");
                        }
                        break;
                    }
                }
            } else {
                dl_log::debug!("smart channel sender closed, closing receiver");
                break;
            };
            ...
            match msg {
                DataSourceMessage::LogMsg(msg) => {
                    ...
                    if event_tx
                        ...
                        .is_err()
                    {
                        dl_log::debug!("shut down, closing sender");
                        break;
                    }
                }
                unsupported => {
                    dl_log::error_once!(
                        "Not implemented: dl_grpc_server support for {}",
    ...
    .await
    {
        dl_log::error!("message proxy server crashed: {err}");
    }
}
...
pub fn spawn_from_rx_set(
    ...
    options: ServerOptions,
    shutdown: shutdown::Shutdown,
    rxs: dl_log_channel::LogReceiverSet,
) -> MessageProxyHandle {
    let message_proxy = MessageProxy::new(options.clone());
...
pub fn spawn_with_recv_and_services(
    ...
    let (channel_log_tx, channel_log_rx) =
        dl_log_channel::log_channel(dl_log_channel::LogSource::MessageProxy(uri));
    ...
    loopback_services.add_service(
        dl_protos::sdk_comms::v1alpha1::viewer_control_service_server::ViewerControlServiceServer::new(
    ...
    tokio::spawn(async move {
        let mut app_id_cache = dl_log_encoding::CachingApplicationIdInjector::default();
            ...
            let msg: anyhow::Result<DataSourceMessage> = match broadcast_log_rx.recv().await {
                Ok(inner) => match inner {
                    LogOrTableMsgProto::LogMsg(msg) => match msg.msg {
                        ...
                        None => Err(dl_protos::missing_field!(
                            dl_protos::log_msg::v1alpha1::LogMsg,
                            "msg"
                        )
                        .into()),
                    },
                    ...
                    LogOrTableMsgProto::Table(msg) => match msg.data.try_into() {
                        ...
                        Err(err) => {
                            dl_log::error!("Dropping LogMsg::Table due to failed decode: {err}");
                            continue;
                        }
                    },
                ...
                Err(async_broadcast_channel::RecvError::Closed) => {
                    dl_log::debug!("message proxy server shut down, closing receiver");
            ...
            match msg {
                Ok(mut log_msg) => {
                    if let Some(metadata_key) =
                        dl_sorbet::TimestampLocation::IPCDecode.metadata_key()
                    {
                        ...
                        log_msg.insert_arrow_record_batch_metadata(
                            ...
                            dl_sorbet::timestamp_metadata::now_timestamp(),
                        );
                    }
                ...
                Err(err) => {
                    dl_log::error!("dropping LogMsg due to failed decode: {err}");
                }
            }
        }
    });
...
impl MessageBuffer {
    ...
    fn add_log_msg(&mut self, msg: LogMsgProto) {
        let Some(inner) = &msg.msg else {
            dl_log::error!(
                "{}",
                dl_protos::missing_field!(dl_protos::log_msg::v1alpha1::LogMsg, "msg")
            );
            return;
        };
    ...
    pub fn gc(&mut self, max_bytes: u64) {
        ...
        dl_tracing::profile_scope!("Drop messages");
        dl_log::info_once!(
            ...
            dl_format::format_bytes(max_bytes as _)
        );
        ...
        dl_log::trace!(
            ...
            dl_format::format_bytes(bytes_dropped as _)
        );
...
impl MessageProxy {
    ...
    fn new_with_recv(
        ...
        let (event_tx, event_rx) = {
            ...
            async_mpsc_channel::channel("dl_grpc_server events", message_queue_capacity)
        };
    ...
    async fn new_client_message_stream(&self) -> ReadMsgStream {
        ...
        if let Err(err) = self.event_tx.send(Event::NewClient(sender)).await {
            dl_log::error!("Error accepting new client: {err}");
        ...
        let channel = BackPressureReceiverStream::new(msg_channel).map(|result| {
            result.map(ReadLogOrTableMsgResponse::from).map_err(|err| {
                dl_log::error!("Error reading message from broadcast channel: {err}");
            ...
            self.new_client_message_stream()
                ...
                .filter_map(|msg| match msg {
                    ...
                    Ok(ReadLogOrTableMsgResponse::TableMsg(_)) => {
                        dl_log::warn_once!("A log stream got a TableMsg");
                        None
                    }
                    Ok(ReadLogOrTableMsgResponse::UiCommand) => {
                        dl_log::warn_once!("A log stream got a UiCommandMsg");
                        None
                    }
            ...
            self.new_client_message_stream()
                ...
                .filter_map(|msg| match msg {
                    Ok(ReadLogOrTableMsgResponse::LogMsg(_)) => {
                        dl_log::warn_once!("A table stream got a LogMsg");
                        None
                    }
...
impl message_proxy_service_server::MessageProxyService for MessageProxy {
        ...
        loop {
            match stream.message().await {
                ...
                Ok(Some(WriteMessagesRequest { log_msg: None })) => {
                    dl_log::warn!("missing log_msg in WriteMessagesRequest");
                }
                ...
                Err(err) => {
                    dl_log::error!("Error while receiving messages: {}", TonicStatusError(err));
                    break;
                }
            }
        }
    ...
    async fn write_table(
        ...
        } = request.into_inner()
        {
            ...
            dl_log::warn!("malformed `WriteTableRequest`");
        }
    ...
    fn fake_log_stream_blueprint(store_id: &StoreId, n: usize) -> Vec<LogMsg> {
        ...
        for _ in 0..n {
            messages.push(LogMsg::ArrowMsg(
                store_id.clone(),
                dl_chunk::Chunk::builder("test_entity")
                    .with_archetype(
                        dl_chunk::RowId::new(),
                        dl_log_types::TimePoint::default().with(
                            dl_log_types::Timeline::new_sequence("blueprint"),
                            dl_log_types::TimeInt::from_millis(dl_log_types::NonMinI64::MIN),
                        ),
                        &dl_sdk_types::blueprint::archetypes::Background::new(
                            dl_sdk_types::blueprint::components::BackgroundKind::SolidColor,
                        )
                        .with_color([255, 0, 0]),
                    )
                    .build()
    ...
    fn generate_log_messages(
        ...
        for _ in 0..n {
            let timepoint = match temporalness {
                Temporalness::Static => dl_log_types::TimePoint::STATIC,
                Temporalness::Temporal => dl_log_types::TimePoint::default().with(
                    dl_log_types::Timeline::new_sequence("log_time"),
                    dl_log_types::TimeInt::from_millis(dl_log_types::NonMinI64::MIN),
                ),
            };
            ...
            messages.push(LogMsg::ArrowMsg(
                ...
                dl_chunk::Chunk::builder("test_entity")
                    .with_archetype(
                        dl_chunk::RowId::new(),
                        timepoint,
                        &dl_sdk_types::archetypes::Points2D::new([
                            (0.0, 0.0),
                            (1.0, 1.0),
                            (2.0, 2.0),
                        ]),
