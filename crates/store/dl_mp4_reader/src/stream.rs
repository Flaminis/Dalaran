//! Stream-mode chunk emission: demux the mp4 with `dl_video` and emit
...
use dl_chunk::{Chunk, ChunkId, EntityPath, RowId, TimeColumn, TimePoint};
use dl_log_types::{TimeType, Timeline, TimelineName};
...
fn transcoded_segments(
    input: StreamInput,
    source_codec: dl_video::VideoCodec,
    transcode: &Mp4TranscodeOptions,
    debug_name: &str,
    ...
    let chunks = dl_video::transcode_mp4(&path, source_codec, transcode, debug_name)
...
/// Map a `dl_video` `FFmpeg` error into an [`Mp4Error`], preserving the original
/// message and appending a download hint when the executable is missing.
#[cfg(not(target_arch = "wasm32"))]
fn map_ffmpeg_err(err: &dl_video::FFmpegError) -> Mp4Error {
    ...
    if let dl_video::FFmpegError::NoEncoderForCodec { codec } = err {
    ...
    if matches!(err, dl_video::FFmpegError::FFmpegNotInstalled)
        && let Some(url) = dl_video::ffmpeg_download_url()
    {
...
///
/// `dl_video` hands back the fragmented mp4 as an iterator of arbitrary byte
...
#[cfg(not(target_arch = "wasm32"))]
impl<I: Iterator<Item = Result<Vec<u8>, dl_video::FFmpegError>>> Read for ChunkReader<I> {
...
fn build_gop_chunk(
    ...
    for sample_idx in range {
        ...
        // produced by `dl_video::demux::mp4`).
