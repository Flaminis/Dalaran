//! Asset-mode chunk emission: lift of `dl_importer::importer_archetype::load_video`.
...
use dl_sdk_types::components::VideoTimestamp;
...
impl AssetChunkIter {
    ...
    fn next_asset_chunk(
        ...
        let video_asset = {
            dl_tracing::profile_scope!("serialize-as-arrow");
    ...
    fn next_index_chunk(&self, frame_timestamps_nanos: Vec<i64>) -> Result<Chunk, Mp4Error> {
        ...
        let video_timestamps = frame_timestamps_nanos
            ...
            .collect::<Vec<_>>();
        let video_timestamp_batch = &video_timestamps as &dyn dl_sdk_types::ComponentBatch;
        let video_timestamp_list_array = video_timestamp_batch
            .to_arrow_list_array()
            .map_err(dl_chunk::ChunkError::from)?;
