//! Integration tests for [`dl_mp4_reader`] stream mode.
//!
...
fn fixture_path(file_name: &str) -> PathBuf {
    // `crates/store/dl_mp4_reader` → workspace root is three levels up.
...
fn collect_chunks(
    iter: impl Iterator<Item = Result<Chunk, dl_mp4_reader::Mp4Error>>,
    label: &str,
) -> Vec<Chunk> {
...
fn transcode_or_skip(
    ...
    let is_env_error = |err: &dl_mp4_reader::Mp4Error| {
