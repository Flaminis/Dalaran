// TODO(andreas): Conceptually these should go to `dl_data_source`.
// However, `dl_data_source` depends on everything that _implements_ a datasource, therefore we would get a circular dependency!

use std::sync::Arc;

use dl_log_encoding::RrdManifest;
use dl_log_types::{LogMsg, StoreId, TableMsg, impl_into_enum};
use dl_protos::sdk_comms::v1alpha1::{GetViewerStateResponse, SetTimeCursorResponse};
...
pub enum DataSourceUiCommand {
    ...
    SetUrlFragment {
        ...
        /// Uri fragment, see `dl_uri::Fragment` on how to parse it.
        // Not using `dl_uri::Fragment` to avoid further dependency entanglement.
        fragment: String, //dl_uri::Fragment,
    },
    ...
    ///
    /// Used by `dl_viewer_mcp`'s `GetViewerState` gRPC method to give an agent context.
    GetViewerState {
    ...
    ///
    /// Used by `dl_viewer_mcp`'s `SetTimeCursor` gRPC method.
    SetTimeCursor {
...
impl dl_byte_size::SizeBytes for DataSourceUiCommand {
