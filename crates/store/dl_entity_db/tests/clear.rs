...
use dl_chunk::{Chunk, ComponentIdentifier, RowId};
...
use dl_types_core::archetypes::Clear;
use dl_types_core::components::ClearIsRecursive;

// ---

fn query_latest_component<C: dl_types_core::Component>(
    db: &EntityDb,
    ...
    // * Query 'parent' at frame #11 and make sure we do _not_ find it.
    {
        ...
        let chunk = Chunk::builder(entity_path_parent.clone())
            .with_component_batches(
                ...
                timepoint,
                [(
                    dl_log_types::example_components::MyIndex::partial_descriptor(),
                    &[instance] as _,
                )],
            )
            .build()?;
