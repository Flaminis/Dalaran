...
mod tests {
    ...
    use dl_types_core::ComponentBatch;
    ...
    fn row_count_tracks_additions() {
        let mut store = ChunkStore::new(
            StoreId::random(dl_log_types::StoreKind::Recording, "test"),
    ...
    fn row_count_tracks_deletions() {
        ...
        let (gc_events, _stats) =
            store.gc(&dl_chunk_store::GarbageCollectionOptions::gc_everything());
    ...
    fn build_manifest_chunks(
        ...
        times: &[i64],
        store_id: &StoreId,
    ) -> (Vec<Arc<Chunk>>, Arc<dl_log_encoding::RrdManifest>) {
        let point = MyPoint::new(1.0, 1.0);
    ...
    fn virtual_addition_row_count() {
        let store_id = StoreId::random(dl_log_types::StoreKind::Recording, "test");
