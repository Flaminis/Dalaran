...
use dl_log::{debug_assert, debug_assert_eq};
...
pub enum EntityDbClass<'a> {
    ...
    DatasetSegment(&'a dl_uri::DatasetSegmentUri),
...
impl EntityDb {
    ...
    pub fn redap_uri(&self) -> Option<&dl_uri::DatasetSegmentUri> {
        if let Some(dl_log_channel::LogSource::RedapGrpcStream { uri, .. }) = &self.data_source {
            Some(uri)
        } else {
            None
        }
    ...
    pub fn recording_info_property<C: dl_types_core::Component>(
        &self,
    ...
    pub fn set_recording_property<Component: dl_types_core::Component>(
        &mut self,
        entity_path: EntityPath,
        component_descr: dl_types_core::ComponentDescriptor,
        value: &Component,
    ) -> Result<(), Error> {
        ...
        self.storage_engine()
            ...
            .unwrap_or_else(|| {
                if timeline_name == &TimelineName::log_time() {
                    ...
                    dl_log::warn_once!("Timeline {timeline_name:?} not found");
                    TimeType::Sequence
                }
            })
    }
    ...
    ///
    /// See [`dl_query::LatestAtResults`] for more information about how to handle the results.
    ///
    /// This is a cached API -- data will be lazily cached upon access.
    #[inline]
    pub fn latest_at(
        &self,
        query: &dl_chunk_store::LatestAtQuery,
        entity_path: &EntityPath,
        components: impl IntoIterator<Item = ComponentIdentifier>,
    ) -> dl_query::LatestAtResults {
    ...
    /// Get the latest index and value for a given dense [`dl_types_core::Component`].
    ///
    ...
    #[inline]
    pub fn latest_at_component<C: dl_types_core::Component>(
        &self,
        entity_path: &EntityPath,
        query: &dl_chunk_store::LatestAtQuery,
        component: ComponentIdentifier,
    ...
    #[inline]
    pub fn latest_at_component_quiet<C: dl_types_core::Component>(
        &self,
        entity_path: &EntityPath,
        query: &dl_chunk_store::LatestAtQuery,
        component: ComponentIdentifier,
    ...
    #[inline]
    pub fn latest_at_component_at_closest_ancestor<C: dl_types_core::Component>(
        &self,
        entity_path: &EntityPath,
        query: &dl_chunk_store::LatestAtQuery,
        component: ComponentIdentifier,
    ...
    #[inline]
    pub fn generation(&self) -> dl_chunk_store::ChunkStoreGeneration {
    ...
    pub fn add_record_batch(
        ...
        dl_tracing::profile_function!(format!(
            "{} rows",
            dl_format::format_uint(record_batch.num_rows())
        ));

        self.last_modified_at = web_time::Instant::now();
        let chunk_batch =
            dl_sorbet::ChunkBatch::try_from(record_batch).map_err(dl_chunk::ChunkError::from)?;
        let mut chunk = dl_chunk::Chunk::from_chunk_batch(&chunk_batch)?;
    ...
    fn add_chunk_with_timestamp_metadata(
        &mut self,
        chunk: &Arc<Chunk>,
        chunk_timestamps: &dl_sorbet::TimestampMetadata,
    ) -> Result<Vec<ChunkStoreEvent>, Error> {
    ...
    pub fn gc(&self, gc_options: &GarbageCollectionOptions) -> Vec<ChunkStoreEvent> {
        ...
        dl_log::trace!(
            ...
            size_bytes_dropped = dl_format::format_bytes(stats_diff.total().total_size_bytes as _),
            "purged datastore"
        );
    ...
    pub fn drop_time_range(
        ...
        reason: dl_chunk_store::ChunkDeletionReason,
    ) -> Vec<ChunkStoreEvent> {
        dl_tracing::profile_function!();
    ...
    pub fn to_messages(
        ...
        let blueprint_ready = if self.store_kind() == StoreKind::Blueprint {
            let activate_cmd =
                dl_log_types::BlueprintActivationCommand::make_active(self.store_id().clone());
...
impl dl_byte_size::SizeBytes for EntityDb {
    #[inline]
...
mod tests {
    ...
    use dl_log_types::{StoreId, TimePoint, Timeline};
