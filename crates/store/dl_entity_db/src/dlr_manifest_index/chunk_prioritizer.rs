...
use crate::{
    ...
    dlr_manifest_index::{LoadState, RootChunkInfo},
    sorted_range_map::{OverlapIterState, SortedRangeMap},
};

#[derive(Clone, Copy, Default, dl_byte_size::SizeBytes)]
...
pub enum PrefetchError {
    ...
    #[error("Codec: {0}")]
    Codec(#[from] dl_log_encoding::CodecError),
...
#[derive(Clone, Default, dl_byte_size::SizeBytes)]
...
impl<'a> ChunkRequestBatcher<'a> {
    ...
    fn finish_batch(&mut self) -> Result<(), PrefetchError> {
        ...
        let rb = dl_arrow_util::take_record_batch(
...
#[derive(Copy, Clone, PartialEq, Eq, Hash, dl_byte_size::SizeBytes)]
...
pub struct ChunkPrioritizer {
    ...
    /// Intervals of all root chunks in the dlr manifest per timeline.
    root_chunk_intervals: BTreeMap<Timeline, SortedRangeMap<TimeInt, ChunkId>>,

    /// All static root chunks in the dlr manifest.
...
impl ChunkPrioritizer {
    pub fn on_dlr_manifest(&mut self, delta: &RrdManifest) {
    ...
    pub fn prepare_chunk_fetcher<'a>(
        ...
        for &physical_chunk_id in &chunk_usage_tracker.used_physical {
            ...
            if let Some(chunk) = store.physical_chunk(&physical_chunk_id) {
                ...
                dl_log::debug_warn_once!("Couldn't get physical chunk from chunk store");
            }
        }
...
impl Drop for ChunkFetcher<'_> {
    fn drop(&mut self) {
        if self.request_batcher.is_some() {
            dl_log::debug_warn_once!("`ChunkFetcher::finish` not called for `ChunkFetcher`");
        }
    }
}
    ...
    fn fetch_inner(
        ...
        loop {
            ...
            let Some(root_chunk) = self.root_chunks.get(&root_chunk_id) else {
                dl_log::debug_warn_once!("Missing root chunk");
                continue;
            };
...
mod tests {
    ...
    use arrow::array::RecordBatch;
    use dl_byte_size::SizeBytes as _;
    use dl_chunk::{Chunk, EntityPath, RowId, TimeInt, Timeline};
    use dl_chunk_store::ChunkStore;
    use dl_log_encoding::RrdManifest;
    use dl_log_types::example_components::{MyPoint, MyPoints};
    use dl_log_types::{AbsoluteTimeRange, StoreId, StoreKind, TimePoint};
    ...
    fn setup_test_recording(chunks: &[Arc<Chunk>]) -> (ChunkStore, RrdManifestIndex) {
        ...
        let _events = store.insert_dlr_manifest(manifest.clone());
    ...
    fn recording_load_fn() -> (
        ...
        Arc<dl_mutex::Mutex<Vec<ChunkId>>>,
    ) {
        let requested = Arc::new(dl_mutex::Mutex::new(Vec::<ChunkId>::new()));
