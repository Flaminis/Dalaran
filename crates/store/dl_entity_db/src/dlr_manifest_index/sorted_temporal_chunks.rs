...
#[derive(Default, Clone, dl_byte_size::SizeBytes)]
pub(super) struct SortedEntityTemporalChunks {
    ...
    per_component: IntMap<dl_chunk::ComponentIdentifier, Vec<ChunkCountInfo>>,
}
...
impl SortedEntityTemporalChunks {
    ...
    pub fn component_chunks(&self, component: &dl_chunk::ComponentIdentifier) -> &[ChunkCountInfo] {
        self.per_component
            .get(component)
            .map_or(&[], |v| v.as_slice())
    }
...
impl SortedTemporalChunks {
    pub fn new(
        ...
        native_temporal_map: &dl_log_encoding::RrdManifestTemporalMap,
    ) -> Self {
    ...
    ///
    /// Should be called when a new dlr manifest is appended.
...
mod tests {
    ...
    fn test_component_chunks_returns_empty_slice_for_missing() {
        ...
        let component = dl_chunk::ComponentIdentifier::from("test:Component");
    ...
    fn test_chunks_sorted_by_time() {
        ...
        let timeline = TimelineName::from("test");
        let entity = EntityPath::from("/test");
        let component = dl_chunk::ComponentIdentifier::from("test:Position");

        // Create a temporal map with chunks in non-sorted order
        let mut temporal_map = dl_log_encoding::RrdManifestTemporalMap::default();
        let chunk1 = ChunkId::new();
        let chunk2 = ChunkId::new();
        let chunk3 = ChunkId::new();

        let timeline_obj = dl_chunk::Timeline::new_sequence(timeline);
    ...
    fn test_duplicate_chunks_merged() {
        ...
        let timeline = TimelineName::from("test");
        let entity = EntityPath::from("/test");
        let component1 = dl_chunk::ComponentIdentifier::from("test:Position");
        let component2 = dl_chunk::ComponentIdentifier::from("test:Color");
