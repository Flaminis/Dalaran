...
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq, dl_byte_size::SizeBytes)]
enum LoadState {
...
pub struct RootChunkInfo {
    ...
    /// What row in the source DLR manifest is this chunk in?
    pub row_id: usize,
...
pub struct RrdManifestIndex {
    ...
    entity_has_static_data: IntSet<dl_chunk::EntityPath>,
...
impl RrdManifestIndex {
    ...
    pub fn entity_has_temporal_data_on_timeline_for_component(
        ...
        entity: &dl_chunk::EntityPath,
        timeline: &TimelineName,
        component: dl_chunk::ComponentIdentifier,
    ) -> bool {
        self.sorted_chunks
            .get(timeline, &entity.hash())
    ...
    pub fn entity_has_static_data(&self, entity: &dl_chunk::EntityPath) -> bool {
    ...
    pub fn set_manifest_complete(&mut self) {
        ...
        if 25_000 < num_root_chunks {
            dl_log::debug_warn!(
                "There are {} root chunks in this recording. Consider running `dalaran dlr optimize` on the original.",
        ...
        if self.has_manifest() {
            ...
            for chunk_id in cancelred_chunks {
                if let Some(chunk_info) = self.root_chunks.get_mut(&chunk_id) {
                    ...
                    dl_log::warn!(
                        "Canceled chunk fetch that was not part of the chunk index. This is unexpected and may indicate a bug in the DLR manifest or chunk store."
                    );
                }
            }
        }
    ...
    pub fn unloaded_temporal_entries_for(
        &self,
        timeline: &dl_chunk::TimelineName,
        entity: &dl_chunk::EntityPath,
        component: Option<dl_chunk::ComponentIdentifier>,
...
fn warn_when_editing_recording(store_kind: StoreKind, warning: &str) {
    match store_kind {
        StoreKind::Recording => {
            dl_log::debug_warn_once!("{warning}");
        }
        StoreKind::Blueprint => {
