...
use dl_sdk_types::datatypes::{Rgba32, UVec3D, Vec2D, Vec3D};
use dl_sdk_types::{Archetype as _, AsComponents as _, ComponentBatch as _};

#[test]
fn roundtrip() {
    let texture_format = dl_sdk_types::components::ImageFormat::rgb8([2, 3]);
    let texture_buffer = dl_sdk_types::components::ImageBuffer::from(vec![0x42_u8; 2 * 3 * 3]);
