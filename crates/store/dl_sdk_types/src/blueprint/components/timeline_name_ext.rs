...
// impl From<dl_log_types::TimelineName> for TimelineName {}

// NOTE: fallible, because the blueprint component is a free-form string that can hold
// an empty value, which `dl_log_types::TimelineName` forbids.
impl TryFrom<TimelineName> for dl_log_types::TimelineName {
    type Error = dl_types_core::InvalidTimelineNameError;
...
impl TimelineName {
    /// Create a [`Self`] from a [`dl_log_types::Timeline`].
    pub fn from_timeline(timeline: &dl_log_types::Timeline) -> Self {
        Self::from(timeline.name().as_str())
    }

    /// The log time timeline (`"log_time"`).
    pub fn log_time() -> Self {
        Self::from(dl_log_types::TimelineName::log_time().as_str())
    }
}
