use dl_types_core::{DeserializationResult, Loggable as _};
...
impl Pinhole {
    ...
    ///
    /// Returns [`dl_types_core::DeserializationError::MissingData`] if the component is not present.
    pub fn image_from_camera_from_arrow(&self) -> DeserializationResult<PinholeProjection> {
        self.image_from_camera.as_ref().map_or(
            Err(dl_types_core::DeserializationError::missing_data()),
            |data| {
                let v = PinholeProjection::from_arrow(&data.array)?;
                v.first()
                    .copied()
                    .ok_or_else(dl_types_core::DeserializationError::missing_data)
            },
        )
    }
