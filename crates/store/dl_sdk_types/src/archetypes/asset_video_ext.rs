...
impl AssetVideo {
    ...
    #[cfg(feature = "video")]
    pub fn read_frame_timestamps_nanos(&self) -> Result<Vec<i64>, dl_video::VideoLoadError> {
        ...
        let Some(blob_bytes) = self.blob.as_ref().and_then(Blob::serialized_blob_as_slice) else {
            return Err(dl_video::VideoLoadError::NoVideoTrack); // Error type is close enough
        };
        ...
        let Some(media_type) = self
            ...
            return Err(dl_video::VideoLoadError::UnrecognizedMimeType);
        };

        Ok(dl_video::VideoDataDescription::load_from_bytes(
            blob_bytes,
            media_type.as_str(),
            "AssetVideo",
        )?
        .frame_timestamps_nanos()
        .ok_or(dl_video::VideoLoadError::NoTimescale)?
        .collect())
    }
}
