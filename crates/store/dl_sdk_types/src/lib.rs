...
//! ### Build cache
//!
//! Updating either the source code of the code generator itself (`dl_types_builder`) or any of the
//! .fbs files should re-trigger the code generation process the next time `dl_sdk_types` is built.
...
//!
//! Simply get rid of the type in question and rebuild `dl_sdk_types` to trigger codegen.
//!
...
pub use dl_types_core::DEFAULT_DISPLAY_DECIMALS;
...
pub mod archetypes {
    ...
    // them to be exposed by `dl_types_core` directly; that way we don't force a dependency on the
    // `dl_sdk_types` behemoth just so one can use one of these fundamental types.
    //
    // To do so, re-inject `dl_types_core`'s archetypes into our own module.

    #[path = "../archetypes/mod.rs"]
    mod _archetypes;

    pub use dl_types_core::archetypes::*;
...
pub mod components {
    ...
    // by `dl_types_core` directly; that way we don't force a dependency on the `dl_sdk_types`
    // behemoth just so one can use one of these fundamental types.
    //
    // To do so, re-inject `dl_types_core`'s components into our own module.

    #[path = "../components/mod.rs"]
    mod _components;

    pub use dl_types_core::components::*;
...
pub mod datatypes {
    ...
    // behemoth just so one can use one of these fundamental types.
    //
    // To do so, re-inject `dl_types_core`'s datatypes into our own module.

    #[path = "../datatypes/mod.rs"]
    mod _datatypes;

    pub use dl_types_core::datatypes::*;
...
pub mod reflection {
    ...
    // exposed by `dl_types_core` directly; that way we don't force a dependency on the `dl_sdk_types`
    ...
    pub use dl_types_core::reflection::*;
...
// One almost never uses `dl_sdk_types` without `dl_types_core`, so we reexport these core types
// for convenience.
// The opposite is not true, though: make sure you're not depending on `dl_sdk_types` if all you need
// in `dl_types_core` in order to prevent some nasty dep cycles!
pub use dl_types_core::*;
...
// Has to live here otherwise we can't export it from `dl_sdk_types`.
