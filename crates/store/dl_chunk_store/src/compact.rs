...
pub struct CompactionOptions {
    ...
    /// for the given codec. Use `dl_video::is_start_of_gop` wrapped in a closure.
    ///
    ...
    pub fn finalize_compaction(
        ...
        for pass in 0..num_extra_passes {
            ...
            if num_after >= num_before {
                dl_log::info!(pass, "converged, stopping early");
                break;
            }
        }
        ...
        if let Some(is_start_of_gop) = is_start_of_gop {
            ...
            match crate::rebatch_videos::rebatch_video_chunks_to_gops(
                ...
                *fix_keyframe,
                ...
                Ok(new_store) => {
                    self = new_store;
                    dl_log::info!(time = ?now.elapsed(), "video GoP rebatching completed");
                }
                Err(err) => {
...
mod tests {
    ...
    use dl_log_types::{EntityPath, StoreId, StoreKind, Timeline, example_components::MyPoint};
    use dl_sdk_types::components::Blob;
    use dl_types_core::{ArchetypeName, ComponentDescriptor};
        ...
        let chunk = Chunk::builder(entity.clone())
            .with_component_batches(
                ...
                [
                    (
                        blob_descriptor,
                        &[blob] as &dyn dl_types_core::ComponentBatch,
                    ),
                    (
                        point_descriptor,
