...
pub struct ChunkComponentMeta {
    pub descriptor: dl_sdk_types::ComponentDescriptor,
...
pub struct ChunkMeta {
    pub entity_path: dl_chunk::EntityPath,
...
impl ChunkStoreDiffVirtualAddition {
    ...
    pub fn chunk_metas(&self) -> impl Iterator<Item = ChunkMeta> {
        ...
        > = self
            ...
            .iter()
            .filter(|f| {
                dl_sorbet::ColumnKind::try_from(f.as_ref()).ok()
                    == Some(dl_sorbet::ColumnKind::Component)
            })
            .filter_map(|field| {
                ...
                let descriptor = dl_sdk_types::ComponentDescriptor::try_from((**field).clone())
                    ...
                    .map(|(component, info)| {
                        ...
                        if let Some(meta) = component_schema_info.get(&component) {
                            ...
                            ChunkComponentMeta {
                                has_data,
                                is_static,
                                descriptor: dl_sdk_types::ComponentDescriptor::partial(component),
...
///
/// A clear, on the other hand, is the act of logging an empty [`dl_types_core::ComponentBatch`],
/// either directly using the logging APIs, or indirectly through the use of a
/// [`dl_types_core::archetypes::Clear`] archetype.
#[derive(Clone)]
...
mod tests {
    ...
    use dl_log_types::example_components::{MyColor, MyIndex, MyPoint, MyPoints};
