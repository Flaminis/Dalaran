...
use dl_log_types::{EntityPath, TimeType, Timeline, TimelineName};
...
use dl_types_core::{ArchetypeName, ComponentSet, ComponentType};
...
impl StoreSchema {
    ...
    /// Get the [`dl_types_core::ComponentType`] and [`ArrowDataType`] for a specific [`EntityPath`] and [`ComponentIdentifier`].
    ...
    fn on_chunk_addition(&mut self, chunk: &dl_chunk::Chunk) -> Vec<ChunkComponentMeta> {
        ...
        for column in chunk.components().values() {
            ...
            let is_semantically_empty =
                dl_arrow_util::is_list_array_semantically_empty(&column.list_array);
