...
///
/// `is_start_of_gop` can use `dl_video::is_start_of_gop`.
/// We do dependency injection here in order to avoid a direct dependency on `dl_video`.
...
fn split_into_gop_groups<'a>(
    ...
    if split_points.first().copied() != Some(0) {
        dl_log::warn!(?entity, "first sample is not a keyframe");
...
fn merge_chunks(config: &ChunkStoreConfig, gop_chunks: Vec<Chunk>) -> anyhow::Result<Vec<Chunk>> {
    ...
    if chunk_max_bytes == 0 || gop_chunks.len() <= 1 {
        dl_log::debug!("skipping merge (max_bytes=0 or ≤1 GoP)");
