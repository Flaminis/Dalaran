//! All the APIs used specifically for `dl_dataframe`.

use std::collections::{BTreeMap, BTreeSet};
use std::ops::{Deref, DerefMut};

use arrow::datatypes::DataType as ArrowDatatype;
use dl_chunk::{ComponentIdentifier, LatestAtQuery, RangeQuery, TimelineName};
use dl_log::ResultExt as _;
use dl_log_types::{AbsoluteTimeRange, EntityPath, TimeInt, Timeline};
use dl_sorbet::{
...
///
/// Contents are expressed as a set of [`EntityPath`]s and their associated [`dl_types_core::ComponentIdentifier`]s.
///
...
///
/// * The view contents specify which subset of the database (i.e., which columns) the query runs on,
///   expressed as a set of [`EntityPath`]s and their associated [`dl_types_core::ComponentIdentifier`]s.
///
...
pub struct QueryExpression {
    ...
    /// associated [`dl_types_core::ComponentIdentifier`]s.
    ///
...
impl ChunkStore {
    ...
    pub fn row_id_descriptor(&self) -> dl_sorbet::RowIdColumnDescriptor {
        dl_sorbet::RowIdColumnDescriptor::from_sorted(false)
    }
    ...
    pub fn resolve_time_selector(&self, selector: &TimeColumnSelector) -> IndexColumnDescriptor {
        ...
        let timeline = timelines
            ...
            .copied()
            .unwrap_or_else(|| {
                dl_log::warn_once!("Unknown timeline {selector:?}; assuming sequence timeline.");
