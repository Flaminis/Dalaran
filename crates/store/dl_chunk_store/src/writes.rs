...
use dl_chunk::{Chunk, EntityPath, RowId};
use dl_log::debug_assert;
use dl_log_encoding::{RrdManifest, RrdManifestTemporalMapEntry};
use dl_sdk_types::{Archetype as _, ArchetypeName, archetypes};
...
impl ChunkStore {
    ...
    pub fn insert_chunk(&mut self, chunk: &Arc<Chunk>) -> ChunkStoreResult<Vec<ChunkStoreEvent>> {
        if !self.is_root_chunk(&chunk.id()) {
            dl_log::debug_warn_once!("Attempted to insert non-root chunk (this has no effect)");
            return Ok(vec![]);
        }
    ...
    fn insert_chunk_impl(
        ...
        if let Some(prev_chunk) = self.physical_chunks_per_chunk_id.get(&chunk.id()) {
            if cfg!(debug_assertions) {
                ...
                dl_log::debug_once!("The same chunk was inserted twice (this has no effect)");
            }
        ...
        if !chunk_is_static && !chunk_descends_from_split && !chunk_descends_from_compaction {
            ...
            loop {
                dl_tracing::profile_scope!("compute-recursive-splits");
            ...
            if split_chunks.len() > 1 {
                dl_tracing::profile_scope!("add-splits");
        ...
        let (chunk_after_processing, diffs) = if chunk.is_static() {
            ...
            dl_tracing::profile_scope!("static");
            ...
            if let Some(per_component) = self.static_chunk_ids_per_entity.get(chunk.entity_path()) {
                dl_tracing::profile_scope!("static dangling checks");
            ...
            dl_tracing::profile_scope!("temporal");

            let (elected_chunk, chunk_or_compacted) = {
                dl_tracing::profile_scope!("election");
                ...
                let chunk_or_compacted = if let Some(elected_chunk) = &elected_chunk {
                    ...
                    dl_log::trace!(
                        ...
                        chunk.id(),
                        dl_format::format_uint(chunk.num_rows()),
                        elected_chunk.id(),
                        dl_format::format_uint(elected_chunk.num_rows()),
                        compacted.id(),
                        dl_format::format_uint(compacted.num_rows()),
                    );
            ...
            {
                dl_tracing::profile_scope!("insertion (w/ component)");
            ...
            {
                dl_tracing::profile_scope!("insertion (w/o component)");
    ...
    fn find_and_elect_compaction_candidate(&self, chunk: &Arc<Chunk>) -> Option<Arc<Chunk>> {
        ...
        {
            dl_tracing::profile_scope!("sort_candidates");
    ...
    pub fn drop_entity_path(&mut self, entity_path: &EntityPath) -> Vec<ChunkStoreEvent> {
        dl_tracing::profile_function!(entity_path.to_string());
...
mod tests {
    ...
    use dl_chunk::{TimeInt, TimePoint, Timeline};
    use dl_log_types::example_components::{MyColor, MyLabel, MyPoint, MyPoints};
    use dl_log_types::{build_frame_nr, build_log_time};
    use dl_sdk_types::components::Blob;
    ...
    fn schema_static_transition_from_manifest() -> anyhow::Result<()> {
        ...
        let manifest_temporal = dl_log_encoding::RrdManifest::build_in_memory_from_chunks(
            store_id.clone(),
        ...
        let manifest_static = dl_log_encoding::RrdManifest::build_in_memory_from_chunks(
            store_id,
    ...
    fn compaction_blobs() -> anyhow::Result<()> {
        ...
        let chunk1 = Chunk::builder(entity_path.clone())
            .with_component_batches(
                ...
                [(
                    ...
                    &[blob1.clone()] as &dyn dl_types_core::ComponentBatch,
                )],
            )
            .build()?;
        ...
        let chunk2 = Chunk::builder(entity_path.clone())
            .with_component_batches(
                ...
                [(
                    blob_descriptor.clone(),
                    &[blob2.clone()] as &dyn dl_types_core::ComponentBatch,
                )],
            )
            .with_component_batches(
                row_id3,
                ...
                [(
                    blob_descriptor.clone(),
                    &[blob3.clone()] as &dyn dl_types_core::ComponentBatch,
                )],
            )
            .with_component_batches(
                row_id4,
                ...
                [(
                    blob_descriptor.clone(),
                    &[blob4.clone()] as &dyn dl_types_core::ComponentBatch,
                )],
            )
            .build()?;
        ...
        let chunk3 = Chunk::builder(entity_path.clone())
            .with_component_batches(
                ...
                [(
                    blob_descriptor.clone(),
                    &[blob5.clone()] as &dyn dl_types_core::ComponentBatch,
                )],
            )
            .build()?;
