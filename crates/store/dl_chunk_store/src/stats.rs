...
use dl_byte_size::{MemUsageNode, MemUsageTree, MemUsageTreeCapture, SizeBytes};
use dl_chunk::{Chunk, ComponentIdentifier, EntityPath, TimelineName};
...
#[derive(Default, Debug, Clone, Copy, dl_byte_size::SizeBytes)]
...
impl std::fmt::Display for ChunkStoreChunkStats {
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        f.write_fmt(format_args!(
            "num_rows: {}\n",
            dl_format::format_uint(num_rows)
        ))?;
        f.write_fmt(format_args!(
            "num_events: {}\n",
            dl_format::format_uint(num_events)
        ))?;
        ...
        self.static_chunk_ids_per_entity
            ...
            .map_or_else(
                ...
                |static_chunks_per_component| {
                    let chunk_ids: ahash::HashSet<dl_chunk::ChunkId> =
...
impl SizeBytes for ChunkStore {
    fn heap_size_bytes(&self) -> u64 {
        ...
        let chunks_size = {
            dl_tracing::profile_scope!("chunks");
            physical_chunks_per_chunk_id
                .iter()
                .map(|(chunk_id, chunk)| {
...
impl MemUsageTreeCapture for ChunkStore {
    fn capture_mem_usage_tree(&self) -> MemUsageTree {
        ...
        {
            dl_tracing::profile_scope!("per-entity-physical-chunks-stats");
