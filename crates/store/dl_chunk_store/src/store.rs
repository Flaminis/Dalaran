...
#[derive(Debug, Default, Clone, dl_byte_size::SizeBytes)]
pub struct ChunkIdSetPerTime {
    ...
    ///   contains data for any component on this particular timeline (see [`dl_chunk::TimeColumn::time_range`]).
    ///
...
pub struct ColumnMetadata {
    ...
    ///
    /// `Clear`: [`dl_types_core::archetypes::Clear`]
...
pub struct ChunkStore {
    ...
    /// [`dl_types_core::ComponentType`] index.
    ///
    /// This index is purely additive: it is never affected by garbage collection in any way.
    /// This implies that the chunk IDs present in this set might be either physical/loaded or
    /// virtual/offloaded.
...
impl ChunkStore {
    ...
    /// Lookup the [`ColumnMetadata`] for a specific [`EntityPath`] and [`dl_types_core::Component`].
    pub fn lookup_column_metadata(
        ...
        let is_tombstone = dl_types_core::archetypes::Clear::all_components()
            .iter()
...
impl ChunkStore {
    ...
    pub fn from_log_msgs(
        store_config: &ChunkStoreConfig,
        log_msgs: impl IntoIterator<Item = dl_log_types::LogMsg>,
    ) -> anyhow::Result<BTreeMap<StoreId, Self>> {
        dl_tracing::profile_function!();
