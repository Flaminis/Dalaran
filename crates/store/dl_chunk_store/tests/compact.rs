...
use dl_chunk_store::{ChunkStore, ChunkStoreConfig, CompactionOptions, IsStartOfGop};
...
fn fragmented_store() -> ChunkStore {
    let store_id = dl_log_types::StoreId::random(dl_log_types::StoreKind::Recording, "test_app");
...
fn synthetic_video_store(entity: &str, num_samples: i64) -> ChunkStore {
    let store_id = dl_log_types::StoreId::random(dl_log_types::StoreKind::Recording, "video_test");
    let mut store = ChunkStore::new(store_id, ChunkStoreConfig::ALL_DISABLED);
...
fn chunk_has_component(chunk: &dl_chunk::ChunkShared, component: &str) -> bool {
    chunk
        .components()
        .values()
...
fn with_captured_warnings<R>(f: impl FnOnce() -> R) -> (R, Vec<String>) {
    ...
    let rx = dl_log::add_log_msg_receiver(dl_log::LevelFilter::WARN);
    let result = f();
    let warnings = rx
        .try_iter()
        .filter(|msg| msg.level == dl_log::Level::WARN)
        .map(|msg| {
            let fields = msg
                .fields
                .iter()
...
fn optimize_breaks_out_keyframes_sharing_chunk_with_other_components() {
    ...
    let mixed_chunk = Chunk::builder(entity_path.clone())
        .with_component_batches(
            ...
            [
                (
                    VideoStream::descriptor_is_keyframe(),
                    &kf as &dyn dl_types_core::ComponentBatch,
                ),
                (
                    MyPoints::descriptor_points(),
                    &[point] as &dyn dl_types_core::ComponentBatch,
                ),
            ],
        )
        .build()
    ...
    let mixed_chunk2 = Chunk::builder(entity_path.clone())
        .with_component_batches(
            ...
            [(
                VideoStream::descriptor_is_keyframe(),
                &kf2 as &dyn dl_types_core::ComponentBatch,
            )],
        )
        .build()
