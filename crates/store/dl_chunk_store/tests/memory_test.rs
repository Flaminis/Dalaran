...
use dl_chunk::external::crossbeam::channel::TryRecvError;
use dl_chunk::{BatcherHooks, ChunkBatcher, ChunkBatcherConfig, PendingRow};
use dl_chunk_store::{ChunkStore, ChunkStoreConfig};
use dl_log_types::{TimePoint, Timeline};
use dl_sdk_types::components::Scalar;
use dl_sdk_types::{Loggable as _, SerializedComponentBatch, archetypes};
...
fn scalar_memory_overhead() {
    ...
    dl_log::warn!(
        "THIS TEST HAS TO ACCOUNT FOR THE MEMORY OF ALL RUNNING THREADS -- IT MUST BE RUN ON ITS OWN, WITH NO OTHER TESTS RUNNING IN PARALLEL: `cargo t --all-features -p dl_chunk_store memory_tests -- scalar_memory_overhead`"
    );
    ...
    let (total_mem_use_global, _total_mem_use_local) = memory_use(|| {
        ...
        for i in 0..NUM_SCALARS {
            let entity_path = dl_log_types::entity_path!("scalar");
        ...
        loop {
            ...
            //     "chunk with {} rows: {}",
            //     chunk.num_rows(),
            //     dl_format::format_bytes(chunk.total_size_bytes() as _)
            // );
    ...
    insta::assert_debug_snapshot!(
        ...
        [
            ...
            format!(
                "{} per row",
                dl_format::format_bytes(total_mem_use_global as f64 / NUM_SCALARS as f64)
            ),
        ]
    );
}
