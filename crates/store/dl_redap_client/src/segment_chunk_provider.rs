...
use dl_log_encoding::{ChunkProvider, ChunkProviderError, RawRrdManifest, RrdManifest};
...
impl ChunkProvider for SegmentChunkProvider {
    ...
    async fn load_chunks(&self, ids: &[ChunkId]) -> Result<Vec<Arc<Chunk>>, ChunkProviderError> {
        ...
        let rb = dl_arrow_util::take_record_batch(self.manifest.chunk_fetcher_rb(), &row_indices)
