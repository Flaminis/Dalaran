...
use dl_auth::Jwt;
use dl_auth::credentials::CredentialsProviderError;
use dl_protos::cloud::v1alpha1::{EntryFilter, FindEntriesRequest, WhoAmIRequest};
use dl_uri::Origin;
...
pub struct ConnectionRegistry {
    ...
    saved_credentials: HashMap<dl_uri::Origin, Credentials>,
    ...
    remote_connections: HashMap<dl_uri::Origin, Connection>,

    /// Latest errors received for specific redap URIs.
    uri_errors: HashMap<dl_uri::DatasetSegmentUri, String>,
}
...
impl ConnectionRegistry {
    ...
    pub fn clear_uri_error(&mut self, uri: dl_uri::DatasetSegmentUri) {
        self.uri_errors.remove(&uri.without_fragment());
    }

    /// Set the latest error for this URI.
    pub fn set_uri_error(&mut self, uri: dl_uri::DatasetSegmentUri, error: String) {
        self.uri_errors.insert(uri.without_fragment(), error);
    }

    /// Get the latest error for this URI.
    pub fn error_for_uri(&self, uri: dl_uri::DatasetSegmentUri) -> Option<&str> {
        self.uri_errors
...
pub enum ClientCredentialsError {
    ...
    #[error("{0}")]
    HostMismatch(dl_auth::HostMismatchError),
...
pub struct ConnectionRegistryHandle {
    ...
    /// lock.
    internal: Arc<OnceLock<(dl_uri::Origin, Connection)>>,
...
impl Credentials {
    ...
    pub fn has_write_permission(&self) -> Option<bool> {
        match self {
            Self::Token(jwt) => {
                let claims: dl_auth::Claims = jwt.decode_claims().ok()?;
...
impl ConnectionRegistryHandle {
    pub fn set_credentials(&self, origin: &dl_uri::Origin, credentials: Credentials) {
        wrap_blocking_lock(|| {
    ...
    pub fn credentials(&self, origin: &dl_uri::Origin) -> Option<Credentials> {
        wrap_blocking_lock(|| {
    ...
    pub fn remove_credentials(&self, origin: &dl_uri::Origin) {
        wrap_blocking_lock(|| {
            let mut inner = self.inner.blocking_write();
    ...
    pub fn with_internal(self, internal: (dl_uri::Origin, Connection)) -> Self {
        self.set_internal(internal);
        self
    }

    pub fn set_internal(&self, internal: (dl_uri::Origin, Connection)) {
    ...
    pub fn internal_origin(&self) -> Option<dl_uri::Origin> {
        self.internal
            .get()
            .map(|(origin, _connection)| origin.clone())
    }

    pub fn is_internal_origin(&self, origin: &dl_uri::Origin) -> bool {
        self.internal
            .get()
    ...
    /// Clear the latest error for this URI.
    pub fn clear_uri_error(&self, uri: dl_uri::DatasetSegmentUri) {
        wrap_blocking_lock(|| {
    ...
    /// Set the latest error for this URI.
    pub fn set_uri_error(&self, uri: dl_uri::DatasetSegmentUri, error: String) {
        wrap_blocking_lock(|| {
    ...
    /// Get the latest error for this URI.
    pub fn error_for_uri(&self, uri: dl_uri::DatasetSegmentUri) -> Option<String> {
        wrap_blocking_lock(|| {
            self.inner
                .blocking_read()
                .error_for_uri(uri)
    ...
    pub async fn connection(&self, origin: dl_uri::Origin) -> ApiResult<Connection> {
    ...
    #[tracing::instrument(level = "info", skip_all)]
    pub async fn client(&self, origin: dl_uri::Origin) -> ApiResult<ConnectionClient> {
    ...
    async fn connect_and_validate_client_stack(
        ...
        let provider: Option<Arc<dyn dl_auth::credentials::CredentialsProvider + Send + Sync>> =
            match credentials.as_ref().map(|c| &c.credentials) {
                ...
                Some(Credentials::Stored) => {
                    ...
                    Some(Arc::new(dl_auth::credentials::CliCredentialsProvider::new()))
                }
                None => None,
            };
        ...
        let request_result = match request_result {
            ...
            Ok(resp) => {
                ...
                let dl_protos::cloud::v1alpha1::WhoAmIResponse {
                    user_id,
                    can_read,
                    can_write,
                } = resp.into_inner();
        ...
        match request_result {
            ...
            Err(err) => {
                if let Some(cred_error) = err.source().and_then(|s| {
                    s.downcast_ref::<dl_auth::credentials::CredentialsProviderError>()
                }) {
                    match cred_error {
                    ...
                    match token {
                        ...
                        SerializedCredentials::Jwt(token) => {
                            if let Ok(jwt) = Jwt::try_from(token) {
                                ...
                                dl_log::debug!("Failed to parse token for origin {origin}");
                            }
                        }
                    }
                } else {
                    dl_log::trace!("Ignoring token for origin {origin} as it is already set");
                }
            }
...
pub struct SerializedTokens {
    tokens_per_origin: Vec<(dl_uri::Origin, SerializedCredentials)>,
    ...
    std::env::var("REDAP_TOKEN")
        .map_err(|err| match err {
            ...
            std::env::VarError::NotUnicode(..) => {
                dl_log::warn_once!("REDAP_TOKEN env var is malformed: {err}");
            }
        })
        .and_then(|t| {
            dl_auth::Jwt::try_from(t).map_err(|err| {
                dl_log::warn_once!(
