...
impl ApiError {
    ...
    /// wrong (404 from a non-Dalaran HTTP server), the server is down (5xx), or they hit a
    ...
    pub fn invalid_server_with_response(
        ...
        let mut msg = format!(
            "{origin} is not a valid Dalaran server (GET /version returned HTTP {status} {status_text})"
        );
