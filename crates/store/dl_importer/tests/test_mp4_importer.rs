...
mod tests {
    ...
    use dl_chunk::Chunk;
    use dl_chunk_store::{ChunkStore, ChunkStoreConfig, ChunkStoreHandle};
    use dl_importer::{ArchetypeImporter, ImportedData, Importer as _, ImporterSettings};
    use dl_log_types::StoreId;

    /// Resolve a path under the workspace-root `tests/assets/video/` directory.
    ///
    /// Mirrors the pattern at `crates/utils/dl_video/src/encode/ffmpeg_cli.rs`
    /// where `env!("CARGO_MANIFEST_DIR").ancestors().nth(3)` walks
    /// `crates/store/dl_importer → crates/store → crates → repo-root`.
    fn fixture(name: &str) -> std::path::PathBuf {
        std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .ancestors()
            .nth(3)
            .expect("workspace root is three ancestors up from crates/store/dl_importer")
    ...
    /// `dl_mp4_reader` is a lenient producer (it emits the asset chunk and skips
    ...
    fn test_mp4_importer_unreadable_video_logs_asset_only() {
        use dl_sdk_types::archetypes::{AssetVideo, VideoFrameReference};
    ...
    fn test_mp4_importer_h264_nobframes() {
        ...
        let store = ChunkStore::new(
            StoreId::random(dl_log_types::StoreKind::Recording, "test_mp4_importer"),
