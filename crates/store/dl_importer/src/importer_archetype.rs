...
use dl_log_types::{ApplicationId, EntityPath, TimePoint};

use crate::{ImportedData, Importer, ImporterError};

// ---

/// Imports data from any supported file or in-memory contents as native [`dl_sdk_types::Archetype`]s.
///
...
impl Importer for ArchetypeImporter {
    ...
    fn import_from_path(
        ...
        let contents = {
            dl_tracing::profile_scope!("fs::read");
    ...
    fn import_from_file_contents(
        ...
        let store_id = settings.opened_store_id.clone().unwrap_or_else(|| {
            dl_log_types::StoreId::recording(
                settings
                    .application_id
                    .clone()
        ...
        let chunks_sent = if crate::SUPPORTED_IMAGE_EXTENSIONS.contains(&extension.as_str()) {
            dl_log::debug!(?filepath, importer = self.name(), "Loading image…",);
            ...
            dl_log::debug!(?filepath, importer = self.name(), "Loading depth image…",);
            ...
            dl_log::debug!(?filepath, importer = self.name(), "Loading video…",);
            ...
            dl_log::debug!(?filepath, importer = self.name(), "Loading 3D model…",);
            load_mesh(
                filepath.clone(),
                timepoint,
                entity_path,
            ...
            dl_log::debug!(?filepath, importer = self.name(), "Loading 3D point cloud…",);
            ...
            dl_log::debug!(?filepath, importer = self.name(), "Loading text document…",);
            load_text_document(
                filepath.clone(),
                timepoint,
                entity_path,
        ...
        if num_chunks == 0 {
            dl_log::warn!("{} is empty", filepath.display());
        }
...
fn load_image(
    ...
    let rows = [
        {
            let mut arch = dl_sdk_types::archetypes::EncodedImage::from_file_contents(contents);
...
fn load_depth_image(
    ...
    let rows = [{
        let mut arch = dl_sdk_types::archetypes::EncodedDepthImage::from_file_contents(contents);
        ...
        {
            arch = arch.with_media_type(dl_sdk_types::components::MediaType::RVL);
        }
...
fn load_video(
    ...
    let video_timeline = dl_log_types::Timeline::new_duration("video");
    timepoint.insert_cell(
        *video_timeline.name(),
        dl_log_types::TimeCell::ZERO_DURATION,
    );

    let config = dl_mp4_reader::Mp4Config {
        mode: dl_mp4_reader::Mode::Asset { timepoint },
        timeline_name: "video".into(),
        timeline_type: dl_log_types::TimeType::DurationNs,
    };

    // An up-front failure (e.g. the asset being too large) aborts the import.
    let debug_name = filepath.display().to_string();
    let chunks = dl_mp4_reader::load_mp4_from_bytes(contents, &config, entity_path, &debug_name)
    ...
    // (Unreadable frame timestamps are not an error at all — `dl_mp4_reader`
    ...
    Ok(chunks.map_while(move |chunk| match chunk {
        Ok(chunk) => Some(chunk),
        Err(err) => {
            dl_log::warn!(?filepath, "Failed to load chunk from video: {err}");
            None
        }
    }))
}
...
fn load_mesh(
    ...
    let rows = [
        {
            let arch = dl_sdk_types::archetypes::Asset3D::from_file_contents(
                contents,
                dl_sdk_types::components::MediaType::guess_from_path(filepath),
            );
            Chunk::builder(entity_path)
                .with_archetype(RowId::new(), timepoint, &arch)
                .build()?
    ...
    let rows = [
        {
            ...
            if dl_sdk_types::archetypes::GaussianSplats3D::is_gaussian_splat_ply(contents) {
                let gaussians = dl_sdk_types::archetypes::GaussianSplats3D::from_ply_file_contents(
                    contents,
                    Some(filepath),
                )
                ...
                let points3d = dl_sdk_types::archetypes::Points3D::from_file_contents(contents)
                    .map_err(anyhow::Error::from)?;
...
fn load_text_document(
    ...
    let rows = [
        {
            let arch = dl_sdk_types::archetypes::TextDocument::from_file_contents(
                contents,
                dl_sdk_types::components::MediaType::guess_from_path(filepath),
            )
            .map_err(anyhow::Error::from)?;
