...
use dl_chunk::{Chunk, ChunkBuilder, ChunkId, EntityPath, RowId, TimePoint};
use dl_sdk_types::archetypes::{Asset3D, CoordinateFrame, InstancePoses3D, Transform3D};
use dl_sdk_types::components::Color;
use dl_sdk_types::datatypes::{Rgba32, Vec3D};
use dl_sdk_types::external::glam;
use dl_sdk_types::{AsComponents, Component as _, ComponentDescriptor};
    ...
    fn import_from_path(
        ...
        let mut emit = |chunk| {
            if send_error.is_none() {
                send_error = dl_quota_channel::send_crossbeam(
                    &tx,
...
fn walk_tree(
    ...
    dl_log::debug_assert_eq!(link_name, link.name);
                ...
                ChunkBuilder::new(ChunkId::new(), collision_entity_path).with_component_batch(
                    ...
                    (
                        ComponentDescriptor {
                            ...
                            component_type: Some(dl_sdk_types::components::Visible::name()),
                        },
                        &dl_sdk_types::components::Visible::from(false),
                    ),
                ),
            )?;
        }
...
fn fetch_http_resource(url: &str) -> anyhow::Result<Vec<u8>> {
    dl_log::debug!("Fetching mesh resource from {url}");
    ...
    let bytes = response
        ...
        .with_context(|| {
            format!(
                ...
                dl_format::format_bytes(MAX_MESH_DOWNLOAD_SIZE as f64)
            )
        })?;

    dl_log::debug!(
        "Fetched {} from {url}",
        dl_format::format_bytes(bytes.len() as f64)
    );
    ...
    match geometry {
        Geometry::Mesh { filename, scale: _ } => {
            ...
            if let Some(material) = material {
                ...
                if texture.is_some() {
                    dl_log::warn_once!("Material texture not supported"); // TODO(emilk): support textures
                }
            }
        ...
        Geometry::Box {
            ...
            emit_archetype(
                ...
                entity_path,
                timepoint,
                &dl_sdk_types::archetypes::Boxes3D::from_sizes([Vec3D::new(
        ...
        Geometry::Cylinder { radius, length } => {
            ...
            emit_archetype(
                ...
                entity_path,
                timepoint,
                &dl_sdk_types::archetypes::Cylinders3D::from_lengths_and_radii(
                    [*length as f32],
                    [*radius as f32],
                )
                .with_colors([material_color(material)]),
        ...
        Geometry::Capsule { radius, length } => {
            ...
            emit_archetype(
                ...
                entity_path,
                timepoint,
                &dl_sdk_types::archetypes::Capsules3D::from_lengths_and_radii(
                    [*length as f32],
                    [*radius as f32],
                )
                .with_colors([material_color(material)]),
        ...
        Geometry::Sphere { radius } => {
            emit_archetype(
                ...
                entity_path,
                timepoint,
                &dl_sdk_types::archetypes::Ellipsoids3D::from_radii([*radius as f32])
                    .with_colors([material_color(material)]),
            )?;
        }
    }
