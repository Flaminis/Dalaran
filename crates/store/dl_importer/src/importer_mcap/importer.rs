...
use dl_chunk::RowId;
use dl_lenses::Lenses;
use dl_log_types::{SetStoreInfo, StoreId, StoreInfo};
use dl_mcap::{DecoderIdentifier, DecoderRegistry, SelectedDecoders, TopicFilter};
...
pub struct McapImporter {
    ...
    recover: bool,
    lenses_by_time_type: HashMap<dl_log_types::TimeType, Arc<Lenses>>,
}
...
impl McapImporter {
    ...
    /// supplied [`dl_mcap::McapFile`].
    pub fn with_recover(mut self, recover: bool) -> Self {
        self.recover = recover;
        self
    }

    /// Returns the cached lenses for the given [`dl_log_types::TimeType`].
    fn lenses_for(&self, time_type: dl_log_types::TimeType) -> Option<Arc<Lenses>> {
        if time_type == dl_log_types::TimeType::Sequence {
            dl_log::error_once!("Sequence is not a supported timeline type for MCAP lenses");
            return None;
        }
    ...
    fn build_lenses(
        selected_decoders: &SelectedDecoders,
        time_type: dl_log_types::TimeType,
    ) -> Option<Arc<Lenses>> {
    ...
    /// Load chunks from an [`dl_mcap::McapFile`], calling `emit_chunk` for each produced chunk.
    ///
    ...
    pub fn emit_chunks<BytesSource>(
        &self,
        mcap_file: &dl_mcap::McapFile<BytesSource>,
        timeline_type: dl_log_types::TimeType,
        timestamp_offset_ns: Option<i64>,
        emit_chunk: &(dyn Fn(dl_chunk::Chunk) + Send + Sync),
    ) -> Result<(), ImporterError>
    where
        BytesSource: AsRef<[u8]>,
    {
        // Tag the scope with the time range so each window of a windowed read is a distinct span.
        dl_tracing::profile_function!(match self.time_range {
        ...
        let emit_final_chunk = |chunk: dl_chunk::Chunk| {
        ...
        let on_chunk_with_transforms = |chunk: dl_chunk::Chunk| {
            if let Some(ref lenses) = lenses {
                for result in lenses.apply(&chunk, &dl_lenses::default_runtime()) {
                    match result {
                        Ok(chunk) => emit_final_chunk(chunk),
                        Err(partial) => {
                            for error in partial.errors() {
                                dl_log::error_once!("Lens error: {error}");
                            }
        ...
        {
            dl_log::warn_once!("Failed to extract URDF from robot_description topics: {err}");
        }
...
impl Importer for McapImporter {
        ...
        std::thread::Builder::new()
            ...
            .spawn(move || {
                let file = match std::fs::File::open(&path) {
                    Ok(f) => f,
                    Err(err) => {
                        dl_log::error!("Failed to open MCAP file: {err}");
                        return;
                    }
                };
                ...
                let mmap = match unsafe { memmap2::Mmap::map(&file) } {
                    Ok(m) => m,
                    Err(err) => {
                        dl_log::error!("Failed to mmap MCAP file: {err}");
                        return;
                    }
                };
                let mcap_file = dl_mcap::McapFile::new(mmap, loader.recover);

                if let Err(err) = loader.load_and_send(&mcap_file, &settings, &tx) {
                    dl_log::error!("Failed to load MCAP file: {err}");
                }
            })
    ...
    fn import_from_file_contents(
        ...
        cfg_select! {
            target_arch = "wasm32" => {
                let mcap_file = dl_mcap::McapFile::new(contents, loader.recover);
        ...
        BytesSource: AsRef<[u8]>,
    {
        ...
        if send_crossbeam(
            ...
            ImportedData::LogMsg(
                ...
                dl_log_types::LogMsg::SetStoreInfo(store_info(store_id.clone())),
            ),
        )
        .is_err()
        {
...
fn apply_timestamp_offset(mut chunk: dl_chunk::Chunk, offset_ns: Option<i64>) -> dl_chunk::Chunk {
    if let Some(offset_ns) = offset_ns {
        let offset_timelines: Vec<_> = chunk
            .timelines()
            .values()
            .filter(|time_col| time_col.timeline().typ() == dl_log_types::TimeType::TimestampNs)
...
fn send_chunk_to_channel(tx: &Sender<ImportedData>, store_id: &StoreId, chunk: dl_chunk::Chunk) {
    if send_crossbeam(
        tx,
...
fn store_info(store_id: StoreId) -> SetStoreInfo {
    SetStoreInfo {
        ...
        info: StoreInfo::new(
            store_id,
            dl_log_types::StoreSource::Other(MCAP_IMPORTER_NAME.to_owned()),
        ),
    }
}
