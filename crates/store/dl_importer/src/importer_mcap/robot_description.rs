...
///
/// TODO(michael): this could be implemented as an `dl_mcap` decoder if the
/// core URDF parsing logic is moved to a separate crate outside of `dl_importer`.
pub(crate) fn extract_urdf_from_robot_descriptions(
    mcap_bytes: &[u8],
    summary: &mcap::Summary,
    topic_filter: &dl_mcap::TopicFilter,
    recover: bool,
    emit: &(dyn Fn(dl_chunk::Chunk) + Send + Sync),
) -> anyhow::Result<()> {
    ...
    for msg in messages {
        let msg = match msg {
            Ok(msg) => msg,
            Err(err) if recover => {
                dl_log::warn!("Stopping URDF scan at a truncated/corrupt MCAP tail: {err}");
                break;
            }
            Err(err) => return Err(err.into()),
        };
        if robot_desc_channels.contains(&msg.channel.id)
            && let Ok(decoded) = dl_mcap::cdr::try_decode_message::<RosString>(&msg.data)
        {
    ...
    for urdf_xml in urdf_by_channel.into_values() {
        match crate::importer_urdf::build_urdf_chunks_from_xml(
            ...
            Err(err) => {
                dl_log::warn_once!("Failed to parse URDF from robot_description topic: {err}");
            }
        }
    }
...
mod tests {
    ...
    fn recover_urdf_survives_truncated_tail() {
        ...
        let summary = dl_mcap::read_or_reconstruct_summary(truncated, true).expect("reconstruct");

        let chunks = parking_lot::Mutex::new(Vec::new());
        super::extract_urdf_from_robot_descriptions(
            truncated,
            &summary,
            &dl_mcap::TopicFilter::default(),
            true, // recover
