...
use dl_log_types::{ApplicationId, StoreId};
...
pub fn prepare_episode_chunks(
    ...
    for episode in episodes {
        ...
        let set_store_info = ImportedData::LogMsg(
            ...
            prepare_store_info(&store_id, dl_log_types::FileSource::Sdk),
        );
...
pub fn load_and_stream_common<Dataset>(
    ...
    for (episode, store_id) in store_ids {
        ...
        match load_episode(dataset, *episode) {
            Ok(chunks) => {
                let recording_info = dl_sdk_types::archetypes::RecordingInfo::new()
...
pub fn load_episode_depth_images(
    ...
    for frame_idx in 0..image_bytes.len() {
        let img_buffer = image_bytes.value(frame_idx);
        let depth_image =
            dl_sdk_types::archetypes::DepthImage::from_file_contents(img_buffer.to_owned())
        ...
        Chunk::builder(entity_path.clone())
            .with_row(
                ...
                std::iter::once((
                    ...
                    Arc::new(StringArray::from_iter(names)) as Arc<dyn dl_chunk::ArrowArray>,
                )),
            )
            .build()?,
    ))
...
fn make_scalar_entity_chunk(
    ...
    let data_field_array: arrow::array::ListArray =
        dl_arrow_util::arrays_to_list_array(data_field_inner.data_type().clone(), &data_arrays)
            .unwrap();
...
mod tests {
    ...
    use dl_sdk_types::archetypes::TextDocument;
    ...
    fn streams_each_episode_to_its_own_recording() {
        ...
        let store_info_ids = imported
            ...
            .filter_map(|data| match data {
                ImportedData::LogMsg(_, dl_log_types::LogMsg::SetStoreInfo(store_info)) => {
