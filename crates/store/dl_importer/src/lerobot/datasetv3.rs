...
use dl_chunk::{Chunk, EntityPath, RowId, TimeColumn, TimePoint, Timeline};
use dl_log_types::ApplicationId;
use dl_sdk_types::archetypes::{TextDocument, VideoStream};
...
impl LeRobotDatasetV3 {
    ...
    fn load_all_episode_data_files(&self) -> Result<(), LeRobotError> {
        dl_tracing::profile_scope!("load_all_episode_data_files");
    ...
    fn load_episode(&self, episode: EpisodeIndex) -> Result<Vec<Chunk>, ImporterError> {
        ...
        {
            let timeline = dl_log_types::Timeline::new_sequence("frame_index");
            ...
            (
                timeline,
                dl_chunk::TimeColumn::new(None, timeline, times.clone()),
            )
        } else if let Some(timestamps) = data.column_by_name("timestamp") {
            let timeline = dl_log_types::Timeline::new_duration("timestamp");
            let times: arrow::buffer::ScalarBuffer<i64> = timestamps
                ...
                .values()
                .iter()
                .map(|t| dl_log_types::Duration::from_secs(*t).as_nanos())
                .collect();
            (timeline, dl_chunk::TimeColumn::new(None, timeline, times))
        } else {
            return Err(
    ...
    fn load_episode_video(
        ...
        let start_video_time = dl_video::Time::from_secs(start_time, timescale);
        let end_video_time = dl_video::Time::from_secs(end_time, timescale);
        ...
        let codec =
            dl_sdk_types::components::VideoCodec::try_from(video.codec.clone()).map_err(|err| {
                anyhow!(
...
impl LeRobotEpisodeData {
    fn load_from_directory(metadir: impl AsRef<Path>) -> Result<Vec<Self>, LeRobotError> {
        ...
        for entry in std::fs::read_dir(metadir).map_err(|err| LeRobotError::io(err, metadir))? {
            ...
            dl_log::trace!("Loading episode metadata from: {path:?}");
...
mod tests {
    ...
    ///   `LEROBOT_LANG_PARQUET=/path/to/file-000.parquet cargo test -p dl_importer --all-features
