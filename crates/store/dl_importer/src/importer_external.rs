...
pub static EXTERNAL_IMPORTER_PATHS: LazyLock<Vec<PathBuf>> = LazyLock::new(|| {
    dl_tracing::profile_scope!("initialize-external-importers");
    ...
    for dirpath in dirpaths {
        dl_tracing::profile_scope!("dir", dirpath.to_string_lossy());
    ...
    executables
        ...
        .filter_map(|(name, paths)| {
            if paths.len() > 1 {
                dl_log::debug!(name, ?paths, "Found duplicated external importer in $PATH");
            }
...
pub fn iter_external_importers() -> impl ExactSizeIterator<Item = PathBuf> {
    dl_tracing::profile_wait!("EXTERNAL_IMPORTER_PATHS");
        ...
        for exe in external_importers {
            ...
            _ = std::thread::Builder::new().name(exe.to_string_lossy().to_string()).spawn(move || {
                ...
                let mut child = match child {
                    Ok(child) => child,
                    Err(err) => {
                        dl_log::error!(?filepath, importer = ?exe, %err, "Failed to execute external importer");
                        return;
                    }
                };
                ...
                let Some(stdout) = child.stdout.take() else {
                    ...
                    dl_log::error!(?filepath, importer = ?exe, %reason, "Failed to execute external importer");
                    return;
                };
                ...
                dl_log::debug!(?filepath, importer = ?exe, "Importing data from filesystem using external importer…",);
                ...
                match dl_log_encoding::Decoder::decode_eager(stdout) {
                    Ok(decoder) => {
                        let filepath = filepath.clone();
                        let tx = tx.clone();
                        ...
                        {
                            dl_log::error!(?filepath, importer = ?exe, %err, "Failed to open spawn IO thread");
                            return;
                        }
                    }
                    Err(dl_log_encoding::DecodeError::Read(_)) => {
                    ...
                    Err(err) => {
                        dl_log::error!(?filepath, importer = ?exe, %err, "Failed to decode external importer's output");
                        return;
                    }
                }
                ...
                loop {
                    dl_tracing::profile_scope!("waiting for compatibility");
                    ...
                    match child.try_wait() {
                        ...
                        Ok(None) => {
                            if is_sending_data.load(std::sync::atomic::Ordering::Relaxed) {
                                ...
                                // Let's get out ASAP.
                                dl_log::debug!(importer = ?exe, ?filepath, "compatible external importer found");
                                dl_quota_channel::send_crossbeam(&tx_feedback, CompatibleImporterFound).ok();
                ...
                if is_compatible {
                    ...
                    if status.success() {
                        ...
                        dl_log::debug!("External importer stderr:\n{stderr_indented}");

                        dl_quota_channel::send_crossbeam(&tx_feedback, CompatibleImporterFound).ok();
                    } else {
                        dl_log::error!(?filepath, importer = ?exe, %stderr_str, "Failed to execute external importer");
                    }
                }
            })?;
        }

        dl_tracing::profile_wait!("compatible_importer");
