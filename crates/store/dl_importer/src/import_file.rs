...
use dl_log_channel::LogSender;
use dl_log_types::{ApplicationId, FileSource, LogMsg};
...
pub fn import_from_path(
    ...
    dl_tracing::profile_function!(path.to_string_lossy());
...
pub fn import_from_file_contents(
    ...
    dl_tracing::profile_function!(filepath.to_string_lossy());

    dl_log::info!("Loading {filepath:?}…");
...
/// Prepares an adequate [`dl_log_types::StoreInfo`] [`LogMsg`] given the input.
pub fn prepare_store_info(store_id: &dl_log_types::StoreId, file_source: FileSource) -> LogMsg {
    dl_tracing::profile_function!();

    use dl_log_types::SetStoreInfo;

    let store_source = dl_log_types::StoreSource::File { file_source };

    LogMsg::SetStoreInfo(SetStoreInfo {
        row_id: *dl_chunk::RowId::new(),
        info: dl_log_types::StoreInfo::new(store_id.clone(), store_source),
    })
}
...
pub(crate) fn import(
    ...
    dl_tracing::profile_function!(path.display().to_string());
    ...
    let rx_importer = {
        ...
        let any_compatible_importer = {
            ...
            for importer in importers {
                ...
                rayon::spawn(move || {
                    dl_tracing::profile_scope!("inner", importer.name());
                    ...
                    if let Some(contents) = contents.as_deref() {
                        ...
                        if let Err(err) = importer.import_from_file_contents(
                            ...
                            dl_log::error!(?path, importer = importer.name(), %err, "Failed to import data");
                        }
                    } else if let Err(err) =
                        importer.import_from_path(&settings, path.clone(), tx_importer)
                    {
                        ...
                        dl_log::error!(?path, importer = importer.name(), %err, "Failed to import data from file");
                    }
...
pub(crate) fn send(
    ...
    spawn({
        ...
        let mut store_info_tracker: HashMap<dl_log_types::StoreId, Tracked> = HashMap::new();
        ...
        move || {
            ...
            for data in rx_importer {
                ...
                let msg = match data.into_log_msg() {
                    ...
                    Err(err) => {
                        dl_log::error!(%err, "Couldn't serialize component data");
                        continue;
                    }
                };
                tx.send(msg.into()).ok();
