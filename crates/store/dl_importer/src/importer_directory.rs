...
impl crate::Importer for DirectoryImporter {
    ...
    fn import_from_path(
        ...
        dl_tracing::profile_function!(dirpath.display().to_string());

        dl_log::debug!(?dirpath, importer = self.name(), "Loading directory…",);

        for entry in walkdir::WalkDir::new(&dirpath) {
            let entry = match entry {
                Ok(entry) => entry,
                Err(err) => {
                    dl_log::error!(importer = self.name(), ?dirpath, %err, "Failed to open filesystem entry");
                    continue;
                }
            };
                ...
                _ = std::thread::Builder::new()
                    ...
                    .spawn(move || {
                        let data = match crate::import_file::import(&settings, &filepath, None) {
                            Ok(data) => data,
                            Err(err) => {
                                dl_log::error!(?filepath, %err, "Failed to load directory entry");
                                return;
                            }
                        };

                        for datum in data {
                            if dl_quota_channel::send_crossbeam(&tx, datum).is_err() {
                                break;
                            }
                        }
                    });
