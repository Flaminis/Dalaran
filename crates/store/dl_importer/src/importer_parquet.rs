//! Thin adapter that wraps [`dl_parquet`] as an [`Importer`].
...
///
/// Delegates to [`dl_parquet`] for the actual loading logic.
#[derive(Default)]
pub struct ParquetImporter {
    pub config: dl_parquet::ParquetConfig,
}
...
impl Importer for ParquetImporter {
    ...
    fn import_from_path(
        ...
        let prefix = settings
            .entity_path_prefix
            .clone()
            .unwrap_or_else(dl_parquet::ParquetConfig::default_entity_path_prefix);
        let store_id = settings.opened_store_id_or_recommended();

        std::thread::Builder::new()
            .name(format!("load_parquet({path:?})"))
            .spawn(
                move || match dl_parquet::load_parquet(&path, &config, &prefix) {
                    Ok(chunks) => forward_chunks(chunks, &tx, &store_id),
                    Err(err) => dl_log::error!("Failed to load Parquet: {err}"),
                },
            )
    ...
    fn import_from_file_contents(
        ...
        std::thread::Builder::new()
            ...
            .spawn(
                move || match dl_parquet::load_parquet_from_bytes(&contents, &config, &prefix) {
                    Ok(chunks) => forward_chunks(chunks, &tx, &store_id),
                    Err(err) => dl_log::error!("Failed to load Parquet: {err}"),
                },
            )
...
/// Forward chunks from a [`dl_parquet`] iterator to the [`Importer`] channel.
///
...
fn forward_chunks(
    chunks: impl Iterator<Item = Result<dl_chunk::Chunk, dl_parquet::ParquetError>>,
    tx: &Sender<ImportedData>,
    store_id: &StoreId,
) {
    let store_info_msg = crate::prepare_store_info(store_id, dl_log_types::FileSource::Sdk);
    if send_crossbeam(
        tx,
    ...
    for chunk_result in chunks {
        match chunk_result {
            ...
            Err(err) => {
                dl_log::error!("Parquet error: {err}");
            }
        }
    }
}
