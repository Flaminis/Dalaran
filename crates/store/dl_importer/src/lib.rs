...
///
/// This merges the built-in MCAP decoders from [`dl_mcap`] and the semantic interpretation (e.g. lenses) that are in this crate.
pub fn supported_mcap_decoder_identifiers(
    raw_fallback_enabled: bool,
) -> Vec<dl_mcap::DecoderIdentifier> {
    let mut identifiers = dl_mcap::DecoderRegistry::all_builtin(raw_fallback_enabled)
        .all_identifiers()
        .into_iter()
        .map(dl_mcap::DecoderIdentifier::from)
        .collect::<BTreeSet<_>>();

    identifiers.extend([
        dl_mcap::DecoderIdentifier::from(FOXGLOVE_LENSES_IDENTIFIER),
        dl_mcap::DecoderIdentifier::from(URDF_DECODER_IDENTIFIER),
    ]);
...
pub struct ImporterSettings {
    /// The recommended [`dl_log_types::ApplicationId`] to log the data to, based on the surrounding context.
    pub application_id: Option<dl_log_types::ApplicationId>,
    ...
    /// The [`dl_log_types::StoreId`] that is currently opened in the viewer, if any.
    ...
    ///
    /// Defaults to [`dl_log_types::TimeType::TimestampNs`].
    /// When set to [`dl_log_types::TimeType::DurationNs`], all timestamp timelines
    /// will be created as duration timelines instead.
    pub timeline_type: dl_log_types::TimeType,
}
...
impl ImporterSettings {
    ...
    /// Returns the recommended [`dl_log_types::StoreId`] to log the data to.
    pub fn recommended_store_id(&self) -> StoreId {
        StoreId::recording(
            self.application_id
                .clone()
                .unwrap_or_else(dl_log_types::ApplicationId::random),
            self.recording_id.clone(),
        )
    }

    /// Returns the currently opened [`dl_log_types::StoreId`] if any. Otherwise, returns the
    ...
    pub fn to_cli_args(&self) -> Vec<String> {
        ...
        if let Some(timepoint) = timepoint {
            ...
            for (timeline, cell) in timepoint.iter() {
                match cell.typ() {
                    dl_log_types::TimeType::Sequence => {
                        args.extend([
...
pub enum ImporterError {
    ...
    #[error(transparent)]
    Chunk(#[from] dl_chunk::ChunkError),

    #[error(transparent)]
    Decode(#[from] dl_log_encoding::DecodeError),
    ...
    Mp4 {
        path: std::path::PathBuf,
        source: dl_mp4_reader::Mp4Error,
    },

    #[error("{}", dl_error::format(.0))]
...
pub enum ImportedData {
    Chunk(ImporterName, dl_log_types::StoreId, Chunk),
    ArrowMsg(ImporterName, dl_log_types::StoreId, ArrowMsg),
...
///
/// Delegates to [`dl_sdk_types::components::MediaType::file_extension`].
pub fn content_type_to_extension(content_type: &str) -> Option<String> {
    ...
    let media_type = dl_sdk_types::components::MediaType(mime.to_owned().into());
    media_type.file_extension().map(|e| e.to_owned())
}
