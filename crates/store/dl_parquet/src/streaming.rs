...
impl ParquetChunkIterator {
    ...
    fn build_timelines(&self, batch: &RecordBatch) -> IntMap<dl_chunk::TimelineName, TimeColumn> {
    ...
    fn build_data_chunks(
        &mut self,
        batch: &RecordBatch,
        timelines: &IntMap<dl_chunk::TimelineName, TimeColumn>,
    ) {
        let num_rows = batch.num_rows();

        for group in &self.column_groups {
            let components: dl_chunk::ChunkComponents =
    ...
    fn build_finalization_chunks(&mut self) {
        if let Some(ref refs) = self.static_reference {
            let components: dl_chunk::ChunkComponents = refs
                .iter()
...
fn build_metadata_chunk(metadata: &parquet::file::metadata::ParquetMetaData) -> Option<Chunk> {
    ...
    let kv_component = dl_sdk_types::components::KeyValuePairs(pairs);
    ...
    let batch = kv_component
        .try_serialized(ComponentDescriptor {
            ...
            component_type: Some(dl_sdk_types::components::KeyValuePairs::name()),
        })
        .ok()?;
...
fn emit_chunk(
    ...
    timelines: &IntMap<dl_chunk::TimelineName, TimeColumn>,
    components: dl_chunk::ChunkComponents,
) {
