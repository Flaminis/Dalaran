...
fn extract_raw_i64(array: &dyn Array) -> Option<ScalarBuffer<i64>> {
    match array.data_type() {
        ...
        other => {
            dl_log::warn_once!("Cannot use column with type {other:?} as a timeline index");
            None
        }
    }
}
...
pub(crate) fn fallback_sequence_timeline(
    offset: i64,
    num_rows: usize,
) -> dl_chunk::external::nohash_hasher::IntMap<dl_chunk::TimelineName, TimeColumn> {
