...
/// See [`dl_tuid`] docs for explanations of TUID namespaces.
///
/// ### Uniqueness
///
...
pub struct RowId(pub(crate) dl_tuid::Tuid);
...
impl std::str::FromStr for RowId {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        dl_tuid::Tuid::from_str(tuid_str)
            .map(Self)
...
impl RowId {
    pub const ZERO: Self = Self(dl_tuid::Tuid::ZERO);
    pub const MAX: Self = Self(dl_tuid::Tuid::MAX);

    /// Create a new unique [`RowId`] based on the current time.
    #[expect(clippy::new_without_default)]
    #[inline]
    pub fn new() -> Self {
        Self(dl_tuid::Tuid::new())
    }

    #[inline]
    pub fn from_tuid(tuid: dl_tuid::Tuid) -> Self {
        Self(tuid)
    }

    #[inline]
    pub fn as_tuid(&self) -> dl_tuid::Tuid {
        self.0
    }
    ...
    pub fn from_u128(id: u128) -> Self {
        Self(dl_tuid::Tuid::from_u128(id))
    }
    ...
    pub fn slice_from_arrow(array: &arrow::array::FixedSizeBinaryArray) -> &[Self] {
        dl_log::debug_assert_eq!(array.data_type(), &Self::arrow_datatype());
        bytemuck::cast_slice(array.value_data())
    }
}

impl std::ops::Deref for RowId {
    type Target = dl_tuid::Tuid;
...
fn test_row_id_parse() {
    let tuid: dl_tuid::Tuid = "182342300C5F8C327a7b4a6e5a379ac4".parse().unwrap();
