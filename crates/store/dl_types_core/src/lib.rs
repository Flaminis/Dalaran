...
/// Fundamental [`Archetype`]s that are implemented in `dl_types_core` directly for convenience and
/// dependency optimization.
///
/// There are also re-exported by `dl_sdk_types`.
pub mod archetypes;

/// Fundamental [`Component`]s that are implemented in `dl_types_core` directly for convenience and
/// dependency optimization.
///
/// There are also re-exported by `dl_sdk_types`.
pub mod components;

/// Fundamental datatypes that are implemented in `dl_types_core` directly for convenience and
/// dependency optimization.
///
/// There are also re-exported by `dl_sdk_types`.
pub mod datatypes;
...
pub mod external {
    pub use {anyhow, arrow, dl_string_interner, dl_tuid};
}

/// Useful macro for statically asserting that a `struct` contains some specific fields.
///
///  ```
/// # #[macro_use] extern crate dl_types_core;
/// struct Data {
///     x: f32,
///     y: String,
///     z: u32,
...
#[doc(hidden)] // public so we can access it from dl_sdk_types too
