    ...
    match datatype {
        ...
        DataType::FixedSizeList(field, size) => {
            ...
            let value_data: ArrayRef = if size == 0 {
                ...
                match dl_arrow_util::concat_arrays(&vec![element.as_ref(); size]) {
                ...
                .build()
            {
                ...
                dl_log::warn_once!("Bug in FixedSizeListArray of {}", field.data_type());
                array::new_empty_array(datatype)
            }
        }
        ...
        | DataType::RunEndEncoded { .. } => {
            // TODO(emilk)
            dl_log::debug_once!("Unimplemented: placeholder value for: {datatype}");
...
/// Information about a Dalaran [`component`](crate::Component), generated by codegen.
#[derive(Clone, Debug)]
...
///
/// Components that are built into the Dalaran viewer follow the convention that
/// the `component` field starts with the short name of its archetype.
pub trait ComponentDescriptorExt {
    /// Returns the field name of a Dalaran-builtin type.
    ///
    /// This is the result of stripping the Dalaran-builtin [`ArchetypeName`]
...
mod test {
    ...
    fn component_descriptor_manipulation() {
        let archetype_name: ArchetypeName = "dalaran.archetypes.MyExample".into();
        ...
        let archetype_name: ArchetypeName = "dalaran.archetypes.MyOtherExample".into();
