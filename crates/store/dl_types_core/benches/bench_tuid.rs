    ...
    for elem_count in [1, 1000] {
        {
            ...
            let tuids = vec![dl_tuid::Tuid::new(); elem_count as usize];

            group.bench_function("arrow", |b| {
                b.iter(|| {
                    let data = dl_tuid::Tuid::to_arrow(tuids.clone()).unwrap();
...

        {
            ...
            let data =
                dl_tuid::Tuid::to_arrow(vec![dl_tuid::Tuid::new(); elem_count as usize]).unwrap();

            group.bench_function("arrow", |b| {
                b.iter(|| {
                    let tuids = dl_tuid::Tuid::from_arrow(data.as_ref()).unwrap();
