...
impl Args {
    ...
    pub async fn create_server_handle(self) -> anyhow::Result<ServerHandle> {
        ...
        let handler = {
            ...
            for table in &tables {
                cfg_select! {
                    ...
                    _ => {
                        _ = table;
                        anyhow::bail!("dl_server was not compiled with the 'lance' feature");
                    }
                }
            }
        ...
        let dalaran_cloud_server =
            ...
            .max_encoding_message_size(dl_grpc_server::MAX_ENCODING_MESSAGE_SIZE);
        ...
        let async_runtime =
            dl_async::AsyncRuntimeHandle::from_current_tokio_runtime_or_wasmbindgen()?;
