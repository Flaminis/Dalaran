...
use dl_log_types::{EntryId, StoreId};
...
impl InMemoryStore {
    ...
    pub async fn chunks_from_chunk_keys(
        ...
        let mut ids_per_lazy: HashMap<StoreSlotId, Vec<dl_chunk_store::ChunkId>> =
            HashMap::default();
        ...
        let mut loaded: HashMap<(StoreSlotId, dl_chunk_store::ChunkId), Arc<Chunk>> =
            HashMap::default();
    ...
    ) -> Result<std::collections::BTreeSet<dl_types_core::SegmentId>, Error> {
        let dataset = self
            .datasets
            .get_mut(&dataset_id)
    ...
    pub async fn load_directory_as_dataset(
        ...
        dl_log::info!("Finished loading {}", directory.display());
    ...
    pub async fn load_directory_as_table(
        ...
        use dl_protos::cloud::v1alpha1::ext::LanceTable;
        ...
        match self.table_by_name(&entry_name) {
            ...
            Some(_) => match on_duplicate {
                IfDuplicateBehavior::Overwrite => {
                    dl_log::info!("Overwriting {entry_name}");
                    self.add_table_entry(entry_name, entry_id, table, provider_details)?;
                }
                IfDuplicateBehavior::Skip => {
                    dl_log::info!("Ignoring {entry_name}: it already exists");
                }
    ...
    pub(crate) fn create_dataset_impl(
        ...
        dl_log::debug!(%name, "create_dataset");
    ...
    pub fn delete_entry(&mut self, entry_id: EntryId) -> Result<(), Error> {
        dl_log::debug!(?entry_id, "delete_entry");
    ...
    fn update_entries_table(&mut self) -> Result<(), Error> {
        ...
        use dl_protos::cloud::v1alpha1::ext::SystemTable;
    ...
    pub async fn create_table_entry(
        ...
        dl_log::debug!(%name, "create_table");
        if self.id_by_name.contains_key(&name) {
            return Err(Error::DuplicateEntryNameError(name));
        }
