...
/// The contents of a ([`dl_types_core::SegmentId`], [`dl_types_core::LayerName`]) pair.
///
...
impl Source {
    ...
    pub fn size_bytes(&self) -> u64 {
        match &self.resolved {
            ...
            ResolvedStore::Lazy(l) => {
                let header = dl_log_encoding::MessageHeader::ENCODED_SIZE_BYTES as u64;
                l.manifest()
                    .col_chunk_byte_size()
                    .iter()
    ...
    pub fn schema_sha256(&self) -> Result<[u8; 32], ArrowError> {
        dl_log_encoding::RawRrdManifest::compute_sorbet_schema_sha256(&self.schema())
    }
    ...
    fn dlr_manifest_from_chunks(
        ...
        let chunks: Vec<Arc<dl_chunk_store::Chunk>> =
            store.iter_physical_chunks().cloned().collect();
        let store_id = store.id().clone();
        drop(store);

        let mut builder = dl_log_encoding::RrdManifestBuilder::default();
...
mod tests {
    ...
    use dl_chunk_store::{Chunk, ChunkStore, ChunkStoreConfig, ChunkStoreHandle, LazyStore};
    use dl_log_encoding::EncodingOptions;
    use dl_log_encoding::RrdChunkProvider;
    use dl_log_types::{
    ...
    async fn dlr_manifest_lazy_and_eager_produce_equivalent_output() {
        ...
        let test_layer_info = Arc::new(LayerInfo {
            name: dl_types_core::LayerName::base(),
        });
