...
use dl_protos::cloud::v1alpha1::ext::{DataSourceKind, DatasetDetails, DatasetEntry, EntryDetails};
use dl_protos::cloud::v1alpha1::{EntryKind, ScanSegmentTableResponse};
...
impl Dataset {
    ...
    pub async fn segment_table(&self) -> Result<RecordBatch, Error> {
        ...
        let properties_record_batch =
            dl_arrow_util::concat_polymorphic_batches(all_segment_properties.as_slice())
                .map_err(Error::failed_to_extract_properties)?;
        let indexes_record_batch =
            dl_arrow_util::concat_polymorphic_batches(all_index_ranges.as_slice())?;
    ...
    pub async fn dataset_manifest_filtered(
        ...
        for (layer_name, segment_id, source) in layers {
            ...
            // In dl_server, only successful registrations exist (schema conflicts fail synchronously),
        ...
        let properties_record_batch =
            dl_arrow_util::concat_polymorphic_batches(properties.as_slice())
                .map_err(Error::failed_to_extract_properties)?;
    ...
    pub async fn add_source(
        ...
        dl_log::debug!(?segment_id, ?layer_name, "add_layer");
        ...
        for new_field in new_layer_schema.fields() {
            ...
            {
                dl_arrow_util::reject_unsupported_widenings(new_field.data_type()).map_err(
                    |err| {
    ...
    pub async fn remove_layers(
        ...
        dl_log::debug!(?segments_to_drop, ?layers_to_drop, "remove_layers");
