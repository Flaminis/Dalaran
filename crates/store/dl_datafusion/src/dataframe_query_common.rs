...
pub trait DataframeClientAPI: std::fmt::Debug + Clone + Send + Sync + Unpin + 'static {
    ...
    async fn fetch_chunks(
        ...
        tonic::Response<tonic::codec::Streaming<dl_protos::cloud::v1alpha1::FetchChunksResponse>>,
    >;
}
...
impl<T: DataframeClientAPI> DataframeQueryTableProvider<T> {
    ...
    pub async fn new_from_client(
        ...
        let (schema, trace_id) = if let Some(schema) = arrow_schema {
            ...
            let trace_id = dl_redap_client::extract_trace_id(response.metadata());
            ...
            let mut response_futures = futures::stream::iter(dataset_queries)
                .map(|dataset_query| {
                    ...
                    async move {
                        ...
                        let proto_request: dl_protos::cloud::v1alpha1::QueryDatasetRequest =
                            dataset_query.into();
                        let response =
                            dl_redap_client::with_retry_resource_exhausted("query_dataset", || {
...
pub(crate) fn group_chunk_infos_by_segment_id(
    ...
    for batch in chunk_info_batches {
        ...
        for (segment_id, row_indices) in segment_rows {
            ...
            let segment_batch = dl_arrow_util::take_record_batch(batch, &row_indices)?;
...
mod tests {
    ...
    fn segment_id_filter_keeps_per_segment_values_aligned() {
        ...
        let request = QueryDatasetRequest {
            ...
            query: Some(Query {
                ...
                range: Some(QueryRange {
                    index: timeline,
                    index_range: dl_log_types::AbsoluteTimeRange::new(at(0), at(100)),
                }),
                ..Default::default()
            }),
            ..Default::default()
        ...
        let wire: dl_protos::cloud::v1alpha1::QueryDatasetRequest = narrowed.clone().into();
        dl_protos::cloud::v1alpha1::ext::QueryDatasetRequest::try_from(wire)
    ...
    fn pushdown_is_noop_unless_scoped_temporal() {
        ...
        let baseline = || QueryDatasetRequest {
            ...
            query: Some(Query {
                ...
                range: Some(QueryRange {
                    index: timeline,
                    index_range: dl_log_types::AbsoluteTimeRange::new(
    ...
    fn test_batches_grouping() {
        ...
        fn chunk_ids_of(batch: &RecordBatch) -> Vec<dl_types_core::ChunkId> {
        ...
        assert_eq!(
            ...
            [[0u8; 16], [2u8; 16]].map(dl_types_core::ChunkId::from)
        );
        ...
        assert_eq!(
            ...
            [[1u8; 16]].map(dl_types_core::ChunkId::from)
        );
        assert_eq!(
            chunk_ids_of(&group_b[1]),
            [[4u8; 16]].map(dl_types_core::ChunkId::from)
        );
        ...
        assert_eq!(
            ...
            [[3u8; 16]].map(dl_types_core::ChunkId::from)
        );
        assert_eq!(
            chunk_ids_of(&group_c[1]),
            [[5u8; 16]].map(dl_types_core::ChunkId::from)
        );
        ...
        assert_eq!(
            ...
            [[6u8; 16]].map(dl_types_core::ChunkId::from)
        );
    }
