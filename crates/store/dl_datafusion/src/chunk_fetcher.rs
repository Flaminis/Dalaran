...
pub async fn fetch_batch_direct(
    ...
    match fetch_batch_via_direct_urls(http_client, batch, request_counter, stats).await {
        ...
        Err(err) => {
            ...
            Err(dl_redap_client::ApiError::connection_with_source(
                None,
                err,
...
pub async fn fetch_batch_group_via_grpc<T: DataframeClientAPI>(
    ...
    for batch in batch_group {
        ...
        let chunk_info: dl_protos::common::v1alpha1::DataframePart = batch.clone().into();
        ...
        req.set_timeout(dl_redap_client::FETCH_CHUNKS_DEADLINE);
        let response = client
            .fetch_chunks(req)
            .instrument(tracing::trace_span!("batched_fetch_chunks"))
            .await
            .map_err(|err| dl_redap_client::ApiError::tonic(err, "FetchChunks request failed"))?;

        let response_stream =
            dl_redap_client::ApiResponseStream::from_tonic_response(response, "/FetchChunks");

        let chunk_stream =
            dl_redap_client::fetch_chunks_response_to_chunk_and_segment_id(response_stream);
    ...
    fn from(err: reqwest::Error) -> Self {
        ...
        for _ in 0..MAX_ERROR_SOURCE_DEPTH {
            ...
            if let Err(err) = write!(msg, " ({cause})") {
                dl_log::debug!("Failed to append error source to message: {err}");
                break;
            }
            source = cause.source();
        }
        ...
        {
            dl_log::debug!("Failed to append URL to message: {err}");
        }
    ...
    let fetches = merged_requests
        ...
        .map(|(req_idx, request)| {
            ...
            async move {
                ...
                let mut backoff_gen = dl_backoff::BackoffGenerator::new(
