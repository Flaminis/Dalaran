...
use dl_dataframe::external::dl_chunk::{Chunk, TimeColumn};
    ...
    for chunk_info in chunk_infos {
        ...
        if is_new_segment && segments_in_wave == segment_limit {
            if !current_batch.is_empty() {
                ...
                let merged_batch = dl_arrow_util::concat_polymorphic_batches(&current_batch)
...
fn split_large_segments(
    ...
    for row_idx in 0..chunk_info.num_rows() {
        ...
        if current_indices.is_empty() || current_size + chunk_size <= target_size {
            ...
            let batch =
                dl_arrow_util::take_record_batch(chunk_info, &current_indices).map_err(take_err)?;
    ...
    tracing::debug!(
        ...
        segment_id,
        dl_format::format_bytes(chunk_sizes.iter().sum::<u64>() as _),
        result_batches.len()
    );
        ...
        create_grpc_batch_groups_with_segment_limit(batches, pipeline_budget.segment_limit())?
    {
        ...
        let actual: usize = all_chunks
            ...
            .flat_map(|segment_chunks| {
                segment_chunks
                    .iter()
                    .map(|(chunk, _)| dl_byte_size::SizeBytes::total_size_bytes(chunk) as usize)
            })
            .sum();
        guard.commit(actual);
...
/// which is different from the Dalaran `segment_id`. The sorting by time index will happen within
...
pub(super) async fn chunk_stream_io_loop<T: DataframeClientAPI>(
    ...
    dl_log::debug!("Fetch tasks: {total_n_direct} direct, {total_n_grpc} gRPC fallback");
            ...
            .map(|(task_idx, task)| {
                ...
                async move {
                    ...
                    let actual: usize = chunks
                        ...
                        .flat_map(|seg| {
                            seg.iter()
                                .map(|(c, _)| dl_byte_size::SizeBytes::total_size_bytes(c) as usize)
                        })
                        .sum();
                    guard.commit(actual);
...
mod tests {
    ...
    fn test_sort_chunks_by_segment_order_simple_case() {
        use dl_dataframe::external::dl_chunk::Chunk;
        use dl_log_types::EntityPath;
    ...
    fn create_chunk_info_with_starts(
        ...
        let schema = Arc::new(Schema::new_with_metadata(
            vec![
                ...
                dl_protos::cloud::v1alpha1::QueryDatasetResponse::field_timeline_start(
                    timeline_name,
                )
                .as_ref()
                .clone(),
