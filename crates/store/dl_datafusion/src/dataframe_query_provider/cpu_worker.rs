...
use dl_dataframe::external::dl_chunk::{Chunk, ChunkId, LatestAtQuery};
...
use dl_log_types::{AbsoluteTimeRange, ApplicationId, StoreId, StoreKind, TimeInt};
...
impl CurrentStores {
    ...
    async fn flush_incremental(
        ...
        // `QueryHandle::range_query` (see `dl_dataframe::query`): the
...
pub(super) async fn chunk_store_cpu_worker_thread(
    ...
    // while the IO side reserves using `dl_byte_size::SizeBytes::total_size_bytes`
        ...
        match msg? {
            ...
            CpuWorkerMsg::Chunks((segment_id, chunks)) => {
                ...
                // would otherwise poison the surrounding future.
                {
                    ...
                    .entered();
                    dl_tracing::profile_scope!("insert_chunks");
                    for chunk in chunks {
...
mod tests {
    ...
    fn temporal_chunk(entity: &str, timeline_name: &'static str, time: i64) -> Chunk {
        use dl_dataframe::external::dl_chunk::RowId;
        use dl_log_types::Timeline;
    ...
    ///
    /// ```text
    /// cargo nextest run -p dl_datafusion --all-features \
