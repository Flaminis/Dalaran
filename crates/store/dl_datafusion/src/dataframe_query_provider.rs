...
use dl_sorbet::{ColumnDescriptor, ColumnSelector};
...
) -> Option<dl_perf_telemetry::external::opentelemetry::ContextGuard> {
...
pub(crate) struct SegmentStreamExec<T: DataframeClientAPI> {
    ...
    server_trace_id: Option<dl_redap_client::TraceId>,
...
fn queried_uncompressed_segment_size(batches: &[RecordBatch]) -> Option<u64> {
    ...
    for batch in batches {
        ...
        for size in sizes.values() {
            if *size == 0 {
                dl_log::debug!("adaptive segment admission: {column_name} contains a zero size");
                return None;
            }
    ...
    if segment_size == 0 {
        dl_log::debug!("adaptive segment admission: no {column_name} rows for this segment");
        return None;
    }
    Some(segment_size)
}
...
impl<T: DataframeClientAPI> ExecutionPlan for SegmentStreamExec<T> {
    ...
    fn execute(
        ...
        let chunk_infos: Vec<_> = {
            dl_tracing::profile_scope!("concat_chunk_infos_per_segment");
            self.chunk_info
                .iter()
                .filter(|(segment_id, _)| {
        ...
        match t {
            ...
            DisplayFormatType::Verbose => {
                ...
                write!(
                    ...
                    s.query_chunks,
                    s.query_segments,
                    dl_format::format_bytes(s.query_bytes as _),
