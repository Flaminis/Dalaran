...
pub(crate) struct SegmentChunkManifest {
    ...
    /// cheap `Arc` bump — consistent with the rest of `dl_datafusion`.
...
impl SegmentChunkManifest {
    ...
    /// divergence". Callers should `dl_log::debug_panic!` +
    /// `dl_log::error_once!` on a `false` return; the chunk is still
