//! In-process `TableProvider` over a local [`dl_dataframe::external::dl_chunk_store::ChunkStore`].
//!
...
use dl_dataframe::external::dl_chunk_store::ChunkStoreHandle;
use dl_dataframe::utils::align_record_batch_to_schema;
use dl_dataframe::{QueryEngine, QueryExpression, StorageEngine};
use dl_sorbet::BatchType;
...
impl LocalChunkStoreTableProvider {
    ...
    ///
    /// The provider holds a fresh [`dl_dataframe::QueryCache`]; callers cannot
...
mod tests {
    ...
    async fn selection_reorders_and_keeps_only_picked_columns() {
        use dl_sorbet::{ColumnSelector, TimeColumnSelector};
