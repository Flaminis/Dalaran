...
//!
//! [`ChunkStore`]: dl_dataframe::external::dl_chunk_store::ChunkStore
...
fn read_env_trimmed(key: &str) -> Option<String> {
    match std::env::var(key) {
        ...
        Err(std::env::VarError::NotUnicode(_)) => {
            dl_log::error!("{key}: value is not valid Unicode; using default");
            None
        }
    }
}
...
/// (`64MB`, `1GiB`, `512KiB`) via [`dl_format::parse_bytes`] or a bare
...
fn parse_bytes_or_default(key: &str, raw: &str, default_bytes: usize) -> usize {
    let parsed = dl_format::parse_bytes(raw).or_else(|| raw.parse::<i64>().ok());
    match parsed {
        Some(n) if n > 0 => n as usize,
        Some(_) => {
            dl_log::error!(
                "{key}={raw:?} must be > 0; falling back to default {}",
                dl_format::format_bytes(default_bytes as f64),
            );
            default_bytes
        }
        None => {
...
fn parse_usize_or_default(key: &str, raw: &str, default: usize) -> usize {
    match raw.parse::<i64>() {
        ...
        Ok(_) => {
            dl_log::error!("{key}={raw:?} must be > 0; falling back to default {default}");
            default
        }
        Err(err) => {
            dl_log::error!(
...
pub(crate) fn direct_fetch_semaphore() -> &'static tokio::sync::Semaphore {
    ...
    SEM.get_or_init(|| {
        ...
        .max(1);
        dl_log::debug!("direct-fetch concurrency cap = {permits}");
...
pub(crate) fn query_dataset_semaphore() -> &'static tokio::sync::Semaphore {
    ...
    SEM.get_or_init(|| {
        ...
        dl_log::debug!("query_dataset concurrency cap = {permits}");
        tokio::sync::Semaphore::new(permits)
    })
}
    ...
    pub(crate) fn resolve_size(
        ...
        if min_per_partition > max_per_partition {
            dl_log::error!(
                ...
                dl_format::format_bytes(min_per_partition as f64),
                dl_format::format_bytes(max_per_partition as f64),
            );
        ...
        dl_log::debug!("Pipeline budget: {}MB", budget / (1024 * 1024));
