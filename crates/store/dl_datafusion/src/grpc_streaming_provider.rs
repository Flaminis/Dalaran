...
impl GrpcStream {
        ...
        T: GrpcStreamToTable + Send + 'static,
    {
        ...
        let adapted_stream = Box::pin(async_stream::try_stream! {
            ...
            // rebuilt fresh.
            let mut stream = dl_redap_client::with_retry_resource_exhausted("grpc_stream_open", || {
...
fn normalize_batch(batch: RecordBatch, expected_schema: &SchemaRef) -> ApiResult<RecordBatch> {
    ...
    let columns = expected_schema
        ...
        .map(|expected_field| {
            ...
            let (index, actual_field) = matching_columns.next().ok_or_else(|| {
                dl_redap_client::ApiError::deserialization(
                    None,
                    format!(
            ...
            if matching_columns.next().is_some() {
                return Err(dl_redap_client::ApiError::deserialization(
                    None,
                    format!(
    ...
    .map_err(|err| {
        dl_redap_client::ApiError::deserialization_with_source(
            None,
            err,
