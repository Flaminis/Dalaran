...
use dl_arrow_util::garbage_collect_string_view_batch;
use dl_dataframe::external::dl_chunk::external::dl_byte_size::SizeBytes as _;
use std::sync::Arc;
...
impl SizedBatchCoalescer {
    ...
    fn max_rows_reached(&mut self, batch: &RecordBatch) -> bool {
            ...
            && self.state.total_rows + batch.num_rows() >= max_rows
        {
            ...
            dl_log::debug_assert!(remaining_rows > 0);
