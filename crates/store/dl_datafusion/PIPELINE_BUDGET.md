...
## Why a byte-bounded channel (`dl_quota_channel`) doesn't fix it

`dl_quota_channel` puts a byte cap on **stage B only** — the channel buffer between IO and CPU. Backpressure activates when the *channel send* would exceed the cap.
