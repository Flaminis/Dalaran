...
impl TryFrom<i32> for NumericType {
    type Error = dl_lenses_core::combinators::Error;
    ...
    fn try_from(value: i32) -> Result<Self, Self::Error> {
        match value {
            ...
            7 => Ok(Self::Float32),
            8 => Ok(Self::Float64),
            _ => Err(dl_lenses_core::combinators::Error::Other(format!(
...
fn find_field_descriptors(
    ...
    names: &[&str],
) -> Result<Vec<Option<FieldDescriptor>>, dl_lenses_core::combinators::Error> {
...
pub(crate) fn extract_positions(
    source: &ArrayRef,
) -> Result<Option<ArrayRef>, dl_lenses_core::combinators::Error> {
    dl_tracing::profile_function!();

    let source = source
        .as_any()
        .downcast_ref::<StructArray>()
        .ok_or_else(|| dl_lenses_core::combinators::Error::TypeMismatch {
