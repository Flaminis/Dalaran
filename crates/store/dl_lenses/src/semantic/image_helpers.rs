...
/// Parses an encoding string into an [`dl_mcap::ImageEncoding`], mapping the error for use in transforms.
fn parse_encoding(s: &str) -> Result<dl_mcap::ImageEncoding, Error> {
    s.parse().map_err(|_err| Error::UnexpectedValue {
        expected: dl_mcap::ImageEncoding::NAMES,
        actual: s.to_owned(),
    })
}
