...
//!
//! Transform frames are identified by a string identifier, see [`dl_sdk_types::components::TransformFrameId`].
//!
...
//! The transform frame can be set with [`dl_sdk_types::archetypes::CoordinateFrame`].
//!
...
//! to their respective parent (unless overwritten by e.g. [`dl_sdk_types::archetypes::Transform3D`]).
//!
//! Example:
...
//!
//! For more details see [`dl_sdk_types::archetypes::InstancePoses3D`].
//!
//!
//! ## Implementation
//!
...
//! This is a fairly complex workload since combining everything in [`dl_sdk_types::archetypes::Transform3D`]
...
//! can be performed efficiently while heeding the query rules established by [`dl_entity_db::external::dl_query`].
//!
//!
//! ### [`TransformFrameIdHash`]
//!
//! Throughout this crate, we almost always use pre-hashed [`dl_sdk_types::components::TransformFrameId`]s, i.e. [`TransformFrameIdHash`].
//! Hashes are assumed to be collision free, allowing use as keys in maps.
//!
//! For performance & convenience, there's a 1:1 mapping from [`dl_log_types::EntityPathHash`] to [`TransformFrameIdHash`]
...
// Re-export the transform frame id types from dl_sdk_types.
pub use dl_sdk_types::TransformFrameIdHash;
pub use dl_sdk_types::components::TransformFrameId;
...
pub fn image_view_coordinates() -> dl_sdk_types::components::ViewCoordinates {
    dl_sdk_types::archetypes::Pinhole::DEFAULT_CAMERA_XYZ
}
