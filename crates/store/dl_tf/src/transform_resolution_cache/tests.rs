...
impl dl_byte_size::MemUsageTreeCapture for TestStoreSubscriber {
    fn capture_mem_usage_tree(&self) -> dl_byte_size::MemUsageTree {
        dl_byte_size::MemUsageTree::Bytes(0)
    }
}
    ...
    for (t, expected) in [
        ...
        (
            ...
            Some(DAffine3::from_quat(
                convert::quaternion_to_dquat(dl_sdk_types::datatypes::Quaternion::from(
...
#[track_caller]
fn ensure_no_logged_error(rx: &dl_log::Receiver<dl_log::LogMsg>) {
...
fn test_error_on_changing_associated_path(time: TimeInt) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let log_rx = dl_log::add_log_msg_receiver(dl_log::LevelFilter::ERROR);
    ...
    assert_eq!(error.level, dl_log::Level::ERROR);
    assert!(
