...
/// * [`dl_sdk_types::components::PinholeProjection`] and [`dl_sdk_types::components::ViewCoordinates`]
...
impl dl_byte_size::MemUsageTreeCapture for TransformResolutionCache {
    fn capture_mem_usage_tree(&self) -> dl_byte_size::MemUsageTree {
        ...
        let mut per_timeline_node = dl_byte_size::MemUsageNode::new();
...
impl TransformResolutionCache {
    ...
    pub fn ensure_timeline_is_initialized(
        ...
        dl_tracing::profile_function!(timeline);
    ...
    pub fn evict_timeline_cache(&mut self, timeline: TimelineName) {
        dl_tracing::profile_function!(); // There can be A LOT of tiny allocations to drop.
    ...
    pub fn process_store_events<'a>(
        &mut self,
        events: impl Iterator<Item = &'a dl_chunk_store::ChunkStoreEvent>,
    ) {
        dl_tracing::profile_function!();

        for event in events {
            match &**event {
                dl_chunk_store::ChunkStoreDiff::Addition(addition) => {
                ...
                dl_chunk_store::ChunkStoreDiff::SchemaAddition(_) => {}
            }
        }
    }
    ...
    fn add_temporal_chunk(&self, chunk: &dl_chunk_store::Chunk, aspects: TransformAspect) {
    ...
    fn add_static_chunk(&mut self, chunk: &dl_chunk_store::Chunk, aspects: TransformAspect) {
        dl_tracing::profile_function!();
    ...
    fn remove_chunk(&mut self, chunk: &dl_chunk_store::Chunk, aspects: TransformAspect) {
        dl_tracing::profile_function!();
