...
use dl_redap_browser::{EXAMPLES_ORIGIN, RedapServers};
use dl_ui::list_item::{LabelContent, ListItemContentButtonsExt as _};
use dl_ui::{ContextExt as _, OnResponseExt as _, UiExt as _, UiLayout, icons, list_item};
use dl_uri::dataset_hierarchy_leaf_name;
...
impl RecordingPanel {
    ...
    pub fn show_panel(
        ...
        egui::ScrollArea::both()
            ...
            .show(ui, |ui| {
                ui.panel_content(|ui| {
                    dl_ui::list_item::list_item_scope(ui, "recording panel", |ui| {
...
fn add_button_ui(
    ...
    ui.add(
        ui.small_icon_button_widget(&dl_ui::icons::ADD, "Add…")
            .on_hover_text("Open a file or connect to a server")
            .on_menu(|ui| {
                if dl_ui::UICommand::Open
                    .menu_button_ui(ui, ctx.command_sender())
                    .clicked()
                {
                    ui.close();
                }
                if dl_ui::UICommand::OpenUrl
                    .menu_button_ui(ui, ctx.command_sender())
                    .clicked()
                {
                    ui.close();
                }
                if dl_ui::UICommand::AddRedapServer
                    .menu_button_ui(ui, ctx.command_sender())
                    .clicked()
                {
                    ui.close();
...
fn welcome_item_ui(
    ...
    if response.clicked() {
        dl_redap_browser::switch_to_welcome_screen(ctx.command_sender());
    }
}

// ---

fn server_title(ctx: &AppContext<'_>, origin: &dl_uri::Origin, is_internal: bool) -> String {
    if is_internal {
...
fn server_section_ui(ctx: &AppContext<'_>, ui: &mut egui::Ui, server_data: &ServerData<'_>) {
    ...
    let content = list_item::LabelContent::header(server_title(ctx, origin, *is_internal))
        .with_menu_button(&icons::MORE, "Actions", move |ui| {
            for command in dl_ui::RedapServerCommand::all_for_server(origin) {
    ...
    match node {
        ...
        EntryTreeNode::Folder {
            ...
            let item = Item::RedapEntry {
                origin: origin.clone(),
                kind: dl_viewer_context::RedapEntryKind::Folder(path_prefix.clone()),
            };
...
fn dataset_entry_ui(ctx: &AppContext<'_>, ui: &mut egui::Ui, dataset_entry_data: &DatasetData<'_>) {
    ...
    let mut list_item_content =
        dl_ui::list_item::LabelContent::new(dataset_hierarchy_leaf_name(name.as_str()))
            .with_icon(icon);
    ...
    let new_route = Route::from(dl_uri::EntryUri::new(origin.clone(), *entry_id));
...
fn failed_entry_ui(ctx: &AppContext<'_>, ui: &mut egui::Ui, failed_entry_data: &FailedEntryData) {
    ...
    if item_response.clicked() {
        ...
        ctx.command_sender().send_system(SystemCommand::SetRoute(
            dl_uri::EntryUri::new(origin.clone(), *entry_id).into(),
        ));
    }
...
fn app_id_section_ui(ctx: &AppContext<'_>, ui: &mut egui::Ui, local_app_id: &AppIdData<'_>) {
    ...
    let mut list_item_content =
        dl_ui::list_item::LabelContent::new(local_app_id.name()).with_icon(&icons::DATASET);
...
fn receiver_ui(
    ...
    let label_content = dl_ui::list_item::LabelContent::new(&name)
        ...
        .with_buttons(|ui| {
            let resp = ui
                .small_icon_button(&dl_ui::icons::REMOVE, "Disconnect")
...
fn server_item_id(origin: &dl_uri::Origin) -> egui::Id {
...
fn ancestor_folder_paths(path: &str) -> impl Iterator<Item = &str> {
    std::iter::chain(
        path.match_indices(dl_uri::DATASET_HIERARCHY_SEPARATOR)
            .map(|(idx, _)| &path[..idx]),
        std::iter::once(path),
    )
    .filter(|s| !s.is_empty())
