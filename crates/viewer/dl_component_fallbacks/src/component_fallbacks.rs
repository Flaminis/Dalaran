use dl_sdk_types::{archetypes, components, datatypes};
...
pub fn type_fallbacks(registry: &mut FallbackProviderRegistry) {
    ...
    registry.register_type_fallback_provider(|ctx| {
        ...
        dl_viewer_context::resolution_of_image_at(
            ctx.viewer_ctx(),
            &ctx.query,
...
pub fn archetype_field_fallbacks(registry: &mut FallbackProviderRegistry) {
            ...
            let is_quantized_rvl = ctx
                ...
                .is_some_and(|(_, blob)| {
                    encoded_depth_image_media_type(ctx, blob.as_ref())
                        ...
                        && dl_rvl::RosRvlMetadata::parse(blob.as_ref())
    ...
    registry.register_component_fallback_provider(
        ...
        |ctx| {
            ...
            {
                let tensor_cache_key = dl_log_types::hash::Hash64::hash(row_id);
    ...
    registry.register_component_fallback_provider(
        ...
        |ctx| {
            components::TransformFrameId::from_entity_path(
                &ctx.target_entity_path
                    .parent()
                    .unwrap_or_else(dl_log_types::EntityPath::root),
            )
        },
    );
...
fn show_labels_fallback(
    ctx: &QueryContext<'_>,
    instance_count_component: dl_sdk_types::ComponentIdentifier,
    text_component: dl_sdk_types::ComponentIdentifier,
...
fn tensor_data_range_heuristic(
    ...
    let (min, max) = dl_viewer_context::gpu_bridge::data_range_heuristic(
