fn main() {
    ...
    // See `Cargo.toml` docs for info about `__disable_server` and `DALARAN_DISABLE_WEB_VIEWER_SERVER`.
    ...
    let disable_web_viewer_server =
        dl_build_tools::is_tracked_env_var_set("DALARAN_DISABLE_WEB_VIEWER_SERVER")
    ...
    let trailing_web_viewer = dl_build_tools::is_tracked_env_var_set("DALARAN_TRAILING_WEB_VIEWER")
    ...
    let external_web_viewer = dl_build_tools::is_tracked_env_var_set("DALARAN_EXTERNAL_WEB_VIEWER")
    ...
    assert!(
        ...
        "DALARAN_TRAILING_WEB_VIEWER and DALARAN_EXTERNAL_WEB_VIEWER are mutually exclusive"
    );
    ...
    if needs_wasm {
        ...
        assert!(
            ...
            "Web viewer not found, run `pixi run dalaran-build-web` to build it!"
        );
    }
}
