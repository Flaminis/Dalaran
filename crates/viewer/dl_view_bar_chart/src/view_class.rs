...
use dl_sdk_types::components::{Color, Visible};
use dl_sdk_types::datatypes::TensorBuffer;
use dl_sdk_types::{View as _, ViewClassIdentifier};
use dl_ui::{Help, IconText, MouseButtonText, icons, list_item};
use dl_view::controls::SELECTION_RECT_ZOOM_BUTTON;
...
type ViewType = dl_sdk_types::blueprint::views::BarChartView;
    ...
    fn ui(
        ...
        ui.scope(|ui| {
            ...
            if let Some((entity_hash, bar_idx)) = hovered_bar {
                    ...
                    .find(|r| r.entity_path.hash() == entity_hash)
                {
                    ...
                    if let Some(ep) = plot_item_id_to_entity_path.get(&egui::Id::new(entity_hash)) {
                        ctx.viewer_ctx.handle_select_hover_drag_interactions(
                            &response,
                            dl_viewer_context::Item::DataResult(
                                dl_viewer_context::DataResultInteractionAddress::from_entity_path(
                                    query.view_id,
                                    ep.clone(),
                                ),
                            ),
                ...
                ctx.viewer_ctx.handle_select_hover_drag_interactions(
                    &response,
                    dl_viewer_context::Item::View(query.view_id),
                    false,
                );
            }
...
#[test]
fn test_help_view() {
    dl_test_context::TestContext::test_help_view(|ctx| BarChartView.help(ctx));
}
