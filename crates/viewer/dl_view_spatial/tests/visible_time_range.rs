...
use dl_log_types::{EntityPath, TimeInt, TimePoint, TimeReal, Timeline};
use dl_sdk_types::Archetype as _;
use dl_sdk_types::archetypes::{Arrows3D, Boxes3D, LineStrips3D, Points2D, Points3D};
use dl_sdk_types::blueprint::archetypes::{EyeControls3D, VisibleTimeRanges};
use dl_sdk_types::components::Position3D;
use dl_sdk_types::datatypes::{TimeRange, TimeRangeBoundary, VisibleTimeRange};
...
use dl_view_spatial::SpatialView2D;
...
fn setup_blueprint_2d(
    ...
    test_context.setup_viewport_blueprint(|ctx, blueprint| {
        ...
        let property_path = dl_viewport_blueprint::entity_path_for_view_property(
            view_id,
            blueprint_tree,
            dl_sdk_types::blueprint::archetypes::VisualBounds2D::name(),
        );
        ctx.save_blueprint_archetype(
            property_path.clone(),
            &dl_sdk_types::blueprint::archetypes::VisualBounds2D::new(
                dl_sdk_types::datatypes::Range2D {
        ...
        if let Some(green_time_range) = green_time_range {
            ctx.save_blueprint_archetype(
                dl_viewport_blueprint::ViewContents::base_override_path_for_entity(
                    view_id,
                    &"green".into(),
                ),
