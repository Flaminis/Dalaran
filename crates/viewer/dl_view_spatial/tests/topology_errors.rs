...
use dl_chunk_store::{LatestAtQuery, external::dl_chunk::external::crossbeam::atomic::AtomicCell};
use dl_log_types::{EntityPath, TimePoint, TimelineName};
use dl_sdk_types::{
...
fn setup_scene(test_context: &mut TestContext) {
    ...
    test_context.log_entity("misplaced_depth_image_entity", |builder| {
        builder
            .with_archetype_auto_row(
                TimePoint::STATIC,
                &archetypes::DepthImage::try_from(dl_sdk_types::datatypes::TensorData::new(
                    vec![1u64, 1u64],
                    dl_sdk_types::datatypes::TensorBuffer::U16(vec![1u16].into()),
                ))
    ...
    visualizer_errors
        ...
        .map(|(visualizer_type, error)| {
            let error = match error {
                dl_viewer_context::VisualizerTypeReport::OverallError(err) => {
                    err.diagnostic.summary.clone()
                }
                dl_viewer_context::VisualizerTypeReport::PerInstructionReport(errors) => errors
                    .iter()
...
fn test_topology_errors() {
    ...
    test_context.register_view_class::<dl_view_spatial::SpatialView3D>();
    test_context.register_view_class::<dl_view_spatial::SpatialView2D>();
    ...
    let scenarios = [
        TestScenario {
            ...
            target_frame: "world",
            view_class: dl_view_spatial::SpatialView3D::identifier(),
        },
        TestScenario {
            name: "2d_view_at_root",
            target_frame: "world",
            view_class: dl_view_spatial::SpatialView2D::identifier(),
        },
        TestScenario {
    ...
    for scenario in scenarios {
        let view_id = test_context.setup_viewport_blueprint(|ctx, blueprint| {
            ...
            .save_blueprint_component(
                ...
                &dl_tf::TransformFrameId::new(scenario.target_frame),
            );
...
fn test_topology_errors_for_nested_pinholes() {
    ...
    let view_id = test_context.setup_viewport_blueprint(|ctx, blueprint| {
        ...
        .save_blueprint_component(
            ctx,
            &blueprint_archetypes::SpatialInformation::descriptor_target_frame(),
            &dl_tf::TransformFrameId::new("inner_pinhole"),
        );
...
fn test_empty_coordinate_frame_name_falls_back_to_implicit_frame() {
    ...
    let stored_frame = test_context
        ...
        .unwrap()
        .latest_at_component::<dl_tf::TransformFrameId>(
        ...
        .map(|(_, frame)| frame);
    assert_eq!(stored_frame, Some(dl_tf::TransformFrameId::new("")));
