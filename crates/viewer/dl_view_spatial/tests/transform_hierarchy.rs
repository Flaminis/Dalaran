...
use dl_viewer_context::{TimeControlCommand, ViewClass as _, ViewId};
    ...
    test_context.log_entity("/", |builder| {
        builder
            .with_archetype(
                RowId::new(),
                TimePoint::STATIC,
                &dl_sdk_types::archetypes::ViewCoordinates::RIGHT_HAND_Y_UP(),
            )
            .with_archetype(
                RowId::new(),
                TimePoint::STATIC,
        ...
        test_context.log_entity(path.clone(), |builder| {
            builder
                .with_archetype(
                    RowId::new(),
                    [(timeline_step, 1)],
                    &dl_sdk_types::archetypes::Transform3D::from_translation((4.0, 4.0, 4.0)),
                )
                .with_archetype(
                    RowId::new(),
                    [(timeline_step, 1)],
        ...
        test_context.log_entity(path.clone(), |builder| {
            builder
                .with_archetype(
                    RowId::new(),
                    [(timeline_step, 3)],
                    &dl_sdk_types::archetypes::Transform3D::from_scale((1.0, 0.2, 1.0)),
                )
                .with_archetype(
                    RowId::new(),
                    [(timeline_step, 3)],
        ...
        test_context.log_entity(path.clone(), |builder| {
            builder
                .with_archetype(
                    RowId::new(),
                    [(timeline_step, 4)],
                    &dl_sdk_types::archetypes::Transform3D::from_mat3x3([
                        [1.0, 0.0, 0.0],
                        [0.0, 5.0, 0.0],
                        [0.0, 0.0, 1.0],
                    ]),
                ...
                .with_archetype(
                    ...
                    &dl_sdk_types::archetypes::Transform3D::from_rotation(
                        dl_sdk_types::components::RotationAxisAngle::new(
                            (0.0, 1.0, 0.0),
                            dl_sdk_types::datatypes::Angle::from_degrees(90.0),
                        ),
                    ),
                )
                .with_archetype(
            ...
            builder
                .with_archetype(
                    ...
                    &dl_sdk_types::archetypes::Transform3D::from_rotation(
                        ...
                        dl_sdk_types::components::RotationQuat(
                            dl_sdk_types::datatypes::Quaternion::from_xyzw([
        ...
        // (We're using a 3D model because it's easier to see the effect of arbitrary transforms here!)
        {
            ...
            test_context.log_entity(path.clone(), |builder| {
                builder.with_archetype(
                    RowId::new(),
                    [(timeline_step, 0)],
                    &dl_sdk_types::archetypes::Asset3D::from_file_path(&obj_path).unwrap(),
                )
            });
        }
    }
