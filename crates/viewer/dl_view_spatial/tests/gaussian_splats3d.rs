...
//!
//! The full `.ply` → `dl_importer` → `GaussianSplats3D` path is covered end-to-end by
//! `crates/store/dl_importer/tests/test_ply_importer.rs`.

use dl_log_types::TimePoint;
use dl_sdk_types::components::RotationAxisAngle;
use dl_sdk_types::datatypes::Angle;
use dl_sdk_types::{
    RowId,
...
use dl_test_context::external::egui_kittest::OsThreshold;
...
pub fn test_gaussian_splats3d() {
    ...
    let default_options = dl_ui::testing::default_snapshot_options_for_3d(size);
...
pub fn test_gaussian_splats3d_cactus() {
    ...
    let gaussians = GaussianSplats3D::from_ply_file_path(&dl_test_context::asset_path(
    ...
    let mut harness = test_context
        .setup_kittest_for_rendering_3d(size)
        .with_options(
            dl_ui::testing::default_snapshot_options_for_3d(size)
