...

    {
        ...
        test_context.log_entity("2d_layering/lines_behind_rect", |builder| {
            builder.with_archetype(
                RowId::new(),
                TimePoint::default(),
                &dl_sdk_types::archetypes::LineStrips2D::new(std::iter::once(
                    (0..20)
                        .map(|i| i as f32)
        ...
        test_context.log_entity("2d_layering/points_between_top_and_middle", |builder| {
            builder.with_archetype(
                RowId::new(),
                TimePoint::default(),
                &dl_sdk_types::archetypes::Points2D::new((0..16 * 16).map(|i| i as f32).map(|i| {
                    (
        ...
        test_context.log_entity("2d_layering/arrow2d_between", |builder| {
            builder.with_archetype(
                RowId::new(),
                TimePoint::default(),
                &dl_sdk_types::archetypes::Arrows2D::from_vectors([(200.0, 200.0)])
