...
use dl_sdk_types::archetypes::{AssetVideo, TextLog, VideoFrameReference, VideoStream};
use dl_sdk_types::components::{self, MediaType, VideoTimestamp};
use dl_sdk_types::datatypes;
use dl_test_context::TestContext;
use dl_test_context::external::egui_kittest::SnapshotOptions;
use dl_test_viewport::TestContextExt as _;
use dl_video::player::VideoSliceSource;
use dl_video::{VideoCodec, VideoDataDescription};
use dl_viewer_context::{TimeControlCommand, ViewClass as _};
...
fn snapshot_options_for_codec(codec: &VideoCodec, viewport_size: egui::Vec2) -> SnapshotOptions {
    match codec {
        ...
        VideoCodec::H264 | VideoCodec::H265 | VideoCodec::VP8 | VideoCodec::VP9 => {
            dl_ui::testing::default_snapshot_options_for_3d(viewport_size)
                .threshold(2.2)
                .max_failed_pixels(300)
        }
        // AV1 has this problem as well but to a lesser extent.
        VideoCodec::AV1 => dl_ui::testing::default_snapshot_options_for_3d(viewport_size)
            .threshold(1.2)
...
fn test_video(video_type: VideoType, codec: &VideoCodec) {
    ...
    if pixi_ffmpeg_path.exists() {
        ...
        dl_log::info!("Using pixi ffmpeg at {pixi_ffmpeg_path:?}");
    } else {
        // End up using system install. Fine usually, no need to force a pixi environment here.
        dl_log::info!("Pixi ffmpeg not found at {pixi_ffmpeg_path:?}");
    }
    ...
    match video_type {
        ...
        VideoType::VideoStream => {
            ...
            let mut annexb_stream_state = dl_video::AnnexBStreamState::default();
            ...
            for (sample_idx, sample) in video_data_description.samples.iter().enumerate() {
                let (codec, sample_bytes) = match video_data_description.codec {
                    VideoCodec::H264 => {
                        ...
                        dl_video::write_avc_chunk_to_nalu_stream(
                            avcc,
                            &mut sample_bytes,
                            &sample
                                .sample()
                    ...
                    VideoCodec::H265 => {
                        ...
                        let mut sample_bytes = Vec::new();
                        dl_video::write_hevc_chunk_to_nalu_stream(
                            hvcc,
                            &mut sample_bytes,
                            &sample
                                .sample()
    ...
    let view_id = test_context.setup_viewport_blueprint(|ctx, blueprint| {
        ...
        let property = ViewProperty::from_archetype_for_view::<
            dl_sdk_types::blueprint::archetypes::Background,
        >(ctx, view_id);
        property.save_blueprint_component(
            ctx,
            &dl_sdk_types::blueprint::archetypes::Background::descriptor_kind(),
            &dl_sdk_types::blueprint::components::BackgroundKind::SolidColor,
        );
        property.save_blueprint_component(
            ctx,
            &dl_sdk_types::blueprint::archetypes::Background::descriptor_color(),
            &dl_sdk_types::components::Color::from_rgb(200, 100, 200),
        );
