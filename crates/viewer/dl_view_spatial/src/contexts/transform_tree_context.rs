...
use dl_sdk_types::{ArchetypeName, archetypes, blueprint};
use dl_tf::{TransformFrameId, TransformFrameIdHash, TreeTransform};
use dl_view::{
...
pub struct TransformTreeContext {
    transform_forest: Arc<dl_tf::TransformForest>,
    transform_infos: IntMap<EntityPathHash, Result<TransformInfo, dl_tf::TransformFromToError>>,
...
impl Default for TransformTreeContext {
    fn default() -> Self {
        Self {
            transform_forest: Arc::new(dl_tf::TransformForest::default()),
...
impl ViewContextSystem for TransformTreeContext {
    ...
    fn execute(
        ...
        missing_chunk_reporter: &dl_viewer_context::MissingChunkReporter,
        query: &dl_viewer_context::ViewQuery<'_>,
        ...
        let frame_id_results = {
            dl_tracing::profile_scope!("frame id queries");
        ...
        self.target_frame = {
            dl_tracing::profile_scope!("target_frame");
            ...
            match target_frame_component {
                ...
                Err(err) => {
                    dl_log::error_once!("Failed to query target frame: {err}");
        ...
        {
            dl_tracing::profile_scope!("add-overrides");
        ...
        let lookup_image_plane_distance = |transform_frame_id_hash: TransformFrameIdHash| -> f64 {
            dl_tracing::profile_scope!("image-plane-distance");
        ...
        let tree_transforms_per_frame = {
            dl_tracing::profile_scope!("transform_from_to");
        ...
        self.transform_infos = {
            dl_tracing::profile_scope!("transform info lookup");
...
fn map_tree_transform_to_transform_info(
    ...
    tree_transform: &Result<TreeTransform, dl_tf::TransformFromToError>,
    transforms: &dl_tf::CachedTransformsForTimeline,
    latest_at_query: &LatestAtQuery,
    entity_path_hash: &EntityPathHash,
) -> Result<TransformInfo, dl_tf::TransformFromToError> {
...
impl TransformTreeContext {
    ...
    pub fn target_from_entity_path(
        &self,
        entity_path_hash: EntityPathHash,
    ) -> Option<&Result<TransformInfo, dl_tf::TransformFromToError>> {
    ...
    #[inline]
    pub fn transform_forest(&self) -> &dl_tf::TransformForest {
        &self.transform_forest
    }
...
mod tests {
    ...
    use dl_sdk_types::archetypes::CoordinateFrame;
    use dl_test_context::TestContext;
    use dl_test_viewport::TestContextExt as _;
    use dl_tf::{TransformFrameId, TransformFrameIdHash};
    use dl_viewer_context::{
    ...
    fn execute_transform_tree_context(
        ...
        ctx: &dl_viewer_context::ViewerContext<'_>,
        view_id: dl_viewer_context::ViewId,
        ...
        let view_query = dl_viewport::new_view_query(ctx, &view_blueprint);
    ...
    fn test_frame_id_lookup_for_overrides_and_target() {
        ...
        let view_with_target = test_context.setup_viewport_blueprint(|ctx, blueprint| {
            ...
            let property = ViewProperty::from_archetype_for_view::<
                dl_sdk_types::blueprint::archetypes::SpatialInformation,
            >(ctx, view_id);
            property.save_blueprint_component(
                ctx,
                &dl_sdk_types::blueprint::archetypes::SpatialInformation::descriptor_target_frame(),
