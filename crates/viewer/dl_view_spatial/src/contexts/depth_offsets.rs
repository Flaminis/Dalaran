...
use dl_sdk_types::components::DrawOrder;
...
/// Context for creating a mapping from [`DrawOrder`] to [`dl_renderer::DepthOffset`].
#[derive(Default)]
pub struct EntityDepthOffsets {
    pub per_entity_and_visualizer:
        HashMap<(ViewSystemIdentifier, EntityPathHash), dl_renderer::DepthOffset>,
}
...
impl ViewContextSystem for EntityDepthOffsets {
    fn execute(
        ...
        _once_per_frame_result: &dl_viewer_context::ViewContextSystemOncePerFrameResult,
    ) {
        ...
        // Determine dl_renderer draw order from this.
        //
        ...
        let mut depth_offset =
            -(num_entities_with_draw_order / 2).saturating_cast::<dl_renderer::DepthOffset>();
...
fn collect_draw_order_per_visualizer(
    ...
    draw_order_descriptor: &dl_sdk_types::ComponentDescriptor,
...
fn determine_default_draworder(
    ctx: &QueryContext<'_>,
    draw_order_component: dl_sdk_types::ComponentIdentifier,
) -> DrawOrder {
    dl_viewer_context::typed_fallback_for(ctx, draw_order_component)
}
