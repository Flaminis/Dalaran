...
use dl_sdk_types::components::{LinearSpeed, Position3D, Vector3D};
...
#[derive(
    Clone, Copy, Debug, PartialEq, serde::Deserialize, serde::Serialize, dl_byte_size::SizeBytes,
)]
pub struct Eye {
...
#[derive(Default, Clone, Debug, PartialEq, dl_byte_size::SizeBytes)]
pub struct EyeState {
...
impl EyeController {
    ...
    fn handle_gamepad_navigation(
        ...
        dl_gamepad::set_event_waker(move || repaint_ctx.request_repaint());

        let dt = egui_ctx.input(|input| input.stable_dt.at_most(0.1));
        let Some(navigation) = dl_gamepad::navigation_from_active_gamepad(dt) else {
            return GamepadNavigationStatus::Disconnected;
        };
...
impl EyeState {
    ...
    fn control_and_sync_with_blueprint(
        ...
        let tracking_entity = eye_property
            .component_or_empty::<dl_sdk_types::components::EntityPath>(
    ...
    fn handle_tracking_entity(
        ...
        old_look_target: Vec3,
        old_eye_up: Vec3,
        tracking_entity: Option<&dl_sdk_types::components::EntityPath>,
    ) -> Option<Eye> {
        if let Some(tracking_entity) = &tracking_entity {
            let tracking_entity = EntityPath::from(tracking_entity.as_str());
