...
use dl_renderer::{LineDrawableBuilder, ViewPickingConfiguration};
use dl_sdk_types::blueprint::archetypes::{
    Background, NearClipPlane, SpatialInformation, VisualBounds2D,
};
use dl_sdk_types::blueprint::components::{self as blueprint_components, Enabled};
use dl_sdk_types::{Archetype as _, archetypes};
use dl_tf::TransformFrameIdHash;
use dl_ui::{ContextExt as _, Help, MouseButtonText, icons};
use dl_view::controls::DRAG_PAN2D_BUTTON;
...
pub fn help(os: egui::os::OperatingSystem) -> Help {
    ...
    Help::new("2D view")
        ...
        .control(
            "Zoom",
            dl_ui::IconText::from_modifiers_and(os, zoom_modifier, icons::SCROLL),
        )
...
impl SpatialView2D {
    pub fn view_2d(
        ...
        // Draw a dl_renderer driven view.
...
fn setup_target_config(
    render_mode: dl_renderer::RenderMode,
    ...
    let projection_from_view = dl_renderer::view_builder::Projection::Perspective {
    ...
    let mut viewport_transformation = dl_renderer::RectTransform {
    ...
    Ok({
        ...
        TargetConfiguration {
            ...
            pixels_per_point,
            outline_config: any_outlines.then(|| dl_view::outline_config(egui_painter.ctx())),
...
fn re_render_rect_from_egui_rect(rect: egui::Rect) -> dl_renderer::RectF32 {
    dl_renderer::RectF32 {
