...
use dl_sdk_types::blueprint::components::{Enabled, GridSpacing};
use dl_sdk_types::components::{ViewCoordinates, Visible};
use dl_tf::image_view_coordinates;
use dl_ui::{ContextExt as _, Help, IconText, MouseButtonText, UiExt as _, icons};
use dl_view::controls::{
...
impl SpatialView3D {
    pub fn view_3d(
        ...
        query: &ViewQuery<'_>,
        mut system_output: dl_viewer_context::SystemExecutionOutput,
    ) -> Result<dl_viewer_context::ViewClassUiOutput, ViewSystemExecutionError> {
        dl_tracing::profile_function!();

        let mut view_ui_output = dl_viewer_context::ViewClassUiOutput::default();
        ...
        line_builder.radius_boost_in_ui_points_for_outlines(
            dl_view::SIZE_BOOST_IN_POINTS_FOR_LINE_OUTLINES,
        );
        ...
        let target_config = TargetConfiguration {
            ...
            outline_config: query
                .highlights
                .any_outlines()
                .then(|| dl_view::outline_config(ui.ctx())),
            blend_with_background: dl_renderer::BlendWithBackground::No,
            picking_config,
        };
                    ...
                    || find_camera(space_cameras, entity_path).is_some()
                {
                    if state.last_tracked_entity() != Some(entity_path) {
                        eye_property.save_blueprint_component(
                            ...
                            &dl_sdk_types::components::EntityPath::from(entity_path),
                        );
        ...
        // Add egui-rendered loading indicators on top of dl_renderer content:
    ...
    fn setup_grid_3d(
    ...
    ) -> Result<Option<dl_renderer::renderer::WorldGridDrawData>, ViewSystemExecutionError> {
        if !**grid_config
        ...
        let thickness_ui = **grid_config
            .component_or_fallback::<dl_sdk_types::components::StrokeWidth>(
                ctx,
                LineGrid3D::descriptor_stroke_width().component,
            )?;
        let color = grid_config.component_or_fallback::<dl_sdk_types::components::Color>(
            ctx,
            LineGrid3D::descriptor_color().component,
        )?;
        let plane = grid_config.component_or_fallback::<dl_sdk_types::components::Plane3D>(
            ctx,
            LineGrid3D::descriptor_plane().component,
        )?;

        Ok(Some(dl_renderer::renderer::WorldGridDrawData::new(
            ctx.render_ctx(),
            &dl_renderer::renderer::WorldGridConfiguration {
                color: color.into(),
                plane: plane.into(),
                spacing,
                thickness_ui,
...
pub fn validate_view_coordinates(coordinates: &mut ViewCoordinates) -> Option<ViewerDiagnostic> {
    match coordinates.handedness() {
        Ok(dl_sdk_types::view_coordinates::Handedness::Right) => None,

        Ok(dl_sdk_types::view_coordinates::Handedness::Left) => Some(ViewerDiagnostic {
...
fn show_orbit_eye_center(
    ...
    if orbit_center_fade > 0.001 {
        ...
        // something dl_renderer doesn't support yet for primitives within renderers.
...
fn show_projections_from_2d_space(
    line_builder: &mut dl_renderer::LineDrawableBuilder<'_>,
...
#[test]
fn test_help_view() {
    dl_test_context::TestContext::test_help_view(help);
}
