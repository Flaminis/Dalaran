...
use dl_view::{view_property_ui, view_property_ui_with_hidden_components};
...
type ViewType = dl_sdk_types::blueprint::views::Spatial2DView;
...
impl ViewClass for SpatialView2D {
        ...
        system_registry.register_fallback_provider(
            ...
            |ctx| {
                let Ok(view_state) = ctx.view_state().downcast_ref::<SpatialViewState>() else {
                    return dl_sdk_types::blueprint::components::VisualBounds2D::default();
                };
                ...
                if valid_bound(&default_scene_rect) {
                    ...
                    dl_sdk_types::blueprint::components::VisualBounds2D::default()
                }
            },
        );
    ...
    fn selection_ui(
        ...
        dl_ui::list_item::list_item_scope(ui, "spatial_view2d_selection_ui", |ui| {
...
fn recommended_views_with_image_splits(
    ...
    let Some(subtree) = tree.subtree(recommended_origin) else {
        dl_log::debug_warn_once!("Ancestor of entity not found in entity tree.");
        return;
    };
