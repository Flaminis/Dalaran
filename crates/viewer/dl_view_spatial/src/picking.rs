...
use dl_renderer::PickingLayerProcessor;

use crate::PickableTexturedRect;
use crate::eye::Eye;

#[derive(Clone, PartialEq, Eq, dl_byte_size::SizeBytes)]
...
pub struct PickingRayHit {
    ...
    pub depth_offset: dl_renderer::DepthOffset,
...
impl PickingContext {
    ...
    pub fn pick<'a>(
        ...
        gpu_readback_identifier: dl_renderer::GpuReadbackIdentifier,
...
fn picking_gpu(
    ...
    if let Some(gpu_picking_result) = gpu_picking_result {
        ...
        if picked_id == dl_renderer::PickingLayerId::default() {
            // Nothing found.
            return None;
        }
        ...
        Some(PickingRayHit {
            instance_path_hash: dl_view::instance_path_hash_from_picking_layer_id(picked_id),
