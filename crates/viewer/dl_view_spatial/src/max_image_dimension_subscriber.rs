...
use dl_sdk_types::external::image;
use dl_sdk_types::{
...
impl dl_byte_size::SizeBytes for ImageTypes {
...
impl dl_byte_size::MemUsageTreeCapture for MaxImageDimensionsStoreSubscriber {
    fn capture_mem_usage_tree(&self) -> dl_byte_size::MemUsageTree {
        ...
        dl_byte_size::MemUsageTree::Bytes(
...
impl PerStoreChunkSubscriber for MaxImageDimensionsStoreSubscriber {
    ...
    fn on_events<'a>(&mut self, events: impl Iterator<Item = &'a dl_chunk_store::ChunkStoreEvent>) {
...
fn try_size_from_blob(
    ...
    if archetype_name == archetypes::EncodedImage::name() {
        dl_tracing::profile_scope!("image");
        ...
        dl_tracing::profile_scope!("encoded_depth_image");

        let media_type = components::MediaType::or_guess_from_data(media_type, blob);

        if media_type == Some(components::MediaType::rvl()) {
            dl_rvl::RosRvlMetadata::parse(blob)
                .ok()
        ...
        dl_tracing::profile_scope!("video asset");

        let media_type = components::MediaType::or_guess_from_data(media_type, blob)?;
        dl_video::VideoDataDescription::load_from_bytes(blob, media_type.as_str(), debug_name)
            .ok()
...
fn try_size_from_video_stream_sample(
    ...
    let codec = match video_codec {
        components::VideoCodec::H264 => dl_video::VideoCodec::H264,
        components::VideoCodec::H265 => dl_video::VideoCodec::H265,
        components::VideoCodec::AV1 => dl_video::VideoCodec::AV1,
        components::VideoCodec::VP8 => dl_video::VideoCodec::VP8,
        components::VideoCodec::VP9 => dl_video::VideoCodec::VP9,
    };

    match dl_video::detect_gop_start(sample, codec).ok()? {
        dl_video::GopStartDetection::StartOfGop(descr) => Some([
            descr.coded_dimensions[0] as _,
            descr.coded_dimensions[1] as _,
        ]),
        dl_video::GopStartDetection::NotStartOfGop => None,
    }
}
