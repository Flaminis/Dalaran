...
use dl_sdk_types::components::{AxisLength, ShowLabels};
...
impl VisualizerSystem for TransformAxes3DVisualizer {
    ...
    fn execute(
        ...
        let latest_at_query = dl_chunk_store::LatestAtQuery::new(query.timeline, query.latest_at);

        // Counting all transforms ahead of time is a bit wasteful, but we also don't expect a huge amount,
        // so let dl_renderer's allocator internally decide what buffer sizes to pick & grow them as we go.
        let mut line_builder = dl_renderer::LineDrawableBuilder::new(ctx.viewer_ctx.render_ctx());
        ...
        for (data_result, instruction) in query.iter_visualizer_instruction_for(Self::identifier())
        {
            ...
            match coordinate_frame_transform_result {
                ...
                Some(Err(dl_tf::TransformFromToError::NoPathBetweenFrames { src, .. }))
                ...
                transforms_to_draw.iter().enumerate()
            {
                if show_frame_label {
                    if let Some(frame_id) = transforms.lookup_frame_id(*label_id_hash) {
                        ...
                        dl_log::debug_panic!("unable to resolve frame id hash {label_id_hash:?}");
                        output.report_unspecified_source(
                            instruction.id,
                            ViewerReportSeverity::Error,
...
pub fn add_axis_arrows(
    ...
    axis_length: f32,
    axes: Axes,
    outline_mask_ids: dl_renderer::OutlineMaskPreference,
    instance_index: u64,
) {
    use dl_renderer::renderer::LineStripFlags;

    let line_radius = dl_renderer::Size::new_ui_points(1.0);
    ...
    let mut line_batch = line_builder
        ...
        .picking_object_id(dl_renderer::PickingLayerObjectId(
            ent_path.map_or(0, |p| p.hash64()),
        ));
    let picking_instance_id = dl_renderer::PickingLayerInstanceId(instance_index);
