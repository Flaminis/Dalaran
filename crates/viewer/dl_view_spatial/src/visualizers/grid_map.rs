...
impl GridMapColorMode {
    fn colormap(&self) -> Option<&dl_viewer_context::ColormapWithRange> {
        match self {
...
impl GridMapVisualizer {
    fn process_grid_map(
        ...
        let grid_maps = dl_query::range_zip_1x7(
    ...
    fn textured_rect_from_grid_map(
        ...
        let image_stats =
            caches.memoizer_read_or_compute::<dl_viewer_context::ImageStatsCache, _, _>(&image);
        ...
        let multiplicative_tint = dl_renderer::Rgba::from_white_alpha(opacity.0.clamp(0.0, 1.0));
    ...
    fn color_mode_for_grid_map(
        ...
        if matches!(
            ...
            dl_sdk_types::datatypes::ChannelDatatype::U8
        ) {
            results.report_for_component(
                GridMap::descriptor_colormap().component,
        ...
        let image_stats = caches
            .memoizer_read_or_compute::<dl_viewer_context::ImageStatsCache, _, _>(
