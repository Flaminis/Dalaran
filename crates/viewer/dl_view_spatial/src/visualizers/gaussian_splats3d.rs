...
/// Renders [`GaussianSplats3D`] via [`dl_renderer`]'s gaussian splat renderer.
///
...
struct GaussianSplats3DComponentData<'a> {
    index: (dl_log_types::TimeInt, dl_chunk_store::RowId),
...
struct GaussianSplats3DCpu {
    ...
    point_cloud_bounds: dl_renderer::util::PointCloudBounds,

    /// Scratch buffers holding the back-to-front gaussian ordering, reused across frames to speed
    /// up the per-frame CPU sort (see [`dl_renderer::GaussianSplatBuilder`]).
    ///
...
impl GaussianSplats3DCpu {
    fn compute(ctx: &QueryContext<'_>, data: &GaussianSplats3DComponentData<'_>) -> Self {
        ...
        dl_tracing::profile_function!(dl_format::format_uint(num_instances));

        let picking_ids = {
            dl_tracing::profile_scope_if!(100_000 < num_instances, "picking_ids");
            (0..num_instances)
        ...
        let rotations: Vec<glam::Quat> = {
            dl_tracing::profile_scope_if!(100_000 < num_instances, "rotations");
            data.quaternions
                .iter()
        ...
        let colors: Vec<Rgba32Unmul> = {
            dl_tracing::profile_scope_if!(100_000 < num_instances, "colors");
        ...
        let sh_coefficients: Vec<[GaussianShCoefficient; 15]> = {
            dl_tracing::profile_scope_if!(100_000 < num_instances, "sh_coefficients");
        ...
        let point_cloud_bounds = {
            dl_tracing::profile_scope_if!(100_000 < num_instances, "bounding_box");
            ...
            if opaque_centers.is_empty() {
                dl_renderer::util::point_cloud_bounds(&centers)
            } else {
                dl_renderer::util::point_cloud_bounds(&opaque_centers)
            }
        };
...
impl Cache for GaussianSplats3DCache {
    ...
    fn on_store_events(
        &mut self,
        _events: &[&dl_chunk_store::ChunkStoreEvent],
        _entity_db: &EntityDb,
    ) {
    }
}

impl dl_byte_size::SizeBytes for GaussianSplats3DCache {
...
impl dl_byte_size::MemUsageTreeCapture for GaussianSplats3DCache {
    fn capture_mem_usage_tree(&self) -> dl_byte_size::MemUsageTree {
        dl_byte_size::MemUsageTree::Bytes(self.total_size_bytes())
    }
}
    ...
    fn process_data<'a>(
        ...
        for data in data {
            ...
            for (transform_index, world_from_obj) in ent_context
                ...
                .enumerate()
            {
                dl_tracing::profile_scope!("one-transform");
    ...
    fn execute(
        ...
        process_archetype::<GaussianSplats3D, _, _>(
            ...
            |ctx, spatial_ctx, results| {
                dl_tracing::profile_scope!("GaussianSplats3D");
                ...
                let num_centers: usize = {
                    dl_tracing::profile_scope!("num_centers");
                    all_centers
                        .chunks()
                        .iter()
                ...
                let results_iter = dl_query::range_zip_1x5(
