...
use dl_log_types::{Instance, TimeInt};
use dl_renderer::RenderContext;
use dl_renderer::renderer::GpuMeshInstance;
use dl_sdk_types::Archetype as _;
use dl_sdk_types::archetypes::Mesh3D;
use dl_sdk_types::components::{ImageFormat, Position3D};
...
struct Mesh3DComponentData<'a> {
    ...
    cull_mode: Option<dl_renderer::external::wgpu::Face>,
}
...
impl Mesh3DVisualizer {
    fn process_data<'a>(
        ...
        for mesh_entry in mesh_data {
            ...
            let picking_instance_hash = dl_entity_db::InstancePathHash::entity_all(entity_path);
            ...
            // but dl_renderer insists on having at a non-zero vertex list.
            ...
            let mesh = ctx.store_ctx().memoizer(|c: &mut MeshCache| {
                ...
                c.entry(
                    ...
                    AnyMesh::Mesh {
                        ...
                        texture_key: dl_log_types::hash::Hash64::hash(&key).hash64(),
                    },
                    render_ctx,
                )
            });
                ...
                for &world_from_instance in ent_context.transform_info.target_from_instances() {
                    ...
                    instances.extend(mesh.mesh_instances.iter().map(move |mesh_instance| {
                        ...
                        GpuMeshInstance {
                            ...
                            additive_tint: dl_renderer::Color32::BLACK,
    ...
    fn execute(
        ...
        process_archetype::<Mesh3D, _, _>(
            ...
            |ctx, spatial_ctx, results| {
                ...
                let mesh_data = dl_query::range_zip_1x8(
        ...
        Ok(output
            .with_draw_data([dl_renderer::renderer::MeshDrawData::new(
...
fn face_rendering_to_wgpu_cull(value: u8) -> Option<dl_renderer::external::wgpu::Face> {
    use dl_sdk_types::components::MeshFaceRendering;
    ...
    match MeshFaceRendering::try_from_integer(value)? {
        ...
        MeshFaceRendering::Front => Some(dl_renderer::external::wgpu::Face::Back),
        // To show only back faces, cull front faces.
        MeshFaceRendering::Back => Some(dl_renderer::external::wgpu::Face::Front),
    }
}
