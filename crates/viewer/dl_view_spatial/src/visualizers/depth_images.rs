...
use dl_renderer::renderer::{ColormappedTexture, DepthCloud, DepthClouds};
use dl_sdk_types::archetypes::DepthImage;
use dl_sdk_types::components::{
    Colormap, DepthMeter, FillRatio, ImageBuffer, ImageFormat, MagnificationFilter,
};
use dl_sdk_types::{Archetype as _, ArchetypeName, ComponentIdentifier};
...
pub fn process_depth_image_data(
    ...
    let textured_rect = match textured_rect_from_image(
        ...
        dl_renderer::Rgba::WHITE,
        magnification_filter,
        archetype_name,
    ) {
        Ok(textured_rect) => textured_rect,
        Err(err) => {
            report_error(dl_error::format(err));
...
fn process_entity_view_as_depth_cloud(
    ent_context: &SpatialSceneVisualizerInstructionContext<'_>,
    ent_path: &EntityPath,
    pinhole_tree_root_info: &dl_tf::PinholeTreeRoot,
    ...
    DepthCloud {
        ...
        colormap: match depth_texture.color_mapper {
            dl_renderer::renderer::ColorMapper::Function(colormap) => colormap,
            _ => dl_renderer::Colormap::Grayscale,
        },
                ...
                for (
                    ...
                    let component_data = DepthImageComponentData {
                        image: ImageInfo::from_stored_blob(
                            ...
                            buffer.clone().into(),
                            format.0,
                            dl_sdk_types::image::ImageKind::Depth,
                        ),
...
pub fn populate_depth_visualizer_execution_result(
    ...
    let depth_cloud = dl_renderer::renderer::DepthCloudDrawData::new(
