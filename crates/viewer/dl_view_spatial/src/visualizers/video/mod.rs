...
use dl_sdk_types::components::Opacity;
use dl_ui::ContextExt as _;
use dl_video::player::{VideoPlaybackIssueSeverity, VideoPlayerError};
...
type GetCodecFn = dyn Fn(
    &ViewContext<'_>,
    &dl_chunk_store::LatestAtQuery,
    &dl_viewer_context::DataResult,
    &dl_viewer_context::VisualizerInstruction,
    &VisualizerExecutionOutput,
) -> Result<dl_video::VideoCodec, VideoStreamProcessingError>;
...
pub(super) struct DepthTextureConfig {
    pub colormap: dl_renderer::Colormap,
    pub range: [f32; 2],
    pub depth_meter: dl_sdk_types::components::DepthMeter,
    pub fill_ratio: dl_sdk_types::components::FillRatio,
}
...
struct VideoStreamCtx<'a> {
    ...
    opacity_component: Option<dl_sdk_types::ComponentIdentifier>,
...
impl<'a> VideoStreamCtx<'a> {
    ...
    pub fn with_opacity_component(
        mut self,
        opacity_component: dl_sdk_types::ComponentIdentifier,
    ) -> Self {
...
fn execute_video_stream_like(
        ...
        .iter_visualizer_instruction_for(ctx.visualizer_name)
    {
        ...
        dl_tracing::profile_scope!("Entity", entity_path.to_string().as_str());
        ...
        let opacity = ctx.opacity_component.map(|opacity_component| {
            ...
            results
                ...
                .unwrap_or_else(|| {
                    typed_fallback_for(
                        &dl_viewer_context::QueryContext {
                            view_ctx: &ctx,
                ...
                .and_then(|t| t.texture.as_ref())
        {
                ...
                && let Some(world_from_view) = transforms.target_from_pinhole_root(tree_root_frame)
            {
                ...
                if dimensions.x == 0 || dimensions.y == 0 {
                    ...
                    let cloud = dl_renderer::renderer::DepthCloud {
                        world_from_rdf,
                        ...
                        picking_object_id: dl_renderer::PickingLayerObjectId(entity_path.hash64()),
                    };
...
fn video_stream_id(
    ...
    sample_component: dl_sdk_types::ComponentIdentifier,
    time_track_salt: u64,
) -> dl_video::player::VideoPlayerStreamId {
    dl_video::player::VideoPlayerStreamId(
        dl_log_types::hash::Hash64::hash((entity_path.hash(), sample_component, time_track_salt))
            .hash64(),
    )
}
...
struct VideoFrameRenderInfo {
    texture: dl_renderer::video::VideoFrameTexture,
    depth_offset: dl_renderer::DepthOffset,
...
fn show_video_frame(
    ...
    highlight: &dl_viewer_context::ViewOutlineMasks,
    ...
    if let Some(reason) = loading_indicator_reason {
        ctx.viewer_ctx.command_sender().send_system(
            dl_viewer_context::SystemCommand::TimeControlCommands {
                store_id: ctx.store_id().clone(),
                time_commands: vec![dl_viewer_context::TimeControlCommand::Buffer],
            },
        );
    ...
    if let Some(frame) = frame {
        let dl_renderer::video::VideoFrameTexture {
            texture,
            decoder_delay_state,
    ...
    let video_error_image = match dl_ui::icons::VIDEO_ERROR
        .load_image(ctx.viewer_ctx.egui_ctx(), egui::SizeHint::default())
    {
        Err(err) => {
            dl_log::error_once!("Failed to load video error icon: {err}");
            return;
        }
        ...
        .get_or_try_create_with::<image::ImageError>(
            ...
            || {
                Ok(ImageDataDesc {
                    ...
                    format: dl_renderer::external::wgpu::TextureFormat::Rgba8UnormSrgb.into(),
                    width_height: [
                        video_error_image.width() as _,
                        video_error_image.height() as _,
                    ],
                    alpha_channel_usage: dl_renderer::AlphaChannelUsage::AlphaChannelInUse,
                })
            },
        );

    let Ok(video_error_texture) = video_error_texture_result.inspect_err(|err| {
        dl_log::error_once!("Failed to show video error icon: {err}");
    }) else {
        return;
    };
    ...
    visualizer_data.ui_labels.push(UiLabel {
        ...
        labeled_instance: dl_entity_db::InstancePathHash::entity_all(entity_path),
