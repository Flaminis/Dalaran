...
use dl_sdk_types::components::{Blob, Colormap, DepthMeter, FillRatio, MediaType, ValueRange};
use dl_view::{DataResultQuery as _, latest_at_with_blueprint_resolved_data};
    ...
    fn execute(
        ...
        let get_depth_config: &crate::visualizers::video::GetDepthConfigFn =
            &|ctx, latest_at, data_result, instruction, output| {
                ...
                let query_ctx = dl_viewer_context::QueryContext {
                    view_ctx: ctx,
