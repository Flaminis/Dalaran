...
use dl_renderer::video::Video;
use dl_sdk_types::Archetype as _;
use dl_sdk_types::archetypes::{AssetVideo, VideoFrameReference};
use dl_sdk_types::components::{Blob, MediaType, Opacity, VideoTimestamp};
    ...
    fn execute(
        ...
        process_archetype::<VideoFrameReference, _, _>(
            ...
            |ctx, spatial_ctx, results| {
                ...
                for (_index, video_timestamps, video_references, opacity) in dl_query::range_zip_1x2(
...
impl VideoFrameReferenceVisualizer {
    fn process_video_frame(
        ...
        video_references: Option<Vec<dl_sdk_types::ArrowString>>,
        opacity: Opacity,
            ...
            Some((video, video_buffer)) => match video.as_ref() {
                Ok(video) => {
                    ...
                    let multiplicative_tint =
                        dl_renderer::Rgba::from_white_alpha(opacity.0.clamp(0.0, 1.0));
...
/// For this, we'd need a [`dl_viewer_context::DataResult`] instead of merely a [`EntityPath`].
///
