...
use dl_sdk_types::{ComponentDescriptor, ComponentIdentifier, archetypes};
...
use dl_sdk_types::datatypes::{KeypointId, KeypointPair};
use dl_view::clamped_or_else;
...
pub type Keypoints =
    HashMap<(dl_sdk_types::components::ClassId, i64), HashMap<KeypointId, glam::Vec3>>;
...
/// List of all visualizers that read [`dl_sdk_types::components::DrawOrder`] and the exact draw order component descriptor they use.
...
/// Process [`dl_sdk_types::components::Radius`] components to [`dl_renderer::Size`] using auto size
/// where no radius is specified.
pub fn process_radius_slice(
    ctx: &QueryContext<'_>,
    entity_path: &EntityPath,
    num_instances: usize,
    radii: &[dl_sdk_types::components::Radius],
    component: ComponentIdentifier,
) -> Vec<dl_renderer::Size> {
    dl_tracing::profile_function!();

    clamped_or_else(radii, || {
        // TODO(RR-3840): Fallback should be already incorporated into the query.
        typed_fallback_for::<dl_sdk_types::components::Radius>(ctx, component)
    })
    .take(num_instances)
...
fn process_radius(
    entity_path: &EntityPath,
    radius: dl_sdk_types::components::Radius,
) -> dl_renderer::Size {
    ...
    if radius.0.is_infinite() {
        dl_log::warn_once!("Found infinite radius in entity {entity_path}");
    } else if radius.0.is_nan() {
        dl_log::warn_once!("Found NaN radius in entity {entity_path}");
    }

    dl_renderer::Size(*radius.0)
}
...
pub fn load_keypoint_connections(
    ...
    ent_path: &dl_entity_db::EntityPath,
    keypoints: &Keypoints,
    ...
    let mut line_batch = line_builder
        ...
        .picking_object_id(dl_renderer::PickingLayerObjectId(ent_path.hash64()));

    // TODO(andreas): Make configurable. Should we pick up the point's radius and make this proportional?
    let line_radius = dl_renderer::Size(*dl_sdk_types::components::Radius::default().0);
    ...
    for ((class_id, _time), keypoints_in_class) in keypoints {
        ...
        } in &class_description.keypoint_connections
        {
            ...
            line_batch
                ...
                .radius(line_radius)
                .color(color)
                .flags(dl_renderer::renderer::LineStripFlags::STRIP_FLAG_COLOR_GRADIENT)
                // Select the entire object when clicking any of the lines.
                .picking_instance_id(dl_renderer::PickingLayerInstanceId(
                    dl_log_types::Instance::ALL.get(),
                ));
        }
    }
