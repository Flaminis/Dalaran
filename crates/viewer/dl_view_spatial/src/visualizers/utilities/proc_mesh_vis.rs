...
use dl_renderer::renderer::{GpuMeshInstance, LineStripFlags};
use dl_renderer::{PickingLayerInstanceId, RenderContext};
use dl_sdk_types::ComponentIdentifier;
use dl_sdk_types::components::{self, FillMode};
use dl_tf::convert;
use dl_view::{clamped_or_nothing, process_annotation_slices, process_color_slice};
#[cfg(doc)]
use dl_viewer_context::VisualizerSystem;
...
/// via [`AppOptions::visualizer_limits_enabled`](dl_viewer_context::AppOptions::visualizer_limits_enabled).
...
pub struct ProcMeshDrawableBuilder<'ctx> {
    ...
    pub line_builder: dl_renderer::LineDrawableBuilder<'ctx>,
    pub line_batch_debug_label: dl_renderer::Label,
...
pub struct ProcMeshBatch<'a, IMesh, IFill> {
    ...
    pub labels: &'a [dl_sdk_types::ArrowString],
...
impl<'ctx> ProcMeshDrawableBuilder<'ctx> {
    pub fn new(
        ...
        render_ctx: &'ctx dl_renderer::RenderContext,
        view_query: &'ctx ViewQuery<'ctx>,
        output: &'ctx VisualizerExecutionOutput,
        line_batch_debug_label: impl Into<dl_renderer::Label>,
    ) -> Self {
    ...
    pub fn add_batch(
            ...
            if let Some(instruction_id) = query_context.instruction_id {
                self.output.report_unspecified_source(
                    ...
                    format!(
                        ...
                        dl_format::format_uint(num_instances),
                        dl_format::format_uint(NUM_INSTANCE_LIMIT_PER_BATCH),
                    ),
                );
            }
        ...
        dl_tracing::profile_function_if!(10_000 < num_instances);
        ...
        .enumerate()
        {
            ...
            if draw_solid {
                ...
                let tint = if fill_mode == FillMode::TransparentFillMajorWireframe {
                    ...
                    dl_renderer::Color32::from_rgba_unmultiplied(
                        color.r(),
                        color.g(),
                        color.b(),
                ...
                self.solid_instances.push(GpuMeshInstance {
                    ...
                    picking_layer_id: dl_view::picking_layer_id_from_instance_path_hash(
                        InstancePathHash::instance(entity_path, instance),
                    ),
                    additive_tint: tint,
                    cull_mode: if tint.is_opaque() {
                        Some(dl_renderer::external::wgpu::Face::Back)
                    } else {
                        None
                    },
                });
    ...
    /// Final operation. Produce the [`dl_renderer::QueueableDrawData`] to actually be drawn.
    pub fn into_draw_data(
        ...
        let wireframe_draw_data: dl_renderer::QueueableDrawData =
            line_builder.into_draw_data()?.into();

        let solid_draw_data: Option<dl_renderer::QueueableDrawData> =
            match dl_renderer::renderer::MeshDrawData::new(render_ctx, &solid_instances) {
