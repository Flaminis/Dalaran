...
use dl_sdk_types::components::{RotationAxisAngle, RotationQuat, Translation3D};
use dl_sdk_types::datatypes::Quaternion;
use dl_tf::TransformFrameIdHash;
use dl_viewer_context::{ViewClass as _, ViewerReportSeverity, VisualizerExecutionOutput};
...
pub fn entity_from_grid_transform(
    ...
    quaternion_component: dl_sdk_types::ComponentIdentifier,
    rotation_axis_angle_component: dl_sdk_types::ComponentIdentifier,
...
pub fn spatial_view_kind_from_view_class(class: ViewClassIdentifier) -> SpaceKind {
    if class == crate::SpatialView3D::identifier() {
        ...
        dl_log::debug_panic!("Not a spatial view class identifier {class:?}");
        SpaceKind::TwoD
    }
}
        ...
        output.report_unspecified_source(
            *instruction_id,
            ...
            format!(
                ...
                dl_tf::TransformFrameId::from_entity_path(entity_path).as_str(),
            ),
        );
    }
...
pub fn format_transform_info_result<'a>(
    ...
    result: Option<&'a Result<TransformInfo, dl_tf::TransformFromToError>>,
    ...
    match result {
        ...
        Some(Err(dl_tf::TransformFromToError::NoPathBetweenFrames { src, target, .. })) => {
        ...
        Some(Err(dl_tf::TransformFromToError::UnknownTargetFrame(target))) => {
            if transform_context
        ...
        Some(Err(dl_tf::TransformFromToError::UnknownSourceFrame(src))) => {
