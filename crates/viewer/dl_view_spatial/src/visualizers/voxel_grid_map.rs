...
use dl_renderer::renderer::{VoxelGridDrawData, VoxelGridInstance, VoxelGridOptions};
use dl_sdk_types::Archetype as _;
use dl_sdk_types::archetypes::VoxelGridMap;
...
impl VisualizerSystem for VoxelGridMapVisualizer {
    fn visualizer_query_info(
        ...
        VisualizerQueryInfo::single_required_component::<dl_sdk_types::components::VoxelIndex>(
    ...
    fn execute(
        ...
        process_archetype::<VoxelGridMap, _, _>(
            ...
            |ctx, spatial_ctx, results| {
                ...
                let voxel_maps = dl_query::range_zip_1x9(
            ...
            if let Some(instruction_id) = ctx.instruction_id {
                output.report_unspecified_source(
                    ...
                    format!(
                        ...
                        dl_format::format_uint(indices.len()),
                        dl_format::format_uint(NUM_VOXEL_LIMIT_PER_BATCH),
                    ),
                );
            }
    ...
    fn resolve_colors(
        ...
        if !colors.is_empty() {
            ...
            return clamped_or_nothing(colors, num_voxels)
                .map(|&color| Color32::from(dl_sdk_types::components::Color::from(color)))
                .collect();
        }
        ...
        if !values.is_empty() {
            let value_range = value_range
                ...
                .unwrap_or_else(|| {
                    let range: dl_sdk_types::components::ValueRange =
            ...
            return clamped_or_nothing(values, num_voxels)
                .map(|&value| {
                    ...
                    let [r, g, b, a] = dl_renderer::colormap_srgba(colormap, t);
        ...
        let fallback_color: dl_sdk_types::components::Color =
