...
#[derive(Debug, PartialEq, Eq, Hash, Clone, Copy, dl_byte_size::SizeBytes)]
...
#[derive(Debug, dl_byte_size::SizeBytes)]
pub struct WireframeMesh {
    ...
    /// `dl_renderer::Renderer` implementation that takes instanced meshes and applies
...
impl dl_byte_size::SizeBytes for SolidMesh {
...
impl WireframeCache {
    pub fn entry(
        ...
        self.0
            .entry(key)
            .or_insert_with(|| {
                dl_tracing::profile_scope!("proc_mesh::WireframeCache(miss)", format!("{key:?}"));

                dl_log::trace!("Generating wireframe mesh {key:?}…");
...
impl dl_byte_size::MemUsageTreeCapture for WireframeCache {
    fn capture_mem_usage_tree(&self) -> dl_byte_size::MemUsageTree {
        dl_byte_size::MemUsageTree::Bytes(self.0.total_size_bytes())
    }
}
...
impl SolidCache {
    pub fn entry(&mut self, key: ProcMeshKey, render_ctx: &RenderContext) -> Option<SolidMesh> {
        self.0
            .entry(key)
            .or_insert_with(|| {
                dl_tracing::profile_scope!("proc_mesh::SolidCache(miss)", format!("{key:?}"));

                dl_log::trace!("Generating solid mesh {key:?}…");
...
impl Cache for SolidCache {
    ...
    fn vram_usage(&self) -> dl_byte_size::MemUsageTree {
        let mut node = dl_byte_size::MemUsageNode::new();
        ...
        for (item_name, bytes_gpu) in items {
            node.add(item_name, dl_byte_size::MemUsageTree::Bytes(bytes_gpu));
        }
...
impl dl_byte_size::MemUsageTreeCapture for SolidCache {
    fn capture_mem_usage_tree(&self) -> dl_byte_size::MemUsageTree {
        dl_byte_size::MemUsageTree::Bytes(self.0.total_size_bytes())
    }
}
    ...
    let mesh: mesh::CpuMesh = match *key {
        ...
        ProcMeshKey::Sphere {
            ...
            mesh::CpuMesh {
                ...
                vertex_colors: vec![dl_renderer::Rgba32Unmul::BLACK; num_vertices],
...
fn materials_for_uncolored_mesh(
    ...
    smallvec![mesh::Material {
        ...
        albedo: render_ctx
            ...
            .clone(),
        albedo_factor: dl_renderer::Rgba::BLACK,
    }]
}
