...
use dl_sdk_types::blueprint::components::Eye3DKind;
use dl_sdk_types::components::{LinearSpeed, Plane3D, Position3D, Vector3D, ViewCoordinates};
use dl_sdk_types::datatypes::Vec3D;
use dl_sdk_types::view_coordinates::SignedAxis3;
use dl_sdk_types::{Archetype as _, Component as _, View as _, ViewClassIdentifier, archetypes};
use dl_tf::query_view_coordinates_at_closest_ancestor;
use dl_ui::{Help, UiExt as _, list_item};
...
type ViewType = dl_sdk_types::blueprint::views::Spatial3DView;
...
impl ViewClass for SpatialView3D {
    ...
    fn on_register(
        ...
        system_registry
            .register_fallback_provider(LineGrid3D::descriptor_color().component, |_| {
                dl_sdk_types::components::Color::from_unmultiplied_rgba(128, 128, 128, 60)
            });
        ...
        system_registry
            .register_fallback_provider(LineGrid3D::descriptor_stroke_width().component, |_| {
                dl_sdk_types::components::StrokeWidth::from(1.0)
            });
        ...
        system_registry.register_fallback_provider(
            ...
            |ctx| match ctx.egui_ctx().theme() {
                egui::Theme::Dark => {
                    dl_sdk_types::blueprint::components::BackgroundKind::GradientDark
                }
                egui::Theme::Light => {
                    dl_sdk_types::blueprint::components::BackgroundKind::GradientBright
                }
            },
        );
        ...
        system_registry.register_fallback_provider(
            dl_sdk_types::blueprint::archetypes::EyeControls3D::descriptor_speed().component,
            |ctx| {
                let Ok(view_state) = ctx.view_state().downcast_ref::<SpatialViewState>() else {
                    dl_log::error_once!(
        ...
        system_registry.register_fallback_provider(
            dl_sdk_types::blueprint::archetypes::EyeControls3D::descriptor_look_target().component,
            |ctx| {
                let Ok(view_state) = ctx.view_state().downcast_ref::<SpatialViewState>() else {
                    dl_log::error_once!(
        ...
        system_registry.register_fallback_provider(
            dl_sdk_types::blueprint::archetypes::EyeControls3D::descriptor_position().component,
            |ctx| {
                let Ok(view_state) = ctx.view_state().downcast_ref::<SpatialViewState>() else {
                    dl_log::error_once!(
                        "Fallback for `Position3D` queried on 3D view outside the context of a spatial view."
        ...
        system_registry.register_fallback_provider(
            dl_sdk_types::blueprint::archetypes::EyeControls3D::descriptor_eye_up().component,
            |ctx| {
                let Ok(view_state) = ctx.view_state().downcast_ref::<SpatialViewState>() else {
                    dl_log::error_once!(
    ...
    fn layout_priority(&self) -> dl_viewer_context::ViewClassLayoutPriority {
        dl_viewer_context::ViewClassLayoutPriority::High
    }
    ...
    ) -> dl_viewer_context::ViewSpawnHeuristics {
    ...
    fn selection_ui(
        &self,
        ctx: &dl_viewer_context::ViewerContext<'_>,
        ui: &mut egui::Ui,
        ...
        dl_ui::list_item::list_item_scope(ui, "spatial_view3d_selection_ui", |ui| {
    ...
    fn ui(
        ...
        state: &mut dyn ViewState,
        query: &ViewQuery<'_>,
        system_output: dl_viewer_context::SystemExecutionOutput,
...
fn view_coordinates_ui(ctx: &ViewContext<'_>, ui: &mut egui::Ui) {
    ...
    if let Some(report) = crate::ui_3d::validate_view_coordinates(&mut coordinates) {
        let text = match &report.details {
            Some(details) => dl_error::format_with_details(report.summary.clone(), details.clone()),
        ...
        ui.list_item().interactive(false).show_flat(
            ui,
            dl_ui::list_item::CustomContent::new(|ui, _| match report.severity {
                dl_viewer_context::ViewerReportSeverity::Warning => {
...
fn view_property_ui_grid3d(ctx: &ViewContext<'_>, ui: &mut egui::Ui) {
    ...
    let sub_prop_ui = |ui: &mut egui::Ui| {
        for field in &reflection.fields {
            ...
            if field.component_type == dl_sdk_types::components::Color::name() {
                dl_view::view_property_component_ui_custom(
                    &query_ctx,
                    ui,
                    &property,
                    field.display_name,
                    ...
                    &|ui| {
                        let Ok(color) = property
                            .component_or_fallback::<dl_sdk_types::components::Color>(
                                ctx,
                        ...
                        .changed()
                        {
                            let color = dl_sdk_types::components::Color::from(edit_color);
                ...
                dl_view::view_property_component_ui(
                    &query_ctx,
                    ui,
                    &property,
                    field.display_name,
