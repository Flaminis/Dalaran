...
use dl_sdk_types::{archetypes, blueprint, components};
...
pub fn register_fallbacks(system_registry: &mut dl_viewer_context::ViewSystemRegistrator<'_>) {
    ...
    system_registry.register_fallback_provider(
        ...
        |ctx| {
            dl_tracing::profile_scope!("SpatialInformation fallback");
            ...
            'scope: {
                ...
                let Some((frame_id_registry, transform_forest)) = caches
                    .memoizer_read::<dl_viewer_context::TransformDatabaseStoreCache, _>(|c| {
                ...
                query_result.tree.visit(&mut |node| {
                    ...
                    dl_tracing::profile_scope!("visit-node");
                    ...
                    let Some(root_from_frame) = node
                        ...
                        .and_then(|frame| {
                            transform_forest
                                .root_from_frame(dl_tf::TransformFrameIdHash::new(&frame))
                        })
                    else {
                        return true;
                    };
