...
#[derive(Debug, PartialEq, Eq, Hash, Clone, dl_byte_size::SizeBytes)]
...
/// Either a [`dl_sdk_types::archetypes::Asset3D`] or [`dl_sdk_types::archetypes::Mesh3D`] to be cached.
#[derive(Debug, Clone)]
pub enum AnyMesh<'a> {
    Asset {
...
impl MeshCache {
    pub fn entry(
        ...
        let entry = self
            ...
            .entry(key)
            .or_insert_with(|| {
                dl_tracing::profile_scope!("MeshCache-miss");
                let name = name.to_string();
                dl_log::trace!("Loading CPU mesh {name:?}…");
                ...
                match result {
                    ...
                    Err(err) => {
                        dl_log::warn!("Failed to load mesh {name:?}: {}", dl_error::format(&err));
                        MeshEntry {
                            mesh: None,
                            last_used_generation: 0,
                        }
...
impl dl_byte_size::MemUsageTreeCapture for MeshCache {
    fn capture_mem_usage_tree(&self) -> dl_byte_size::MemUsageTree {
        ...
        for (item_name, bytes_cpu) in items {
            node.add(item_name, dl_byte_size::MemUsageTree::Bytes(bytes_cpu));
        }
