...
pub struct NativeAsset3D<'a> {
    ...
    pub albedo_factor: Option<dl_renderer::Rgba>,
}
...
pub struct LoadedMesh {
    ...
    #[size_bytes(ignore)]
    pub mesh_instances: Vec<dl_renderer::renderer::GpuMeshInstance>,
...
impl LoadedMesh {
    ...
    fn load_asset3d(
        ...
        let mut cpu_model = match media_type.as_str() {
            MediaType::DAE => dl_renderer::importer::dae::load_dae_from_buffer(bytes, render_ctx)?,
            MediaType::GLTF | MediaType::GLB => {
                dl_renderer::importer::gltf::load_gltf_from_buffer(&name, bytes, render_ctx)?
            }
            MediaType::OBJ => dl_renderer::importer::obj::load_obj_from_buffer(bytes, render_ctx)?,
            MediaType::STL => dl_renderer::importer::stl::load_stl_from_buffer(bytes, render_ctx)?,
    ...
    fn load_mesh3d(
        ...
        let triangle_indices = if let Some(triangle_indices) = triangle_indices {
            dl_tracing::profile_scope!("copy_indices");
            triangle_indices.to_vec()
        } else {
            dl_tracing::profile_scope!("generate_indices");
        ...
        let vertex_colors = if let Some(vertex_colors) = vertex_colors {
            dl_tracing::profile_scope!("copy_colors");
            std::iter::chain(
                vertex_colors
                    .iter()
                    .map(|c| dl_renderer::Rgba32Unmul::from_rgba_unmul_array(c.to_array())),
                std::iter::repeat(dl_renderer::Rgba32Unmul::WHITE),
            )
            .take(num_positions)
            .collect::<Vec<_>>()
        } else {
            vec![dl_renderer::Rgba32Unmul::WHITE; num_positions]
        };

        let vertex_normals = if let Some(normals) = vertex_normals {
            dl_tracing::profile_scope!("collect_normals");
        ...
        let vertex_texcoords = if let Some(texcoords) = vertex_texcoords {
            dl_tracing::profile_scope!("collect_texcoords");
        ...
        let bbox = {
            dl_tracing::profile_scope!("bbox");
            dl_renderer::util::bounding_box_from_points(vertex_positions.iter().copied())
        };
        ...
        let mesh = dl_renderer::mesh::CpuMesh {
            ...
            vertex_normals,
            vertex_texcoords,
            materials: smallvec::smallvec![dl_renderer::mesh::Material {
                label: name.clone().into(),
        ...
        let mesh_instances = vec![dl_renderer::renderer::GpuMeshInstance::new(
...
fn try_get_or_create_albedo_texture(
    ...
    texture_key: u64,
    name: &str,
) -> Option<dl_renderer::resource_managers::GpuTexture2D> {
    ...
    if dl_viewer_context::gpu_bridge::required_shader_decode(
    ...
    let texture =
        dl_viewer_context::gpu_bridge::get_or_create_texture(render_ctx, texture_key, || {
    ...
    match texture {
        ...
        Err(err) => {
            dl_log::warn_once!("Failed to create mesh albedo texture for {name:?}: {err}");
            None
        }
    }
}
