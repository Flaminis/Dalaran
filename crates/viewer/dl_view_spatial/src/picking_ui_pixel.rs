...
use dl_renderer::resource_managers::GpuTexture2D;
use dl_sdk_types::datatypes::ColorModel;
use dl_sdk_types::image::ImageKind;
use dl_sdk_types::tensor_data::TensorElement;
...
pub fn textured_rect_hover_ui(
    ...
    ui: &mut egui::Ui,
    instance_path: &dl_entity_db::InstancePath,
    query: &ViewQuery<'_>,
...
pub struct TextureInteractionId<'a> {
    pub entity_path: &'a dl_log_types::EntityPath,
...
impl TextureInteractionId<'_> {
    pub fn debug_label(&self, topic: &str) -> dl_renderer::Label {
        format!("{topic}__{:?}_{}", self.entity_path, self.interaction_idx).into()
    }

    pub fn gpu_readback_id(&self) -> dl_renderer::GpuReadbackIdentifier {
        dl_log_types::hash::Hash64::hash((self.entity_path, self.interaction_idx)).hash64()
    }

    fn render_view_id(&self, topic: &str) -> dl_renderer::ViewBuilderId {
        dl_renderer::ViewBuilderId::new(
            dl_log_types::hash::Hash64::hash((self.entity_path, self.interaction_idx, topic))
                .hash64(),
        )
    }
}
    ...
    egui::Grid::new("hovered pixel properties").show(ui, |ui| {
        ...
        if let PixelValueSource::Image(image) = &pixel_value_source {
                ...
                && let Some(u16_val) = raw_value.try_as_u16()
            {
                ...
                ui.label(
                    annotations
                        .resolved_class_description(Some(dl_sdk_types::components::ClassId::from(
                            u16_val,
                        )))
                        .annotation_info()
                        .label(None)
...
struct TextureReadbackUserdata {
    ...
    readback_rect: dl_renderer::RectInt,

    /// Info about the buffer we're reading back.
    buffer_info: dl_renderer::Texture2DBufferInfo,
}
...
fn readback_pixel_from_gpu_texture(
    ...
    // TODO(andreas): Should parts of this be a utility in dl_renderer?
    // Note that before this was implemented the readback belt was private to `dl_renderer` because it is fairly advanced in its usage.
    ...
    let readback_result = readback_belt.readback_newest_available(
        ...
        |data, userdata: Box<TextureReadbackUserdata>| {
            dl_log::debug_assert!(data.len() == userdata.buffer_info.buffer_size_padded as usize);
    ...
    let readback_result = {
        ...
        struct PreviousReadbackResult {
            frame_nr: u64,
            interaction_id: dl_renderer::GpuReadbackIdentifier,
            pixel_bytes: Vec<u8>,
        }
    ...
    let readback_rect = dl_renderer::RectInt {
        min: readback_rect_min,
        extent: readback_area_size,
    };

    let buffer_info =
        dl_renderer::Texture2DBufferInfo::new(texture.format(), readback_rect.wgpu_extent());
...

    {
        ...
        if let Err(err) = readback_buffer.read_texture2d(
            ...
            dl_log::error_once!("Failed to read back texture: {err}");
        }
    }
