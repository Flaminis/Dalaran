...
use dl_data_ui::item_ui::{self, timeline_button};
use dl_log::ResultExt as _;
use dl_log_types::{EntityPath, TimelineName};
use dl_sdk_types::blueprint::archetypes::{TextLogColumns, TextLogFormat, TextLogRows};
use dl_sdk_types::blueprint::components::{Enabled, TextLogColumn, TimelineColumn};
use dl_sdk_types::blueprint::datatypes as bp_datatypes;
use dl_sdk_types::components::TextLogLevel;
use dl_sdk_types::{View as _, ViewClassIdentifier, datatypes};
use dl_ui::list_item::LabelContent;
use dl_ui::{DesignTokens, Help, UiExt as _};
...
#[derive(Clone, PartialEq, Eq, Default, dl_byte_size::SizeBytes)]
...
type ViewType = dl_sdk_types::blueprint::views::TextLogView;
...
Filter message types and toggle column visibility in a selection panel.",
    ...
    fn selection_ui(
        ...
        ui.list_item_scope("text_log_selection_ui", |ui| {
            ...
            dl_view::view_property_ui::<TextLogColumns>(&ctx, ui);
            view_property_ui_rows(&ctx, ui);
            dl_view::view_property_ui::<TextLogFormat>(&ctx, ui);
        });
    ...
    fn ui(
        ...
        .show(ui, |ui| {
            ...
            let scroll_to_row = (time_cursor_moved || anchor_moved).then(|| {
                dl_tracing::profile_scope!("search scroll time");
            ...
            ui.with_layout(egui::Layout::top_down(egui::Align::Center), |ui| {
                egui::ScrollArea::horizontal().show(ui, |ui| {
                    dl_tracing::profile_scope!("render table");
                    table_ui(
                        ctx,
                        ui,
                        state,
                ...
                for col in timeline_columns {
                    ...
                    row.col(|ui| {
                        let row_time = entry
                            .timepoint
                            .get(&timeline)
                            .map(dl_log_types::TimeInt::from)
                            .unwrap_or(dl_log_types::TimeInt::STATIC);
...
fn calc_row_height(tokens: &DesignTokens, table_style: dl_ui::TableStyle, entry: &Entry) -> f32 {
...
#[test]
fn test_help_view() {
    dl_test_context::TestContext::test_help_view(|ctx| TextView.help(ctx));
}
