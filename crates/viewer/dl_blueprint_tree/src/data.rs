...
use dl_log_types::external::dl_types_core::ViewClassIdentifier;
            ...
            .filter_map(|content| match content {
                ...
                Contents::View(view_id) => {
                    if let Some(view_blueprint) = viewport_blueprint.view(view_id) {
                        ...
                        dl_log::warn_once!("Failed to find view {view_id} in ViewportBlueprint");
                        None
                    }
                }
            })
...
impl ViewData {
    fn from_blueprint_and_filter(
        ...
        let projection_trees = projections
            .into_iter()
            .filter_map(|node| {
                dl_tracing::profile_scope!("from_data_result_and_filter");
