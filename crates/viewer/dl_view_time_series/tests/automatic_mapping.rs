...
use dl_log_types::{EntityPath, TimePoint, Timeline};
use dl_sdk_types::{
...
fn setup_store(test_context: &mut TestContext) {
    ...
    for i in 0..10 {
        ...
        let outer_struct = StructArray::from(vec![
            ...
            (
                ...
                Arc::new(dl_log_types::external::arrow::array::Float32Array::from(
                    vec![i as f32 * 15.0],
    ...
    for i in 0..10 {
        use dl_log_types::external::arrow::array::ListArray;
        use dl_log_types::external::arrow::buffer::OffsetBuffer;
...
fn visualizers_for<'a>(
    data_result_tree: &'a dl_viewer_context::DataResultTree,
    entity_path: &str,
) -> &'a [dl_viewer_context::VisualizerInstruction] {
...
) -> &dl_viewer_context::VisualizerComponentSource {
    instruction
        .component_mappings
...
fn source_component_for(instruction: &dl_viewer_context::VisualizerInstruction) -> (&str, &str) {
...
fn check_visualizer_instructions(test_context: &TestContext, view_id: ViewId) {
    ...
    // limits auto-spawned visualizers to MAX_NUM_NON_INDICATED_RECOMMENDED_VISUALIZERS_PER_ENTITY.
    {
        ...
        assert_eq!(
            instructions.len(),
            dl_view_time_series::MAX_NUM_NON_INDICATED_RECOMMENDED_VISUALIZERS_PER_ENTITY,
