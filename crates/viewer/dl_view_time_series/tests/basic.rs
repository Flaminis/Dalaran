...
use dl_chunk::{Chunk, ChunkBuilder};
use dl_log_types::external::arrow::array::{Array, Float64Array, StringArray, StructArray};
use dl_log_types::external::arrow::datatypes::{DataType, Field};
use dl_log_types::{EntityPath, TimeInt, TimePoint, Timeline};
use dl_sdk_types::blueprint::{archetypes::PlotLegend, components::Corner2D};
use dl_sdk_types::{
    Component as _, ComponentDescriptor, DynamicArchetype, RowId, SerializedComponentBatch,
};
use dl_test_context::TestContext;
use dl_test_context::external::egui_kittest::SnapshotResults;
use dl_test_viewport::TestContextExt as _;
use dl_view_time_series::TimeSeriesView;
use dl_viewer_context::{BlueprintContext as _, TimeControlCommand, ViewClass as _, ViewId};
use dl_viewport_blueprint::{ViewBlueprint, ViewContents, ViewProperty};

fn color_gradient0(step: i64) -> dl_sdk_types::components::Color {
    dl_sdk_types::components::Color::from_rgb((step * 8) as u8, 255 - (step * 8) as u8, 0)
}

fn color_gradient1(step: i64) -> dl_sdk_types::components::Color {
    dl_sdk_types::components::Color::from_rgb(255 - (step * 8) as u8, 0, (step * 8) as u8)
}
...
fn test_clear_series_points_and_line_impl(
    ...
    test_context.log_entity("plots/line", |builder| {
        builder.with_archetype_auto_row(
            TimePoint::default(),
            &dl_sdk_types::archetypes::SeriesLines::new(),
        )
    });
    test_context.log_entity("plots/point", |builder| {
        builder.with_archetype_auto_row(
            TimePoint::default(),
            &dl_sdk_types::archetypes::SeriesPoints::new()
                .with_markers([dl_sdk_types::components::MarkerShape::Circle]),
        )
    });
        ...
        match i {
            15 => {
                test_context.log_entity("plots", |builder| {
                    builder.with_archetype_auto_row(
                        timepoint,
                        &dl_sdk_types::archetypes::Clear::new(true),
                    )
                });
            }
            16..18 => {
            ...
            _ => {
                let data = if two_series_per_entity {
                    dl_sdk_types::archetypes::Scalars::default().with_scalars([
                        (i as f64 / 5.0).sin(),
                        (i as f64 / 5.0 + 1.0).cos(), // Shifted a bit to make the cap more visible
                    ])
                } else {
                    dl_sdk_types::archetypes::Scalars::single((i as f64 / 5.0).sin())
                };
        ...
        test_context.log_entity("custom_scalar", |builder| {
            builder.with_archetype_auto_row(
                ...
                &DynamicArchetype::new("custom")
                    .with_component::<dl_sdk_types::components::Scalar>("custom_scalar", [t.sin()]),
            )
        });
            ...
            builder.with_serialized_batch(
                ...
                SerializedComponentBatch {
                    descriptor: ComponentDescriptor::partial("custom_struct")
                        ...
                        .with_component_type(dl_sdk_types::components::Scalar::name()),
...
fn scalars_for_properties_test(
    ...
    dl_sdk_types::archetypes::Scalars,
    dl_sdk_types::archetypes::Scalars,
) {
    if multiple_scalars {
        (
            dl_sdk_types::archetypes::Scalars::new([
        ...
        (
            dl_sdk_types::archetypes::Scalars::single((step as f64 / 5.0).sin()),
            dl_sdk_types::archetypes::Scalars::single((step as f64 / 5.0).cos()),
        )
    }
}
...
fn test_line_properties_impl(
    ...
    let properties_static = if multiple_properties {
        dl_sdk_types::archetypes::SeriesLines::new()
            .with_widths([4.0, 8.0])
            .with_colors([
                dl_sdk_types::components::Color::from_rgb(255, 0, 255),
                dl_sdk_types::components::Color::from_rgb(0, 255, 0),
            ])
            .with_names(["static_0", "static_1"])
    } else {
        dl_sdk_types::archetypes::SeriesLines::new()
            .with_widths([4.0])
            .with_colors([dl_sdk_types::components::Color::from_rgb(255, 0, 255)])
            .with_names(["static"])
    };
...
fn test_per_series_visibility() {
    ...
    for (name, visibility) in [
        ...
        test_context.log_entity("plots", |builder| {
            builder.with_archetype_auto_row(
                TimePoint::default(),
                &dl_sdk_types::archetypes::SeriesLines::new().with_visible_series(visibility),
            )
        });
...
const MARKER_LIST: [dl_sdk_types::components::MarkerShape; 10] = [
    dl_sdk_types::components::MarkerShape::Circle,
    dl_sdk_types::components::MarkerShape::Diamond,
    dl_sdk_types::components::MarkerShape::Square,
    dl_sdk_types::components::MarkerShape::Cross,
    dl_sdk_types::components::MarkerShape::Plus,
    dl_sdk_types::components::MarkerShape::Up,
    dl_sdk_types::components::MarkerShape::Down,
    dl_sdk_types::components::MarkerShape::Left,
    dl_sdk_types::components::MarkerShape::Right,
    dl_sdk_types::components::MarkerShape::Asterisk,
];
...
fn test_point_properties_impl(
    ...
    let static_props = if multiple_properties {
        ...
        dl_sdk_types::archetypes::SeriesPoints::new()
            ...
            .with_markers([dl_sdk_types::components::MarkerShape::Cross])
            .with_colors([dl_sdk_types::components::Color::from_rgb(255, 0, 255)])
            .with_names(["static"])
    };
...
fn test_non_finite_islands() {
    ...
    for (entity, gap_value, offset) in series {
        test_context.log_entity(entity, |builder| {
            builder.with_archetype_auto_row(
                TimePoint::default(),
                &dl_sdk_types::archetypes::SeriesLines::new().with_widths([3.0]),
            )
        });
        for i in 0..num_points {
            ...
            test_context.log_entity(entity, |builder| {
                builder.with_archetype_auto_row(
                    timepoint,
                    &dl_sdk_types::archetypes::Scalars::single(value),
                )
            });
        }
    }
...
fn test_series_lines_single_logged_point() {
    ...
    test_context.log_entity("plots/line", |builder| {
        builder.with_archetype_auto_row(
            TimePoint::default(),
            &dl_sdk_types::archetypes::SeriesLines::new().with_widths([8.0]),
        )
    });
    test_context.log_entity("plots/line", |builder| {
        builder.with_archetype_auto_row(
            [(timeline, 10)],
            &dl_sdk_types::archetypes::Scalars::single(1.0),
        )
    });
    ...
    let view_id = test_context.setup_viewport_blueprint(|ctx, blueprint| {
        ...
        ViewProperty::from_archetype_for_view::<dl_sdk_types::blueprint::archetypes::TimeAxis>(
            ctx, view.id,
        )
        .save_blueprint_component(
            ctx,
            &dl_sdk_types::blueprint::archetypes::TimeAxis::descriptor_view_range(),
            &dl_sdk_types::datatypes::TimeRange {
                start: dl_sdk_types::datatypes::TimeRangeBoundary::Absolute(0.into()),
                end: dl_sdk_types::datatypes::TimeRangeBoundary::Absolute(20.into()),
            },
        );
    ...
    for (name, inf_value) in [
        ...
        for i in 0..8 {
            ...
            test_context.log_entity("plots/all_inf", |builder| {
                builder.with_archetype_auto_row(
                    timepoint,
                    &dl_sdk_types::archetypes::Scalars::single(inf_value),
                )
            });
        }
...
fn test_bootstrapped_secondaries_impl(partial_range: bool, snapshot_results: &mut SnapshotResults) {
    ...
    fn with_scalar(builder: ChunkBuilder, value: i64) -> ChunkBuilder {
        builder.with_archetype_auto_row(
            ...
            &dl_sdk_types::archetypes::Scalars::new([value as f64]),
        )
    }
    ...
    test_context.log_entity("scalars", |builder| {
        let mut builder = builder
            .with_archetype_auto_row(
                ...
                &dl_sdk_types::archetypes::SeriesLines::new()
                    .with_widths([5.0])
                    .with_colors([dl_sdk_types::components::Color::from_rgb(0, 255, 255)])
    ...
    let view_id = test_context.setup_viewport_blueprint(|ctx, blueprint| {
        ...
        if partial_range {
            ...
            ctx.save_blueprint_archetype(
                override_path.clone(),
                &dl_sdk_types::blueprint::archetypes::VisibleTimeRanges::new([
                    dl_sdk_types::blueprint::components::VisibleTimeRange(
                        dl_sdk_types::datatypes::VisibleTimeRange {
                            timeline: "log_tick".into(),
                            range: dl_sdk_types::datatypes::TimeRange {
                                start: dl_sdk_types::datatypes::TimeRangeBoundary::Absolute(
                                    70.into(),
                                ),
                                end: dl_sdk_types::datatypes::TimeRangeBoundary::Infinite,
                            },
                        },
                    ),
                ]),
...
fn time_series_chunks(timeline: Timeline) -> impl Iterator<Item = Chunk> {
    (0_i64..=200).step_by(10).map(move |tick| {
        Chunk::builder("scalars")
            .with_archetype_auto_row(
                [(timeline, tick)],
                &dl_sdk_types::archetypes::Scalars::single((tick as f64 / 30.0).sin()),
            )
            .build()
