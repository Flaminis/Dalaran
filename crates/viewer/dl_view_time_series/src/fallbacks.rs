...
use dl_sdk_types::blueprint::archetypes::{PlotLegend, ScalarAxis, TimeAxis};
use dl_sdk_types::datatypes::TimeRange;
use dl_sdk_types::{
    archetypes::{SeriesLines, SeriesPoints},
    datatypes::TimeRangeBoundary,
};
use dl_viewer_context::{ViewStateExt as _, VisualizerComponentSource};
...
pub fn register_fallbacks(system_registry: &mut dl_viewer_context::ViewSystemRegistrator<'_>) {
    for component in [
        SeriesLines::descriptor_names().component,
        SeriesPoints::descriptor_names().component,
    ] {
        system_registry.register_array_fallback_provider::<dl_sdk_types::components::Name, _>(
            component,
            |ctx| {
    ...
    for component in [
        ...
        system_registry.register_array_fallback_provider::<dl_sdk_types::components::Color, _>(
            component,
            |ctx| {
                let state = ctx.view_state().downcast_ref::<TimeSeriesViewState>();
                let Ok(state) = state else {
                    return vec![dl_viewer_context::auto_color_for_entity_path(
                        ctx.target_entity_path,
                    )];
                };
                ...
                (0..num_series)
                    .map(|i| {
                        let hash = dl_log_types::hash::Hash64::hash((
                            ctx.target_entity_path,
                            source_selector,
                            i,
                        ))
                        .hash64()
                            % u16::MAX as u64;
                        dl_viewer_context::auto_color_egui(hash as u16).into()
                    })
                    .collect()
            },
        );
        ...
        ctx.view_state()
            ...
            .map(|s| {
                let range = make_range_sane(
                    s.scalar_range
                        .unwrap_or(dl_sdk_types::components::Range1D::EMPTY),
                );
    ...
    system_registry.register_fallback_provider::<dl_sdk_types::components::Visible>(
    ...
    system_registry.register_fallback_provider(
        ...
        |ctx| -> dl_sdk_types::blueprint::components::TimeRange {
