...
use dl_view::{ChunksWithComponent, clamped_or_nothing, range_with_blueprint_resolved_data};
...
impl SeriesPointsSystem {
    fn build_draw_data(
    ...
    ) -> Vec<dl_renderer::QueueableDrawData> {
        dl_tracing::profile_function!();
        ...
        match dl_renderer::renderer::MeshDrawData::new(render_ctx, &instances) {
            ...
            Err(err) => {
                dl_log::error_once!("Failed to build scatter marker MeshDrawData: {err}");
                Vec::new()
            }
        }
    }
...
impl SeriesPointsSystem {
    fn load_series(
        ...
        {
            dl_tracing::profile_scope!("fill marker shapes");

            {
                    ...
                    && all_marker_shapes_chunks[0].chunk.num_rows() == 1
                {
                    ...
                    dl_tracing::profile_scope!("fallback markers");
                    ...
                    let all_frames = dl_query::range_zip_1x1(
            ...
            itertools::izip!(points_per_series, series_names, series_visibility).enumerate()
        {
            ...
            util::points_to_series(
                ...
                dl_sdk_types::components::AggregationPolicy::Off,
                &mut series,
                instruction.id,
            );
        }
