...
use dl_log_types::{AbsoluteTimeRange, ComponentPath, EntityPath};
use dl_sdk_types::archetypes::{Scalars, SeriesLines, SeriesPoints};
use dl_sdk_types::blueprint::archetypes::{PlotBackground, PlotLegend, ScalarAxis, TimeAxis};
use dl_sdk_types::blueprint::components::{
    Corner2D, Enabled, LinkAxis, LockRangeDuringZoom, VisualizerInstructionId,
};
use dl_sdk_types::components::{AggregationPolicy, Color, Range1D, Visible};
use dl_sdk_types::datatypes::{TimeRange, TimeRangeBoundary};
use dl_sdk_types::{ComponentBatch as _, ComponentIdentifier, View as _, ViewClassIdentifier};
use dl_ui::{Help, IconText, MouseButtonText, UiExt as _, icons, list_item};
use dl_view::controls::{MOVE_TIME_CURSOR_BUTTON, SELECTION_RECT_ZOOM_BUTTON};
use dl_view::view_property_ui;
...
pub struct TimeSeriesViewState {
    ...
    /// data-space points to screen-space for `dl_renderer` primitives.
    ///
...
impl ViewState for TimeSeriesViewState {
    ...
    fn heap_size_bytes(&self) -> u64 {
        dl_byte_size::SizeBytes::heap_size_bytes(self)
    }
}
...
type ViewType = dl_sdk_types::blueprint::views::TimeSeriesView;
...
impl ViewClass for TimeSeriesView {
        ...
        list_item::list_item_scope(ui, "time_series_selection_ui", |ui| {
            ...
            match link_x_axis {
                ...
                LinkAxis::LinkToGlobal => {
                    dl_view::view_property_ui_with_redirect::<TimeAxis>(
                        &ctx,
                        ui,
                        TimeAxis::descriptor_view_range().component,
                        dl_viewer_context::GLOBAL_VIEW_ID,
                    );
                }
            }
    ...
    fn visualizers_section<'a>(
        &'a self,
        ctx: &'a dl_viewer_context::ViewContext<'a>,
    ) -> Option<dl_viewer_context::VisualizersSectionOutput<'a>> {
        ...
        Some(dl_viewer_context::VisualizersSectionOutput {
    ...
    fn handle_component_drop(
        ...
        match dl_view::handle_component_drop(
            ctx,
            view_id,
            component_paths,
            released,
            ...
            dl_view::ComponentDropResult::Accept => DragAndDropFeedback::Accept,
            dl_view::ComponentDropResult::CompatibleButAlreadyVisualized => {
                DragAndDropFeedback::Reject(Some("Already visualized"))
            }
            dl_view::ComponentDropResult::Incompatible => {
    ...
    fn ui(
    ...
    ) -> Result<dl_viewer_context::ViewClassUiOutput, ViewSystemExecutionError> {
        dl_tracing::profile_function!();
        ...
        {
            dl_tracing::profile_scope!("iterate all_plot_series");
                        ...
                        .find(|instr| instr.id == instruction_id)
                {
                    let (component, selector) = instruction
                        ...
                        .and_then(|mapping| match mapping {
                            dl_viewer_context::VisualizerComponentSource::SourceComponent {
                                source_component,
                                selector,
        ...
        let view_current_time = dl_sdk_types::datatypes::TimeInt(
            current_time
                .unwrap_or_default()
        ...
        let (time_range_property, time_range_ctx) = match link_x_axis {
            ...
            LinkAxis::LinkToGlobal => {
                query_result = dl_viewer_context::DataQueryResult::default();
                ...
                (
                    ...
                    &dl_viewer_context::ViewContext {
                        viewer_ctx: ctx,
                        view_id: dl_viewer_context::GLOBAL_VIEW_ID,
        ...
        let view_time_range = time_range_property
            .component_or_fallback::<dl_sdk_types::blueprint::components::TimeRange>(
                time_range_ctx,
                TimeAxis::descriptor_view_range().component,
            )?;

        let resolve_time_range =
            |view_time_range: &dl_sdk_types::blueprint::components::TimeRange| {
                make_range_sane(Range1D::new(
                    match view_time_range.start {
                        dl_sdk_types::datatypes::TimeRangeBoundary::Infinite => {
        ...
        let legend_hovered = ui
            .ctx()
            .read_response(dl_plot::legend::legend_frame_id(legend_id))
        ...
        ui.scope(|ui| {
            ...
            let tokens = dl_ui::design_tokens_of_visuals(ui.visuals());
            ...
            let egui_plot::PlotResponse {
                ...
                {
                    let time = dl_log_types::TimeReal::from(pointer.x as i64 + time_offset);
            ...
            if let Some(highlight) = ctx.time_ctrl.highlighted_range()
                ...
                && highlight.kind == dl_viewer_context::TimeRangeHighlightKind::StateTimeline
            ...
            // Render dl_renderer draw data (already in screen space) via ViewBuilder.
            ...
            if let Some(hovered) = hovered_data_result.clone().or_else(|| {
                if response.hovered() {
                    Some(dl_viewer_context::Item::View(query.view_id))
                } else {
                    None
                }
            }) {
                    ...
                    if let Some(new_view_time_range) = new_view_time_range
                        .or_else(|| {
                            ...
                            let new_view_time_range = TimeRange {
                                start: dl_sdk_types::datatypes::TimeRangeBoundary::Absolute(
                                    dl_sdk_types::datatypes::TimeInt(
                                        (new_x_range_rounded.start() as i64)
                                            .saturating_add(time_offset),
                                    ),
                                ),
                                end: dl_sdk_types::datatypes::TimeRangeBoundary::Absolute(
                                    dl_sdk_types::datatypes::TimeInt(
                        ...
                        .map(dl_sdk_types::blueprint::components::TimeRange)
            ...
            if *legend_visible.0 {
                    ...
                    .filter(|s| !matches!(s.kind, PlotSeriesKind::Clear))
                {
                    ...
                    let is_hovered = prev_hovered_items
                        .contains_item(&dl_viewer_context::Item::DataResult(address.clone()))
                        || prev_hovered_items.contains_item(&dl_viewer_context::Item::DataResult(
                ...
                let legend_widget =
                    dl_plot::legend::LegendWidget::new(dl_plot::legend::LegendConfig {
                ...
                let legend_output = legend_widget.show_entries(
                    ...
                    all_plot_series
                        .iter()
                        .filter(|s| !matches!(s.kind, PlotSeriesKind::Clear))
                        .map(|s| dl_plot::legend::LegendEntry {
                            id: s.id(),
                            label: s.label.clone(),
                            color: s.color,
                            visible: s.visible,
                ...
                {
                    ctx.selection_state()
                        .set_hovered(dl_viewer_context::Item::DataResult(address.clone()));
                }
...
fn set_time(
    ...
    time: dl_log_types::TimeReal,
) {
    if let Some(current_time) = current_time {
        let current_time = dl_log_types::TimeInt::new_temporal(current_time);
        let time = time.floor();
...
fn visualizers_section_ui(ui: &mut egui::Ui, ctx: &dl_viewer_context::ViewContext<'_>) {
...
fn scalar_datatype_priority(datatype: &dl_log_types::external::arrow::datatypes::DataType) -> u32 {
    use dl_log_types::external::arrow::datatypes::DataType;
    match datatype {
        DataType::Float64 => 0,
        DataType::Float32 => 1,
...
fn all_scalar_mappings(
    ...
    let dl_viewer_context::VisualizableReason::SingleRequiredComponentMatch(m) = reason else {
...
fn find_nearest_data_point_and_show_tooltip(
    ...
    let time_label = {
        let tooltip_format = if time_type == TimeType::TimestampNs {
            timestamp_format.with_date_visibility(dl_log_types::DateVisibility::HideDate)
        } else {
            timestamp_format
        };
        time_type.format(
            dl_log_types::TimeInt::new_temporal(closest_time),
            tooltip_format,
        )
    };
    let y_value_text = dl_format::format_f64(closest_value);
    ...
    dl_plot::tooltip::show_plot_tooltip(
        ui,
        response,
        series.id(),
        &time_label,
    ...
    plot_item_id_to_data_result_address
        .get(&series.id())
        .map(|address| dl_viewer_context::Item::DataResult(address.clone()))
}
...
fn paint_time_range_highlight(
    ...
    transform: &egui_plot::PlotTransform,
    time_offset: i64,
    range: dl_log_types::AbsoluteTimeRange,
    color: egui::Color32,
) {
    let plot_rect = response.rect;
...
fn format_y_axis(mark: egui_plot::GridMark) -> String {
    ...
    dl_format::FloatFormatOptions::DEFAULT_f64
...
/// Render `dl_renderer` draw data (lines, points) produced by visualizers in screen space.
///
...
fn render_re_renderer_draw_data(
    ...
    ui: &egui::Ui,
    response: &egui::Response,
    view_id: dl_renderer::ViewBuilderId,
    draw_data: Vec<dl_renderer::QueueableDrawData>,
) {
    ...
    let resolution_in_pixel =
        dl_viewer_context::gpu_bridge::viewport_resolution_in_pixels(plot_rect, pixels_per_point);
    ...
    let target_config = dl_renderer::view_builder::TargetConfiguration {
        name: "time_series_plot".into(),
        resolution_in_pixel,
        view_from_world,
        projection_from_view: dl_renderer::view_builder::Projection::Orthographic {
            camera_mode: dl_renderer::view_builder::OrthographicCameraMode::TopLeftCornerAndExtendZ,
            vertical_world_size: height,
            far_plane_distance: 1000.0,
        },
        viewport_transformation: dl_renderer::RectTransform::IDENTITY,
        pixels_per_point,
        blend_with_background: dl_renderer::BlendWithBackground::Premultiplied,
        ..Default::default()
    };

    let Ok(mut view_builder) = dl_renderer::ViewBuilder::new(render_ctx, target_config, view_id)
    else {
        return;
    };
    ...
    painter.add(dl_viewer_context::gpu_bridge::new_renderer_callback(
        view_builder,
        plot_rect,
        dl_renderer::Rgba::TRANSPARENT,
    ));
}
...
mod tests {
    use super::*;
    use dl_viewer_context::SingleRequiredComponentMatch;

    #[test]
    fn test_help_view() {
        dl_test_context::TestContext::test_help_view(|ctx| TimeSeriesView.help(ctx));
    }
