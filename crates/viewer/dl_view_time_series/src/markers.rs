...
impl MarkerMeshCache {
    ...
    pub fn get_or_build(&mut self, render_ctx: &RenderContext) -> Option<Arc<MarkerMeshes>> {
        ...
        match MarkerMeshes::build(render_ctx) {
            ...
            Err(err) => {
                dl_log::error_once!("Failed to upload marker meshes: {err}");
...
impl Cache for MarkerMeshCache {
    ...
    fn vram_usage(&self) -> dl_byte_size::MemUsageTree {
        ...
        dl_byte_size::MemUsageTree::Bytes(bytes)
    }
}

impl dl_byte_size::SizeBytes for MarkerMeshCache {
...
impl dl_byte_size::MemUsageTreeCapture for MarkerMeshCache {
    fn capture_mem_usage_tree(&self) -> dl_byte_size::MemUsageTree {
        dl_byte_size::MemUsageTree::Bytes(<Self as dl_byte_size::SizeBytes>::total_size_bytes(self))
    }
}
...
/// `dl_renderer` negates the world-z axis when going to view space (`view_builder.rs`),
