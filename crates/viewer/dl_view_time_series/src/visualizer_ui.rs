...
use dl_component_ui::color_swatch::ColorSwatch;
use dl_log_types::external::arrow::array::AsArray as _;
use dl_sdk_types::archetypes::{SeriesLines, SeriesPoints};
use dl_sdk_types::blueprint::archetypes::ActiveVisualizers;
use dl_sdk_types::components::{self, Color, Name};
use dl_sdk_types::{ComponentDescriptor, Loggable as _};
...
pub fn visualizer_ui_element(
    ...
    node: &dl_viewer_context::DataResultNode,
    ...
    let query_result = dl_view::latest_at_with_blueprint_resolved_data(
        ctx,
        None,
        &ctx.current_query(),
        &node.data_result,
            ...
            |ui| {
                ...
                if show_visibility_icon {
                    let response = if all_series_invisible {
                        ui.small_icon_button(&dl_ui::icons::INVISIBLE, "Show series")
                    } else {
                        ui.small_icon_button(&dl_ui::icons::VISIBLE, "Hide series")
                    };
    ...
    if frame_response.clicked() {
        ctx.viewer_ctx
            .command_sender()
            .send_system(dl_viewer_context::SystemCommand::set_selection(item));
    }
...
impl TimeSeriesColors {
    ...
    fn ui(&self, ui: &mut egui::Ui) {
        ...
        for color in self.colors[..num_boxes].iter().rev() {
            let rgba = dl_sdk_types::datatypes::Rgba32::from(*color);
            let mut color_ref = dl_viewer_context::MaybeMutRef::Ref(&rgba);
...
fn extract_series_colors(
    ...
    let Ok(color_components) = Color::from_arrow(&raw_color_cell) else {
        dl_log::error_once!("Failed to cast color array to Color");
