...
use dl_chunk_store::RangeQuery;
use dl_log_types::TimeInt;
use dl_log_types::external::arrow::array::AsArray as _;
use dl_log_types::external::arrow::buffer::BooleanBuffer;
use dl_log_types::external::arrow::datatypes::UInt32Type;
use dl_sdk_types::external::arrow::datatypes::DataType as ArrowDatatype;
use dl_sdk_types::{ComponentDescriptor, Loggable as _, RowId, archetypes, components};
use dl_view::clamped_or_nothing;
use dl_viewer_context::{ViewQuery, ViewerReportSeverity};
...
fn iter_scalar_slices<'a>(
    all_scalar_chunks: &'a dl_view::ChunksWithComponent<'_>,
...
pub fn determine_num_series(
    all_scalar_chunks: &dl_view::ChunksWithComponent<'_>,
    ...
    if !is_identity && limits_enabled && count > MAX_NUM_SERIES_FOR_REMAPPED_SCALARS {
        results.report_unspecified_source(
            ...
            format!(
                ...
                dl_format::format_uint(count),
                dl_format::format_uint(MAX_NUM_SERIES_FOR_REMAPPED_SCALARS),
            ),
        );
        MAX_NUM_SERIES_FOR_REMAPPED_SCALARS
    } else {
...
pub fn collect_scalars(
    ...
    dl_tracing::profile_function!(format!("points_per_series={}", points_per_series.len()));
    ...
    if width_mismatch {
        results.report_unspecified_source(
            ...
            format!(
                ...
                results.entity_path(),
                dl_format::format_uint(num_series),
            ),
        );
    }
}
...
pub fn collect_colors(
    query: &RangeQuery,
    query_results: &dl_view::VisualizerInstructionQueryResults<'_>,
    all_scalar_chunks: &dl_view::ChunksWithComponent<'_>,
    points_per_series: &mut PlotPointsPerSeries,
    ...
    dl_log::debug_assert_eq!(components::Color::arrow_datatype(), ArrowDatatype::UInt32);

    fn map_raw_color(raw: &u32) -> dl_renderer::Color32 {
        let [a, b, g, r] = raw.to_le_bytes();
        #[expect(clippy::disallowed_methods)] // This is not a hard-coded color.
        dl_renderer::Color32::from_rgba_unmultiplied(r, g, b, a)
    }
    ...
    if all_color_chunks.len() == 1 && all_color_chunks[0].chunk.num_rows() == 1 {
        dl_tracing::profile_scope!("override/default fast path");
        ...
        dl_tracing::profile_scope!("fallback colors");
        ...
        if let Some(color_array) = fallback_array.as_primitive_opt::<UInt32Type>() {
            ...
            dl_log::error_once!("Failed to cast builtin color fallback to UInt32Array");
        }
    } else {
        dl_tracing::profile_scope!("standard path");
        ...
        let all_frames =
            dl_query::range_zip_1x1(all_scalars_indices(query, all_scalar_chunks), all_colors)
                .enumerate();
        ...
        .filter(|slice| !slice.is_empty())
    {
        ...
        if let Some(string_array) = fallback_array.as_string_opt::<i32>() {
            ...
            if fallback_names.is_empty() {
                dl_log::error_once!("Failed to retrieve fallback names");
                vec![]
            } else {
            ...
            dl_log::error_once!("Failed to cast builtin name fallback to StringArray");
            vec![]
        }
    }
}
...
pub fn collect_radius_ui(
...

    {
        ...
        if all_radius_chunks.len() == 1 && all_radius_chunks[0].chunk.num_rows() == 1 {
            ...
            let all_frames =
                dl_query::range_zip_1x1(all_scalars_indices(query, all_scalar_chunks), all_radii)
                    .enumerate();
