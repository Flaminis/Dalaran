...
use dl_view::{ChunksWithComponent, collect_recursive_clears, range_with_blueprint_resolved_data};
use dl_viewer_context::external::dl_entity_db::InstancePath;
...
impl VisualizerSystem for SeriesLinesSystem {
    ...
    fn execute(
        ...
        ctx: &ViewContext<'_>,
        query: &ViewQuery<'_>,
        _context: &dl_viewer_context::ViewContextCollection,
        ...
        // Build dl_renderer line draw data from the collected series.
...
impl SeriesLinesSystem {
    fn build_draw_data(
    ...
    ) -> Result<Vec<dl_renderer::QueueableDrawData>, ViewSystemExecutionError> {
        dl_tracing::profile_function!();
        ...
        let mut line_builder = dl_renderer::LineDrawableBuilder::new(render_ctx);
        ...
        for series in all_series {
            ...
            let mut batch = line_builder.batch(series.label.clone()).picking_object_id(
                dl_renderer::PickingLayerObjectId(series.instance_path.entity_path.hash64()),
            );

            batch
                .add_strip_2d(screen_points.into_iter())
                .color(color)
                .radius(dl_renderer::Size::new_ui_points(radius_ui))
                .flags(
                    dl_renderer::renderer::LineStripFlags::STRIP_FLAGS_OUTWARD_EXTENDING_ROUND_CAPS,
                );
        }
        ...
        let mut draw_data: Vec<dl_renderer::QueueableDrawData> =
...
fn build_nan_island_marker_draw_data(
...
) -> Option<dl_renderer::QueueableDrawData> {
    let marker_meshes = ctx
        .viewer_ctx
        .store_context
        .memoizer(|cache: &mut crate::markers::MarkerMeshCache| cache.get_or_build(render_ctx))?;

    let circle_mesh = marker_meshes.for_shape(dl_sdk_types::components::MarkerShape::Circle);
    ...
    match dl_renderer::renderer::MeshDrawData::new(render_ctx, &instances) {
        Ok(draw_data) => Some(draw_data.into()),
        Err(err) => {
            dl_log::error_once!("Failed to build NaN-island marker MeshDrawData: {err}");
            None
        }
    }
}
...
impl SeriesLinesSystem {
    fn load_series(
        ...
        output: &dl_viewer_context::VisualizerExecutionOutput,
    ) -> Vec<PlotSeries> {
        dl_tracing::profile_function!(data_result.entity_path.to_string());
        ...
        let query = dl_chunk_store::RangeQuery::new(view_query.timeline, query_range)
        ...
        results.merge_bootstrapped_data(dl_view::latest_at_with_blueprint_resolved_data(
            ctx,
            None,
            &dl_chunk_store::LatestAtQuery::new(query.timeline, query.range.min()),
            data_result,
        ...
        let results = dl_view::BlueprintResolvedResults::Range(query.clone(), results);
        let results =
            dl_view::VisualizerInstructionQueryResults::new(instruction, &results, output);
        ...
        let has_discontinuities = {
            ...
            dl_tracing::profile_scope!("discontinuities");
        ...
        if !all_chunks_sorted_and_not_overlapped || has_discontinuities {
            dl_tracing::profile_scope!("sort");
            for points in &mut points_per_series {
                dl_tracing::profile_scope!("sort_by_key", points.len().to_string());
