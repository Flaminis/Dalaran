...
use dl_sdk_types::blueprint::components::{LinkAxis, VisualizerInstructionId};
use dl_sdk_types::components::AggregationPolicy;
use dl_sdk_types::datatypes::TimeRange;
use dl_viewer_context::external::dl_entity_db::InstancePath;
use dl_viewer_context::{ViewContext, ViewQuery, ViewerContext};
...
pub fn data_result_time_range(
    ...
    let current_time = ctx
        .time_ctrl
        .time_int()
        .unwrap_or(dl_log_types::TimeInt::ZERO);

    let query_range = match data_result.query_range() {
        dl_viewer_context::QueryRange::TimeRange(time_range) => *time_range,

        dl_viewer_context::QueryRange::LatestAt => {
...
pub fn determine_query_range(
    ...
    let view_time_range = time_range_property
        .component_or_fallback::<dl_sdk_types::blueprint::components::TimeRange>(
            ctx,
            dl_sdk_types::blueprint::archetypes::TimeAxis::descriptor_view_range().component,
        )?;
...
pub fn points_to_series(
    ...
    store: &dl_chunk_store::ChunkStore,
    query: &ViewQuery<'_>,
    series_label: String,
    ...
    dl_tracing::profile_function!(&instance_path.to_string());
...
pub fn apply_aggregation(
    ...
    let points = if should_aggregate {
        dl_tracing::profile_scope!("aggregate", aggregator.to_string());
