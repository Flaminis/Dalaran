...
impl ViewContents {
    ...
    pub fn build_data_result_tree(
        &self,
        ctx: &dl_viewer_context::ActiveStoreContext<'_>,
        active_timeline: Option<&Timeline>,
        view_class_registry: &dl_viewer_context::ViewClassRegistry,
        ...
        app_options: &dl_viewer_context::AppOptions,
    ) -> DataQueryResult {
        ...
        let visualizable_entities_per_visualizer_in_view: VisualizableEntitiesPerVisualizerInView<
            '_,
        > = {
            dl_tracing::profile_scope!("visualizable_entities_per_visualizer_in_view");
        ...
        let root_handle = {
            dl_tracing::profile_scope!("add_entity_tree_to_data_results_recursive");
        ...
        let component_defaults = {
            dl_tracing::profile_scope!("component_defaults");
...
struct DataQueryPropertyResolver<'a> {
    ...
    view_class: &'a dyn dl_viewer_context::ViewClass,
    visualizable_entities_per_visualizer_in_view: &'a VisualizableEntitiesPerVisualizerInView<'a>,
    ...
    fn update_overrides_recursive(
                ...
                .component_batch::<blueprint_components::VisualizerInstructionId>(id_component)
            {
                    ...
                    .subtree(&node.data_result.override_base_path)
                {
                    for child_tree in subtree.children.values() {
                        ...
                        let Ok(uuid) = last
                            .unescaped_str()
                            .parse::<dl_sdk_types::external::uuid::Uuid>()
                        else {
                            continue;
                        };
                        ...
                        let id = VisualizerInstructionId::from(
                            dl_sdk_types::datatypes::Uuid::from(uuid),
                        );
            ...
            .flatten()
        {
            if let Some(component_data) = blueprint_engine
                    .cache()
                    .latest_at(dl_chunk_store::ChunkTrackingMode::Report, blueprint_query, override_base_path, [component])
        ...
        for instruction in &mut node.data_result.visualizer_instructions {
                ...
                .flatten()
            {
                if let Some(component_data) = blueprint_engine
                        .cache()
                        .latest_at(dl_chunk_store::ChunkTrackingMode::Report, blueprint_query, &instruction.override_path, [component])
                        .component_batch_raw(component) &&
                    // We regard empty overrides as non-existent. This is important because there is no other way of doing component-clears.
                     !component_data.is_empty()
                {
                ...
                )
            {
                instruction
                    ...
                    .extend(mappings_from_store.into_iter().filter_map(|mapping| {
                        ...
                        let source =
                            dl_viewer_context::VisualizerComponentSource::from_blueprint_mapping(
                                &mapping.0,
                            )
                            .ok_or_log_error()?;
                        Some((target, source))
    ...
    fn test_query_results() {
        ...
        view_class_registry
            .add_class::<TestViewClass>(
                &Reflection::default(),
                &dl_viewer_context::AppOptions::test(),
        ...
        let time_ctrl = dl_viewer_context::TimeControl::default();
