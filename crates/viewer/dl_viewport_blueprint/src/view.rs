...
use dl_log_types::{EntityPathSubs, Timeline};
use dl_sdk_types::ViewClassIdentifier;
use dl_sdk_types::blueprint::archetypes as blueprint_archetypes;
use dl_sdk_types::blueprint::components::{self as blueprint_components, ViewOrigin};
...
/// `dl_viewport::ViewportBlueprint::duplicate_view`.
#[derive(Clone, Debug)]
...
impl ViewBlueprint {
                        ...
                        blueprint_engine
                            ...
                            .filter_map(|component| {
                                let array = blueprint_engine
                                    .cache()
                                    .latest_at(dl_chunk_store::ChunkTrackingMode::Report, query, path, [component])
    ...
    pub fn class<'a>(
        &self,
        view_class_registry: &'a dl_viewer_context::ViewClassRegistry,
    ) -> &'a dyn ViewClass {
...
mod tests {
    ...
    use dl_log_types::example_components::{MyLabel, MyPoint, MyPoints};
    use dl_sdk_types::blueprint::archetypes::EntityBehavior;
    ...
    fn test_visible_interactive_overrides() {
        ...
        struct Scenario {
            base_overrides: Vec<(EntityPath, Box<dyn dl_types_core::AsComponents>)>,
        ...
        ) in scenarios.into_iter().enumerate()
        {
            ...
            let mut add_to_blueprint =
                |path: &EntityPath, archetype: &dyn dl_types_core::AsComponents| {
    ...
    ) -> dl_viewer_context::DataQueryResult {
        let mut result = None;
