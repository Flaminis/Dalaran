...
use dl_mutex::Mutex;
use dl_sdk_types::blueprint::archetypes as blueprint_archetypes;
use dl_sdk_types::blueprint::components::{
    AutoLayout, AutoViews, RootContainer, ViewMaximized, ViewerRecommendationHash,
};
use dl_sdk_types::{Archetype as _, ViewClassIdentifier};
...
impl ViewportBlueprint {
    ...
    pub fn from_db(blueprint_db: &dl_entity_db::EntityDb, query: &LatestAtQuery) -> Self {
        ...
        dl_log::trace_once!("Loaded root_container: {root_container:?}");
        ...
        if let Some(root_container) = root_container {
            dl_tracing::profile_scope!("visit_all_containers");
            ...
            while let Some(id) = container_ids_to_visit.pop() {
                if let Some(container) = ContainerBlueprint::try_from_db(blueprint_db, query, id) {
                    dl_log::trace_once!("Container {id} contents: {:?}", container.contents);
                    ...
                    dl_log::warn_once!("Failed to load container {id}");
                }
            }
        }
        ...
        dl_log::trace_once!("Loaded tree: {tree:#?}");
    ...
    pub fn is_item_valid(
        &self,
        storage_ctx: &dl_viewer_context::StorageContext<'_>,
        item: &Item,
    ) -> bool {
        match item {
    ...
    pub fn mark_user_interaction(&self, ctx: &ViewerContext<'_>) {
        if self.auto_layout() {
            dl_log::trace!("User edits - will no longer auto-layout");
        }
    ...
    pub fn spawn_heuristic_views(&self, ctx: &ViewerContext<'_>) {
        ...
        for entry in ctx.view_class_registry().iter_registry() {
            ...
            let excluded_entities = dl_log_types::ResolvedEntityPathFilter::properties();
            ...
            dl_tracing::profile_scope!("filter_recommendations_for", class_id);
            ...
            Contents::View(view_id) => {
                if let Some(view) = self.view(view_id) {
                    if visible != view.visible {
                        if self.auto_layout() {
                            dl_log::trace!("view visibility changed - will no longer auto-layout");
                        }
    ...
    pub fn save_tree_as_containers(&self, ctx: &ViewerContext<'_>) {
        ...
        dl_log::trace!("Saving tree: {:#?}", self.tree);
        ...
        {
            dl_log::trace!("Saving with a root container");
            ctx.save_blueprint_component(
                VIEWPORT_PATH.into(),
            ...
            dl_log::trace!("Saving empty viewport");
...
fn apply_viewport_command(
    ...
    dl_log::trace!("Processing viewport command: {command:?}");
    match command {
        ...
        ViewportCommand::AddView {
            ...
            if bp.auto_layout() {
                ...
                *run_auto_layout = true;
                ...
                dl_log::trace!("Adding view {view_id} to parent {parent_id}");
                ...
                {
                    dl_log::trace!("Inserting new view into root container");
        ...
        ViewportCommand::AddContainer {
            ...
            dl_log::trace!("Adding container {container_kind:?} to parent {parent_id}");

            if let Some(egui_tiles::Tile::Container(parent_container)) =
                bp.tree.tiles.get_mut(blueprint_id_to_tile_id(&parent_id))
            {
                dl_log::trace!("Inserting new view into container {parent_id:?}");
                parent_container.add_child(tile_id);
            } else {
                dl_log::trace!("Parent or root was not a container - will re-run auto-layout");
                *run_auto_layout = true;
            }
        }
        ...
        ViewportCommand::SetContainerKind(container_id, container_kind) => {
            ...
            {
                dl_log::trace!("Mutating container {container_id:?} to {container_kind:?}");
                container.set_kind(container_kind);
            } else {
                dl_log::trace!("No root found - will re-run auto-layout");
            }
        }
        ...
        ViewportCommand::FocusTab(view_id) => {
            ...
            dl_log::trace!("Found tab to focus on for view ID {view_id}: {found}");
        }
        ...
        ViewportCommand::RemoveContents(contents) => {
            ...
            for tile in bp.tree.remove_recursively(tile_id) {
                dl_log::trace!("Removing tile {tile_id:?}");
                match tile {
                    egui_tiles::Tile::Pane(view_id) => {
                        dl_log::trace!("Removing view {view_id}");
        ...
        ViewportCommand::SimplifyContainer(container_id, options) => {
            dl_log::trace!("Simplifying tree with options: {options:?}");
            let tile_id = blueprint_id_to_tile_id(&container_id);
