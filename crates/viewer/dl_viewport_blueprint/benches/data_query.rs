...
use dl_log_types::{EntityPath, EntityPathFilter, EntityPathSubs, StoreId, TimePoint, Timeline};
use dl_sdk_types::archetypes::Points2D;
use dl_sdk_types::components::Position2D;
...
use dl_viewport_blueprint::ViewContents;
...
fn query_tree_many_entities(c: &mut Criterion) {
    ...
    let indicated_entities = PerVisualizerType(
        ...
        visualizable_entities
            ...
            .map(|(v, e)| {
                (
                    *v,
                    dl_viewer_context::IndicatedEntities(e.keys().cloned().collect()),
                )
            })
            .collect(),
    );
    ...
    let app_options = dl_viewer_context::AppOptions::test();

    // Benchmark with simple include-all filter
    {
        let view_contents = ViewContents::new(
            dl_viewer_context::ViewId::random(),
            view_class_id,
