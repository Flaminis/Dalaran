...
use dl_log_types::{EntityPath, TimeInt, Timeline};
        ...
        match column_data.data_type() {
            ...
            ArrowDataType::List(_) => Self::ListArray(
                column_data
                    ...
                    .expect("`data_type` checked, failure is a bug in dl_dataframe")
                    .clone(),
            ),
...
impl DisplayComponentColumn {
    fn blobs(&self, row: usize) -> Option<Vec<Blob>> {
        if self.component_descr.component_type != Some(dl_sdk_types::components::Blob::name()) {
            return None;
        }
    ...
    pub fn is_image(&self) -> bool {
        self.component_descr.component_type == Some(dl_sdk_types::components::Blob::name())
            && self
                .blobs(0)
                .as_ref()
    ...
    fn data_ui(
        ...
        if let Some(data) = data {
                ...
                && Self::is_blob_image(blob)
            {
                ...
                // ui.
                if row_id.is_none() {
                    dl_tracing::profile_scope!("Blob hash");
