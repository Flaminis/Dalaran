...
use dl_types_core::{Component as _, FIELD_METADATA_KEY_COMPONENT_TYPE, Loggable as _};
...
impl TypedFilter {
    ...
    fn default_for_primitive_datatype(
        ...
        match primitive_datatype {
            ...
            data_type
                if data_type == &dl_sdk_types::components::Timestamp::arrow_datatype()
                    && column_field
                        .metadata()
                        .get(FIELD_METADATA_KEY_COMPONENT_TYPE)
                        == Some(&dl_sdk_types::components::Timestamp::name().to_string()) =>
            {
