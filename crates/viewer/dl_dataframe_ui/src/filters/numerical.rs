...
impl SyntaxHighlighting for IntFilter {
    fn syntax_highlight_into(&self, builder: &mut SyntaxHighlightedBuilder) {
        ...
        if let Some(value) = self.rhs_value {
            builder.append_primitive(&dl_format::format_int(value));
        } else {
...
impl Filter for IntFilter {
    ...
    fn popup_ui(
        &mut self,
        ui: &mut egui::Ui,
        _timestamp_format: dl_log_types::TimestampFormat,
        column_name: &str,
...
impl SyntaxHighlighting for FloatFilter {
    fn syntax_highlight_into(&self, builder: &mut SyntaxHighlightedBuilder) {
        ...
        if let Some(value) = self.rhs_value {
            builder.append_primitive(&dl_format::format_f64(value.into_inner()));
        } else {
            builder.append_primitive("…");
        }
    }
