...
use dl_chunk_store::{ChunkTrackingMode, LatestAtQuery};
...
use dl_sorbet::{BatchType, ColumnDescriptorRef};
use dl_types_core::Archetype as _;
use dl_ui::UiExt as _;
use dl_viewer_context::{VariantName, blueprint_timeline};
...
impl SortDirection {
    ...
    pub fn icon(&self) -> &'static dl_ui::Icon {
        match self {
            Self::Ascending => &dl_ui::icons::ARROW_DOWN,
            Self::Descending => &dl_ui::icons::ARROW_UP,
        }
    }
...
impl TableBlueprint {
    ...
    pub fn apply_heuristics(
        ...
        current_server_origin: Option<&dl_uri::Origin>,
    ) -> Self {
        ...
        if self.url_column.is_none() && self.segment_preview_column.is_none() {
            let first_url = columns.columns.iter().enumerate().find_map(|(idx, col)| {
                ...
                let uri = dl_uri::RedapUri::from_str(&sample).ok()?;
...
fn test_default_column_display_name() {
    ...
    assert_eq!(
        default_display_name_for_column(&ColumnDescriptorRef::Component(
            &dl_sorbet::ComponentColumnDescriptor {
