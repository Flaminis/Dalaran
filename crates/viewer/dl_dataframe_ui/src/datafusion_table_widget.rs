...
use dl_log::error;
use dl_log_types::{EntryId, Timestamp};
use dl_protos::cloud::v1alpha1::ext;
use dl_sorbet::{ColumnDescriptorRef, SorbetSchema};
...
pub struct Column<'a> {
    /// The ID of the column (based on it's corresponding [`dl_sorbet::ColumnDescriptor`]).
    pub id: egui::Id,
...
/// Keep track of a [`dl_sorbet::SorbetBatch`]'s columns, along with their order and their blueprint.
...
pub struct DataFusionTableWidget<'a> {
    ...
    /// Table ID used for looking up a cached table blueprint from the [`StoreHub`](dl_viewer_context::StoreHub).
    ///
    /// `None` for tables that are not backed by a [`TableStore`](dl_viewer_context::TableStore)
    /// (e.g. redap browser entries), in which case no hub lookup or registration is performed.
    table_id: Option<dl_log_types::TableId>,
    ...
    registered_table_blueprint: Option<&'a dl_entity_db::EntityDb>,
}
...
impl<'a> DataFusionTableWidget<'a> {
    ...
    /// Associate this widget with a [`TableId`](dl_log_types::TableId) for blueprint caching
    /// via the [`StoreHub`](dl_viewer_context::StoreHub).
    pub fn table_id(mut self, table_id: dl_log_types::TableId) -> Self {
    ...
    pub fn registered_table_blueprint(
        mut self,
        blueprint: Option<&'a dl_entity_db::EntityDb>,
    ) -> Self {
    ...
    fn readp_uri(&self) -> Option<dl_uri::RedapUri> {
        dl_uri::RedapUri::from_str(self.url.as_ref()?).ok()
    }
    ...
    fn flag_column_field(
        ...
        remote_table: Option<&dl_uri::EntryUri>,
        ...
        let Some((flag_col_idx, flag_col)) = columns.find_by_display_name(flag_col_name) else {
            dl_log::warn_once!("Flag column {flag_col_name:?} does not exist in the table");
            return None;
        };

        if !matches!(&flag_col.desc, dl_sorbet::ColumnDescriptorRef::Component(c)
        ...
        let Some(flag_field) = original_schema
            ...
            dl_log::warn_once!("Flag column {flag_col_name:?} is missing from the original schema");
            return None;
        };
    ...
    pub fn show(
        ...
        runtime: &AsyncRuntimeHandle,
        ui: &mut egui::Ui,
        view_states: &mut dl_viewer_context::ViewStates,
    ) -> TableStatus {
        ...
        let query_result = match (requested_query_result, &table_state.last_query_results) {
            ...
            (Some(Err(err)), _) => {
                ...
                ui.horizontal(|ui| {
                    ...
                    if ui
                        .small_icon_button(&dl_ui::icons::RESET, "Refresh")
                        .clicked()
                    {
            ...
            && let Some(flag_col) = &output.blueprint.flag_column
        {
            ...
            if let Some(dl_uri::RedapUri::Entry(entry_uri)) = &redap_uri
    ...
    fn table_ui(
        ...
        let table_style = dl_ui::TableStyle::Spacious;
        ...
        let migrated_fields = query_result
            .sorbet_schema
            .columns
            .arrow_fields(dl_sorbet::BatchType::Dataframe);

        let show_segment_previews =
            view_renderer.is_some() && new_blueprint.segment_preview_column.is_some();

        let entry_uri = if let Some(dl_uri::RedapUri::Entry(entry_uri)) = &redap_uri {
            Some(entry_uri)
        } else {
            None
        };
            ...
            .show(ui, |ui| {
                ...
                ui.horizontal_centered(|ui| {
                    ...
                    egui::Sides::new().show(
                        ...
                        |ui| {
                            ui.set_height(height);
                            let refresh_tooltip = match dl_ui::TableCommandKind::Refresh
        ...
        .show(ui, |ui| {
            egui::Sides::new().show(
                ...
                |ui| {
                    ...
                    if let Some(url) = url
                        && ui
                            .small_icon_button(&dl_ui::icons::COPY, "Copy URL")
                            .on_hover_text(url)
                            .clicked()
                    {
                        ctx.command_sender()
...
pub fn resolve_recording_for_row<'a>(
    ...
    display_record_batches: &[DisplayRecordBatch],
    row_idx: u64,
    already_requested_uris: &mut ahash::HashSet<dl_uri::DatasetSegmentUri>,
    ...
    let uri = uri_str.parse::<dl_uri::DatasetSegmentUri>().ok()?;
    ...
    if already_requested_uris.insert(uri.clone()) {
        ctx.command_sender
            .send_system(SystemCommand::LoadDataSource(
                dl_data_source::LogDataSource::RedapDatasetSegment {
                    uri: uri.clone(),
                    open_behavior: dl_data_source::RecordingOpenBehavior::Background,
                },
            ));
    }
...
fn table_index_column_index(schema: &arrow::datatypes::Schema) -> Option<usize> {
    schema.fields.iter().position(|field| {
        field
            .metadata()
            .get(dl_sorbet::metadata::SORBET_IS_TABLE_INDEX)
            .map(|v| v.as_str())
            == Some("true")
    })
}
...
fn upsert_flag_changes(
    ...
    let build_upsert_batch = || -> Option<arrow::array::RecordBatch> {
        ...
        let index_array = dl_arrow_util::concat_arrays(&index_refs).ok()?;
    ...
    let Some(upsert_batch) = build_upsert_batch() else {
        dl_log::warn_once!("Failed to build upsert RecordBatch for flag changes");
        return;
    };
    ...
    runtime.spawn_future(async move {
        ...
        if let Err(err) = result {
            dl_log::warn_once!("Failed to upsert flag changes: {err}");
        } else {
            dl_log::debug!("Successfully upserted flag changes");
        }
    });
}
...
struct DataFusionTableDelegate<'a> {
    ...
    view_states: &'a mut dl_viewer_context::ViewStates,
}
...
impl DataFusionTableDelegate<'_> {
    ...
    pub fn row_context_menu(&self, ui: &Ui, _row_number: u64) {
        ...
        ui.response().container_context_menu(|ui| {
            ...
            // dl_table will ensure that the right-clicked row is always selected.
                ...
                .show(
                    ...
                    |ui| {
                        ...
                        egui::containers::menu::MenuButton::from_button(
                            ui.small_icon_button_widget(&dl_ui::icons::MORE, "More options"),
                        )
                        ...
                        .ui(ui, |ui| {
                            ...
                            {
                                if ui
                                    .icon_and_text_menu_item(&dl_ui::icons::FILTER, "Filter")
                                    .clicked()
                                {
