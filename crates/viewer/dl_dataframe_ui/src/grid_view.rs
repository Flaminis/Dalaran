...
use dl_ui::egui_ext::card_layout::CardLayout;
use dl_ui::{UiExt as _, UiLayout};
use dl_viewer_context::{AppContext, ViewStates};
...
fn lookup_column(columns: &Columns<'_>, name: &str, kind: &str) -> Option<usize> {
    ...
    if found.is_none() {
        dl_log::warn_once!("{kind} column {name:?} was not found in the table.");
    }
    found
}
...
fn card_content_ui(
    ...
    if card_click_response.clicked()
        ...
        && dl_uri::RedapUri::from_str(&url_str).is_ok()
    {
...
fn flag_button(ui: &mut Ui, is_flagged: bool, card_hovered: bool) -> egui::Response {
    ...
    if ui.is_rect_visible(rect) {
        ...
        let icon = if is_flagged {
            &dl_ui::icons::FLAG_TOGGLED
        } else {
            &dl_ui::icons::FLAG_UNTOGGLED
        };
