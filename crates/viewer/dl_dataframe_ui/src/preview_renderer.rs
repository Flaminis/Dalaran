...
pub(crate) enum PreviewRecording<'a> {
    Resolved(&'a EntityDb),
    Unresolved(dl_uri::DatasetSegmentUri),
}
...
/// [`StoreHub`](dl_viewer_context::StoreHub).
///
...
impl<'a> RecordingPreviewRenderer<'a> {
    ...
    pub fn show_preview(
        &self,
        app_ctx: &dl_viewer_context::AppContext<'_>,
        ui: &mut egui::Ui,
        row_nr: u64,
        row_hovered: bool,
        ...
        let ctx = ViewerContext {
            app_ctx: dl_viewer_context::AppContext {
        ...
        ui.columns(resolved.len(), |cols| {
            for (col_ui, resolved) in std::iter::zip(cols, resolved) {
                ...
                dl_viewport::paint_view_loading_indicator(
                    col_ui,
                    (row_nr, view_id),
                    view_rect,
                ...
                && let Some(pos) = ui.input(|i| i.pointer.interact_pos())
            {
                ...
                app_ctx.command_sender.send_system(
                    dl_viewer_context::SystemCommand::TimeControlCommands {
                        ...
                        time_commands: vec![dl_viewer_context::TimeControlCommand::SetTime(
                            dl_log_types::TimeReal::from(time),
                        )],
                    },
                );
                    ...
                    for (store_id, active_preview) in preview_state.iter_active_previews() {
                        ...
                        app_ctx.command_sender.send_system(
                            dl_viewer_context::SystemCommand::TimeControlCommands {
                                ...
                                time_commands: vec![
                                    dl_viewer_context::TimeControlCommand::SetTime(
                                        dl_log_types::TimeReal::from(time),
                                    ),
                                ],
                            },
