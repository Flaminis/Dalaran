...
pub struct DataFusionQueryResult {
    ...
    pub sorbet_schema: dl_sorbet::SorbetSchema,
...
impl DataFusionQuery {
    ...
    async fn batch_stream(self) -> Result<SendableRecordBatchStream, DataFusionError> {
        ...
        if let Some(segment_links) = segment_links {
            //TODO(ab): we should get this from `dl_uri::DatasetDataUri` instead of hardcoding
            let uri = format!(
            ...
            .filter_map(|filter| {
                filter
                    ...
                    .inspect_err(|err| {
                        ...
                        dl_log::warn_once!("invalid filter: {err}");
                    })
                    .ok()
            })
            .collect();
...
pub enum QueryEvent {
    Schema {
        ...
        sorbet_schema: dl_sorbet::SorbetSchema,
    },
    Batch(SorbetBatch),
...
impl DataFusionAdapter {
    ...
    pub fn apply_flag_changes(
        ...
        let Some(col_idx) = results.sorbet_schema.columns.iter().position(|desc| {
            matches!(desc, dl_sorbet::ColumnDescriptor::Component(c) if c.component.as_str() == flag_column_name)
        }) else {
            return;
        };
...
fn update_existing_flag_column(
    ...
    for change in changes {
        ...
        let Some(old_col) = batch
            ...
            dl_log::warn_once!("Flag column at index {col_idx} is not a boolean column");
            break;
        };
