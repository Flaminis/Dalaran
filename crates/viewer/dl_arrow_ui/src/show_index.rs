...
numeric_primitive_display!(dl_format::format_int: Int8Type, Int16Type, Int32Type, Int64Type);
numeric_primitive_display!(dl_format::format_uint: UInt8Type, UInt16Type, UInt32Type, UInt64Type);
numeric_primitive_display!(dl_format::format_f32: Float32Type);
numeric_primitive_display!(dl_format::format_f64: Float64Type);
numeric_primitive_display!(dl_format::format_f16: Float16Type);
        ...
        impl<'a> ShowIndexState<'a> for &'a $t {
            ...
            fn write(
                ...
                if self.is_null(idx) {
                    ...
                    f.append_primitive(&dl_log_types::Timestamp::from(timestamp).format(*state));
                }
...
impl<OffsetSize: OffsetSizeTrait> ShowIndex for &GenericBinaryArray<OffsetSize> {
    fn write(&self, idx: usize, f: &mut SyntaxHighlightedBuilder) -> EmptyArrowResult {
        let value = self.value(idx);
        f.append_primitive(&dl_format::format_bytes(value.len() as f64));
        Ok(())
    }
