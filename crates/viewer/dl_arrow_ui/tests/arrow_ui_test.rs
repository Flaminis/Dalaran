...
fn arrow_list_ui(ui: &mut egui::Ui) {
    ...
    use dl_sdk_types::datatypes::{Utf8, Vec3D};
    ...
    let tests = [
        ...
        ("String with URL", Utf8::from("https://dalaran.dev").to_arrow()),
        (
            "Two URLs in strings",
            [
                Utf8::from("https://dalaran.dev"),
                Utf8::from("https://dalaran.rs"),
            ]
            .to_arrow(),
        ),
    ...
    egui::Grid::new("entity_db").num_columns(2).show(ui, |ui| {
        ...
        for (name, arrow_result) in tests {
            ...
            dl_arrow_ui::arrow_ui(ui, UiLayout::List, Default::default(), &arrow);
            ui.end_row();
        }
    });
}
...
fn arrow_tree_ui() {
    ...
    for (array_name, arrow) in arrays {
        let mut harness = dl_ui::testing::new_harness(
        ...
        .build_ui(|ui| {
            ...
            dl_arrow_ui::arrow_ui(ui, UiLayout::SelectionPanel, Default::default(), &arrow);
        });
...
fn inline_formatting() {
    for (name, data) in arrow_test_data::all_arrays() {
        let highlighted =
            dl_arrow_ui::arrow_syntax_highlighted(&data).expect("Failed to format data");
