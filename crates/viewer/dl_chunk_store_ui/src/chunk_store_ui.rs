...
use dl_ui::{ContextExt as _, text_edit::autocomplete_text_edit};
...
/// Browser UI for [`dl_chunk_store::ChunkStore`].
...
impl DatastoreUi {
    ...
    pub fn ui(
        ...
        return_route: &Route,
        command_sender: &dl_viewer_context::CommandSender,
    ) {
            ...
            let row_ui = ui
                .scope(|ui| {
                    ...
                    |mut row: TableRow<'_, '_>| {
                        ...
                        row.col(|ui| {
                            ui.label(dl_format::format_uint(chunk.num_rows()));
                        });

                        row.col(|ui| {
                            ui.label(dl_format::format_bytes(
    ...
    fn filter_controls_ui(&mut self, ui: &mut egui::Ui, chunk_store: &ChunkStore) {
        ...
        ui.horizontal(|ui| {
            ...
            let filter_icon_rect = ui.small_icon(&dl_ui::icons::FILTER, None);
            ui.interact(
                filter_icon_rect,
            ...
            if ui
                .small_icon_button(&dl_ui::icons::TRASH, "Clear filters")
                .clicked()
            {
                self.entity_path_filter.clear();
                self.component_filter.clear();
        ...
        match &self.chunk_list_sort_column.column {
            ...
            ChunkListColumn::Timeline(timeline_name) => chunks.sort_by_key(|chunk| {
                chunk
                    ...
                    .map_or(dl_log_types::TimeInt::MIN, |(_, time_column)| {
                        ...
                        ui.list_item_collapsible_noninteractive_label("Info", true, |ui| {
                            ...
                            ui.list_item_flat_noninteractive(
                                list_item::PropertyContent::new("Heap size").value_text(
                                    dl_format::format_bytes(stats.total_size_bytes as f64),
                                ),
                            );
    ...
    fn switch_recording_ui(&mut self, ui: &mut egui::Ui, storage_context: &StorageContext<'_>) {
        ...
        let icon_rect = ui.small_icon(&dl_ui::icons::DATASET, None);
        ui.interact(
            icon_rect,
    ...
    egui::ComboBox::new(combo_id, "")
        ...
        .show_ui(ui, |ui| {
            for store in stores {
                ...
                if ui
                    .add(dl_ui::ComboItem::new(label).selected(is_selected))
                    .clicked()
                {
...
fn format_time_range(
    ...
    if time_range.min() == time_range.max() {
        ...
        let length = match timeline.typ() {
            TimeType::Sequence => {
                format!("{} ticks", dl_format::format_uint(time_range.abs_length()))
            }
