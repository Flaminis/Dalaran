...
//! (via [`dl_renderer::FileResolver`]) and then runs the resolved source through `naga`.
...
use dl_renderer::{FileResolver, FileSystem, SearchPath};
use wgpu::naga;

/// A [`FileSystem`] backed by `std::fs`.
///
/// We can't reuse `dl_renderer`'s own `OsFileSystem` because it's only compiled in for native
...
fn all_wgsl_shaders_are_valid() {
    ...
    let workspace_root = crate_dir
        .ancestors()
        .nth(3)
        .expect("dl_renderer should live at crates/viewer/dl_renderer");
    ...
    // and the example shaders all import from `dl_renderer/shader`, so that's the search root.
