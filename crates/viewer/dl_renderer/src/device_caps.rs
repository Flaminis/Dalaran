...
/// Device tiers `dl_renderer` distinguishes.
///
...
/// Returns an instance descriptor with settings preferred by `dl_renderer`.
///
/// `dl_renderer` should work fine with any instance descriptor, but those are the settings we generally assume.
    ...
    let backends = if let Some(force_backend) = force_backend {
        if let Some(backend) = parse_graphics_backend(force_backend) {
            if let Err(err) = validate_graphics_backend_applicability(backend) {
                ...
                dl_log::info!("Forcing graphics backend to {backend:?}.");
                backend.into()
            }
        } else {
            dl_log::error!(
...
pub async fn select_testing_adapter(instance: &wgpu::Instance) -> wgpu::Adapter {
    ...
    dl_log::info!("Found the following graphics adapters:");
    for adapter in &adapters {
        dl_log::info!("* {}", crate::adapter_info_summary(&adapter.get_info()));
    }
    ...
    dl_log::info!("Picked adapter: {:?}", adapter.get_info());
...
pub fn select_adapter(
    ...
    dl_log::debug!("Found the following viable graphics adapters:");
    for adapter in &adapters {
        dl_log::debug!("* {}", crate::adapter_info_summary(&adapter.get_info()));
    }
    ...
    let adapter = adapters.remove(0);
    dl_log::debug!("Picked adapter: {:?}", adapter.get_info());

    Ok(adapter)
}

/// Backends that are officially supported by `dl_renderer`.
///
