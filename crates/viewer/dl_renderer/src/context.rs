...
pub enum RenderContextError {
    #[error(
        ...
        Check the troubleshooting guide at https://dalaran.dev/docs/overview/installing-dalaran/troubleshooting and consider updating your graphics driver."
    )]
...
pub struct RenderContext {
    ...
    /// TODO(andreas): We rely on egui for the "primary" submissions in `dl_viewer`. It would be nice to take full control over all submissions.
...
impl Renderers {
    pub fn get_or_create<R: 'static + Renderer + Send + Sync>(
        ...
        self.renderers.entry().or_insert_with(|| {
            dl_tracing::profile_scope!("create_renderer", std::any::type_name::<R>());

            let key = RendererTypeId(u8::try_from(self.renderers_by_key.len()).unwrap_or_else(
                |_| {
                    dl_log::error!("Supporting at most {} distinct renderer types.", u8::MAX);
                    u8::MAX
                },
            ));
    ...
    pub fn begin_frame(&mut self) {
            ...
            .is_some()
        {
            if self.active_frame.frame_index != STARTUP_FRAME_IDX {
                dl_log::error!("There was still a command encoder from the previous frame at the beginning of the current.
This means, either a call to RenderContext::before_submit was omitted, or the previous frame was unexpectedly cancelled.");
        ...
        if !modified_paths.is_empty() {
            dl_log::debug!(?modified_paths, "got some filesystem events");
        }
    ...
    pub fn before_submit(&mut self) {
        ...
        if let Some(command_encoder) = self
            ...
            .take()
        {
            dl_tracing::profile_scope!("finish & submit frame-global encoder");
...
fn log_adapter_info(info: &wgpu::AdapterInfo) {
    ...
    if cfg!(test) {
        ...
        dl_log::debug_once!("wgpu adapter {human_readable_summary}");
    } else if is_software_rasterizer_with_known_crashes {
        dl_log::warn_once!(
            "Bad software rasterizer detected - expect poor performance and crashes. See: https://www.dalaran.dev/docs/overview/installing-dalaran/troubleshooting#graphics-issues"
        );
        dl_log::info_once!("wgpu adapter {human_readable_summary}");
    } else if info.device_type == wgpu::DeviceType::Cpu {
        dl_log::warn_once!(
            "Software rasterizer detected - expect poor performance. See: https://www.dalaran.dev/docs/overview/installing-dalaran/troubleshooting#graphics-issues"
        );
        dl_log::info_once!("wgpu adapter {human_readable_summary}");
    } else {
        dl_log::debug_once!("wgpu adapter {human_readable_summary}");
