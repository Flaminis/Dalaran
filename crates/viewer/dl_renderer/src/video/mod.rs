...
use dl_mutex::Mutex;
use dl_video::player::{DecoderDelayState, GetVideoSource, VideoPlayerError, VideoPlayerStreamId};
use dl_video::{DecodeSettings, VideoDataDescription};

use crate::RenderContext;
use crate::resource_managers::{GpuTexture2D, SourceImageDataFormat};

/// A [`dl_video::player::VideoPlayer`] with GPU texture output.
pub type VideoPlayer = dl_video::player::VideoPlayer<VideoTexture>;
...
pub struct VideoFrameTexture {
    ...
    pub frame_info: Option<dl_video::FrameInfo>,
}
...
pub struct Video {
    debug_name: String,
    video_description: dl_video::VideoDataDescription,
...
impl Drop for Video {
    fn drop(&mut self) {
        dl_log::trace!("Dropping Video {:?}", self.debug_name);
    }
}
...
impl Video {
    ...
    #[inline]
    pub fn data_descr(&self) -> &dl_video::VideoDataDescription {
    ...
    #[inline]
    pub fn data_descr_mut(&mut self) -> &mut dl_video::VideoDataDescription {
    ...
    pub fn handle_sample_insertion(
        &self,
        change_start: dl_video::SampleIndex,
        change_end: dl_video::SampleIndex,
        size_delta: isize,
    ) {
        let mut players = self.players.lock();
