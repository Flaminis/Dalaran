...
pub mod gpu_data {
    ...
    impl dl_byte_size::SizeBytes for PositionRadius {
    ...
    impl PositionRadius {
        ...
        pub fn from_many(positions: &[glam::Vec3], radii: &[Size]) -> Vec<Self> {
            ...
            dl_tracing::profile_function_if!(10_0000 < positions.len());
            if positions.len() == radii.len() {
                // Optimize common-case with simpler iterators.
                dl_tracing::profile_scope_if!(10_000 < positions.len(), "zipped");
                izip!(positions.iter().copied(), radii.iter().copied())
                    .map(|(pos, radius)| Self { pos, radius })
                    .collect()
            } else {
                dl_tracing::profile_scope_if!(10_000 < positions.len(), "extended-radius");
                izip!(
