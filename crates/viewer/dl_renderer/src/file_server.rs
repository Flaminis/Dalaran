...
macro_rules! include_file {
    ($path:expr $(,)?) => {{
        ...
        // We're packing shaders using the dl_renderer crate as root instead (to avoid nasty
        // problems when publishing: as we lose workspace information when publishing!), so we
        // need to make sure to strip the path down.
        let path = if let Ok(root_path) = root_path.strip_prefix("crates/viewer/dl_renderer") {
            ...
            // path that assumes the CWD is the root of dl_renderer, we need to make it absolute as
            // there is no guarantee that this is where `cargo run` is being run from.
            let manifest_path = ::std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"));
            manifest_path.join(path)
        } else {
            // If we're not inside dl_renderer, just take the path as-is, ignoring any implications of publishing.
...
macro_rules! include_file {
    ($path:expr $(,)?) => {{
        ...
        // need to make sure to strip the path down.
        let path = path
            .strip_prefix("crates/viewer/dl_renderer")
        ...
        // dl_renderer path no matter what.
        let manifest_path = ::std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"));
        let path = path
    ...
    impl FileServer {
        fn new() -> anyhow::Result<Self> {
            ...
            let watcher = notify::recommended_watcher(move |res| match res {
                Ok(event) => {
                    if let Err(err) = dl_quota_channel::send_crossbeam(&events_tx, event) {
                        dl_log::error!(%err, "filesystem watcher disconnected, discarding event");
                    }
                }
                Err(err) => {
                    dl_log::error!(%err, "filesystem watcher failure");
                }
            })?;
        ...
        pub fn collect<Fs: FileSystem>(&mut self, resolver: &FileResolver<Fs>) -> HashSet<PathBuf> {
            fn canonicalize_opt(path: impl AsRef<Path>) -> Option<PathBuf> {
                ...
                std::fs::canonicalize(path)
                    .map_err(|err| dl_log::error!(%err, ?path, "couldn't canonicalize path"))
                    .ok()
            }
            ...
            for path in &paths {
                if let Err(err) = self.watch(resolver, path, false) {
                    dl_log::error!(err=%dl_error::format(err), "couldn't watch imported dependency");
                }
            }
