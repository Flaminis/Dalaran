    ...
    Desc: Clone + Eq + Hash + Debug + DynamicResourcesDesc,
{
    pub fn alloc<F: FnOnce(&Desc) -> Res>(
        ...
        {
            dl_log::trace!(?desc, "Reclaimed previously discarded resource");
        ...
        dl_log::trace!(?desc, "Allocated new resource");
        let inner_resource = {
            dl_tracing::profile_scope!("creation_func");
            creation_func(desc)
        };
...
{
    fn drop(&mut self) {
        // TODO(andreas): We're failing this check currently on dl_viewer's shutdown.
