...
/// The highest level rendering block in `dl_renderer`.
...
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash, dl_byte_size::SizeBytes)]
...
impl ViewBuilder {
    ...
    pub fn draw(
        ...
        {
            dl_tracing::profile_scope!("main target pass");
        ...
        if let Some(picking_processor) = &self.picking_processor {
            ...
            match picking_processor.end_render_pass(&mut encoder, &pipelines) {
                ...
                Err(PickingLayerError::ReadbackError(err)) => {
                    dl_log::warn_once!("Failed to schedule picking data readback: {err}");
                }
                Ok(()) => {}
            }
        }
        ...
        if let Some(outline_mask_processor) = &self.outline_mask_processor {
            dl_tracing::profile_scope!("outlines");
            {
                dl_tracing::profile_scope!("outline mask pass");
        ...
        if let Some(screenshot_processor) = &self.screenshot_processor {
            ...
            match screenshot_processor.end_render_pass(&mut encoder) {
                Ok(()) => {}
                Err(err) => {
                    dl_log::warn_once!("Failed to schedule screenshot data readback: {err}");
                }
            }
        }
    ...
    /// ```no_run
    /// use dl_renderer::{view_builder::ViewBuilder, RenderContext, ScreenshotProcessor};
    ...
    ///             dl_log::info!("Received screenshot {}", user_data);
    ///         },
    ///     ).is_some()
    ///     {}
    /// }
