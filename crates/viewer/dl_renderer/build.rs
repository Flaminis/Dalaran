...
//!
//! It finds all WGSL shaders defined anywhere within `dl_renderer`, and embeds them
//! directly into the released artifact for our `dl_renderer` library.
//!
...
//!
//! See `dl_renderer/src/workspace_shaders.rs` for the end result.
...
fn main() {
    ...
    // Root path of the dl_renderer crate.
    //
    ...
    // This means all the shaders we pack must live under `dl_renderer/shader` for now.
...
"#
    ...
    assert!(
        !entries.is_empty(),
        "dl_renderer build.rs found no shaders - I think some path is wrong!"
    );
