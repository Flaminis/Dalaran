...
use dl_ui::{Help, UiExt as _};
use dl_viewer_context::external::dl_log_types::EntityPath;
...
impl dl_byte_size::SizeBytes for TextDocumentViewState {
...
type ViewType = dl_sdk_types::blueprint::views::TextDocumentView;
...
impl ViewClass for TextDocumentView {
    ...
    fn selection_ui(
        ...
        if !state.only_showing_markdown {
            ui.list_item_scope("text_document_selection_ui", |ui| {
                ...
                dl_view::view_property_ui::<TextDocumentFormat>(&ctx, ui);
            });
        }
    ...
    fn ui(
        ...
        state.only_showing_markdown = !text_entries.is_empty()
            && text_entries
                .iter()
                .all(|entry| entry.media_type == dl_sdk_types::components::MediaType::markdown());
...
fn text_document_ui(
    ...
    if text_entries.is_empty() {
        ...
        if media_type == &dl_sdk_types::components::MediaType::markdown() {
            dl_tracing::profile_scope!("egui_commonmark");
...
#[test]
fn test_help_view() {
    dl_test_context::TestContext::test_help_view(|ctx| TextDocumentView.help(ctx));
}
