...
pub enum Node {
    /// An explicit node is a node that was provided via [`dl_sdk_types::archetypes::GraphNodes`].
    ///
    ...
    /// An implicit node is a node that was provided via [`dl_sdk_types::archetypes::GraphEdges`], but does not have a corresponding [`dl_sdk_types::components::GraphNode`] in an [`dl_sdk_types::archetypes::GraphNodes`] archetype.
    ///
