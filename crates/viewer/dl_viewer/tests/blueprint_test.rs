...
use dl_chunk::{RowId, TimePoint};
use dl_sdk_types::ViewClassIdentifier;
use dl_test_context::TestContext;
use dl_test_context::VisualizerBlueprintContext as _;
use dl_test_viewport::TestContextExt as _;
use dl_viewer_context::{BlueprintContext as _, ViewClass as _, ViewerReportSeverity};
...
fn log_test_data_and_register_views(test_context: &mut TestContext, scalars_count: usize) {
    test_context.register_view_class::<dl_view_dataframe::DataframeView>();
    test_context.register_view_class::<dl_view_bar_chart::BarChartView>();

    let timeline_a = dl_chunk::Timeline::new_sequence("timeline_a");
    test_context.log_entity("scalar", |builder| {
        builder.with_archetype(
            RowId::new(),
            [(timeline_a, 0)],
            &dl_sdk_types::archetypes::Scalars::single(scalars_count as f32),
        )
    });
    ...
    test_context.log_entity("vector", |builder| {
        builder.with_archetype(
            RowId::new(),
            TimePoint::STATIC,
            &dl_sdk_types::archetypes::BarChart::new(vector),
        )
    });
}
...
fn setup_viewport(test_context: &mut TestContext) {
    let view_1 =
        ViewBlueprint::new_with_root_wildcard(dl_view_bar_chart::BarChartView::identifier());
    let view_2 =
        ViewBlueprint::new_with_root_wildcard(dl_view_dataframe::DataframeView::identifier());

    test_context.setup_viewport_blueprint(|ctx, blueprint| {
        // Set the color override for the bar chart view.
        ctx.save_visualizers(
            &dl_chunk::EntityPath::from("vector"),
            view_1.id,
            [
                &dl_sdk_types::archetypes::BarChart::default().with_color([255, 144, 1]), // #FF9001
            ],
        );

        // Set the timeline for the dataframe view.
        let query = dl_view_dataframe::Query::from_blueprint(ctx, view_2.id);
        query.save_timeline_name(ctx, &dl_chunk::TimelineName::from("timeline_a"));
    ...
    let mut harness = test_context
        ...
        .build_ui(|ui| {
            test_context.run_ui(ui, |ctx, ui| {
                let viewport_blueprint = dl_viewport_blueprint::ViewportBlueprint::from_db(
                    ctx.blueprint_db(),
...
fn test_blueprint_change_and_restore() {
    ...
    test_context.setup_viewport_blueprint(|_ctx, blueprint| {
        ...
        blueprint.remove_contents(dl_viewer_context::Contents::View(first_view_id));
        blueprint.add_views([
            ViewBlueprint::new_with_root_wildcard(dl_view_bar_chart::BarChartView::identifier()),
            ViewBlueprint::new_with_root_wildcard(dl_view_bar_chart::BarChartView::identifier()),
            ViewBlueprint::new_with_root_wildcard(dl_view_bar_chart::BarChartView::identifier()),
...
fn test_blueprint_override_legacy_hash() {
    ...
    let entity_path = dl_chunk::EntityPath::from("vector");
    ...
    let view_id = test_context.setup_viewport_blueprint(|ctx, blueprint| {
        ...
        let visualizer_path =
            base_override_path
                .clone()
                .join(&dl_chunk::EntityPath::from_single_string(
                    legacy_id.to_string(),
                ));
        ...
        let color_override = dl_sdk_types::archetypes::BarChart::default().with_color([255, 0, 0]);
    ...
    let color_component = dl_sdk_types::archetypes::BarChart::descriptor_color().component;
    assert!(
        data_result
