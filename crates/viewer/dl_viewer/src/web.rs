...
use dl_log_types::{TableId, TableMsg, TimelineName};
...
impl WebHandle {
    ...
    pub fn open_channel(&mut self, id: &str, channel_name: &str) {
        ...
        if self.log_senders.contains_key(id) {
            dl_log::warn!("Channel with id '{}' already exists.", id);
            return;
        }

        let (log_tx, log_rx) = dl_log_channel::log_channel(dl_log_channel::LogSource::JsChannel {
                        ...
                        match msg {
                            HttpMessage::LogMsg(msg) => {
                                if log_tx.send(msg.into()).is_ok() {
                                    ...
                                    dl_log::info_once!("Failed to dispatch log message to viewer.");
                                    ControlFlow::Break(())
                                }
                            }
    ...
    pub fn send_table_to_channel(&self, id: &str, data: &[u8]) {
        ...
        if let Some(log_tx) = self.log_senders.get(id) {
            ...
            let stream_reader = match arrow::ipc::reader::StreamReader::try_new(cursor, None) {
                ...
                Err(err) => {
                    dl_log::error_once!("Failed to interpret data as IPC-encoded arrow: {err}");
                    return;
                }
            };
            ...
            let mut batches: Vec<_> = match stream_reader.try_collect() {
                Ok(batches) => batches,
                Err(err) => {
                    dl_log::error_once!("Could not read from IPC stream: {err}");
                    return;
                }
            };

            if batches.len() != 1 {
                dl_log::warn_once!("Expected exactly one record batch, got {}", batches.len());
                return;
            }
            ...
            let msg = match table_msg_from_record_batch(record_batch) {
                Ok(msg) => msg,
                Err(err) => {
                    dl_log::error_once!("Failed to decode Arrow message: {err}");
                    return;
                }
            };
            ...
            match log_tx.send(msg.into()) {
                ...
                Err(err) => {
                    dl_log::info_once!("Failed to dispatch log message to viewer: {err}");
                }
            }
        }
    }
    ...
    pub fn set_active_recording_id(&self, recording_id: &str) {
        ...
        if store_id.is_recording() {
            app.command_sender.send_system(SystemCommand::SetRoute(
                dl_viewer_context::Route::LocalRecording {
...
fn store_id_from_recording_id(
    store_hub: &dl_viewer_context::StoreHub,
    recording_id: &str,
) -> Option<dl_log_types::StoreId> {
    store_hub
        .store_bundle()
        .recordings()
...
impl From<PanelState> for dl_sdk_types::blueprint::components::PanelState {
...
fn create_app(
    ...
    connection_registry: dl_redap_client::ConnectionRegistryHandle,
    app_options: AppOptions,
) -> Result<crate::App, dl_renderer::RenderContextError> {
    ...
    if let Some(fallback_token) = fallback_token {
        match dl_auth::Jwt::try_from(fallback_token) {
            Ok(token) => connection_registry.set_fallback_token(token),
            Err(err) => {
                dl_log::warn!("Failed to parse JWT token: {err}");
            }
        }
    }
