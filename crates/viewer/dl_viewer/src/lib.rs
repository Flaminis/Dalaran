...
//! are reported via [`dl_log`] (i.e. [`dl_log::error!`] / [`dl_log::warn!`]).
//! These are not scoped to a specific view or entity and are often intermittent.
//! They are shown in the notification panel.
//!
//! ## Per-visualizer-type in a view ([`dl_viewer_context::VisualizerTypeReport::OverallError`])
//!
//! A specific visualizer type fails entirely for a view.
//! Rare and almost always a Viewer bug.
//!
//! ## Per-instruction (per entity × visualizer × view) ([`dl_viewer_context::VisualizerInstructionReport`])
//!
...
//! Collected in [`dl_viewer_context::VisualizerTypeReport::PerInstructionReport`].
//!
//! See [`dl_viewer_context::VisualizerInstructionReport`] for how these break down further.
...
///
/// They all implement the [`dl_types_core::Component`] trait.
///
...
pub mod external {
    ...
    pub use dl_chunk_store::external::*;
...
pub enum AppEnvironment {
    ...
    /// Some custom application wrapping `dl_viewer`.
    Custom(String),
...
impl AppEnvironment {
    pub fn from_store_source(source: &dl_log_types::StoreSource) -> Self {
        use dl_log_types::StoreSource;
        match source {
            ...
            | StoreSource::Other(_) => {
                ...
                dl_log::warn_once!("Asked to create an AppEnvironment from {source:?}");
...
pub(crate) fn wgpu_options(force_wgpu_backend: Option<&str>) -> egui_wgpu::WgpuConfiguration {
    ...
    let instance_descriptor = dl_renderer::device_caps::instance_descriptor(force_wgpu_backend);
    ...
    egui_wgpu::WgpuConfiguration {
        wgpu_setup: egui_wgpu::WgpuSetup::CreateNew(egui_wgpu::WgpuSetupCreateNew {
            ...
            native_adapter_selector: Some(std::sync::Arc::new(move |adapters, surface| {
                dl_renderer::device_caps::select_adapter(adapters, backends, surface)
            })),
            device_descriptor: std::sync::Arc::new(|adapter| {
                dl_renderer::device_caps::DeviceCaps::from_adapter_without_validation(adapter)
                    .device_descriptor()
            }),
...
pub fn customize_eframe_and_setup_renderer(
    ...
    if let Some(render_state) = &cc.wgpu_render_state {
        ...
        let render_ctx = RenderContext::new(
            ...
            dl_renderer::RenderConfig::best_for_device_caps,
        )?;
        paint_callback_resources.insert(render_ctx);
    }

    dl_ui::apply_style_and_install_loaders(&cc.egui_ctx);
    Ok(())
}
...
pub fn reset_viewer_persistence() -> anyhow::Result<()> {
    cfg_select! {
        ...
        _ => {
            ...
            if let Some(cache_dir) = dl_viewer_context::AppOptions::default_cache_directory() {
                if let Err(err) = std::fs::remove_dir_all(&cache_dir) {
                    if err.kind() != std::io::ErrorKind::NotFound {
                        anyhow::bail!("Failed to remove {cache_dir:?}: {err}");
                    }
                } else {
                    dl_log::info!("Cleared {cache_dir:?}.");
                }
            }
        }
    }
...
/// Hook into [`dl_log`] to receive copies of text log messages on a channel,
/// which we will then show in the notification panel.
pub fn register_text_log_receiver() -> crossbeam::channel::Receiver<dl_log::LogMsg> {
    dl_log::add_log_msg_receiver(dl_log::LevelFilter::INFO)
}
