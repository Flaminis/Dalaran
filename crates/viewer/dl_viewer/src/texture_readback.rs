...
impl TextureReadbacks {
    ...
    pub fn poll_and_save_texture_readbacks(
        ...
        notifications: &mut dl_ui::notifications::NotificationUi,
    ) {
        self.active_readbacks.retain(|(id, action)| {
            if let Some(readback) = dl_renderer::poll_read_texture(render_ctx, *id) {
                let Some(color_type) = texture_format_to_color_type(readback.format) else {
                    dl_log::warn!("Can't download texture with format {:?}", readback.format);
                    return false;
                };
                match action {
                    ...
                    DownloadAction::Save => {
                        ...
                        {
                            dl_log::error!("Failed to encode preview image as PNG: {err}");
                        } else {
                            command_sender.save_file_dialog(
                                dl_capabilities::MainThreadToken::from_egui_ui(ui),
                                "preview.png",
...
fn to_color_image(
    ...
    Some(match color_type {
        ...
        ExtendedColorType::L16 => {
            dl_log::warn!("16 bit image copied as 8 bit image, some precision was lost.");
        ...
        _ => {
            dl_log::error!("Can't copy textures with color type `{color_type:?}`");
            return None;
        }
    })
}
...
fn texture_format_to_color_type(
    format: dl_renderer::external::wgpu::TextureFormat,
