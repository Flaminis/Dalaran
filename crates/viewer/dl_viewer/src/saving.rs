...
pub fn sanitize_app_id(app_id: &ApplicationId) -> String {
    dl_viewer_context::sanitize_file_name(&app_id.as_str().to_lowercase())
}
...
pub fn delete_blueprint(app_id: &ApplicationId) -> anyhow::Result<()> {
    ...
    if blueprint_path.exists() {
        ...
        dl_log::debug!("Deleted persisted blueprint for {app_id} at {blueprint_path:?}");
    }
    Ok(())
}
...
pub fn encode_to_file(
    ...
    path: &std::path::Path,
    messages: impl Iterator<Item = dl_chunk::ChunkResult<dl_log_types::LogMsg>>,
) -> anyhow::Result<()> {
