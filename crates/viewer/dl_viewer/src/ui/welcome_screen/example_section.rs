...
fn load_file_size(egui_ctx: &egui::Context, url: String) -> Promise<Option<u64>> {
    ...
    fetch(request, move |response| {
        match response {
            ...
            Err(err) => {
                dl_log::debug!(?url, "Failed to load file size: {err}");
                sender.send(None);
            }
        }
        egui_ctx.request_repaint();
            ...
            ui.vertical(|ui| {
                ...
                let examples = match examples {
                    ...
                    Err(err) => {
                        ...
                        dl_log::warn_once!("Failed to load examples: {err}");
        ...
        .show(ui, |ui| {
            ui.horizontal(|ui| {
                if ui
                    .add_enabled(
                        source_url.is_some(),
                        dl_ui::icons::GITHUB.as_button_with_label(ui.tokens(), "Source code"),
                    )
