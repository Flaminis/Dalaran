...
use dl_memory::util::sec_since_start;
use dl_query::{QueryCacheStats, QueryCachesStats};
...
impl DevPanel {
    ...
    pub fn ui(
        ...
        ui: &mut egui::Ui,
        limit: &MemoryLimit,
        mem_usage_tree: Option<dl_byte_size::NamedMemUsageTree>,
        external_trees: &[dl_byte_size::NamedMemUsageTree],
        gpu_resource_stats: &WgpuResourcePoolStatistics,
        store_stats: Option<&StoreHubStats>,
        ...
        let ((), close_clicked) = egui::Sides::new().shrink_left().show(
            ...
            |ui| {
                ui.small_icon_button(&dl_ui::icons::CLOSE, "Close dev panel")
                    .clicked()
            },
        );
    ...
    fn allocation_tracking_ui(ui: &mut egui::Ui) {
        let mut is_tracking_callstacks = dl_memory::accounting_allocator::is_tracking_callstacks();
        ui.re_checkbox(
            &mut is_tracking_callstacks,
            "Enable detailed allocation tracking",
        )
        .on_hover_text("This will slow down the program");
        dl_memory::accounting_allocator::set_tracking_callstacks(is_tracking_callstacks);

        ui.add_space(8.0);

        if let Some(tracking_stats) = dl_memory::accounting_allocator::tracking_stats() {
    ...
    fn gpu_stats(ui: &mut egui::Ui, gpu_resource_stats: &WgpuResourcePoolStatistics) {
        ...
        egui::Grid::new("gpu resource grid")
            ...
            .show(ui, |ui| {
                ...
                ui.label(dl_format::format_bytes(*total_buffer_size_in_bytes as _));
                ui.end_row();
                ui.label("Texture Memory:");
                ui.label(dl_format::format_bytes(*total_texture_size_in_bytes as _));
                ui.end_row();
            });
    }
        ...
        egui::Grid::new("store stats grid 2")
            ...
            .show(ui, |ui| {
                ...
                fn label_chunk_stats(ui: &mut egui::Ui, stats: ChunkStoreChunkStats) {
                    ...
                    ui.label(dl_format::format_uint(num_chunks));
                    ui.label(dl_format::format_uint(num_rows));
                    ui.label(dl_format::format_uint(num_events));
                    ui.label(dl_format::format_bytes(total_size_bytes as _));
                }
                    ...
                    egui::Grid::new("latest_at cache stats grid")
                        ...
                        .show(ui, |ui| {
                            ...
                            for (cache_key, stats) in latest_at {
                                ...
                                ui.label(dl_format::format_uint(total_chunks));
                                ui.label(dl_format::format_bytes(total_effective_size_bytes as _));
                                ui.label(dl_format::format_bytes(total_actual_size_bytes as _));
                                ui.end_row();
                            }
                        });
                });
    ...
    fn tracking_stats(
        ui: &mut egui::Ui,
        tracking_stats: dl_memory::accounting_allocator::TrackingStatistics,
    ) {
...
pub fn memory_tree_ui(
    ui: &mut egui::Ui,
    tree: Option<dl_byte_size::NamedMemUsageTree>,
    external_trees: &[dl_byte_size::NamedMemUsageTree],
    ...
    ui.horizontal(|ui| {
        ...
        ui.hyperlink_to(
            "Learn more",
            "https://docs.rs/dl_byte_size/latest/dl_byte_size/trait.MemUsageTreeCapture.html",
        );

        #[expect(dead_code)]
        fn foo(_: &dyn dl_byte_size::MemUsageTreeCapture) {
    ...
    let dl_memory::MemoryUse { resident, counted } = dl_memory::MemoryUse::capture();

    let include_counted = true; // What our allocator counts. Perfectly accurate.

    if include_counted && let Some(counted) = counted {
        let mut node = dl_byte_size::MemUsageNode::new().with_named_child(tree);

        for tree in external_trees {
            node = node.with_named_child(tree.clone());
        }

        tree = dl_byte_size::NamedMemUsageTree::new("counted", node.with_total_size_bytes(counted));
    }

    if *include_rss && let Some(resident) = resident {
        tree = dl_byte_size::NamedMemUsageTree::new(
            "RSS",
            dl_byte_size::MemUsageNode::new()
    ...
    egui::ScrollArea::vertical()
        .auto_shrink(false)
        .show(ui, |ui| {
            dl_memory_view::memory_flamegraph_ui(ui, &tree);
        });
}
