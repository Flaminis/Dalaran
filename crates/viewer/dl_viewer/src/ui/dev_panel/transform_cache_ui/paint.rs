...
fn edge_time_icon(time: TimeInt) -> &'static dl_ui::Icon {
    if time.is_static() {
...
fn edge_time_icon_rect(start: egui::Pos2, end: egui::Pos2, icon_size: egui::Vec2) -> egui::Rect {
    ...
    let icon_offset = dl_ui::DesignTokens::menu_button_padding();
...
fn subspace_icon(subspace_kind: SubspaceKind) -> &'static dl_ui::Icon {
    match subspace_kind {
        SubspaceKind::TwoD => &dl_ui::icons::VIEW_2D,
        SubspaceKind::ThreeD => &dl_ui::icons::VIEW_3D,
    }
}
...
fn format_dvec3(vec: glam::DVec3) -> String {
    format!(
        "[{}, {}, {}]",
        dl_format::format_f64(vec.x),
        dl_format::format_f64(vec.y),
        dl_format::format_f64(vec.z)
    )
}
...
fn matrix3x3_ui(ui: &mut egui::Ui, matrix_cols: [f64; 9]) {
    egui::Grid::new("transform_edge_matrix3x3")
        ...
        .show(ui, |ui| {
            for row in 0..3 {
                for col in 0..3 {
                    ui.monospace(dl_format::format_f64(matrix_cols[row + col * 3]));
                }
                ui.end_row();
            }
        });
