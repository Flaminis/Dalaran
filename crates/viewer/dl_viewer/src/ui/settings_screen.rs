...
fn settings_screen_ui_impl(ui: &mut egui::Ui, app_options: &mut AppOptions, keep_open: &mut bool) {
    ...
    ui.horizontal(|ui| {
        ...
        ui.allocate_ui_with_layout(
            ...
            |ui| {
                if ui
                    .small_icon_button(&dl_ui::icons::CLOSE, "Close")
                    .clicked()
                {
                    *keep_open = false;
                }
...
fn origin_private_filesystem_section_ui(ui: &mut Ui) {
    if ui
        ...
        .clicked()
    {
        let request = dl_web::browser::window().and_then(|window| {
            window
                .navigator()
                .storage()
                .persist()
        ...
        dl_async::spawn_local(async move {
            ...
            match result {
                Ok(true) => dl_log::info!("Persistent browser storage granted"),
                Ok(false) => dl_log::warn!("Persistent browser storage denied"),
                Err(err) => {
                    dl_log::error!("Failed to request persistent browser storage: {err}");
                }
            }
        });
    }
        ...
        egui::DragValue::new(&mut bytes)
            ...
            .custom_parser(|s| {
                ...
                if s.chars().all(|c| c.is_numeric()) {
                    ...
                    Some(dl_format::parse_bytes(s)? as f64)
                }
            })
...
fn time_format_section_ui(ui: &mut Ui, timestamp_format: &mut TimestampFormat) {
    ...
    let timestamp = dl_log_types::Timestamp::from(
...
fn video_section_ui(ui: &mut Ui, options: &mut VideoOptions) {
    cfg_select! {
        target_arch = "wasm32" => {
            ...
            use dl_video::DecodeHardwareAcceleration;
...
fn ffmpeg_path_status_ui(ui: &mut Ui, options: &VideoOptions) {
    ...
    use dl_video::{FFmpegVersion, FFmpegVersionParseError};
