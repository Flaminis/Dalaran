...
pub fn top_panel(
    ...
    let native_window_bar = !dl_ui::fullsize_content(ui.os()) && !app.custom_window_decorations();
...
fn software_rasterizer_warning_ui(ui: &mut egui::Ui, info: &wgpu::AdapterInfo) {
    ...
    .on_hover_ui(|ui| {
        ...
        ui.label(format!(
            "wgpu adapter {}",
            dl_renderer::adapter_info_summary(info)
        ));
    });
}
        ...
        && origin != *dl_redap_browser::EXAMPLES_ORIGIN
    {
        ...
        match latency {
            ...
            LatencyResult::MostRecent(duration) => {
                ...
                RichText::new(format!("{} ms", dl_format::format_uint(ms)))
                    .strong()
                    .append_to(
                        &mut layout_job,
                        ui.style(),
...
fn panel_buttons_r2l(
    ...
    if app.is_fullscreen_allowed() {
        let (icon, label) = if app.is_fullscreen_mode() {
            (&dl_ui::icons::MINIMIZE, "Minimize")
        } else {
            (&dl_ui::icons::MAXIMIZE, "Maximize")
        };
        ...
        |ui| {
            if ui
                .medium_icon_toggle_button(
                    &dl_ui::icons::RIGHT_PANEL_TOGGLE,
    ...
    ui.add_enabled_ui(
        ...
        |ui| {
            if ui
                .medium_icon_toggle_button(
                    &dl_ui::icons::BOTTOM_PANEL_TOGGLE,
                    "Time panel toggle",
                    &mut app_blueprint.time_panel_state().is_expanded(),
                )
                .on_hover_ui(|ui| dl_ui::RecordingCommandKind::ToggleTimePanel.tooltip_ui(ui))
                .clicked()
            {
            ...
            .show(|ui| {
                ...
                if !ui.is_sizing_pass() {
                    ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                        if ui
                            .add(dl_ui::ReButton::new("Log out").small().primary())
                            .clicked()
                        {
...
fn memory_use_label_ui(
    ...
    const CODE: &str = "use dl_memory::AccountingAllocator;\n\
                        #[global_allocator]\n\
    ...
    let mem = dl_memory::MemoryUse::capture();

    if let Some(count) = dl_memory::accounting_allocator::global_allocs() {
        // we use monospace so the width doesn't fluctuate as the numbers change.
        let bytes_used_text = dl_format::format_bytes(count.size as _);
        ...
        .on_hover_ui(|ui| {
            egui::Grid::new("memory usage hover")
                ...
                .show(ui, |ui| {
                    ...
                    ui.label("Viewer");
                    ui.monospace(dl_format::format_bytes(viewer_mem as _));
                    ui.end_row();

                    if external_mem > 0 {
                        ui.label("External");
                        ui.monospace(dl_format::format_bytes(external_mem as _));
                        ui.end_row();
                    }
                    ...
                    ui.label("GPU");
                    ui.monospace(dl_format::format_bytes(
...
fn latency_snapshot_button_ui(
    ui: &mut egui::Ui,
    latency: dl_entity_db::LatencySnapshot,
...
fn latency_details_ui(ui: &mut egui::Ui, latency: dl_entity_db::LatencySnapshot) {
    let Some(e2e) = latency.e2e() else {
    ...
    let dl_entity_db::LatencySnapshot { secs_since_log } = latency;
    ...
    ui.vertical_centered(|ui| {
        ...
        for (&location, &latency_sec) in &secs_since_log {
            if location == TimestampLocation::Log {
                dl_log::debug_assert_eq!(latency_sec, 0.0);
            } else {
