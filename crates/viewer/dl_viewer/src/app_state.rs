...
use dl_log_channel::{LogReceiverSet, LogSource, RecordingOpenBehavior};
use dl_log_types::StoreId;
use dl_redap_browser::RedapServers;
use dl_redap_client::ConnectionRegistryHandle;
use dl_sdk_types::blueprint::components::{PanelState, PlayState};
use dl_ui::{ContextExt as _, UiExt as _, WindowFrameConfig};
use dl_viewer_context::open_url::{self, ViewerOpenUrl};
...
#[cfg(feature = "testing")]
pub type TestHookAppFn = Box<dyn FnOnce(&dl_viewer_context::AppContext<'_>)>;
...
pub struct AppState {
    ...
    selection_panel: dl_selection_panel::SelectionPanel,
    time_panel: dl_time_panel::TimePanel,
    blueprint_time_panel: dl_time_panel::TimePanel,
    #[serde(skip)]
    blueprint_tree: dl_blueprint_tree::BlueprintTree,
    #[serde(skip)]
    pub(crate) recording_panel: dl_recording_panel::RecordingPanel,

    #[serde(skip)]
    welcome_screen: crate::ui::WelcomeScreen,

    #[serde(skip)]
    datastore_ui: dl_chunk_store_ui::DatastoreUi,
    ...
    /// such as [`ViewportUi`] and [`dl_selection_panel::SelectionPanel`].
    #[serde(skip)]
...
impl Default for AppState {
    fn default() -> Self {
        Self {
            ...
            blueprint_time_panel: dl_time_panel::TimePanel::new_blueprint_panel(),
...
impl AppState {
    ...
    pub fn time_cursor_for(&self, store_id: &StoreId) -> Option<dl_entity_db::PrefetchTimeCursor> {
    ...
    pub fn show(
        ...
        ui: &mut egui::Ui,
        render_ctx: &dl_renderer::RenderContext,
        ...
        match self.navigation.current() {
            ...
            Route::LocalRecording { recording_id: _ } => {
                ...
                if viewport_ui.blueprint.is_invalid() {
                    dl_log::warn!("Incompatible blueprint detected. Resetting to default.");
                    command_sender
                        .send_system(dl_viewer_context::SystemCommand::ClearActiveBlueprint);
                ...
                let query_results = {
                    dl_tracing::profile_wait!("query_results");
                    ...
                    viewport_ui
                        ...
                        .into_par_iter()
                        .map(|view| {
                            dl_tracing::profile_scope!("view", view.display_name_or_default().to_string().as_str());
            ...
            Route::Loading(log_source) => {
                ...
                egui::CentralPanel::default()
                    .frame(viewport_frame)
                    .show(ui, |ui| {
                        if let Some(dl_uri::RedapUri::DatasetData(uri)) = log_source.redap_uri()
            ...
            Route::RedapEntry {
                origin,
                kind: dl_viewer_context::RedapEntryKind::Folder(path_prefix),
            } => {
                Self::left_panel_ui(
                    &mut self.recording_panel,
                    &mut self.blueprint_tree,
    ...
    fn left_panel_ui(
        recording_panel: &mut dl_recording_panel::RecordingPanel,
        blueprint_tree: &mut dl_blueprint_tree::BlueprintTree,
    ...
    pub fn blueprint_query_for_viewer(&mut self, blueprint: Option<&EntityDb>) -> LatestAtQuery {
        if self.app_options.inspect_blueprint_timeline {
            if self.blueprint_time_control.play_state() == PlayState::Following {
                ...
                LatestAtQuery::latest(dl_viewer_context::blueprint_timeline())
            } else {
        ...
        if self.app_options.inspect_blueprint_timeline {
            if self.blueprint_time_control.play_state() == PlayState::Following {
                ...
                Some(LatestAtQuery::latest(
                    dl_viewer_context::blueprint_timeline(),
                ))
            } else {
...
///
/// See [`dl_ui::UiExt::re_hyperlink`] for displaying hyperlinks in the UI.
...
impl dl_byte_size::MemUsageTreeCapture for AppState {
    fn capture_mem_usage_tree(&self) -> dl_byte_size::MemUsageTree {
        use dl_byte_size::SizeBytes as _;

        let mut tree = dl_byte_size::MemUsageNode::default();
        tree.add(
            "blueprint_undo_state",
        ...
        tree.add(
            "view_states",
            dl_byte_size::MemUsageTreeCapture::capture_mem_usage_tree(&self.view_states),
        );
        tree.into_tree()
    }
}
