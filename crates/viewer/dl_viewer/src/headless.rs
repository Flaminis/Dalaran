...
pub fn run_headless_app(
    ...
    dl_log::info!("Headless viewer running at {}x{}.", size.x, size.y);
    ...
    loop {
        ...
        if has_pending_close(&harness) {
            dl_log::info!("Headless viewer received close request, shutting down.");
            return Ok(());
        }
...
fn handle_pending_screenshots(harness: &mut egui_kittest::Harness<'_, App>) {
    ...
    let rgba = match harness.render() {
        Ok(rgba) => rgba,
        Err(err) => {
            dl_log::error!("Failed to render headless screenshot: {err}");
            return;
        }
    };
