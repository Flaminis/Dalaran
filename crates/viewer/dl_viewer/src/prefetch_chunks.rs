...
pub struct RecordingPrefetchInfo {
    ...
    pub time_cursor: Option<dl_entity_db::PrefetchTimeCursor>,
...
fn make_load_fn<'a>(
    ...
    connection_registry: &'a dl_redap_client::ConnectionRegistryHandle,
    origin: &'a dl_uri::Origin,
) -> impl Fn(RecordBatch) -> dl_entity_db::ChunkPromise + 'a {
    move |rb| {
        ...
        let fut = async move {
            let mut client = connection_registry.client(origin).await.map_err(|err| {
                dl_log::warn_once!("Failed to connect to server: {err}");
            })?;
            load_chunks(&mut client, &rb).await.map_err(|err| {
                dl_log::warn_once!("{err}");
            })
        };
...
async fn load_chunks(client: &mut ConnectionClient, batch: &RecordBatch) -> ApiResult<Vec<Chunk>> {
    ...
    dl_log::trace!("Requesting {} chunk(s) from server…", batch.num_rows());

    let chunk_stream = client.fetch_segment_chunks_by_id(batch).await?;
    let mut chunk_stream =
        dl_redap_client::fetch_chunks_response_to_chunk_and_segment_id(chunk_stream);
    ...
    dl_log::trace!("Finished downloading {} chunk(s).", batch.num_rows());
