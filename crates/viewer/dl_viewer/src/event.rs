...
use dl_log_types::{ApplicationId, RecordingId, TimeReal, Timeline, TimelineName};
use dl_sdk_types::SegmentId;
use dl_viewer_context::{ContainerId, Item, ItemCollection, ItemContext, ViewId};
    ...
    fn from_db_and_kind(db: &EntityDb, kind: ViewerEventKind) -> Self {
        let segment_id = db.data_source.as_ref().and_then(|ds| {
            if let LogSource::RedapGrpcStream {
                uri: dl_uri::DatasetSegmentUri { segment_id, .. },
                ..
            } = ds
            {
...
pub enum SelectionChangeItem {
    ...
    Entity {
        ...
        instance_id: dl_log_types::Instance,
        ...
        match item {
            ...
            Item::View(view_id) => Some(Self::View {
                ...
                view_name: if let Some(name) = get_view_name(blueprint, view_id) {
                    ...
                    dl_log::debug!("failed to get view name for view id {view_id}");
                    return None;
                },
            }),
            Item::Container(container_id) => Some(Self::Container {
                ...
                container_name: if let Some(name) = get_container_name(blueprint, container_id) {
                    ...
                    dl_log::debug!("failed to get container name for container id {container_id}");
                    return None;
                },
            }),
...
impl ViewerEventDispatcher {
    ...
    pub fn on_recording_open(&self, db: &EntityDb) {
        let source = db
            ...
            .unwrap_or(dl_log_types::StoreSource::Unknown);
        self.dispatch(ViewerEvent::from_db_and_kind(
            db,
            ViewerEventKind::RecordingOpen {
                source: match &source {
                    dl_log_types::StoreSource::Unknown => "unknown",
                    dl_log_types::StoreSource::CSdk => "cpp",
                    dl_log_types::StoreSource::PythonSdk(_python_version) => "python",
                    dl_log_types::StoreSource::RustSdk {
                        rustc_version: _,
                        llvm_version: _,
                    } => "rust",
                    dl_log_types::StoreSource::File { file_source: _ } => "file",
                    dl_log_types::StoreSource::Viewer => "viewer",
                    dl_log_types::StoreSource::Other(v) => v.as_str(),
                }
                .into(),
                version: db
                    .store_info()
...
fn instance_is_all(v: &dl_log_types::Instance) -> bool {
    v.is_all()
}
...
mod serde {
    ...
    pub mod entity_path {
        ...
        pub fn serialize<S>(v: &dl_log_types::EntityPath, serializer: S) -> Result<S::Ok, S::Error>
        where
            S: Serializer,
        {
        ...
        pub fn deserialize<'de, D>(deserializer: D) -> Result<dl_log_types::EntityPath, D::Error>
        where
            D: Deserializer<'de>,
        {
            let s: String = Deserialize::deserialize(deserializer)?;
            dl_log_types::EntityPath::parse_strict(&s).map_err(serde::de::Error::custom)
        }
    }
    ...
    pub mod instance_id {
        ...
        pub fn serialize<S>(v: &dl_log_types::Instance, serializer: S) -> Result<S::Ok, S::Error>
        where
            S: Serializer,
        {
        ...
        pub fn deserialize<'de, D>(deserializer: D) -> Result<dl_log_types::Instance, D::Error>
        where
            D: Deserializer<'de>,
        {
            let v: Option<u64> = Deserialize::deserialize(deserializer)?;
            match v {
                Some(v) => Ok(dl_log_types::Instance::from(v)),
                None => Ok(dl_log_types::Instance::ALL),
            }
        }
    }
    ...
    pub mod blueprint_id {
        ...
        pub fn serialize<S, T>(
            v: &dl_viewer_context::BlueprintId<T>,
            serializer: S,
        ) -> Result<S::Ok, S::Error>
        where
            S: Serializer,
            T: dl_viewer_context::BlueprintIdRegistry,
        {
        ...
        pub fn deserialize<'de, D, T>(
            deserializer: D,
        ) -> Result<dl_viewer_context::BlueprintId<T>, D::Error>
        where
            D: Deserializer<'de>,
            T: dl_viewer_context::BlueprintIdRegistry,
        {
            let s: String = Deserialize::deserialize(deserializer)?;
            dl_sdk_types::external::uuid::Uuid::try_parse(&s)
                .map_err(serde::de::Error::custom)
                .map(dl_viewer_context::BlueprintId::from)
        }
    }
    ...
    pub mod recording_id {
        ...
        pub fn serialize<S>(v: &dl_log_types::RecordingId, serializer: S) -> Result<S::Ok, S::Error>
        where
            S: Serializer,
        {
        ...
        pub fn deserialize<'de, D>(deserializer: D) -> Result<dl_log_types::RecordingId, D::Error>
        where
            D: Deserializer<'de>,
        {
            let s: String = Deserialize::deserialize(deserializer)?;
    ...
    pub mod time_real {
        ...
        pub fn serialize<S>(v: &dl_log_types::TimeReal, serializer: S) -> Result<S::Ok, S::Error>
        where
            S: Serializer,
        {
        ...
        pub fn deserialize<'de, D>(deserializer: D) -> Result<dl_log_types::TimeReal, D::Error>
        where
            D: Deserializer<'de>,
        {
            let v: f64 = Deserialize::deserialize(deserializer)?;
            Ok(dl_log_types::TimeReal::from(v))
        }
    }
}
