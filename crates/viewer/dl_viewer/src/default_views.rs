...
fn populate_view_class_registry_with_builtin(
    ...
    dl_tracing::profile_function!();
    view_class_registry.add_class::<dl_view_bar_chart::BarChartView>(
        reflection,
        app_options,
        fallback_registry,
    )?;
    view_class_registry.add_class::<dl_view_dataframe::DataframeView>(
        reflection,
        app_options,
        fallback_registry,
    )?;
    view_class_registry.add_class::<dl_view_graph::GraphView>(
        reflection,
        app_options,
        fallback_registry,
    )?;
    #[cfg(feature = "map_view")]
    view_class_registry.add_class::<dl_view_map::MapView>(
        reflection,
        app_options,
        fallback_registry,
    )?;
    view_class_registry.add_class::<dl_view_spatial::SpatialView2D>(
        reflection,
        app_options,
        fallback_registry,
    )?;
    view_class_registry.add_class::<dl_view_spatial::SpatialView3D>(
        reflection,
        app_options,
        fallback_registry,
    )?;
    view_class_registry.add_class::<dl_view_tensor::TensorView>(
        reflection,
        app_options,
        fallback_registry,
    )?;
    view_class_registry.add_class::<dl_view_text_document::TextDocumentView>(
        reflection,
        app_options,
        fallback_registry,
    )?;
    view_class_registry.add_class::<dl_view_text_log::TextView>(
        reflection,
        app_options,
        fallback_registry,
    )?;
    view_class_registry.add_class::<dl_view_time_series::TimeSeriesView>(
        reflection,
        app_options,
        fallback_registry,
    )?;
    view_class_registry.add_class::<dl_view_state_timeline::StateTimelineView>(
        reflection,
        app_options,
        fallback_registry,
    )?;
    ...
    fn test_view_selection_ui() {
        ...
        for egui_theme in [egui::Theme::Light, egui::Theme::Dark] {
            for entry in view_class_registry.iter_registry() {
                ...
                let snapshot_options = dl_ui::testing::default_snapshot_options_for_ui()
                    .output_path(format!(
