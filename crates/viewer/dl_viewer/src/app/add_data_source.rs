use dl_data_source::{LogDataSource, LogDataSourceAnalytics};
use dl_entity_db::LogSource;
use dl_log_channel::{LogReceiver, RecordingOpenBehavior};
use dl_log_encoding::RrdMetadata;
use dl_log_types::{EntryName, StoreId};
use dl_viewer_context::{StoreHub, SystemCommand, SystemCommandSender as _};
...
use dl_protos::cloud::v1alpha1::ext::DataSource;
use dl_protos::common::v1alpha1::ext::{IfDuplicateBehavior, SegmentId};

impl App {
    #[expect(clippy::needless_pass_by_ref_mut)]
    pub fn add_log_receiver(&mut self, rx: LogReceiver) {
        dl_log::debug!("Adding new log receiver: {}", rx.source());
    ...
    pub(super) fn load_data_source(
            ...
            LogDataSource::HttpUrl { url } => {
                ...
                if all_sources.any(|source| source.is_same_ignoring_uri_fragments(&new_source)) {
                    if let Some(entity_db) = store_hub.find_recording_store_by_source(&new_source) {
                        ...
                        dl_log::debug_assert!(store_id.is_recording()); // `find_recording_store_by_source` should have filtered for recordings rather than blueprints.
                        drop(all_sources);
        ...
        let stream = data_source.clone().stream_with_options(
                ...
                && matches!(open_behavior, RecordingOpenBehavior::Background)
            {
                ...
                dl_redap_client::StreamingOptions {
                    download: dl_redap_client::SegmentDownload::SEGMENT,
                    ..Default::default()
                }
            } else {
                Default::default()
        ...
        match stream {
            ...
            Err(err) => {
                dl_log::error!("Failed to open data source: {}", dl_error::format(err));
            }
        }
    }
    ...
    pub(super) fn fetch_pending_blueprint(&mut self, store_hub: &mut StoreHub, store_id: &StoreId) {
        ...
        match data_source.stream_with_options(
            ...
            &self.connection_registry,
            dl_redap_client::StreamingOptions {
                download: dl_redap_client::SegmentDownload::BLUEPRINT,
                ..Default::default()
            },
        ) {
            Ok(rx) => {
            ...
            Err(err) => {
                dl_log::error!("Failed to fetch blueprint: {}", dl_error::format(err));
            }
        }
    }
    ...
    fn register_via_internal_catalog(
        ...
        registration: impl std::future::Future<Output = anyhow::Result<dl_uri::DatasetSegmentUri>>
        + 'static,
    ) {
        let sender = self.command_sender.clone();
...
fn record_catalog_load_analytics(
    ...
    if let Some(analytics) = dl_analytics::Analytics::global_or_init() {
        analytics.record(dl_analytics::event::LoadDataSource {
...
async fn prepare_opfs_upload(
    reader: &impl dl_async::AsyncReadAt,
    source_path: &std::path::Path,
...
async fn read_rrd_metadata(reader: &impl dl_async::AsyncReadAt) -> anyhow::Result<RrdMetadata> {
            ...
            .all(|store_id| !store_id.is_blueprint())
    {
        ...
        Ok(dl_log_encoding::enumerate_legacy_metadata(reader).await?)
    }
}

#[cfg(target_arch = "wasm32")]
async fn opfs_upload_matches(path: &std::path::Path, expected_size: u64) -> anyhow::Result<bool> {
    match dl_web::fs::metadata(path).await {
