use dl_byte_size::NamedMemUsageTree;
use dl_entity_db::LogSource;
use dl_renderer::WgpuResourcePoolStatistics;
use dl_ui::{HasDesignTokens as _, UiExt as _, WindowFrameConfig};
use dl_viewer_context::{ActiveStoreContext, StorageContext, store_hub::StoreHubStats};
...
impl App {
            ...
            .show(ui, |ui| {
                    ...
                    .get_mut::<dl_renderer::RenderContext>()
                {
                    ...
                    if is_start_of_new_frame {
                        ...
                        dl_ui::UrlDecorator::set(
                            &self.egui_ctx,
                            dl_viewer_context::make_url_decorator(
                                url_name_lookup,
                                self.egui_ctx.theme(),
                            ),
                        );
    ...
    fn dev_panel_ui(
        ...
        storage_context: &dl_viewer_context::StorageContext<'_>,
    ) {
    ...
    /// [`dl_viewer_context::AppOptions::custom_window_decorations`]: it is the state the
    ...
    pub(crate) fn custom_window_decorations(&self) -> bool {
        let custom = match self.window_decorations_request {
            WindowDecorationsRequest::NotSent => dl_ui::custom_window_decorations_default(),
...
pub(super) fn file_saver_progress_ui(
    ...
    if background_tasks.is_file_save_in_progress() {
        ...
        if let Some(res) = background_tasks.poll_file_saver_promise() {
            ...
            match res {
                Ok(path) => {
                    dl_log::info!("File saved to {path:?}."); // this will also show a notification the user
                }
                Err(err) => {
                    dl_log::error!("{err}"); // this will also show a notification the user
                }
            }
        } else {
