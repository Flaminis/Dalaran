...
impl App {
    ...
    ///
    /// Only this may act on [`dl_viewer_context::AppOptions::custom_window_decorations`];
    ...
    fn sync_native_window_decorations(&mut self, egui_ctx: &egui::Context) {
        if !dl_ui::supports_custom_decorations(egui_ctx.os()) {
            return;
        }
    ...
    fn receive_messages(&mut self, store_hub: &mut StoreHub, egui_ctx: &egui::Context) {
        ...
        while let Some((channel_source, msg)) = self.rx_log.try_recv() {
            dl_log::trace!("Received a message from {channel_source:?}"); // Used by `test_ui_wakeup` test app!

            if let Some(dl_uri::RedapUri::DatasetData(uri)) = channel_source.redap_uri() {
                self.connection_registry.clear_uri_error(uri);
            }

            let msg = match msg.payload {
                dl_log_channel::SmartMessagePayload::Msg(msg) => msg,

                dl_log_channel::SmartMessagePayload::Flush { on_flush_done } => {
                    dl_tracing::profile_scope!("on_flush_done");
                    on_flush_done();
                    continue;
                }

                dl_log_channel::SmartMessagePayload::Quit(err) => {
                    if let Some(err) = err {
                        dl_log::warn!(
                        ...
                        dl_log::debug!("Data source {} has finished", msg.source);
                        ...
                        {
                            dl_log::warn!("Failed to register table blueprint: {err}");
                        }
                    }
                    continue;
                }
    ...
    fn receive_log_msg(
        ...
        match entity_db_add_result {
            ...
            Err(err) => {
                dl_log::error_once!("Failed to add incoming msg: {err}");
            }
        }
                    ...
                    if let Some(info) = entity_db.store_info() {
                        ...
                        if cmd.make_default {
                            store_hub
                                ...
                                .unwrap_or_else(|err| {
                                    dl_log::warn!("Failed to make blueprint default: {err}");
                                });
                        }
                        if cmd.make_active {
                            store_hub
                                ...
                                .unwrap_or_else(|err| {
                                    dl_log::warn!("Failed to make blueprint active: {err}");
                                });
    ...
    fn process_store_events_for_db(
        ...
        store_hub: &mut StoreHub,
        store_id: &StoreId,
        store_events: &[dl_chunk_store::ChunkStoreEvent],
    ) {
        dl_tracing::profile_function!();
    ...
    fn receive_table_msg(
        ...
        if let Err(err) = store.add_record_batch(data) {
            dl_log::error!("Failed to load table {id}: {err}");
        } else {
            if store_hub.insert_table_store(id.clone(), store).is_some() {
                dl_log::debug!("Overwritten table store with id: `{id}`");
            } else {
                dl_log::debug!("Inserted table store with id: `{id}`");
            }
            self.command_sender
                .send_system(SystemCommand::set_selection(
                    dl_viewer_context::Item::TableId(id),
                ));
    ...
    fn on_new_store(
        ...
        match channel_source.open_behavior() {
            ...
            RecordingOpenBehavior::OpenAndSelect => {
                ...
                match store_id.kind() {
                    StoreKind::Recording => {
                        dl_log::trace!("Opening a new recording: '{store_id:?}'");
    ...
    fn receive_data_source_ui_command(
        ...
        match ui_command {
            DataSourceUiCommand::SetUrlFragment { store_id, fragment } => {
                match dl_uri::Fragment::from_str(&fragment) {
                    Ok(fragment) => {
                        self.command_sender
            ...
            DataSourceUiCommand::SaveScreenshot {
                ...
                self.command_sender
                    .send_system(SystemCommand::SaveScreenshot {
                        target: dl_viewer_context::ScreenshotTarget::SaveToPath(file_path),
                        view_id,
                        notify: false,
                    });
            }
            ...
            // Move the time cursor of a recording (dl_viewer_mcp's `SetTimeCursor`).
    ...
    /// Snapshot the current viewer state for `dl_viewer_mcp`'s `GetViewerState`:
    ...
    /// Resolve and apply a time-cursor move for `dl_viewer_mcp`'s `SetTimeCursor`.
    ///
    ...
    fn receive_fetched_chunks(&self, store_hub: &mut StoreHub) {
        ...
        for store_id in store_ids {
            ...
            if cfg!(debug_assertions) && db.can_fetch_chunks_from_redap() {
                dl_tracing::profile_scope!("debug-sanity-check");
                ...
                for missing_chunk_id in store.tracked_chunk_ids().missing_virtual {
                    ...
                    let all_roots_are_fully_loaded = roots.iter().all(|root_id| {
                        ...
                        if let Some(root_info) = root_info {
                            ...
                            dl_log::debug_warn_once!("Failed to find root chunk");
                            false
                        }
                    });
            ...
            if db.can_fetch_chunks_from_redap() {
                dl_tracing::profile_scope!("recording");
                    ...
                    .receive_finished(self.egui_ctx.time())
                {
                    match db.add_chunk(&std::sync::Arc::new(chunk)) {
                        ...
                        Err(err) => {
                            dl_log::warn_once!("add_chunk failed: {err}");
                        }
                    }
                }
    ...
    pub(super) fn make_store_active_and_highlight(
        ...
        self.command_sender
            .send_system(SystemCommand::set_selection(
                dl_viewer_context::Item::StoreId(store_id.clone()),
            ));
    ...
    fn validate_loaded_events(&self, store_events: &[dl_chunk_store::ChunkStoreEvent]) {
        dl_tracing::profile_function!();
            ...
            for component_descr in chunk.components().component_descriptors() {
                if let Some(archetype_name) = component_descr.archetype {
                    if let Some(archetype) = self.reflection.archetypes.get(&archetype_name) {
                        ...
                        dl_log::trace_once!("Unknown archetype: {archetype_name}");
                    }
                }
            }
        }
    ...
    pub(super) fn purge_memory_if_needed(&mut self, store_hub: &mut StoreHub) {
        ...
        use dl_memory::MemoryUse;

        let limit = self.app_options().memory_limit;
        let mut mem_use_before = MemoryUse::capture();

        let default_limit = dl_memory::MemoryLimit::default_for_current_platform();
        ...
        if let Some(minimum_fraction_to_purge) = limit.is_exceeded_by(&mem_use_before) {
            dl_log::info_once!("Reached memory limit of {limit}. Freeing up data…");

            let fraction_to_purge = (minimum_fraction_to_purge + 0.2).clamp(0.25, 1.0);

            dl_log::trace!("RAM limit: {limit}");
            if let Some(resident) = mem_use_before.resident {
                dl_log::trace!("Resident: {}", format_bytes(resident as _),);
            }
            if let Some(counted) = mem_use_before.counted {
                dl_log::trace!("Counted: {}", format_bytes(counted as _));
            }
            if external_mem > 0 {
                dl_log::trace!("External: {}", format_bytes(external_mem as _));
            }

            dl_tracing::profile_scope!("pruning");
            if let Some(counted) = mem_use_before.counted {
                dl_log::trace!(
    ...
    fn prefetch_chunks(&self, store_hub: &mut StoreHub) {
        ...
        use dl_entity_db::ChunkPrefetchOptions;
        ...
        if memory_limit == dl_memory::MemoryLimit::ZERO {
            dl_log::warn_once!("Very little memory budget left for prefetching recordings.");
        }
        ...
        for recording in store_hub.store_bundle().recordings() {
            ...
            let time_cursor = match open_kind {
                ...
                RecordingOpenKind::Preview => {
                    ...
                    let timeline =
                        dl_chunk::Timeline::pick_best_timeline(timelines.values(), |t| {
                            recording.num_temporal_rows_on_timeline(t.name())
                        });

                    Some(dl_entity_db::PrefetchTimeCursor {
                        time_cursor: dl_log_types::TimelinePoint {
                            name: *timeline.name(),
                            typ: timeline.typ(),
