...
use dl_log_types::{ApplicationId, LogMsg, RecordingId, StoreId, StoreKind};
use dl_sdk_types::blueprint::components::PlayState;
use dl_ui::{RecordingCommand, UICommand, UICommandSender as _};
...
impl App {
    ...
    fn run_system_command(
        ...
        dl_tracing::profile_function!(cmd.debug_name());
        ...
        match cmd {
            ...
            SystemCommand::ActivateApp(app_id) => {
                ...
                if let Some(recording_id) = store_hub.earliest_recording_for_app(&app_id) {
                    ...
                    dl_log::warn_once!("Can't switch app-id - we have no recording for it");
            ...
            SystemCommand::AddReceiver(rx) => {
                dl_log::debug!("Received AddReceiver");
            ...
            SystemCommand::SetRoute(new_route) => {
                ...
                if let Route::Loading(source) = &new_route
                    && let Some(dl_uri::RedapUri::DatasetData(dataset_uri)) = source.redap_uri()
                    && store_hub
                        .store_bundle()
                        .entity_dbs()
                ...
                if matches!(new_route, Route::Loading(_)) {
                    self.state
                        .selection_state
                        .set_selection(dl_viewer_context::ItemCollection::default());
                }
            ...
            SystemCommand::OpenSettings => {
                ...
                dl_analytics::record(|| dl_analytics::event::SettingsOpened {});
            }
            ...
            SystemCommand::AddRedapServer(origin) => {
                if origin == *dl_redap_browser::EXAMPLES_ORIGIN {
                    return;
                }
            ...
            SystemCommand::RefreshRedapServer(origin) => {
                ...
                if self.state.redap_servers.has_server(&origin) {
                    self.state
                        .redap_servers
                        .send_command(dl_redap_browser::Command::RefreshCollection(origin));
                }
            }
            ...
            SystemCommand::RedapServer(command) => {
                let dl_ui::RedapServerCommand { origin, kind } = command;
                match kind {
                    dl_ui::RedapServerCommandKind::Refresh => {
                        self.command_sender
                            .send_system(SystemCommand::RefreshRedapServer(origin));
                    }
                    dl_ui::RedapServerCommandKind::Edit => {
                        self.command_sender
                            .send_system(SystemCommand::EditRedapServerModal(
                                dl_viewer_context::EditRedapServerModalCommand::new(origin),
                            ));
                    }
                    dl_ui::RedapServerCommandKind::CopyUrl => {
                        let url = origin.to_string();
                        dl_log::info!("Copied {url:?} to clipboard");
                        egui_ctx.copy_text(url);
                    }
                    dl_ui::RedapServerCommandKind::Remove => {
                        self.command_sender
            ...
            SystemCommand::Table(command) => {
                let dl_ui::TableCommand {
                    origin,
                    entry_id,
                    kind,
                } = command;
                match kind {
                    dl_ui::TableCommandKind::Refresh => {
                        self.state
                            .redap_servers
                            .refresh_entry(&origin, entry_id, egui_ctx);
                    }
            ...
            SystemCommand::ClearActiveBlueprintAndEnableHeuristics => {
                dl_log::debug!("Clear and generate new blueprint");
            ...
            SystemCommand::ClearActiveBlueprint => {
                ...
                dl_log::debug!("Reset blueprint to default");
            ...
            SystemCommand::AppendToStore(store_id, chunks) => {
                ...
                for chunk in chunks {
                    match db.add_chunk(&Arc::new(chunk)) {
                        Ok(_store_events) => {}
                        Err(err) => {
                            dl_log::warn_once!("Failed to append chunk: {err}");
                        }
                    }
                }
            }
            ...
            SystemCommand::FileSaver(file_saver) => {
                if let Err(err) = self.background_tasks.spawn_file_saver(file_saver) {
                    dl_log::error!("Failed to save file: {err}");
                }
            }
            ...
            SystemCommand::SetAuthCredentials {
                ...
                let credentials =
                    match dl_auth::oauth::Credentials::try_new(access_token, None, email) {
                        Ok(credentials) => credentials,
                        Err(err) => {
                            dl_log::error!("Failed to create credentials: {err}");
                            return;
                        }
                    };
                if let Err(err) = credentials.ensure_stored() {
                    dl_log::error!("Failed to store credentials: {err}");
                }
            }
            SystemCommand::Logout => {
                ...
                match dl_auth::oauth::clear_credentials(signed_out_url) {
                    Ok(Some(outcome)) => {
                    ...
                    Ok(None) => {
                        dl_log::debug!("No session to logout from");
                    }
                    Err(err) => {
                        dl_log::error!("Failed to logout: {err}");
                    }
                }
            ...
            SystemCommand::SaveScreenshot {
                ...
                if let Some(view_id) = view_id {
                    ...
                    if let Some(view_info) = self.egui_ctx.memory_mut(|mem| {
                        mem.caches
                            .cache::<dl_viewer_context::ViewRectPublisher>()
                            .get(&view_id)
                            .cloned()
                    }) {
                        let dl_viewer_context::PublishedViewInfo { name, rect } = view_info;
                        let rect = rect.shrink(2.5); // Hacky: Shrink so we don't accidentally include the border of the view.
                        if !rect.is_positive() {
                            dl_log::warn!("View too small for a screenshot");
                            return;
                        }

                        self.egui_ctx
                            .send_viewport_cmd(egui::ViewportCommand::Screenshot(
                                egui::UserData::new(dl_viewer_context::ScreenshotInfo {
                                    ui_rect: Some(rect),
    ...
    fn run_ui_command(
        ...
        match cmd {
            #[cfg(not(target_arch = "wasm32"))]
            UICommand::Open => {
                use dl_data_source::LogDataSource;
                use dl_log_types::FileSource;
            ...
            UICommand::NextRecording => {
                self.state
                    .recording_panel
                    .send_command(dl_recording_panel::RecordingPanelCommand::SelectNextRecording);
            }
            UICommand::PreviousRecording => {
                self.state.recording_panel.send_command(
                    dl_recording_panel::RecordingPanelCommand::SelectPreviousRecording,
                );
            }
            ...
            UICommand::CaptureProfileTrace => {
                if self.profile_capture.is_none() {
                    self.profile_capture = Some(dl_tracing::ProfileCapture::start(5));
            ...
            UICommand::ResetEguiMemory => {
                ...
                dl_ui::apply_style_and_install_loaders(egui_ctx);
            }
                ...
                match ViewerOpenUrl::from_route(storage_context.hub, route) {
                    Ok(mut url) => {
                        if let Some(fragment) = url.fragment_mut() {
                            ...
                            {
                                fragment.time_selection = Some(dl_uri::TimeSelection {
                                    timeline: *timeline,
                                    range: time_selection.to_int(),
                                });
                            } else {
                                dl_log::warn!("No timeline selection to copy");
                            }
                        } else {
                            dl_log::warn!(
            ...
            UICommand::RestartWithWebGl => {
                if let Err(err) =
                    dl_web::browser::set_url_parameter_and_refresh("renderer", "webgl")
                {
                    dl_log::error!(
            ...
            UICommand::RestartWithWebGpu => {
                if let Err(err) =
                    dl_web::browser::set_url_parameter_and_refresh("renderer", "webgpu")
                {
                    dl_log::error!(
    ...
    fn run_recording_command(
        ...
        use dl_ui::RecordingCommandKind;
        ...
        match kind {
            RecordingCommandKind::Save => {
                #[cfg(target_arch = "wasm32")] // Web
                {
                    if let Err(err) = save_active_recording(self, store_context) {
                        dl_log::error!("Failed to save recording: {err}");
                    }
                }
                ...
                #[cfg(not(target_arch = "wasm32"))] // Native
                {
                    ...
                    if selected_stores.is_empty() {
                            ...
                            .pick_folder()
                        {
                            ...
                            dl_log::info!("No folder selected - recordings not saved.");
                        }
                    }
                }
            }
            ...
            RecordingCommandKind::SaveBlueprint => {
                if let Err(err) = save_blueprint(self, store_context) {
                    dl_log::error!("Failed to save blueprint: {err}");
                }
            }
            ...
            RecordingCommandKind::AddViewOrContainer => {
                if let Some(ctx) = store_context {
                    ...
                    let viewport = dl_viewport_blueprint::ViewportBlueprint::from_db(
                        ctx.blueprint,
                        &blueprint_query,
                    );
                    ...
                    let target_container_id =
                        if let Some(dl_viewer_context::Item::Container(container_id)) =
                    ...
                    dl_viewport_blueprint::ui::show_add_view_or_container_modal(
                        target_container_id,
                    );
                }
            }
    ...
    fn save_many_recordings(&mut self, stores: &[&EntityDb], folder: &std::path::Path) {
        ...
        dl_log::info!("Saving {num_stores} recordings to {}…", folder.display());
                ...
                .spawn_threaded_promise(file_name, move || {
                    ...
                    if num_remaining == 0 {
                        if any_error.load(Ordering::Relaxed) {
                            dl_log::error!("Some recordings failed to save.");
                        } else {
                            dl_log::info!("{num_stores} recordings successfully saved to {folder}");
                        }
                    }
    ...
    fn copy_entity_hierarchy_to_clipboard(
        ...
        let Some(entity_db) = store_context.as_ref().map(|ctx| ctx.recording) else {
            dl_log::warn!("Could not copy entity hierarchy: No active recording");
            return;
        };
    ...
    fn reset_viewer(&mut self, store_hub: &mut StoreHub, egui_ctx: &egui::Context) {
        ...
        if let Err(err) = crate::reset_viewer_persistence() {
            dl_log::warn!("Failed to reset viewer: {err}");
        }
    }
    ...
    fn is_back_destination_open(
        ...
        let origin_reachable = |origin: &dl_uri::Origin| {
            origin == &*dl_redap_browser::EXAMPLES_ORIGIN
    ...
    fn fallback_route_after_close(store_hub: &StoreHub, closing: &StoreId) -> Route {
        ...
        match redap_uri {
            Some(dl_uri::RedapUri::DatasetData(uri)) => Route::RedapEntry {
                origin: uri.origin,
                kind: dl_viewer_context::RedapEntryKind::Entry(uri.dataset_id.into()),
            },

            Some(uri) if !matches!(uri, dl_uri::RedapUri::Proxy(_)) => {
...
/// Propagates [`dl_viewer_context::TimeControlResponse`] to [`ViewerEventDispatcher`].
pub(super) fn handle_time_ctrl_event(
    recording: &EntityDb,
    events: Option<&ViewerEventDispatcher>,
    response: &dl_viewer_context::TimeControlResponse,
) {
...
fn open_file_dialog_native(_: crate::MainThreadToken) -> Vec<std::path::PathBuf> {
    ...
    let supported: Vec<_> = if dl_importer::iter_external_importers().len() == 0 {
...
fn save_entity_db(
    ...
    cfg_select! {
        ...
        _ => {
            // Native
            let path = {
                dl_tracing::profile_scope!("file_dialog");
                rfd::FileDialog::new()
