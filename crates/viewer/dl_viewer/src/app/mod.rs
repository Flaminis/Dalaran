...
use dl_entity_db::entity_db::EntityDb;
use dl_log_channel::{LogReceiverSet, RecordingOpenBehavior, SaveScreenshotError};
use dl_log_types::{ApplicationId, FileSource, RecordingId, StoreId};
use dl_redap_client::ConnectionRegistryHandle;
use dl_sdk_types::blueprint::components::PlayState;
use dl_types_core::reflection::ComponentReflectionMap;
use dl_ui::{ContextExt as _, UICommand, UICommandSender as _, notifications};
use dl_viewer_context::open_url::{OpenUrlOptions, ViewerOpenUrl};
use dl_viewer_context::store_hub::{BlueprintPersistence, StoreHub};
...
pub struct App {
    ...
    ram_limit_warner: dl_memory::RamLimitWarner,
    ...
    #[cfg(not(target_arch = "wasm32"))]
    profile_capture: Option<dl_tracing::ProfileCapture>,

    /// Listens to the local text log stream
    text_log_rx: crossbeam::channel::Receiver<dl_log::LogMsg>,
    ...
    cmd_palette: dl_ui::CommandPalette,
    ...
    reflection: dl_types_core::reflection::Reflection,
...
impl App {
    ...
    pub fn with_commands(
        ...
        let connection_registry = connection_registry
            .unwrap_or_else(dl_redap_client::ConnectionRegistry::new_with_stored_credentials);
        ...
        if connection_registry.should_use_stored_credentials() {
            ...
            dl_auth::credentials::subscribe_auth_changes(move |user| {
            ...
            async_runtime.spawn_future(async move {
                dl_auth::credentials::CliCredentialsProvider::new()
                    .get_token()
                    .await
                    .ok();
            });
        ...
        let reflection = dl_sdk_types::reflection::generate_reflection().unwrap_or_else(|err| {
            dl_log::error!(
        ...
        let mut component_fallback_registry =
            dl_component_fallbacks::create_component_fallback_registry();
        ...
        .unwrap_or_else(|err| {
            dl_log::error!("Failed to create view class registry: {err}");
            Default::default()
        });
        ...
        let mut component_ui_registry = dl_component_ui::create_component_ui_registry();
        dl_data_ui::register_component_uis(&mut component_ui_registry);

        let (_adapter_backend, _device_tier) = creation_context.wgpu_render_state.as_ref().map_or(
            (
                wgpu::Backend::Noop,
                dl_renderer::device_caps::DeviceCapabilityTier::Limited,
            ),
            |render_state| {
                let egui_renderer = render_state.renderer.read();
                let render_ctx = egui_renderer
                    .callback_resources
                    .get::<dl_renderer::RenderContext>();
        ...
        if let Some(analytics) = dl_analytics::Analytics::global_or_init() {
                ...
                ctx.output_mut(|o| {
                    for command in &mut o.commands {
                        if let egui::output::OutputCommand::CopyText(text) = command {
                            *text = dl_format::remove_number_formatting(text);
                        }
                    }
                });
            }),
...

        {
            ...
            creation_context.egui_ctx.on_begin_pass(
                ...
                Arc::new(move |ctx| {
                    ...
                    if let Some(kind) = dl_ui::consume_timeline_shortcut(ctx) {
                        ctx.data_mut(|data| {
        ...
        Self {
            ...
            ram_limit_warner: dl_memory::RamLimitWarner::warn_at_fraction_of_max(0.75),
    ...
    #[cfg(not(target_arch = "wasm32"))]
    pub fn set_profiler(&mut self, profiler: dl_tracing::Profiler) {
    ...
    pub fn set_examples_manifest_url(&mut self, url: String) {
        dl_log::info!("Using manifest_url={url:?}");
        self.state.set_examples_manifest_url(&self.egui_ctx, url);
    }

    pub fn build_info(&self) -> &dl_build_info::BuildInfo {
        &self.build_info
    }
    ...
    pub fn reflection(&self) -> &dl_types_core::reflection::Reflection {
        &self.reflection
    }
        ...
        match ViewerOpenUrl::parse_with_options(
            ...
            Err(err) => {
                if err.to_string().contains(url) {
                    ...
                    dl_log::error!(?url, "Failed to open URL: {err}");
                }
            }
        }
    }
    ...
    /// Update the active [`dl_viewer_context::TimeControl`]. And if the blueprint inspection
    ...
    fn move_time(&mut self) {
            ...
            && let Some(blueprint) = store_hub.active_blueprint_for_app(store_id.application_id())
        {
            ...
            let blueprint_query = self
                ...
                .unwrap_or_else(|| {
                    dl_chunk::LatestAtQuery::latest(dl_viewer_context::blueprint_timeline())
                });
            ...
            if let Some(recording) = store_hub.entity_db(store_id) {
                ...
                let response = time_ctrl.update(
                    recording,
                    &dl_viewer_context::TimeControlUpdateParams {
                        stable_dt,
            ...
            if self.app_options().inspect_blueprint_timeline {
                ...
                let dl_viewer_context::TimeControlResponse {
                    needs_repaint,
                    playing_change: _,
                    timeline_change: _,
                    time_change: _,
        ...
        if self
            ...
            == dl_viewer_context::NeedsRepaint::Yes
        {
            self.egui_ctx.request_repaint();
        }
    }
    ...
    pub fn add_archetype_reflection(
        &mut self,
        archetype_name: dl_sdk_types::ArchetypeName,
        archetype_reflection: dl_sdk_types::reflection::ArchetypeReflection,
    ) {
    ...
    pub fn extend_view_class(
        &mut self,
        view_class: dl_sdk_types::ViewClassIdentifier,
        register_fn: impl FnOnce(
            &mut dl_viewer_context::ViewSystemRegistrator<'_>,
        ) -> Result<(), ViewClassRegistryError>,
    ) -> Result<(), ViewClassRegistryError> {
    ...
    fn update_web_history(&self, store_hub: &StoreHub) {
        ...
        dl_log::trace!("Updating navigation bar");
        ...
        if let Some(history) = dl_web::browser::history().ok_or_log_error() {
    ...
    fn go_to_dataset_data(&self, store_id: StoreId, fragment: dl_uri::Fragment) {
        ...
        if let Some(selection) = fragment.selection {
            ...
            let item = if let Some(component) = component {
                Item::from(dl_log_types::ComponentPath::new(entity_path, component))
    ...
    /// Returns a [`dl_chunk_store::LatestAtQuery`] for the active recording's current timeline
    /// position, suitable for querying frame data from [`Self::recording_db`].
    pub fn current_query(&self) -> Option<dl_chunk_store::LatestAtQuery> {
    ...
    fn process_screenshot_result(
        ...
        use dl_viewer_context::ScreenshotInfo;
            ...
            .and_then(|data| data.downcast_ref::<ScreenshotInfo>())
        {
            ...
            match target {
                dl_viewer_context::ScreenshotTarget::CopyToClipboard => {
                    self.egui_ctx.copy_image((*rgba).clone());
                }

                dl_viewer_context::ScreenshotTarget::SaveToPathFromFileDialog => {
                    ...
                    {
                        dl_log::error!("Failed to encode screenshot as PNG: {err}");
                    } else {
                ...
                dl_viewer_context::ScreenshotTarget::SaveToPath(file_path) => {
                    cfg_select! {
                        target_arch = "wasm32" => {
                            dl_log::error!(
                        ...
                        _ => {
                            ...
                            let Some(rgba_image) = image::RgbaImage::from_vec(
                                ...
                                dl_log::error!("Failed to create image from screenshot data");
                            ...
                            let result = match rgb_image.save(&file_path) {
                                Ok(()) => {
                                    ...
                                    if notify {
                                        dl_log::info!("Saved screenshot to {file_path:?}");
                                    } else {
                                        dl_log::debug!("Saved screenshot to {file_path:?}");
                                    }
                                    Ok(())
                                }
                                Err(err) => {
                                    dl_log::error!(?file_path, "Failed to save screenshot: {err}");
...
impl eframe::App for App {
    ...
    fn save(&mut self, storage: &mut dyn eframe::Storage) {
        ...
        if let Some(hub) = &mut self.store_hub {
            ...
            if let Err(err) = hub.save_app_blueprints() {
                dl_log::error!("Saving blueprints failed: {err}");
            }
        } else {
            dl_log::error!("Could not save blueprints: the store hub is not available");
        }
    }
    ...
    fn ui(&mut self, ui: &mut egui::Ui, frame: &mut eframe::Frame) {
        ...
        dl_perf_telemetry::external::tracing_tracy::client::frame_mark();
        ...
        if let Some(capture) = &self.profile_capture {
            if capture.is_done() {
                ...
                {
                    dl_log::error!("Failed to save profile trace: {err}");
                }
            } else {
        ...
        let mem_usage_tree = self
            .dev_panel_open
            .then(|| dl_byte_size::NamedMemUsageTree::new("App", self.capture_mem_usage_tree()));
        ...
        let gpu_resource_stats = {
            dl_tracing::profile_scope!("gpu_resource_stats");
        ...
        let store_bundle_for_streaming = self
            .dev_panel_open
            .then(|| store_hub.store_bundle() as &dl_entity_db::StoreBundle);
        ...
        if let Route::Loading(source) = self.state.navigation.current() {
            if !self.msg_receive_set().contains(source) {
                ...
                if let Some(store_id) = loaded_recording {
                    dl_log::debug!("Stream completed, navigating to loaded recording {store_id:?}");
                ...
                } else if let Some(dl_uri::RedapUri::DatasetData(uri)) = source.redap_uri()
                    && self.connection_registry.error_for_uri(uri).is_some()
                {
                    ...
                    dl_log::debug!("No recording found from loading source, resetting navigation");
...

        {
            ...
            let cmd_env = dl_ui::CommandEnvironment {
            ...
            // Handle keyboard shortcuts, now that we have a live `CommandEnvironment`:
            {
                ...
                if let Some(resolved) = dl_ui::listen_for_kb_shortcuts(ui.ctx(), &cmd_env) {
                    match resolved {
                        dl_ui::ResolvedCommand::Ui(cmd) => self.command_sender.send_ui(cmd),
                        dl_ui::ResolvedCommand::Recording(cmd) => {
                            self.command_sender.send_recording_command(cmd);
                        }
                        dl_ui::ResolvedCommand::RedapServer(cmd) => {
                            self.command_sender.send_redap_server_command(cmd);
                        }
                        dl_ui::ResolvedCommand::Table(cmd) => {
                ...
                let pending_timeline = ui.ctx().data_mut(|data| {
                    ...
                    let kind = data.get_temp::<dl_ui::RecordingCommandKind>(key);
                    data.remove::<dl_ui::RecordingCommandKind>(key);
                    kind
                });
            ...
            if let Some(cmd) = self.cmd_palette.show(ui.ctx(), &mut cmd_palette_provider) {
                match cmd {
                    ...
                    CommandPaletteAction::RecordingCommand(cmd) => {
                        use dl_ui::RecordingCommandSender as _;
                        self.command_sender.send_recording_command(cmd);
                    }
                    CommandPaletteAction::RedapServerCommand(cmd) => {
                        use dl_ui::RedapServerCommandSender as _;
                        self.command_sender.send_redap_server_command(cmd);
                    }
                    ...
                    CommandPaletteAction::SelectRedapEntry {
                        ...
                        self.select_and_navigate_to(&Item::RedapEntry {
                            origin,
                            kind: dl_viewer_context::RedapEntryKind::Entry(entry_id),
                        });
                    }
                    CommandPaletteAction::TableCommand(cmd) => {
                        use dl_ui::TableCommandSender as _;
                        self.command_sender.send_table_command(cmd);
                    }
...
fn save_profile_trace(view: &dl_tracing::reexports::puffin::FrameView) -> anyhow::Result<()> {
    let Some(path) = rfd::FileDialog::new()
        ...
        dl_log::info!("Profile trace capture cancelled by user.");
        return Ok(());
    };
    ...
    dl_log::info!("Saved profile trace to {}", path.display());
    Ok(())
}
...
impl MemUsageTreeCapture for App {
    fn capture_mem_usage_tree(&self) -> MemUsageTree {
        dl_tracing::profile_function!();
        let mut node = dl_byte_size::MemUsageNode::default();
        ...
        node.add(
            "store_subscribers",
            dl_chunk_store::ChunkStore::capture_all_subscribers_mem_usage_tree(),
        );

        let mut globals = dl_byte_size::MemUsageNode::new();
        globals.add(
            "forgiving_parse_cache",
            dl_log_types::forgiving_parse_cache_bytes_used(),
        );
        globals.add("string_interner", dl_string_interner::bytes_used() as u64);
...
#[cfg(not(target_arch = "wasm32"))]
fn blueprint_loader(component_reflection: Arc<ComponentReflectionMap>) -> BlueprintPersistence {
    use dl_entity_db::{EntityDb, StoreBundle};
    use dl_log_types::{ApplicationId, StoreKind};
    ...
    fn load_blueprint_from_disk(
        ...
        dl_log::debug!("Trying to load blueprint for {app_id} from {blueprint_path:?}");
