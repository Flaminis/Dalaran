...
const FALLBACK_PANEL_STATES_ID: &str = "dl_viewer.app_blueprint.fallback_panel_states";
...
pub fn setup_welcome_screen_blueprint(welcome_screen_blueprint: &mut EntityDb) {
    ...
    for (panel_name, value) in [
        ...
        let timepoint = dl_viewer_context::blueprint_timepoint_for_writes(welcome_screen_blueprint);
        ...
        if let Some(blueprint_db) = self.blueprint_db {
            ...
            self.egui_ctx.memory_mut(|m| {
                ...
                match panel_name {
                    ...
                    _ => {
                        dl_log::debug_assert!(false, "Unknown panel name: {panel_name}");
                    }
                }
...
fn load_panel_state(
    path: &EntityPath,
    blueprint_db: &dl_entity_db::EntityDb,
    query: &LatestAtQuery,
) -> Option<PanelState> {
