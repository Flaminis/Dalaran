...
pub struct StartupOptions {
    ...
    pub video_decoder_hw_acceleration: Option<dl_video::DecodeHardwareAcceleration>,
...
impl StartupOptions {
    ...
    pub fn web_viewer_base_url(&self) -> Option<url::Url> {
            ...
            && let Ok(url) = url.parse::<url::Url>()
        {
            ...
            cfg_select! {
                target_arch = "wasm32" => {
                    dl_web::browser::current_page_url()
                        .ok()
                        .and_then(|url| url.parse().ok())
                        .map(|url| dl_viewer_context::open_url::base_url(&url))
                }
                _ => {
                    None
                }
