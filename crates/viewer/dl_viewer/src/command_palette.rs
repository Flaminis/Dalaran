...
use dl_log_types::{ComponentPath, EntityPath, EntryId};
...
pub enum CommandPaletteAction {
    ...
    SelectRedapServer(dl_uri::Origin),
...
/// Feeds the viewer's commands into the [`dl_ui::CommandPalette`].
...
impl CommandPaletteProvider<CommandPaletteAction> for CommandPaletteProviderImpl<'_> {
    ...
    fn cmd_row(
        ...
        if matched.command.is_debug_only() {
            dl_ui::debug_only::append_debug_only_badge(&mut job, ui.style());
        }
