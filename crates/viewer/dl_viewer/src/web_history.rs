...
impl HistoryEntry {
    ...
    pub fn new(url: String) -> Self {
        if url == dl_redap_browser::EXAMPLES_ORIGIN.as_url()
            || url
                == dl_uri::RedapUri::Catalog(dl_uri::CatalogUri::new(
                    dl_redap_browser::EXAMPLES_ORIGIN.clone(),
                ))
                .to_string()
        {
            Self::default()
...
pub fn install_popstate_listener(app: &mut crate::App) -> Result<(), dl_web::Error> {
    ...
    set_stored_history_entry(
        dl_web::browser::history()?
            .current_entry()
            .map_err(dl_web::Error::from)?,
    );

    dl_web::browser::window()?
...
impl Drop for PopstateListener {
    fn drop(&mut self) {
        let Some(window) = dl_web::browser::window().ok_or_log_error() else {
            return;
        };
...
fn handle_popstate(
    ...
    dl_log::debug!("popstate: prev={prev_state:?} new={new_state:?}");

    if prev_state == new_state {
        dl_log::debug!("popstate: no change");

        return;
    }

    if new_state.is_none() || new_state.as_ref().is_some_and(|v| v.url.is_empty()) {
        dl_log::debug!("popstate: go to welcome screen");
        dl_redap_browser::switch_to_welcome_screen(command_sender);
    ...
    match entry.url.parse::<open_url::ViewerOpenUrl>() {
        ...
        Err(err) => {
            dl_log::warn!("Failed to open URL {:?}: {err}", entry.url);
        }
    }
    dl_log::debug!("popstate: add receiver {}", entry.url);

    set_stored_history_entry(Some(entry));
}

pub fn go_back() -> Option<()> {
    let history = dl_web::browser::history().ok_or_log_error()?;
