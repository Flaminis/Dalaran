...
pub enum ServerModalMode {
    ...
    ///
    /// You should ensure that the [`dl_uri::Origin`] exists. (Otherwise, this leads to bad UX,
    ...
    pub fn open(
        ...
        *self = match mode {
            ...
            ServerModalMode::Edit(edit) => {
                let dl_uri::Origin { scheme, host, port } = edit.origin.clone();

                let credentials = connection_registry.credentials(&edit.origin);
                let auth = match credentials {
                    Some(dl_redap_client::Credentials::Token(token)) => {
    ...
    pub fn ui(&mut self, app_ctx: &AppContext<'_>, ctx: &Context<'_>, ui: &egui::Ui) {
        ...
        self.modal.ui(
            ...
            |ui| {
                ...
                let origin = host.map(|host| dl_uri::Origin {
                    scheme: self.scheme,
                    host,
                    port: self.port,
                });
        ...
        if was_open && !self.modal.is_open() {
            dl_log::debug!("modal closed; reset login flow");
            self.auth.reset_login_flow();
        }
    }
}
