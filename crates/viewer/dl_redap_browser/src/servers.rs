...
use dl_sorbet::ColumnDescriptorRef;
use dl_ui::alert::Alert;
use dl_ui::{UiExt as _, icons};
use dl_uri::DATASET_HIERARCHY_SEPARATOR;
...
impl Server {
    ...
    fn server_ui(
        ...
        ctx: &Context<'_>,
        ui: &mut egui::Ui,
        inline_login_flow: &mut Option<(dl_uri::Origin, Box<LoginFlow>)>,
        ...
        dl_dataframe_ui::DataFusionTableWidget::new(self.tables_session_ctx.clone(), "__entries")
            ...
            .column_blueprint(|desc| {
                ...
                {
                    blueprint = blueprint.variant_ui(dl_component_ui::REDAP_ENTRY_KIND_VARIANT);
                }
                ...
                if desc.display_name().as_str() == ENTRY_LINK_COLUMN_NAME {
                    blueprint = blueprint.variant_ui(dl_component_ui::REDAP_URI_BUTTON_VARIANT);
                }
    ...
    fn table_entry_ui(
        ...
        .url(dl_uri::EntryUri::new(table.origin.clone(), table.id()).to_string())
        .show(app_ctx, &self.runtime, ui, view_states);
    }
}
...
fn error_ui(
    ...
    if let Some(conn_err) = err.as_client_credentials_error() {
                ...
                ui.vertical(|ui| {
                    ...
                    if has_active_login_flow {
                        ui.horizontal_centered(|ui| {
                            ...
                            if ui
                                .add(
                                    dl_ui::ReButton::new(("Cancel", &icons::CLOSE))
                                        .small()
                                        .primary(),
                                )
                                .clicked()
                        ...
                        ui.horizontal(|ui| {
                            if let Some(auth) = app_ctx.auth_context {
                                ...
                                if ui
                                    .add(
                                        dl_ui::ReButton::new(format!("Continue as {}", auth.email))
                                            .primary()
                                            .small(),
                                    )
                                    .clicked()
                                ...
                                if ui
                                    .add(dl_ui::ReButton::new("Log in").primary().small())
                                    .clicked()
                                {
                                    match LoginFlow::open_and_start(
                                        ...
                                        Err(err) => {
                                            dl_log::error!("Failed to start login: {err}");
                                        }
                                    }
                                }
                            }
                            if ui
                                .add(dl_ui::ReButton::new("Edit connection").small())
                                .clicked()
                            {
                                send_crossbeam(
                                    ctx.command_sender,
        ...
        &err.kind,
        dl_redap_client::ApiErrorKind::InvalidServer | dl_redap_client::ApiErrorKind::Connection
    ) {
...
fn warning_with_edit_button(
    ...
    origin: &dl_uri::Origin,
    message: &str,
    auth_context: Option<&dl_viewer_context::AuthContext>,
) {
...
pub struct RedapServers {
    servers: BTreeMap<dl_uri::Origin, Server>,
    ...
    pending_servers: Vec<dl_uri::Origin>,
    ...
    inline_login_flow: Option<(dl_uri::Origin, Box<LoginFlow>)>,
}
...
impl<'de> serde::Deserialize<'de> for RedapServers {
    ...
    {
        let origins = Vec::<dl_uri::Origin>::deserialize(deserializer)?;
...
pub enum Command {
    ...
    /// are not ran in order with [`dl_viewer_context::SystemCommand`]s).
    AddServer {
        origin: dl_uri::Origin,
        credentials: Option<dl_redap_client::Credentials>,
        on_add: Option<Box<dyn FnOnce() + Send>>,
    },

    RefreshCollection(dl_uri::Origin),

    /// Use the stored account credentials for a server and refresh.
    UseStoredCredentials(dl_uri::Origin),
}
...
impl RedapServers {
    ...
    pub fn add_server(&self, origin: dl_uri::Origin) {
        send_crossbeam(
            &self.command_sender,
            Command::AddServer {
                origin,
    ...
    pub fn refresh_entry(
        ...
        {
            dl_dataframe_ui::DataFusionTableWidget::refresh(
                &server.runtime,
                egui_ctx.clone(),
    ...
    pub fn build_url_lookup(&self) -> dl_viewer_context::UrlNameLookup {
        let mut lookup = dl_viewer_context::UrlNameLookup::default();
        ...
        for server in self.servers.values() {
            for entry in server.entries().iter_loaded() {
                lookup.insert(
                    ...
                    dl_viewer_context::ResolvedEntry {
    ...
    pub fn is_authenticated(&self, origin: &dl_uri::Origin) -> bool {
        self.servers
            .get(origin)
    ...
    pub fn logout(&mut self) -> Vec<dl_uri::Origin> {
    ...
    pub fn send_command(&self, command: Command) {
        ...
        if let Err(err) = result {
            dl_log::warn_once!("Failed to send command: {err}");
        }
    }
}
...
async fn watch_events_loop(
    ...
    let mut backoff = dl_backoff::BackoffGenerator::new(
    ...
    loop {
        ...
        {
            Ok(()) => {}
            Err(err) if err.kind == dl_redap_client::ApiErrorKind::Unimplemented => {
                // Permanent condition (e.g. an older server), so don't keep reconnecting.
                dl_log::debug!("Server does not support event listening\nServer: {origin}");
                return;
            }
            Err(err) => {
                dl_log::debug!("Event stream failed, will reconnect: {err}\nServer: {origin}");
            }
        }
...
async fn run_event_listener(
    ...
    command_sender: &crossbeam::channel::Sender<Command>,
    egui_ctx: &egui::Context,
    backoff: &mut dl_backoff::BackoffGenerator,
) -> dl_redap_client::ApiResult<()> {
