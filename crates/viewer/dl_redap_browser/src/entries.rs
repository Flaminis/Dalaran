...
use dl_log_types::{EntryId, EntryName, TableId};
use dl_protos::TypeConversionError;
use dl_protos::cloud::v1alpha1::ext::{DatasetEntry, EntryDetails, ProviderDetails, TableEntry};
use dl_protos::cloud::v1alpha1::{EntryFilter, EntryKind};
use dl_protos::external::prost;
use dl_redap_client::{
...
impl Entry {
    ...
    pub fn link_kind(&self) -> dl_viewer_context::LinkKind {
        match &self.details.kind {
            EntryKind::Table | EntryKind::TableView => dl_viewer_context::LinkKind::Table,
            EntryKind::Dataset
            | EntryKind::DatasetView
            | EntryKind::BlueprintDataset
            | EntryKind::AssetDataset
            | EntryKind::Unspecified => dl_viewer_context::LinkKind::Dataset,
        }
    }
...
fn start_streaming_segment_table_blueprint(
    ...
    #[expect(deprecated)]
    let application_id = dl_log_types::ApplicationId::from_entry_id(dataset_id);
    let blueprint_store_id =
        dl_log_types::StoreId::random(dl_log_types::StoreKind::Blueprint, application_id);

    let (tx, rx) = dl_redap_client::table_blueprint_log_channel(
        origin.clone(),
        blueprint_dataset,
        &blueprint_segment,
    ...
    runtime.spawn_future(async move {
        if let Err(err) = dl_redap_client::stream_table_blueprint_segment_from_server(
            client,
            tx,
            blueprint_store_id,
            blueprint_dataset,
        ...
        .await
        {
            dl_log::warn!("Failed to stream segment table blueprint: {err}");
        }
    });
}
...
fn start_registered_table_blueprint_stream(
    ...
    #[expect(deprecated)]
    let application_id = dl_log_types::ApplicationId::from_entry_id(table_id);
    let blueprint_store_id =
        dl_log_types::StoreId::random(dl_log_types::StoreKind::Blueprint, application_id);
    ...
    runtime.spawn_future(async move {
        ...
        .await
        {
            dl_log::warn!("Failed to stream table blueprint: {err}");
        }
    });
}
