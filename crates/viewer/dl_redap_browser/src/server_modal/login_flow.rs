        ...
        cfg_select! {
            target_arch = "wasm32" => {
                if !self.started {
                    ...
                    if ActionButton::new(&dl_ui::icons::EXTERNAL_LINK, "Log in", "Log in")
                        .variant(dl_ui::Variant::Outlined)
                        .show(ui, &mut false)
                        .clicked()
                    {
...
pub struct ActionButton<'a> {
    ...
    action_text: &'a str,
    feedback_text: &'a str,
    variant: dl_ui::Variant,
}

impl<'a> ActionButton<'a> {
    pub fn new(icon: &'a dl_ui::Icon, action_text: &'a str, feedback_text: &'a str) -> Self {
        Self {
            icon,
            action_text,
            feedback_text,
            variant: dl_ui::Variant::default(),
        }
    }

    pub fn variant(mut self, style: dl_ui::Variant) -> Self {
        self.variant = style;
        self
    }
