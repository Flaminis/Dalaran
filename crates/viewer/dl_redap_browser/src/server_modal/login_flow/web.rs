...
use dl_auth::oauth::api::{AuthenticateWithCode, Pkce, authorization_url, send_async};
...
impl Drop for State {
    fn drop(&mut self) {
        dl_log::debug!("dropping auth state");
...
impl State {
    pub fn start(&mut self) -> Result<(), String> {
        ...
        let Some(child_window) = parent_window
            ...
            .map_err(|err| dl_web::Error::from(err).to_string())?
        else {
...
async fn try_handle_storage_event(
    ...
    if payload.type_ != "auth" {
        dl_log::error!("storage event payload.type != auth");
        return;
    }
    ...
    let credentials = match dl_auth::oauth::Credentials::from_auth_response(res.into()) {
        Ok(v) => v,
        Err(err) => {
            bail!(err);
        }
