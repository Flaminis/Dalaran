...
use dl_sdk_types::ArrowString;
use dl_sdk_types::archetypes::{StateChange, StateConfiguration};
use dl_sdk_types::components::Text;
use dl_view::{ComponentCastRule, collect_recursive_clears};
...
pub fn current_state_value_kind(
    ctx: &ViewContext<'_>,
    data_result: &dl_viewer_context::DataResult,
    instruction: &dl_viewer_context::VisualizerInstruction,
    ...
    let result = dl_view::latest_at_with_blueprint_resolved_data_polymorphic(
        ctx,
        None,
        &ctx.current_query(),
        data_result,
...
fn color_for_value(value: &str) -> egui::Color32 {
    let hash = dl_log_types::hash::Hash64::hash(value).hash64();
...
fn resolve_state_config(
    results: &dl_view::VisualizerInstructionQueryResults<'_>,
...
impl VisualizerSystem for StateVisualizer {
    fn selection_ui(
        ...
        data_result: &dl_viewer_context::DataResult,
        instruction: &dl_viewer_context::VisualizerInstruction,
        type_report: Option<&dl_viewer_context::VisualizerTypeReport>,
    ) -> bool {
        ...
        let selectors = dl_selection_panel::SourceSelectorContext::new(
            ctx,
            data_result,
            instruction,
            self,
    ...
    fn execute(
        ...
        let query = dl_chunk_store::RangeQuery::new(view_query.timeline, visible_range)
        ...
        let state_component = StateChange::descriptor_state().component;
        let cast_rules: IntMap<dl_sdk_types::ComponentIdentifier, ComponentCastRule> =
            std::iter::once((state_component, state_cast_rule as ComponentCastRule)).collect();
            ...
            view_query.iter_visualizer_instruction_for(Self::identifier())
        {
            ...
            // In-window data.
            let range_results = dl_view::BlueprintResolvedResults::from((
                query.clone(),
                dl_view::range_with_blueprint_resolved_data_polymorphic(
                    ctx,
                    None,
                    &query,
                    data_result,
            ...
            let range_results = dl_view::VisualizerInstructionQueryResults::new(
                instruction,
                &range_results,
                &output,
            );
            ...
            let latest_query =
                dl_chunk_store::LatestAtQuery::new(query.timeline, window_start_query_time);
            let bootstrap_results = dl_view::BlueprintResolvedResults::from((
                latest_query.clone(),
                dl_view::latest_at_with_blueprint_resolved_data_polymorphic(
                    ctx,
                    None,
                    &latest_query,
                    data_result,
            ...
            let bootstrap_results = dl_view::VisualizerInstructionQueryResults::new(
                instruction,
                &bootstrap_results,
                &output,
            );
            ...
            let element_type = element_types.into_iter().next().or_else(|| {
                ...
                let latest_query =
                    dl_chunk_store::LatestAtQuery::new(view_query.timeline, TimeInt::MAX);
                let probe = dl_view::BlueprintResolvedResults::from((
                    latest_query.clone(),
                    dl_view::latest_at_with_blueprint_resolved_data_polymorphic(
                        ctx,
                        None,
                ...
                let probe =
                    dl_view::VisualizerInstructionQueryResults::new(instruction, &probe, &output);
            ...
            let lane_label = {
                ...
                if let Some(dl_viewer_context::VisualizerComponentSource::SourceComponent {
                    source_component,
                    ..
...
fn collect_state_rows(
    values: &dl_view::HybridResultsChunkIter<'_>,
...
fn state_chunk_element_types(
    all_values: &dl_view::HybridResultsChunkIter<'_>,
...
fn calculate_instance_count(
    ...
    instruction: &dl_viewer_context::VisualizerInstruction,
    state_component: dl_sdk_types::ComponentIdentifier,
) -> usize {
    use dl_chunk_store::external::arrow::array::Array as _;
    ...
    let source_component = match instruction.component_mappings.get(&state_component) {
        Some(dl_viewer_context::VisualizerComponentSource::SourceComponent {
            source_component,
            ..
        }) => *source_component,
        _ => state_component,
    ...
    let full_range_query = dl_chunk_store::RangeQuery::new(timeline, AbsoluteTimeRange::EVERYTHING);
    let engine = ctx.recording_engine();
    let results = engine.store().range_relevant_chunks(
        dl_chunk_store::ChunkTrackingMode::Ignore,
        &full_range_query,
        entity_path,
        source_component,
    );
