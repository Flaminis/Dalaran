...
use dl_sdk_types::archetypes::StateConfiguration;
use dl_sdk_types::components::{Color, Text, Visible};
use dl_sdk_types::datatypes::{Bool, Rgba32};
...
use dl_viewer_context::{DataResultInteractionAddress, Item, MaybeMutRef};
...
fn render_freeform_mapping_rows(
    ...
    for (i, mapping) in mappings.iter_mut().enumerate() {
        ...
        ui.push_id(row_id, |ui| {
            let (row_changes, remove_clicked) = egui::Sides::new().shrink_left().show(
                ...
                |ui| {
                    ui.small_icon_button(&dl_ui::icons::REMOVE, "Remove mapping")
                        .clicked()
                },
            );
    ...
    if ui
        .small_icon_button(&dl_ui::icons::ADD, "Add state mapping")
        .clicked()
    {
