...
use dl_sdk_types::{ArchetypeName, ComponentIdentifier, DynamicArchetype, Visualizer};
...
use dl_view_state_timeline::{
    StateLanesData, StateTimelineView, StateTimelineViewState, StateValueKind, StateVisualizer,
};
use dl_viewer_context::{IdentifiedViewSystem as _, ViewClass as _, ViewId};
use dl_viewport::execute_systems_for_view;
...
/// staying [`dl_viewer_context::ActiveTimeline::Pending`]) once some data exists on it.
...
where
    F: Into<dl_log_types::external::arrow::array::ArrayRef>,
{
...
fn test_dynamic_archetype_multiple_same_type() {
    ...
    let phase_label = |p: &dl_view_state_timeline::StateLanePhase| {
        p.content
            .as_ref()
            .map_or_else(String::new, |s| s.label.clone())
    };
...
fn test_cast_nested_struct_u8_field_via_selector() {
    use dl_log_types::external::arrow::array::{ArrayRef, StructArray, UInt8Array};
...
/// `debug_panic!` in `dl_chunk::iter`. The visualizer must detect this and skip the lane
/// rather than panic.
#[test]
