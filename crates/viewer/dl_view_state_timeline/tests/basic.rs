...
use dl_log_types::{TimePoint, Timeline};
...
use dl_view_state_timeline::StateTimelineView;
use dl_viewer_context::{GLOBAL_VIEW_ID, TimeControlCommand, ViewClass as _, ViewId};
...
fn log_multi_state(
    ...
    let state_change =
        dl_sdk_types::archetypes::StateChange::new().with_state_opt(states.iter().copied());
    ...
    for (tick, state) in [(0, "On"), (25, "Off")] {
        test_context.log_entity("state/power", |builder| {
            builder.with_archetype(
                RowId::new(),
                TimePoint::from([(timeline, tick)]),
                &dl_sdk_types::archetypes::StateChange::single(state),
            )
        });
    }
...
fn test_state_timeline_multi_instance_bootstrap() {
    ...
    let view_id = test_context.setup_viewport_blueprint(|ctx, blueprint_ctx| {
        ...
        global_time_axis.save_blueprint_component(
            ...
            &blueprint::components::TimeRange(dl_sdk_types::datatypes::TimeRange {
                start: dl_sdk_types::datatypes::TimeRangeBoundary::Absolute(
                    dl_sdk_types::datatypes::TimeInt(12),
                ),
                end: dl_sdk_types::datatypes::TimeRangeBoundary::Absolute(
                    dl_sdk_types::datatypes::TimeInt(30),
                ),
            }),
        );
...
fn test_state_timeline_basic() {
    ...
    test_context.send_time_commands(
        ...
        [TimeControlCommand::SetTime(
            dl_log_types::TimeInt::new_temporal(20).into(),
        )],
    );
...
fn test_state_timeline_time_cursor() {
    ...
    test_context.send_time_commands(
        store_id,
        [TimeControlCommand::SetTime(
            dl_log_types::TimeInt::new_temporal(30).into(),
        )],
    );
    test_context.handle_system_commands(&egui::Context::default());
...
fn test_state_timeline_null_is_reset() {
    ...
    test_context.log_entity("state/mode", |builder| {
        builder.with_archetype(
            RowId::new(),
            timepoint_0,
            &dl_sdk_types::archetypes::StateChange::single("Idle"),
        )
    });

    let timepoint_20 = TimePoint::from([(timeline, 20)]);
    let null_state = dl_sdk_types::archetypes::StateChange::new().with_state_opt([None::<&str>]);
    test_context.log_entity("state/mode", |builder| {
    ...
    test_context.log_entity("state/mode", |builder| {
        builder.with_archetype(
            RowId::new(),
            timepoint_40,
            &dl_sdk_types::archetypes::StateChange::single("Active"),
        )
    });
...
fn test_state_timeline_empty_and_clear() {
    ...
    test_context.log_entity("cleared", |builder| {
        builder.with_archetype(
            ...
            &dl_sdk_types::archetypes::StateChange::single("Running"),
        )
    });
    test_context.log_entity("cleared", |builder| {
        builder.with_archetype(
            ...
            &dl_sdk_types::archetypes::Clear::new(false),
        )
    });
    test_context.log_entity("cleared", |builder| {
        builder.with_archetype(
...
fn test_state_timeline_timeline_switch() {
    ...
    test_context.send_time_commands(
        store_id,
        [TimeControlCommand::SetTime(
            dl_log_types::TimeInt::new_temporal(base_ns + 20 * step_ns).into(),
        )],
    );
    test_context.handle_system_commands(&egui_ctx);
    snapshot_results.add(test_context.run_view_ui_and_save_snapshot(
...
fn test_state_configuration() {
    ...
    test_context.log_entity("state/robot_mode", |builder| {
        builder.with_archetype(
            RowId::new(),
            TimePoint::STATIC,
            &dl_sdk_types::archetypes::StateConfiguration::new()
...
fn test_state_timeline_edge_cases() {
    ...
    test_context.log_entity(entity, |builder| {
        builder.with_archetype_auto_row(
            ...
            &dl_sdk_types::archetypes::StateChange::single("hi!"),
        )
    });
    ...
    test_context.log_entity(entity, |builder| {
        builder.with_archetype_auto_row(
            ...
            &dl_sdk_types::archetypes::StateChange::clear_fields(),
        )
    });
    test_context.log_entity(entity, |builder| {
        builder.with_archetype_auto_row(
            TimePoint::from([(timeline, 3)]),
            &dl_sdk_types::archetypes::Scalars::single(2.0),
        )
    });
    ...
    test_context.log_entity(entity, |builder| {
        builder.with_archetype_auto_row(
            ...
            &dl_sdk_types::archetypes::StateChange::single(""),
        )
    });
    test_context.log_entity(entity, |builder| {
        builder.with_archetype_auto_row(
            TimePoint::from([(timeline, 5)]),
            &dl_sdk_types::archetypes::Scalars::new([1.0, 2.0, 3.0]),
        )
    });
    ...
    test_context.log_entity(entity, |builder| {
        builder.with_archetype_auto_row(
            ...
            &dl_sdk_types::archetypes::StateChange::single("bye!"),
        )
    });
    ...
    test_context.log_entity(entity, |builder| {
        builder.with_archetype_auto_row(
            ...
            &dl_sdk_types::archetypes::Scalars::single(4.0),
        )
    });
    ...
    test_context.log_entity(entity, |builder| {
        builder.with_archetype_auto_row(
            ...
            &dl_sdk_types::archetypes::StateChange::single("end"),
        )
    });
...
fn test_state_timeline_link_to_global() {
    ...
    assert!(
        matches!(
            global_range.start,
            dl_sdk_types::datatypes::TimeRangeBoundary::Absolute(_)
        ) && matches!(
            global_range.end,
            dl_sdk_types::datatypes::TimeRangeBoundary::Absolute(_)
        ),
