...
mod design_token_access {
    ...
    pub fn hot_reload_design_tokens() {
        let design_tokens = match DesignTokensPerTheme::load() {
            ...
            Err(err) => {
                dl_log::error!("Failed to reload design tokens: {err:#}");
                return;
            }
        };
        ...
        if let Some(current) = CURRENT_TOKENS.get() {
            *current.write() = Box::leak(Box::new(design_tokens));
            ...
            dl_log::warn!("Failed to update design tokens: CURRENT_TOKENS is not initialized.");
        }
    }
    ...
    fn setup_file_watcher() {
        ...
        WATCHER_INIT.get_or_init(|| {
            ...
            thread::Builder::new()
                .name("dl_ui design token hot reloader".to_owned())
                .spawn(|| {
                    ...
                    let mut watcher: RecommendedWatcher = match Watcher::new(
                        ...
                        Ok(watcher) => watcher,
                        Err(err) => {
                            dl_log::error!("Failed to create file watcher: {err}");
                            return;
                        }
                    };
                    ...
                    match watcher.watch(&path, RecursiveMode::Recursive) {
                        Ok(()) => {
                            dl_log::debug!("Watching for file changes in {}", path.display());
                            while rx.recv().is_ok() {
                                run_all_hot_reloading();
                            }
                        }
                        Err(err) => {
                            dl_log::error!("Failed to watch design tokens directory: {err}");
                        }
                    }
                })
                .expect("Failed to spawn watcher thread");

            dl_log::debug!("Hot-reloading of design tokens enabled.");
        });
    }
