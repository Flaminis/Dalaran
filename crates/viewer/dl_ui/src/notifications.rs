...
//!
//! ## Special cased text
//! - If a notifications text contains [`dl_error::DETAILS_SEPARATOR`] the section after that
...
impl From<dl_log::Level> for NotificationLevel {
    fn from(value: dl_log::Level) -> Self {
        match value {
            dl_log::Level::TRACE | dl_log::Level::DEBUG | dl_log::Level::INFO => Self::Info,
            dl_log::Level::WARN => Self::Warning,
            dl_log::Level::ERROR => Self::Error,
        }
    }
}
...
fn is_relevant(target: &str, level: dl_log::Level) -> bool {
    ...
    matches!(
        level,
        dl_log::Level::WARN | dl_log::Level::ERROR | dl_log::Level::INFO
    )
}
...
pub struct Notification {
    ...
    fields: Vec<(&'static str, dl_log::FieldValue)>,
...
impl Notification {
    ...
    fn copy_text(&self) -> String {
        ...
        if let Some(details) = &self.details {
            text = dl_error::format_with_details(text, details.as_str());
        }
        text
    }
    ...
    ///
    /// Field values containing [`dl_error::DETAILS_SEPARATOR`] are split: the part after the
    ...
    /// `error` field.
    pub fn with_fields(mut self, fields: Vec<(&'static str, dl_log::FieldValue)>) -> Self {
        self.fields = fields
            .into_iter()
            .map(|(key, value)| {
...
/// Split a field value on [`dl_error::DETAILS_SEPARATOR`], returning the value with the details
/// removed, plus the details themselves (if any).
fn split_field_details(value: dl_log::FieldValue) -> (dl_log::FieldValue, Option<String>) {
    use dl_log::FieldValue;

    fn split(text: &str) -> Option<(String, String)> {
        let (summary, details) = dl_error::split_details(text);
...
impl NotificationUi {
    ...
    /// - If a notifications text contains [`dl_error::DETAILS_SEPARATOR`] the section after that
    ///   will be displayed inside a collapsible details header.
    pub fn add_log(&mut self, log_msg: dl_log::LogMsg) {
        let dl_log::LogMsg {
            level,
            target,
            message,
        ...
        if is_relevant(&target, level) {
            let (summary, details) = dl_error::split_details(&message);
