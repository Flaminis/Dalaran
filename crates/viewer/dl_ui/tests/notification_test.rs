...
fn test_notification_with_fields() {
    ...
    let log_rx = dl_log::add_log_msg_receiver(dl_log::LevelFilter::INFO);
    ...
    let mut harness =
        dl_ui::testing::new_harness(dl_ui::testing::TestOptions::Gui, Vec2::new(400.0, 200.0))
            .build_ui(move |ui| {
...
/// A field value containing [`dl_error::DETAILS_SEPARATOR`] should have the part after the
...
fn test_notification_with_details_in_field() {
    ...
    let error_text = dl_error::format_with_details(
    ...
    dl_log::error!(target: "dl_ui", error = %error_text);
