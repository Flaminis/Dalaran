...
fn command_palette_enabled_vs_disabled() {
    let mut harness =
        dl_ui::testing::new_harness(dl_ui::testing::TestOptions::Gui, Vec2::new(320.0, 100.0))
            .build_ui(|ui| {
...
fn command_palette_wraps_long_paths() {
    let mut harness =
        dl_ui::testing::new_harness(dl_ui::testing::TestOptions::Gui, Vec2::new(320.0, 160.0))
            .build_ui(|ui| {
                dl_ui::apply_style_and_install_loaders(ui.ctx());
...
impl dl_ui::CommandPaletteProvider<String> for ManyCommandsProvider {
    fn all_matching(&mut self, _query: &FuzzyQuery) -> Vec<dl_ui::MatchGroup<String>> {
        let group_a = (0..8)
            .map(|i| dl_ui::MatchedCmd {
                command: format!("First group command {i}"),
                fuzzy_match: dl_ui::FuzzyMatch::lowest(format!("First group command {i}")),
                enabled: true,
            })
            .collect();
        ...
        let group_b = (0..2)
            .map(|i| dl_ui::MatchedCmd {
                ...
                fuzzy_match: dl_ui::FuzzyMatch::lowest(format!("Second group command {i}")),
                enabled: true,
            })
            .collect();
    ...
    fn cmd_row(&self, ui: &egui::Ui, cmd: &dl_ui::MatchedCmd<String>, selected: bool) -> CmdRow {
...
fn command_palette_expand_truncated_group() {
    let mut palette = dl_ui::CommandPalette::default();
    palette.toggle(); // Make it visible.

    let mut harness =
        dl_ui::testing::new_harness(dl_ui::testing::TestOptions::Gui, Vec2::new(500.0, 320.0))
            .build_ui(move |ui| {
                dl_ui::apply_style_and_install_loaders(ui.ctx());
