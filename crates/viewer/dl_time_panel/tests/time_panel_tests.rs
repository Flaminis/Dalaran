...
use dl_chunk::Chunk;
use dl_chunk_store::{ChunkStoreConfig, LatestAtQuery, RowId};
use dl_entity_db::InstancePath;
use dl_log_types::example_components::{MyPoint, MyPoints};
use dl_log_types::{EntityPath, TimeInt, TimePoint, TimeReal, TimeType, Timeline, build_frame_nr};
...
use dl_time_panel::TimePanel;
use dl_viewer_context::{CollapseScope, TimeControlCommand, TimeView, blueprint_timeline};
...
pub fn time_panel_two_sections() {
    ...
    let mut test_context = TestContext::new_with_store_info_and_config(
        dl_log_types::StoreInfo::testing(),
        ...
        .build_ui(|ui| {
            test_context.run(&ui.ctx().clone(), |viewer_ctx| {
                ...
                if options.expand_all {
                    dl_context_menu::collapse_expand::collapse_expand_instance_path(
                        viewer_ctx,
