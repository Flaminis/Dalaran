...
use dl_log_types::{AbsoluteTimeRange, StoreId, StoreKind, Timeline};
use dl_time_panel::__bench::{
...
fn run(b: &mut Bencher<'_, WallTime>, config: DensityGraphBuilderConfig, entry: ChunkEntry) {
    egui::__run_test_ui(|ui| {
        egui::CentralPanel::default().show(ui, |ui| {
            ...
            let entity_path = dl_log_types::EntityPath::parse_strict("/data").unwrap();
            let timeline = dl_log_types::Timeline::log_time();
    ...
    for _ in 0..num_chunks {
        ...
        let components = (0..num_rows_per_chunk).map(|i| {
            ...
            dl_sdk_types::archetypes::Transform3D::from_rotation(
                dl_sdk_types::datatypes::RotationAxisAngle {
                    ...
                    angle: dl_sdk_types::datatypes::Angle::from_degrees(angle_deg),
                },
            )
        });

        let mut chunk = dl_chunk_store::Chunk::builder(entity_path.clone());

        // points
        chunk = chunk.with_archetype(
            dl_chunk_store::RowId::new(),
            dl_log_types::TimePoint::default().with(
                timeline,
                dl_log_types::TimeInt::from_millis(dl_log_types::NonMinI64::ZERO),
            ),
            &dl_sdk_types::archetypes::Points3D::new([(10.0, 10.0, 10.0)]),
        );
        ...
        for (time, component) in std::iter::zip(&log_times, components) {
            chunk = chunk.with_archetype(
                ...
                dl_log_types::TimePoint::default().with(
                    timeline,
                    dl_log_types::TimeInt::from_millis((*time).try_into().unwrap_or_default()),
                ),
                &component,
            );
        }
