...
use dl_sdk_types::blueprint::components::LoopMode;
use dl_ui::{HasDesignTokens as _, UiExt as _, list_item};
...
fn selection_context_menu(
    ...
    if let Some(shortcut) =
        dl_ui::RecordingCommandKind::SaveTimeSelection.formatted_kb_shortcut(ui.ctx())
    {
        ...
        .clicked()
    {
        ...
        ctx.command_sender()
            .send_recording_command(dl_ui::RecordingCommand {
                recording_id: ctx.store_context.recording.store_id().clone(),
                kind: dl_ui::RecordingCommandKind::SaveTimeSelection,
            });
    }
...
fn format_duration(time_typ: TimeType, duration: TimeReal) -> String {
    match time_typ {
        ...
        TimeType::Sequence => dl_format::format_int(duration.round().as_i64()),
    }
}
