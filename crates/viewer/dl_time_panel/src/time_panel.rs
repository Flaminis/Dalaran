...
use dl_time_ruler::{self, TimeRangesUi};
...
impl From<TimePanelSource> for dl_log_types::StoreKind {
    fn from(source: TimePanelSource) -> Self {
        match source {
...
impl TimePanel {
    ...
    fn expanded_ui(
        ...
        let time_fg_x_range = {
            ...
            dl_log::debug_assert!(time_x_left < right);
    ...
    fn show_entity(
        ...
        let list_item::ShowCollapsingResponse {
            ...
            .show_hierarchical_with_children(
                ...
                .with_icon(dl_data_ui::item_ui::instance_path_icon(
                    store_ctx,
        ...
        let response = response.on_hover_ui(|ui| {
            ...
            dl_data_ui::item_ui::entity_hover_card_ui(ui, store_ctx, entity_path, include_subtree);
        });
                    ...
                    .show_hierarchical(
                        ...
                        list_item::LabelContent::new(component_descr.archetype_field_name())
                            .with_icon(if is_static {
                                &dl_ui::icons::COMPONENT_STATIC
                            } else {
                                &dl_ui::icons::COMPONENT_TEMPORAL
                            })
                            .truncate(false),
                    );
                    ...
                    if total_num_messages == 0 {
                        ...
                        list_item::list_item_scope(ui, "hover tooltip", |ui| {
                            ...
                            let num_messages = if num_messages == 1 {
                                ...
                                format!("{} times", dl_format::format_uint(num_messages))
                            };
    ...
    fn data_density_graph_ui(
        ...
        {
            dl_tracing::profile_scope!("protect hovered chunks");
        ...
        if ui.max_rect().width() < 600.0 {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ...
                if store_ctx.app_options.show_metrics && freshness > 0.0 {
                    ...
                    ui.label(
                        RichText::new(format!("{}/s", dl_format::format_bytes(rate)))
    ...
    fn collapsed_time_marker_and_time(
        ...
        if time_range.min() == time_range.max() {
            ...
            let space_needed_for_current_time = match time_ctrl.time_type() {
                Some(dl_chunk_store::TimeType::Sequence) | None => 100.0,
                Some(dl_chunk_store::TimeType::DurationNs) => 200.0,
                Some(dl_chunk_store::TimeType::TimestampNs) => 220.0,
            };
            ...
            && let Some(time_type) = time_ctrl.time_type()
        {
            ...
            if response.lost_focus() {
                    ...
                    time_type.parse_time(&time_str, app_options.timestamp_format)
                {
                    ...
                    dl_log::warn!("Failed to parse {time_str:?}");
                }
                self.time_edit_string = None;
            }
            let response = response.on_hover_text(format!(
                "Timestamp: {}",
                dl_format::format_int(time_int.as_i64())
            ));
...
fn archetype_label_ui(
    ui: &mut Ui,
    archetype: Option<dl_sdk_types::ArchetypeName>,
) -> egui::Response {
    ui.list_item()
        .with_y_offset(1.0)
        .with_height(20.0)
...
fn view_everything(x_range: &Rangef, timeline_axis: &TimelineAxis) -> TimeView {
    let gap_width = dl_time_ruler::gap_width(x_range, &timeline_axis.ranges) as f32;
...
fn timeline_properties_context_menu(
    ...
    if ui.button("Copy timestamp").clicked() {
        ...
        dl_log::info!("Copied hovered timestamp: {}", time);
        ui.copy_text(time);
    }
}
