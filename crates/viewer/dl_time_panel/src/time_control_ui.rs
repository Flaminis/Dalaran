...
"
                    ...
                    ui.re_hyperlink(
                        "Full documentation",
                        "https://dalaran.dev/docs/concepts/logging-and-ingestion/timelines",
                ...
                if ui
                    .add(
                        ComboItem::new(timeline.name().as_str())
                            .value(format!("{} rows", dl_format::format_uint(num_rows)))
        ...
        egui::Popup::menu(&response)
            ...
            .show(|ui| {
                if ui.button("Copy timeline name").clicked() {
                    ...
                    dl_log::info!("Copied timeline: {}", timeline);
    ...
    fn playhead_nav_ui(&self, ui: &mut egui::Ui, time_commands: &mut Vec<TimeControlCommand>) {
        ...
        let button = ui
            .scope(|ui| {
                ...
                ui.add(
                    dl_ui::ReButton::new((
                        dl_ui::icons::PLAYHEAD_NAV,
                        dl_ui::icons::DROPDOWN_ARROW,
                    ))
                    .secondary()
                    .size(TIME_CONTROL_ROW_SIZE)
    ...
    fn loop_button_ui(
        ...
        let button = ReButton::icon(dl_ui::icons::LOOP)
            .size(TIME_CONTROL_ROW_SIZE)
            .secondary();
