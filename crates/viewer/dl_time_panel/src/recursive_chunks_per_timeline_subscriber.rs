...
#[derive(Debug, Default, dl_byte_size::SizeBytes)]
...
impl dl_byte_size::MemUsageTreeCapture for PathRecursiveChunksPerTimelineStoreSubscriber {
    fn capture_mem_usage_tree(&self) -> dl_byte_size::MemUsageTree {
        use dl_byte_size::SizeBytes as _;
        dl_byte_size::MemUsageTree::Bytes(self.chunks_per_timeline_per_entity.total_size_bytes())
    }
}
...
mod tests {
    ...
    use dl_chunk_store::{Chunk, ChunkStore, ChunkStoreConfig, GarbageCollectionOptions, RowId};
    use dl_log_types::example_components::{MyPoint, MyPoints};
    use dl_log_types::{AbsoluteTimeRange, StoreId, TimeInt, Timeline, TimelineName};
