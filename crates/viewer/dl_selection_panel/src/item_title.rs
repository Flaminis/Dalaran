...
pub struct ItemTitle {
    pub icon: &'static dl_ui::Icon,
    pub label: egui::WidgetText,
    pub label_style: Option<dl_ui::LabelStyle>,
...
impl ItemTitle {
    ...
    pub fn from_store_id(ctx: &ViewerContext<'_>, store_id: &dl_log_types::StoreId) -> Self {
        let title = if let Some(entity_db) = ctx.store_bundle().get(store_id) {
            if let Some(started) = entity_db.recording_info_property::<Timestamp>(
                ...
                let time = dl_log_types::Timestamp::from(started.0)
    ...
    pub fn from_container_id(viewport: &ViewportBlueprint, container_id: &ContainerId) -> Self {
        if let Some(container_blueprint) = viewport.container(container_id) {
            ...
            Self::new(
                ...
                dl_viewer_context::icon_for_container_kind(&container_blueprint.container_kind),
            )
    ...
    fn new(name: impl Into<egui::WidgetText>, icon: &'static dl_ui::Icon) -> Self {
        Self {
            label: name.into(),
            tooltip: None,
            icon,
    ...
    #[inline]
    fn with_label_style(mut self, label_style: dl_ui::LabelStyle) -> Self {
