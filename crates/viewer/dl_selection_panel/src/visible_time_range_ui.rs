...
use dl_sdk_types::blueprint::components::VisibleTimeRange;
use dl_sdk_types::datatypes::{TimeInt, TimeRange, TimeRangeBoundary};
use dl_ui::list_item::{LabelContent, ListItemContentButtonsExt as _};
use dl_ui::{RelativeTimeRange, TimeDragValue, UiExt as _, relative_time_range_label_text};
...
pub fn visible_time_range_ui_for_view(
    ...
    let property_path = entity_path_for_view_property(
        ...
        dl_sdk_types::blueprint::archetypes::VisibleTimeRanges::name(),
    );
            ...
            if offset != 0 {
                match time_type {
                    TimeType::DurationNs | TimeType::TimestampNs => {
                        // This looks like it should be generically handled somewhere like dl_format,
                    ...
                    TimeType::Sequence => {
                        write!(
                            label,
                            " with {} offset",
                            dl_format::format_plural_signed_s(offset, "frame")
                        )
                        .ok();
                    }
                }
