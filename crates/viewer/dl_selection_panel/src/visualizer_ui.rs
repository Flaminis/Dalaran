...
use dl_sdk_types::blueprint::archetypes::ActiveVisualizers;
...
use dl_ui::{ComboItem, OnResponseExt as _, UiExt as _, design_tokens_of_visuals, list_item};
use dl_view::{
...
fn visualizers_for_entity<'a>(
    viewer_ctx: &'a dl_viewer_context::ViewerContext<'a>,
...
pub fn visualizer_ui(
    ...
    let button = ui
        .small_icon_button_widget(&dl_ui::icons::ADD, "Add new visualizer…")
        .on_menu(|ui| {
...
pub fn visualizer_ui_impl(
    ...
    let remove_visualizer_button = |ui: &mut egui::Ui, visualizer_id: &VisualizerInstructionId| {
        let response = ui.small_icon_button(&dl_ui::icons::CLOSE, "Remove visualizer");
        if response.clicked() {
...
pub struct SourceSelectorContext<'a> {
    ...
    type_report: Option<&'a dl_viewer_context::VisualizerTypeReport>,
    query_info: VisualizerQueryInfo,
    query_result: dl_view::BlueprintResolvedLatestAtResults<'a>,
...
fn show_visualizer_report(
    ui: &mut egui::Ui,
    report: &dl_viewer_context::VisualizerInstructionReport,
) {
    match report.diagnostic.severity {
        dl_viewer_context::ViewerReportSeverity::Error => {
        ...
        dl_viewer_context::ViewerReportSeverity::Info => {
...
fn raw_default_without_fallback(
    query_result: &dl_view::BlueprintResolvedLatestAtResults<'_>,
    target_component_descr: &ComponentDescriptor,
) -> Option<Arc<dyn dl_chunk::ArrowArray>> {
    let target_component = target_component_descr.component;
...
fn raw_default_or_fallback(
    query_ctx: &dl_viewer_context::QueryContext<'_>,
    query_result: &dl_view::BlueprintResolvedLatestAtResults<'_>,
    target_component_descr: &ComponentDescriptor,
) -> Arc<dyn dl_chunk::ArrowArray> {
...
fn collect_source_component_options(
    ...
    let allowed_physical_types =
        if let dl_viewer_context::VisualizabilityConstraints::SingleRequiredComponent(constraint) =
    ...
    entity_components_with_datatype
        ...
        .flat_map(|(source_component, datatype)| {
            ...
            else if let Some(selectors) = dl_lenses_core::extract_nested_fields(datatype, |dt| {
...
struct SourceMappingContext<'a> {
    data_result: &'a DataResult,
    query_ctx: &'a dl_viewer_context::QueryContext<'a>,
...
impl<'a> SourceMappingContext<'a> {
    ...
    fn viewer_ctx(&self) -> &dl_viewer_context::ViewerContext<'a> {
...
fn source_component_items_ui(
    ...
    if show_sections {
        ui.add(dl_ui::ComboItemHeader::new("Recommended:"));
    }
    ...
    if show_sections {
        ui.add(dl_ui::ComboItemHeader::new("Other values:"));
    }
...
fn source_component_item_ui(
    ...
    if let Some(raw_value) = raw_value {
        ...
        item = item.value_widget(move |ui: &mut Ui| {
            ...
            if num_values > 1 {
                ui.label(format!("{} values", dl_format::format_uint(num_values)));
            } else {
                let viewer_ctx = mapping_ctx.viewer_ctx();
...
///
/// If none is encoded in the visualizer instruction, we apply the same logic as `dl_view::query`.
...
fn menu_add_new_visualizer(
    ...
    if show_sections {
        ui.add(dl_ui::ComboItemHeader::new("Other:"));
    }
...
mod tests {
    ...
    use dl_sdk_types::archetypes::{GaussianSplats3D, Points3D};
    use dl_viewer_context::{DatatypeMatch, SingleRequiredComponentMatch};
    ...
    fn vec3d() -> DataType {
        <dl_sdk_types::datatypes::Vec3D as dl_types_core::Loggable>::arrow_datatype()
    }
