...
use dl_ui::filter_widget::{FilterMatcher, FilterState, PathRanges, format_matching_text};
use dl_ui::{UiExt as _, list_item};
use dl_viewer_context::{DataQueryResult, ViewId, ViewerContext};
...
///
/// Delegates to [`dl_ui::modal::ModalHandler`]
#[derive(Default)]
...
impl ViewEntityPicker {
    ...
    pub fn ui(
        ...
        self.modal_handler.ui(
            egui_ctx,
            || {
                dl_ui::modal::ModalWrapper::new("Add/remove Entities")
...
fn add_entities_line_ui(
    ...
    let Some(add_info) = entities_add_info.get(entity_path) else {
        ...
        dl_log::debug_panic!("No add info for entity path: {entity_path:?}");
        return;
    };
        ...
        if entity_path_filter.contains_rule_for_exactly(entity_path) {
            if ResolvedEntityPathFilter::properties().matches(entity_path) {
                ...
                ui.add_enabled_ui(enabled, |ui| {
                    let response = ui.small_icon_button(&dl_ui::icons::ADD, "Include entity");
                ...
                let response = ui.small_icon_button(&dl_ui::icons::RESET, "Remove this rule");
            ...
            let response = ui.small_icon_button(&dl_ui::icons::REMOVE, "Exclude entity");
