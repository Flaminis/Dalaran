...
use dl_entity_db::{EntityPath, InstancePath};
use dl_log_types::{ComponentPath, EntityPathFilter, EntityPathSubs, ResolvedEntityPathFilter};
use dl_sdk_types::blueprint::components::VisualizerInstructionId;
use dl_sdk_types::{ComponentDescriptor, components::TransformFrameId};
use dl_tracing::profile_function;
use dl_ui::list_item::{self, ListItemContentButtonsExt as _, PropertyContent};
use dl_ui::text_edit::autocomplete_text_edit;
use dl_ui::{
                ...
                ui.list_item()
                    ...
                    .show_flat(
                        ...
                        list_item::LabelContent::new(format!(
                            "{} selected items",
                            dl_format::format_uint(selection.len())
                        )),
                    );
...
fn visualizer_section_plus_button(
    ...
    ui.add(
        ui.small_icon_button_widget(&dl_ui::icons::ADD, "Add new visualizer…")
            .enabled(!options.is_empty())
            .on_custom_menu(
                move |popup| popup.style(dl_ui::menu::menu_style()),
                move |ui| {
...
struct AddVisualizerOptionPerEntity {
    ...
    pub recommended_mappings: dl_viewer_context::RecommendedMappings,
...
fn add_new_visualizer(
    ...
    use dl_sdk_types::blueprint::archetypes::{ActiveVisualizers, ViewContents};
...
fn coordinate_frame_ui(ui: &mut egui::Ui, ctx: &ViewContext<'_>, data_result: &DataResult) {
    ...
    use dl_view::latest_at_with_blueprint_resolved_data;
    ...
    let property_content = list_item::PropertyContent::new("Coordinate frame")
        .value_fn(|ui, _| {
            ...
            let suggestions = {
                ...
                let frame_id_registry =
                    caches.memoizer(|c: &mut dl_viewer_context::TransformDatabaseStoreCache| {
        ...
        .with_menu_button(&dl_ui::icons::MORE, "More options", |ui: &mut egui::Ui| {
...
To learn more about coordinate frames, see the [Spaces & Transforms](https://dalaran.dev/docs/concepts/logging-and-ingestion/transforms) in the manual.",
            );
        });
        ...
        list_item::list_item_scope(ui, "recording_properties", |ui| {
            for suffix_path in property_entities {
                ...
                if suffix_path.is_root() {
                    ...
                    ui.add_space(8.0);
                    ui.label(dl_case::to_human_case(suffix_path[0].unescaped_str()))
                        .on_hover_ui(|ui| {
...
fn view_button(
    ctx: &ViewerContext<'_>,
    ui: &mut egui::Ui,
    view: &dl_viewport_blueprint::ViewBlueprint,
) -> egui::Response {
...
// TODO(#4560): this code should be generic and part of dl_data_ui
    ...
    let (item, list_item_content) = match child_contents {
        Contents::View(view_id) => {
            let Some(view) = viewport.view(view_id) else {
                dl_log::warn_once!("Could not find view with ID {view_id:?}",);
                return false;
            };
        ...
        Contents::Container(container_id) => {
            let Some(container) = viewport.container(container_id) else {
                dl_log::warn_once!("Could not find container with ID {container_id:?}",);
                return false;
            };
...
#[cfg(test)]
mod tests {
    use dl_chunk::{LatestAtQuery, RowId, TimePoint, Timeline};
    use dl_log_types::TimeType;
    use dl_log_types::example_components::{MyPoint, MyPoints};
    use dl_sdk_types::archetypes;
    use dl_test_context::TestContext;
    use dl_test_context::external::egui_kittest::kittest::Queryable as _;
    use dl_test_viewport::{TestContextExt as _, TestView};
    use dl_viewer_context::{
    ...
    fn get_test_context() -> TestContext {
        let mut test_context = TestContext::new_with_store_info(
            dl_log_types::StoreInfo::testing_with_recording_id("test_recording"),
        );
    ...
    fn selection_panel_ui(
        ...
        test_context.run(&ui.ctx().clone(), |viewer_ctx| {
            ui.scope_builder(
                // We need this to for `dl_ui::is_in_resizable_panel` to return the correct thing…
                egui::UiBuilder::new()
    ...
    fn selection_panel_static_component_snapshot() {
        ...
        let component_path = dl_log_types::ComponentPath {
            entity_path,
    ...
    fn selection_panel_component_static_overwrite_snapshot() {
        ...
        test_context
            .selection_state
            .lock()
            .set_selection(Item::ComponentPath(dl_log_types::ComponentPath {
                entity_path,
                component: archetypes::Points2D::descriptor_positions().component,
            }));
