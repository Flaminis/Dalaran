...
use dl_ui::{OnResponseExt as _, SyntaxHighlighting as _, UiExt as _};
...
pub fn view_components_defaults_section_ui(
    ...
    let mut add_button = ui
        .small_icon_button_widget(&dl_ui::icons::ADD, "Add overrides…")
        .on_menu(|ui| {
...
fn active_default_ui(
    ...
    dl_ui::list_item::list_item_scope(ui, "defaults", |ui| {
        ...
        for (component, default_value) in active_defaults {
            ...
            let response = ui.list_item_flat_noninteractive(
                dl_ui::list_item::PropertyContent::new(component_descr.archetype_field_name())
                    .min_desired_width(150.0)
                    .with_action_button(&dl_ui::icons::CLOSE, "Clear blueprint component", || {
            ...
            if let Some(component_type) = component_descr.component_type {
                response.on_hover_ui(|ui| {
                    component_type.data_ui(
                        ...
                        ui,
                        dl_viewer_context::UiLayout::Tooltip,
                    );
                });
            }
        }
...
fn add_new_default(
        ...
        .build()
    {
        ...
        Err(err) => {
            dl_log::warn!("Failed to create Chunk for blueprint component: {err}");
        }
    }
}
