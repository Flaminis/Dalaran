...
use dl_sdk_types::blueprint::archetypes::{self, TensorScalarMapping, TensorViewFit};
use dl_sdk_types::blueprint::components::ViewFit;
...
type ViewType = dl_sdk_types::blueprint::views::TensorView;
...
Set the displayed dimensions in a selection panel.",
    ...
    fn layout_priority(&self) -> dl_viewer_context::ViewClassLayoutPriority {
        dl_viewer_context::ViewClassLayoutPriority::Medium
    }
    ...
    fn selection_ui(
        ...
        if let Some(TensorVisualization { tensor, .. }) = &state.tensor {
            let slice_property = ViewProperty::from_archetype_for_view::<
                dl_sdk_types::blueprint::archetypes::TensorSliceSelection,
            >(ctx, view_id);
...
impl TensorView {
    ...
    fn paint_tensor_slice(
        ...
        gpu_bridge::render_image(
            ...
            dl_renderer::Label::from("tensor_slice"),
        )?;
...
#[test]
fn test_help_view() {
    dl_test_context::TestContext::test_help_view(|ctx| TensorView.help(ctx));
}
