...
struct TensorDimension {
    pub size: u64,
    pub name: Option<dl_sdk_types::ArrowString>,
}

impl TensorDimension {
    pub fn from_tensor_data(tensor_data: &dl_sdk_types::datatypes::TensorData) -> Vec<Self> {
        tensor_data
            .shape
            .iter()
            .enumerate()
    ...
    #[cfg_attr(not(test), expect(dead_code))] // only used in tests
    pub fn named(size: u64, name: impl Into<dl_sdk_types::ArrowString>) -> Self {
        Self {
            size,
            name: Some(name.into()),
        }
