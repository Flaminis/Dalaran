...
use dl_query::LatestAllComponentResults;
    ...
    fn data_ui(&self, ctx: &StoreViewContext<'_>, ui: &mut egui::Ui, ui_layout: UiLayout) {
        ...
        if !ui_layout.is_single_line() {
            ...
            if time.is_static() {
                ...
                dl_log::debug_assert_eq!(hits.num_rows(), 1);
                ...
                if temporal_message_count > 0 {
                    ui.error_label(format!(
                        ...
                        dl_format::format_plural_s(temporal_message_count, "temporal event")
                    ))
                    .on_hover_text(
                ...
                if 1 < hits.num_rows() {
                    ui.horizontal(|ui| {
                        ui.add(dl_ui::icons::COMPONENT_TEMPORAL.as_image());
                        ui.label(format!(
