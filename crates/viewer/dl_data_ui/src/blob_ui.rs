...
use dl_sdk_types::components::{Blob, MediaType, VideoTimestamp};
use dl_sdk_types::{ComponentDescriptor, ComponentIdentifier, RowId, archetypes, components};
use dl_types_core::Component as _;
use dl_ui::list_item::{self, ListItemContentButtonsExt as _, PropertyContent};
use dl_ui::{UiExt as _, icons};
use dl_viewer_context::{AppContext, StoreViewContext, StoredBlobCacheKey, UiLayout};
...
impl EntityDataUi for Blob {
    fn entity_data_ui(
        ...
        let compact_size_string = dl_format::format_bytes(self.len() as _);
        ...
        if ui_layout.is_single_line() {
            ...
            let all_digits_size_string = format!("{} B", dl_format::format_uint(self.len()));
            ...
            dl_ui::list_item::list_item_scope(ui, "blob_info", |ui| {
...
fn exif_ui(ui: &mut egui::Ui, key: StoredBlobCacheKey, blob: &dl_sdk_types::datatypes::Blob) {
    let exif_result = ui.memory_mut(|mem| {
        ...
        cache.get(&key).cloned().unwrap_or_else(|| {
            dl_tracing::profile_scope!("exif-parse");
    ...
    pub fn inline_download_button<'a>(
        ...
        property_content.with_action_button(&icons::DOWNLOAD, "Save blob…", || {
            ...
            ctx.command_sender.save_file_dialog(
                dl_capabilities::MainThreadToken::i_promise_i_am_on_the_main_thread(),
                &file_name,
                "Save blob".to_owned(),
                self.blob.to_vec(),
            );
