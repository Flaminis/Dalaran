//! Basic ui elements & interaction for most `dl_viewer_context::Item`.
//!
//! TODO(andreas): This is not a `data_ui`, can this go somewhere else, shouldn't be in `dl_data_ui`.

use dl_entity_db::entity_db::EntityDbClass;
use dl_entity_db::{EntityTree, InstancePath};
use dl_format::format_uint;
use dl_log_types::{ApplicationId, EntityPath, TableId, TimeInt, TimeType, TimelineName};
...
// TODO(andreas): This is where we want to go, but we need to figure out how get the [`dl_viewer_context::ViewClass`] from the `ViewId`.
...
fn entity_tree_stats_ui(
    ...
    let (static_stats, timeline_stats) = if include_subtree {
        (
            dl_entity_db::EntityDb::subtree_stats_static(&engine, &tree.path),
            dl_entity_db::EntityDb::subtree_stats_on_timeline(&engine, &tree.path, timeline),
        )
    } else {
        (
...
pub fn app_id_button_ui(
    ...
    let response = response.on_hover_ui(|ui| {
        app_id.app_ui(ctx, ui, dl_viewer_context::UiLayout::Tooltip);
    });
...
pub fn data_source_button_ui(
    ...
    let response = response.on_hover_ui(|ui| {
        data_source.app_ui(ctx, ui, dl_viewer_context::UiLayout::Tooltip);
    });
...
pub fn entity_db_button_ui(
        ...
        entity_db.recording_info_property::<Name>(RecordingInfo::descriptor_name().component)
    {
        ...
        entity_db
            ...
            .map(|started| {
                dl_log_types::Timestamp::from(started.0)
    ...
    let size = dl_format::format_bytes(entity_db.byte_size_of_physical_chunks() as _);
    let title = format!("{app_id_prefix}{recording_name} - {size}");

    let store_id = entity_db.store_id().clone();
    let item = dl_viewer_context::Item::StoreId(store_id.clone());

    let icon = match entity_db.store_kind() {
        dl_log_types::StoreKind::Recording => &icons::RECORDING,
        dl_log_types::StoreKind::Blueprint => &icons::BLUEPRINT,
    };
        ...
        item_content = item_content.with_buttons(|ui| {
            ...
            let resp = ui
                ...
                .on_hover_text(match store_id.kind() {
                    ...
                    dl_log_types::StoreKind::Blueprint => {
    ...
    let response = list_item::list_item_scope(ui, "entity db button", |ui| {
        list_item
            ...
            .on_hover_ui(|ui| {
                entity_db.app_ui(ctx, ui, dl_viewer_context::UiLayout::Tooltip);
            })
    })
    .inner;
    ...
    let new_entry: dl_viewer_context::RecordingOrTable = store_id.clone().into();
    ...
    response.context_menu(|ui| {
        ...
        if ui.button("Copy segment name").clicked() {
            dl_log::info!("Copied {recording_name:?} to clipboard");
...
pub fn table_id_button_ui(
    ...
    let item = dl_viewer_context::Item::TableId(table_id.clone());
