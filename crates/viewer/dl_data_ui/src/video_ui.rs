...
use dl_renderer::resource_managers::SourceImageDataFormat;
use dl_renderer::video::VideoFrameTexture;
use dl_sdk_types::components::{MediaType, VideoTimestamp};
use dl_sdk_types::{Archetype as _, archetypes};
use dl_types_core::{ComponentDescriptor, ComponentIdentifier, RowId};
use dl_ui::UiExt as _;
use dl_ui::list_item::{self, PropertyContent};
use dl_video::player::{GetVideoSource, VideoSliceSource};
use dl_video::{FrameInfo, VideoDataDescription};
...
pub fn video_asset_result_ui(
    ui: &mut egui::Ui,
    ui_layout: UiLayout,
    video_result: &Result<dl_renderer::video::Video, VideoLoadError>,
) {
...
fn video_data_ui(
    ...
    if let Some(encoding_details) = &video_descr.encoding_details {
        ...
        if let Some(bit_depth) = encoding_details.bit_depth {
            ...
            let is_image_sequence =
                matches!(video_descr.codec, dl_video::VideoCodec::ImageSequence(_));
            ui.list_item_flat_noninteractive(PropertyContent::new("Bit depth").value_fn(
                |ui, _| {
                    ...
                    if encoding_details.chroma_subsampling
                        == Some(dl_video::ChromaSubsamplingModes::Monochrome)
                    {
                        ui.label("(monochrome)");
                    }
                },
        ...
        if let Some(chroma_subsampling) = encoding_details.chroma_subsampling {
            ...
            if chroma_subsampling != dl_video::ChromaSubsamplingModes::Monochrome {
                ui.list_item_flat_noninteractive(
    ...
    if let Some(duration) = video_descr.duration() {
        ui.list_item_flat_noninteractive(
            PropertyContent::new("Duration")
                .value_text(format!("{}", dl_log_types::Duration::from(duration))),
        );
    }
    ...
    if let Some(bitrate_bps) = video_descr.average_bitrate() {
        ...
        let fully_loaded = video_descr
            .samples
            .iter()
            .all(|s| matches!(s.source(), dl_video::VideoSource::Span(_)));

        ui.list_item_flat_noninteractive(
            PropertyContent::new("Average bitrate")
                .value_text(dl_format::format_bits_per_second(bitrate_bps)),
        )
        ...
        ui.list_item_collapsible_noninteractive_label("MP4 tracks", false, |ui| {
            for (track_id, track_kind) in &video_descr.mp4_tracks {
                let track_kind_string = match track_kind {
                    Some(dl_video::TrackKind::Audio) => "audio",
                    Some(dl_video::TrackKind::Subtitle) => "subtitle",
                    Some(dl_video::TrackKind::Video) => "video",
                    None => "unknown",
                };
                ui.list_item_flat_noninteractive(
    ...
    if stream_kind == StreamKind::Video {
        ui.list_item_collapsible_noninteractive_label("More video statistics", false, |ui| {
            ...
            let dl_video::SamplesStatistics {
...
fn samples_table_ui(ui: &mut egui::Ui, video_descr: &VideoDataDescription) {
    ...
    let table_style = dl_ui::TableStyle::Dense;
    ...
    egui_extras::TableBuilder::new(ui)
        ...
        .header(tokens.deprecated_table_header_height(), |mut header| {
            dl_ui::DesignTokens::setup_table_header(&mut header);
            header.col(|ui| {
                ui.strong("Sample");
            });
            header.col(|ui| {
        ...
        .body(|mut body| {
            ...
            body.rows(
                ...
                |mut row| {
                    ...
                    row.col(|ui| {
                        ui.monospace(dl_format::format_uint(sample_idx));
                    });
                    row.col(|ui| {
                        ui.monospace(dl_format::format_uint(frame_nr));
                    });
                    row.col(|ui| {
                        if let Some(keyframe_index) =
                            video_descr.presentation_time_keyframe_index(presentation_timestamp)
                        {
                            ui.monospace(dl_format::format_uint(keyframe_index));
                        }
                    });
                    row.col(|ui| {
                        if is_sync {
                    ...
                    row.col(|ui| {
                        ...
                        {
                            ui.monospace(
                                dl_log_types::Duration::from(duration.duration(timescale))
                                    .to_string(),
                            );
                        } else {
                    ...
                    row.col(|ui| match source {
                        dl_video::VideoSource::Span(span) => {
                            ui.monospace(dl_format::format_bytes(span.len as _));
                        }
                        dl_video::VideoSource::Id { .. } => {
                            ui.monospace("N/A");
                        }
                    });
                },
...
fn timestamp_ui(
    ...
    timestamp: dl_video::Time,
) {
    let response = ui.monospace(dl_format::format_int(timestamp.0));
...
fn decoded_frame_ui(
    ...
    ui: &mut egui::Ui,
    ui_layout: UiLayout,
    video: &dl_renderer::video::Video,
    video_time: dl_video::Time,
    stream_kind: StreamKind,
    video_source: &dyn GetVideoSource,
) {
    let player_stream_id = dl_video::player::VideoPlayerStreamId(
    ...
    }) = frame_output.output
    {
        ...
        {
            dl_ui::list_item::list_item_scope(
                ui,
                ...
                |ui| {
                    ...
                    let label = if let Some(frame_nr) = frame_info.frame_nr {
                        format!(
                            ...
                            dl_format::format_uint(frame_nr)
                        )
                    } else {
        ...
        let response = if let Some(texture) = texture {
            crate::image_ui::texture_preview_ui(
                ...
                &dl_renderer::renderer::ColormappedTexture::from_video_frame(
                    texture.clone(),
                    video
                        .data_descr()
                        .encoding_details
                ...
                preview_size,
                &|action| match dl_renderer::schedule_read_texture(
                    ctx.render_ctx,
                    &texture.inner.texture,
                ) {
                    Ok(texture) => ctx.command_sender.send_system(
                        dl_viewer_context::SystemCommand::ReadbackAndSaveTexture {
                            texture,
                            action,
                        },
                    ),
                    Err(err) => {
                        dl_log::error!("Failed to save {stream_kind} preview: {err}");
                    }
                },
            )
        } else {
...

        {
            ...
            if 0.0 < loading_indicator_opacity {
                dl_ui::loading_indicator::paint_loading_indicator_inside(
                    ui,
    ...
    if let Some(err) = frame_output.error {
        ...
        if let dl_video::player::VideoPlayerError::Decoding(dl_video::DecodeError::Ffmpeg(err)) =
            &err
        {
            match err.as_ref() {
                dl_video::FFmpegError::UnsupportedFFmpegVersion { .. }
        ...
        if let Some(sample_range) = video_descr.gop_sample_range_for_keyframe(keyframe_idx) {
            ...
            if let Some((first_sample, last_sample)) = Option::zip(
                ...
                ui.list_item_flat_noninteractive(PropertyContent::new("GOP DTS range").value_text(
                    format!("{} - {}", dl_format::format_int(first_sample.decode_timestamp.0), dl_format::format_int(last_sample.decode_timestamp.0))
                ))
                .on_hover_text(
...
pub enum VideoUi {
    ...
    Asset(
        Arc<Result<dl_renderer::video::Video, VideoLoadError>>,
        Option<VideoTimestamp>,
        dl_sdk_types::datatypes::Blob,
    ),
}
...
impl VideoUi {
    pub fn from_blob(
        ...
        blob: &dl_sdk_types::datatypes::Blob,
        media_type: Option<&MediaType>,
        video_timestamp: Option<VideoTimestamp>,
    ) -> Option<Self> {
        let result = ctx.memoizer(|c: &mut dl_viewer_context::VideoAssetCache| {
            ...
            == archetypes::VideoStream::descriptor_sample().component
        {
            ...
            let res = ctx.memoizer(|c: &mut VideoStreamCache| {
                ...
                let query_result = ctx.db.storage_engine().cache().latest_at(
                    ...
                    &dl_chunk_store::LatestAtQuery::new(
                        ctx.timeline_name(),
                        dl_log_types::TimeInt::MAX,
                    ),
                    entity_path,
                    [codec_component],
                );
                ...
                c.entry(
                    ...
                    ctx.app_ctx.app_options.video_decoder_settings(),
                    descr.component,
                    dl_video::VideoCodec::ImageSequence(last_codec.map(|s| s.to_string())),
                )
            });
        ...
        match self {
            ...
            Self::Asset(video_result, timestamp, blob) => {
                ...
                if let Ok(video) = video_result.as_ref() {
                    ...
                    let video_time = dl_viewer_context::video_timestamp_component_to_video_time(
                        Some(ctx.time_ctrl),
                        video_timestamp,
