...
use dl_query::{LatestAllComponentResults, LatestAllResults};
use dl_sdk_types::reflection::ComponentDescriptorExt as _;
use dl_sdk_types::{ArchetypeName, Component as _, ComponentDescriptor, components};
...
fn component_ui(
    ...
    let response = list_item.show_flat(ui, content).on_hover_ui(|ui| {
        ...
        if let Some(column) = ctx.db.storage_engine().store().resolve_component_selector(
            &dl_sorbet::ComponentColumnSelector::from_descriptor(
                entity_path.clone(),
                component_descr,
            ),
        ) {
            dl_ui::list_item::list_item_scope(ui, component_descr, |ui| {
                ui.list_item_flat_noninteractive(PropertyContent::new("Data type").value_text(
                    // TODO(#11071): use dl_arrow_ui to format the datatype here
