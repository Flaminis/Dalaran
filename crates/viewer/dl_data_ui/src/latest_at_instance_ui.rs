...
use dl_log_types::{EntityPath, Instance, TimePoint};
...
/// All the values of a specific [`dl_log_types::ComponentPath`].
#[derive(Clone)]
    ...
    fn data_ui(&self, ctx: &StoreViewContext<'_>, ui: &mut egui::Ui, ui_layout: UiLayout) {
        ...
        if num_instances == 1 || instance.is_specific() {
            ...
            if num_instances == 1
                ...
                ) != dl_viewer_context::TryShowEditUiResult::NotShown
            {
                return;
            }
            ...
            if num_instances > num_displayed_rows {
                ui.label(format!(
                    "…and {} more.",
                    dl_format::format_uint(num_instances - num_displayed_rows)
                ));
            }
        }
