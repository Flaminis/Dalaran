...
use dl_viewer_context::{AppContext, StoreViewContext, UiLayout};
...
use dl_types_core::{ArchetypeName, Component};
...
pub fn annotations(
    ctx: &StoreViewContext<'_>,
    entity_path: &dl_entity_db::EntityPath,
) -> std::sync::Arc<dl_viewer_context::Annotations> {
    dl_tracing::profile_function!();
    let mut annotation_map = dl_viewer_context::AnnotationMap::default();
