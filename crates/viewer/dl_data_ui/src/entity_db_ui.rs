...
impl crate::AppUi for EntityDb {
    fn app_ui(&self, ctx: &AppContext<'_>, ui: &mut egui::Ui, ui_layout: UiLayout) {
        ...
        if !ctx.is_test {
            let title = dl_ui::debug_only::with_debug_only_badge(ui.style(), "Debug info");
...
fn grid_content_ui(ctx: &AppContext<'_>, db: &EntityDb, ui: &mut egui::Ui, ui_layout: UiLayout) {
        ...
        && ui_layout.is_selection_panel()
    {
        ...
        if let Some(store_version) = store_version {
            ...
            dl_log::trace_once!("store version is undefined for this recording, this is a bug");
        }
    ...
    {
        let time = dl_log_types::Timestamp::from_nanos_since_epoch(nanos_since_epoch);
    ...
    if let Some(tl_name) = db
        .timelines()
        .keys()
        .find(|k| **k == dl_log_types::TimelineName::log_time())
            ...
            ui.horizontal(|ui| {
                if db.redap_connection_state() == RedapConnectionState::PartialManifest {
                    ...
                    if max_downloaded_bytes < full_size_bytes {
                        let rect =
                            ui.small_icon(&dl_ui::icons::INFO, Some(ui.visuals().text_color()));
...

    {
        ...
        ui.label(dl_format::format_uint(store.all_entities().len()));
        ui.end_row();

        ui.grid_left_hand_label("Timeline columns");
        ui.label(dl_format::format_uint(schema.indices.len()));
        ui.end_row();

        ui.grid_left_hand_label("Data columns");
        ui.label(dl_format::format_uint(schema.components.len()));
        ui.end_row();

        ui.grid_left_hand_label("Rows");
        ui.label(dl_format::format_uint(store.stats().total().num_rows));
        ui.end_row();
    }
    ...
    if ui_layout.is_selection_panel() {
        ...
        ui.label(format!(
            "{} rows ({} if unsorted) or {}",
            dl_format::format_uint(chunk_max_rows),
            dl_format::format_uint(chunk_max_rows_if_unsorted),
            dl_format::format_bytes(chunk_max_bytes as _),
        ))
            .on_hover_text(
                unindent::unindent(&format!("\
                    ...
                    * {ENV_CHUNK_MAX_BYTES}
                    ...
                    ",
                        ...
                        chunk_max_rows_if_unsorted = dl_format::format_uint(chunk_max_rows_if_unsorted),
                        chunk_max_bytes = dl_format::format_bytes(chunk_max_bytes as _),
