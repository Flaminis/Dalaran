...
use dl_sdk_types::datatypes::{ChannelDatatype, ColorModel};
use dl_sdk_types::image::ImageKind;
use dl_types_core::{Component as _, ComponentDescriptor};
use dl_ui::list_item::ListItemContentButtonsExt as _;
use dl_ui::{UiExt as _, icons, list_item};
use dl_viewer_context::gpu_bridge::{self, image_data_range_heuristic, image_to_gpu};
...
pub fn image_preview_ui(
    ...
    texture_preview_ui(
        ...
        &|action| {
            ...
            match action {
                dl_viewer_context::DownloadAction::CopyToClipboard => image_ui.copy_image(app_ctx),
                dl_viewer_context::DownloadAction::Save => {
...
pub fn texture_preview_ui(
    ...
    if ui_layout.is_single_line() {
        ...
        .unwrap_or_else(|(response, err)| {
            dl_log::warn_once!("Failed to show texture {debug_name}: {err}");
            response
        })
    }
}
...
impl ImageUi {
    ...
    pub fn copy_image<'a>(&'a self, ctx: &'a AppContext<'_>) {
        if let Some(rgba) = self.image.to_rgba8_image(self.data_range.into()) {
            ...
            dl_log::info!("Copied image to clipboard");
        } else {
            dl_log::error!("Invalid image");
        }
    }
    ...
    pub fn inline_download_button<'a>(
        ...
        entity_path: &'a dl_log_types::EntityPath,
        property_content: list_item::PropertyContent<'a>,
    ) -> list_item::PropertyContent<'a> {
