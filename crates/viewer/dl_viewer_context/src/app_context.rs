...
use dl_log_types::{EntityPath, StoreId, TimePoint};
use dl_sdk_types::ComponentDescriptor;
use dl_ui::ContextExt as _;
use dl_ui::list_item::ListItem;
...
pub struct AppContext<'a> {
    ...
    pub reflection: &'a dl_types_core::reflection::Reflection,

    /// The [`egui::Context`].
    pub egui_ctx: &'a egui::Context,

    /// The global `dl_renderer` context, holds on to all GPU resources.
    pub render_ctx: &'a dl_renderer::RenderContext,

    /// Interface for sending commands back to the app
    pub command_sender: &'a CommandSender,

    /// Registry of authenticated redap connections
    pub connection_registry: &'a dl_redap_client::ConnectionRegistryHandle,
    ...
    pub connected_receivers: &'a dl_log_channel::LogReceiverSet,

    /// Are we logged in to Dalaran Hub?
...
impl AppContext<'_> {
    ...
    pub fn active_redap_entry(&self) -> Option<dl_log_types::EntryId> {
        match self.route {
    ...
    pub fn active_table_id(&self) -> Option<&dl_log_types::TableId> {
        match self.route {
    ...
    pub fn tokens(&self) -> &'static dl_ui::DesignTokens {
        self.egui_ctx.tokens()
    }

    /// How to configure the renderer.
    #[inline]
    pub fn render_mode(&self) -> dl_renderer::RenderMode {
        if self.is_test {
            dl_renderer::RenderMode::Deterministic
        } else {
            dl_renderer::RenderMode::Beautiful
        }
    }
    ...
    pub fn store_bundle(&self) -> &dl_entity_db::StoreBundle {
    ...
    pub fn send_time_commands_to_active_recording(
        ...
        if !commands.is_empty() {
            if let Some(store_ctx) = self.active_store_context {
                ...
                dl_log::debug_warn_once!("Time command ignored - no active recording");
            }
        }
    }
    ...
    pub fn append_array_to_store(
            ...
            .build()
        {
            Ok(chunk) => chunk,
            Err(err) => {
                dl_log::error_once!("Failed to create Chunk: {err}");
                return;
            }
        };
