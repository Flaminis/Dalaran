...
use dl_log_types::{ComponentPath, DataPath, EntityPath, EntryId, TableId};
...
pub struct DataResultInteractionAddress {
    ...
    ///
    /// Note that this may be an individual instance or the entire entity if the instance index is [`dl_log_types::Instance::ALL`].
...
impl Item {
    ...
    pub fn redap_origin(&self) -> Option<&dl_uri::Origin> {
        match self {
...
impl From<dl_uri::EntryUri> for Item {
    fn from(uri: dl_uri::EntryUri) -> Self {
        Self::RedapEntry {
            origin: uri.origin,
...
impl std::str::FromStr for Item {
    type Err = dl_log_types::PathParseError;
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        match (instance, component) {
            (Some(instance), Some(_component_descriptor)) => {
                ...
                Err(dl_log_types::PathParseError::UnexpectedInstance(instance))
            }
    ...
    pub fn kind(&self) -> &'static str {
        match self {
            ...
            Self::StoreId(store_id) => match store_id.kind() {
                dl_log_types::StoreKind::Recording => "Recording ID",
                dl_log_types::StoreKind::Blueprint => "Blueprint ID",
            },
...
pub fn resolve_mono_instance_path(
    entity_db: &EntityDb,
    query: &dl_chunk_store::LatestAtQuery,
    instance: &dl_entity_db::InstancePath,
) -> dl_entity_db::InstancePath {
    ...
    if instance.instance.get() == 0 {
        ...
        let Some(components) = engine
            ...
            return dl_entity_db::InstancePath::entity_all(instance.entity_path.clone());
        };
        ...
        for component in components {
            if let Some(array) = engine
                .cache()
                .latest_at(
                    dl_chunk_store::ChunkTrackingMode::ReportTransient,
                    query,
