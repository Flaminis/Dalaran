...
//!      [`dl_ui::DesignTokens::drop_target_container_stroke`].
    ...
    pub fn payload_cursor_ui(&self, ctx: &egui::Context) {
            ...
            && let Some(pointer_pos) = ctx.pointer_interact_pos()
        {
            ...
            let (icon, text) = if let DragAndDropFeedback::Reject(Some(reason)) = feedback {
                (&dl_ui::icons::WARNING, reason)
            } else {
                let icon = match payload.as_ref() {
                    DragAndDropPayload::Contents { .. } => &dl_ui::icons::DND_MOVE,
                    DragAndDropPayload::Entities { .. } | DragAndDropPayload::Components { .. } => {
                        &dl_ui::icons::DND_ADD_TO_EXISTING
                    }
...
fn drag_pill_frame(tokens: &dl_ui::DesignTokens, droppable: bool) -> egui::Frame {
    egui::Frame {
        fill: if droppable {
