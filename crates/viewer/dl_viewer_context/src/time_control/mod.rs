...
impl dl_byte_size::SizeBytes for TimeRangeHighlight {
...
#[derive(
    Clone, Copy, Debug, serde::Deserialize, serde::Serialize, PartialEq, dl_byte_size::SizeBytes,
)]
pub struct TimeView {
...
#[derive(
    serde::Deserialize, serde::Serialize, Clone, PartialEq, Debug, dl_byte_size::SizeBytes,
)]
enum ActiveTimeline {
    ...
    /// a timeline that doesn't exist in the current [`dl_entity_db::EntityDb`]. We store only the name
...
/// applicable.
#[derive(Clone, PartialEq, dl_byte_size::SizeBytes)]
...
impl TimeControl {
    ...
    /// Get the current [`dl_entity_db::PrefetchTimeCursor`].
    ///
    /// If the whole recording is looped the loop range is
    /// `TimeInt::MIN..=TimeInt::MAX`.
    pub fn time_cursor(&self) -> Option<dl_entity_db::PrefetchTimeCursor> {
    ...
    pub fn current_query(&self) -> dl_chunk_store::LatestAtQuery {
