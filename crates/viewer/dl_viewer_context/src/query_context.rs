...
pub struct QueryContext<'a> {
    ...
    pub target_entity_path: &'a dl_log_types::EntityPath,
    ...
    pub archetype_name: Option<dl_sdk_types::ArchetypeName>,

    /// Query which didn't yield a result for the component at the target entity path.
    pub query: dl_chunk_store::LatestAtQuery,
}
...
pub struct DataQueryResult {
    ...
    pub view_defaults: dl_query::LatestAtResults,
}
...
impl Default for DataQueryResult {
    fn default() -> Self {
        Self {
            ...
            view_defaults: dl_query::LatestAtResults::empty(
                "<defaults>".into(),
                dl_chunk_store::LatestAtQuery::latest(blueprint_timeline()),
            ),
        }
    }
}
...
impl ViewerContext<'_> {
    pub fn lookup_query_result(&self, id: ViewId) -> &DataQueryResult {
        self.query_results.get(&id).unwrap_or_else(|| {
            dl_log::debug_warn!("Tried looking up a query that doesn't exist: {id:?}");
            &EMPTY_QUERY
        })
    }
}
