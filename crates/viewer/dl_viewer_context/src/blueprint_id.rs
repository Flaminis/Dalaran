...
use dl_log_types::{EntityPath, EntityPathPart};
...
impl<T: BlueprintIdRegistry> dl_byte_size::SizeBytes for BlueprintId<T> {
...
impl<T: BlueprintIdRegistry> BlueprintId<T> {
    ...
    pub fn gpu_readback_id(self) -> dl_renderer::GpuReadbackIdentifier {
        dl_log_types::hash::Hash64::hash(self.id).hash64()
    }
...
impl<T: BlueprintIdRegistry> From<dl_sdk_types::datatypes::Uuid> for BlueprintId<T> {
    #[inline]
    fn from(id: dl_sdk_types::datatypes::Uuid) -> Self {
        Self {
            id: id.into(),
            _registry: std::marker::PhantomData,
        }
...
impl<T: BlueprintIdRegistry> From<BlueprintId<T>> for dl_sdk_types::datatypes::Uuid {
    #[inline]
...
impl ViewId {
    ...
    pub fn render_view_id(self) -> dl_renderer::ViewBuilderId {
        dl_renderer::ViewBuilderId::new(dl_log_types::hash::Hash64::hash(self.id).hash64())
    }
}
