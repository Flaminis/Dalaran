...
use ahash::HashMap;
use dl_chunk::{ComponentIdentifier, RowId, TimePoint, UnitChunkShared};
use dl_entity_db::EntityPath;
use dl_log::{ResultExt as _, debug_assert_eq};
use dl_log_types::{Instance, StoreId};
use dl_sdk_types::{ComponentDescriptor, ComponentType};
...
bitflags::bitflags! {
    ...
    pub struct ComponentUiTypes: u8 {
        ...
        /// Edit the component in a single [`dl_ui::list_item::ListItem`] line.
        const SingleLineEditor = 0b0000010;

        /// Edit the component over multiple [`dl_ui::list_item::ListItem`]s.
...
impl ComponentUiRegistry {
    ...
    /// * This is the value column function for a [`dl_ui::list_item::PropertyContent`], behave accordingly!
    ///     * Unless you introduce hierarchy yourself, use [`dl_ui::list_item::ListItem::show_flat`].
    ...
    pub fn add_singleline_edit_or_view<C: dl_sdk_types::Component>(
        &mut self,
    ...
    pub fn add_singleline_edit_or_view_for_component<C: dl_sdk_types::Component>(
        &mut self,
    ...
    ///
    /// Design principles for writing editors:
    /// * This is the content function for hierarchical [`dl_ui::list_item::ListItem`], behave accordingly!
    /// * Try not to assume context of the component beyond its inherent semantics
    ///   (e.g. if you get a `Color` you can't assume whether it's a background color or a point color)
    /// * The returned [`egui::Response`] should be for the widget that has the tooltip, not any pop-up content.
    ///     * Make sure that changes are propagated via [`egui::Response::mark_changed`] if necessary.
    pub fn add_multiline_edit_or_view<C: dl_sdk_types::Component>(
        &mut self,
        callback: impl Fn(&AppContext<'_>, &mut egui::Ui, &mut MaybeMutRef<'_, C>) -> egui::Response
        + Send
        + Sync
    ...
    /// * The returned [`egui::Response`] should be for the widget that has the tooltip, not any pop-up content.
    ///     * Make sure that changes are propagated via [`egui::Response::mark_changed`] if necessary.
    pub fn add_singleline_array_edit_or_view<C: dl_sdk_types::Component>(
        &mut self,
        callback: impl Fn(
            &AppContext<'_>,
            &mut egui::Ui,
    ...
    /// * The returned [`egui::Response`] should be for the widget that has the tooltip, not any pop-up content.
    ///     * Make sure that changes are propagated via [`egui::Response::mark_changed`] if necessary.
    pub fn add_multiline_array_edit_or_view<C: dl_sdk_types::Component>(
        &mut self,
        callback: impl Fn(
            &AppContext<'_>,
            &mut egui::Ui,
    ...
    fn add_editor_ui<C: dl_sdk_types::Component>(
        &mut self,
        multiline: bool,
        callback: impl Fn(&AppContext<'_>, &mut egui::Ui, &mut MaybeMutRef<'_, C>) -> egui::Response
        + Send
    ...
    fn add_array_editor_ui<C: dl_sdk_types::Component>(
        &mut self,
        multiline: bool,
        callback: impl Fn(
            &AppContext<'_>,
        ...
        let untyped_callback: UntypedComponentEditOrViewCallback = Box::new(
            move |ctx, ui, component_descriptor, row_id, value, edit_or_view| {
                match edit_or_view {
                    EditOrView::View => {}
                    EditOrView::Edit => {
                        dl_log::error_once!("Editing variant UIs is not supported.");
                        return None;
                    }
                }
    ...
    pub fn component_ui(
        ...
        let Some(array) = unit.component_batch_raw(component_descr.component) else {
            dl_log::error_once!("Couldn't get {component_descr}: missing");
    ...
    pub fn component_ui_raw(
        ...
        dl_tracing::profile_function!(component_descr.display_name());
    ...
    pub fn variant_ui_raw(
        ...
        dl_tracing::profile_function!(variant_name);
...
fn try_deserialize_array<C: dl_sdk_types::Component>(
...
fn try_deserialize<C: dl_sdk_types::Component>(value: &dyn arrow::array::Array) -> Option<C> {
    let component_type = C::name();
    ...
    if let Some(v) = values.into_iter().next() {
        ...
        dl_log::warn_once!("Editor UI for {component_type} needs a start value to operate on.");
        None
    }
}
...
fn fallback_ui(
    ...
    dl_arrow_ui::arrow_ui(ui, ui_layout, timestamp_format, component);
}
