...
use dl_log_channel::LogReceiver;
use dl_log_types::StoreId;
use dl_ui::{RecordingCommand, RecordingCommandSender, UICommand, UICommandSender};
...
pub enum SystemCommand {
    ...
    ActivateApp(dl_log_types::ApplicationId),

    /// Close this app and all its recordings.
    CloseApp(dl_log_types::ApplicationId),
    ...
    RemoveRedapServer(dl_uri::Origin),
    ...
    /// Navigate to time/entities/anchors/etc. that are set in a [`dl_uri::Fragment`].
    SetUrlFragment {
        store_id: StoreId,
    ...
    ShowNotification(dl_ui::notifications::Notification),
    ...
    ReadbackAndSaveTexture {
        texture: dl_renderer::texture_readback::TextureReadbackId,
...
impl SystemCommandSender for CommandSender {
    ...
    fn send_system(&self, command: SystemCommand) {
        ...
        dl_quota_channel::send_crossbeam(&self.system_sender, (Location::caller(), command)).ok();
    }
}
...
impl UICommandSender for CommandSender {
    ...
    fn send_ui(&self, command: UICommand) {
        // The only way this can fail is if the receiver has been dropped.
        dl_quota_channel::send_crossbeam(&self.ui_sender, command).ok();
    }
}
...
impl RecordingCommandSender for CommandSender {
    ...
    fn send_recording_command(&self, command: RecordingCommand) {
        // The only way this can fail is if the receiver has been dropped.
        dl_quota_channel::send_crossbeam(&self.recording_sender, command).ok();
    }
}

impl dl_ui::RedapServerCommandSender for CommandSender {
    fn send_redap_server_command(&self, command: dl_ui::RedapServerCommand) {
        self.send_system(SystemCommand::RedapServer(command));
    }
}

impl dl_ui::TableCommandSender for CommandSender {
    fn send_table_command(&self, command: dl_ui::TableCommand) {
...
impl EditRedapServerModalCommand {
    pub fn new(origin: dl_uri::Origin) -> Self {
        Self {
            origin,
