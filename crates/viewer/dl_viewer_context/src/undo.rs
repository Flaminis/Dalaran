...
use dl_chunk::{LatestAtQuery, TimeInt};
...
impl BlueprintUndoState {
    ...
    pub fn clear_redo_buffer(&mut self, blueprint_db: &mut EntityDb) {
        ...
        if let Some(last_kept_event_time) = self.current_time.take() {
            ...
            let events = blueprint_db.drop_time_range(
                &blueprint_timeline(),
                AbsoluteTimeRange::new(first_dropped_event_time, dl_chunk::TimeInt::MAX),
                dl_chunk_store::ChunkDeletionReason::ExplicitDrop,
            );
            ...
            dl_log::trace!("{} chunks affected when clearing redo buffer", events.len());
        }
    }
    ...
    pub fn update(&mut self, egui_ctx: &egui::Context, blueprint_db: &EntityDb) {
        ...
        if inserted {
            dl_log::trace!("Inserted new inflection point at {time:?}");
        }
