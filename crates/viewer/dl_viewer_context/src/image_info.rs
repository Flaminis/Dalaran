...
use dl_sdk_types::components::{self, Colormap};
use dl_sdk_types::datatypes::{Blob, ChannelDatatype, ColorModel, ImageFormat};
use dl_sdk_types::image::{ImageKind, rgb_from_yuv};
use dl_sdk_types::tensor_data::TensorElement;
use dl_sdk_types::{ComponentIdentifier, archetypes};
...
pub fn resolution_of_image_at(
    ...
    if let Some(((_time, _), _)) = entity_db
        .latest_at_component::<dl_sdk_types::components::VideoSample>(
            entity_path,
            query,
        ...
        let video = ctx
            ...
            .memoizer(|c: &mut crate::VideoStreamCache| {
                let codec = entity_db
                    ...
                    .map(|(_, c)| dl_video::VideoCodec::from(c))
    ...
    let encoded_image_resolution = |image_blob_component, media_type_component| {
        if let Some(((_time, _), _)) = entity_db
            .latest_at_component::<dl_sdk_types::components::Blob>(
                entity_path,
                query,
                ...
                .memoizer(|c: &mut crate::VideoStreamCache| {
                    ...
                    c.entry(
                        ...
                        *ctx.time_ctrl.timeline_name(),
                        ctx.app_options().video_decoder_settings(),
                        image_blob_component,
                        dl_video::VideoCodec::ImageSequence(media_type),
                    )
                });
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, dl_byte_size::SizeBytes)]
...
impl ImageInfo {
    ...
    pub fn get_xyc(&self, x: u32, y: u32, channel: u32) -> Option<TensorElement> {
        ...
        if let Some(pixel_format) = self.format.pixel_format {
            ...
            dl_log::debug_assert!(channel < num_channels as u32);
    ...
    pub fn to_slice<T: bytemuck::Pod>(&self) -> Cow<'_, [T]> {
        ...
        if let Ok(slice) = bytemuck::try_cast_slice(bytes) {
            ...
            dl_tracing::profile_scope!("copy_image_buffer");
...
mod tests {
    ...
    use dl_sdk_types::image::ImageChannelType;
    ...
    fn new_2x2_image_info<T: ImageChannelType>(
        ...
        ImageInfo {
            ...
            buffer: dl_sdk_types::datatypes::Blob::from(bytemuck::cast_slice::<_, u8>(elements)),
            format: dl_sdk_types::datatypes::ImageFormat::from_color_model(
                [2, 2],
                color_model,
                T::CHANNEL_TYPE,
            ),
            kind: dl_sdk_types::image::ImageKind::Color,
        }
    }
