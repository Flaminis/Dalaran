...
use dl_log_types::{EntityPath, EntityPathFilter, EntityPathRule, EntityPathSubs};
...
impl RecommendedView {
    ...
    ) -> dl_sdk_types::blueprint::components::ViewerRecommendationHash {
        let Self {
            origin,
            query_filter,
        } = self;
        ...
        for e in excluded {
            if filter.matches(e) {
                self.query_filter.insert_rule(
                    dl_log_types::RuleEffect::Exclude,
