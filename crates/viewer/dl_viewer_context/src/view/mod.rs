...
pub enum ViewSystemExecutionError {
    ...
    #[error(transparent)]
    QueryError(Arc<dl_query::QueryError>),

    #[error(transparent)]
    DeserializationError(Arc<dl_sdk_types::DeserializationError>),
    ...
    #[error(transparent)]
    GpuTransferError(#[from] dl_renderer::CpuWriteGpuReadError),
    ...
    #[error(transparent)]
    ViewBuilderError(#[from] dl_renderer::view_builder::ViewBuilderError),
}
...
// Convenience conversions for some dl_renderer error types since these are so frequent.

impl From<dl_renderer::renderer::LineDrawDataError> for ViewSystemExecutionError {
    fn from(val: dl_renderer::renderer::LineDrawDataError) -> Self {
        Self::DrawDataCreationError(Arc::new(val))
    }
}

impl From<dl_renderer::renderer::PointCloudDrawDataError> for ViewSystemExecutionError {
    fn from(val: dl_renderer::renderer::PointCloudDrawDataError) -> Self {
        Self::DrawDataCreationError(Arc::new(val))
    }
}

impl From<dl_renderer::renderer::GaussianSplatDrawDataError> for ViewSystemExecutionError {
    fn from(val: dl_renderer::renderer::GaussianSplatDrawDataError) -> Self {
        Self::DrawDataCreationError(Arc::new(val))
    }
}

impl From<dl_sdk_types::DeserializationError> for ViewSystemExecutionError {
    fn from(val: dl_sdk_types::DeserializationError) -> Self {
