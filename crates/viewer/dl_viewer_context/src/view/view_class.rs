...
pub trait ViewClass: Send + Sync {
    ...
    fn help(&self, os: egui::os::OperatingSystem) -> dl_ui::Help;
    ...
    fn recommended_origin_for_entities(
        ...
        _entity_db: &dl_entity_db::EntityDb,
    ) -> Option<EntityPath> {
    ...
    ///
    /// TODO(wumpf): Right now the ui methods control when and how to create [`dl_renderer::ViewBuilder`]s.
    ///              In the future, we likely want to move view builder handling to `dl_viewport` with
...
impl dl_byte_size::SizeBytes for dyn ViewState {
    #[inline]
    fn heap_size_bytes(&self) -> u64 {
