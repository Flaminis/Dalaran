...
pub struct PreviewState {
    ...
    pub requested_uris: ahash::HashSet<dl_uri::DatasetSegmentUri>,
}
...
impl PreviewState {
    ...
    pub fn register_recording(
        &mut self,
        store_id: &StoreId,
        store_bundle: &dl_entity_db::StoreBundle,
    ) {
        self.active_previews.entry(store_id.clone()).or_default();

        if let Some(db) = store_bundle.get(store_id)
            && let Some(dl_entity_db::LogSource::RedapGrpcStream { uri, .. }) = &db.data_source
        {
    ...
    pub fn tick<'db>(
        &mut self,
        resolve: impl Fn(&StoreId) -> Option<&'db dl_entity_db::EntityDb>,
        stable_dt: f32,
    ) -> NeedsRepaint {
...
impl dl_byte_size::MemUsageTreeCapture for ViewStates {
    fn capture_mem_usage_tree(&self) -> dl_byte_size::MemUsageTree {
        ...
        let mut states_node = dl_byte_size::MemUsageNode::default();
