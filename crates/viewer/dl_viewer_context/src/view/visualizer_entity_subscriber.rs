...
#[derive(Clone, dl_byte_size::SizeBytes)] // Cheap to clone; uses ref-counted data internally.
...
impl VisualizerEntitySubscriber {
    /// Bootstrap from an existing [`dl_entity_db::EntityDb`], processing all existing data
    /// so that the subscriber is up-to-date without having received incremental events.
    pub fn bootstrap(&mut self, entity_db: &dl_entity_db::EntityDb) {
        ...
        for chunk in store.iter_physical_chunks() {
            let meta = dl_chunk_store::ChunkMeta::from_chunk(chunk);
            process_entity_components(&self.config, &mut self.mapping, &store_id, &meta);
        }
    }
    ...
    pub fn on_events(&mut self, events: &[ChunkStoreEvent]) {
        ...
        for event in events {
            match &event.diff {
                dl_chunk_store::ChunkStoreDiff::SchemaAddition(schema_add) => {
                ...
                dl_chunk_store::ChunkStoreDiff::Addition(..)
                | dl_chunk_store::ChunkStoreDiff::Deletion(_)
                | dl_chunk_store::ChunkStoreDiff::VirtualAddition(..) => {
...
mod tests {
    ...
    use dl_log_types::{example_components::MyPoint, example_components::MyPoints};
    ...
    fn schema_addition_event(
        ...
        let meta = dl_chunk_store::ChunkMeta {
            entity_path: entity.clone(),
            components: columns
                .iter()
                .map(|(desc, dt)| dl_chunk_store::ChunkComponentMeta {
    ...
    fn empty_required_component_is_still_visualizable() {
        ...
        sub.on_events(&[ChunkStoreEvent {
            ...
            event_id: 0,
            diff: ChunkStoreDiff::SchemaAddition(ChunkStoreDiffSchemaAddition {
                new_columns: vec![dl_chunk_store::ChunkMeta {
                    entity_path: entity.clone(),
                    components: vec![dl_chunk_store::ChunkComponentMeta {
