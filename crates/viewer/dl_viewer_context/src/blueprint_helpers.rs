...
use dl_chunk::{ComponentIdentifier, LatestAtQuery, RowId, TimelineName};
use dl_chunk_store::external::dl_chunk::Chunk;
use dl_entity_db::EntityDb;
use dl_log_types::{EntityPath, StoreId, TimeInt, TimePoint, Timeline};
use dl_sdk_types::{AsComponents, ComponentBatch, ComponentDescriptor, SerializedComponentBatch};
...
pub fn blueprint_timeline() -> TimelineName {
    dl_string_interner::intern_static!(TimelineName, "blueprint")
}

/// The timepoint to use when writing an update to the blueprint.
pub fn blueprint_timepoint_for_writes(blueprint: &dl_entity_db::EntityDb) -> TimePoint {
...
pub trait BlueprintContext {
    ...
    fn save_blueprint_archetypes<'a>(
        ...
        let chunk = match chunk_builder.build() {
            Ok(chunk) => chunk,
            Err(err) => {
                dl_log::error_once!("Failed to create Chunk for blueprint components: {err}");
                return;
            }
        };
    ...
    fn save_blueprint_component(
        ...
        let Some(serialized) = component_batch.serialized(component_descr.clone()) else {
            dl_log::warn!("could not serialize components with descriptor `{component_descr}`");
            return;
        };
    ...
    fn clear_blueprint_component(
        ...
        let chunk = Chunk::builder(entity_path)
            .with_row(
                ...
                [(
                    component_descr,
                    dl_chunk::external::arrow::array::new_empty_array(&datatype),
                )],
            )
            .build();
        ...
        match chunk {
            ...
            Err(err) => {
                dl_log::error_once!("Failed to create Chunk for blueprint component: {err}");
            }
        }
    }
