...
use dl_log_types::{AbsoluteTimeRange, ApplicationId, StoreId, StoreKind, TableId, TimelinePoint};
...
impl StoreHub {
    ...
    pub fn read_context<'a>(
        ...
        static EMPTY_RECORDING: LazyLock<EntityDb> =
            LazyLock::new(|| EntityDb::new(dl_log_types::StoreId::empty_recording()));
        static EMPTY_CACHES: LazyLock<StoreCache> = LazyLock::new(|| {
            StoreCache::empty(
                &ViewClassRegistry::default(),
                dl_log_types::StoreId::empty_recording(),
            )
        });
        ...
        let store_context = 'ctx: {
            ...
            let caches = caches.unwrap_or_else(|| {
                if recording.is_some() {
                    dl_log::debug_warn!("Active recording is missing cache");
                }
                &EMPTY_CACHES
            });
    ...
    pub fn add_chunk_for_tests(
        &mut self,
        store_id: &StoreId,
        chunk: &std::sync::Arc<dl_chunk::Chunk>,
    ) -> anyhow::Result<Vec<dl_chunk_store::ChunkStoreEvent>> {
        let entity_db = self
            .store_bundle
            .get_mut(store_id)
    ...
    fn remove_store(&mut self, store_id: &StoreId) {
        ...
        match removed_store.store_kind() {
            StoreKind::Recording => {
                ...
                if !any_other_recordings_for_this_app {
                    dl_log::trace!("Removed last recording of {app_id}. Closing app.");
        ...
        let (Ok(_) | Err(_)) = std::thread::Builder::new()
            ...
            .spawn(|| {
                dl_tracing::profile_scope!("drop store");
                drop(removed_store);
            });
    }
    ...
    pub fn load_persisted_blueprints_for_app(&mut self, app_id: &ApplicationId) {
        ...
        {
            dl_log::warn!("Failed to load persisted blueprint: {err}");
        }
    }
    ...
    pub fn close_app(&mut self, app_id: &ApplicationId) {
        if let Err(err) = self.save_app_blueprints() {
            dl_log::warn!("Failed to save blueprints: {err}");
        }
    ...
    fn clear_active_blueprint_for_app_id(&mut self, app_id: &ApplicationId) {
        if let Some(blueprint_id) = self.active_blueprint_by_app_id.remove(app_id) {
            dl_log::debug!("Clearing blueprint for {app_id}: {blueprint_id:?}");
    ...
    pub fn clear_blueprints_for_origin(&mut self, origin: &dl_uri::Origin) {
        ...
        if let Some(deleter) = &self.persistence.deleter {
            for app_id in &affected_app_ids {
                if let Err(err) = (deleter)(app_id) {
                    dl_log::warn!("Failed to delete persisted blueprint for {app_id}: {err}");
                }
            }
        }
    }
        ...
        if num_bytes_freed == 0 {
            ...
            if closed_count > 0 {
                dl_log::warn!(
                    ...
                    dl_format::format_plural_s(closed_count, "background recording")
                );
            }
        }
    ...
    pub fn find_recording_by_uri(&self, uri: &dl_uri::DatasetSegmentUri) -> Option<&EntityDb> {
    ...
    pub fn remove_recording_by_uri(&mut self, uri: &str) {
        self.retain_recordings(|db| {
            ...
            match data_source {
                dl_log_channel::LogSource::HttpStream { url, .. } => url != uri,

                dl_log_channel::LogSource::RedapGrpcStream { uri: redap_uri, .. } => {
            ...
            if let Some(blueprint) = self.store_bundle.get_mut(blueprint_id) {
                    ...
                    && let Some(time) = undo.oldest_undo_point()
                {
                    ...
                    protected_time_ranges.insert(
                        ...
                        AbsoluteTimeRange::new(time, dl_chunk::TimeInt::MAX),
                    );
                }
                ...
                let store_events = blueprint.gc(&GarbageCollectionOptions {
                    ...
                    time_budget: dl_entity_db::DEFAULT_GC_TIME_BUDGET,
                ...
                if !store_events.is_empty() {
                    dl_log::debug!("Garbage-collected blueprint store");
    ...
    pub fn save_app_blueprints(&mut self) -> anyhow::Result<()> {
        ...
        for (app_id, blueprint_id) in &self.active_blueprint_by_app_id {
            ...
            let Some(blueprint) = self.store_bundle.get_mut(blueprint_id) else {
                dl_log::debug!("Failed to find blueprint {blueprint_id:?}.");
                continue;
            };
