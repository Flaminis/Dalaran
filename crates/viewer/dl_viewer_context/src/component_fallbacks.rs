...
pub fn typed_fallback_for<C: Component>(
    ...
    let array = query_context
        ...
        .fallback_for(
            &dl_types_core::ComponentDescriptor::partial(component).with_component_type(C::name()),
            query_context,
        );
...
/// `dl_component_fallbacks` crate. For view specific fallbacks that is usually done in
...
impl FallbackProviderRegistry {
    ...
    /// Registers a fallback provider function for a given component type.
    pub fn register_type_fallback_provider<C: dl_sdk_types::Component>(
        &mut self,
    ...
    /// Registers a fallback provider function for a given component identifier.
    pub fn register_component_fallback_provider<C: dl_sdk_types::Component>(
        &mut self,
        component: ComponentIdentifier,
    ...
    /// Registers a fallback provider function for a given component identifier
    /// in a specific view.
    pub fn register_view_fallback_provider<C: dl_sdk_types::Component>(
        &mut self,
        view: ViewClassIdentifier,
        component: ComponentIdentifier,
        provider: impl Fn(&QueryContext<'_>) -> C + Send + Sync + 'static,
    ...
    pub fn fallback_for(
        &self,
        descr: &dl_types_core::ComponentDescriptor,
        ctx: &QueryContext<'_>,
    ) -> ArrayRef {
        let component = descr.component;
        let component_type = descr.component_type;

        dl_tracing::profile_function!(component);
...
/// It can be set as part of the reflection information, see [`dl_types_core::reflection::ComponentReflection::custom_placeholder`].
...
fn placeholder_for(
    ...
    let datatype = if let Some(reflection) = viewer_ctx.reflection().components.get(&component) {
        ...
        viewer_ctx.recording_engine()
                ...
                .unwrap_or_else(|| {
                         dl_log::error_once!("Could not find datatype for component {component}. Using null array as placeholder.");
    ...
    dl_sdk_types::reflection::generic_placeholder_for_datatype(&datatype)
}
