...
pub fn new_renderer_callback(
    view_builder: dl_renderer::ViewBuilder,
    viewport: egui::Rect,
    clear_color: dl_renderer::Rgba,
) -> egui::PaintCallback {
...
struct ReRendererCallback {
    view_builder: Mutex<dl_renderer::ViewBuilder>,
    clear_color: dl_renderer::Rgba,
}
...
impl egui_wgpu::CallbackTrait for ReRendererCallback {
    ...
    fn prepare(
        ...
        let Some(ctx) = paint_callback_resources.get::<dl_renderer::RenderContext>() else {
            dl_log::error_once!(
        ...
        match self.view_builder.lock().draw(ctx, self.clear_color) {
            ...
            Err(err) => {
                dl_log::error_once!("Failed to fill view builder: {err}");
