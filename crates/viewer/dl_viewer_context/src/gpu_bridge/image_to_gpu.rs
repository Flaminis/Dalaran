...
use dl_renderer::{RenderContext, pad_rgb_to_rgba};
...
fn color_image_to_gpu(
    ...
    let color_mapper = if let Some(shader_decoding) = shader_decoding {
        ...
        if let Some(color_mapper) = color_mapper {
            ...
            ColorMapper::Function(dl_renderer::Colormap::Grayscale)
        }
    } else {
        ColorMapper::OffRGB
    };
...
pub fn texture_creation_desc_from_color_image<'a>(
    ...
    // TODO(#10648): All image data ingestion conversions should all be handled by dl_renderer!
    ...
    let (data, format) = if let Some(pixel_format) = image.format.pixel_format {
        ...
        let coefficients = match pixel_format.yuv_matrix_coefficients() {
            dl_sdk_types::image::YuvMatrixCoefficients::Bt601 => YuvMatrixCoefficients::Bt601,
            dl_sdk_types::image::YuvMatrixCoefficients::Bt709 => YuvMatrixCoefficients::Bt709,
        };
        ...
        match (color_model, datatype) {
            ...
            //
            // (in some hopefully not too far future, dl_renderer will have an internal conversion pipeline
    ...
    let colormap_texture_handle = get_or_create_texture(render_ctx, colormap_key, || {
        ...
        let data: Vec<u8> = (0..(colormap_width * colormap_height))
            .flat_map(|id| {
                let color = annotations
                    ...
                    .annotation_info()
                    .color()
                    .unwrap_or(dl_renderer::Color32::TRANSPARENT);
                if !color.is_opaque() {
                    has_alpha = true;
                }
        ...
        ImageDataDesc {
            ...
            alpha_channel_usage: if has_alpha {
                dl_renderer::AlphaChannelUsage::AlphaChannelInUse
            } else {
                dl_renderer::AlphaChannelUsage::Opaque
            },
        }
    })
