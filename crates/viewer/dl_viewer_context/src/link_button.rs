//! Turn built-in viewer URLs into [`dl_ui::LinkButton`]s.
...
use dl_ui::{Icon, LinkButton, icons};
...
pub type UrlNameLookup = HashMap<(dl_uri::Origin, EntryId), ResolvedEntry>;

/// Build the global URL decorator closure for this frame's `lookup` snapshot.
///
/// Install it with [`dl_ui::UrlDecorator::set`].
