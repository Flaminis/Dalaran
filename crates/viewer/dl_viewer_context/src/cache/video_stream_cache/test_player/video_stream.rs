...
fn cache_with_streaming() {
    ...
    let mut store = EntityDb::with_store_config(
        ...
        true,
        dl_chunk_store::ChunkStoreConfig {
...
fn cache_with_streaming_splits() {
    ...
    player.expect_decoded_samples(0..sample_count as dl_video::SampleIndex);
...
fn cache_with_unordered_chunks() {
    use dl_chunk::{RowId, TimeInt, Timeline};
    use dl_video::AV1_TEST_INTER_FRAME;
    use dl_video::AV1_TEST_KEYFRAME;
    ...
    assert!(
        chunks.iter().any(|chunk| {
            chunk
                .timelines()
                .get(&dl_chunk::TimelineName::from(TIMELINE_NAME))
    ...
    assert!(
        cache
            ...
            .contains_key(&crate::cache::video_stream_cache::VideoStreamKey {
                entity_path: dl_chunk::EntityPath::from(STREAM_ENTITY).hash(),
                timeline: dl_chunk::TimelineName::from(TIMELINE_NAME),
...
fn cache_out_of_order_arrival_with_compaction() {
    ...
    assert!(
        store
            ...
            .any(|c| {
                if let Some(dl_chunk_store::ChunkDirectLineage::CompactedFrom(chunks)) =
