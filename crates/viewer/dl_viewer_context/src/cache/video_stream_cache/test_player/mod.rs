...
use dl_chunk::{Chunk, RowId, TimeInt, Timeline};
use dl_entity_db::EntityDb;
use dl_log_types::{AbsoluteTimeRange, external::dl_tuid::Tuid};
use dl_sdk_types::archetypes::VideoStream;
use dl_sdk_types::components::VideoCodec;
use dl_video::player::{GetVideoSource, VideoPlayer, VideoPlayerError, VideoSampleDecoder};
use dl_video::{
...
struct TestDecoder {
    sender: dl_video::Sender<Result<dl_video::Frame, dl_video::DecodeError>>,
    sample_tx: Sender<SampleIndex>,
    min_num_samples_to_enqueue_ahead: usize,
}

impl AsyncDecoder for TestDecoder {
    fn submit_chunk(&mut self, chunk: dl_video::Chunk) -> dl_video::DecodeResult<()> {
        dl_quota_channel::send_crossbeam(&self.sample_tx, chunk.sample_idx).unwrap();

        self.sender
            .send(Ok(dl_video::Frame {
                content: dl_video::FrameContent {
                    data: Vec::new(),
                    width: 0,
                    height: 0,
                    format: dl_video::PixelFormat::Rgb8Unorm,
                },
                info: dl_video::FrameInfo {
    ...
    fn reset(
        &mut self,
        _video_descr: &dl_video::VideoDataDescription,
    ) -> dl_video::DecodeResult<()> {
        Ok(())
    }
...
impl<F: Fn(dl_video::VideoSource)> TestVideoSource<F> {
    fn new(on_source: F) -> Self {
        Self { on_source }
    }
}

impl<F: Fn(dl_video::VideoSource)> GetVideoSource for TestVideoSource<F> {
    fn get_video_chunk(&self, source: dl_video::VideoSource) -> &[u8] {
        (self.on_source)(source);
        &[0]
    }

    fn require_video_source(&self, source: dl_video::VideoSource) {
        (self.on_source)(source);
    }

    fn indicate_video_source(&self, source: dl_video::VideoSource) {
        (self.on_source)(source);
    }
}
...
impl TestVideoPlayer {
    ...
    fn play(&mut self, range: Range<f64>, time_step: f64) -> Result<(), VideoPlayerError> {
        self.play_with_buffer(
            range,
            time_step,
            &TestVideoSource::new(|_: dl_video::VideoSource| {}),
        )
    }
    ...
    fn frame_at(
        ...
        self.video.frame_at(
            Time::from_secs(time, dl_video::Timescale::NANOSECOND),
            &self.video_descr,
            &mut |(), _| Ok(()),
            video_source,
        )?;
    ...
    fn set_sample(&mut self, idx: SampleIndex, mut sample: SampleMetadataState) {
        if let Some(sample) = sample.sample_mut() {
            ...
            sample.decode_timestamp = dl_video::Time::from_secs(
                sample
                    .decode_timestamp
                    .into_secs(dl_video::Timescale::NANOSECOND)
                    - 0.1,
                dl_video::Timescale::NANOSECOND,
            );
        }
...
fn unloaded(time: f64) -> SampleMetadataState {
    let time = Time::from_secs(time, dl_video::Timescale::NANOSECOND);
...
fn frame(time: f64) -> SampleMetadataState {
    ...
    SampleMetadataState::Present(dl_video::SampleMetadata {
        ...
        source: dl_video::VideoSource::id(Tuid::new(), Tuid::new()),
    })
}
...
fn create_video(
    ...
    let mut samples: dl_video::StableIndexDeque<SampleMetadataState> =
    ...
    let video_descr = VideoDataDescription {
        delivery_method: dl_video::VideoDeliveryMethod::Stream {
            last_time_updated_samples: std::time::Instant::now(),
        },
        keyframe_indices,
        samples_statistics: dl_video::SamplesStatistics::new(&samples),
        samples,

        // Unused for these tests.
        codec: dl_video::VideoCodec::H265,
...
fn player_fetching_unloaded() {
    ...
    assert_loading(video.play_with_buffer(
        2.0..4.0,
        1.0,
        &TestVideoSource::new(|source: dl_video::VideoSource| {
...
impl TestVideoPlayer {
    ...
    pub(super) fn play_store_with_component(
        ...
        time_step: f64,
        store: &dl_entity_db::EntityDb,
        sample_component: dl_sdk_types::ComponentIdentifier,
    ) -> Result<(), VideoPlayerError> {
...
pub(super) fn unload_chunks(
    ...
    let store_events = store.gc(&dl_chunk_store::GarbageCollectionOptions {
        target: dl_chunk_store::GarbageCollectionTarget::Everything,
        time_budget: std::time::Duration::from_secs(u64::MAX),
        protect_latest: 0,
        protected_chunks: Default::default(),
        protected_time_ranges: std::iter::once((
            dl_chunk::TimelineName::from(TIMELINE_NAME),
...
pub(super) fn load_chunks(
    ...
    let mut store_events = Vec::<dl_chunk_store::ChunkStoreEvent>::new();
...
pub(super) fn playable_stream(
    ...
    cache
        .video_entry(
            store,
            &dl_chunk::EntityPath::from(STREAM_ENTITY),
            TIMELINE_NAME.into(),
            dl_video::DecodeSettings {
