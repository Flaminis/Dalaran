...
use dl_chunk::{Chunk, RowId, TimeInt, TimePoint, Timeline};
...
fn playable_stream(cache: &mut VideoStreamCache, store: &EntityDb) -> SharablePlayableVideoStream {
    ...
    let query_result = store.storage_engine().cache().latest_at(
        ...
        &dl_chunk::LatestAtQuery::new(TIMELINE_NAME.into(), dl_chunk::TimeInt::MAX),
    ...
    let media_type = query_result
        ...
        .and_then(|chunk| {
            chunk
                .component_mono::<dl_sdk_types::components::MediaType>(media_type_component)?
                .ok()
                .map(|mt| mt.to_string())
        });
    cache
...
fn static_image_survives_gc() {
    ...
    let events = store.gc(&dl_chunk_store::GarbageCollectionOptions {
