...
use dl_log_types::{EntityPathHash, TimeType};
...
pub struct VideoStoreSource<'a> {
    pub store: &'a dl_chunk_store::ChunkStore,
    pub sample_component: dl_chunk::ComponentIdentifier,
...
impl GetVideoSource for VideoStoreSource<'_> {
    fn get_video_chunk(&self, source: VideoSource) -> &[u8] {
        let lookup = |id: dl_log_types::external::dl_tuid::Tuid,
                      sub_id: Option<dl_log_types::external::dl_tuid::Tuid>|
         -> Option<&[u8]> {
            ...
            let sub_id = sub_id?;
            let (offsets, buffer) = dl_arrow_util::blob_arrays_offsets_and_buffer(
                chunk.raw_component_array(self.sample_component)?,
            )?;
            let row_idx = chunk.row_index_of(dl_sdk_types::RowId::from_tuid(sub_id))?;
...
pub struct PlayableVideoStream {
    pub video_renderer: dl_renderer::video::Video,
}

impl PlayableVideoStream {
    pub fn video_descr(&self) -> &dl_video::VideoDataDescription {
...
#[derive(Clone, Copy, Hash, Eq, PartialEq, dl_byte_size::SizeBytes)]
struct VideoStreamKey {
    entity_path: EntityPathHash,
    timeline: TimelineName,
    sample_component: dl_chunk::ComponentIdentifier,
}
...
pub enum VideoStreamProcessingError {
    ...
    FailedReadingCodec(Box<dl_chunk::ChunkError>),
...
impl VideoStreamCache {
    ...
    pub fn video_entry(
        ...
        let query_result = store.storage_engine().cache().latest_at(
            ...
            &dl_chunk::LatestAtQuery::new(timeline, dl_chunk::TimeInt::MAX),
            entity_path,
            [codec_component],
        );
    ...
    pub fn entry(
        ...
        let entry = match self.entries.entry(key) {
            ...
            entry => {
                ...
                let video = dl_renderer::video::Video::load(
    ...
    fn handle_store_event(
        ...
        video_data.delivery_method = dl_video::VideoDeliveryMethod::new_stream();
        ...
        video_data.samples_statistics = dl_video::SamplesStatistics::new(&video_data.samples);
        ...
        match result {
            ...
            Err(VideoStreamProcessingError::UnexpectedChunkChanges) => {
                dl_log::debug_warn_once!("Unexpected chunk changes for video samples");
                drop(video_stream);
...
fn handle_deletion(
    entity_db: &EntityDb,
    timeline: TimelineName,
    video_data: &mut dl_video::VideoDataDescription,
    deleted_chunk: &dl_chunk::Chunk,
...
fn handle_compacted_chunk_addition(
    ...
    video_data: &mut dl_video::VideoDataDescription,
    compacted_chunk: &dl_chunk::Chunk,
    old_chunks: &BTreeMap<ChunkId, Arc<dl_chunk::Chunk>>,
    ...
    let mut update_min_max = |idx: dl_video::SampleIndex| {
...
fn handle_split_chunk_addition(
    ...
    known_chunk_ranges: &mut BTreeMap<ChunkId, ChunkSampleRange>,
    video_data: &mut dl_video::VideoDataDescription,
    split_chunk: &dl_chunk::Chunk,
    original_chunk: &dl_chunk::Chunk,
    siblings: &[Arc<dl_chunk::Chunk>],
    sample_component: dl_chunk::ComponentIdentifier,
) -> Result<(), VideoStreamProcessingError> {
...
fn load_video_data_from_chunks(
    ...
    timeline: TimelineName,
    sample_component: dl_chunk::ComponentIdentifier,
    codec: dl_video::VideoCodec,
) -> Result<
    (
        dl_video::VideoDataDescription,
...
> {
    ...
    let entire_timeline_query =
        dl_chunk::RangeQuery::new(timeline, dl_log_types::AbsoluteTimeRange::EVERYTHING);
    ...
    let mut video_descr = dl_video::VideoDataDescription {
        codec,
        encoding_details: None, // Unknown so far, we'll find out later.
        timescale: timescale_for_timeline(store, timeline),
        delivery_method: dl_video::VideoDeliveryMethod::new_stream(),
        ...
        samples_statistics: dl_video::SamplesStatistics::NO_BFRAMES, // TODO(#10090): No b-frames for now.
    ...
    dl_tracing::profile_scope!("read", format!("{} chunks", sorted_chunks.len()));
    ...
    video_descr.samples_statistics = dl_video::SamplesStatistics::new(&video_descr.samples);
...
fn timescale_for_timeline(
    ...
    match timeline_typ {
        ...
        Some(TimeType::DurationNs | TimeType::TimestampNs) => Some(dl_video::Timescale::NANOSECOND),
    }
}
...
struct ChunkSampleRange {
    first_sample: dl_video::SampleIndex,

    /// Last sample (inclusive).
    last_sample: dl_video::SampleIndex,
...
impl ChunkSampleRange {
    ...
    fn one(idx: dl_video::SampleIndex) -> Self {
        Self {
            first_sample: idx,
            last_sample: idx,
            sample_count: 1,
    ...
    fn add_sample(&mut self, idx: dl_video::SampleIndex) {
    ...
    fn idx_range(&self) -> std::ops::Range<dl_video::SampleIndex> {
...
fn read_samples_from_known_chunk(
    timeline: TimelineName,
    chunk: &dl_chunk::Chunk,
    known_range: &ChunkSampleRange,
    load_range: &ChunkSampleRange,
    video_descr: &mut dl_video::VideoDataDescription,
    sample_component: dl_chunk::ComponentIdentifier,
) -> Result<(), VideoStreamProcessingError> {
    dl_tracing::profile_function!(format!("{} rows", chunk.num_rows()));

    let dl_video::VideoDataDescription {
        codec,
        samples,
        keyframe_indices,
        encoding_details,
    ...
    let (offsets, values) = dl_arrow_util::blob_arrays_offsets_and_buffer(&raw_array).ok_or(
    ...
    for (component_offset, (time, row_id)) in rows {
        ...
        let Some((sample_idx, sample)) = samples_iter.next() else {
            dl_log::error_once!("Failed to add all video stream samples from chunk");
            break;
        };
        ...
        let decode_timestamp = dl_video::Time(time.as_i64());
        ...
        *sample = SampleMetadataState::Present(dl_video::SampleMetadata {
            is_sync,

            // TODO(#10090): No b-frames for now. Therefore sample_idx == frame_nr.
            frame_nr: sample_idx as dl_video::FrameNumber,
            decode_timestamp,
            ...
            source: dl_video::VideoSource::id(chunk.id().as_tuid(), row_id.as_tuid()),
        });
    }

    {
        let _samples_iter = samples_iter;
        dl_log::debug_assert_eq!(
            _samples_iter
...
fn is_sample_sync(
    codec: &dl_video::VideoCodec,
    encoding_details: &mut Option<dl_video::VideoEncodingDetails>,
    sample_bytes: &[u8],
) -> bool {
    match dl_video::detect_gop_start(sample_bytes, codec.clone()) {
        Ok(dl_video::GopStartDetection::StartOfGop(new_encoding_details)) => {
        ...
        Ok(dl_video::GopStartDetection::NotStartOfGop) => false,

        Err(err) => {
            dl_log::error_once!("Failed to detect GOP for video sample: {err}");
            false
        }
    }
}
...
fn update_sample_durations(
    known_range: std::ops::Range<dl_video::SampleIndex>,
...
fn read_samples_from_new_chunk(
    ...
    let mut previous_max_presentation_timestamp = samples
        ...
        .rev()
        .find_map(|s| s.sample())
        .map_or(dl_video::Time::MIN, |s| s.presentation_timestamp);
    ...
    .filter(|(component_offset, _)| {
        if component_offset.len != 1 {
            dl_log::warn_once!("Expected only a single sample per row (it is a mono-component)");
            return false;
        }
    ...
    samples.extend(
        rows.enumerate()
            .map(move |(idx, (component_offset, (time, row_id)))| {
                ...
                SampleMetadataState::Present(dl_video::SampleMetadata {
                    is_sync,
                    ...
                    source: dl_video::VideoSource::id(chunk_id.as_tuid(), row_id.as_tuid()),
                })
            }),
    );
...
impl dl_byte_size::MemUsageTreeCapture for VideoStreamCache {
    fn capture_mem_usage_tree(&self) -> dl_byte_size::MemUsageTree {
        let Self {
            entries,
            keys_per_entity,
        } = self;

        dl_byte_size::MemUsageNode::default()
...
impl ChunkSamples {
        ...
        if chunk.is_static() {
            ...
            let mut samples: Vec<_> = chunk
                ...
                .map(|time| SampleMetadataState::Unloaded {
                    source_id: chunk.id().as_tuid(),
                    min_dts: dl_video::Time::new(time.get()),
                })
                .collect();
    ...
    /// Create new chunk samples from a [`dl_log_encoding::RrdManifestTemporalMapEntry`].
    ///
    ...
    fn from_temporal_root(
        id: ChunkId,
        entry: &dl_log_encoding::RrdManifestTemporalMapEntry,
    ) -> Option<Self> {
        if entry.num_rows == 0 {
            return None;
        }
        ...
        Some(Self {
            ...
            samples: (0..entry.num_rows)
                .map(|_| SampleMetadataState::Unloaded {
                    source_id: id.as_tuid(),
                    min_dts: dl_video::Time::new(entry.time_range.min.as_i64()),
                })
                .collect(),
        })
    }

    fn min_time(&self) -> dl_video::Time {
...
impl ChunkSampleIterators {
    ...
    fn next_if(&mut self, f: impl FnOnce(dl_video::Time) -> bool) -> Option<SampleMetadataState> {
    ...
    fn handle_samples(
        &mut self,
        known_chunk_ranges: &mut BTreeMap<ChunkId, ChunkSampleRange>,
        predicate: impl Fn(dl_video::Time) -> bool,
...
/// by time.
///
/// `handle_sample` should return the [`dl_video::SampleIndex`] that the sample
...
fn load_known_chunk_ranges(
    data_descr: &mut dl_video::VideoDataDescription,
    store: &EntityDb,
    known_chunk_ranges: &mut BTreeMap<ChunkId, ChunkSampleRange>,
    loaded_chunks: &[dl_chunk::Chunk],
    entity_path: &EntityPath,
    timeline: TimelineName,
    sample_component: dl_chunk::ComponentIdentifier,
) {
    ...
    let mut loaded_chunks_counts: ahash::HashMap<dl_chunk::ChunkId, u64> = HashMap::default();
        ...
        temporal_chunks_from_manifest
            ...
            .filter_map(|(id, entry)| {
                ...
                ChunkSamples::from_temporal_root(
                    *id,
                    &dl_log_encoding::RrdManifestTemporalMapEntry {
                        num_rows: remaining,
                        ..*entry
                    },
                )
...
pub struct SampleReorderInfo {
    ...
    pub change_start: dl_video::SampleIndex,

    /// The last sample that was in the reorder-range (inclusive range).
    pub change_end: dl_video::SampleIndex,
...
fn find_affected_sample_range(
    video_descr: &dl_video::VideoDataDescription,
    conflicting_chunk: &dl_chunk::Chunk,
    conflicting_chunk_samples: &ChunkSamples,
    known_range: Option<&ChunkSampleRange>,
) -> Result<std::ops::RangeInclusive<dl_video::SampleIndex>, VideoStreamProcessingError> {
...
fn handle_out_of_order_chunk(
        ...
        .iter_index_range_clamped_mut(&(*sample_range.start()..samples_end))
    {
        if let SampleMetadataState::Present(meta) = sample {
            meta.frame_nr = idx as dl_video::FrameNumber;
        }
    }
...
fn adjust_keyframes_for_removed_samples(descr: &mut dl_video::VideoDataDescription) {
...
mod tests {
    ...
    use dl_chunk::{ChunkBuilder, RowId, TimePoint, Timeline};
    use dl_chunk_store::ChunkStoreDiff;
    use dl_log_types::StoreId;
    use dl_sdk_types::archetypes::VideoStream;
    use dl_sdk_types::components::VideoCodec;
    use dl_video::{VideoDataDescription, VideoEncodingDetails};
    ...
    fn validate_stream_from_test_data(
        ...
        assert_eq!(codec, dl_video::VideoCodec::H264);
        assert_eq!(timescale, None); // Sequence timeline doesn't have a timescale.
        assert!(matches!(
            delivery_method,
            dl_video::VideoDeliveryMethod::Stream { .. }
        ));
        ...
        assert_eq!(
            chroma_subsampling,
            Some(dl_video::ChromaSubsamplingModes::Yuv420)
        );
        assert_eq!(stsd, None);
    ...
    fn video_stream_cache_from_single_chunk() {
        ...
        let mut store = dl_entity_db::EntityDb::new(StoreId::random(
    ...
    fn video_stream_cache_from_chunk_per_frame() {
        ...
        let mut store = dl_entity_db::EntityDb::with_store_config(
            StoreId::random(dl_log_types::StoreKind::Recording, "test_app"),
            enable_viewer_indexes,
            dl_chunk_store::ChunkStoreConfig::COMPACTION_DISABLED,
        );
        let timeline = Timeline::new_sequence("frame");
        ...
        for compaction_enabled in [true, false] {
            ...
            let mut store = dl_entity_db::EntityDb::with_store_config(
                ...
                if compaction_enabled {
                    dl_chunk_store::ChunkStoreConfig::DEFAULT
                } else {
                    dl_chunk_store::ChunkStoreConfig::COMPACTION_DISABLED
                },
            );
        ...
        cache.on_store_events(
            &[&ChunkStoreEvent {
                ...
                diff: ChunkStoreDiff::deletion(
                    ...
                    dl_chunk_store::ChunkDeletionReason::GarbageCollection,
                ),
            }],
            &store,
        );
    ...
    fn video_stream_cache_from_single_static_encoded_image() {
        ...
        let chunk = ChunkBuilder::new(ChunkId::new(), "img".into())
            .with_archetype_auto_row(
                TimePoint::default(),
                &dl_sdk_types::archetypes::EncodedImage::new(jpeg_data)
        ...
        let video_stream_lock = cache
            .entry(
                ...
                *timeline.name(),
                DecodeSettings::default(),
                dl_sdk_types::archetypes::EncodedImage::descriptor_blob().component,
                dl_video::VideoCodec::ImageSequence(Some("image/jpeg".to_owned())),
            )
            .unwrap();
        let video_stream = video_stream_lock.read();
    ...
    fn video_stream_cache_static_picks_highest_row_id() {
        ...
        let jpeg_data = std::fs::read(jpeg_path).unwrap();
        let image =
            dl_sdk_types::archetypes::EncodedImage::new(jpeg_data).with_media_type("image/jpeg");
        ...
        let dl_video::VideoSource::Id { sub_id, .. } = sample.source else {
