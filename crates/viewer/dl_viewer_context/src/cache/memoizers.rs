...
use dl_mutex::{RwLock, RwLockReadGuard, RwLockWriteGuard};
...
impl Memoizers {
    ...
    pub fn begin_frame(&self) {
        ...
        for cache in self.caches.read().values() {
            dl_tracing::profile_scope!(cache.name);
    ...
    fn shared_cache_entry<C: Cache + Default>(&self) -> Arc<SharedCache> {
        ...
        if let Some(cache) = {
            dl_tracing::profile_wait!("master-cache-read-lock");
        ...
        let mut caches = {
            dl_tracing::profile_wait!("master-cache-write-lock");
            self.caches.write()
        };
        caches
            .entry(type_id)
    ...
    pub fn entry<C: Cache + Default, R>(&self, f: impl FnOnce(&mut C) -> R) -> R {
        ...
        let mut cache_guard = {
            dl_tracing::profile_wait!("cache-write-lock", shared_cache.name);
            shared_cache.write()
        };
    ...
    pub fn read<C: Cache, R>(&self, f: impl FnOnce(&C) -> R) -> Option<R> {
        ...
        let cache_guard = {
            dl_tracing::profile_wait!("cache-read-lock", shared_cache.name);
            shared_cache.read()
        };
    ...
    pub fn read_or_compute<C: CacheEntryAccess<Key, Value> + Default, Key, Value>(
        ...
        {
            let cache = {
                dl_tracing::profile_wait!("cache-read-lock");
                cache_entry.read()
            };
        ...
        let mut cache = {
            dl_tracing::profile_wait!("cache-write-lock");
            cache_entry.write()
        };
