...
use dl_sdk_types::image::{ImageKind, ImageLoadError};
...
/// ([`dl_sdk_types::archetypes::EncodedDepthImage`]), e.g. to derive a depth range.
///
        ...
        *self
            ...
            .or_insert_with(|| {
                match decode_depth_image(blob_row_id, blob_component, image_bytes, media_type) {
                    ...
                    Err(err) => {
                        dl_log::warn_once!("Failed to decode depth image: {err}");
                        None
                    }
                }
            })
...
fn decode_depth_image(
    ...
    if media_type.as_str() == MediaType::RVL {
        let metadata = dl_rvl::RosRvlMetadata::parse(image_bytes)
            .map_err(|err| ImageLoadError::DecodeError(err.to_string()))?;
        let depths = dl_rvl::decode_rvl_with_quantization(image_bytes, &metadata)
            .map_err(|err| ImageLoadError::DecodeError(err.to_string()))?;

        let format = dl_sdk_types::datatypes::ImageFormat::depth(
            [metadata.width, metadata.height],
            dl_sdk_types::datatypes::ChannelDatatype::F32,
        );
...
impl dl_byte_size::MemUsageTreeCapture for EncodedDepthImageStatsCache {
...
#[cfg(test)]
mod tests {
    use dl_sdk_types::archetypes::EncodedDepthImage;
