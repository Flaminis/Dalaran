...
//! `make_url_decorator` → `dl_ui` rendering path across the different built-in URL types, so we
...
use dl_ui::{UiLayout, UrlDecorator};
use dl_viewer_context::{LinkKind, ResolvedEntry, UrlNameLookup, make_url_decorator};
...
fn link_buttons_match_snapshot() {
    ...
    for (theme, suffix) in [(egui::Theme::Dark, "dark"), (egui::Theme::Light, "light")] {
        ...
        let mut harness =
            dl_ui::testing::new_harness(dl_ui::testing::TestOptions::Gui, [480.0, 360.0])
                .with_theme(theme)
                .build_ui(move |ui| {
