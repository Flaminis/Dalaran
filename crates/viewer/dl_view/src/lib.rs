...
pub enum ComponentMappingError {
    ...
    SelectorParseFailed(dl_lenses_core::SelectorError),

    /// Failed to execute a selector.
    #[error("Failed to select data: {0}")]
    SelectorExecutionFailed(dl_lenses_core::SelectorError),
    ...
    ComponentNotPresentOnEntity(dl_types_core::ComponentIdentifier),

    #[error("Component '{0}' exists on the entity but no data is available at the given time.")]
    NoComponentDataForQuery(dl_types_core::ComponentIdentifier),
    ...
    NoComponentDataForQueryButIsFetchable(dl_types_core::ComponentIdentifier),
}
