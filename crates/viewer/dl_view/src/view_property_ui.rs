...
use dl_types_core::reflection::ArchetypeFieldReflection;
use dl_types_core::{Archetype, ArchetypeReflectionMarker};
...
pub fn view_property_ui_with_redirect<A: Archetype + ArchetypeReflectionMarker>(
    ...
    redirect_with_view_id: dl_viewer_context::ViewId,
) {
    let view_property = ViewProperty::from_archetype::<A>(ctx);
...
pub fn view_property_component_ui_custom(
    ...
    let singleline_list_item_content = list_item::PropertyContent::new(display_name)
        .with_menu_button(&dl_ui::icons::MORE, "More options", |ui| {
