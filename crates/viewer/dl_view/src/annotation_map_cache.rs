...
use dl_viewer_context::{AnnotationMap, Cache, CacheEntryAccess, SizeBytes as _};

struct AnnotationMapCacheAccessor<'a> {
    recording: &'a dl_entity_db::EntityDb,
    query: &'a dl_chunk_store::LatestAtQuery,
}
...
/// [`dl_viewer_context::StoreViewContext::memoizer_read_or_compute`], and frame boundaries discard
/// every entry.
#[derive(Default)]
pub struct AnnotationMapCache {
    maps: HashMap<dl_chunk_store::LatestAtQuery, Arc<AnnotationMap>>,
}
