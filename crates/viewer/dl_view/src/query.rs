...
fn transform_chunk(
    ...
    selector: Option<&dl_lenses_core::Selector>,
    cast: &CastTarget,
    chunk: &dl_chunk_store::Chunk,
) -> Result<dl_chunk_store::Chunk, ComponentMappingError> {
...
///
/// Also applies a [`dl_lenses_core::Selector`], if specified.
...
fn component_not_found_error(
    ...
    missing_virtual_chunks: &[dl_chunk_store::ChunkId],
    entity_db: &dl_entity_db::EntityDb,
    store_engine: &dl_query::StorageEngineReadGuard<'_>,
    timeline_name: Option<dl_log_types::TimelineName>,
...
pub fn range_with_blueprint_resolved_data<'a>(
    ctx: &'a ViewContext<'a>,
    annotations: Option<&dl_viewer_context::Annotations>,
    range_query: &RangeQuery,
    data_result: &'a dl_viewer_context::DataResult,
    components: impl IntoIterator<Item = ComponentIdentifier>,
    visualizer_instruction: &dl_viewer_context::VisualizerInstruction,
...
pub fn range_with_blueprint_resolved_data_polymorphic<'a>(
    ctx: &'a ViewContext<'a>,
    _annotations: Option<&dl_viewer_context::Annotations>,
    range_query: &RangeQuery,
    data_result: &'a dl_viewer_context::DataResult,
    components: impl IntoIterator<Item = ComponentIdentifier>,
    visualizer_instruction: &dl_viewer_context::VisualizerInstruction,
    ...
    let store_results = {
        ...
        for (target_component, source) in &visualizer_instruction.component_mappings {
            let source = if let dl_viewer_context::VisualizerComponentSource::SourceComponent {
                source_component,
                selector,
            } = source
                && queried_components.remove(target_component)
            {
                ...
                if selector.is_empty() {
                    ...
                    match selector.parse::<dl_lenses_core::Selector>() {
                        Ok(selector) => {
                            active_remappings.push(ActiveRemapping {
                                target: *target_component,
                                source: *source_component,
...
pub fn latest_at_with_blueprint_resolved_data<'a>(
    ctx: &'a ViewContext<'a>,
    annotations: Option<&'a dl_viewer_context::Annotations>,
    latest_at_query: &LatestAtQuery,
    data_result: &'a dl_viewer_context::DataResult,
    components: impl IntoIterator<Item = ComponentIdentifier>,
    visualizer_instruction: Option<&dl_viewer_context::VisualizerInstruction>,
...
pub fn latest_at_with_blueprint_resolved_data_polymorphic<'a>(
    ctx: &'a ViewContext<'a>,
    _annotations: Option<&'a dl_viewer_context::Annotations>,
    latest_at_query: &LatestAtQuery,
    data_result: &'a dl_viewer_context::DataResult,
    components: impl IntoIterator<Item = ComponentIdentifier>,
    visualizer_instruction: Option<&dl_viewer_context::VisualizerInstruction>,
    ...
    if let Some(visualizer_instruction) = visualizer_instruction {
        for (target_component, source) in &visualizer_instruction.component_mappings {
            let source_result =
                if let dl_viewer_context::VisualizerComponentSource::SourceComponent {
                    source_component,
                    selector,
                } = source
                    && queried_components.remove(target_component)
...
fn query_overrides_at_path(
    ctx: &ViewerContext<'_>,
    blueprint_path: &dl_log_types::EntityPath,
    components: impl IntoIterator<Item = ComponentIdentifier>,
) -> LatestAtResults {
    ...
    for component in components {
        ...
        if let Some(value) = component_override_result.get(component) {
            ...
            dl_log::debug_assert!(index.is_some(), "{value:#?}");
...
pub trait DataResultQuery {
    fn latest_at_with_blueprint_resolved_data<'a, A: dl_types_core::Archetype>(
        &'a self,
        ctx: &'a ViewContext<'a>,
        ...
        match self.query_range() {
            QueryRange::TimeRange(time_range) => {
                let range_query = RangeQuery::new(
                    view_query.timeline,
                    dl_log_types::AbsoluteTimeRange::from_relative_time_range(
                        time_range,
