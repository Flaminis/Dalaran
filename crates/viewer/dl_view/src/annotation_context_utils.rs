...
use dl_viewer_context::{Annotations, QueryContext, ResolvedAnnotationInfos, typed_fallback_for};
    ...
    if let Some(last_color) = colors.last() {
        ...
        match annotation_infos {
            ResolvedAnnotationInfos::Same(count, annotation_info) => {
                dl_tracing::profile_scope!("no colors, same annotation");
            ...
            ResolvedAnnotationInfos::Many(annotation_info) => {
                dl_tracing::profile_scope!("no-colors, many annotations");
                // TODO(RR-3840): Fallback should be already incorporated into the query.
...
pub type Keypoints = HashMap<
    (dl_sdk_types::components::ClassId, i64),
    HashMap<dl_sdk_types::datatypes::KeypointId, glam::Vec3>,
>;

/// Resolves all annotations and keypoints for the given entity view.
pub fn process_annotation_and_keypoint_slices(
    latest_at: dl_log_types::TimeInt,
    num_instances: usize,
    positions: impl Iterator<Item = glam::Vec3>,
    keypoint_ids: &[dl_sdk_types::components::KeypointId],
    class_ids: &[dl_sdk_types::components::ClassId],
    annotations: &Annotations,
) -> (ResolvedAnnotationInfos, Keypoints) {
    dl_tracing::profile_function_if!(100_000 < num_instances);
