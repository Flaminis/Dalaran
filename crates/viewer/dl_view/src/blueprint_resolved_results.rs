...
use dl_query::{LatestAtResults, RangeResults};
            ...
            ComponentSourceKind::SourceComponent => {
                if let Some(unit_chunk) = self.store_results.get(component) {
                    let unit_chunk = if force_preserve_store_row_ids {
                        ...
                        let chunk: dl_chunk::Chunk = (**unit_chunk).clone();
...
impl BlueprintResolvedRangeResults<'_> {
    ...
    pub fn entity_path(&self) -> &dl_log_types::EntityPath {
...
impl BlueprintResolvedLatestAtResults<'_> {
    ...
    #[inline]
    pub fn get_mono<C: dl_types_core::Component>(
        &self,
        component: ComponentIdentifier,
    ) -> Option<C> {
    ...
    /// required vs. optional distinction.
    #[inline]
    pub fn get_mono_with_fallback<C: dl_types_core::Component>(
        &self,
        component: ComponentIdentifier,
    ) -> C {
    ...
    pub fn get_fallback_for(&self, descr: &dl_types_core::ComponentDescriptor) -> ArrayRef {
        self.query_context
            .viewer_ctx()
...
impl BlueprintResolvedResults<'_> {
    pub fn timeline(&self) -> dl_log_types::TimelineName {
        match self {
            Self::LatestAt(query, _) => query.timeline().unwrap_or_else(|| {
                dl_log::error_once!("Blueprint latest-at query unexpectedly missing a timeline");
                dl_log_types::TimelineName::log_time()
            }),
...
use dl_chunk::{ChunkComponentIterItem, RowId, TimeInt, TimelineName};
use dl_chunk_store::external::dl_chunk;
...
impl<'a> HybridResultsChunkIter<'a> {
    ...
    ///
    /// See [`dl_chunk::Chunk::iter_component`] for more information.
    pub fn component_slow<C: dl_types_core::Component>(
        &'a self,
    ...
    ///
    /// See [`dl_chunk::Chunk::iter_slices`] for more information.
    pub fn slice<S: 'a + dl_chunk::ChunkComponentSlicer>(
        &'a self,
    ...
    ///
    /// See [`dl_chunk::Chunk::iter_slices_from_struct_field`] for more information.
    pub fn slice_from_struct_field<S: 'a + dl_chunk::ChunkComponentSlicer>(
        &'a self,
        field_name: &'a str,
    ) -> impl Iterator<Item = ((TimeInt, RowId), S::Item<'a>)> + 'a {
        self.chunks_with_component
