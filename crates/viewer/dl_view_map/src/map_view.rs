...
use dl_renderer::view_builder::ViewBuilderError;
use dl_renderer::{ViewBuilder, ViewPickingConfiguration};
use dl_sdk_types::blueprint::archetypes::{MapBackground, MapZoom};
use dl_sdk_types::blueprint::components::{MapProvider, ZoomLevel};
use dl_sdk_types::{View as _, ViewClassIdentifier};
use dl_ui::{Help, IconText, icons, list_item};
...
pub struct MapViewState {
    ...
    /// Because `dl_renderer` can have varying, multiple frames of delay, we must keep track of the
...
impl dl_byte_size::SizeBytes for MapViewState {
...
type ViewType = dl_sdk_types::blueprint::views::MapView;
...
impl ViewClass for MapView {
    ...
    fn selection_ui(
        ...
        dl_ui::list_item::list_item_scope(ui, "map_selection_ui", |ui| {
            let ctx = self.view_context(ctx, view_id, state, space_origin);
            dl_view::view_property_ui::<MapZoom>(&ctx, ui);
            dl_view::view_property_ui::<MapBackground>(&ctx, ui);
        });
    ...
    fn ui(
        ...
        if Some(map_memory.zoom()) != blueprint_zoom_level {
            map_zoom.save_blueprint_component(
                ...
                &ZoomLevel(dl_sdk_types::datatypes::Float64(map_memory.zoom())),
            );
        }

        //
        // Draw all objects using dl_renderer
        //
...
fn create_view_builder(
    ...
    dl_renderer::ViewBuilder::new(
        ctx.render_ctx(),
        dl_renderer::view_builder::TargetConfiguration {
            name: "MapView".into(),
            ...
            projection_from_view: dl_renderer::view_builder::Projection::Orthographic {
                camera_mode:
                    dl_renderer::view_builder::OrthographicCameraMode::TopLeftCornerAndExtendZ,
            ...
            outline_config: highlights
                .any_outlines()
                .then(|| dl_view::outline_config(egui_ctx)),

            // Make sure the map in the background is not completely overwritten
            blend_with_background: dl_renderer::BlendWithBackground::AlphaToCoverage,
...
fn handle_picking_and_ui_interactions(
    ...
    if let Some(pointer_in_ui) = map_response.hover_pos() {
        ...
        Some(ViewPickingConfiguration {
            picking_rect: dl_renderer::RectInt::from_middle_and_extent(
...
// TODO(ab, andreas): this is a partial copy past of dl_view_spatial::picking_gpu. Should be
...
fn picking_gpu(
    ...
    let gpu_picking_result =
        dl_renderer::PickingLayerProcessor::readback_result(render_ctx, gpu_readback_identifier);

    if let Some(gpu_picking_result) = gpu_picking_result {
        // TODO(ab, andreas): the block inside this particular branch is so reusable, it should probably live on dl_renderer! (on picking result?)
        ...
        let mut picked_id = dl_renderer::PickingLayerId::default();
        ...
        let new_result = if picked_id == dl_renderer::PickingLayerId::default() {
            // Nothing found.
            None
        } else {
            Some(dl_view::instance_path_hash_from_picking_layer_id(picked_id))
        };
...
#[test]
fn test_help_view() {
    dl_test_context::TestContext::test_help_view(|ctx| MapView.help(ctx));
}
