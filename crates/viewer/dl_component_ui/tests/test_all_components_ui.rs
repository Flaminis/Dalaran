...
use dl_sdk_types::blueprint::components::{ComponentColumnSelector, QueryExpression};
use dl_sdk_types::components::{self, GraphEdge, GraphNode, ImageFormat, Text};
use dl_sdk_types::datatypes::{ChannelDatatype, PixelFormat};
use dl_test_context::TestContext;
use dl_types_core::reflection::Reflection;
use dl_types_core::{Component, ComponentBatch, ComponentType};
...
fn test_cases(reflection: &Reflection) -> Vec<TestCase> {
    ...
    let excluded_components = std::iter::chain(
        [
            ...
            // `EntityDatatUi`. The hooks are registered by `dl_data_ui::register_component_uis`, which
            // is not available here. So basically no point testing them here.
            dl_sdk_types::components::AnnotationContext::name(),
            dl_sdk_types::components::Blob::name(),
            dl_sdk_types::components::ClassId::name(),
            dl_sdk_types::components::ImageBuffer::name(), // this one is not technically handled by `DataUi`, but should get a custom ui first (it's using default ui right now).
            dl_sdk_types::components::KeypointId::name(),
            dl_sdk_types::components::TensorData::name(),
            //
            // no need to clutter the tests with these internal blueprint types
            dl_sdk_types::blueprint::components::ActiveTab::name(),
            dl_sdk_types::blueprint::components::AutoLayout::name(),
            dl_sdk_types::blueprint::components::AutoViews::name(),
            dl_sdk_types::blueprint::components::ColumnShare::name(),
            dl_sdk_types::blueprint::components::IncludedContent::name(),
            dl_sdk_types::blueprint::components::PanelState::name(),
            dl_sdk_types::blueprint::components::RootContainer::name(),
            dl_sdk_types::blueprint::components::RowShare::name(),
            dl_sdk_types::blueprint::components::ViewMaximized::name(),
            dl_sdk_types::blueprint::components::ViewOrigin::name(),
            dl_sdk_types::blueprint::components::ViewerRecommendationHash::name(),
            dl_sdk_types::blueprint::components::VisualizerInstructionId::name(),
        ],
...
fn run_all_component_tests(width: f32, theme: egui::Theme, output_dir: &str) {
    ...
    let snapshot_options =
        dl_ui::testing::default_snapshot_options_for_ui().output_path(output_dir);
...
// dl_viewer_context, which cannot depend on dl_component_ui.
...
fn placeholder_for_component(
    ...
    component: dl_chunk::ComponentType,
) -> Option<ArrayRef> {
    ...
    datatype.map(|datatype| dl_sdk_types::reflection::generic_placeholder_for_datatype(&datatype))
}
