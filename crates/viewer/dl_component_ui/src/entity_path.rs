use dl_sdk_types::components::EntityPath;
use dl_viewer_context::{AppContext, MaybeMutRef, StoreViewContext};
    ...
    if let Some(path) = path.as_mut() {
        ...
        *path = string.into();
        ...
        if let Some(store_view_ctx) = StoreViewContext::for_active_recording(ctx) {
            dl_data_ui::item_ui::entity_path_button(&store_view_ctx, ui, None, &entity_path)
        } else {
