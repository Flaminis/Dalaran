...
//! This should be called by `dl_viewer` on startup.
...
use dl_sdk_types::{archetypes, components};
use dl_viewer_context::gpu_bridge::colormap_edit_or_view_ui_with_selection;
...
/// This crate is meant to be a leaf crate in the viewer ecosystem and should only be used by the `dl_viewer` crate itself.
pub fn create_component_ui_registry() -> dl_viewer_context::ComponentUiRegistry {
    dl_tracing::profile_function!();

    let mut registry = dl_viewer_context::ComponentUiRegistry::new();
    ...
    registry.add_singleline_edit_or_view::<IsKeyframe>(|ctx, ui, value| {
        edit_bool(
            ctx,
            ui,
            &mut dl_viewer_context::MaybeMutRef::Ref(value.as_ref()),
        )
    });
