...
fn run_view_systems(
    ...
    dl_tracing::profile_function!(view.class_identifier().as_str());

    let view_ctx = view.bundle_context_with_state(ctx, view_state);

    {
        dl_tracing::profile_wait!("ViewContextSystem::execute");
        context_systems
            .systems
            .par_iter_mut()
            .for_each(|(name, (view_ctx_system, state))| {
                dl_tracing::profile_scope!("ViewContextSystem::execute", name.as_str());
    ...
    dl_tracing::profile_wait!("VisualizerSystem::execute");
    let per_visualizer_type_results = view_systems
        ...
        .map(|(name, vis_system)| {
            ...
            dl_tracing::profile_scope!("VisualizerSystem::execute", name.as_str());
...
pub fn new_view_query<'a>(ctx: &'a ViewerContext<'a>, view: &'a ViewBlueprint) -> ViewQuery<'a> {
    ...
    {
        dl_tracing::profile_scope!("active_visualizer_instructions_per_type");
...
pub fn execute_systems_for_all_views<'a>(
    ...
    dl_tracing::profile_wait!("execute_systems");
            ...
            match tile {
                egui_tiles::Tile::Pane(view_id) => {
                    ...
                    let Some(view_state) = view_states.get(store_id, *view_id) else {
                        dl_log::debug_panic!("View state for view {view_id:?} not found. That shouldn't be possible since we just ensured they exist above.");
                        return None;
                    };
