...
impl ViewportUi {
    ...
    pub fn viewport_ui(
        ...
        ui.scope(|ui| {
            ...
            dl_tracing::profile_scope!("tree.ui");
...
impl<'a> egui_tiles::Behavior<ViewId> for TilesDelegate<'a, '_> {
    fn pane_ui(
        ...
        let (query, system_output) = self.executed_systems_per_view.remove(view_id).unwrap_or_else(|| {
            ...
            dl_tracing::profile_scope!("late-system-execute", view.class_identifier().as_str());
    ...
    fn tab_title_for_pane(&mut self, view_id: &ViewId) -> egui::WidgetText {
        if let Some(view) = self.viewport_blueprint.view(view_id) {
            ...
            dl_log::warn_once!("ViewId missing during egui_tiles");
    ...
    fn top_bar_right_ui(
        ...
        if *self.maximized == Some(view_id) {
            ...
            if ui
                .small_icon_button(&dl_ui::icons::MINIMIZE, "Restore viewport")
                .on_hover_ui(|ui| {
            ...
            if ui
                .small_icon_button(&dl_ui::icons::MAXIMIZE, "Maximize view")
                .on_hover_ui(|ui| {
    ...
    fn tab_bar_color(&self, visuals: &egui::Visuals) -> egui::Color32 {
        ...
        dl_ui::design_tokens_of(theme).tab_bar_color
    }
    ...
    fn tab_bar_height(&self, style: &egui::Style) -> f32 {
        dl_ui::design_tokens_of_visuals(&style.visuals).title_bar_height()
    }
    ...
    fn simplification_options(&self) -> egui_tiles::SimplificationOptions {
        dl_viewport_blueprint::tree_simplification_options()
    }

    // Callbacks:

    fn on_edit(&mut self, edit_action: egui_tiles::EditAction) {
        dl_log::trace!("Tree edit: {edit_action:?}");
        match edit_action {
...
impl TilesDelegate<'_, '_> {
    fn reports_button(&self, ui: &mut egui::Ui, view_id: ViewId) {
        ...
        for report in self.view_states.view_reports(self.ctx.store_id(), view_id) {
            if report.severity != dl_viewer_context::ViewerReportSeverity::Info {
                grouped_reports
        ...
        for (_id, report) in per_visualizer_type_reports {
            match report {
                dl_viewer_context::VisualizerTypeReport::OverallError(error) => {
                    grouped_reports
                        .entry(Item::View(view_id))
                        .or_default()
                        .push(&error.diagnostic);
                }
                dl_viewer_context::VisualizerTypeReport::PerInstructionReport(reports_map) => {
        ...
        ui.scope(|ui| {
            let report_image =
                if max_severity == Some(dl_viewer_context::ViewerReportSeverity::Warning) {
                    icons::WARNING
                        .as_image()
            ...
            let response = ui
                ...
                .on_hover_text(format!(
                    "Show {}",
                    dl_format::format_plural_s(report_count, "report")
                ));
    ...
    fn show_item_reports(
        ...
        ui: &mut egui::Ui,
        item: Item,
        reports: &[&dl_viewer_context::ViewerDiagnostic],
    ) {
            ...
            .show(ui, |ui| {
                ui.vertical(|ui| {
                        ...
                        .clicked()
                    {
                        ...
                        self.ctx
                            .command_sender()
                            .send_ui(dl_ui::UICommand::ExpandSelectionPanel);
                    }
                    ui.separator();
                    ...
                    for report in reports {
                        let (icon, color) = match report.severity {
                            ...
                            dl_viewer_context::ViewerReportSeverity::Info => continue,
                        };
        ...
        let tab_desc = match tiles.get(tile_id) {
            Some(egui_tiles::Tile::Pane(view_id)) => {
                if let Some(view) = tab_viewer.viewport_blueprint.view(view_id) {
                    ...
                    dl_log::warn_once!("View {view_id} not found");

                    TabDesc {
                        widget_text: tab_viewer.ctx.egui_ctx().error_text("Unknown view").into(),
                        icon: &dl_ui::icons::VIEW_GENERIC,
                        user_named: false,
                        item: None,
                        label: None,
                    }
            ...
            Some(egui_tiles::Tile::Container(container)) => {
                    ...
                    tab_viewer.contents_per_tile_id.get(&tile_id)
                {
                        ...
                        tab_viewer.viewport_blueprint.container(container_id)
                    {
                        ...
                        dl_log::warn_once!("Container {container_id} missing during egui_tiles");
                        (
                            tab_viewer
                                .ctx
                                .egui_ctx()
                    ...
                    dl_log::warn_once!("Container for tile ID {tile_id:?} not found");
            ...
            None => {
                dl_log::warn_once!("Tile {tile_id:?} not found");
