//! Provides a test context that builds on `dl_viewer_context`.
...
use dl_sdk_types::{Component as _, ComponentDescriptor};
use dl_types_core::reflection::Reflection;
use dl_ui::Help;
...
pub fn asset_path(relative_path: impl AsRef<std::path::Path>) -> std::path::PathBuf {
    // `crates/viewer/dl_test_context` → `crates/viewer` → `crates` → repo-root.
    std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .ancestors()
        .nth(3)
        .expect("workspace root is three ancestors up from crates/viewer/dl_test_context")
        .join("tests/assets")
        .join(relative_path)
}
...
/// // test_context.component_ui_registry = dl_component_ui::create_component_ui_registry();
/// // dl_data_ui::register_component_uis(&mut test_context.component_ui_registry);
/// ```
pub struct TestContext {
    ...
    pub connection_registry: dl_redap_client::ConnectionRegistryHandle,
...
pub struct TestBlueprintCtx<'a> {
    ...
    blueprint_query: &'a dl_chunk::LatestAtQuery,
}
...
impl BlueprintContext for TestBlueprintCtx<'_> {
    ...
    fn blueprint_query(&self) -> &dl_chunk::LatestAtQuery {
...
pub trait VisualizerBlueprintContext: BlueprintContext {
    ...
    fn save_visualizers(
        ...
        view_id: ViewId,
        visualizers: impl IntoIterator<Item = impl Into<dl_sdk_types::Visualizer>>,
    ) {
        use dl_sdk_types::AsComponents;
        use dl_sdk_types::blueprint::archetypes as bp_archetypes;
...
impl TestContext {
    ...
    pub fn new_with_store_info(store_info: StoreInfo) -> Self {
        Self::new_with_store_info_and_config(
            store_info,
            dl_chunk_store::ChunkStoreConfig::from_env().unwrap_or_default(),
        )
    }
    ...
    pub fn new_with_store_info_and_config(
        ...
        });
        {
            ...
            recording_store
                .set_recording_property(
                    ...
                    &dl_sdk_types::components::Name::from("Test recording"),
                )
                .unwrap();
            recording_store
                .set_recording_property(
                    ...
                    &dl_sdk_types::components::Timestamp::from(
            ...
            recording_store
                .set_recording_property(
                    ...
                    ComponentDescriptor {
                        ...
                        component_type: Some(dl_sdk_types::components::Text::name()),
                    },
                    &dl_sdk_types::components::Text::from("Swallow Falls"),
                )
                .unwrap();
            recording_store
                .set_recording_property(
                    ...
                    &dl_sdk_types::components::Text::from("Cloudy with meatballs"),
                )
                .unwrap();
        }
        ...
        let reflection =
            dl_sdk_types::reflection::generate_reflection().expect("Failed to generate reflection");
        ...
        Self {
            ...
            reflection,
            connection_registry:
                dl_redap_client::ConnectionRegistry::new_without_stored_credentials(),
...
fn create_egui_renderstate() -> egui_wgpu::RenderState {
    ...
    // Put dl_renderer::RenderContext into the callback resources so that render callbacks can access it.
    render_state.renderer.write().callback_resources.insert(
        dl_renderer::RenderContext::new(
            &shared_wgpu_setup.adapter,
            shared_wgpu_setup.device.clone(),
            shared_wgpu_setup.queue.clone(),
            wgpu::TextureFormat::Rgba8Unorm,
            |_| dl_renderer::RenderConfig::testing(),
        )
        .expect("Failed to initialize dl_renderer"),
    );
...
fn init_shared_renderer_setup() -> SharedWgpuResources {
    let instance = wgpu::Instance::new(dl_renderer::device_caps::testing_instance_descriptor());
    let adapter = pollster::block_on(dl_renderer::device_caps::select_testing_adapter(&instance));
    ...
    let device_caps = dl_renderer::device_caps::DeviceCaps::from_adapter(&adapter)
...
impl TestContext {
    ...
    pub fn setup_kittest_for_rendering_ui(
        ...
        self.setup_kittest_for_rendering(dl_ui::testing::TestOptions::Gui, size.into())
    }
    ...
    pub fn setup_kittest_for_rendering_3d(
        ...
        self.setup_kittest_for_rendering(dl_ui::testing::TestOptions::Rendering3D, size.into())
    }

    fn setup_kittest_for_rendering(
        &self,
        option: dl_ui::testing::TestOptions,
        size: egui::Vec2,
    ) -> egui_kittest::HarnessBuilder<()> {
        // Egui kittests insists on having a fresh render state for each test.
        let new_render_state = create_egui_renderstate();
        let builder = dl_ui::testing::new_harness(option, size).renderer(
            // Note that render state clone is mostly cloning of inner `Arc`.
            // This does _not_ duplicate dl_renderer's context contained within.
    ...
    pub fn active_timeline(&self) -> Option<dl_chunk::Timeline> {
    ...
    pub fn run(&self, egui_ctx: &egui::Context, func: impl FnOnce(&ViewerContext<'_>)) {
        dl_log::PanicOnWarnScope::new(); // TODO(andreas): There should be a way to opt-out of this.
        ...
        let drag_and_drop_manager =
            dl_viewer_context::DragAndDropManager::new(ItemCollection::default());
        ...
        let render_ctx = egui_renderer
            .callback_resources
            .get_mut::<dl_renderer::RenderContext>()
            .expect("No dl_renderer::RenderContext in egui_render_state");
        ...
        // If dl_renderer was used, `setup_kittest_for_rendering` should have been called.
        ...
        assert!(num_view_builders_created == 0 || called_setup_kittest_for_rendering,
                "Rendering with `dl_renderer` requires setting up kittest with `TestContext::setup_kittest_for_rendering`
                to ensure that kittest & dl_renderer use the same graphics device.");
    ...
    fn go_to_dataset_data(&self, store_id: StoreId, fragment: dl_uri::Fragment) {
        ...
        if let Some(selection) = fragment.selection {
            ...
            let item = if let Some(component) = component {
                dl_viewer_context::Item::from(dl_log_types::ComponentPath::new(
                    entity_path,
                    component,
                ))
            } else if let Some(instance) = instance {
                dl_viewer_context::Item::from(InstancePath::instance(entity_path, instance))
            } else {
                dl_viewer_context::Item::from(entity_path)
            };
        ...
        while let Some((_from_where, command)) = self.command_receiver.recv_system() {
            ...
            match command {
                ...
                SystemCommand::LoadDataSource(data_source) => {
                    if let Some(dl_uri::RedapUri::DatasetData(uri)) = data_source
                        .as_uri()
                        .and_then(|uri| uri.parse::<dl_uri::RedapUri>().ok())
                    {
