...
use dl_log_types::{Timeline, TimelineName};
...
use dl_view_dataframe::DataframeView;
...
fn get_test_context() -> TestContext {
    let mut test_context = TestContext::new_with_view_class::<dl_view_dataframe::DataframeView>();
...
fn setup_blueprint(test_context: &mut TestContext, timeline_name: &TimelineName) -> ViewId {
    test_context.setup_viewport_blueprint(|ctx, blueprint| {
        let view_id = blueprint.add_view_at_root(ViewBlueprint::new_with_root_wildcard(
            dl_view_dataframe::DataframeView::identifier(),
        ));

        // the dataframe view to the desired timeline
        let query = dl_view_dataframe::Query::from_blueprint(ctx, view_id);
