...
use dl_log_types::{AbsoluteTimeRange, EntityPath, Timeline, TimelineName};
use dl_sdk_types::blueprint::archetypes::DataframeQuery;
use dl_sdk_types::blueprint::{components, datatypes};
use dl_sorbet::{ColumnSelector, ComponentColumnSelector};
use dl_viewer_context::{ViewSystemExecutionError, ViewerContext};
...
impl Query {
    ...
    pub(crate) fn timeline_name(
        &self,
        ctx: &ViewerContext<'_>,
    ) -> Result<dl_log_types::TimelineName, ViewSystemExecutionError> {
        ...
        let result = view_columns
            ...
            .filter(|column| match column {
                ...
                ColumnDescriptor::Component(desc) => selected_component_columns
                    .contains(&desc.column_name(dl_sorbet::BatchType::Dataframe)),
            })
            .cloned()
            .map(ColumnSelector::from)
            .collect();
