...
use dl_dataframe_ui::dl_table_utils::{apply_table_style_fixes, cell_ui, header_ui};
use dl_dataframe_ui::{ColumnBlueprint, DisplayRecordBatch, DisplayRecordBatchError};
...
pub(crate) fn dataframe_ui(
    ...
    ui: &mut egui::Ui,
    query_handle: &mut dl_dataframe::QueryHandle<StorageEngineArcReadGuard>,
...
impl egui_table::TableDelegate for DataframeTableDelegate<'_> {
        ...
        if cell.row_nr == 0 {
            header_ui(ui, table_style, false, |ui| {
                if let Some(entity_path) = &self.header_entity_paths[cell.group_index] {
                    ...
                    let item = dl_viewer_context::Item::from(entity_path.clone());
                ...
                let header_content = |ui: &mut egui::Ui| {
                    ...
                    let is_selected = match column {
                        ...
                        ColumnDescriptor::Component(component_column_descriptor) => self
                            .ctx
                            .selection()
                            .contains_item(&dl_viewer_context::Item::ComponentPath(
                    ...
                    match column {
                        ...
                        ColumnDescriptor::Component(component_column_descriptor) => {
                            self.ctx.handle_select_hover_drag_interactions(
                                &response,
                                dl_viewer_context::Item::ComponentPath(
                                    component_column_descriptor.component_path(),
                                ),
                                false,
                            );
    ...
    fn cell_ui(&mut self, ui: &mut egui::Ui, cell: &egui_table::CellInfo) {
        ...
        dl_log::debug_assert!(cell.row_nr < self.num_rows, "Bug in egui_table");
        ...
        let display_data = match &self.display_data {
            ...
            Err(err) => {
                error_ui(
                    ui,
                    format!("Error with display data: {}", dl_error::format(err)),
                );
                return;
            }
        };
        ...
        if Some(cell.row_nr) == self.time_cursor_row {
            ...
            ui.painter().hline(
                rect.x_range(),
                rect.max.y - dl_dataframe_ui::dl_table_utils::CELL_SEPARATOR_STROKE_OFFSET,
    ...
    match subcell_kind {
        ...
        SubcellKind::SummaryWithExpand => {
            let cell_clicked = cell_with_hover_button_ui(
                ...
                |ui| {
                    ui.label(dl_format::format_plural_s(instance_count, "instance"));
                },
            );
        ...
        SubcellKind::MoreInstancesSummary {
            ...
            let cell_clicked = cell_with_hover_button_ui(
                ...
                |ui| {
                    ui.label(format!(
                        "{} more…",
                        dl_format::format_uint(remaining_instances)
                    ));
                },
            );
