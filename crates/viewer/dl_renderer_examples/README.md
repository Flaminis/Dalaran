Standalone examples for `dl_renderer`
-----------------------------------------------

To get a list of all examples run:
```
cargo run -p dl_renderer_examples --bin
```

E.g. to run the `multiview`` example run
```
cargo run -p dl_renderer_examples --bin multiview
```


To run the same example on the web:
```
cargo run-wasm -p dl_renderer_examples --bin multiview
```
