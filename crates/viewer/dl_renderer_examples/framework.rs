...
use dl_renderer::view_builder::ViewBuilder;
use dl_renderer::{RenderConfig, RenderContext};
...
impl<E: Example + 'static> Application<E> {
    async fn new(window: Window) -> anyhow::Result<Self> {
        ...
        let mut instance_desc = dl_renderer::device_caps::instance_descriptor(None);
        ...
        let adapter = {
            let enabled_backends = dl_renderer::device_caps::default_backends();
            let adapters = instance.enumerate_adapters(enabled_backends).await;
            dl_renderer::device_caps::select_adapter(&adapters, enabled_backends, Some(&surface))
    ...
    fn resumed(&mut self, _event_loop: &ActiveEventLoop) {
        #[cfg(not(target_arch = "wasm32"))]
        {
            let attributes = winit::window::WindowAttributes::default()
                .with_title(format!("dl_renderer sample - {}", E::title()))
...
pub fn start<E: Example + 'static>() {
    ...
    cfg_select! {
        target_arch = "wasm32" => {
            ...
            dl_async::spawn_local(run::<E>(event_loop, window));
        }
        _ => {
