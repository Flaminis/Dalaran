...
const MESH_ID: PickingLayerId = PickingLayerId {
    object: dl_renderer::PickingLayerObjectId(0x1234_5678_9012_3456),
    instance: dl_renderer::PickingLayerInstanceId(0x3456_1234_5678_9012),
};
    ...
    fn draw(
        ...
        for (i, point_set) in self.point_sets.iter().enumerate() {
            point_builder
                ...
                .picking_object_id(dl_renderer::PickingLayerObjectId(i as u64 + 1)) // offset by one since 0=default=no hit
                .add_points_slow(
                    &point_set.positions,
                    &point_set.radii,
                    &point_set.colors,
        ...
        view_builder.queue_draw(
            re_ctx,
            dl_renderer::renderer::GenericSkyboxDrawData::new(re_ctx, Default::default()),
        );
        view_builder.queue_draw(
            re_ctx,
            dl_renderer::renderer::MeshDrawData::new(re_ctx, &instances)?,
        );

        let command_buffer = view_builder.draw(re_ctx, dl_renderer::Rgba::TRANSPARENT)?;
