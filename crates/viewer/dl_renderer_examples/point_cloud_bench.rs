...
//! Native:
//! ```sh
//! cargo run -p dl_renderer_examples --bin point_cloud_bench --release
//! cargo run -p dl_renderer_examples --bin point_cloud_bench --release -- --benchmark
//! cargo run -p dl_renderer_examples --bin point_cloud_bench --release -- --benchmark --frames 200
//! cargo run -p dl_renderer_examples --bin point_cloud_bench --release -- --benchmark --points 16m --camera near --size world
//! cargo run -p dl_renderer_examples --bin point_cloud_bench --release -- --benchmark --points 16m --camera away --size world
//! cargo run -p dl_renderer_examples --bin point_cloud_bench --release -- --experiment sorted-front-to-back
//! ```
//!
//! Keys:
...
impl PointCloudBench {
    ...
    fn set_config(&mut self, config: Config) {
        self.config = config;
        dl_log::info!("Config: {}", Self::config_label(config));
    }
    ...
    fn point_cloud_draw_data(
        &mut self,
        re_ctx: &dl_renderer::RenderContext,
    ) -> anyhow::Result<dl_renderer::renderer::PointCloudDrawData> {
    ...
    fn occluder_draw_data(re_ctx: &dl_renderer::RenderContext) -> anyhow::Result<MeshDrawData> {
...
impl framework::Example for PointCloudBench {
    ...
    fn new(re_ctx: &dl_renderer::RenderContext) -> Self {
        ...
        let mut this = Self {
            ...
            adapter_info: Some(dl_renderer::adapter_info_summary(re_ctx.adapter_info())),
        };
        this.log_controls();
    ...
    fn draw(
        ...
        let command_buffer = view_builder
            ...
            .draw(re_ctx, dl_renderer::Rgba::TRANSPARENT)?;
    ...
    fn on_frame_finished(
        ...
        if let Err(err) = re_ctx.device.poll(wgpu::PollType::Wait {
            ...
            dl_log::warn_once!("Failed to wait for submitted GPU work: {err}");
        }
