...
impl RustCodeGenerator {
    fn generate_folder(
        ...
        for obj in objects.objects_of_kind(object_kind) {
            ...
            if crate_name == "dl_types_core" {
                code = code.replace("::dl_types_core", "crate");
            }
...
fn generate_object_file(
    ...
    code.push_str("use ::dl_types_core::try_serialize_field;\n");
    code.push_str("use ::dl_types_core::SerializationResult;\n");
    code.push_str("use ::dl_types_core::{DeserializationResult, DeserializationError};\n");
    code.push_str("use ::dl_types_core::{ComponentDescriptor, ComponentType};\n");
    code.push_str("use ::dl_types_core::{ComponentBatch as _, SerializedComponentBatch};\n");
...
fn quote_struct(
    ...
    let quoted_derive_size_bytes = quote!(#[derive(::dl_byte_size::SizeBytes)]);
...
fn quote_union(
    ...
    let quoted_fields = fields.iter().map(|obj_field| {
        let name = format_ident!("{}", dl_case::to_pascal_case(&obj_field.name));
...
fn quote_enum(
    ...
    let tokens = quote! {
        ...
        impl ::dl_types_core::reflection::Enum for #name {
            type Repr = #repr_type;
...
impl quote::ToTokens for TypeTokenizer<'_> {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        match typ {
            ...
            Type::String => quote!(::dl_types_core::ArrowString),
...
impl quote::ToTokens for &ElementType {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        match self {
            ...
            ElementType::String => quote!(::dl_types_core::ArrowString),
...
fn quote_trait_impls_for_datatype_or_component(
    ...
    let quoted_impl_component = (obj.kind == ObjectKind::Component).then(|| {
        if let Some(forwarded_type) = forwarded_type.as_ref() {
            quote! {
                impl ::dl_types_core::WrapperComponent for #name {
            ...
            quote! {
                impl ::dl_types_core::Component for #name {
                    #[inline]
                    fn name() -> ComponentType {
                        #fqname.into()
                    }
    ...
    let quoted_impl_loggable = if forwarded_type.is_some() {
        ...
        let quoted_from_arrow = if optimize_for_buffer_slice {
            let from_arrow_body = {
                ...
                quote! {
                    ...
                    use ::dl_types_core::{arrow_zip_validity::ZipValidity, Loggable as _, ResultExt as _};
        ...
        let quoted_serializer = {
            ...
            quote! {
                    ...
                    Self: Clone + 'a
                {
                    ...
                    use arrow::{array::*, buffer::*, datatypes::*};
                    use ::dl_types_core::{Loggable as _, ResultExt as _, arrow_helpers::as_array_ref};
        ...
        quote! {
            impl ::dl_types_core::Loggable for #name {
    ...
    quote! {
        ...
        ::dl_types_core::macros::impl_into_cow!(#name);
...
fn quote_trait_impls_for_archetype(reporter: &Reporter, obj: &Object) -> TokenStream {
    ...
    let display_name = dl_case::to_human_case(name);
    let name = format_ident!("{name}");
    ...
    quote! {
        ...
        impl ::dl_types_core::Archetype for #name {
            #[inline]
        ...
        impl ::dl_types_core::AsComponents for #name {
            #as_components_impl
        }

        impl ::dl_types_core::ArchetypeReflectionMarker for #name { }
...
fn quote_trait_impls_for_view(reporter: &Reporter, obj: &Object) -> TokenStream {
    ...
    quote! {
        impl ::dl_types_core::View for #name {
            #[inline]
...
fn quote_builder_from_obj(reporter: &Reporter, objects: &Objects, obj: &Object) -> TokenStream {
    ...
    let columnar_methods = obj.scope().is_none().then(|| {
        ...
        [`SerializedComponentColumn`]: [::dl_types_core::SerializedComponentColumn]
        ");
        ...
        quote! {
            ...
            ) -> SerializationResult<impl Iterator<Item = ::dl_types_core::SerializedComponentColumn>>
            where
