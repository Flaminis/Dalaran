...
pub fn generate_reflection(
    ...
    let path = Utf8PathBuf::from("crates/store/dl_sdk_types/src/reflection/mod.rs");
...
fn generate_archetype_reflection(reporter: &Reporter, objects: &Objects) -> TokenStream {
        ...
        .filter(|obj| !obj.is_testing())
    {
        let quoted_field_reflections = obj.fields.iter().map(|field| {
            ...
            let name = &field.name;
            let display_name = dl_case::to_human_case(&field.name);
            let docstring_md = doc_as_lines(
                reporter,
                objects,
                &field.virtpath,
        ...
        let display_name = dl_case::to_human_case(&obj.name);
        if false {
...
/// Returns `crate_name` as is, unless it's `dl_sdk_types`, in which case it's replace by `crate`,
/// because that's where this code lives.
fn patched_crate_name(crate_name: &str) -> String {
    if crate_name == "dl_sdk_types" {
        "crate".to_owned()
    } else {
        crate_name.to_owned()
    }
