    ...
    if is_enum {
        ...
        let quoted_remapping = quote! {
            .map(|typ| {
                match typ {
                    Some(val) => {
                        <Self as ::dl_types_core::reflection::Enum>::try_from_integer(val).map(Some).ok_or_else(|| {
    ...
    match datatype.to_logical_type() {
        ...
        DataType::Binary => {
            ...
            quote! {{
                    ...
                    O: ::arrow::array::OffsetSizeTrait,
                {
                    ...
                    use ::dl_types_core::arrow_zip_validity::ZipValidity;
        ...
        DataType::Utf8 => {
            ...
            let quoted_iter_transparency = quote_iterator_transparency(
                ...
                quote!(::dl_types_core::ArrowString::from).into(),
            );
                ...
                if #data_src.is_empty() {
                    ...
                    ZipValidity::new_with_validity(offsets, #data_src.nulls())
                        .map(|elem| elem.map(|(start, end): (usize, usize)| {
                                ...
                                // must be correct.
                                dl_log::debug_assert!(end - start == #length);
    ...
    if is_arrow_transparent {
        ...
        quote! {{
...

            {
                ...
                // dl_tracing::profile_scope!("collect");
