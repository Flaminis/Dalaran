...
use dl_types_builder::{SourceLocations, compute_re_types_hash};

const RE_TYPES_SOURCE_HASH_PATH: &str = "crates/store/dl_sdk_types/source_hash.txt";
...
fn main() {
    ...
    #[cfg(feature = "tracing")]
    let mut profiler = dl_tracing::Profiler::default(); // must be started early and dropped late to catch everything
    ...
    dl_log::debug!("cur_hash: {cur_hash:?}");
    ...
    if let Some(cur_hash) = cur_hash {
        if cur_hash == new_hash {
            if always_run {
                ...
                dl_log::info!("Returning early: no changes detected (and --force wasn't set).");
                return;
            }
        } else {
            dl_log::info!("Change detected");
        }
    } else {
        dl_log::info!("Missing {re_types_source_hash_path:?} (first time running codegen)");
    }

    dl_log::info!("Running codegen…");
    let (report, reporter) = dl_types_builder::report::init();

    dl_log::info!("Generating flatbuffers code…");
    dl_types_builder::generate_fbs(&reporter, &definitions_dir_path, check);

    let (objects, type_registry) =
        dl_types_builder::generate_lang_agnostic(&reporter, definitions_dir_path, entrypoint_path);

    dl_tracing::profile_scope!("Language-specific code-gen");
    join!(
        || dl_types_builder::generate_cpp_code(
            &reporter,
            cpp_output_dir_path,
            &objects,
            &type_registry,
        ...
        || dl_types_builder::generate_rust_code(
            &reporter,
            workspace_dir,
            &objects,
            &type_registry,
        ...
        || dl_types_builder::generate_python_code(
            &reporter,
        ...
        || dl_types_builder::generate_docs(
            &reporter,
        ...
        || dl_types_builder::generate_snippets_ref(
            &reporter,
