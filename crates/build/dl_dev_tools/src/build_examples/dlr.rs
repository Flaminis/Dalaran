...
impl Dlr {
    pub fn run(self) -> anyhow::Result<()> {
        ...
        let workspace_root = dl_build_tools::cargo_metadata()?.workspace_root;
        ...
        let results: Vec<anyhow::Result<PathBuf>> = examples
            .into_par_iter()
            .map(|example| example.build_dlr(&progress, &self.output_dir))
            .collect();
...
impl Example {
    fn build_dlr(self, progress: &MultiProgress, output_dir: &Path) -> anyhow::Result<PathBuf> {
        let tempdir = tempfile::tempdir()?;

        let initial_dlr_path = tempdir.path().join(&self.name).with_extension("dlr");

        {
            let mut cmd = Command::new("python3");
            cmd.arg("-m").arg(&self.name);
            cmd.arg("--save").arg(&initial_dlr_path);
        ...
        let final_dlr_path = output_dir.join(&self.name).with_extension("dlr");
        ...
        cmd.arg(&initial_dlr_path);
        cmd.arg("-o").arg(&final_dlr_path);
