...
int main() {
    ...
    std::vector<dalaran::Position3D> points1, points2;
    std::vector<dalaran::Color> colors1, colors2;
    ...
    rec.log_static(
        "dna/structure/left",
        dalaran::Points3D(points1).with_colors(colors1).with_radii({0.08f})
    );
    rec.log_static(
        "dna/structure/right",
        dalaran::Points3D(points2).with_colors(colors2).with_radii({0.08f})
    );

    std::vector<dalaran::LineStrip3D> lines;
    for (size_t i = 0; i < points1.size(); ++i) {
        lines.emplace_back(dalaran::LineStrip3D({points1[i].xyz, points2[i].xyz}));
    }

    rec.log_static(
        "dna/structure/scaffolding",
        dalaran::LineStrips3D(lines).with_colors(dalaran::Color(128, 128, 128))
    );
    ...
    std::vector<dalaran::Position3D> beads_positions(lines.size());
    std::vector<dalaran::Color> beads_colors(lines.size());
    ...
    for (int t = 0; t < 400; t++) {
        ...
        for (size_t i = 0; i < lines.size(); ++i) {
            ...
            beads_positions[i] = dalaran::Position3D(
            ...
            beads_colors[i] = dalaran::Color(c, c, c);
        }

        rec.log(
            "dna/structure/scaffolding/beads",
            dalaran::Points3D(beads_positions).with_colors(beads_colors).with_radii({0.06f})
        );

        rec.log(
            "dna/structure",
            dalaran::archetypes::Transform3D(dalaran::RotationAxisAngle(
                {0.0f, 0.0f, 1.0f},
                dalaran::Angle::radians(time.count() / 4.0f * TAU)
            ))
        );
    }
}
