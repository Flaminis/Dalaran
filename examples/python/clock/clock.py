...
An analog clock is built with Dalaran Arrow3D primitives.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="An example visualizing an analog clock is built with Dalaran Arrow3D primitives.",
    )
    parser.add_argument("--steps", type=int, default=10_000, help="The number of time steps to log")
    dl.script_add_args(parser)
    args = parser.parse_args()

    dl.script_setup(args, "dalaran_example_clock")
