...
It's important to note that in this example, only the LiDAR data is visualized. For a more extensive example including other sensors and annotations check out the [nuScenes example](https://www.dalaran.dev/examples/robotics/nuscenes_dataset).
...
```python
dl.set_time("timestamp", timestamp=sample_data["timestamp"] * 1e-6)  # Setting the time
dl.log("world/lidar", dl.Points3D(points, colors=point_colors))  # Log the 3D data
```

When logging data to Dalaran, it's possible to associate it with specific time by using the Dalaran's [`timelines`](https://www.dalaran.dev/docs/concepts/logging-and-ingestion/timelines).
