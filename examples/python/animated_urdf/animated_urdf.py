...
def run_single(args: argparse.Namespace, urdf_path: Path) -> None:
    duration = 0.0
    rec = dl.script_setup(args, "dalaran_example_animated_urdf")
    ...
    urdf_tree = dl.urdf.UrdfTree.from_file_path(urdf_path)

    # Hide the collision geometries by default in the viewer.
    blueprint = dlb.Grid(
        dlb.Spatial3DView(
            name="Animated URDF", overrides={"so_arm100/collision_geometries": dlb.EntityBehavior(visible=False)}
        )
    )
            ...
            if joint.joint_type == "revolute":
                ...
                if link.name == "jaw":
                    for visual_path in urdf_tree.get_visual_geometry_paths(link):
                        ...
                        rec.log(visual_path, dl.Asset3D.from_fields(albedo_factor=rgba))
...
def run_dual(args: argparse.Namespace, urdf_path: Path) -> None:
    ...
    left = dl.urdf.UrdfTree.from_file_path(urdf_path, entity_path_prefix="left", frame_prefix="left/")
    right = dl.urdf.UrdfTree.from_file_path(urdf_path, entity_path_prefix="right", frame_prefix="right/")
    ...
    rec.log("left", dl.Transform3D(translation=[-0.2, 0, 0]), static=True)
    rec.log("right", dl.Transform3D(translation=[0.2, 0, 0]), static=True)

    blueprint = dlb.Grid(
        dlb.Spatial3DView(
            name="Dual Arm",
            overrides={
                "left/so_arm100/collision_geometries": dlb.EntityBehavior(visible=False),
                "right/so_arm100/collision_geometries": dlb.EntityBehavior(visible=False),
            },
        )
    )
    rec.send_blueprint(blueprint)
...
def _fake_angle(joint: dl.urdf.UrdfJoint, step: int, joint_index: int, phase: float) -> float:
