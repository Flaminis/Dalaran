...
## Used Dalaran types
[`Image`](https://www.dalaran.dev/docs/reference/types/archetypes/image), [`Points2D`](https://www.dalaran.dev/docs/reference/types/archetypes/points2d), [`Points3D`](https://www.dalaran.dev/docs/reference/types/archetypes/points3d), [`LineStrips2D`](https://www.dalaran.dev/docs/reference/types/archetypes/line_strips2d), [`ClassDescription`](https://www.dalaran.dev/docs/reference/types/datatypes/class_description), [`AnnotationContext`](https://www.dalaran.dev/docs/reference/types/archetypes/annotation_context), [`TextDocument`](https://www.dalaran.dev/docs/reference/types/archetypes/text_document)
...
### Video
The input video is logged as a sequence of [`Image`](https://www.dalaran.dev/docs/reference/types/archetypes/image) objects to the `Media/Video` entity.
```python
dl.log("Media/Video", dl.Image(frame).compress(jpeg_quality=75))
```

### Hand landmark points
Logging the hand landmarks involves specifying connections between the points, extracting pose landmark points and logging them to the Dalaran SDK.
...
As for the 3D points, the logging process involves two steps. First, a static [`ClassDescription`](https://www.dalaran.dev/docs/reference/types/datatypes/class_description) is logged, that contains the information which maps keypoint ids to labels and how to connect
...
Second, the actual keypoint positions are logged in 3D [`Points3D`](https://www.dalaran.dev/docs/reference/types/archetypes/points3d) archetype.
...
for log_key in ["Media/Points", "Hand/Points"]:
    dl.log(log_key, dl.Points2D(points, radii=10, colors=[255, 0, 0]))

# Log connections to the image and Hand entity [128, 128, 128]
for log_key in ["Media/Connections", "Hand/Connections"]:
    dl.log(log_key, dl.LineStrips2D(np.stack((points1, points2), axis=1), colors=[255, 165, 0]))
```
