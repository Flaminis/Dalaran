...
to the Viewer using the [`VideoStream`](https://www.dalaran.dev/docs/reference/types/archetypes/video_stream) archetype
...
To learn more about video support in general check [the docs page on the topic](https://dalaran.dev/docs/concepts/logging-and-ingestion/video).

The example first sets up a video stream on an entity called `"video_stream"` with the H264 codec.
```py
dl.log("video_stream", dl.VideoStream(codec=dl.VideoCodec.H264), static=True)
```
...
⚠️ Latency is expected to be well below a second, but as of writing Dalaran is not well
...
configure Dalaran's [Micro Batching](https://dalaran.dev/docs/reference/sdk/micro-batching).
...
Note that unlike with unlike with [`VideoAsset`](https://www.dalaran.dev/docs/reference/types/archetypes/asset_video),
there's no need to log [`VideoFrameReference`](https://www.dalaran.dev/docs/reference/types/archetypes/video_frame_reference),
to map the video's PTS to the Dalaran timeline, since the time at which video samples
...
```py
dl.log("video_stream", dl.VideoStream(codec=dl.VideoCodec.H264, sample=bytes(packet)))
```
