...
For most examples you can instead save the log data to an `.rrd` file using `plots --save data.rrd`. You can then view that `.rrd` file with `dalaran data.rrd`.

(`dalaran` is an alias for `python -m dalaran`).

NOTE: `.rrd` files do not yet guarantee any backwards or forwards compatibility. One version of Dalaran will likely not be able to open an `.rrd` file generated by another Dalaran version.
