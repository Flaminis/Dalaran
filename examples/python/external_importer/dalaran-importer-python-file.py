#!/usr/bin/env python3
"""Example of an executable importer plugin for the Dalaran Viewer."""
...
To try it out, copy it in your $PATH as `dalaran-importer-python-file`, then open a Python source
file with Dalaran (`dalaran file.py`).
""",
)
...
def main() -> None:
    ...
    if not is_file or not is_python_file:
        sys.exit(dl.EXTERNAL_IMPORTER_INCOMPATIBLE_EXIT_CODE)

    app_id = "dalaran_example_external_importer"
    if args.application_id is not None:
        app_id = args.application_id
    dl.init(app_id, recording_id=args.recording_id)
    ...
    with open(args.filepath, encoding="utf8") as file:
        ...
        dl.log(entity_path, dl.TextDocument(text, media_type=dl.MediaType.MARKDOWN), static=args.static)
...
def set_time_from_args() -> None:
    if not args.static and args.time is not None:
        for time_str in args.time_sequence:
            ...
            dl.set_time(timeline_name, sequence=int(sequence))
        ...
        for time_str in args.time_duration_nanos:
            ...
            dl.set_time(timeline_name, duration=1e-9 * int(nanos))
        ...
        for time_str in args.time_timestamp_nanos:
            ...
            if len(parts) != 2:
                continue
            timeline_name, nanos = parts
            dl.set_time(timeline_name, timestamp=1e-9 * int(nanos))
