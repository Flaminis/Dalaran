...
On Linux & Mac you can simply copy it in your $PATH as `dalaran-importer-python-file.py`, then open a Python source file with Dalaran (`dalaran file.py`).
...
One way to do this is to use `pyinstaller`: `pyinstaller .\examples\python\external_importer\dalaran-importer-python-file.py -n dalaran-importer-python-file --onefile`

Consider using the [`send_columns`](https://ref.dalaran.dev/docs/python/stable/common/columnar_api/#dalaran.send_columns) API for importers that ingest time series data from a file.
