//! Builds an analog clock using Dalaran's `Vector3D` primitive.
//!
...
fn main() -> anyhow::Result<()> {
    ...
    let (rec, _serve_guard) = args.dalaran.init("dalaran_example_clock")?;
    run(&rec, &args)
}
...
fn run(rec: &dalaran::RecordingStream, args: &Args) -> anyhow::Result<()> {
    ...
    rec.log_static(
        "world/frame",
        &dalaran::Boxes3D::from_half_sizes([(LENGTH_S, LENGTH_S, 1.0)]),
    )?;
    ...
    fn color(angle: f32, blue: u8) -> dalaran::Color {
        let c = (angle * 255.0) as u8;
        dalaran::Color::from_unmultiplied_rgba(255 - c, c, blue, u8::max(128, blue))
    }
    ...
    fn log_hand(
        ...
        rec.log(
            ...
            &dalaran::Points3D::new([pos]).with_colors([color]),
        )?;
        rec.log(
            format!("world/{name}_hand"),
            &dalaran::Arrows3D::from_vectors([pos])
