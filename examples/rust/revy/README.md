<!--[metadata]
title = "Revy - Dalaran integration for Bevy"
description = "Revy is a time-travel debugger for Bevy: each frame's database is snapshot into Dalaran for scrubbable engine state."
...
<table>
  ...
  <tbody>
    <tr>
      <td>
        <a href="https://app.dalaran.dev/version/0.14.1/index.html?url=https://storage.googleapis.com/rerun-example-datasets/revy/breakout_014_001.rrd">Live demo</a>
        <picture>
      ...
      <td>
        <a href="https://app.dalaran.dev/version/0.14.1/index.html?url=https://storage.googleapis.com/rerun-example-datasets/revy/3d_shapes_014_001.rrd">Live demo</a>
        <picture>
      ...
      <td>
        <a href="https://app.dalaran.dev/version/0.14.1/index.html?url=https://storage.googleapis.com/rerun-example-datasets/revy/alien_014_001.rrd">Live demo</a>
        <picture>
...
1. [Install the Dalaran Viewer](https://www.dalaran.dev/docs/overview/installing-dalaran/viewer) (`0.15`).

2. Add `revy` to your dependencies:
    ```toml
    revy = "0.15"  # always matches the Dalaran version
    ```

3. Initialize the `dalaran` plugin:
    ```rust
    .add_plugins({
        let rec = revy::RecordingStreamBuilder::new("<your_app_name>").spawn().unwrap();
        revy::DalaranPlugin { rec }
    })
    ```
    This will start a Dalaran Viewer in the background and stream the recording data to it.\
    Check out the [`RecordingStreamBuilder`](https://docs.rs/dalaran/latest/dalaran/struct.RecordingStreamBuilder.html) docs for other options (saving to file, connecting to a remote viewer, etc).
