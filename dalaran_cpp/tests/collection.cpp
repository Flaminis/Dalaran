...
#include <dalaran/components/position2d.hpp>
...
void check_for_expected_list(const dalaran::Collection<Element>& collection) {
...
void check_for_expected_single(const dalaran::Collection<Element>& collection) {
...
SCENARIO("Default constructing a collection", TEST_TAG) {
    GIVEN("a default constructed collection") {
        dalaran::Collection<Element> collection;
        ...
        THEN("it is borrowed") {
            CHECK(collection.get_ownership() == dalaran::CollectionOwnership::Borrowed);
        }
    }
}
...
SCENARIO(
    ...
    GIVEN("a vector of elements") {
        ...
        THEN("a collection created from it borrows its data") {
            ...
            const dalaran::Collection<Element> collection(elements);
        ...
        THEN("a collection created from moving it owns the data") {
            ...
            const dalaran::Collection<Element> collection(std::move(elements));
            check_for_expected_list(collection);
            CHECK(collection.get_ownership() == dalaran::CollectionOwnership::VectorOwned);
        }
    }
    ...
    GIVEN("a temporary vector of elements") {
        THEN("a collection created from it owns its data") {
            ...
            const dalaran::Collection<Element> collection(std::vector<Element> EXPECTED_ELEMENT_LIST);
            check_for_expected_list(collection);
            CHECK(collection.get_ownership() == dalaran::CollectionOwnership::VectorOwned);
        }
    }
    ...
    GIVEN("a temporary std::array of elements") {
        THEN("a collection created from it owns its data") {
            ...
            const dalaran::Collection<Element> collection(std::array<Element, 2> EXPECTED_ELEMENT_LIST
            );
            check_for_expected_list(collection);
            CHECK(collection.get_ownership() == dalaran::CollectionOwnership::VectorOwned);
        }
    ...
    GIVEN("a single element") {
        ...
        THEN("a collection created from it borrows its data") {
            ...
            const dalaran::Collection<Element> collection(component);
        ...
        THEN("a collection created from moving it owns the data") {
            ...
            const dalaran::Collection<Element> collection(std::move(component));
            check_for_expected_single(collection);
            CHECK(collection.get_ownership() == dalaran::CollectionOwnership::VectorOwned);
        }
    }
    GIVEN("a single temporary component") {
        THEN("a collection created from it owns the data") {
            ...
            const dalaran::Collection<Element> collection(Element(EXPECTED_SINGLE));
            check_for_expected_single(collection);
            CHECK(collection.get_ownership() == dalaran::CollectionOwnership::VectorOwned);
        }
    }
...
SCENARIO(
    ...
    GIVEN("a temporary vector of convertible elements") {
        THEN("a collection created from it moves its data") {
            ...
            const dalaran::Collection<Element>
    ...
    GIVEN("a single convertible element") {
        ...
        THEN("a collection created from it copies its data") {
            ...
            const dalaran::Collection<Element> collection(element);
            check_for_expected_single(collection);
            CHECK(collection.get_ownership() == dalaran::CollectionOwnership::VectorOwned);
        }
        THEN("a collection created from it move its data") {
            ...
            const dalaran::Collection<Element> collection(std::move(element));
            check_for_expected_single(collection);
            CHECK(collection.get_ownership() == dalaran::CollectionOwnership::VectorOwned);
        }
    }
    GIVEN("a single temporary convertible element") {
        THEN("a collection created from it move its data") {
            ...
            const dalaran::Collection<Element> collection(ConvertibleElement(EXPECTED_SINGLE));
            check_for_expected_single(collection);
            CHECK(collection.get_ownership() == dalaran::CollectionOwnership::VectorOwned);
        }
    }
...
SCENARIO("Collection creation via a custom adapter for a datalayout compatible type", TEST_TAG) {
    GIVEN("A custom vec2 container with a defined adapter") {
        ...
        THEN("a collection created from it that its data") {
            const dalaran::Collection<Position2D> batch(container);
            CHECK(batch.size() == 2);
            CHECK(batch.get_ownership() == dalaran::CollectionOwnership::Borrowed);
        }
        THEN("A Point2D archetype can be directly created from this container") {
            const dalaran::archetypes::Points2D from_custom_container(container);

            CHECK(from_custom_container.positions.has_value());

            AND_THEN("it can be serialized and is identical to creation from dalaran types directly"
            ) {
                const dalaran::archetypes::Points2D from_dalaran_vector({{0.0f, 1.0f}, {2.0f, 3.0f}});

                test_compare_archetype_serialization(from_custom_container, from_dalaran_vector);
            }
        }
    }
}
...
SCENARIO("Move construction/assignment of collections", TEST_TAG) {
    ...
    GIVEN("A borrowed collection") {
        auto borrowed = dalaran::Collection<Position2D>::borrow(components.data(), 2);

        THEN("then moving to a new batch moves the data and clears the source") {
            auto target(std::move(borrowed));
            CHECK(target.size() == 2);
            CHECK(target.get_ownership() == dalaran::CollectionOwnership::Borrowed);
            CHECK(borrowed.size() == 0);
            CHECK(borrowed.empty());

            CHECK(borrowed.get_ownership() == dalaran::CollectionOwnership::Borrowed);
        }

        THEN("moving it to an owned collection swaps their data") {
            auto target = dalaran::Collection<Position2D>::take_ownership(std::vector(components));

            target = std::move(borrowed);
            CHECK(target.size() == 2);
            CHECK(target.get_ownership() == dalaran::CollectionOwnership::Borrowed);
            CHECK(borrowed.size() == 2);
            CHECK(borrowed.get_ownership() == dalaran::CollectionOwnership::VectorOwned);
        }
        THEN("moving it to an borrowed collection swaps their data") {
            auto target = dalaran::Collection<Position2D>::borrow(components.data(), 2);
    ...
    GIVEN("A owned collection") {
        auto borrowed = dalaran::Collection<Position2D>::take_ownership(std::vector(components));
        ...
        THEN("moving it to an owned collection swaps their data") {
            ...
            target = std::move(borrowed);
            CHECK(target.size() == 2);
            CHECK(target.get_ownership() == dalaran::CollectionOwnership::VectorOwned);
            CHECK(borrowed.size() == 2);
            CHECK(borrowed.get_ownership() == dalaran::CollectionOwnership::VectorOwned);
        }
        THEN("moving it to an borrowed collection swaps their data") {
...
SCENARIO("Copy/move construction/assignment of collections", TEST_TAG) {
    GIVEN("A default constructed collection") {
        ...
        THEN("it can be move assigned") {
            dalaran::Collection<int> collection2;
        ...
        THEN("it can be copy constructed") {
            dalaran::Collection<int> collection2(collection);
            CHECK(collection2.size() == 0);
            CHECK(collection2.empty());
        }
    ...
    GIVEN("a collection with owned data") {
        auto collection =
            dalaran::Collection<Element>::take_ownership(std::vector<Element> EXPECTED_ELEMENT_LIST);
        const Element* old_data_ptr = collection.data();

        THEN("it can be move constructed") {
            CheckElementMoveAndCopyCount check; // No move or copy.

            dalaran::Collection<Element> collection2(std::move(collection));
        ...
        THEN("it can be move assigned") {
            ...
            dalaran::Collection<Element> collection2;
            collection2 = std::move(collection);
            check_for_expected_list(collection2);
            CHECK(collection2.data() == old_data_ptr);
        }
        ...
        THEN("it can be copy constructed") {
            ...
            dalaran::Collection<Element> collection2(collection);
            check_for_expected_list(collection2);
        }
        THEN("it can be copy assigned") {
            CheckElementMoveAndCopyCount check;
    ...
    GIVEN("a collection with borrowed data") {
        ...
        auto collection = dalaran::Collection<Element>::borrow(data.data(), data.size());
...
SCENARIO("Borrow and take ownership if easy with the free utility functions") {
    GIVEN("A vector") {
        ...
        THEN("it can be borrowed without via `dalaran::borrow` without specifying template arguments"
        ) {
            CheckElementMoveAndCopyCount check; // No element copies or moves expected.

            const auto collection = dalaran::borrow(data);
            check_for_expected_list(collection);
            CHECK(collection.get_ownership() == dalaran::CollectionOwnership::Borrowed);
        }
        THEN(
            "it can be taken ownership via `dalaran::take_ownership` without specifying template arguments"
        ) {
            CheckElementMoveAndCopyCount check; // No element copies or moves expected.

            const auto collection = dalaran::take_ownership(std::move(data));
            check_for_expected_list(collection);
            CHECK(collection.get_ownership() == dalaran::CollectionOwnership::VectorOwned);
        }
    }
    GIVEN("A pointer to an array") {
        ...
        THEN("it can be borrowed via `dalaran::borrow` without specifying template arguments") {
            CheckElementMoveAndCopyCount check; // No element copies or moves expected.

            const auto collection = dalaran::borrow(data.data(), data.size());
            check_for_expected_list(collection);
            CHECK(collection.get_ownership() == dalaran::CollectionOwnership::Borrowed);
        }
    }
    GIVEN("A single element") {
        ...
        THEN(
            ...
            WHEN("passed by value") {
                ...
                check.expect_move(1); // move to dalaran::Collection

                const auto collection = dalaran::take_ownership(data);
                check_for_expected_single(collection);
                CHECK(collection.get_ownership() == dalaran::CollectionOwnership::VectorOwned);
            }
            WHEN("passed moved") {
                CheckElementMoveAndCopyCount check;
                check.expect_move(2); // move on call, move to dalaran::Collection
