...
#include <dalaran/as_components.hpp>
#include <dalaran/collection.hpp>
#include <dalaran/component_batch.hpp>

template <typename T>
void test_compare_archetype_serialization(const T& arch_a, const T& arch_b) {
    THEN("convert to component lists") {
        auto arch_b_serialized_result = dalaran::AsComponents<T>::as_batches(arch_b);
        auto arch_a_serialized_result = dalaran::AsComponents<T>::as_batches(arch_a);
