...
namespace dalaran {
    const std::shared_ptr<arrow::DataType>& Loggable<components::AffixFuzzer18>::arrow_datatype() {
        static const auto datatype = arrow::list(
            arrow::field("item", Loggable<dalaran::datatypes::NestedUnion>::arrow_datatype(), true)
        );
        return datatype;
    }
    ...
    Result<std::shared_ptr<arrow::Array>> Loggable<components::AffixFuzzer18>::to_arrow(
        ...
        if (instances && num_instances > 0) {
            DL_RETURN_NOT_OK(Loggable<components::AffixFuzzer18>::fill_arrow_array_builder(
    ...
    dalaran::Error Loggable<components::AffixFuzzer18>::fill_arrow_array_builder(
        ...
        for (size_t elem_idx = 0; elem_idx < num_elements; elem_idx += 1) {
            ...
            if (element.many_optional_unions.has_value()) {
                ...
                if (element.many_optional_unions.value().data()) {
                    DL_RETURN_NOT_OK(
                        Loggable<dalaran::datatypes::NestedUnion>::fill_arrow_array_builder(
                            value_builder,
