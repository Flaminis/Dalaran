...
static std::vector<dalaran::Position3D> create_positions() {
    return {};
}

static std::vector<dalaran::Color> create_colors() {
    return {};
}
...
[[maybe_unused]] static void log() {
    ...
    std::vector<dalaran::Position3D> points = create_positions();
    std::vector<dalaran::Color> colors = create_colors();
    std::vector<uint8_t> image_data = create_image();

    // Log a batch of points.
    rec.log("path/to/points", dalaran::Points3D(points).with_colors(colors));

    // Log an image.
    rec.log("path/to/image", dalaran::Image::from_rgba32(image_data, {786, 1024}));
    /// [Logging]
}
