    ...
    struct ComponentBatch {
      ...
      public:
        ...
        static Result<ComponentBatch> from_loggable(
            const dalaran::Collection<T>& components, const ComponentDescriptor& descriptor
        ) {
            static_assert(
                dalaran::is_loggable<T>,
                "The given type does not implement the dalaran::Loggable trait."
            );
        ...
        template <typename T>
        static Result<ComponentBatch> from_loggable(
            const std::optional<dalaran::Collection<T>>& components,
