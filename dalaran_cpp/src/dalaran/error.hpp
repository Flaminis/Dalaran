...
/// Return error if a given dalaran::Error producing expression is not dalaran::ErrorCode::Ok.
#define DL_RETURN_NOT_OK(status_expr)              \
    do {                                           \
        const dalaran::Error _status_ = status_expr; \
        if (_status_.is_err()) {                   \
            return _status_;                       \
        }                                          \
    } while (false)
...
namespace dalaran {
    ...
    class [[nodiscard]] Error {
      ...
      public:
        ...
        ///
        /// The default will log to stderr, unless `DALARAN_STRICT` is set to something truthy.
        ///
        ...
        /// Else if the `DALARAN_STRICT` env-var is set to something truthy,
