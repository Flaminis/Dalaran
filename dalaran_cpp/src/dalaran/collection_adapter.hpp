...
namespace dalaran {
    /// The `dalaran::CollectionAdapter` trait is responsible for mapping an input argument to a `dalaran::Collection`.
    ///
    /// There are default implementations for standard containers, as well as single
    /// elements. These can be found in `dalaran/collection_adapter_builtins.hpp`.
    ///
    ...
    /// Dalaran types which then can be logged.
    ///
    ...
    struct CollectionAdapter {
        ...
        static_assert(
            ...
            "dalaran::CollectionAdapter<TElement, TContainer> for a given "
