...
namespace dalaran::archetypes {
    ...
    struct Asset3D {
      ...
      public:
        ...
        static constexpr const char ArchetypeName[] = "dalaran.archetypes.Asset3D";

        /// `ComponentDescriptor` for the `blob` field.
        static constexpr auto Descriptor_blob = ComponentDescriptor(
            ArchetypeName, "Asset3D:blob", Loggable<dalaran::components::Blob>::ComponentType
        );
      ...
      public:
        ...
        explicit Asset3D(dalaran::components::Blob _blob)
        ...
        /// The asset's bytes.
        Asset3D with_blob(const dalaran::components::Blob& _blob) && {
        ...
        Asset3D with_many_blob(const Collection<dalaran::components::Blob>& _blob) && {
            blob = ComponentBatch::from_loggable(_blob, Descriptor_blob).value_or_throw();
            return std::move(*this);
        }
        ...
        /// If omitted, the viewer will try to guess from the data blob.
        /// If it cannot guess, it won't be able to render the asset.
        Asset3D with_media_type(const dalaran::components::MediaType& _media_type) && {
            media_type =
        ...
        /// be used when logging a single row's worth of data.
        Asset3D with_many_media_type(const Collection<dalaran::components::MediaType>& _media_type
        ) && {
            media_type =
                ComponentBatch::from_loggable(_media_type, Descriptor_media_type).value_or_throw();
            return std::move(*this);
        ...
        /// For mesh who already have `albedo_factor` in materials,
        /// it will be overwritten by actual `albedo_factor` of `archetypes::Asset3D` (if specified).
        Asset3D with_albedo_factor(const dalaran::components::AlbedoFactor& _albedo_factor) && {
