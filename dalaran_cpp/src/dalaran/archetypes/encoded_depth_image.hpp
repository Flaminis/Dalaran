...
// Based on "crates/store/dl_sdk_types/definitions/dalaran/archetypes/encoded_depth_image.fbs".
...
namespace dalaran::archetypes {
    ...
    struct EncodedDepthImage {
      ...
      public:
        ...
        static constexpr const char ArchetypeName[] = "dalaran.archetypes.EncodedDepthImage";

        /// `ComponentDescriptor` for the `blob` field.
        static constexpr auto Descriptor_blob = ComponentDescriptor(
            ArchetypeName, "EncodedDepthImage:blob",
            Loggable<dalaran::components::Blob>::ComponentType
        );
      ...
      public:
        ...
        explicit EncodedDepthImage(dalaran::components::Blob _blob)
        ...
        /// * single channel PNG
        /// * RVL with ROS2 metadata (for details see <https://github.com/ros-perception/image_transport_plugins/tree/jazzy>)
        EncodedDepthImage with_blob(const dalaran::components::Blob& _blob) && {
        ...
        EncodedDepthImage with_many_blob(const Collection<dalaran::components::Blob>& _blob) && {
            blob = ComponentBatch::from_loggable(_blob, Descriptor_blob).value_or_throw();
            return std::move(*this);
        }
        ...
        ///  * `application/rvl` (RVL-compressed 16-bit)
        ///  * `image/png`
        EncodedDepthImage with_media_type(const dalaran::components::MediaType& _media_type) && {
            media_type =
        ...
        ///
        /// If omitted, the Viewer defaults to `1.0` for floating-point depth formats and `1000.0` for integer formats (millimeters).
        EncodedDepthImage with_meter(const dalaran::components::DepthMeter& _meter) && {
        ...
        /// be used when logging a single row's worth of data.
        EncodedDepthImage with_many_meter(const Collection<dalaran::components::DepthMeter>& _meter
        ) && {
            meter = ComponentBatch::from_loggable(_meter, Descriptor_meter).value_or_throw();
            return std::move(*this);
        }
        ...
        /// Optional colormap for visualization of decoded depth.
        EncodedDepthImage with_colormap(const dalaran::components::Colormap& _colormap) && {
            colormap =
        ...
        EncodedDepthImage with_many_colormap(
            const Collection<dalaran::components::Colormap>& _colormap
        ) && {
            colormap =
                ComponentBatch::from_loggable(_colormap, Descriptor_colormap).value_or_throw();
            return std::move(*this);
        ...
        /// Optional visualization range for depth values.
        EncodedDepthImage with_depth_range(const dalaran::components::ValueRange& _depth_range) && {
        ...
        EncodedDepthImage with_point_fill_ratio(
            const dalaran::components::FillRatio& _point_fill_ratio
        ) && {
            point_fill_ratio =
        ...
        /// Optional 2D draw order.
        EncodedDepthImage with_draw_order(const dalaran::components::DrawOrder& _draw_order) && {
            draw_order =
