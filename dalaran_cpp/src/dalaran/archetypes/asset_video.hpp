...
// Based on "crates/store/dl_sdk_types/definitions/dalaran/archetypes/asset_video.fbs".
...
namespace dalaran::archetypes {
    ...
    struct AssetVideo {
      ...
      public:
        ...
        static constexpr const char ArchetypeName[] = "dalaran.archetypes.AssetVideo";

        /// `ComponentDescriptor` for the `blob` field.
        static constexpr auto Descriptor_blob = ComponentDescriptor(
            ArchetypeName, "AssetVideo:blob", Loggable<dalaran::components::Blob>::ComponentType
        );
      ...
      public:
        ...
        explicit AssetVideo(dalaran::components::Blob _blob)
        ...
        /// The asset's bytes.
        AssetVideo with_blob(const dalaran::components::Blob& _blob) && {
        ...
        AssetVideo with_many_blob(const Collection<dalaran::components::Blob>& _blob) && {
            blob = ComponentBatch::from_loggable(_blob, Descriptor_blob).value_or_throw();
            return std::move(*this);
        }
        ...
        /// If omitted, the viewer will try to guess from the data blob.
        /// If it cannot guess, it won't be able to render the asset.
        AssetVideo with_media_type(const dalaran::components::MediaType& _media_type) && {
            media_type =
        ...
        /// be used when logging a single row's worth of data.
        AssetVideo with_many_media_type(const Collection<dalaran::components::MediaType>& _media_type
        ) && {
            media_type =
                ComponentBatch::from_loggable(_media_type, Descriptor_media_type).value_or_throw();
            return std::move(*this);
