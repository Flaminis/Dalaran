...
namespace dalaran::archetypes {
    ...
    struct Tensor {
      ...
      public:
        ...
        static constexpr const char ArchetypeName[] = "dalaran.archetypes.Tensor";

        /// `ComponentDescriptor` for the `data` field.
        static constexpr auto Descriptor_data = ComponentDescriptor(
            ArchetypeName, "Tensor:data", Loggable<dalaran::components::TensorData>::ComponentType
        );
      ...
      public:
        ...
        explicit Tensor(dalaran::components::TensorData _data)
        ...
        /// The tensor data
        Tensor with_data(const dalaran::components::TensorData& _data) && {
        ...
        Tensor with_many_data(const Collection<dalaran::components::TensorData>& _data) && {
            data = ComponentBatch::from_loggable(_data, Descriptor_data).value_or_throw();
            return std::move(*this);
        }
        ...
        /// E.g. if all values are positive, some bigger than 1.0 and all smaller than 255.0,
        /// the Viewer will guess that the data likely came from an 8bit image, thus assuming a range of 0-255.
        Tensor with_value_range(const dalaran::components::ValueRange& _value_range) && {
        ...
        /// be used when logging a single row's worth of data.
        Tensor with_many_value_range(const Collection<dalaran::components::ValueRange>& _value_range
        ) && {
            value_range = ComponentBatch::from_loggable(_value_range, Descriptor_value_range)
                              .value_or_throw();
            return std::move(*this);
