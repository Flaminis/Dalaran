...
namespace dalaran::archetypes {
    ...
    ///
    /// If no `MediaType` is specified, the Dalaran Viewer will try to guess one from the data
    /// at render-time. If it can't, rendering will fail with an error.
    static EncodedImage from_bytes(
        dalaran::Collection<uint8_t> image_contents,
        std::optional<dalaran::components::MediaType> media_type = {}
    ) {
    ...
    Result<EncodedImage> EncodedImage::from_file(const std::filesystem::path& filepath) {
        ...
        return EncodedImage::from_bytes(
            file_bytes,
            dalaran::components::MediaType::guess_from_path(filepath)
        );
    }
} // namespace dalaran::archetypes
