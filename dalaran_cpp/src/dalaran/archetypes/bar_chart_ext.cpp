...
namespace dalaran::archetypes {
    ...
    BarChart(dalaran::datatypes::TensorBuffer buffer) {
        // Forwarding like this can spuriously fail, since the move might be evaluated before `num_elems`:
        //BarChart(dalaran::components::TensorData({buffer.num_elems()}, std::move(buffer)));

        auto num_elems = buffer.num_elems();
        *this = std::move(*this).with_values(
            dalaran::components::TensorData({num_elems}, std::move(buffer))
        );
    }
    ...
    static BarChart f16(Collection<dalaran::half> f16) {
