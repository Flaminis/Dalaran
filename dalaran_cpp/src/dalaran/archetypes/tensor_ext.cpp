...
DL_DISABLE_MAYBE_UNINITIALIZED_POP
    ...
    ///
    /// Type must be one of the types supported by `dalaran::datatypes::TensorData`.
    /// \param shape
    ...
    Result<std::shared_ptr<arrow::Array>> tensor_data_with_dim_names(
        const std::optional<dalaran::ComponentBatch>& data, Collection<std::string> names
    ) {
        if (!data.has_value()) {
            return Error(
        ...
        auto datatype = dalaran::Loggable<dalaran::datatypes::TensorData>::arrow_datatype();
        ...
        // wrap in `ARROW_RETURN_NOT_OK` instead of returning directly to do the conversion to dalaran::Result.
        ...
        return dalaran::Result(std::static_pointer_cast<arrow::Array>(result));
    }
