...
namespace dalaran {
    ...
    ///
    /// The most notable feature of the `dalaran::Collection` is that its data may be either **owned** or **borrowed**:
    ...
    /// be expensive. Typically, there should be no need to copy dalaran collections, so this more
    /// than likely indicates a bug inside the Dalaran SDK.
    ...
    class Collection {
      public:
        ...
        ///
        /// Since `dalaran::Collection` does not provide write access, data is guaranteed to be unchanged by
        ...
        ///
        /// Since `dalaran::Collection` does not provide write access, data is guaranteed to be unchanged by
        /// any function or operation taking on a `dalaran::Collection`.
    ...
    // E.g. instead of `dalaran::Collection<uint8_t>::borrow(data, num_instances)`,
    // you can just write `dalaran::borrow(data, num_instances)`.
...
// Needs to know about `dalaran::Collection` which means that it needs to be included after `dalaran::Collection` is defined.
