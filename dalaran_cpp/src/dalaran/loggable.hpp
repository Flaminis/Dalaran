...
namespace dalaran {
    /// The Loggable trait is used by all built-in implementation of `dalaran::AsComponents`
    /// to serialize a collection for logging.
    ///
    /// It is implemented for all built-in `dalaran::component`s and `dalaran::datatype`s.
    template <typename T>
    struct Loggable {
        /// \private
    ...
    namespace detail {
        ...
        ///
        /// Uses SFINAE to ensure that a given type T *specializes* the `dalaran::Loggable` trait.
