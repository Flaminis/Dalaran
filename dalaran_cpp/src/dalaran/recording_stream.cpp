...
namespace dalaran {
    ...
    Result<std::string> RecordingStream::serve_grpc(
        ...
        std::stringstream ss;
        ss << "dalaran+http://" << bind_ip << ":" << port << "/proxy";
        return ss.str();
    }
    ...
    Error RecordingStream::spawn(const SpawnOptions& options) const {
        ...
        dl_error status = {};
        dl_recording_stream_spawn(_id, &dalaran_c_options, &status);
        return status;
    }
    ...
    Error RecordingStream::try_send_columns(
        std::string_view entity_path, dalaran::Collection<TimeColumn> time_columns,
        dalaran::Collection<ComponentColumn> component_columns
    ) const {
        if (!is_enabled()) {
            return Error::ok();
        }
    ...
    Error RecordingStream::try_send_recording_name(std::string_view name) const {
        ...
        log_static(
            ...
            dalaran::archetypes::RecordingInfo::update_fields().with_name(std::string(name))
        );
        return status;
    }
    ...
    Error RecordingStream::try_send_recording_start_time_nanos(int64_t nanos) const {
        ...
        log_static(
            this->PROPERTIES_ENTITY_PATH,
            dalaran::archetypes::RecordingInfo::update_fields().with_start_time(nanos)
        );
        return status;
    }
} // namespace dalaran
