...
namespace dalaran::components {
    ...
    struct TensorDataExt {
        ...
        TensorData(
            dalaran::Collection<uint64_t> shape,
            dalaran::datatypes::TensorBuffer buffer
        )
            : data(dalaran::datatypes::TensorData(std::move(shape), std::move(buffer))) {}
        ...
        explicit TensorData(Collection<uint64_t> shape, const TElement* data_)
            : data(dalaran::datatypes::TensorData(std::move(shape), data_)) {}
