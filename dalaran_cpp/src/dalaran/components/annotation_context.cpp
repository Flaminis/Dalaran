...
namespace dalaran {
    const std::shared_ptr<arrow::DataType>& Loggable<components::AnnotationContext>::arrow_datatype(
        ...
        static const auto datatype = arrow::list(arrow::field(
            "item",
            Loggable<dalaran::datatypes::ClassDescriptionMapElem>::arrow_datatype(),
            false
        ));
        return datatype;
    }
    ...
    Result<std::shared_ptr<arrow::Array>> Loggable<components::AnnotationContext>::to_arrow(
        ...
        if (instances && num_instances > 0) {
            DL_RETURN_NOT_OK(Loggable<components::AnnotationContext>::fill_arrow_array_builder(
    ...
    dalaran::Error Loggable<components::AnnotationContext>::fill_arrow_array_builder(
        ...
        for (size_t elem_idx = 0; elem_idx < num_elements; elem_idx += 1) {
            ...
            if (element.class_map.data()) {
                DL_RETURN_NOT_OK(
                    Loggable<dalaran::datatypes::ClassDescriptionMapElem>::fill_arrow_array_builder(
                        value_builder,
