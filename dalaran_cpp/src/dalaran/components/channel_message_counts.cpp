...
namespace dalaran {
    const std::shared_ptr<arrow::DataType>&
        ...
        static const auto datatype = arrow::list(arrow::field(
            "item",
            Loggable<dalaran::datatypes::ChannelCountPair>::arrow_datatype(),
            false
        ));
        return datatype;
    }
    ...
    Result<std::shared_ptr<arrow::Array>> Loggable<components::ChannelMessageCounts>::to_arrow(
        ...
        if (instances && num_instances > 0) {
            DL_RETURN_NOT_OK(Loggable<components::ChannelMessageCounts>::fill_arrow_array_builder(
    ...
    dalaran::Error Loggable<components::ChannelMessageCounts>::fill_arrow_array_builder(
        ...
        for (size_t elem_idx = 0; elem_idx < num_elements; elem_idx += 1) {
            ...
            if (element.counts.data()) {
                DL_RETURN_NOT_OK(
                    Loggable<dalaran::datatypes::ChannelCountPair>::fill_arrow_array_builder(
                        value_builder,
                        element.counts.data(),
                        element.counts.size()
                    )
