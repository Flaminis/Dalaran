...
namespace dalaran {
    ...
    struct AsComponents {
        ...
        static_assert(
            ...
            "It is implemented for all built-in archetypes as well as invidiual & collections of `dalaran::ComponentBatch`."
    ...
    struct AsComponents<ComponentBatch> {
        static Result<Collection<ComponentBatch>> as_batches(ComponentBatch components) {
            return dalaran::take_ownership(std::move(components));
        }
    };
    ...
    struct AsComponents<Collection<Result<ComponentBatch>>> {
        static Result<Collection<ComponentBatch>> as_batches(
            ...
            return dalaran::take_ownership(std::move(result));
        }
    };
    ...
    struct AsComponents<Result<ComponentBatch>> {
        static Result<Collection<ComponentBatch>> as_batches(Result<ComponentBatch> components) {
            DL_RETURN_NOT_OK(components.error);
            return dalaran::take_ownership(std::move(components.value));
        }
    };
