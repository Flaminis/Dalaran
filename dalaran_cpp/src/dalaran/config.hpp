...
#ifndef DALARAN_ENABLED
#define DALARAN_ENABLED 1
#endif

namespace dalaran {
    /// Configuration singleton that applies to the entire SDK.
    struct DalaranGlobalConfig {
        static DalaranGlobalConfig& instance();

        DalaranGlobalConfig(const DalaranGlobalConfig&) = delete;
        DalaranGlobalConfig& operator=(const DalaranGlobalConfig&) = delete;
    ...
    /// be overridden by the DALARAN environment variable.
    ///
    /// If DALARAN is set to `1`, `true`, or `yes`, then Dalaran is enabled. If DALARAN is
    /// set to `0`, `false`, or `no`, then Dalaran is disabled.
    inline void set_default_enabled(bool default_enabled) {
        DalaranGlobalConfig::instance().default_enabled.store(
            default_enabled,
            std::memory_order_seq_cst
        );
    }

    /// Check if Dalaran is enabled.
    inline bool is_default_enabled() {
        ...
        return DalaranGlobalConfig::instance().default_enabled.load(std::memory_order_seq_cst);
    }
} // namespace dalaran
