...
namespace dalaran {
    ...
    /// dalaran::Status. For simplicity, the wrapped type must be default constructible.
    template <typename T>
    class [[nodiscard]] Result {
      ...
      public:
        ...
        Result(dalaran::Error _error) : value(), error(std::move(_error)) {}
        ...
        /// Returns true if error is set to dalaran::ErrorCode::Ok, implying that a value is
        /// contained, false otherwise.
        bool is_ok() const {
            return error.is_ok();
        }

        /// Returns true if error is not set to dalaran::ErrorCode::Ok, implying that no value is
        /// contained, false otherwise.
        bool is_err() const {
            return error.is_err();
        }
