...
namespace dalaran {
    bool is_strict_mode() {
        ...
        char env[512] = {};
        size_t env_length = 0;
        if (getenv_s(&env_length, env, sizeof(env), "DALARAN_STRICT") != 0) {
            return false;
        }
#else
        const char* env = std::getenv("DALARAN_STRICT");
        if (env == nullptr) {
            return false;
        }
#endif
        ...
        if (v == "1" || v == "true" || v == "yes" || v == "on") {
            ...
            fprintf(
                stderr,
                "Expected env-var DALARAN_STRICT to be 0/1 true/false yes/no on/off, found '%s'\n",
                env
            );
            return false;
        }
    ...
    void Error::handle() const {
        if (is_ok()) {
            ...
            throw_on_failure();
#else
            fprintf(stderr, "Dalaran ERROR: %s\n", description.c_str());
            abort();
#endif
        } else {
            fprintf(stderr, "Dalaran ERROR: %s\n", description.c_str());
