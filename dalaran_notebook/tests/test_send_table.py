"""Tests for the high-level send_table orchestration in dalaran.notebook.Viewer."""
...
def mocked_viewer() -> tuple[MagicMock, MagicMock]:
    """Create a high-level dalaran.notebook.Viewer with all heavy deps mocked out.

    Yields the Viewer along with the mock for its low-level dalaran_notebook.Viewer
    ...
    with ExitStack() as stack:
        stack.enter_context(patch("dalaran.notebook._ErrorWidget"))
        MockViewer = stack.enter_context(patch("dalaran.notebook._Viewer"))
        stack.enter_context(patch("dalaran.notebook._HTML"))
        stack.enter_context(patch("dalaran.notebook._VBox"))
        stack.enter_context(patch("dalaran.notebook._flush_ui_events"))
        mock_bindings = stack.enter_context(patch("dalaran.notebook.bindings"))
...
def test_display_groups_loading_and_viewer_in_vbox(mocked_viewer: tuple[MagicMock, MagicMock]) -> None:
    ...
    with patch("dalaran.notebook._VBox") as mock_vbox, patch("IPython.display.display"):
        viewer.display()
