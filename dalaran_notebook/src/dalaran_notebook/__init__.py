...
try:
    __version__ = importlib.metadata.version("dalaran_notebook")
...
# We allow customization through the `DALARAN_NOTEBOOK_ASSET` environment variable.
# The default value is hosted at `app.dalaran.dev`.
...
#  One way to run a custom asset server is to run `python -m dalaran_notebook serve`.
...
def _inline_widget() -> str:
    """
    For `DALARAN_NOTEBOOK_ASSET=inline`, we need a single file to pass to `anywidget`.
    ...
    """
    print("Loading inline widget due to DALARAN_NOTEBOOK_ASSET=inline")
...
ASSET_IS_URL = False
ASSET_ENV = os.environ.get("DALARAN_NOTEBOOK_ASSET", None)
if ASSET_ENV is None:
    # if we're in the `dalaran` repository, default to `serve-local`
    # as we don't upload widgets for `+dev` versions anywhere
    if "DALARAN_DEV_ENVIRONMENT" in os.environ:
        ASSET_ENV = "serve-local"
    else:
        ASSET_ENV = f"https://app.dalaran.dev/version/{__version__}/notebook/widget.js"
...
if ASSET_ENV == ASSET_MAGIC_SERVE:  # localhost widget
    ...
    if not (ASSET_ENV.startswith(("http://", "https://"))):
        raise ValueError(f"DALARAN_NOTEBOOK_ASSET should be a URL starting with http or https. Found: {ASSET_ENV}")
    if not (ASSET_ENV.endswith("widget.js")):
        raise ValueError(f"DALARAN_NOTEBOOK_ASSET should be a URL pointing to a `widget.js` file. Found: {ASSET_ENV}")
    ASSET_IS_URL = True
        ...
        with jupyter_ui_poll.ui_events() as poll:
            while self._ready is False:
                if time.time() - start > timeout:
                    logging.warning(
                        f"Timed out waiting for viewer to load. Consult https://pypi.org/project/dalaran-notebook/{__version__}/ for details."
                    )
                    return
                poll(1)
                time.sleep(0.1)
