...
/* Specifies attributes defined with traitlets in ../dalaran_notebook/__init__.py */
...
const render: Render<WidgetModel> = ({ model, el }) => {
  el.classList.add("dalaran_notebook");
...
function error_boundary<Fn extends (...args: any[]) => any>(f: Fn): Fn {
  const wrapper = (...args: any[]) => {
    try {
      ...
      const el = document.querySelector(".dalaran_notebook");
      if (el) {
